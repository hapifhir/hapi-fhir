...
public class FhirTesterConfig {
     ...
     *
     * <p>Note the use of the ${serverBase} variable below. This will be replaced with the base URL
     * as reported by the server itself. Often for a simple Tomcat (or other container)
     * installation, this will end up being something like
     * "http://localhost:8080/hapi-fhir-jpaserver-example". If you are deploying your server to a
     * place with a fully qualified domain name, you might want to use that instead of using the
     * variable.
     */
    @Bean
    public TesterConfig testerConfig() {
        ...
        retVal.addServer()
                ...
                .withSearchResultRowOperation(
                        "$diff",
                        id -> id.isVersionIdPartValidLong() && id.getVersionIdPartAsLong() > 1)
                .withSearchResultRowOperation(
                        "$everything", id -> "Patient".equals(id.getResourceType()))
                .withSearchResultRowOperation(
                        "$summary", id -> "Patient".equals(id.getResourceType()))
                .addServer()
                .withId("home_r5")
