...
public class AnalyticsInterceptor extends InterceptorAdapter implements IHasScheduledJobs {
    private static final org.slf4j.Logger ourLog =
            org.slf4j.LoggerFactory.getLogger(AnalyticsInterceptor.class);
    ...
    public void incomingRequestPreHandled(
            RestOperationTypeEnum theOperation, RequestDetails theRequest) {
        ...
        synchronized (myEventBuffer) {
            if (myEventBuffer.size() > myCollectThreshold) {
                ourLog.warn(
                        "Not collecting analytics on request! Event buffer has {} items in it",
                        myEventBuffer.size());
            }
    ...
    public static class Job implements HapiJob {
        @Autowired private AnalyticsInterceptor myAnalyticsInterceptor;
