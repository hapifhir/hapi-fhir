...
public class InstanceReindexServiceImplR5Test extends BaseJpaR5Test {
    ...
    @Autowired private IInstanceReindexService mySvc;
    ...
    public void testDryRunMissing() {
        ...
        List<Parameters.ParametersParameterComponent> sections =
                outcome.getParameters("MissingIndexes");
        assertEquals(1, sections.size());

        List<String> indexInstances =
                sections.get(0).getPart().stream()
                        .map(
                                t ->
                                        t.getName()
                                                + " "
                                                + getPartValue("Action", t)
                                                + " "
                                                + getPartValue("Type", t)
                                                + " "
    ...
    public void testDryRunTypes_ComboNonUniqueSearchParam() {
        ...
        Parameters.ParametersParameterComponent index =
                findSingleIndex(
                        outcome, "Patient?family=SIMPSON%5C%7C&given=HOMER", "NonUniqueIndexes");
    ...
    public void testDryRunTypes_ComboUniqueSearchParam() {
        ...
        Parameters.ParametersParameterComponent index =
                findIndexes(outcome, "Patient?family=Simpson%5C%7C&given=Homer", 1, "UniqueIndexes")
                        .get(0);
        assertEquals("NO_CHANGE", getPartValue("Action", index));
    }
    ...
    public void testDryRunTypes_Number() {
        IIdType id =
                createResource(
                        "ResearchStudy",
                        withResourcePrimitiveAttribute("recruitment.targetNumber", "3"));
    ...
    public void testDryRunTypes_Quantity() {
        ...
        IIdType id =
                createObservation(
                        withQuantityAtPath(
                                "valueQuantity", 1.2, "http://unitsofmeasure.org", "kg"));
        ...
        Parameters.ParametersParameterComponent index =
                findSingleIndex(outcome, "value-quantity", "QuantityIndexes");
        assertEquals("NO_CHANGE", getPartValue("Action", index));
    ...
    public void testDryRunTypes_QuantityNormalized() {
        ...
        IIdType id =
                createObservation(
                        withQuantityAtPath(
                                "valueQuantity", 1.2, "http://unitsofmeasure.org", "mg"));
        ...
        runInTransaction(
                () -> {
                    assertEquals(
                            ...
                            myEntityManager
                                    .createNativeQuery(
                                            "update HFJ_RES_LINK set TARGET_RESOURCE_ID = null")
                                    .executeUpdate());
                    assertEquals(
                            2,
                            myEntityManager
                                    .createNativeQuery(
                                            "update HFJ_RES_LINK set TARGET_RESOURCE_URL ="
                                                    + " 'http://foo'")
                                    .executeUpdate());
                    assertEquals(
                            2,
                            myEntityManager
                                    .createNativeQuery(
                                            "update HFJ_RES_LINK set TARGET_RESOURCE_VERSION = 1")
                                    .executeUpdate());
                });
    ...
    public void testDryRunTypes_String() {
        IIdType id =
                createPatient(withIdentifier("http://identifiers", "123"), withFamily("Smith"));
        ...
        Parameters.ParametersParameterComponent index =
                findSingleIndex(outcome, "family", "StringIndexes");
        assertEquals("NO_CHANGE", getPartValue("Action", index));
    ...
    public void testDryRunTypes_String_SpecificParameter() {
        IIdType id =
                createPatient(
                        withIdentifier("http://identifiers", "123"),
                        withFamily("Simpson"),
                        withGiven("Homer"));

        Parameters outcome =
                (Parameters) mySvc.reindexDryRun(new SystemRequestDetails(), id, Set.of("family"));
        ourLog.info(
                "Output:{}",
                myFhirContext.newJsonParser().setPrettyPrint(true).encodeResourceToString(outcome));
    ...
    public void testDryRunTypes_Token() {
        ...
        Parameters.ParametersParameterComponent index =
                findSingleIndex(outcome, "identifier", "TokenIndexes");
        assertEquals("NO_CHANGE", getPartValue("Action", index));
    ...
    public void testDryRunTypes_Uri() {
        IIdType id =
                createResource("CodeSystem", withResourcePrimitiveAttribute("url", "http://foo"));
        ...
        Parameters.ParametersParameterComponent index =
                findSingleIndex(outcome, "system", "UriIndexes");
        assertEquals("NO_CHANGE", getPartValue("Action", index));
    ...
    public void testReindexInstance() {
        ...
        SearchParameterMap map =
                SearchParameterMap.newSynchronous("eyecolour", new StringParam("GOLD"));
        ...
        Parameters.ParametersParameterComponent index =
                findSingleIndex(outcome, "eyecolour", "StringIndexes");
        assertEquals("ADD", getPartValue("Action", index));
        assertEquals("String", getPartValue("Type", index));
    ...
    private static List<Parameters.ParametersParameterComponent> findIndexes(
        ...
        List<Parameters.ParametersParameterComponent> indexes =
                theResponse.getParameters(theSectionName);
        assertEquals(1, indexes.size());

        List<Parameters.ParametersParameterComponent> indexInstances =
                indexes.get(0).getPart().stream()
    ...
    private static String getPartValue(
            String thePartName, Parameters.ParametersParameterComponent theParent) {
                ...
                .orElseThrow(
                        () ->
                                new IllegalArgumentException(
                                        "Couldn't find part with name: " + thePartName));
    }
}
