 ...
 *
 * <ul>
 *   <li>The <b>Local Scheduler</b> handles tasks that need to execute locally. This typically means
 *       things that should happen on all nodes in a clustered environment.
 *   <li>The <b>Cluster Scheduler</b> handles tasks that are distributed and should be handled by
 *       only one node in the cluster (assuming a clustered server). If the server is not clustered,
 *       this scheduler acts the same way as the local scheduler.
 * </ul>
 */
