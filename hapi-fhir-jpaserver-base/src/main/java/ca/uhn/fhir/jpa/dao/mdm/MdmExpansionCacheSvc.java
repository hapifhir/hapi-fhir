 ...
 *
 * <p>This cache allows you to port state between reader/processor/writer. In this case, we are
 * maintaining a cache of Source Resource ID -> Golden Resource ID, so that we can annotate outgoing
 * resources with their golden owner if applicable.
 */
public class MdmExpansionCacheSvc {
	private static final Logger ourLog = getLogger(MdmExpansionCacheSvc.class);

	private final ConcurrentHashMap<String, String> mySourceToGoldenIdCache =
				new ConcurrentHashMap<>();

	/**
	* Lookup a given resource's golden resource ID in the cache. Note that if you pass this
	* function the resource ID of a golden resource, it will just return itself.
	*
	...
	* @param theAddCacheContentContent If true, will annotate the log message with the current
	*     cache contents.
	...
	*/
	public String buildLogMessage(String message, boolean theAddCacheContentContent) {
		...
		if (ourLog.isDebugEnabled() || theAddCacheContentContent) {
				...
				mySourceToGoldenIdCache.entrySet().stream()
						.forEach(
									entry ->
	...
	* Populate the cache
	*
	* @param theSourceResourceIdToGoldenResourceIdMap the source ID -> golden ID map to populate
	*     the cache with.
	*/
	...
	/**
	* Since this cache is used at @JobScope, we can skip a whole whack of expansions happening by
	* simply checking if one of our child steps has populated the cache yet. .
	*/
