...
public class MemberMatcherR4Helper {
    static final org.slf4j.Logger ourLog =
            org.slf4j.LoggerFactory.getLogger(MemberMatcherR4Helper.class);

    private static final String OUT_COVERAGE_IDENTIFIER_CODE_SYSTEM =
     ...
     * only
     */
    public Optional<Coverage> findMatchingCoverage(
            Coverage theCoverageToMatch, RequestDetails theRequestDetails) {
        // search by received old coverage id
        List<IBaseResource> foundCoverages =
                findCoverageByCoverageId(theCoverageToMatch, theRequestDetails);
    ...
    private List<IBaseResource> findCoverageByCoverageIdentifier(
        ...
        ca.uhn.fhir.rest.api.server.IBundleProvider retVal =
                myCoverageDao.search(paramMap, theRequestDetails);
    ...
    private List<IBaseResource> findCoverageByCoverageId(
        ...
        SearchParameterMap paramMap =
                new SearchParameterMap().add("_id", new StringParam(theCoverageToMatch.getId()));
        ca.uhn.fhir.rest.api.server.IBundleProvider retVal =
                myCoverageDao.search(paramMap, theRequestDetails);
    ...
    public void updateConsentForMemberMatch(
        ...
        // Trust RequestTenantPartitionInterceptor or PatientIdPartitionInterceptor to assign the
        // partition.
        myConsentDao.create(theConsent, theRequestDetails);
    }

    public Parameters buildSuccessReturnParameters(
            Patient theMemberPatient, Coverage theCoverage, Consent theConsent) {
        IBaseParameters parameters = ParametersUtil.newInstance(myFhirContext);
        ParametersUtil.addParameterToParameters(
                myFhirContext, parameters, PARAM_MEMBER_PATIENT, theMemberPatient);
        ParametersUtil.addParameterToParameters(
                myFhirContext, parameters, PARAM_NEW_COVERAGE, theCoverage);
        ParametersUtil.addParameterToParameters(
                myFhirContext, parameters, PARAM_CONSENT, theConsent);
        ParametersUtil.addParameterToParameters(
                myFhirContext,
                parameters,
                PARAM_MEMBER_IDENTIFIER,
                getIdentifier(theMemberPatient));
    ...
    public void addMemberIdentifierToMemberPatient(
            Patient theMemberPatient, Identifier theNewIdentifier) {
        Coding coding =
                new Coding()
    ...
    public Optional<Patient> getBeneficiaryPatient(
            Coverage theCoverage, RequestDetails theRequestDetails) {
        ...
        Patient beneficiary =
                myPatientDao.read(new IdDt(beneficiaryRef.getReference()), theRequestDetails);
    ...
    public boolean validPatientMember(
            Patient thePatientFromContract,
            Patient thePatientToMatch,
            RequestDetails theRequestDetails) {
                        ...
                        .add(
                                ...
                                new DateParam(
                                        thePatientToMatch
                                                .getBirthDateElement()
                                                .getValueAsString()));
        ca.uhn.fhir.rest.api.server.IBundleProvider bundle =
                myPatientDao.search(map, theRequestDetails);
    ...
    /**
     * The consent policy rules are <a
     * href="https://build.fhir.org/ig/HL7/davinci-ehrx/StructureDefinition-hrex-consent.html#notes">
     * described here.</a>
     */
