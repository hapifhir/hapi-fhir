 ...
 * to create an entitymanager for a HAPI FHIR JPA server. This class also injects a starter template
 * into the ES cluster.
 */
public class ElasticsearchHibernatePropertiesBuilder {
	...
	private SchemaManagementStrategyName myIndexSchemaManagementStrategy =
	...
	public void apply(Properties theProperties) {
		...
		theProperties.put(
					BackendSettings.backendKey(ElasticsearchBackendSettings.PROTOCOL), myProtocol);

		if (StringUtils.isNotBlank(myUsername)) {
				theProperties.put(
						BackendSettings.backendKey(ElasticsearchBackendSettings.USERNAME), myUsername);
		}
		if (StringUtils.isNotBlank(myPassword)) {
				theProperties.put(
						BackendSettings.backendKey(ElasticsearchBackendSettings.PASSWORD), myPassword);
		}
		theProperties.put(
		...
		// This tells hibernate search to use this custom file for creating index settings. We use
		// this to add a custom max_ngram_diff
		theProperties.put(
					BackendSettings.backendKey(
		...
		if (!StringUtils.isBlank(myAwsRegion)) {
				theProperties.put(
						BackendSettings.backendKey(ElasticsearchAwsBackendSettings.REGION),
						myAwsRegion);
				theProperties.put(
						BackendSettings.backendKey(ElasticsearchAwsBackendSettings.SIGNING_ENABLED),
						true);
				if (!StringUtils.isBlank(myUsername) && !StringUtils.isBlank(myPassword)) {
					theProperties.put(
								BackendSettings.backendKey(
										ElasticsearchAwsBackendSettings.CREDENTIALS_TYPE),
								ElasticsearchAwsCredentialsTypeNames.STATIC);
					theProperties.put(
								BackendSettings.backendKey(
										ElasticsearchAwsBackendSettings.CREDENTIALS_ACCESS_KEY_ID),
								myUsername);
					theProperties.put(
								BackendSettings.backendKey(
										ElasticsearchAwsBackendSettings.CREDENTIALS_SECRET_ACCESS_KEY),
								myPassword);
				} else {
	...
	public ElasticsearchHibernatePropertiesBuilder setRequiredIndexStatus(
				IndexStatus theRequiredIndexStatus) {
	...
	public ElasticsearchHibernatePropertiesBuilder setDebugIndexSyncStrategy(
				String theSyncStrategy) {
		myDebugSyncStrategy = theSyncStrategy;
		return this;
	}

	public ElasticsearchHibernatePropertiesBuilder setDebugPrettyPrintJsonLog(
	...
	/**
	* If this is set to `true`, the AWS region will be used to configure the AWS client.
	* Additionally, this will trigger HibernateSearch to attempt to use IAM Authentication. If the
	* username and password are set in addition to the region, then the username and password will
	* be used as the AWS_ACCESS_KEY_ID and AWS_SECRET_KEY_ID for a static credentials file for IAM.
	*
