...
public class ValueSetAutocompleteOptions {
    ...
    static final List<String> ourSupportedModifiers =
            Arrays.asList("", TokenParamModifier.TEXT.getBareModifier());
    ...
    public static ValueSetAutocompleteOptions validateAndParseOptions(
        ...
        if (haveId || haveIdentifier || haveValueSet) {
            throw new InvalidRequestException(
                    Msg.code(2020)
                            + "$expand with contexDirection='existing' is only supported at the"
                            + " type leve. It is not supported at instance level, with a url"
                            + " specified, or with a ValueSet .");
        }
        if (!theStorageSettings.isAdvancedHSearchIndexing()) {
            throw new InvalidRequestException(
                    Msg.code(2022)
                            + "$expand with contexDirection='existing' requires Extended Lucene"
                            + " Indexing.");
        }
        ...
        ValueSetAutocompleteOptions result =
                new ValueSetAutocompleteOptions(
                        theContext.getValue(), filter, IPrimitiveType.toValueOrNull(theCount));

        if (!ourSupportedModifiers.contains(defaultString(result.getSearchParamModifier()))) {
            throw new InvalidRequestException(
                    Msg.code(2069)
                            + "$expand with contexDirection='existing' only supports plain token"
                            + " search, or the :text modifier.  Received "
