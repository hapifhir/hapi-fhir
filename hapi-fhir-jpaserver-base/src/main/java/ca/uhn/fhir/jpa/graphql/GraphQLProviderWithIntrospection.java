...
public class GraphQLProviderWithIntrospection extends GraphQLProvider {
    ...
    private static final Logger ourLog =
            LoggerFactory.getLogger(GraphQLProviderWithIntrospection.class);
        ...
        if (theQueryBody.contains("__schema")) {
            ...
            if (theId != null) {
                throw new InvalidRequestException(
                        Msg.code(2035)
                                + "GraphQL introspection not supported at instance level. Please"
                                + " try at server- or instance- level.");
            }

            operations =
                    EnumSet.of(
                            GraphQLSchemaGenerator.FHIROperationType.READ,
                            GraphQLSchemaGenerator.FHIROperationType.SEARCH);
            ...
            return super.processGraphQlPostRequest(
                    theServletRequestDetails, theRequestDetails, theId, theQueryBody);
        }
    }
    ...
    private String generateSchema(
        ...
        for (InterfaceTypeDefinition next :
                typeDefinitionRegistry.getTypes(InterfaceTypeDefinition.class)) {
            TypeResolver resolver = new TypeResolverProxy();
            TypeRuntimeWiring wiring =
                    TypeRuntimeWiring.newTypeWiring(next.getName()).typeResolver(resolver).build();
