...
public class NumberPredicateBuilder extends BaseSearchParamPredicateBuilder {
    ...
    static Condition createPredicateNumeric(
        ...
        // Per discussions with Grahame Grieve and James Agnew on 11/13/19, modified logic for EQUAL
        // and NOT_EQUAL operators below so as to
        //   use exact value matching.  The "fuzz amount" matching is still used with the
        ...
        switch (operation) {
            case gt:
                num =
                        BinaryCondition.greaterThan(
                                theColumn, theIndexTable.generatePlaceholder(theValue));
                break;
            case ge:
                num =
            ...
            case ne:
                num =
                        BinaryCondition.notEqualTo(
                                theColumn, theIndexTable.generatePlaceholder(theValue));
                break;
            case ap:
                BigDecimal mul =
                        SearchFuzzUtil.calculateFuzzAmount(ParamPrefixEnum.APPROXIMATE, theValue);
                BigDecimal low = theValue.subtract(mul, MathContext.DECIMAL64);
                BigDecimal high = theValue.add(mul, MathContext.DECIMAL64);
                Condition lowPred =
                        BinaryCondition.greaterThanOrEq(
                                theColumn, theIndexTable.generatePlaceholder(low));
                Condition highPred =
                        BinaryCondition.lessThanOrEq(
                                theColumn, theIndexTable.generatePlaceholder(high));
