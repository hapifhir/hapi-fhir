...
public class CoordCalculator {
    ...
    static GeoPoint findTarget(
            double theLatitudeDegrees,
            double theLongitudeDegrees,
            double theBearingDegrees,
    ...
    /**
     * Find a box around my coordinates such that the closest distance to each edge is the provided
     * distance
     *
     * @return
     */
    public static GeoBoundingBox getBox(
            double theLatitudeDegrees, double theLongitudeDegrees, Double theDistanceKm) {
        double diagonalDistanceKm = theDistanceKm * Math.sqrt(2.0);

        GeoPoint topLeft =
                CoordCalculator.findTarget(
                        theLatitudeDegrees, theLongitudeDegrees, 315.0, diagonalDistanceKm);
        GeoPoint bottomRight =
                CoordCalculator.findTarget(
                        theLatitudeDegrees, theLongitudeDegrees, 135.0, diagonalDistanceKm);
