...
public class SearchQueryBuilder {
    ...
    /**
     * Add and return a predicate builder (or a root query if no root query exists yet) for
     * selecting on a Composite Unique search parameter
     */
    ...
    /**
     * Add and return a predicate builder (or a root query if no root query exists yet) for
     * selecting on a COORDS search parameter
     */
    public CoordsPredicateBuilder addCoordsPredicateBuilder(
    ...
    /**
     * Create, add and return a predicate builder (or a root query if no root query exists yet) for
     * selecting on a DATE search parameter
     */
    ...
    /**
     * Add and return a predicate builder for selecting a forced ID. This is only intended for use
     * with sorts so it can not be the root query.
     */
    public ForcedIdPredicateBuilder addForcedIdPredicateBuilder(
            @Nonnull DbColumn theSourceJoinColumn) {
    ...
    /**
     * Create, add and return a predicate builder (or a root query if no root query exists yet) for
     * selecting on a NUMBER search parameter
     */
    public NumberPredicateBuilder addNumberPredicateBuilder(
            @Nullable DbColumn theSourceJoinColumn) {
    ...
    /**
     * Add and return a predicate builder (or a root query if no root query exists yet) for
     * selecting on the Resource table
     */
    public ResourceTablePredicateBuilder addResourceTablePredicateBuilder(
            @Nullable DbColumn theSourceJoinColumn) {
    ...
    /**
     * Create, add and return a predicate builder (or a root query if no root query exists yet) for
     * selecting on a QUANTITY search parameter
     */
    public QuantityPredicateBuilder addQuantityPredicateBuilder(
            @Nullable DbColumn theSourceJoinColumn) {
    ...
    public QuantityNormalizedPredicateBuilder addQuantityNormalizedPredicateBuilder(
        ...
        QuantityNormalizedPredicateBuilder retVal =
                mySqlBuilderFactory.quantityNormalizedIndexTable(this);
        addTable(retVal, theSourceJoinColumn);
    ...
    /**
     * Add and return a predicate builder (or a root query if no root query exists yet) for
     * selecting on a <code>_source</code> search parameter
     */
    ...
    /**
     * Create, add and return a predicate builder (or a root query if no root query exists yet) for
     * selecting on a REFERENCE search parameter
     */
    ...
    /**
     * Add and return a predicate builder (or a root query if no root query exists yet) for
     * selecting on a resource link where the source and target are reversed. This is used for _has
     * queries.
     */
    public ResourceLinkPredicateBuilder addReferencePredicateBuilderReversed(
        ...
        ResourceLinkPredicateBuilder retVal =
                mySqlBuilderFactory.referenceIndexTable(theQueryStack, this, true);
        addTable(retVal, theSourceJoinColumn);
        return retVal;
    }
    ...
    /**
     * Create, add and return a predicate builder (or a root query if no root query exists yet) for
     * selecting on a STRING search parameter
     */
    public StringPredicateBuilder addStringPredicateBuilder(
            @Nullable DbColumn theSourceJoinColumn) {
    ...
    /**
     * Add and return a predicate builder (or a root query if no root query exists yet) for
     * selecting on a <code>_tag</code> search parameter
     */
    ...
    /**
     * Create, add and return a predicate builder (or a root query if no root query exists yet) for
     * selecting on a TOKEN search parameter
     */
    ...
    /**
     * Add and return a predicate builder (or a root query if no root query exists yet) for
     * selecting on a <code>:missing</code> search parameter
     */
    public SearchParamPresentPredicateBuilder addSearchParamPresentPredicateBuilder(
            @Nullable DbColumn theSourceJoinColumn) {
        SearchParamPresentPredicateBuilder retVal =
                mySqlBuilderFactory.searchParamPresentPredicateBuilder(this);
        addTable(retVal, theSourceJoinColumn);
        return retVal;
    }
    ...
    /**
     * Create, add and return a predicate builder (or a root query if no root query exists yet) for
     * selecting on a URI search parameter
     */
    ...
    /**
     * Add and return a predicate builder (or a root query if no root query exists yet) for an
     * arbitrary table
     */
            ...
            if (myFirstPredicateBuilder == null) {
                ...
                if (myCountQuery) {
                    mySelect.addCustomColumns(
                            FunctionCall.count()
                                    .setIsDistinct(true)
                                    .addColumnParams(root.getResourceIdColumn()));
                } else {
                ...
                if (!myNeedResourceTableRoot
                        || (thePredicateBuilder instanceof ResourceTablePredicateBuilder)) {
                    return;
                }
            }
    ...
    public void addJoin(
        ...
        Join join =
                new DbJoin(
                        ...
                        theFromTable,
                        theToTable,
                        new DbColumn[] {theFromColumn},
                        new DbColumn[] {theToColumn});
    ...
    public GeneratedSql generate(
            @Nullable Integer theOffset, @Nullable Integer theMaxResultsToFetch) {
        ...
        while (true) {
            ...
            String substitutionIndexString =
                    sql.substring(idx + myBindVariableSubstitutionBase.length(), endIdx);
        ...
        if (maxResultsToFetch != null || offset != null) {
            ...
            if (isSqlServer) {
                ...
                // a limit can be used, so we can't rely on the flags that the limithandler exposes
                // since
    ...
    /**
     * If at least one predicate builder already exists, return the last one added to the chain. If
     * none has been selected, create a builder on HFJ_RESOURCE, add it and return it.
     */
    ...
    /**
     * The SQL Builder library has one annoying limitation, which is that it does not use/understand
     * bind variables for its generated SQL. So we work around this by replacing our contents with a
     * string in the SQL consisting of <code>[random UUID]-[value index]</code> and then
     */
    ...
    public ComboCondition addPredicateLastUpdated(DateRangeParam theDateRange) {
        ResourceTablePredicateBuilder resourceTableRoot =
                getOrCreateResourceTablePredicateBuilder(false);
    ...
    public void excludeResourceIdsPredicate(Set<JpaPid> theExistingPidSetToExclude) {
        ...
        InCondition predicate =
                new InCondition(resourceIdColumn, generatePlaceholders(excludePids));
    ...
    public void addSortCoordsNear(
        ...
        ComboExpression absLatitudeMiddle =
                new ComboExpression(
                        ComboExpression.Op.SUBTRACT,
                        theCoordsBuilder.getColumnLatitude(),
                        latitudePlaceholder);
        ...
        ComboExpression absLongitudeMiddle =
                new ComboExpression(
                        ComboExpression.Op.SUBTRACT,
                        theCoordsBuilder.getColumnLongitude(),
                        longitudePlaceholder);
        absLongitude = absLongitude.addCustomParams(absLongitudeMiddle);

        ComboExpression sum =
                new ComboExpression(ComboExpression.Op.ADD, absLatitude, absLongitude);
        String ordering;
        if (theAscending) {
            ordering = "";
        } else {
    ...
    public void addSortString(
            DbColumn theColumnValueNormalized, boolean theAscending, boolean theUseAggregate) {
    ...
    public void addSortString(
        ...
        if ((dialectIsMySql || dialectIsMsSql)) {
            ...
            String sortColumnName =
                    theTheColumnValueNormalized.getTable().getAlias()
                            + "."
                            + theTheColumnValueNormalized.getName();
            ...
            if ((theTheAscending && theNullOrder == OrderObject.NullOrder.LAST)
              ...
              */
            sortColumnName =
                    formatColumnNameForAggregate(theTheAscending, theUseAggregate, sortColumnName);
    ...
    public void addSortNumeric(
        ...
        if ((dialectIsMySql || dialectIsMsSql)) {
            ...
            if ((theAscending && theNullOrder == OrderObject.NullOrder.LAST)
                ...
                // Negating the numeric column value and reversing the sort order will ensure that
                // the rows appear
            ...
            sortColumnName =
                    formatColumnNameForAggregate(theAscending, theUseAggregate, sortColumnName);
    ...
    private void addSort(
            ...
            OrderObject.NullOrder theNullOrder,
            boolean theUseAggregate) {
        OrderObject.Dir direction =
                theTheAscending ? OrderObject.Dir.ASCENDING : OrderObject.Dir.DESCENDING;
    ...
    /**
     * If set to true (default is false), force the generated SQL to start with the {@link
     * ca.uhn.fhir.jpa.model.entity.ResourceTable HFJ_RESOURCE} table at the root of the query.
     *
     * <p>This seems to perform better if there are multiple joins on the resource ID table.
     */
