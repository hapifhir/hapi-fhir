...
public class DeleteExpungeFileWriter implements ItemWriter<ParsedDeleteExpungeRecord> {

	private static final Logger ourLog = LoggerFactory.getLogger(DeleteExpungeFileWriter.class);
	@Value("#{stepExecutionContext['" + DeleteExpungeJobConfig.JOB_UUID_PARAMETER + "']}")
	private String myJobUuid;
	@Value("#{stepExecutionContext['" + DeleteExpungePartitioner.FILE_INDEX + "']}")
	private int myFileIndex;
	@Value("#{stepExecutionContext['" + DeleteExpungePartitioner.ROW_PROCESSING_MODE + "']}")
	...
	@Override
	public void write(List<? extends ParsedDeleteExpungeRecord> theItemLists) throws Exception {
		...
		for (ParsedDeleteExpungeRecord nextItem : theItemLists) {
