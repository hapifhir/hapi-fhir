...
@Subselect(
         ...
         */
        "SELECT CONCAT_WS(' ', vsc.PID, vscd.PID) AS PID,        vsc.PID                         AS"
            + " CONCEPT_PID,        vsc.VALUESET_PID                AS CONCEPT_VALUESET_PID,       "
            + " vsc.VALUESET_ORDER              AS CONCEPT_VALUESET_ORDER,        vsc.SYSTEM_URL   "
            + "               AS CONCEPT_SYSTEM_URL,        vsc.CODEVAL                     AS"
            + " CONCEPT_CODEVAL,        vsc.DISPLAY                     AS CONCEPT_DISPLAY,       "
            + " vsc.SYSTEM_VER                  AS SYSTEM_VER,        vsc.SOURCE_PID               "
            + "   AS SOURCE_PID,        vsc.SOURCE_DIRECT_PARENT_PIDS   AS"
            + " SOURCE_DIRECT_PARENT_PIDS,        vscd.PID                        AS"
            + " DESIGNATION_PID,        vscd.LANG                       AS DESIGNATION_LANG,       "
            + " vscd.USE_SYSTEM                 AS DESIGNATION_USE_SYSTEM,        vscd.USE_CODE    "
            + "               AS DESIGNATION_USE_CODE,        vscd.USE_DISPLAY                AS"
            + " DESIGNATION_USE_DISPLAY,        vscd.VAL                        AS DESIGNATION_VAL"
            + " FROM TRM_VALUESET_CONCEPT vsc LEFT OUTER JOIN TRM_VALUESET_C_DESIGNATION vscd ON"
            + " vsc.PID = vscd.VALUESET_CONCEPT_PID")
public class TermValueSetConceptView implements Serializable, ITermValueSetConceptView {
    ...
    @Column(
            name = "PID",
            length = 1000 /* length only needed to satisfy JpaEntityTest, it's not used*/)
