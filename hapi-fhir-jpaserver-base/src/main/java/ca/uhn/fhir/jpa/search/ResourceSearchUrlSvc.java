...
/**
 * This service ensures uniqueness of resources during create or create-on-update by storing the
 * resource searchUrl.
 */
@Transactional
@Service
public class ResourceSearchUrlSvc {
	...
	/**
	* Perform removal of entries older than {@code theCutoffDate} since the create operations are
	* done.
	*/
	...
	/**
	* Once a resource is updated or deleted, we can trust that future match checks will find the
	* committed resource in the db. The use of the constraint table is done, and we can delete it
	...
	*/
	...
	/**
	* We store a record of match urls with res_id so a db constraint can catch simultaneous creates
	* that slip through.
	*/
	public void enforceMatchUrlResourceUniqueness(
				String theResourceName, String theMatchUrl, JpaPid theResourcePersistentId) {
		String canonicalizedUrlForStorage =
					createCanonicalizedUrlForStorage(theResourceName, theMatchUrl);

		ResourceSearchUrlEntity searchUrlEntity =
					ResourceSearchUrlEntity.from(
								canonicalizedUrlForStorage, theResourcePersistentId.getId());
		// calling dao.save performs a merge operation which implies a trip to
		// the database to see if the resource exists.  Since we don't need the check, we avoid the
		// trip by calling em.persist.
	...
	private String createCanonicalizedUrlForStorage(String theResourceName, String theMatchUrl) {
		...
		SearchParameterMap matchUrlSearchParameterMap =
					myMatchUrlService.translateMatchUrl(theMatchUrl, resourceDef);

		String canonicalizedMatchUrl =
					matchUrlSearchParameterMap.toNormalizedQueryString(myFhirContext);
