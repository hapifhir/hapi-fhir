 ...
 */
public class DeleteExpungeStepListener implements StepExecutionListener {
	...
	public ExitStatus afterStep(StepExecution theStepExecution) {
		if (theStepExecution.getExitStatus().getExitCode().equals(ExitStatus.FAILED.getExitCode())) {
			...
			if (jobUuid == null) {
				jobUuid = theStepExecution.getJobExecution().getExecutionContext().getString(DeleteExpungeJobConfig.JOB_UUID_PARAMETER);
			}
