...
public class QueryStack {
    ...
    // used for _offset queries with sort, should be removed once the fix is applied to the async
    // path too.
    ...
    public void addSortOnCoordsNear(
            String theParamName, boolean theAscending, SearchParameterMap theParams) {
        boolean handled = false;
        if (myParamNameToPredicateBuilderMap != null) {
            BaseJoiningPredicateBuilder builder =
                    myParamNameToPredicateBuilderMap.get(theParamName);
            if (builder instanceof CoordsPredicateBuilder) {
                ...
                if (params.size() > 0 && params.get(0).size() > 0) {
                    ...
                    mySqlBuilder.addSortCoordsNear(
                            coordsBuilder, latitudeValue, longitudeValue, theAscending);
                    handled = true;
                }
            }
        }
    ...
    public void addSortOnDate(String theResourceName, String theParamName, boolean theAscending) {
        ...
        mySqlBuilder.addSortDate(
                datePredicateBuilder.getColumnValueLow(), theAscending, myUseAggregate);
    }
    ...
    public void addSortOnLastUpdated(boolean theAscending) {
        ...
        if (firstPredicateBuilder instanceof ResourceTablePredicateBuilder) {
            ...
            resourceTablePredicateBuilder =
                    mySqlBuilder.addResourceTablePredicateBuilder(
                            firstPredicateBuilder.getResourceIdColumn());
        }
        mySqlBuilder.addSortDate(
                resourceTablePredicateBuilder.getColumnLastUpdated(), theAscending, myUseAggregate);
    }
    ...
    public void addSortOnNumber(String theResourceName, String theParamName, boolean theAscending) {
        ...
        mySqlBuilder.addSortNumeric(
                numberPredicateBuilder.getColumnValue(), theAscending, myUseAggregate);
    }

    public void addSortOnQuantity(
            String theResourceName, String theParamName, boolean theAscending) {
        BaseJoiningPredicateBuilder firstPredicateBuilder =
                mySqlBuilder.getOrCreateFirstPredicateBuilder();

        BaseQuantityPredicateBuilder quantityPredicateBuilder =
                mySqlBuilder.createQuantityPredicateBuilder();
        ...
        mySqlBuilder.addSortNumeric(
                quantityPredicateBuilder.getColumnValue(), theAscending, myUseAggregate);
    }
    ...
    public void addSortOnResourceId(boolean theAscending) {
        ...
        ForcedIdPredicateBuilder sortPredicateBuilder =
                mySqlBuilder.addForcedIdPredicateBuilder(
                        firstPredicateBuilder.getResourceIdColumn());
        if (!theAscending) {
            mySqlBuilder.addSortString(
                    sortPredicateBuilder.getColumnForcedId(),
                    false,
                    OrderObject.NullOrder.FIRST,
                    myUseAggregate);
        } else {
            mySqlBuilder.addSortString(
                    sortPredicateBuilder.getColumnForcedId(), true, myUseAggregate);
        }
        mySqlBuilder.addSortNumeric(
                firstPredicateBuilder.getResourceIdColumn(), theAscending, myUseAggregate);
    }
    ...
    public void addSortOnResourceLink(
        ...
        ResourceLinkPredicateBuilder resourceLinkPredicateBuilder =
                mySqlBuilder.createReferencePredicateBuilder(this);
        ...
        if (theReferenceTargetType != null) {
            ...
            throw new InvalidRequestException(
                    Msg.code(2287)
                            + "Unable to sort on a chained parameter from '"
                            + theParamName
                            + "' as this parameter has multiple target types. Please specify the"
                            + " target type.");
        } else if (param.getTargets().size() == 1) {
        ...
        if (isBlank(targetType)) {
            throw new InvalidRequestException(
                    Msg.code(2288)
                            + "Unable to sort on a chained parameter from '"
                            + theParamName
                            + "' as this parameter as this parameter does not define a target type."
                            + " Please specify the target type.");
        }

        RuntimeSearchParam targetSearchParameter =
                mySearchParamRegistry.getActiveSearchParam(targetType, theChain);
        ...
        switch (targetSearchParameter.getParamType()) {
            case STRING:
                StringPredicateBuilder stringPredicateBuilder =
                        mySqlBuilder.createStringPredicateBuilder();
                sortColumn = new DbColumn[] {stringPredicateBuilder.getColumnValueNormalized()};
                chainedPredicateBuilder = stringPredicateBuilder;
                break;
            case TOKEN:
                TokenPredicateBuilder tokenPredicateBuilder =
                        mySqlBuilder.createTokenPredicateBuilder();
                sortColumn =
                        new DbColumn[] {
            ...
            case DATE:
                DatePredicateBuilder datePredicateBuilder =
                        mySqlBuilder.createDatePredicateBuilder();
            ...
            default:
                throw new InvalidRequestException(
                        Msg.code(2290)
                                + "Unable to sort on a chained parameter "
                                + theParamName
                                + "."
                                + theChain
                                + " as this parameter. Can not sort on chains of target type: "
                                + targetSearchParameter.getParamType().name());
        }

        addSortCustomJoin(
                resourceLinkPredicateBuilder.getColumnTargetResourceId(),
                chainedPredicateBuilder,
                null);
        Condition predicate =
                chainedPredicateBuilder.createHashIdentityPredicate(targetType, theChain);
    ...
    public void addSortOnToken(String theResourceName, String theParamName, boolean theAscending) {
        ...
        mySqlBuilder.addSortString(
                tokenPredicateBuilder.getColumnSystem(), theAscending, myUseAggregate);
        mySqlBuilder.addSortString(
                tokenPredicateBuilder.getColumnValue(), theAscending, myUseAggregate);
    }
    ...
    public void addSortOnUri(String theResourceName, String theParamName, boolean theAscending) {
        ...
        mySqlBuilder.addSortString(
                uriPredicateBuilder.getColumnValue(), theAscending, myUseAggregate);
    }
    ...
    private void addSortCustomJoin(
        ...
        ComboCondition onCondition =
                mySqlBuilder.createOnCondition(
                        theFromDbColumn, theToJoiningPredicateBuilder.getResourceIdColumn());
    ...
    private <T extends BaseJoiningPredicateBuilder>
            PredicateBuilderCacheLookupResult<T> createOrReusePredicateBuilder(
        ...
        if (myReusePredicateBuilderTypes.contains(theType)) {
            PredicateBuilderCacheKey key =
                    new PredicateBuilderCacheKey(theSourceJoinColumn, theType, theParamName);
            if (myJoinMap == null) {
    ...
    private Condition createPredicateComposite(
        ...
        for (IQueryParameterType next : theNextAnd) {
            ...
            if (!(next instanceof CompositeParam<?, ?>)) {
                throw new InvalidRequestException(
                        Msg.code(1203)
                                + "Invalid type for composite param (must be "
                                + CompositeParam.class.getSimpleName()
                                + ": "
                                + next.getClass());
            }
    ...
    private Condition createMissingParameterQuery(MissingParameterQueryParams theParams) {
        if (theParams.getParamType() == RestSearchParameterTypeEnum.COMPOSITE) {
            ...
            if (isEligibleForEmbeddedChainedResourceSearch(
                            ...
                            theParams.getQueryParameterTypes())
                    .supportsUplifted()) {
                ourLog.error(
                        "Cannot construct missing query parameter search for ContainedResource"
                                + " REFERENCE search.");
                return null;
            }
        }
        ...
        /**
         * How we search depends on if the {@link JpaStorageSettings#getIndexMissingFields()}
         * property is Enabled or Disabled.
         *
         * <p>If it is, we will use the SP_MISSING values set into the various SP_INDX_X tables and
         * search on those ("old" search).
         *
         * <p>If it is not set, however, we will try and construct a query that looks for missing
         * SearchParameters in the SP_IDX_* tables ("new" search).
         *
         * <p>You cannot mix and match, however (SP_MISSING is not in HASH_IDENTITY information). So
         * setting (or unsetting) the IndexMissingFields property should always be followed up with
         * a /$reindex call.
         *
         * <p>---
         *
         * <p>Current limitations: Checking if a row exists ("new" search) for a given missing field
         * in an SP_INDX_* table (ie, :missing=true) is slow when there are many resources in the
         * table. (Defaults to a table scan, since HASH_IDENTITY isn't part of the index).
         *
         * <p>However, the "old" search method was slow for the reverse: when looking for resources
    ...
    /** Old way of searching. Missing values must be indexed! */
    private Condition createMissingPredicateForIndexedMissingFields(
        ...
        switch (theParams.getParamType()) {
            case STRING:
                ...
                supplier =
                        () -> sqlBuilder.addStringPredicateBuilder(theParams.getSourceJoinColumn());
                break;
            case NUMBER:
                predicateType = PredicateBuilderTypeEnum.NUMBER;
                supplier =
                        () -> sqlBuilder.addNumberPredicateBuilder(theParams.getSourceJoinColumn());
                break;
            case DATE:
                predicateType = PredicateBuilderTypeEnum.DATE;
                supplier =
                        () -> sqlBuilder.addDatePredicateBuilder(theParams.getSourceJoinColumn());
                break;
            case TOKEN:
                predicateType = PredicateBuilderTypeEnum.TOKEN;
                supplier =
                        () -> sqlBuilder.addTokenPredicateBuilder(theParams.getSourceJoinColumn());
                break;
            case QUANTITY:
                predicateType = PredicateBuilderTypeEnum.QUANTITY;
                supplier =
                        () ->
                                sqlBuilder.addQuantityPredicateBuilder(
            ...
            case SPECIAL:
                ...
                supplier =
                        () -> sqlBuilder.addCoordsPredicateBuilder(theParams.getSourceJoinColumn());
                break;
            case COMPOSITE:
            default:
                break;
        ...
        if (supplier != null) {
            BaseSearchParamPredicateBuilder join =
            ...
            if (theParams.getParamType() == RestSearchParameterTypeEnum.REFERENCE) {
                SearchParamPresentPredicateBuilder join =
                        sqlBuilder.addSearchParamPresentPredicateBuilder(
                                theParams.getSourceJoinColumn());
                ...
                UriPredicateBuilder join =
                        sqlBuilder.addUriPredicateBuilder(theParams.getSourceJoinColumn());
                return join.createPredicateParamMissingForNonReference(
                        theParams.getResourceType(),
                        theParams.getParamName(),
                        theParams.isMissing(),
    ...
    /** New way of searching for missing fields. Missing values must not indexed! */
    private Condition createMissingPredicateForUnindexedMissingFields(
        ...
        ICanMakeMissingParamPredicate innerQuery =
                PredicateBuilderFactory.createPredicateBuilderForParamType(
    ...
    public Condition createPredicateCoords(
        ...
        if (isMissing != null) {
            ...
            CoordsPredicateBuilder predicateBuilder =
                    createOrReusePredicateBuilder(
            ...
            for (IQueryParameterType nextOr : theList) {
                Condition singleCode =
                        predicateBuilder.createPredicateCoords(
                                mySearchParameters,
                                nextOr,
                                theResourceName,
                                theSearchParam,
            ...
            return predicateBuilder.combineWithRequestPartitionIdPredicate(
                    theRequestPartitionId,
                    ComboCondition.or(codePredicates.toArray(new Condition[0])));
        }
    }
    ...
    public Condition createPredicateDate(
        ...
        if (isMissing != null) {
            ...
            for (IQueryParameterType nextOr : theList) {
                Condition p =
                        predicateBuilder.createPredicateDateWithoutIdentityPredicate(
                                nextOr, theOperation);
            ...
            if (!cacheHit) {
                predicate =
                        predicateBuilder.combineWithHashIdentityPredicate(
                                theResourceName, paramName, predicate);
                predicate =
                        predicateBuilder.combineWithRequestPartitionIdPredicate(
    ...
    private Condition createPredicateFilter(
        ...
        if (theFilter instanceof SearchFilterParser.FilterParameter) {
            ...
            if (((SearchFilterParser.FilterLogical) theFilter).getOperation()
                ...
                throw new InvalidRequestException(
                        Msg.code(1205)
                                + "Don't know how to handle operation "
        ...
        switch (paramName) {
            ...
            default:
                RuntimeSearchParam searchParam =
                        mySearchParamRegistry.getActiveSearchParam(theResourceName, paramName);
                if (searchParam == null) {
                ...
                if (typeEnum == RestSearchParameterTypeEnum.URI) {
                    ...
                    return theQueryStack3.createPredicateDate(
                            ...
                            Collections.singletonList(
                                    new DateParam(
                                            fromOperation(theFilter.getOperation()),
                                            theFilter.getValue())),
                            theFilter.getOperation(),
                            theRequestPartitionId);
                    ...
                    String resourceType =
                            null; // The value can either have (Patient/123) or not have (123) a
                    // resource type, either way it's not needed here
                    String chain =
                            (theFilter.getParamPath().getNext() != null)
                    ...
                    throw new InvalidRequestException(
                            Msg.code(1207)
                                    + "Composite search parameters not currently supported with"
                                    + " _filter clauses");
    ...
    private Condition createPredicateHas(
        ...
        for (List<? extends IQueryParameterType> nextOrList : theHasParameters) {
            ...
            for (IQueryParameterType nextParam : nextOrList) {
                ...
                parameters.add(
                        QualifiedParamList.singleton(
                                null, next.getValueAsQueryToken(myFhirContext)));
            }
            ...
            try {
                ...
                throw new InvalidRequestException(
                        Msg.code(1208) + "Invalid resource type: " + targetResourceType);
            }
            ...
            if (paramName.startsWith("_has:")) {
                ...
                for (IQueryParameterType next : nextOrList) {
                    ...
                    nextHasParam.setValueAsQueryToken(
                            ...
                            PARAM_HAS,
                            qualifier,
                            next.getValueAsQueryToken(myFhirContext));
                ...
                // Ensure that the name of the back-referenced search param on the target (e.g. the
                // `subject` in Patient?_has:Observation:subject:code=sys|val)
            ...
            Condition typePredicate =
                    BinaryCondition.equalTo(
                            join.getColumnTargetResourceType(),
                            mySqlBuilder.generatePlaceholder(theResourceType));
            Condition pathPredicate =
                    toEqualToOrInPredicate(
                            join.getColumnSourcePath(), mySqlBuilder.generatePlaceholders(paths));
            ...
            andPredicates.add(
                    toAndPredicate(
                            partitionPredicate, pathPredicate, typePredicate, linkedPredicate));
        }
    ...
    public Condition createPredicateNumber(
        ...
        if (isMissing != null) {
            ...
            NumberPredicateBuilder join =
                    createOrReusePredicateBuilder(
                                    PredicateBuilderTypeEnum.NUMBER,
                                    theSourceJoinColumn,
                                    paramName,
                                    () ->
                                            theSqlBuilder.addNumberPredicateBuilder(
                                                    theSourceJoinColumn))
                            .getResult();
            ...
            for (IQueryParameterType nextOr : theList) {
                ...
                if (nextOr instanceof NumberParam) {
                    ...
                    throw new IllegalArgumentException(
                            Msg.code(1211) + "Invalid token type: " + nextOr.getClass());
                }
            }
    ...
    public Condition createPredicateQuantity(
        ...
        if (isMissing != null) {
            ...
            boolean normalizedSearchEnabled =
                    myStorageSettings
            ...
            if (normalizedSearchEnabled) {
                List<QuantityParam> normalizedQuantityParams =
                        quantityParams.stream()
            ...
            if (join == null) {
                join =
                        createOrReusePredicateBuilder(
                                        ...
                                        paramName,
                                        () ->
                                                theSqlBuilder.addQuantityPredicateBuilder(
                                                        theSourceJoinColumn))
                                .getResult();
            }
    ...
    public Condition createPredicateReference(
        ...
        if ((theOperation != null)
            ...
            throw new InvalidRequestException(
                    Msg.code(1212)
                            + "Invalid operator specified for reference predicate.  Supported"
                            + " operators for reference predicate are \"eq\" and \"ne\".");
        }
        ...
        if (isMissing != null) {
            ...
            ResourceLinkPredicateBuilder predicateBuilder =
                    createOrReusePredicateBuilder(
                                    ...
                                    theParamName,
                                    () ->
                                            theSqlBuilder.addReferencePredicateBuilder(
                                                    this, theSourceJoinColumn))
                            .getResult();
    ...
    public Condition createPredicateReferenceForEmbeddedChainedSearchResource(
        ...
        boolean wantChainedAndNormal =
                theEmbeddedChainedSearchModeEnum
                        == EmbeddedChainedSearchModeEnum.UPLIFTED_AND_REF_JOIN;

        // A bit of a hack, but we need to turn off cache reuse while in this method so that we
        // don't try to reuse builders across different subselects
        ...
        for (List<ChainElement> nextChain : chains.keySet()) {
            ...
            collateChainedSearchOptions(
                    referenceLinks, nextChain, leafNodes, theEmbeddedChainedSearchModeEnum);
        }
        ...
        for (List<String> nextReferenceLink : referenceLinks.keySet()) {
            for (LeafNodeDefinition leafNodeDefinition : referenceLinks.get(nextReferenceLink)) {
                ...
                // Create a reference link predicates to the subselect for every link but the last
                // one
                for (String nextLink : nextReferenceLink) {
                    // We don't want to call createPredicateReference() here, because the whole
                    // point is to avoid the recursion.
                    // TODO: Are we missing any important business logic from that method? All tests
                    // are passing.
                    ResourceLinkPredicateBuilder resourceLinkPredicateBuilder =
                            builder.addReferencePredicateBuilder(this, previousJoinColumn);
                    builder.addPredicate(
                            resourceLinkPredicateBuilder.createPredicateSourcePaths(
                                    Lists.newArrayList(nextLink)));
                    previousJoinColumn = resourceLinkPredicateBuilder.getColumnTargetResourceId();
                }

                Condition containedCondition =
                        createIndexPredicate(
                                previousJoinColumn,
        ...
        if (wantChainedAndNormal) {
            ...
            if (theSourceJoinColumn == null) {
                retVal =
                        new InCondition(
                                mySqlBuilder
                                        .getOrCreateFirstPredicateBuilder(false)
                                        .getResourceIdColumn(),
                                union);
            } else {
    ...
    private void collateChainedSearchOptions(
        ...
        // This is a bit excruciating to extend beyond three references. Do we want to find a way to
        // automate this someday?
        // Note: the first element in each chain is assumed to be discrete. This may need to change
        // when we add proper support for `_contained`
        if (nextChain.size() == 1) {
            // discrete -> discrete
            if (theEmbeddedChainedSearchModeEnum
                    == EmbeddedChainedSearchModeEnum.UPLIFTED_AND_REF_JOIN) {
            ...
            updateMapOfReferenceLinks(
                    ...
                    leafNodes.stream()
                            ...
                            // When we're handling discrete->contained the differences between
                            // search
            ...
            updateMapOfReferenceLinks(
                    referenceLinks,
                    Lists.newArrayList(nextChain.get(0).getPath(), nextChain.get(1).getPath()),
                    leafNodes);
            ...
            updateMapOfReferenceLinks(
                    referenceLinks,
                    Lists.newArrayList(
                            mergePaths(nextChain.get(0).getPath(), nextChain.get(1).getPath())),
                    leafNodes);
            ...
            updateMapOfReferenceLinks(
                    ...
                    Lists.newArrayList(
                            nextChain.get(0).getPath(),
                            mergePaths(nextChain.get(1).getPath(), nextChain.get(2).getPath())),
                    leafNodes);
            ...
            // TODO: the chain is too long, it isn't practical to hard-code all the possible
            // patterns. If anyone ever needs this, we should revisit the approach
            throw new InvalidRequestException(
                    Msg.code(2011)
        ...
        switch (theParamDefinition.getParamType()) {
            ...
            case REFERENCE:
                containedCondition =
                        createPredicateReference(
                                theSourceJoinColumn,
                                theResourceName,
                                isBlank(theSpnamePrefix)
                                        ? theParamName
                                        : theSpnamePrefix + "." + theParamName,
                                theQualifiers,
                                theOrValues,
                                theOperation,
                                theRequest,
            ...
            default:
                throw new InvalidRequestException(
                        Msg.code(1215)
                                + "The search type:"
                                + theParamDefinition.getParamType()
                                + " is not supported.");
        }
    ...
    private Condition createPredicateSource(
        ...
        if (myStorageSettings.getStoreMetaSourceInformation()
            ...
            String msg =
                    myFhirContext
                            .getLocalizer()
                            .getMessage(QueryStack.class, "sourceParamDisabled");
        ...
        for (IQueryParameterType nextParameter : theList) {
            SourceParam sourceParameter =
                    new SourceParam(nextParameter.getValueAsQueryToken(myFhirContext));
    ...
    public Condition createPredicateString(
        ...
        return join.combineWithRequestPartitionIdPredicate(
                theRequestPartitionId, toOrPredicate(codePredicates));
    }
    ...
    public Condition createPredicateTag(
        ...
        if (Constants.PARAM_TAG.equals(theParamName)) {
            ...
            throw new IllegalArgumentException(
                    Msg.code(1217) + "Param name: " + theParamName); // shouldn't happen
        }
        ...
        for (List<? extends IQueryParameterType> nextAndParams : theList) {
            ...
            if (paramInverted) {
                ...
                sqlBuilder.addPredicate(
                        tagSelector.createPredicateTag(
                                tagType, tokens, theParamName, theRequestPartitionId));
                ...
                tagPredicate =
                        new InCondition(join.getResourceIdColumn(), subSelect).setNegate(true);

            } else {
                // Tag table can't be a query root because it will include deleted resources, and
                // can't select by resource type
                mySqlBuilder.getOrCreateFirstPredicateBuilder();

                TagPredicateBuilder tagJoin =
                        createOrReusePredicateBuilder(
                                        PredicateBuilderTypeEnum.TAG,
                                        theSourceJoinColumn,
                                        theParamName,
                                        () ->
                                                mySqlBuilder.addTagPredicateBuilder(
                                                        theSourceJoinColumn))
                                .getResult();
                tagPredicate =
                        tagJoin.createPredicateTag(
                                tagType, tokens, theParamName, theRequestPartitionId);
                join = tagJoin;
            }

            andPredicates.add(
                    join.combineWithRequestPartitionIdPredicate(
                            theRequestPartitionId, tagPredicate));
        }
    ...
    private boolean populateTokens(
            ...
            List<? extends IQueryParameterType> theAndParams) {
    ...
    private boolean checkHaveTags(
            List<? extends IQueryParameterType> theParams, String theParamName) {
            ...
            if (nextParamUncasted instanceof TokenParam) {
                ...
                if (isNotBlank(nextParam.getSystem())) {
                    throw new TokenParamFormatInvalidRequestException(
                            Msg.code(1218),
                            theParamName,
                            nextParam.getValueAsQueryToken(myFhirContext));
                }
            }
    ...
    public Condition createPredicateToken(
            ...
            if (nextOr instanceof TokenParam) {
                if (!((TokenParam) nextOr).isEmpty()) {
                    ...
                    if (modifier == TokenParamModifier.NOT) {
                        tokens.add(
                                new TokenParam(
                                        ((TokenParam) nextOr).getSystem(),
                                        ((TokenParam) nextOr).getValue()));
                        paramInverted = true;
                    } else {
                        tokens.add(nextOr);
                    }
        ...
        if (paramInverted) {
            ...
            TokenPredicateBuilder tokenJoin =
                    createOrReusePredicateBuilder(
                                    PredicateBuilderTypeEnum.TOKEN,
                                    theSourceJoinColumn,
                                    paramName,
                                    () ->
                                            theSqlBuilder.addTokenPredicateBuilder(
                                                    theSourceJoinColumn))
                            .getResult();

            predicate =
                    tokenJoin.createPredicateToken(
                            tokens,
                            theResourceName,
                            theSpnamePrefix,
                            theSearchParam,
    ...
    public Condition createPredicateUri(
        ...
        if (isMissing != null) {
            ...
            Condition predicate =
                    join.addPredicate(theList, paramName, theOperation, theRequestDetails);
            return join.combineWithRequestPartitionIdPredicate(theRequestPartitionId, predicate);
        }
    }
    ...
    public Condition searchForIdsWithAndOr(
        ...
        switch (theParamName) {
            ...
            case Constants.PARAM_SECURITY:
                if (myStorageSettings.getTagStorageMode()
                    ...
                    for (List<? extends IQueryParameterType> nextAnd : theAndOrParams) {
                        ...
                        EmbeddedChainedSearchModeEnum embeddedChainedSearchModeEnum =
                                isEligibleForEmbeddedChainedResourceSearch(
                                        theResourceName, theParamName, nextAnd);
                        if (embeddedChainedSearchModeEnum
                                == EmbeddedChainedSearchModeEnum.REF_JOIN_ONLY) {
                            andPredicates.add(
                                    createPredicateReference(
                                            theSourceJoinColumn,
                                            theResourceName,
    ...
    /**
     * This method handles the case of Search Parameters where the name/code in the SP is a full
     * chain expression. Normally to handle an expression like <code>Observation?subject.name=foo
     * </code> are handled by a SP with a type of REFERENCE where the name is "subject". That is not
     * handled here. On the other hand, if the SP has a name value containing the full chain (e.g.
     * "subject.name") we handle that here.
     *
     * @return Returns {@literal true} if the search parameter was handled by this method
     */
        ...
        if (!nextAnd.isEmpty() && nextAnd.get(0) instanceof ReferenceParam) {
            ...
            if (isNotBlank(param.getChain())) {
                ...
                if (fullChainParam != null) {
                    List<IQueryParameterType> swappedParamTypes =
                            nextAnd.stream()
                                    .map(
                                            t ->
                                                    toParameterType(
                                                            fullChainParam,
                                                            null,
                                                            t.getValueAsQueryToken(myFhirContext)))
                                    .collect(Collectors.toList());
                    List<List<IQueryParameterType>> params = List.of(swappedParamTypes);
                    Condition predicate =
                            createPredicateSearchParameter(
                                    theSourceJoinColumn,
                                    theResourceName,
                                    fullName,
                                    params,
    ...
    /**
     * When searching using a chained search expression (e.g. "Patient?organization.name=foo") we
     * have a few options:
     *
     * <ul>
     *   <li>A. If we want to match only {@link ca.uhn.fhir.jpa.model.entity.ResourceLink} for
     *       paramName="organization" with a join on {@link
     *       ca.uhn.fhir.jpa.model.entity.ResourceIndexedSearchParamString} with paramName="name",
     *       that's {@link EmbeddedChainedSearchModeEnum#REF_JOIN_ONLY} which is the standard
     *       searching case. Let's guess that 99.9% of all searches work this way.
     * </ul>
     *
     * <li>B. If we want to match only {@link
     *     ca.uhn.fhir.jpa.model.entity.ResourceIndexedSearchParamString} with
     *     paramName="organization.name", that's {@link
     *     EmbeddedChainedSearchModeEnum#UPLIFTED_ONLY}. We only do this if there is an uplifted
     *     refchain declared on the "organization" search parameter for the "name" search parameter,
     *     and contained indexing is disabled. This kind of index can come from indexing normal
     *     references where the search parameter has an uplifted refchain declared, and it can also
     *     come from indexing contained resources. For both of these cases, the actual index in the
     *     database is identical. But the important difference is that when you're searching for
     *     contained resources you also want to search for normal references. When you're searching
     *     for explicit refchains, no normal indexes matter because they'd be a duplicate of the
     *     uplifted refchain.
     * <li>C. We can also do both and return a union of the two, using {@link
     *     EmbeddedChainedSearchModeEnum#UPLIFTED_AND_REF_JOIN}. We do that if contained resource
     *     indexing is enabled since we have to assume there may be indexes on "organization" for
     *     both contained and non-contained Organization. resources.
     */
    private EmbeddedChainedSearchModeEnum isEligibleForEmbeddedChainedResourceSearch(
            String theResourceType,
            String theParameterName,
            List<? extends IQueryParameterType> theParameter) {
        ...
        boolean haveUpliftCandidates =
                theParameter.stream()
                        ...
                        // Chains on _has can't be indexed for contained searches - At least not
                        // yet. It's not clear to me if we ever want to support this, it would be
                        // really hard to do.
                                ...
                                t -> {
                                    ...
                                    RuntimeSearchParam param =
                                            mySearchParamRegistry.getActiveSearchParam(
                                                    theResourceType, theParameterName);
    ...
    public void addPredicateCompositeUnique(
            String theIndexString, RequestPartitionId theRequestPartitionId) {
        ComboUniqueSearchParameterPredicateBuilder predicateBuilder =
                mySqlBuilder.addComboUniquePredicateBuilder();
        Condition predicate =
                predicateBuilder.createPredicateIndexString(theRequestPartitionId, theIndexString);
        mySqlBuilder.addPredicate(predicate);
    }
    ...
    public void addPredicateCompositeNonUnique(
        ...
        Condition predicate =
                predicateBuilder.createPredicateHashComplete(theRequestPartitionId, theIndexString);
        mySqlBuilder.addPredicate(predicate);
    }
    ...
    public void addPredicateEverythingOperation(
            String theResourceName,
            List<String> theTypeSourceResourceNames,
            Long... theTargetPids) {
        ResourceLinkPredicateBuilder table = mySqlBuilder.addReferencePredicateBuilder(this, null);
        Condition predicate =
                table.createEverythingPredicate(
                        theResourceName, theTypeSourceResourceNames, theTargetPids);
        mySqlBuilder.addPredicate(predicate);
    }
    ...
    public IQueryParameterType toParameterType(
        ...
        qp.setValueAsQueryToken(
                myFhirContext, theParam.getName(), theQualifier, theValueAsQueryToken);
        return qp;
    }
                ...
                if (compositeOf.size() != 2) {
                    throw new InternalErrorException(
                            Msg.code(1224)
                                    ...
                                    + " has "
                                    + compositeOf.size()
                                    + " composite parts. Don't know how handlt this.");
                }
    ...
    /**
     * @see #isEligibleForEmbeddedChainedResourceSearch(String, String, List) for an explanation of
    ...
    private class ReferenceChainExtractor {
        ...
        private List<String> extractPaths(
                String theResourceType, RuntimeSearchParam theSearchParam) {
            List<String> pathsForType =
                    theSearchParam.getPathsSplit().stream()
                            .map(String::trim)
                        ...
                        if (!isReferenceParamValid(referenceParam)) {
                            throw new InvalidRequestException(
                                    Msg.code(2007)
                                            ...
                                            + theSearchParam.getName()
                                            + "."
                                            + referenceParam.getChain()
                                            + " is too long. Only chains up to three references are"
                                            + " supported.");
                        }

                        String targetChain = referenceParam.getChain();
                        List<String> qualifiers =
                                Lists.newArrayList(referenceParam.getResourceType());
        ...
        private void processNextLinkInChain(
            ...
            for (String nextTarget : thePreviousSearchParam.getTargets()) {
                ...
                if (isBlank(theResourceType) || theResourceType.equals(nextTarget)) {
                    nextSearchParam =
                            mySearchParamRegistry.getActiveSearchParam(nextTarget, nextParamName);
                }
                if (nextSearchParam != null) {
                    searchParamFound = true;
                    // If we find a search param on this resource type for this parameter name, keep
                    // iterating
                    ...
                    if (StringUtils.isEmpty(nextChain)) {
                        ...
                        if (RestSearchParameterTypeEnum.REFERENCE.equals(
                                nextSearchParam.getParamType())) {
                            orValues.add(new ReferenceParam(nextQualifier, "", theTargetValue));
                        } else {
                            IQueryParameterType qp = toParameterType(nextSearchParam);
                            qp.setValueAsQueryToken(
                                    myFhirContext, nextSearchParam.getName(), null, theTargetValue);
                            orValues.add(qp);
                        }
                        ...
                        for (String nextPath : nextPaths) {
                            ...
                            searchParamBranch.add(
                                    new ChainElement(
                                            nextTarget, nextSearchParam.getName(), nextPath));
                            processNextLinkInChain(
                                    searchParamBranch,
                                    nextSearchParam,
                                    nextChain,
    ...
    private static class LeafNodeDefinition {
        ...
        /**
         * Return a copy of this object with the given {@link RuntimeSearchParam} but all other
         * values unchanged.
         */
