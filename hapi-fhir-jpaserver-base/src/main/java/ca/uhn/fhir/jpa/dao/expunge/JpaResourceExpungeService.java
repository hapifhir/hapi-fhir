...
public class JpaResourceExpungeService implements IResourceExpungeService<JpaPid> {
    ...
    @Autowired private IResourceIndexedSearchParamUriDao myResourceIndexedSearchParamUriDao;
    @Autowired private IResourceIndexedSearchParamStringDao myResourceIndexedSearchParamStringDao;
    @Autowired private IResourceIndexedSearchParamTokenDao myResourceIndexedSearchParamTokenDao;
    @Autowired private IResourceIndexedSearchParamDateDao myResourceIndexedSearchParamDateDao;

    @Autowired
    private IResourceIndexedSearchParamQuantityDao myResourceIndexedSearchParamQuantityDao;

    @Autowired
    private IResourceIndexedSearchParamQuantityNormalizedDao
            myResourceIndexedSearchParamQuantityNormalizedDao;

    @Autowired private IResourceIndexedSearchParamCoordsDao myResourceIndexedSearchParamCoordsDao;
    @Autowired private IResourceIndexedSearchParamNumberDao myResourceIndexedSearchParamNumberDao;
    ...
    public List<JpaPid> findHistoricalVersionsOfNonDeletedResources(
        ...
        if (theJpaPid != null && theJpaPid.getId() != null) {
            if (theJpaPid.getVersion() != null) {
                ...
                ids =
                        myResourceHistoryTableDao.findIdsOfPreviousVersionsOfResourceId(
                                page, theJpaPid.getId());
            }
        } else {
            if (theResourceName != null) {
                ids =
                        myResourceHistoryTableDao.findIdsOfPreviousVersionsOfResources(
                                page, theResourceName);
            } else {
    ...
    public List<JpaPid> findHistoricalVersionsOfDeletedResources(
        ...
        if (theResourceId != null) {
            ids =
                    myResourceTableDao.findIdsOfDeletedResourcesOfType(
                            page, theResourceId.getId(), theResourceName);
            ourLog.info(
            ...
            if (theResourceName != null) {
                ...
                ourLog.info(
                        "Expunging {} deleted resources of type[{}]",
                        ids.getNumberOfElements(),
                        theResourceName);
            } else {
                ids = myResourceTableDao.findIdsOfDeletedResources(page);
                ourLog.info(
                        "Expunging {} deleted resources (all types)", ids.getNumberOfElements());
            }
        }
        return JpaPid.fromLongList(ids.getContent());
    }
    ...
    private void callHooks(
        ...
        if (CompositeInterceptorBroadcaster.hasHooks(
                Pointcut.STORAGE_PRESTORAGE_EXPUNGE_RESOURCE,
    ...
    public void expungeHistoricalVersionsOfIds(
        ...
        List<ResourceTable> resourcesToDelete =
                myResourceTableDao.findAllByIdAndLoadForcedIds(pids);
    ...
    private void expungeCurrentVersionOfResource(
        ...
        ResourceTable resource =
                myResourceTableDao.findById(theResourceId).orElseThrow(IllegalStateException::new);
    ...
    private void expungeHistoricalVersionsOfId(
        ...
        Slice<Long> versionIds =
                myResourceHistoryTableDao.findForResourceId(
                        page, theResource.getId(), theResource.getVersion());
        ourLog.debug(
