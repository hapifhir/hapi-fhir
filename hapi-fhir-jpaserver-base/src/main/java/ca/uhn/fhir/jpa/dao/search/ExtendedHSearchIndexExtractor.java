 ...
 *
 * <p>This class re-uses the extracted JPA entities to build an ExtendedHSearchIndexData instance.
 */
public class ExtendedHSearchIndexExtractor {
    ...
    public ExtendedHSearchIndexData extract(
        ...
        ExtendedHSearchIndexData retVal =
                new ExtendedHSearchIndexData(myContext, myJpaStorageSettings, theResource);

        if (myJpaStorageSettings.isStoreResourceInHSearchIndex()) {
            retVal.setRawResourceData(
                    myContext.newJsonParser().encodeResourceToString(theResource));
        }
        ...
        theNewParams.myStringParams.stream()
                ...
                .forEach(
                        nextParam ->
                                retVal.addStringIndexData(
                                        nextParam.getParamName(), nextParam.getValueExact()));

        theNewParams.myTokenParams.stream()
                .filter(nextParam -> !nextParam.isMissing())
                .forEach(
                        nextParam ->
                                retVal.addTokenIndexDataIfNotPresent(
                                        nextParam.getParamName(),
                                        nextParam.getSystem(),
                                        nextParam.getValue()));
                ...
                .forEach(
                        nextParam ->
                                retVal.addNumberIndexDataIfNotPresent(
                                        nextParam.getParamName(), nextParam.getValue()));
                ...
                .forEach(
                        nextParam ->
                                retVal.addDateIndexData(
                                        nextParam.getParamName(), convertDate(nextParam)));

        theNewParams.myQuantityParams.stream()
                .filter(nextParam -> !nextParam.isMissing())
                .forEach(
                        nextParam ->
                                retVal.addQuantityIndexData(
                                        nextParam.getParamName(), convertQuantity(nextParam)));

        theNewParams.myUriParams.stream()
                .filter(nextParam -> !nextParam.isMissing())
                .forEach(
                        nextParam ->
                                retVal.addUriIndexData(
                                        nextParam.getParamName(), nextParam.getUri()));
        ...
        theResource
                .getMeta()
                .getSecurity()
                .forEach(sec -> retVal.addTokenIndexData("_security", sec));

        theResource
                .getMeta()
                .getProfile()
                .forEach(prof -> retVal.addUriIndexData("_profile", prof.getValue()));
        ...
        theNewParams.myCompositeParams.forEach(
                nextParam ->
                        retVal.addCompositeIndexData(
                                nextParam.getSearchParamName(),
                                buildCompositeIndexData(nextParam)));
        ...
        if (!theNewParams.myLinks.isEmpty()) {
            ...
            for (ResourceLink nextLink : theNewParams.getResourceLinks()) {
                ...
                List<String> paramNames =
                        linkPathToParamName.getOrDefault(insensitivePath, Collections.emptyList());
    ...
    private void extractAutocompleteTokens(
            IBaseResource theResource, ExtendedHSearchIndexData theRetVal) {
                ...
                .forEach(
                        p ->
                                mySearchParamExtractor
                                        ...
                                        .forEach(
                                                nextValue ->
                                                        indexTokenValue(theRetVal, p, nextValue)));
    }

    private void indexTokenValue(
            ExtendedHSearchIndexData theRetVal, RuntimeSearchParam p, IBase nextValue) {
        ...
        switch (nextType) {
            ...
            case "Coding":
                ...
                // This is just for autocomplete, and we are focused on Observation.code, category,
                // combo-code, etc.
                //					case "Identifier":
                //						mySearchParamExtractor.addToken_Identifier(myResourceTypeName, params,
                // searchParam, value);
                //						break;
                //					case "ContactPoint":
                //						mySearchParamExtractor.addToken_ContactPoint(myResourceTypeName, params,
                // searchParam, value);
                //						break;
            default:
                break;
    ...
    private void addToken_CodeableConcept(
            ExtendedHSearchIndexData theRetVal, String theSpName, IBase theValue) {
    ...
    private void addToken_Coding(
            ExtendedHSearchIndexData theRetVal, String theSpName, IBaseCoding theNextValue) {
    ...
    @Nonnull
    public static QuantitySearchIndexData convertQuantity(
            ResourceIndexedSearchParamQuantity nextParam) {
        return new QuantitySearchIndexData(
                nextParam.getUnits(), nextParam.getSystem(), nextParam.getValue().doubleValue());
    }
}
