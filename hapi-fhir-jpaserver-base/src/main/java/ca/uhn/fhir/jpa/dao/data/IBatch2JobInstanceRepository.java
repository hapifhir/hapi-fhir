...
public interface IBatch2JobInstanceRepository
	...
	@Modifying
	@Query(
				"UPDATE Batch2JobInstanceEntity e SET e.myStatus = :status WHERE e.myId = :id and"
						+ " e.myStatus IN ( :prior_states )")
	...
	int updateInstanceUpdateTime(
				@Param("id") String theInstanceId, @Param("updated") Date theUpdated);

	@Modifying
	@Query("UPDATE Batch2JobInstanceEntity e SET e.myCancelled = :cancelled WHERE e.myId = :id")
	int updateInstanceCancelled(
				@Param("id") String theInstanceId, @Param("cancelled") boolean theCancelled);
	...
	@Query(
				"SELECT b from Batch2JobInstanceEntity b WHERE b.myDefinitionId = :defId AND"
						+ " b.myParamsJson = :params AND b.myStatus IN( :stats )")
	...
	List<Batch2JobInstanceEntity> findInstancesByJobStatus(
				@Param("status") StatusEnum theState, Pageable thePageable);
	...
	@Query(
				"SELECT b from Batch2JobInstanceEntity b WHERE b.myDefinitionId = :defId  AND"
						+ " b.myStatus IN( :stats ) AND b.myEndTime < :cutoff")
	...
	@Query(
				"SELECT e FROM Batch2JobInstanceEntity e WHERE e.myDefinitionId = :jobDefinitionId AND"
						+ " e.myStatus IN :statuses")
