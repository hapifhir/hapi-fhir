 ...
 * disable referential integrity for target resources at those paths.
 *
 * <p>For example, suppose this interceptor is configured with a path of <code>
 * AuditEvent.entity.what</code>, and an AuditEvent resource exists in the repository that has a
 * reference in that path to resource <code>Patient/123</code>. Normally this reference would
 * prevent the Patient resource from being deleted unless the AuditEvent was first deleted as well
 * (or a <a href="/hapi-fhir/docs/server_jpa/configuration.html#cascading-deletes">cascading
 * delete</a> was used). With this interceptor in place, the Patient resource could be deleted, and
 * the AuditEvent would remain intact.
 */
@Interceptor
