...
public class DatePredicateBuilder extends BaseSearchParamPredicateBuilder {
	...
	public Condition createPredicateDateWithoutIdentityPredicate(
		...
		if (theParam instanceof DateParam) {
				...
				throw new IllegalArgumentException(
						Msg.code(1251) + "Invalid token type: " + theParam.getClass());
		}
	...
	private Condition createPredicateDateFromRange(
		...
		/*
			...
			*/
		boolean isOrdinalComparison =
					isNullOrDatePrecision(lowerBound)
		...
		if (isOrdinalComparison) {
				...
				if (upperBound != null
						&& upperBound.getPrecision().ordinal()
									<= TemporalPrecisionEnum.MONTH.ordinal()) {
					genericUpperBound =
								Integer.parseInt(
										DateUtils.getCompletedDate(upperBound.getValueAsString())
													.getRight()
													.replace("-", ""));
				}
		} else {
		...
		if (theOperation == SearchFilterParser.CompareOperation.lt
				...
				if (lowerBoundInstant != null) {
					...
					throw new InvalidRequestException(
								Msg.code(1252)
										+ "lowerBound and upperBound value not correctly specified for"
										+ " comparing "
										+ theOperation);
				}
				...
				if (upperBoundInstant != null) {
					...
					throw new InvalidRequestException(
								Msg.code(1253)
										+ "upperBound and lowerBound value not correctly specified for"
										+ " compare theOperation");
				}
		} else if (theOperation == SearchFilterParser.CompareOperation.ne) {
				if ((lowerBoundInstant == null) || (upperBoundInstant == null)) {
					throw new InvalidRequestException(
								Msg.code(1254)
										+ "lowerBound and/or upperBound value not correctly specified for"
										+ " compare theOperation");
				}
				...
				throw new InvalidRequestException(
						Msg.code(1255)
									+ String.format(
												"Unsupported operator specified, operator=%s",
												theOperation.name()));
		}
		if (isOrdinalComparison) {
				ourLog.trace(
						"Ordinal date range is {} - {} ", lowerBoundAsOrdinal, upperBoundAsOrdinal);
		} else {
	...
	private boolean isNullOrDatePrecision(DateParam theDateParam) {
		return theDateParam == null
					|| theDateParam.getPrecision().ordinal() <= TemporalPrecisionEnum.DAY.ordinal();
	}

	private Condition createPredicate(
				ColumnEnum theColumn, ParamPrefixEnum theComparator, Object theValue) {
