...
public class SearchCoordinatorSvcImpl implements ISearchCoordinatorSvc<JpaPid> {
    ...
    private final ConcurrentHashMap<String, SearchTask> myIdToSearchTask =
     ...
     *
     * <p>This method must not be called from inside a transaction. The rationale is that the {@link
     * Search} entity is treated as a piece of shared state across client threads accessing the same
     * thread, so we need to be able to update that table in a transaction and commit it right away
     * in order for that to work. Examples of needing to do this include if two different clients
     * request the same search and are both paging at the same time, but also includes clients that
     * are hacking the paging links to fetch multiple pages of a search result in parallel. In both
     * cases we need to only let one of them actually activate the search, or we will have conficts.
     * The other thread just needs to wait until the first one actually fetches more results.
     */
    @Override
    public List<JpaPid> getResources(
        ...
        // If we're actively searching right now, don't try to do anything until at least one batch
        // has been
        // persisted in the DB
        ...
        while (true) {
            ...
            Callable<Search> searchCallback =
                    () ->
                            mySearchCacheSvc
                                    .fetchByUuid(theUuid, theRequestPartitionId)
                                    .orElseThrow(
                                            () ->
                                                    myExceptionSvc.newUnknownSearchException(
                                                            theUuid));
            search =
                    myTxService
            ...
            if (search.getStatus() == SearchStatusEnum.FINISHED) {
                ourLog.trace(
                        "Search entity marked as finished with {} results", search.getNumFound());
                break;
            }
            ...
            if (sw.getMillis() > myMaxMillisToWaitForRemoteResults) {
                ...
                throw new InternalErrorException(
                        Msg.code(1163) + "Request timed out after " + sw.getMillis() + "ms");
            }
            ...
            if (search.getStatus() == SearchStatusEnum.PASSCMPLET) {
                ...
                if (newSearch.isPresent()) {
                    ...
                    SearchParameterMap params =
                            search.getSearchParameterMap()
                                    .orElseThrow(
                                            () ->
                                                    new IllegalStateException(
                                                            "No map in PASSCOMPLET search"));
                    ...
                    SearchContinuationTask task =
                            (SearchContinuationTask)
                                    myBeanFactory.getBean(SearchConfig.CONTINUE_TASK, parameters);
        ...
        List<JpaPid> pids =
                fetchResultPids(
                        theUuid, theFrom, theTo, theRequestDetails, search, theRequestPartitionId);
    ...
    private List<JpaPid> fetchResultPids(
            ...
            RequestPartitionId theRequestPartitionId) {
        List<JpaPid> pids =
                mySearchResultCacheSvc.fetchResultPids(
    ...
    public IBundleProvider registerSearch(
        ...
        final ISearchBuilder<JpaPid> sb =
                mySearchBuilderFactory.newSearchBuilder(
                        theCallingDao, theResourceType, resourceTypeClass);
        ...
        //		SearchStrategyFactory.ISearchStrategy searchStrategy =
        // mySearchStrategyFactory.pickStrategy(theResourceType, theParams, theRequestDetails);
        //		return searchStrategy.get();

        if (theParams.isLoadSynchronous() || loadSynchronousUpTo != null || isOffsetQuery) {
            if (mySearchStrategyFactory.isSupportsHSearchDirect(
                    theResourceType, theParams, theRequestDetails)) {
                ourLog.info("Search {} is using direct load strategy", searchUuid);
                SearchStrategyFactory.ISearchStrategy direct =
                        mySearchStrategyFactory.makeDirectStrategy(
                ...
                try {
                    ...
                    // some resources were not found in index, so we will inform this and resort to
                    // JPA search
                    ourLog.warn(
    ...
    private void validateIncludes(Set<Include> includes, String name) {
        for (Include next : includes) {
            ...
            if (isNotBlank(paramTargetType)
                    && !myDaoRegistry.isResourceTypeSupported(paramTargetType)) {
                String resourceTypeMsg =
                        myContext
                                .getLocalizer()
                                .getMessageSanitized(
            ...
            if (!Constants.INCLUDE_STAR.equals(paramName)
                ...
                List<String> validNames =
                        mySearchParamRegistry.getActiveSearchParams(paramType).values().stream()
                                .filter(
                                        t ->
                                                t.getParamType()
    ...
    private PersistedJpaSearchFirstPageBundleProvider submitSearch(
        ...
        PersistedJpaSearchFirstPageBundleProvider retVal =
                myPersistedJpaBundleProviderFactory.newInstanceFirstPage(
        ...
        return myTxService
                ...
                .execute(
                        () -> {
                            ...
                            Search searchToUse =
                                    findSearchToUseOrNull(
                                            theQueryString, theResourceType, theRequestPartitionId);
                            if (searchToUse == null) {
                                return null;
                            }
                            ...
                            return myPersistedJpaBundleProviderFactory.newInstance(
                                    theRequestDetails, searchToUse.getUuid());
                        });
    }
    ...
    private Search findSearchToUseOrNull(
        ...
        final Instant createdCutoff =
                Instant.now()
                        .minus(
                                myStorageSettings.getReuseCachedSearchResultsForMillis(),
                                ChronoUnit.MILLIS);

        Optional<Search> candidate =
                mySearchCacheSvc.findCandidatesForReuse(
