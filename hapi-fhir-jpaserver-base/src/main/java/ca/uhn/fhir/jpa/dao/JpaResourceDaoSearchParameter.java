...
public class JpaResourceDaoSearchParameter<T extends IBaseResource>
		extends BaseHapiFhirResourceDao<T> implements IFhirResourceDaoSearchParameter<T> {
	...
	@Autowired private SearchParameterDaoValidator mySearchParameterDaoValidator;
	...
	protected void reindexAffectedResources(T theResource, RequestDetails theRequestDetails) {
		...
		List<String> base =
					theResource != null
								? searchParameter.getBase().stream()
	...
	@VisibleForTesting
	public void setSearchParameterDaoValidatorForUnitTest(
				SearchParameterDaoValidator theSearchParameterDaoValidator) {
