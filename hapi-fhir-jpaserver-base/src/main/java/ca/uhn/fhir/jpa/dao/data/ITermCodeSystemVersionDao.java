...
public interface ITermCodeSystemVersionDao
		extends JpaRepository<TermCodeSystemVersion, Long>, IHapiFhirJpaRepository {
	...
	@Query(
				"SELECT myId FROM TermCodeSystemVersion WHERE myCodeSystemPid = :codesystem_pid order"
						+ " by myId")
	List<Long> findSortedPidsByCodeSystemPid(@Param("codesystem_pid") Long theCodeSystemPid);

	@Query(
				"SELECT cs FROM TermCodeSystemVersion cs WHERE cs.myCodeSystemPid = :codesystem_pid AND"
						+ " cs.myCodeSystemVersionId = :codesystem_version_id")
	...
	@Query(
				"SELECT cs FROM TermCodeSystemVersion cs WHERE cs.myCodeSystemPid = :codesystem_pid AND"
						+ " cs.myCodeSystemVersionId IS NULL")
	TermCodeSystemVersion findByCodeSystemPidVersionIsNull(
				@Param("codesystem_pid") Long theCodeSystemPid);

	@Query("SELECT cs FROM TermCodeSystemVersion cs WHERE cs.myResourcePid = :resource_id")
	List<TermCodeSystemVersion> findByCodeSystemResourcePid(
				@Param("resource_id") Long theCodeSystemResourcePid);

	@Query(
				"SELECT cs FROM TermCodeSystemVersion cs WHERE"
						+ " cs.myCodeSystemHavingThisVersionAsCurrentVersionIfAny.myResource.myId ="
						+ " :resource_id")
