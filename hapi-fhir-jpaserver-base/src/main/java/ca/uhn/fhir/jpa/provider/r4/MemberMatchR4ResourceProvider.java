...
public class MemberMatchR4ResourceProvider {
    ...
    public MemberMatchR4ResourceProvider(
            FhirContext theFhirContext, MemberMatcherR4Helper theMemberMatcherR4Helper) {
        myFhirContext = theFhirContext;
        myMemberMatcherR4Helper = theMemberMatcherR4Helper;
    }

    /**
     * /Patient/$member-match operation Basic implementation matching by coverage id or by coverage
     * identifier. Matching by Beneficiary (Patient) demographics on family name and birthdate in
     * this version
     */
    @Operation(
    ...
    public Parameters patientMemberMatch(
            ...
            @Description(
                            shortDefinition =
                                    "The target of the operation. Will be returned with Identifier"
                                            + " for matched coverage added.")
                    @OperationParam(name = Constants.PARAM_MEMBER_PATIENT, min = 1, max = 1)
                    Patient theMemberPatient,
            @Description(
                            shortDefinition =
                                    "Old coverage information as extracted from beneficiary's"
                                            + " card.")
                    @OperationParam(name = Constants.PARAM_OLD_COVERAGE, min = 1, max = 1)
                    Coverage oldCoverage,
            @Description(
                            shortDefinition =
                                    "New Coverage information. Provided as a reference. Optionally"
                                            + " returned unmodified.")
                    @OperationParam(name = Constants.PARAM_NEW_COVERAGE, min = 1, max = 1)
                    Coverage newCoverage,
            @Description(
                            shortDefinition =
                                    "Consent information. Consent held by the system seeking the"
                                            + " match that grants permission to access the patient"
                                            + " information.")
                    @OperationParam(name = Constants.PARAM_CONSENT, min = 1, max = 1)
                    Consent theConsent,
            RequestDetails theRequestDetails) {
        return doMemberMatchOperation(
                theMemberPatient, oldCoverage, newCoverage, theConsent, theRequestDetails);
    }
    ...
    private Parameters doMemberMatchOperation(
        ...
        if (coverageOpt.isEmpty()) {
            String i18nMessage =
                    myFhirContext
                            .getLocalizer()
                            .getMessage("operation.member.match.error.coverage.not.found");
        ...
        Optional<Patient> patientOpt =
                myMemberMatcherR4Helper.getBeneficiaryPatient(coverage, theRequestDetails);
        if (patientOpt.isEmpty()) {
            String i18nMessage =
                    myFhirContext
                            .getLocalizer()
                            .getMessage("operation.member.match.error.beneficiary.not.found");
            throw new UnprocessableEntityException(Msg.code(1156) + i18nMessage);
        }

        Patient patient = patientOpt.get();
        if (!myMemberMatcherR4Helper.validPatientMember(
                patient, theMemberPatient, theRequestDetails)) {
            String i18nMessage =
                    myFhirContext
                            .getLocalizer()
                            .getMessage("operation.member.match.error.patient.not.found");
        ...
        myMemberMatcherR4Helper.addMemberIdentifierToMemberPatient(
                theMemberPatient, patient.getIdentifierFirstRep());
        myMemberMatcherR4Helper.updateConsentForMemberMatch(
                theConsent, patient, theMemberPatient, theRequestDetails);
        return myMemberMatcherR4Helper.buildSuccessReturnParameters(
                theMemberPatient, theCoverageToLink, theConsent);
    }

    private void validateParams(
            Patient theMemberPatient,
            Coverage theOldCoverage,
            Coverage theNewCoverage,
            Consent theConsent) {
    ...
    private void validateMemberPatientParam(Patient theMemberPatient) {
        ...
        validateParam(
                theMemberPatient.getName().get(0).getFamily(), Constants.PARAM_MEMBER_PATIENT_NAME);
