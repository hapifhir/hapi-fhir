...
/**
 * This interceptor uses semaphores to avoid multiple concurrent FHIR transaction bundles from
 * processing the same records at the same time, avoiding concurrency issues.
 */
@Interceptor
public class TransactionConcurrencySemaphoreInterceptor {

    private static final Logger ourLog =
            LoggerFactory.getLogger(TransactionConcurrencySemaphoreInterceptor.class);
                ...
                for (Map.Entry<String, Semaphore> nextEntry : pendingAndHeldSemaphores.entrySet()) {
                    ...
                    try {
                        if (nextSemaphore.tryAcquire(10, TimeUnit.SECONDS)) {
                            ourLog.trace(
                                    "Acquired semaphore {} on request URL: {}",
                                    nextSemaphore,
                                    nextEntry.getKey());
    ...
    private void acquireSemaphoresForUrlList(
        ...
        for (String nextUrl : urls) {
            ...
            if (isConditionalCreates) {
                if (myMemoryCacheService.getIfPresent(
                                MemoryCacheService.CacheEnum.MATCH_URL, nextUrl)
                        != null) {
                    continue;
                }
            }
    ...
    /**
     * Returns a count of all semaphores currently in the cache (incuding held and unheld
     * semaphores)
     */
