...
public class JpaStorageResourceParser implements IJpaStorageResourceParser {
	public static final LenientErrorHandler LENIENT_ERROR_HANDLER =
				new LenientErrorHandler(false).disableAllErrors();
	...
	@Override
	public IBaseResource toResource(
				IBasePersistedResource theEntity, boolean theForHistoryOperation) {
		RuntimeResourceDefinition type =
					myFhirContext.getResourceDefinition(theEntity.getResourceType());
		Class<? extends IBaseResource> resourceType = type.getImplementingClass();
		return toResource(
					resourceType, (IBaseResourceEntity) theEntity, null, theForHistoryOperation);
	}
	...
	public <R extends IBaseResource> R toResource(
		...
		String decodedResourceText =
					decodedResourceText(resourceBytes, resourceText, resourceEncoding);
		...
		retVal =
					populateResourceMetadata(
								theEntity, theForHistoryOperation, tagList, version, retVal);

		// 6. Handle source (provenance)
		MetaUtil.populateResourceSource(
					myFhirContext, provenanceSourceUri, provenanceRequestId, retVal);
	...
	private <R extends IBaseResource> void populateResourcePartitionInformation(
		...
		if (myPartitionSettings.isPartitioningEnabled()) {
				...
				if (partitionId != null && partitionId.getPartitionId() != null) {
					...
					retVal.setUserData(
								Constants.RESOURCE_PARTITION_ID, persistedPartition.toRequestPartitionId());
				} else {
	...
	private <R extends IBaseResource> R parseResource(
		...
		if (theResourceEncoding == ResourceEncodingEnum.ESR) {
				...
				Validate.notBlank(
						providerId, "No provider ID in ESR address: %s", theDecodedResourceText);
				...
				IParser parser =
						new TolerantJsonParser(
									getContext(theEntity.getFhirVersion()),
					...
					for (BaseTag nextTag : tagList) {
						if (nextTag.getTag().getTagType() == TagTypeEnum.PROFILE) {
								...
								if (isNotBlank(profile)) {
									Class<? extends IBaseResource> newType =
												myFhirContext.getDefaultTypeForProfile(profile);
									if (newType != null && theResourceType.isAssignableFrom(newType)) {
										ourLog.debug(
													"Using custom type {} for profile: {}",
													newType.getName(),
													profile);
	...
	public <R extends IBaseResource> R populateResourceMetadata(
		...
		if (theResourceTarget instanceof IResource) {
				...
				theResourceTarget =
						populateResourceMetadataRi(
									theEntitySource, tagList, theForHistoryOperation, res, theVersion);
		}
		...
		if (theEntity.getDeleted() != null) {
				...
				if (theForHistoryOperation) {
					ResourceMetadataKeyEnum.ENTRY_TRANSACTION_METHOD.put(
								res, BundleEntryTransactionMethodEnum.DELETE);
				}
				...
				if (published.equals(updated)) {
					ResourceMetadataKeyEnum.ENTRY_TRANSACTION_METHOD.put(
								res, BundleEntryTransactionMethodEnum.POST);
				} else {
					ResourceMetadataKeyEnum.ENTRY_TRANSACTION_METHOD.put(
								res, BundleEntryTransactionMethodEnum.PUT);
				}
		}
		...
		if (theTagList != null) {
				if (theEntity.isHasTags()) {
					...
					if (securityLabels.size() > 0) {
						ResourceMetadataKeyEnum.SECURITY_LABELS.put(
									res, toBaseCodingList(securityLabels));
					}
					if (profiles.size() > 0) {
				...
				for (BaseTag next : theTagList) {
					switch (next.getTag().getTagType()) {
						...
						case TAG:
								...
								// the tag is created with a null userSelected, but the api is primitive
								// boolean.
	...
	public void updateResourceMetadata(
		...
		if (theResourceTarget instanceof IResource) {
				ResourceMetadataKeyEnum.VERSION.put(
						(IResource) theResourceTarget, id.getVersionIdPart());
				ResourceMetadataKeyEnum.UPDATED.put(
						(IResource) theResourceTarget, theEntitySource.getUpdated());
		} else {
