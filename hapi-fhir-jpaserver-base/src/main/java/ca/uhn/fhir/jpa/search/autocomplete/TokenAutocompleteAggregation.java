...
class TokenAutocompleteAggregation {
    ...
    public TokenAutocompleteAggregation(
            String theSpName, int theCount, String theSearchText, String theSearchModifier) {
        Validate.notEmpty(theSpName);
        Validate.isTrue(theCount > 0, "count must be positive");
        Validate.isTrue(
                "text".equalsIgnoreCase(theSearchModifier)
                        || "".equals(theSearchModifier)
                        || theSearchModifier == null,
                "Unsupported search modifier " + theSearchModifier);
        mySpName = theSpName;
        myCount = theCount;
    ...
    private JsonObject makeSearch(String theSearchText, String theSearchModifier) {
        ...
        if (StringUtils.isEmpty(theSearchText)) {
            ...
            return RawElasticJsonBuilder.makeMatchBoolPrefixPredicate(
                    NESTED_SEARCH_PARAM_ROOT + "." + mySpName + ".string." + IDX_STRING_TEXT,
                    theSearchText);
        } else {
            return RawElasticJsonBuilder.makeWildcardPredicate(
                    NESTED_SEARCH_PARAM_ROOT + "." + mySpName + ".token.code", theSearchText + "*");
        }
    }
    ...
    JsonObject toJsonAggregation() {
        ...
        documentContext.set(
                "aggs.search.aggs.group_by_token.terms.field",
                NESTED_SEARCH_PARAM_ROOT + "." + mySpName + ".token" + ".code-system");
        documentContext.set("aggs.search.aggs.group_by_token.terms.size", myCount);
        documentContext.set(
                "aggs.search.aggs.group_by_token.aggs.top_tags_hits.top_hits._source.includes[0]",
                nestedSearchParamPath);
        return result;
    }
    ...
    List<TokenAutocompleteHit> extractResults(@Nonnull JsonObject theAggregationResult) {
        ...
        List<TokenAutocompleteHit> result =
                StreamSupport.stream(buckets.spliterator(), false)
    ...
    /**
     * Extract the result from the top-n aggregation bucket. The inner bucket contains matching hits
     */
    @Nonnull
    TokenAutocompleteHit bucketToEntry(JsonObject theBucketJson) {
        ...
        String displayText =
                documentContext.read(
                        "top_tags_hits.hits.hits[0]._source.string.text", String.class);
