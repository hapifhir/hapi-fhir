 ...
 *
 * <p>For example, if <code>DiagnosticReport/A</code> has a reference to <code>Observation/B</code>
 * it is not normally possible to delete <code>Observation/B</code> without first deleting <code>
 * DiagnosticReport/A</code>. With this interceptor in place, it is.
 *
 * <p>When using this interceptor, client requests must include the parameter <code>_cascade=delete
 * </code> on the DELETE URL in order to activate cascading delete, or include the request header
 * <code>X-Cascade-Delete: delete</code>
 */
@Interceptor
