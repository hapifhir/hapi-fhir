 ...
 * instance is created for any requested search and runs from the beginning to the end of the
 * search.
 *
 * <p>Understand: This class executes in its own thread separate from the web server client thread
 * that made the request. We do that so that we can return to the client as soon as possible, but
 * keep the search going in the background (and have the next page of results ready to go when the
 * client asks).
 */
public class SearchTask implements Callable<Void> {

    private static final org.slf4j.Logger ourLog =
            org.slf4j.LoggerFactory.getLogger(SearchTask.class);
    // injected beans
    ...
    public SearchTask(
        ...
        mySearchRuntimeDetails.setQueryString(
                myParams.toNormalizedQueryString(myCallingDao.getContext()));
    ...
    /**
     * This method is called by the server HTTP thread, and will block until at least one page of
     * results have been fetched from the DB, and will never block after that.
     */
    ...
    private ISearchBuilder newSearchBuilder() {
        ...
        return mySearchBuilderFactory.newSearchBuilder(
                myCallingDao, myResourceType, resourceTypeClass);
    }
        ...
        myTxService
                ...
                .execute(
                        () -> {
                            ...
                            // This can be used to remove results from the search result details
                            // before
                            // the user has a chance to know that they were in the results
                            if (mySearchRuntimeDetails.getRequestDetails() != null
                                    && unsyncedPids.isEmpty() == false) {
                                JpaPreResourceAccessDetails accessDetails =
                                        new JpaPreResourceAccessDetails(
                                                unsyncedPids, () -> newSearchBuilder());
                                HookParams params =
                                        new HookParams()
                                                .add(IPreResourceAccessDetails.class, accessDetails)
                                                .add(
                                                        RequestDetails.class,
                                                        mySearchRuntimeDetails.getRequestDetails())
                                                .addIfMatchesType(
                                ...
                                if (theResultIter.hasNext() == false) {
                                    ...
                                    int totalFetched =
                                            skippedCount
                                                    ...
                                                    + myCountBlockedThisPass;
                                    ourLog.trace(
                                            "MaxToFetch[{}] SkippedCount[{}] CountSavedThisPass[{}]"
                                                    + " CountSavedThisTotal[{}]"
                                                    + " AdditionalPrefetchRemaining[{}]",
                                            myMaxResultsToFetch,
                                            skippedCount,
                                            myCountSavedThisPass,
                                            myCountSavedTotal,
                                    ...
                                    if (nonSkippedCount == 0
                                            || (myMaxResultsToFetch != null
                                                    && totalFetched < myMaxResultsToFetch)) {
    ...
    /**
     * This is the method which actually performs the search. It is called automatically by the
     * thread pool.
     */
    @Override
    public Void call() {
        ...
        try {
            ...
            if (mySearch.getStatus() == SearchStatusEnum.FINISHED) {
                ...
                CompositeInterceptorBroadcaster.doCallHooks(
                        myInterceptorBroadcaster,
                        myRequest,
                        Pointcut.JPA_PERFTRACE_SEARCH_COMPLETE,
                        params);
            } else {
                HookParams params =
                        new HookParams()
                ...
                CompositeInterceptorBroadcaster.doCallHooks(
                        myInterceptorBroadcaster,
                        myRequest,
                        Pointcut.JPA_PERFTRACE_SEARCH_PASS_COMPLETE,
                        params);
            }
            ...
            CompositeInterceptorBroadcaster.doCallHooks(
                    myInterceptorBroadcaster,
                    myRequest,
                    Pointcut.JPA_PERFTRACE_SEARCH_FAILED,
                    params);
    ...
    /** This method actually creates the database query to perform the search, and starts it. */
    private void doSearch() {
        ...
        boolean myParamOrDefaultWantCount =
                nonNull(myParams.getSearchTotalMode())
                        ? isWantCount(myParams)
                        : SearchParameterMapCalculator.isWantCount(
                                myStorageSettings.getDefaultTotalMode());
        ...
        if (myParamWantOnlyCount || myParamOrDefaultWantCount) {
            ...
            Long count =
                    sb.createCountQuery(
                            myParams.clone(), mySearch.getUuid(), myRequest, myRequestPartitionId);
        ...
        if (myParams.getCount() != null) {
            minWanted =
                    myParams.getCount()
                            + 1; // Always fetch one past this page, so we know if there is a next
            // page.
            minWanted = Math.min(minWanted, myPagingProvider.getMaximumPageSize());
            minWanted += currentlyLoaded;
        }

        for (Iterator<Integer> iter = myStorageSettings.getSearchPreFetchThresholds().iterator();
                iter.hasNext(); ) {
            int next = iter.next();
