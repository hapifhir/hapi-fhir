...
public class PackageLoaderSvc extends BasePackageCacheManager {
    ...
    public NpmPackageData fetchPackageFromPackageSpec(PackageInstallationSpec theSpec)
            throws IOException {
    ...
    /**
     * Loads the package, but won't save it anywhere. Returns the data to the caller
     *
     * @return - a POJO containing information about the NpmPackage, as well as it's contents as
    ...
    private NpmPackageData fetchPackageFromServerInternal(
            String thePackageId, String thePackageVersion) throws IOException {
        BasePackageCacheManager.InputStreamWithSrc pkg =
                this.loadFromPackageServer(thePackageId, thePackageVersion);
     ...
     * @param thePackageTgzInputStream - the package contents. Typically fetched from a server, but
     *     can be added directly to the package spec
     * @return
     * @throws IOException
     */
    ...
    public NpmPackage loadPackageFromCacheOnly(String theS, @Nullable String theS1) {
        throw new UnsupportedOperationException(
                Msg.code(2215)
                        + "Cannot load from cache. Caching not supported in PackageLoaderSvc. Use"
                        + " JpaPackageCache instead.");
    }

    @Override
    public NpmPackage addPackageToCache(
            String theS, String theS1, InputStream theInputStream, String theS2)
            throws IOException {
        throw new UnsupportedOperationException(
                Msg.code(2216)
                        + "Cannot add to cache. Caching not supported in PackageLoaderSvc. Use"
                        + " JpaPackageCache instead.");
    }
    ...
    public byte[] loadPackageUrlContents(String thePackageUrl) {
        if (thePackageUrl.startsWith("classpath:")) {
            return ClasspathUtil.loadResourceAsByteArray(
                    thePackageUrl.substring("classpath:".length()));
            ...
            try (CloseableHttpResponse request =
