...
@Table(
        ...
        indexes = {
            ...
            @Index(
                    name = "FK_TRM_VALUESET_CONCEPT_PID",
                    columnList = "VALUESET_CONCEPT_PID",
                    unique = false)
        })
@Entity()
public class TermValueSetConceptDesignation implements Serializable {
    ...
    @Id()
    @SequenceGenerator(
            name = "SEQ_VALUESET_C_DSGNTN_PID",
            sequenceName = "SEQ_VALUESET_C_DSGNTN_PID")
    ...
    public TermValueSetConceptDesignation setLanguage(String theLanguage) {
        ValidateUtil.isNotTooLongOrThrowIllegalArgument(
                ...
                "Language exceeds maximum length ("
                        + TermConceptDesignation.MAX_LENGTH
                        + "): "
                        + length(theLanguage));
    ...
    public TermValueSetConceptDesignation setUseSystem(String theUseSystem) {
        ValidateUtil.isNotTooLongOrThrowIllegalArgument(
                theUseSystem,
                TermConceptDesignation.MAX_LENGTH,
                "Use system exceeds maximum length ("
                        + TermConceptDesignation.MAX_LENGTH
                        + "): "
                        + length(theUseSystem));
    ...
    public TermValueSetConceptDesignation setUseCode(String theUseCode) {
        ValidateUtil.isNotTooLongOrThrowIllegalArgument(
                theUseCode,
                TermConceptDesignation.MAX_LENGTH,
                "Use code exceeds maximum length ("
                        + TermConceptDesignation.MAX_LENGTH
                        + "): "
                        + length(theUseCode));
    ...
    public String toString() {
        return new ToStringBuilder(this, ToStringStyle.SHORT_PREFIX_STYLE)
                ...
                .append(
                        myConcept != null
                                ? ("myConcept - id=" + myConcept.getId())
                                : ("myConcept=(null)"))
                .append("myConceptPid", myConceptPid)
                .append(
                        myValueSet != null
                                ? ("myValueSet - id=" + myValueSet.getId())
                                : ("myValueSet=(null)"))
