...
public class LuceneSearchMappingFactory {
	...
	public SearchMapping getSearchMapping() {
		...
		mapping.analyzerDef("autocompleteEdgeAnalyzer", PatternTokenizerFactory.class)
			...
			// define analyzer for use on the search side - typically should use different analyzers for
			// indexing and search when it comes to ngrams.
			.analyzerDef("autocompleteEdgeSearchAnalyzer", PatternTokenizerFactory.class)
				.tokenizerParam("pattern", "(.*)")
				.tokenizerParam("group", "1")
				.filter(LowerCaseFilterFactory.class)
				.filter(StopFilterFactory.class)
			...
			// Indexing ngrams from the middle of a word does not have many use cases and
			// produces excessive irrelevant results. use edge ngram instead - see below.
