...
public class SearchFilterParser {
	...
	private static final List<String> CODES_CompareOperation =
			Arrays.asList("eq", "ne", "co", "sw", "ew", "gt", "lt", "ge", "le", "pr", "po", "ss", "sb", "in", "re");
		...
		if (cursor >= original.length()) {
			...
			if (((original.charAt(cursor) >= 'a') && (original.charAt(cursor) <= 'z'))
				...
				throw new FilterSyntaxException(
						Msg.code(1052) + String.format("Unknown Character \"%s\" at %d", peekCh(), cursor));
			}
		}
		return result;
	}
	...
	private String consumeString() throws FilterSyntaxException {
		...
		while ((cursor <= original.length()) && (original.charAt(cursor) != '"')) {
			...
			if (original.charAt(cursor) != '\\') {
				...
				if (original.charAt(cursor) == '"') {
					...
					throw new FilterSyntaxException(
							Msg.code(1053) + String.format("Unknown escape sequence at %d", cursor));
				}
			}
			cursor++;
		}
		...
		if ((cursor > original.length()) || (original.charAt(cursor) != '"')) {
			throw new FilterSyntaxException(
					Msg.code(1054) + String.format("Problem with string termination at %d", cursor));
		}

		if (str.length() == 0) {
			throw new FilterSyntaxException(
					Msg.code(1055) + String.format("Problem with string at %d cannot be empty", cursor));
		}
	...
	private BaseFilter parse() throws FilterSyntaxException {
		...
		if (cursor < original.length()) {
			throw new FilterSyntaxException(
					Msg.code(1056) + String.format("Expression did not terminate at %d", cursor));
		}
		return result;
	}
	...
	private BaseFilter parseOpen() throws FilterSyntaxException {
		...
		if (peek() == FilterLexType.fsltOpen) {
			...
			if (peek() != FilterLexType.fsltClose) {
				throw new FilterSyntaxException(
						Msg.code(1057) + String.format("Expected ')' at %d but found %s", cursor, peekCh()));
			}
			cursor++;
			FilterLexType lexType = peek();
			if (lexType == FilterLexType.fsltName) {
				result = parseLogical(grp);
			} else if ((lexType == FilterLexType.fsltEnded)
					|| (lexType == FilterLexType.fsltClose)
					|| (lexType == FilterLexType.fsltCloseSq)) {
				result = grp;
			} else {
				throw new FilterSyntaxException(
						Msg.code(1058) + String.format("Unexpected Character %s at %d", peekCh(), cursor));
			}
		} else {
			s = consumeName();
	...
	private FilterParameterPath parsePath(String name) throws FilterSyntaxException {
		...
		if (peek() == FilterLexType.fsltOpenSq) {
			...
			if (peek() != FilterLexType.fsltCloseSq) {
				throw new FilterSyntaxException(
						Msg.code(1060) + String.format("Expected ']' at %d but found %s", cursor, peekCh()));
			}
			cursor++;
		}
		...
		if (peek() == FilterLexType.fsltDot) {
			...
			if (peek() != FilterLexType.fsltName) {
				throw new FilterSyntaxException(
						Msg.code(1061) + String.format("Unexpected Character %s at %d", peekCh(), cursor));
			}
			result.setNext(parsePath(consumeName()));
		} else if (result.getFilter() != null) {
			throw new FilterSyntaxException(
					Msg.code(1062) + String.format("Expected '.' at %d but found %s", cursor, peekCh()));
		}
	...
	private BaseFilter parseParameter(String name) throws FilterSyntaxException {
		...
		if (peek() != FilterLexType.fsltName) {
			throw new FilterSyntaxException(
					Msg.code(1063) + String.format("Unexpected Character %s at %d", peekCh(), cursor));
		}
		s = consumeName();
		int index = CODES_CompareOperation.indexOf(s);
		if (index == -1) {
			throw new FilterSyntaxException(Msg.code(1064) + String.format("Unknown operation %s at %d", s, cursor));
		}
		...
		if (lexType == FilterLexType.fsltName) {
			...
			throw new FilterSyntaxException(
					Msg.code(1065) + String.format("Unexpected Character %s at %d", peekCh(), cursor));
		}
		...
		if (filter.getOperation() == CompareOperation.pr) {
			if ((filter.getValue().compareToIgnoreCase("true") != 0)
				...
				throw new FilterSyntaxException(Msg.code(1066)
						+ String.format(
								"Value %s not valid for operation %s at %d",
								filter.getValue(),
			...
			if (!isDate(filter.getValue())) {
				throw new FilterSyntaxException(Msg.code(1067)
						+ String.format(
								"Value %s not valid for operation %s at %d",
								filter.getValue(),
								CODES_CompareOperation.get(filter.getOperation().ordinal()),
		...
		if (lexType == FilterLexType.fsltName) {
			...
			throw new FilterSyntaxException(
					Msg.code(1068) + String.format("Unexpected Character %s at %d", peekCh(), cursor));
		}
		return result;
	}
	...
	public static BaseFilter parse(String expression) throws FilterSyntaxException {
