...
public class JpaResourceDaoValueSet<T extends IBaseResource> extends BaseHapiFhirResourceDao<T>
    ...
    @Override
    public T expand(
            IIdType theId, ValueSetExpansionOptions theOptions, RequestDetails theRequestDetails) {
    ...
    public T expandByIdentifier(String theUri, ValueSetExpansionOptions theOptions) {
        IValidationSupport.ValueSetExpansionOutcome expansionOutcome =
    ...
    public T expand(
        ...
        boolean haveContextDirection =
                theContextDirection != null && !theContextDirection.isEmpty();
        boolean haveContext = theContext != null && !theContext.isEmpty();

        boolean isAutocompleteExtension =
                haveContext
                        && haveContextDirection
                        && "existing".equals(theContextDirection.getValue());

        if (isAutocompleteExtension) {
            // this is a funky extension for NIH.  Do our own thing and return.
            ValueSetAutocompleteOptions options =
            ...
            if (myFulltextSearch == null || myFulltextSearch.isDisabled()) {
                throw new InvalidRequestException(
                        Msg.code(2083)
                                + " Autocomplete is not supported on this server, as the fulltext"
        ...
        if (!haveId && !haveIdentifier && !haveValueSet) {
            if (myFhirContext.getVersion().getVersion() == FhirVersionEnum.DSTU2) {
                ...
                throw new InvalidRequestException(
                        Msg.code(1130)
                                + "$expand operation at the type level (no ID specified) requires"
                                + " an identifier or a valueSet as a part of the request");
            }
            throw new InvalidRequestException(
                    Msg.code(1133)
            ...
            if (myFhirContext.getVersion().getVersion() == FhirVersionEnum.DSTU2) {
                ...
                throw new InvalidRequestException(
                        Msg.code(1131)
                                ...
                                + " identifier specified, or have a ValueSet specified. Can not"
        ...
        if (haveId) {
            ...
            outcome =
                    myValidationSupport.expandValueSet(
                            new ValidationSupportContext(myValidationSupport), options, url);
        } else {
            outcome =
                    myValidationSupport.expandValueSet(
    ...
    private T extractValueSetOrThrowException(IValidationSupport.ValueSetExpansionOutcome outcome) {
        if (outcome == null) {
            throw new InternalErrorException(
                    Msg.code(2028)
                            + "No validation support module was able to expand the given valueset");
        }
    ...
    public IValidationSupport.CodeValidationResult validateCode(
        ...
        Coding canonicalCodingToValidate =
                myVersionCanonicalizer.codingToCanonical((IBaseCoding) theCoding);
        boolean haveCoding =
                canonicalCodingToValidate != null && !canonicalCodingToValidate.isEmpty();
        ...
        if (!LogicUtil.multiXor(haveCodeableConcept, haveCoding, haveCode)) {
            throw new InvalidRequestException(
                    Msg.code(900)
                            + "$validate-code can only validate (system AND code) OR (coding) OR"
                            + " (codeableConcept)");
        }
        ...
        if (theValueSetId != null) {
            ...
            StringBuilder valueSetIdentifierBuilder =
                    new StringBuilder(
                            CommonCodeSystemsTerminologyService.getValueSetUrl(
                                    myFhirContext, valueSet));
        ...
        if (haveCodeableConcept) {
            ...
            for (int i = 0; i < codeableConcept.getCoding().size(); i++) {
                ...
                String system =
                        createVersionedSystemIfVersionIsPresent(
                                nextCoding.getSystem(), nextCoding.getVersion());
            ...
            String system =
                    createVersionedSystemIfVersionIsPresent(
                            canonicalCodingToValidate.getSystem(),
                            canonicalCodingToValidate.getVersion());
    ...
    public ResourceTable updateEntity(
        ...
        if (getStorageSettings().isPreExpandValueSets()
                && !retVal.isUnchangedInCurrentOperation()) {
