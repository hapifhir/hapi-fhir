...
public class ResourceReindexer {
	...
	public void reindexResourceEntity(ResourceTable theResourceTable) {
		...
		if (resource == null) {
				throw new InternalErrorException(
						Msg.code(1171)
									+ "Could not find resource version "
									+ theResourceTable.getIdDt().toUnqualified().getValue()
									+ " in database");
		}
		...
		if (actualVersion < expectedVersion) {
				...
				myResourceHistoryTableDao.updateVersion(
						theResourceTable.getId(), actualVersion, expectedVersion);
		}
	...
	<T extends IBaseResource> void doReindex(ResourceTable theResourceTable, T theResource) {
		RuntimeResourceDefinition resourceDefinition =
					myFhirContext.getResourceDefinition(theResource.getClass());
