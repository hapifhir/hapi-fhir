...
public class JpaPackageCache extends BasePackageCacheManager implements IHapiPackageCacheManager {
    ...
    private final Map<FhirVersionEnum, FhirContext> myVersionToContext =
            Collections.synchronizedMap(new HashMap<>());
    @PersistenceContext protected EntityManager myEntityManager;
    ...
    @Autowired private PackageLoaderSvc myPackageLoaderSvc;

    @Autowired(
            required = false) // It is possible that some implementers will not create such a bean.
    ...
    @Override
    protected BasePackageCacheManager.InputStreamWithSrc loadFromPackageServer(
            String id, String version) {
        throw new UnsupportedOperationException(
                Msg.code(2220) + "Use PackageLoaderSvc for loading packages.");
    }
    ...
    public NpmPackage loadPackageFromCacheOnly(String theId, @Nullable String theVersion) {
        Optional<NpmPackageVersionEntity> packageVersion =
                loadPackageVersionEntity(theId, theVersion);
        if (!packageVersion.isPresent() && theVersion.endsWith(".x")) {
            ...
            List<String> candidateVersionIds =
                    myPackageVersionDao.findVersionIdsByPackageIdAndLikeVersion(
                            theId, lookupVersion + ".%");
            if (candidateVersionIds.size() > 0) {
                candidateVersionIds.sort(PackageVersionComparator.INSTANCE);
                packageVersion =
                        loadPackageVersionEntity(
                                theId, candidateVersionIds.get(candidateVersionIds.size() - 1));
            }
        }
    ...
    private Optional<NpmPackageVersionEntity> loadPackageVersionEntity(
            String theId, @Nullable String theVersion) {
    ...
    private IHapiPackageCacheManager.PackageContents loadPackageContents(
            NpmPackageVersionEntity thePackageVersion) {
        IFhirResourceDao<? extends IBaseBinary> binaryDao = getBinaryDao();
        IBaseBinary binary =
                binaryDao.readByPid(JpaPid.fromId(thePackageVersion.getPackageBinary().getId()));
        try {
            byte[] content = fetchBlobFromBinary(binary);
            PackageContents retVal =
                    new PackageContents()
                            .setBytes(content)
            ...
            throw new InternalErrorException(
                    Msg.code(1295) + "Failed to load package. There was a problem reading binaries",
                    e);
        }
    }

    /**
     * Helper method which will attempt to use the IBinaryStorageSvc to resolve the binary blob if
     * available. If the bean is unavailable, fallback to assuming we are using an embedded base64
     * in the data element.
     *
    ...
    private byte[] fetchBlobFromBinary(IBaseBinary theBinary) throws IOException {
        if (myBinaryStorageSvc != null
                && !(myBinaryStorageSvc instanceof NullBinaryStorageSvcImpl)) {
            ...
            if (value == null) {
                throw new InternalErrorException(
                        Msg.code(1296)
                                + "Failed to fetch blob from Binary/"
                                + theBinary.getIdElement());
            }
            return value;
        }
    }
    ...
    private NpmPackage addPackageToCacheInternal(NpmPackageData thePackageData) {
        ...
        if (fhirVersion == null) {
            throw new InvalidRequestException(
                    Msg.code(1299) + "Unknown FHIR version: " + npmPackage.fhirVersion());
        }
        ...
        return newTxTemplate()
                .execute(
                        tx -> {
                            ...
                            NpmPackageEntity pkg =
                                    myPackageDao
                                            .findByPackageId(packageId)
                                            .orElseGet(() -> createPackage(npmPackage));
                            NpmPackageVersionEntity packageVersion =
                                    myPackageVersionDao
                            ...
                            if (packageVersion != null) {
                                NpmPackage existingPackage =
                                        loadPackageFromCacheOnly(
                                                ...
                                                packageVersion.getVersionId());
                                String msg =
                                        "Package version already exists in local storage, no action"
                                                + " taken: "
                                                + packageId
                                                + "#"
                                                + packageVersionId;
                            ...
                            boolean currentVersion =
                                    updateCurrentVersionFlagForAllPackagesBasedOnNewIncomingVersion(
                                            packageId, packageVersionId);
                            String packageDesc = null;
                            if (npmPackage.description() != null) {
                                if (npmPackage.description().length()
                                        > NpmPackageVersionEntity.PACKAGE_DESC_LENGTH) {
                                    packageDesc =
                                            npmPackage
                                                            .description()
                                                            .substring(
                            ...
                            NpmPackage.NpmPackageFolder packageFolder =
                                    npmPackage.getFolders().get(dirName);
                            for (Map.Entry<String, List<String>> nextTypeToFiles :
                                ...
                                for (String nextFile : nextTypeToFiles.getValue()) {
                                    ...
                                    if (nextFile.toLowerCase().endsWith(".xml")) {
                                        ...
                                        getProcessingMessages(npmPackage)
                                                .add("Not indexing file: " + nextFile);
                                        continue;
                                    }
                                    ...
                                    IBaseBinary resourceBinary =
                                            createPackageResourceBinary(
                                                    nextFile, minimizedContents, contentType);
                                    ResourceTable persistedResource =
                                            createResourceBinary(resourceBinary);

                                    NpmPackageVersionResourceEntity resourceEntity =
                                            new NpmPackageVersionResourceEntity();
                                    ...
                                    BaseRuntimeChildDefinition urlChild =
                                            packageContext
                                                    .getResourceDefinition(nextType)
                                                    .getChildByName("url");
                                    ...
                                    ourLog.info(
                                            "Package[{}#{}] " + msg, packageId, packageVersionId);
                                }
                            }
    ...
    public NpmPackage addPackageToCache(
            ...
            throws IOException {
        NpmPackageData npmData =
                myPackageLoaderSvc.createNpmPackageDataFromData(
    ...
    private boolean updateCurrentVersionFlagForAllPackagesBasedOnNewIncomingVersion(
        ...
        Collection<NpmPackageVersionEntity> existingVersions =
                myPackageVersionDao.findByPackageId(thePackageId);
        boolean retVal = true;

        for (NpmPackageVersionEntity next : existingVersions) {
            int cmp =
                    PackageVersionComparator.INSTANCE.compare(
                            next.getVersionId(), thePackageVersion);
            assert cmp != 0;
            if (cmp < 0) {
    ...
    private IBaseBinary createPackageResourceBinary(
            String theFileName, byte[] theBytes, String theContentType) {
        IBaseBinary binary = BinaryUtil.newBinary(myCtx);
    ...
    @Override
    @Transactional
    public NpmPackage loadPackage(String thePackageId, String thePackageVersion)
        ...
        NpmPackageData pkgData =
                myPackageLoaderSvc.fetchPackageFromPackageSpec(thePackageId, thePackageVersion);
    ...
    public NpmPackage installPackage(PackageInstallationSpec theInstallationSpec)
            throws IOException {
        ...
        if (isNotBlank(theInstallationSpec.getPackageUrl())) {
            byte[] contents =
                    myPackageLoaderSvc.loadPackageUrlContents(theInstallationSpec.getPackageUrl());
    ...
    @Override
    @Transactional
    public IBaseResource loadPackageAssetByUrl(
        ...
        if (versionSeparator != -1) {
            ...
            slice =
                    myPackageVersionResourceDao.findCurrentVersionByCanonicalUrlAndVersion(
    ...
    private IBaseResource loadPackageEntity(NpmPackageVersionResourceEntity contents) {
        try {
            ...
            throw new RuntimeException(
                    Msg.code(1305) + "Failed to load package resource " + contents, e);
        }
    }
    ...
    public NpmPackageMetadataJson loadPackageMetadata(String thePackageId) {
        ...
        if (!pkg.isPresent()) {
            throw new ResourceNotFoundException(
                    Msg.code(1306) + "Unknown package ID: " + thePackageId);
        }
        ...
        packageVersions.sort(
                new ReverseComparator<>(
                        (o1, o2) ->
                                PackageVersionComparator.INSTANCE.compare(
                                        o1.getVersionId(), o2.getVersionId())));

        for (NpmPackageVersionEntity next : packageVersions) {
            if (next.isCurrentVersion()) {
                retVal.setDistTags(
                        new NpmPackageMetadataJson.DistTags().setLatest(next.getVersionId()));
            }
    ...
    public PackageContents loadPackageContents(String thePackageId, String theVersion) {
        Optional<NpmPackageVersionEntity> entity =
                loadPackageVersionEntity(thePackageId, theVersion);
    ...
    public NpmPackageSearchResultJson search(PackageSearchSpec thePackageSearchSpec) {
        ...
        // Query for total
        {
            ...
            Root<NpmPackageVersionEntity> countCriteriaRoot =
                    countCriteriaQuery.from(NpmPackageVersionEntity.class);
            countCriteriaQuery.multiselect(cb.countDistinct(countCriteriaRoot.get("myPackageId")));

            List<Predicate> predicates =
                    createSearchPredicates(thePackageSearchSpec, cb, countCriteriaRoot);
        ...
        // Query for results
        {
            CriteriaQuery<NpmPackageVersionEntity> criteriaQuery =
                    cb.createQuery(NpmPackageVersionEntity.class);
            ...
            for (NpmPackageVersionEntity next : resultList) {
                ...
                if (!retVal.hasPackageWithId(next.getPackageId())) {
                    ...
                    NpmPackageSearchResultJson.Package retPackage =
                            retVal.getPackageWithId(next.getPackageId());
                    retPackage.addFhirVersion(next.getFhirVersionId());

                    int cmp =
                            PackageVersionComparator.INSTANCE.compare(
                                    next.getVersionId(), retPackage.getVersion());
                    if (cmp > 0) {
    ...
    public PackageDeleteOutcomeJson uninstallPackage(String thePackageId, String theVersion) {
        ...
        if (packageVersion.isPresent()) {
            ...
            Collection<NpmPackageVersionEntity> remainingVersions =
                    myPackageVersionDao.findByPackageId(thePackageId);
            if (remainingVersions.size() == 0) {
                ...
                List<NpmPackageVersionEntity> versions =
    ...
    @Override
    @Transactional
    public List<IBaseResource> loadPackageAssetsByType(
        ...
        Slice<NpmPackageVersionResourceEntity> outcome =
                myPackageVersionResourceDao.findCurrentVersionByResourceType(
    ...
    private void deleteAndExpungeResourceBinary(
            IIdType theResourceBinaryId, ExpungeOptions theOptions) {
        getBinaryDao().delete(theResourceBinaryId, new SystemRequestDetails()).getEntity();
        getBinaryDao()
                .forceExpungeInExistingTransaction(
                        theResourceBinaryId, theOptions, new SystemRequestDetails());
    }

    @Nonnull
    public List<Predicate> createSearchPredicates(
            PackageSearchSpec thePackageSearchSpec,
            CriteriaBuilder theCb,
            Root<NpmPackageVersionEntity> theRoot) {
        ...
        if (isNotBlank(thePackageSearchSpec.getResourceUrl())) {
            ...
            predicates.add(
                    theCb.equal(
                            resources.get("myCanonicalUrl").as(String.class),
                            thePackageSearchSpec.getResourceUrl()));
        }
        ...
        if (isNotBlank(thePackageSearchSpec.getDescription())) {
            ...
            predicates.add(
                    theCb.like(theRoot.get("myDescriptionUpper").as(String.class), searchTerm));
        }
        ...
        if (isNotBlank(thePackageSearchSpec.getFhirVersion())) {
            if (!thePackageSearchSpec.getFhirVersion().matches("([0-9]+\\.)+[0-9]+")) {
                FhirVersionEnum versionEnum =
                        FhirVersionEnum.forVersionString(thePackageSearchSpec.getFhirVersion());
                if (versionEnum != null) {
                    predicates.add(
                            theCb.equal(
                                    theRoot.get("myFhirVersion").as(FhirVersionEnum.class),
                                    versionEnum));
                }
            } else {
                predicates.add(
                        theCb.like(
                                theRoot.get("myFhirVersionId").as(String.class),
                                thePackageSearchSpec.getFhirVersion() + "%"));
            }
        }
    ...
    public static List<String> getProcessingMessages(NpmPackage thePackage) {
        return (List<String>)
                thePackage
                        .getUserData()
                        .computeIfAbsent(
                                "JpPackageCache_ProcessingMessages", t -> new ArrayList<>());
    }
}
