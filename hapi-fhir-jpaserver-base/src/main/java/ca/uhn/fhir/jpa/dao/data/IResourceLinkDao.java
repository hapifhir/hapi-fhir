...
public interface IResourceLinkDao extends JpaRepository<ResourceLink, Long>, IHapiFhirJpaRepository {
	...
	/**
	 * Deletes link rows where the given resource is the link <strong>target</strong>.
	 * Counterpart to {@link #deleteByResourceId(JpaPid)}, which deletes link rows where the
	 * resource is the link source. Both halves must be cleared before a row in
	 * {@code HFJ_RESOURCE} can be deleted without violating {@code FK_RESLINK_SOURCE} or
	 * {@code FK_RESLINK_TARGET}.
	 */
	// Generated by claude-opus-4-7
	@Modifying
	@Query("DELETE FROM ResourceLink t WHERE t.myTargetResource.myPid = :resId")
	void deleteByTargetResourceId(@Param("resId") JpaPid theResourcePid);
