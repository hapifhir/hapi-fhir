...
/**
 * This class instructs hibernate search on how to create index names for indexed entities. In our
 * case, we use this class to add an optional prefix to all indices which are created, which can be
 * controlled via {@link JpaStorageSettings#setHSearchIndexPrefix(String)}.
 */
@Service
public class IndexNamePrefixLayoutStrategy implements IndexLayoutStrategy {
	...
	public String extractUniqueKeyFromElasticsearchIndexName(String elasticsearchIndexName) {
		...
		if (!matcher.matches()) {
				throw log.invalidIndexPrimaryName(
	...
	private String removePrefixIfNecessary(String theCandidateUniqueKey) {
		validateStorageSettingsIsPresent();
		if (!StringUtils.isBlank(myStorageSettings.getHSearchIndexPrefix())) {
				return theCandidateUniqueKey.replace(
						myStorageSettings.getHSearchIndexPrefix() + "-", "");
		} else {
	...
	private void validateStorageSettingsIsPresent() {
		if (myStorageSettings == null) {
				throw new ConfigurationException(
						Msg.code(1168)
									+ "While attempting to boot HAPI FHIR, the Hibernate Search"
									+ " bootstrapper failed to find the StorageSettings. This probably"
									+ " means Hibernate Search has been recently upgraded, or somebody"
									+ " modified HapiFhirLocalContainerEntityManagerFactoryBean.");
		}
	}
}
