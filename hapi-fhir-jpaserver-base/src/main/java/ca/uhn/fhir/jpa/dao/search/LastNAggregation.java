...
public class LastNAggregation {
	...
	private static final String SP_CODE_TOKEN_CODE_AND_SYSTEM =
				SEARCH_PARAM_ROOT + ".code.token.code-system";
	...
	*
	* <p>https://www.elastic.co/guide/en/elasticsearch/reference/current/search-aggregations.html
	*/
	public JsonObject toAggregation() {
		JsonObject lastNAggregation =
					myJsonParser.fromJson(
								"{"
										+ "   \"terms\":{"
										+ "      \"field\":\""
										+ SP_CODE_TOKEN_CODE_AND_SYSTEM
										+ "\","
										+ "      \"size\":10000,"
										+ "      \"min_doc_count\":1"
										+ "   },"
				...
				lastNAggregation =
						myJsonParser.fromJson(
									"{"
												...
												+ "  \"aggs\": {"
												+ "    \""
												+ GROUP_BY_CODE_SYSTEM_SUB_AGGREGATION
												+ "\": "
												+ myJsonParser.toJson(lastNAggregation)
												+ ""
												+ "  }"
												+ "}",
									JsonObject.class);
	...
	/**
	* Parses the JSONObject aggregation result from ES to extract observation resource ids E.g
	* aggregation result payload
	*
	* <pre>{@code
	* {
	...
	*/
	public List<Long> extractResourceIds(@Nonnull JsonObject theAggregationResult) {
		ourLog.trace(
					"extractResourceIds - hasSubject {} aggregation {}",
					myAggregateOnSubject,
		...
		if (myAggregateOnSubject) {
				resultBuckets =
						StreamSupport.stream(
												theAggregationResult.getAsJsonArray("buckets").spliterator(),
												false)
									.map(
												bucket ->
														bucket.getAsJsonObject()
																	.getAsJsonObject(
																				GROUP_BY_CODE_SYSTEM_SUB_AGGREGATION));
		}
		...
		return resultBuckets
					.flatMap(
								grouping ->
										StreamSupport.stream(
													grouping.getAsJsonArray("buckets").spliterator(), false))
					.flatMap(
								bucket -> {
									JsonArray hits =
												bucket.getAsJsonObject()
