 ...
 *
 * <p>These properties are added in a specific handler which is involved after all TermConcepts are
 * created, because they require a 'display' value associated to other TermConcept (pointed by the
 * 'code' property value), which require that concept to have been created.
 */
public class LoincCodingPropertiesHandler implements IZipContentsHandlerCsv {
    private static final Logger ourLog =
            LoggerFactory.getLogger(LoincCodingPropertiesHandler.class);
    ...
    public void accept(CSVRecord theRecord) {
        ...
        if (isNotBlank(associatedObservationsValue)) {
            addCodingProperties(
                    srcTermConcept, ASSOCIATED_OBSERVATIONS_PROP_NAME, associatedObservationsValue);
        }
    }

    /**
     * Validates that at least one ot target properties is defined in loinc.xml file and is of type
     * "CODING"
     */
    private boolean anyValidProperty() {
        CodeSystem.PropertyType askAtOrderEntryPropType =
                myPropertyNameTypeMap.get(ASK_AT_ORDER_ENTRY_PROP_NAME);
    ...
    private void addCodingProperties(
            TermConcept theSrcTermConcept, String thePropertyName, String thePropertyValue) {
        ...
        for (String propertyCodeValue : propertyCodeValues) {
            ...
            if (targetTermConcept == null) {
                ourLog.error(
                        "Couldn't find TermConcept for code: '{}'. Display property set to blank"
                                + " for property: '{}'",
                        propertyCodeValue,
                        thePropertyName);
                continue;
            ...
            theSrcTermConcept.addPropertyCoding(
                    ...
                    propertyCodeValue,
                    targetTermConcept.getDisplay());
            ourLog.trace(
                    "Adding coding property: {} to concept.code {}",
                    thePropertyName,
                    theSrcTermConcept.getCode());
        }
    }
