...
public class ValueSetConceptAccumulator implements IValueSetConceptAccumulator {
	private static final org.slf4j.Logger ourLog =
				org.slf4j.LoggerFactory.getLogger(ValueSetConceptAccumulator.class);
	...
	public boolean excludeConcept(String theSystem, String theCode) {
		...
		if (versionIdx >= 0) {
				...
				optionalConcept =
						myValueSetConceptDao.findByTermValueSetIdSystemAndCodeWithVersion(
									myTermValueSet.getId(), systemUrl, systemVersion, theCode);
		} else {
				optionalConcept =
						myValueSetConceptDao.findByTermValueSetIdSystemAndCode(
									myTermValueSet.getId(), theSystem, theCode);
		}
		...
		if (optionalConcept.isPresent()) {
				...
				if (++myConceptsExcluded % 250 == 0) {
					ourLog.info(
								"Have excluded {} concepts from ValueSet[{}]",
								myConceptsExcluded,
								myTermValueSet.getUrl());
				}
		}
	...
	private TermValueSetConcept saveConcept(
		...
		ValidateUtil.isNotBlankOrThrowInvalidRequest(
					theSystem, "ValueSet contains a concept with no system value");
		ValidateUtil.isNotBlankOrThrowInvalidRequest(
					theCode, "ValueSet contains a concept with no code value");
		...
		if (++myConceptsSaved % 250 == 0) {
				ourLog.info(
						"Have pre-expanded {} concepts in ValueSet[{}]",
						myConceptsSaved,
						myTermValueSet.getUrl());
		}
	...
	public Boolean removeGapsFromConceptOrder() {
		...
		List<Long> conceptIds =
					myValueSetConceptDao.findIdsByTermValueSetId(myTermValueSet.getId());
	...
	// TODO: DM 2019-07-16 - We may need TermValueSetConceptProperty, similar to
	// TermConceptProperty.
	// TODO: DM 2019-07-16 - If so, we should also populate TermValueSetConceptProperty entities
	// here.
	// TODO: DM 2019-07-30 - Expansions don't include the properties themselves; they may be needed
	// to facilitate filters and parameterized expansions.
}
