...
public class TolerantJsonParser extends JsonParser {
	...
	* Constructor
	*
	* @param theResourcePid The ID of the resource that will be parsed with this parser. It would
	*     be ok to change the datatype for this param if we ever need to since it's only used for
	*     logging.
	*/
								...
								.visit(
										...
										(theElement,
												...
												BaseRuntimeElementDefinition<?> def =
														theElementDefinitionPath.get(
																	theElementDefinitionPath.size() - 1);
												if (def.getName().equals("decimal")) {
													IPrimitiveType<BigDecimal> decimal =
																(IPrimitiveType<BigDecimal>) theElement;
													String oldValue = decimal.getValueAsString();
													String newValue = decimal.getValue().toPlainString();
													ourLog.warn(
																"Correcting invalid previously saved decimal number"
																	+ " for Resource[pid={}] - Was {} and now is"
																	+ " {}",
