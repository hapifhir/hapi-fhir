...
public interface ITermValueSetConceptViewOracleDao
	...
	@Query(
				"SELECT v FROM TermValueSetConceptViewOracle v WHERE v.myConceptValueSetPid = :pid AND"
					+ " v.myConceptOrder >= :from AND v.myConceptOrder < :to ORDER BY v.myConceptOrder")
	...
	@Query(
				"SELECT v FROM TermValueSetConceptViewOracle v WHERE v.myConceptValueSetPid = :pid AND"
						+ " LOWER(v.myConceptDisplay) LIKE :display ORDER BY v.myConceptOrder")
	List<TermValueSetConceptViewOracle> findByTermValueSetId(
