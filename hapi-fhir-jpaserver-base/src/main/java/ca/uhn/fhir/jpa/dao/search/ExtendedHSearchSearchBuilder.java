...
public class ExtendedHSearchSearchBuilder {
     ...
     *
     * <p>NOTE - keep this in sync with addAndConsumeAdvancedQueryClauses() below.
     */
    private boolean isParamTypeSupported(IQueryParameterType param) {
        String modifier =
                StringUtils.defaultString(param.getQueryParameterQualifier(), EMPTY_MODIFIER);
    ...
    public void addAndConsumeAdvancedQueryClauses(
        ...
        for (String nextParam : paramNames) {
            ...
            RuntimeSearchParam activeParam =
                    theSearchParamRegistry.getActiveSearchParam(theResourceType, nextParam);
            if (activeParam == null) {
            ...
            switch (activeParam.getParamType()) {
                case TOKEN:
                    List<List<IQueryParameterType>> tokenTextAndOrTerms =
                            theParams.removeByNameAndModifier(
                                    nextParam, Constants.PARAMQUALIFIER_TOKEN_TEXT);
                ...
                case DATE:
                    List<List<IQueryParameterType>> dateAndOrTerms =
                            nextParam.equalsIgnoreCase("_lastupdated")
                ...
                case COMPOSITE:
                    List<List<IQueryParameterType>> compositeAndOrTerms =
                            theParams.removeByNameUnmodified(nextParam);
                    // RuntimeSearchParam only points to the subs by reference.  Resolve here while
                    // we have ISearchParamRegistry
                    List<RuntimeSearchParam> subSearchParams =
                            JpaParamUtil.resolveCompositeComponentsDeclaredOrder(
                                    theSearchParamRegistry, activeParam);
                    builder.addCompositeUnmodifiedSearch(
                            activeParam, subSearchParams, compositeAndOrTerms);
                    break;
                ...
                case NUMBER:
                    List<List<IQueryParameterType>> numberUnmodifiedAndOrTerms =
                            theParams.remove(nextParam);
                    builder.addNumberUnmodifiedSearch(nextParam, numberUnmodifiedAndOrTerms);
                    break;

                default:
                    // ignore unsupported param types/modifiers.  They will be processed up in
                    // SearchBuilder.
            }
        }
    }
    ...
    private List<List<IQueryParameterType>> getLastUpdatedAndOrList(SearchParameterMap theParams) {
        DateParam activeBound =
                theParams.getLastUpdated().getLowerBound() != null
    ...
    /**
     * Param name list is not only the params.keySet, but also the "special" parameters extracted
     * from input (as _lastUpdated when the input myLastUpdated field is not null, etc).
     */
