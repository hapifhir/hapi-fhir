...
public class PackageInstallerSvcImpl implements IPackageInstallerSvc {
	...
	void install(
		...
		IBaseResource existingResource = findExistingResource(dao, theResource, searchResult);
	...
	private IBaseResource findExistingResource(
			IFhirResourceDao<?> theDao, IBaseResource theResource, IBundleProvider theSearchResult) {
		if (!theSearchResult.isEmpty()) {
			return theSearchResult.getResources(0, 1).get(0);
		}
		if ("CodeSystem".equals(theResource.fhirType())) {
			IBaseResource fromTermLayer = findExistingCodeSystemViaTermLayer(theDao, theResource);
			if (fromTermLayer != null) {
				ourLog.info(
						"FHIR search index missed existing CodeSystem but term layer found it at {}; routing to update",
						fromTermLayer.getIdElement().toUnqualifiedVersionless());
			}
	...
	// Generated by Claude Opus 4.6
	// Fallback lookup via term tables when the FHIR search index is stale/misconfigured.
	// No performance concern: all queries hit indexed columns
	@SuppressWarnings("unchecked")
	private IBaseResource findExistingCodeSystemViaTermLayer(IFhirResourceDao theDao, IBaseResource theResource) {
		String url = extractSimpleValueIfPresent(theResource, "url");
		String version = extractSimpleValueIfPresent(theResource, "version");
		if (url.isEmpty()) {
			return null;
		}
		Long resourcePid = myTxService
				.withSystemRequest()
				.withRequestPartitionId(myPartitionSettings.getDefaultRequestPartitionId())
				.execute(() -> {
					TermCodeSystemVersion csv;
					if (version.isEmpty()) {
						TermCodeSystem tcs = myTermCodeSystemDao.findByCodeSystemUri(url);
						if (tcs == null) {
							return null;
						}
						csv = myTermCodeSystemVersionDao.findByCodeSystemPidVersionIsNull(tcs.getPid());
					} else {
						csv = myTermCodeSystemVersionDao.findByCodeSystemUriAndVersion(url, version);
					}
					if (csv == null) {
						return null;
					}
					return csv.getResource().getId().getId();
				});
		if (resourcePid == null) {
			return null;
		}
		return theDao.readByPid(JpaPid.fromId(resourcePid));
	}
