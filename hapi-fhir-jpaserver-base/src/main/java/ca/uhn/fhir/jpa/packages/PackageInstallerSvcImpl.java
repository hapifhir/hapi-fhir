...
public class PackageInstallerSvcImpl implements IPackageInstallerSvc {
    ...
    @Autowired private IHapiPackageCacheManager myPackageCacheManager;
    @Autowired private IHapiTransactionService myTxService;
    @Autowired private INpmPackageVersionDao myPackageVersionDao;
    @Autowired private ISearchParamRegistryController mySearchParamRegistryController;
    @Autowired private PartitionSettings myPartitionSettings;
    @Autowired private SearchParameterHelper mySearchParameterHelper;
    @Autowired private PackageResourceParsingSvc myPackageResourceParsingSvc;
    ...
    public PackageDeleteOutcomeJson uninstall(PackageInstallationSpec theInstallationSpec) {
        PackageDeleteOutcomeJson outcome =
                myPackageCacheManager.uninstallPackage(
    ...
    /**
     * Loads and installs an IG from a file on disk or the Simplifier repo using the {@link
     * IPackageCacheManager}.
     *
     * <p>Installs the IG by persisting instances of the following types of resources:
     *
     * <p>- NamingSystem, CodeSystem, ValueSet, StructureDefinition (with snapshots), ConceptMap,
     * SearchParameter, Subscription
     *
     * <p>Creates the resources if non-existent, updates them otherwise.
     *
            ...
            try {
                ...
                boolean exists =
                        myTxService
                                ...
                                .execute(
                                        () -> {
                                            Optional<NpmPackageVersionEntity> existing =
                                                    myPackageVersionDao.findByPackageIdAndVersion(
     ...
     *
     * <p>Fails fast if one of its dependencies could not be installed.
     *
    ...
    private void install(
        ...
        for (int i = 0; i < installTypes.size(); i++) {
            ...
            Collection<IBaseResource> resources =
                    myPackageResourceParsingSvc.parseResourcesOfType(type, npmPackage);
            count[i] = resources.size();

            for (IBaseResource next : resources) {
                try {
                    next =
                            isStructureDefinitionWithoutSnapshot(next)
                                    ? generateSnapshot(next)
                                    : next;
                    ...
                    throw new ImplementationGuideInstallationException(
                            Msg.code(1286)
                                    + String.format(
                                            "Error installing IG %s#%s: %s", name, version, e),
                            e);
                }
            }
        }
        ...
        for (int i = 0; i < count.length; i++) {
            ourLog.info(
                    String.format(
                            "-- Created or updated %s resources of type %s",
                            count[i], installTypes.get(i)));
        }
    }
        ...
        if (npmPackage.getNpm().has("dependencies")) {
            ...
            for (String id : dependenciesElement.getNames()) {
                ...
                try {
                    ...
                    for (String next : theInstallationSpec.getDependencyExcludes()) {
                        if (id.matches(next)) {
                            theOutcome
                                    .getMessage()
                                    .add(
                                            "Not installing dependency "
                                                    + id
                                                    + " because it matches exclude criteria: "
                                                    + next);
                            skip = true;
                            break;
                        }
                    ...
                    throw new ImplementationGuideInstallationException(
                            Msg.code(1287)
                                    + String.format("Cannot resolve dependency %s#%s", id, ver),
                            e);
                }
            }
        }
    ...
    /**
     * Asserts if package FHIR version is compatible with current FHIR version by using semantic
     * versioning rules.
     */
    protected void assertFhirVersionsAreCompatible(String fhirVersion, String currentFhirVersion)
        ...
        FhirVersionEnum currentFhirVersionEnum =
                FhirVersionEnum.forVersionString(currentFhirVersion);
        Validate.notNull(fhirVersionEnum, "Invalid FHIR version string: %s", fhirVersion);
        Validate.notNull(
                currentFhirVersionEnum, "Invalid FHIR version string: %s", currentFhirVersion);
        ...
        if (!compatible) {
            throw new ImplementationGuideInstallationException(
                    Msg.code(1288)
                            + String.format(
                                    "Cannot install implementation guide: FHIR versions mismatch"
                                            + " (expected <=%s, package uses %s)",
    ...
    private void create(
        ...
        if (validForUpload(theResource)) {
            if (searchResult.isEmpty()) {
                ...
                ourLog.info(
                        "Creating new resource matching {}",
                ...
                if (theInstallationSpec.isReloadExisting()) {
                    ourLog.info(
                            "Updating existing resource matching {}",
                            map.toNormalizedQueryString(myFhirContext));
                    theResource.setId(
                            searchResult
                                    .getResources(0, 1)
                    ...
                    if (!outcome.isNop()) {
                        theOutcome.incrementResourcesInstalled(
                                myFhirContext.getResourceType(theResource));
                    }
                } else {
                    ourLog.info(
            ...
            ourLog.warn(
                    "Failed to upload resource of type {} with ID {} - Error: Resource failed"
                            + " validation",
    ...
    boolean validForUpload(IBaseResource theResource) {
        ...
        if ("SearchParameter".equals(resourceType)) {
            ...
            if (defaultString(code).startsWith("_")) {
                ourLog.warn(
                        "Failed to validate resource of type {} with url {} - Error: Resource code"
                                + " starts with \"_\"",
                        theResource.fhirType(),
            ...
            if (isBlank(expression)) {
                ourLog.warn(
                        "Failed to validate resource of type {} with url {} - Error: Resource"
                                + " expression is blank",
                        theResource.fhirType(),
                        SearchParameterUtil.getURL(myFhirContext, theResource));
                return false;
            ...
            if (SearchParameterUtil.getBaseAsStrings(myFhirContext, theResource).isEmpty()) {
                ourLog.warn(
                        "Failed to validate resource of type {} with url {} - Error: Resource base"
                                + " is empty",
                        theResource.fhirType(),
                        SearchParameterUtil.getURL(myFhirContext, theResource));
                return false;
        ...
        if (!isValidResourceStatusForPackageUpload(theResource)) {
            ourLog.warn(
                    "Failed to validate resource of type {} with ID {} - Error: Resource status not"
                            + " accepted value.",
                    theResource.fhirType(),
                    theResource.getIdElement().getValue());
            return false;
    ...
    /**
     * For resources like {@link org.hl7.fhir.r4.model.Subscription}, {@link
     * org.hl7.fhir.r4.model.DocumentReference}, and {@link org.hl7.fhir.r4.model.Communication},
     * the status field doesn't necessarily need to be set to 'active' for that resource to be
     * eligible for upload via packages. For example, all {@link org.hl7.fhir.r4.model.Subscription}
     * have a status of {@link org.hl7.fhir.r4.model.Subscription.SubscriptionStatus#REQUESTED} when
     * they are originally inserted into the database, so we accept that value for {@link
     * org.hl7.fhir.r4.model.Subscription} isntead. Furthermore, {@link
     * org.hl7.fhir.r4.model.DocumentReference} and {@link org.hl7.fhir.r4.model.Communication} can
     * exist with a wide variety of values for status that include ones such as {@link
     * org.hl7.fhir.r4.model.Communication.CommunicationStatus#ENTEREDINERROR}, {@link
     * org.hl7.fhir.r4.model.Communication.CommunicationStatus#UNKNOWN}, {@link
     * org.hl7.fhir.r4.model.DocumentReference.ReferredDocumentStatus#ENTEREDINERROR}, {@link
     * org.hl7.fhir.r4.model.DocumentReference.ReferredDocumentStatus#PRELIMINARY}, and others,
     * which while not considered 'final' values, should still be uploaded for reference.
     *
     * @return {@link Boolean#TRUE} if the status value of this resource is acceptable for package
     *     upload.
     */
    ...
    private IBaseResource generateSnapshot(IBaseResource sd) {
        try {
            ...
            throw new ImplementationGuideInstallationException(
                    Msg.code(1290)
                            + String.format(
                                    "Failure when generating snapshot of StructureDefinition: %s",
                                    sd.getIdElement()),
                    e);
        }
    }
    ...
    private SearchParameterMap createSearchParameterMapFor(IBaseResource resource) {
        if (resource.getClass().getSimpleName().equals("NamingSystem")) {
            ...
            return SearchParameterMap.newSynchronous()
                    .add("value", new StringParam(uniqueId).setExact(true));
    ...
    /**
     * Strategy is to build a SearchParameterMap same way the SearchParamValidatingInterceptor does,
     * to make sure that the loader search detects existing resources and routes process to 'update'
     * path, to avoid treating it as a new upload which validator later rejects as duplicated. To
     * achieve this, we try canonicalizing the SearchParameter first (as the validator does) and if
     * that is not possible we cascade to building the map from 'url' or 'identifier'.
     */
    private SearchParameterMap buildSearchParameterMapForSearchParameter(
            IBaseResource theResource) {
    ...
    private String extractUniqeIdFromNamingSystem(IBaseResource resource) {
        ...
        IPrimitiveType<?> asPrimitiveType =
                (IPrimitiveType<?>) terser.getSingleValueOrNull(uniqueIdComponent, "value");
    ...
    private String extractIdFromSubscription(IBaseResource resource) {
        FhirTerser terser = myFhirContext.newTerser();
        IPrimitiveType<?> asPrimitiveType =
                (IPrimitiveType<?>) terser.getSingleValueOrNull(resource, "id");
        return (String) asPrimitiveType.getValue();
    }
    ...
    private String extractUniqueUrlFromMetadataResource(IBaseResource resource) {
        FhirTerser terser = myFhirContext.newTerser();
        IPrimitiveType<?> asPrimitiveType =
                (IPrimitiveType<?>) terser.getSingleValueOrNull(resource, "url");
        return (String) asPrimitiveType.getValue();
    }
    ...
    private TokenParam extractIdentifierFromOtherResourceTypes(IBaseResource resource) {
        ...
        if (identifier != null) {
            ...
            throw new UnsupportedOperationException(
                    Msg.code(1292)
                            + "Resources in a package must have a url or identifier to be loaded by"
    ...
    private boolean resourceHasUrlElement(IBaseResource resource) {
        BaseRuntimeElementDefinition<?> def =
                myFhirContext.getElementDefinition(resource.getClass());
        if (!(def instanceof BaseRuntimeElementCompositeDefinition)) {
            throw new IllegalArgumentException(
                    Msg.code(1293)
                            + "Resource is not a composite type: "
