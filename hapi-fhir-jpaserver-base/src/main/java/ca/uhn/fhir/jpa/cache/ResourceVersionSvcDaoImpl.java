...
/**
 * This service builds a map of resource ids to versions based on a SearchParameterMap. It is used
 * by the in-memory resource-version cache to detect when resource versions have been changed by
 * remote processes.
 */
@Service
public class ResourceVersionSvcDaoImpl implements IResourceVersionSvc {
    ...
    @Autowired IResourceTableDao myResourceTableDao;
    @Autowired IIdHelperService<JpaPid> myIdHelperService;
    ...
    public ResourceVersionMap getVersionMap(
        ...
        List<JpaPid> jpaPids =
                dao.searchForIds(
                        theSearchParamMap,
                        new SystemRequestDetails().setRequestPartitionId(theRequestPartitionId));
    ...
    @Override
    /**
     * Retrieves the latest versions for any resourceid that are found. If they are not found, they
     * will not be contained in the returned map. The key should be the same value that was passed
     * in to allow consumer to look up the value using the id they already have.
     *
     * <p>This method should not throw, so it can safely be consumed in transactions.
     *
    ...
    public ResourcePersistentIdMap getLatestVersionIdsForResourceIds(
        ...
        for (String resourceType : resourceTypeToIds.keySet()) {
            ResourcePersistentIdMap idAndPID =
                    getIdsOfExistingResources(
                            theRequestPartitionId, resourceTypeToIds.get(resourceType));
    ...
    /**
     * Helper method to determine if some resources exist in the DB (without throwing). Returns a
     * set that contains the IIdType for every resource found. If it's not found, it won't be
     * included in the set.
     *
    ...
    private ResourcePersistentIdMap getIdsOfExistingResources(
        ...
        List<JpaPid> jpaPids =
                myIdHelperService.resolveResourcePersistentIdsWithCache(
                        thePartitionId, new ArrayList<>(theIds));
        ...
        for (JpaPid pid : jpaPids) {
            ...
            Optional<IIdType> idOp =
                    theIds.stream()
                            .filter(
                                    i ->
                                            i.getIdPart()
                                                    .equals(
                                                            pid.getAssociatedResourceId()
                                                                    .getIdPart()))
                            .findFirst();
        ...
        if (!pidsToVersionToResourcePid.isEmpty()) {
            Collection<Object[]> resourceEntries =
                    myResourceTableDao.getResourceVersionsForPid(
                            new ArrayList<>(pidsToVersionToResourcePid.keySet()));
