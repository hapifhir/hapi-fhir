...
public abstract class BaseHapiFhirSystemDao<T extends IBaseBundle, MT> extends BaseStorageDao
	...
	private static final org.slf4j.Logger ourLog =
				org.slf4j.LoggerFactory.getLogger(BaseHapiFhirSystemDao.class);
	...
	@Autowired private ExpungeService myExpungeService;
	...
	public ExpungeOutcome expunge(
				ExpungeOptions theExpungeOptions, RequestDetails theRequestDetails) {
	...
	private void validateExpungeEnabled(ExpungeOptions theExpungeOptions) {
		if (!getStorageSettings().isExpungeEnabled()) {
				throw new MethodNotAllowedException(
						Msg.code(2080) + "$expunge is not enabled on this server");
		}

		if (theExpungeOptions.isExpungeEverything()
					&& !getStorageSettings().isAllowMultipleDelete()) {
				throw new MethodNotAllowedException(
						Msg.code(2081) + "Multiple delete is not enabled on this server");
		}
	}
	...
	public Map<String, Long> getResourceCountsFromCache() {
		if (myResourceCountsCache == null) {
				...
				myResourceCountsCache =
						myApplicationContext.getBean("myResourceCountsCache", ResourceCountCache.class);
		}
	...
	public IBundleProvider history(
				Date theSince, Date theUntil, Integer theOffset, RequestDetails theRequestDetails) {
		StopWatch w = new StopWatch();
		ReadPartitionIdRequestDetails details =
					ReadPartitionIdRequestDetails.forHistory(null, null);
		...
		IBundleProvider retVal =
					myTransactionService
								...
								.execute(
										() ->
													myPersistedJpaBundleProviderFactory.history(
																theRequestDetails,
																null,
																null,
																theSince,
	...
	public <P extends IResourcePersistentId> void preFetchResources(
		...
		HapiTransactionService.requireTransaction();
		List<Long> pids =
					theResolvedIds.stream().map(t -> ((JpaPid) t).getId()).collect(Collectors.toList());
								...
								ids -> {
									...
									*/
									if (ids.size() >= 2) {
										...
										preFetchIndexes(
													ids, "forcedId", "myForcedId", loadedResourceTableEntries);

										List<Long> entityIds;

										if (thePreFetchIndexes) {
												entityIds =
														loadedResourceTableEntries.stream()
																	.filter(ResourceTable::isParamsStringPopulated)
																	.map(ResourceTable::getId)
																	.collect(Collectors.toList());
												if (entityIds.size() > 0) {
													preFetchIndexes(
																entityIds, "string", "myParamsString", null);
												}
												...
												if (entityIds.size() > 0) {
													preFetchIndexes(
																entityIds, "quantity", "myParamsQuantity", null);
												}
												...
												if (myStorageSettings.getIndexMissingFields()
														== JpaStorageSettings.IndexEnabledEnum.ENABLED) {
													preFetchIndexes(
																entityIds,
																"searchParamPresence",
										...
										new QueryChunker<ResourceTable>()
													.chunk(
																...
																entries -> {
																	...
																	CriteriaBuilder b =
																				myEntityManager.getCriteriaBuilder();
																	CriteriaQuery<ResourceHistoryTable> q =
																				b.createQuery(
																						ResourceHistoryTable.class);
																	Root<ResourceHistoryTable> from =
																				q.from(ResourceHistoryTable.class);
																	...
																	for (ResourceHistoryTable next : resultList) {
																		ResourceTable nextEntity =
																					entities.get(next.getResourceId());
																		if (nextEntity != null) {
																				nextEntity.setCurrentVersionEntity(
																						next);
																		}
																	}
																});
	...
	private void preFetchIndexes(
		...
		new QueryChunker<Long>()
					.chunk(
								theIds,
								ids -> {
									TypedQuery<ResourceTable> query =
												myEntityManager.createQuery(
														"FROM ResourceTable r LEFT JOIN FETCH r."
																	+ fieldName
																	+ " WHERE r.myId IN ( :IDS )",
														ResourceTable.class);
