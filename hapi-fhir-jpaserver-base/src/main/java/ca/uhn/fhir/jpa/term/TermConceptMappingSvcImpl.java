...
public class TermConceptMappingSvcImpl implements ITermConceptMappingSvc {
	...
	@Autowired protected ITermConceptMapDao myConceptMapDao;
	@Autowired protected ITermConceptMapGroupDao myConceptMapGroupDao;
	@Autowired protected ITermConceptMapGroupElementDao myConceptMapGroupElementDao;
	@Autowired protected ITermConceptMapGroupElementTargetDao myConceptMapGroupElementTargetDao;
	...
	@Override
	@Transactional
	public void storeTermConceptMapAndChildren(
				ResourceTable theResourceTable, ConceptMap theConceptMap) {
		...
		String source =
					theConceptMap.hasSourceUriType()
								? theConceptMap.getSourceUriType().getValueAsString()
								: null;
		String target =
					theConceptMap.hasTargetUriType()
		...
		if (isBlank(conceptMapVersion)) {
				optionalExistingTermConceptMapByUrl =
						myConceptMapDao.findTermConceptMapByUrlAndNullVersion(conceptMapUrl);
		} else {
				optionalExistingTermConceptMapByUrl =
				...
				for (ConceptMap.ConceptMapGroupComponent group : theConceptMap.getGroup()) {
					...
					if (isBlank(groupSource)) {
						throw new UnprocessableEntityException(
									Msg.code(838)
												+ "ConceptMap[url='"
												+ theConceptMap.getUrl()
												+ "'] contains at least one group without a value in"
					...
					if (group.hasElement()) {
						...
						for (ConceptMap.SourceElementComponent element : group.getElement()) {
								...
								termConceptMapGroup
										...
										.add(termConceptMapGroupElement);
								...
								if (element.hasTarget()) {
									...
									for (ConceptMap.TargetElementComponent elementTarget :
												element.getTarget()) {
										...
										termConceptMapGroupElementTarget.setConceptMapGroupElement(
													termConceptMapGroupElement);
										termConceptMapGroupElementTarget.setCode(elementTarget.getCode());
										termConceptMapGroupElementTarget.setDisplay(
													elementTarget.getDisplay());
										termConceptMapGroupElementTarget.setEquivalence(
													elementTarget.getEquivalence());
										termConceptMapGroupElement
													.getConceptMapGroupElementTargets()
													.add(termConceptMapGroupElementTarget);
										myConceptMapGroupElementTargetDao.save(
													termConceptMapGroupElementTarget);
	...
	public TranslateConceptResults translate(TranslationRequest theTranslationRequest) {
		...
		Root<TermConceptMapGroupElementTarget> root =
					query.from(TermConceptMapGroupElementTarget.class);

		Join<TermConceptMapGroupElementTarget, TermConceptMapGroupElement> elementJoin =
					root.join("myConceptMapGroupElement");
		Join<TermConceptMapGroupElement, TermConceptMapGroup> groupJoin =
					elementJoin.join("myConceptMapGroup");
		...
		// -- get the latest ConceptMapVersion if theTranslationRequest has ConceptMap url but no
		// ConceptMap version
		...
		for (TranslationQuery translationQuery : translationQueries) {
				...
				if (cachedTargets == null) {
					...
					if (coding.hasCode()) {
						predicates.add(
									criteriaBuilder.equal(elementJoin.get("myCode"), coding.getCode()));
					} else {
					...
					if (coding.hasSystem()) {
						predicates.add(
									criteriaBuilder.equal(groupJoin.get("mySource"), coding.getSystem()));
					}

					if (coding.hasVersion()) {
						predicates.add(
									criteriaBuilder.equal(
												groupJoin.get("mySourceVersion"), coding.getVersion()));
					}

					if (translationQuery.hasTargetSystem()) {
						predicates.add(
									criteriaBuilder.equal(
												groupJoin.get("myTarget"), translationQuery.getTargetSystem()));
					}

					if (translationQuery.hasUrl()) {
						predicates.add(
									criteriaBuilder.equal(
												conceptMapJoin.get("myUrl"), translationQuery.getUrl()));
						if (translationQuery.hasConceptMapVersion()) {
								// both url and conceptMapVersion
								predicates.add(
										criteriaBuilder.equal(
													conceptMapJoin.get("myVersion"),
													translationQuery.getConceptMapVersion()));
						} else {
					...
					if (translationQuery.hasSource()) {
						predicates.add(
									criteriaBuilder.equal(
												conceptMapJoin.get("mySource"), translationQuery.getSource()));
					}

					if (translationQuery.hasTarget()) {
						predicates.add(
									criteriaBuilder.equal(
												conceptMapJoin.get("myTarget"), translationQuery.getTarget()));
					}
					...
					if (translationQuery.hasResourceId()) {
						...
						JpaPid resourcePid =
									myIdHelperService.getPidOrThrowException(
												RequestPartitionId.defaultPartition(), resourceId);
						predicates.add(
									criteriaBuilder.equal(
												conceptMapJoin.get("myResourcePid"), resourcePid.getId()));
					}
					...
					try (ScrollableResultsIterator<TermConceptMapGroupElementTarget>
					...
					myMemoryCacheService.put(
								MemoryCacheService.CacheEnum.CONCEPT_TRANSLATION,
								translationQuery,
								targets);
	...
	public TranslateConceptResults translateWithReverse(TranslationRequest theTranslationRequest) {
		...
		CriteriaBuilder criteriaBuilder = myEntityManager.getCriteriaBuilder();
		CriteriaQuery<TermConceptMapGroupElement> query =
					criteriaBuilder.createQuery(TermConceptMapGroupElement.class);
		...
		for (TranslationQuery translationQuery : translationQueries) {
				cachedElements =
						myMemoryCacheService.getIfPresent(
									MemoryCacheService.CacheEnum.CONCEPT_TRANSLATION_REVERSE,
									translationQuery);
				if (cachedElements == null) {
					...
					if (coding.hasCode()) {
						predicates.add(
									criteriaBuilder.equal(targetJoin.get("myCode"), coding.getCode()));
					...
					if (coding.hasSystem()) {
						predicates.add(
									criteriaBuilder.equal(groupJoin.get("myTarget"), coding.getSystem()));
					...
					if (coding.hasVersion()) {
						predicates.add(
									criteriaBuilder.equal(
												groupJoin.get("myTargetVersion"), coding.getVersion()));
					}
					...
					if (translationQuery.hasTargetSystem()) {
						predicates.add(
									criteriaBuilder.equal(
												groupJoin.get("mySource"), translationQuery.getTargetSystem()));
					}

					if (translationQuery.hasSource()) {
						predicates.add(
									criteriaBuilder.equal(
												conceptMapJoin.get("myTarget"), translationQuery.getSource()));
					}

					if (translationQuery.hasTarget()) {
						predicates.add(
									criteriaBuilder.equal(
												conceptMapJoin.get("mySource"), translationQuery.getTarget()));
					}
					...
					ScrollableResults scrollableResults =
								hibernateQuery.scroll(ScrollMode.FORWARD_ONLY);
					try (ScrollableResultsIterator<TermConceptMapGroupElement>
								scrollableResultsIterator =
										new ScrollableResultsIterator<>(scrollableResults)) {
									...
									for (TermConceptMapGroupElementTarget next :
										...
										if (matches.add(next)) {
												if (isBlank(targetCodeSystem)
														|| StringUtils.equals(
																	targetCodeSystem, next.getSystem())) {
													if (StringUtils.equals(targetCode, next.getCode())) {
														...
														translationMatch.setSystemVersion(
																	nextElement.getSystemVersion());
														translationMatch.setDisplay(nextElement.getDisplay());
														translationMatch.setValueSet(nextElement.getValueSet());
														translationMatch.setSystemVersion(
																	nextElement.getSystemVersion());
														translationMatch.setConceptMapUrl(
																	nextElement.getConceptMapUrl());
														if (next.getEquivalence() != null) {
		...
		for (TranslateConceptResult nextExistingElement : elements) {
				if (StringUtils.equals(nextExistingElement.getSystem(), translationMatch.getSystem())) {
					if (StringUtils.equals(
								nextExistingElement.getSystemVersion(),
								translationMatch.getSystemVersion())) {
						if (StringUtils.equals(
									nextExistingElement.getCode(), translationMatch.getCode())) {
								return true;
						}
					}
				}
	...
	public void deleteConceptMap(ResourceTable theResourceTable) {
		...
		if (optionalExistingTermConceptMapById.isPresent()) {
				...
				ourLog.info(
						"Deleting existing TermConceptMap[{}] and its children...",
						existingTermConceptMap.getId());
				for (TermConceptMapGroup group : existingTermConceptMap.getConceptMapGroups()) {
					...
					for (TermConceptMapGroupElement element : group.getConceptMapGroupElements()) {
						...
						for (TermConceptMapGroupElementTarget target :
									element.getConceptMapGroupElementTargets()) {

								myConceptMapGroupElementTargetDao
										.deleteTermConceptMapGroupElementTargetById(target.getId());
						}

						myConceptMapGroupElementDao.deleteTermConceptMapGroupElementById(
									element.getId());
					}
				...
				ourLog.info(
						"Done deleting existing TermConceptMap[{}] and its children.",
						existingTermConceptMap.getId());
		}
	}
	...
	private String getLatestConceptMapVersion(TranslationRequest theTranslationRequest) {
		...
		List<TermConceptMap> theConceptMapList =
					myConceptMapDao.getTermConceptMapEntitiesByUrlOrderByMostRecentUpdate(
	...
	private void buildTranslationResult(TranslateConceptResults theTranslationResult) {
		...
		if (theTranslationResult.getResults().isEmpty()) {
				...
				msg =
						myContext
									.getLocalizer()
									.getMessage(TermConceptMappingSvcImpl.class, "noMatchesFound");
				...
				msg =
						myContext
									.getLocalizer()
									.getMessage(TermConceptMappingSvcImpl.class, "matchesFound");
				theTranslationResult.setMessage(msg);
		}
	}
	...
	public static Parameters toParameters(TranslateConceptResults theTranslationResult) {
		...
		retVal.addParameter()
					.setName("result")
					.setValue(new BooleanType(theTranslationResult.getResult()));

		if (theTranslationResult.getMessage() != null) {
				retVal.addParameter()
						.setName("message")
						.setValue(new StringType(theTranslationResult.getMessage()));
		}
	...
	private static void populateTranslateMatchParts(
				TranslateConceptResult theTranslationMatch,
				Parameters.ParametersParameterComponent theParam) {
		if (theTranslationMatch.getEquivalence() != null) {
				theParam.addPart()
						.setName("equivalence")
						.setValue(new CodeType(theTranslationMatch.getEquivalence()));
		}
		...
		if (isNotBlank(theTranslationMatch.getSystem())
				...
				Coding value =
						new Coding(
									theTranslationMatch.getSystem(),
									theTranslationMatch.getCode(),
									theTranslationMatch.getDisplay());
		...
		if (isNotBlank(theTranslationMatch.getConceptMapUrl())) {
				theParam.addPart()
						.setName("source")
						.setValue(new UriType(theTranslationMatch.getConceptMapUrl()));
		}
	}
}
