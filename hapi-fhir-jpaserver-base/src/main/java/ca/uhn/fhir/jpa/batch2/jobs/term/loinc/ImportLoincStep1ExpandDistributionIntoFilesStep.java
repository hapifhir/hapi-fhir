...
public class ImportLoincStep1ExpandDistributionIntoFilesStep extends BaseExpandDistributionIntoFilesStep<LoincJobImportParameters, ImportLoincFileSetJson> {
	...
	@Override
	protected void handleSynchronous(StepExecutionDetails<LoincJobImportParameters, VoidModel> theStepExecutionDetails, String theFileName, byte[] theBytes, LoincJobImportParameters theJobParameters, ImportLoincFileSetJson theFileSet) {
		...
		if (theFileName.endsWith("loinc.xml")) {
			...
			String codeSystemVersionId = theJobParameters.getProperties().getProperty(LOINC_CODESYSTEM_VERSION.getCode());
			...
			ITermCodeSystemStorageSvc.StartStagingCodeSystemVersionResponse response = myTermCodeSystemStorageSvc.startStagingCodeSystemVersion(cs.getUrl(), cs.getVersion());
