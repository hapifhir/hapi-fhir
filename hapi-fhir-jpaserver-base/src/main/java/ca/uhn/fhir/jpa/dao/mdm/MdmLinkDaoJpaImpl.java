...
public class MdmLinkDaoJpaImpl implements IMdmLinkDao<JpaPid, MdmLink> {
	...
	@Override
	public int deleteWithAnyReferenceToPidAndMatchResultNot(
				JpaPid thePid, MdmMatchResultEnum theMatchResult) {
		return myMdmLinkDao.deleteWithAnyReferenceToPidAndMatchResultNot(
					thePid.getId(), theMatchResult);
	}
	...
	public List<MdmPidTuple<JpaPid>> expandPidsFromGroupPidGivenMatchResult(
		...
		return myMdmLinkDao
					.expandPidsFromGroupPidGivenMatchResult(
								(theGroupPid).getId(), theMdmMatchResultEnum)
					.stream()
					.map(this::daoTupleToMdmTuple)
					.collect(Collectors.toList());
	}

	private MdmPidTuple<JpaPid> daoTupleToMdmTuple(
				IMdmLinkJpaRepository.MdmPidTuple theMdmPidTuple) {
	...
	public List<MdmPidTuple<JpaPid>> expandPidsBySourcePidAndMatchResult(
		...
		return myMdmLinkDao
					.expandPidsBySourcePidAndMatchResult((theSourcePid).getId(), theMdmMatchResultEnum)
					.stream()
					.map(this::daoTupleToMdmTuple)
					.collect(Collectors.toList());
	}
	...
	public List<MdmPidTuple<JpaPid>> expandPidsByGoldenResourcePidAndMatchResult(
		...
		return myMdmLinkDao
					.expandPidsByGoldenResourcePidAndMatchResult(
								(theSourcePid).getId(), theMdmMatchResultEnum)
					.stream()
					.map(this::daoTupleToMdmTuple)
					.collect(Collectors.toList());
	}
	...
	public List<JpaPid> findPidByResourceNameAndThreshold(
		...
		return myMdmLinkDao
					.findPidByResourceNameAndThreshold(theResourceName, theHighThreshold, thePageable)
					.stream()
					.map(JpaPid::fromId)
					.collect(Collectors.toList());
	}
	...
	public MdmLink validateMdmLink(IMdmLink theMdmLink) throws UnprocessableEntityException {
		if (theMdmLink instanceof MdmLink) {
				...
				throw new UnprocessableEntityException(
						Msg.code(2109) + "Unprocessable MdmLink implementation");
		}
	}
	...
	public Page<MdmLink> search(MdmQuerySearchParameters theParams) {
		...
		TypedQuery<MdmLink> typedQuery =
					myEntityManager.createQuery(criteriaQuery.where(finalQuery));
	...
	private List<Predicate> buildPredicates(
				MdmQuerySearchParameters theParams,
				CriteriaBuilder criteriaBuilder,
				Root<MdmLink> from) {
		List<Predicate> andPredicates = new ArrayList<>();
		if (theParams.getGoldenResourceId() != null) {
				Predicate goldenResourcePredicate =
		...
		if (theParams.getMatchResult() != null) {
				Predicate matchResultPredicate =
						criteriaBuilder.equal(
									from.get(MATCH_RESULT_NAME).as(MdmMatchResultEnum.class),
									theParams.getMatchResult());
				andPredicates.add(matchResultPredicate);
		}
		if (theParams.getLinkSource() != null) {
				Predicate linkSourcePredicate =
						criteriaBuilder.equal(
									from.get(LINK_SOURCE_NAME).as(MdmLinkSourceEnum.class),
	...
	@Override
	public Optional<MdmLink> findBySourcePidAndMatchResult(
				JpaPid theSourcePid, MdmMatchResultEnum theMatch) {
	...
	public void deleteLinksWithAnyReferenceToPids(List<JpaPid> theResourcePersistentIds) {
		...
		// Split into chunks of 500 so older versions of Oracle don't run into issues (500 = 1000 /
		// 2 since the dao
	...
	public Revisions<Long, MdmLink> findHistory(JpaPid theMdmLinkPid) {
		final Revisions<Long, MdmLink> revisions =
					myMdmLinkDao.findRevisions(theMdmLinkPid.getId());
	...
	public List<MdmLinkWithRevision<MdmLink>> getHistoryForIds(
		...
		try {
				final AuditCriterion goldenResourceIdCriterion =
						AuditEntity.property(GOLDEN_RESOURCE_PID_NAME)
									.in(
												convertToLongIds(
														theMdmHistorySearchParameters.getGoldenResourceIds()));
				final AuditCriterion resourceIdCriterion =
						AuditEntity.property(SOURCE_PID_NAME)
				...
				if (!theMdmHistorySearchParameters.getGoldenResourceIds().isEmpty()
					...
					throw new IllegalArgumentException(
								Msg.code(2298)
										+ "$mdm-link-history Golden resource and source query IDs cannot"
										+ " both be empty.");
				}

				@SuppressWarnings("unchecked")
				final List<Object[]> mdmLinksWithRevisions =
						auditQueryCreator
				...
				throw new IllegalStateException(
						Msg.code(2291)
									+ "Hibernate envers AuditReader is returning Service is not yet"
									+ " initialized but front-end validation has not caught the error that"
									+ " envers is disabled");
		}
	}
	...
	private List<Long> convertToLongIds(List<IIdType> theMdmHistorySearchParameters) {
		return theMdmHistorySearchParameters.stream()
					.map(
								id ->
										myIdHelperService.getPidOrThrowException(
													RequestPartitionId.allPartitions(), id))
					.map(JpaPid::getId)
					.collect(Collectors.toUnmodifiableList());
	}
	...
	private MdmLinkWithRevision<MdmLink> buildRevisionFromObjectArray(Object[] theArray) {
		...
		return new MdmLinkWithRevision<>(
					...
					new EnversRevision(
								(RevisionType) revisionTypeUncast,
								revision.getRev(),
								revision.getRevtstmp()));
	}
}
