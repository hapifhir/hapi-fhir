...
public class ValueSetOperationProvider extends BaseJpaProvider {
    ...
    @Autowired protected IValidationSupport myValidationSupport;
    ...
    public IBaseResource expand(
            ...
            @OperationParam(name = "url", min = 0, max = 1, typeName = "uri")
            ...
            @OperationParam(name = "filter", min = 0, max = 1, typeName = "string")
                    IPrimitiveType<String> theFilter,
            @OperationParam(name = "context", min = 0, max = 1, typeName = "string")
            ...
            @OperationParam(name = "offset", min = 0, max = 1, typeName = "integer")
                    IPrimitiveType<Integer> theOffset,
            @OperationParam(name = "count", min = 0, max = 1, typeName = "integer")
    ...
    public IBaseParameters validateCode(
            ...
            @OperationParam(name = "valueSetVersion", min = 0, max = 1, typeName = "string")
                    IPrimitiveType<String> theValueSetVersion,
            @OperationParam(name = "code", min = 0, max = 1, typeName = "code")
            ...
            @OperationParam(name = "display", min = 0, max = 1, typeName = "string")
        ...
        try {
            ...
            if (myValidationSupportChain != null
                    && myValidationSupportChain.isRemoteTerminologyServiceConfigured()) {
                String theSystemString =
                        (theSystem != null && theSystem.hasValue())
                                ? theSystem.getValueAsString()
                                : null;
                String theCodeString =
                        (theCode != null && theCode.hasValue()) ? theCode.getValueAsString() : null;
                String theDisplayString =
                        (theDisplay != null && theDisplay.hasValue())
                                ? theDisplay.getValueAsString()
                                : null;
                String theValueSetUrlString =
                        (theValueSetUrl != null && theValueSetUrl.hasValue())
                                ? theValueSetUrl.getValueAsString()
                                : null;
                if (theCoding != null) {
                    if (isNotBlank(theCoding.getSystem())) {
                        if (theSystemString != null
                                && !theSystemString.equalsIgnoreCase(theCoding.getSystem())) {
                ...
                result =
                        validateCodeWithTerminologyService(
                                        theSystemString,
                                        theCodeString,
                                        theDisplayString,
                                        theValueSetUrlString)
                                .orElseGet(
                                        supplyUnableToValidateResult(
                                                theSystemString,
                                                theCodeString,
                                                theValueSetUrlString));
            } else {
                ...
                if (theValueSetUrl != null && theValueSetVersion != null) {
                    ...
                    valueSetIdentifier.setValue(
                            theValueSetUrl.getValue() + "|" + theValueSetVersion);
                } else {
    ...
    private Optional<CodeValidationResult> validateCodeWithTerminologyService(
        ...
        return Optional.ofNullable(
                myValidationSupportChain.validateCode(
    ...
    private Supplier<CodeValidationResult> supplyUnableToValidateResult(
        ...
        return () ->
                new CodeValidationResult()
                        .setMessage(
                                "Validator is unable to provide validation for "
                                        + theCode
                                        + "#"
                                        + theSystem
        ...
        try {
            ...
            String outcome =
                    myTermReadSvc.invalidatePreCalculatedExpansion(
                            theValueSetId, theRequestDetails);
    ...
    public static ValueSetExpansionOptions createValueSetExpansionOptions(
        ...
        if (theOffset != null && theOffset.hasValue()) {
            if (theOffset.getValue() >= 0) {
                ...
                throw new InvalidRequestException(
                        Msg.code(1135)
                                + "offset parameter for $expand operation must be >= 0 when"
                                + " specified. offset: "
                                + theOffset.getValue());
            }
        }
        ...
        if (theCount != null && theCount.hasValue()) {
            if (theCount.getValue() >= 0) {
                ...
                throw new InvalidRequestException(
                        Msg.code(1136)
                                + "count parameter for $expand operation must be >= 0 when"
                                + " specified. count: "
                                + theCount.getValue());
            }
        }
        ...
        if (count > countMax) {
            ourLog.warn(
                    "count parameter for $expand operation of {} exceeds maximum value of {}; using"
                            + " maximum value.",
                    count,
                    countMax);
            count = countMax;
        ...
        ValueSetExpansionOptions options =
                ValueSetExpansionOptions.forOffsetAndCount(offset, count);
    ...
    public static IBaseParameters toValidateCodeResult(
            FhirContext theContext, CodeValidationResult theResult) {
        IBaseParameters retVal = ParametersUtil.newInstance(theContext);

        ParametersUtil.addParameterToParametersBoolean(
                theContext, retVal, "result", theResult.isOk());
        if (isNotBlank(theResult.getMessage())) {
            ParametersUtil.addParameterToParametersString(
                    theContext, retVal, "message", theResult.getMessage());
        }
        if (isNotBlank(theResult.getDisplay())) {
            ParametersUtil.addParameterToParametersString(
                    theContext, retVal, "display", theResult.getDisplay());
        }
