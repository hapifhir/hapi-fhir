...
public class ParsedLocationParam {
    ...
    private ParsedLocationParam(
            String theLatitudeValue, String theLongitudeValue, double theDistanceKm) {
    ...
    private static double parseLatLonParameter(String theValue) {
        try {
            ...
            throw new InvalidRequestException(
                    Msg.code(2308)
                            + "Invalid lat/lon parameter value: "
                            + UrlUtil.sanitizeUrlPart(theValue));
        }
    }
    ...
    public static ParsedLocationParam from(
            SearchParameterMap theParams, IQueryParameterType theParam) {
        ...
        if (theParam instanceof TokenParam) { // DSTU3
            ...
            if (parts.length < 2 || parts.length > 4) {
                throw new IllegalArgumentException(
                        Msg.code(1230)
                                + "Invalid position format '"
                                + value
                                + "'.  Required format is 'latitude|longitude' or"
                                + " 'latitude|longitude|distance' or"
            ...
            throw new IllegalArgumentException(
                    Msg.code(1232) + "Invalid position type: " + theParam.getClass());
        }
