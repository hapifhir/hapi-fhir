...
public class ResourceReindexingSvcImpl implements IResourceReindexingSvc, IHasScheduledJobs {
    ...
    @Autowired private IResourceReindexJobDao myReindexJobDao;
    ...
    @Autowired private ResourceReindexer myResourceReindexer;
    ...
    public Long markAllResourcesForReindexing(String theType) {
        ...
        if (isNotBlank(theType)) {
            try {
                ...
                throw new InvalidRequestException(
                        Msg.code(1170) + "Unknown resource type: " + theType);
            }
        ...
        ourLog.info(
                "Marking all resources of type {} for reindexing - Got job ID[{}]",
                typeDesc,
                job.getId());
        return job.getId();
    }
    ...
    public static class Job implements HapiJob {
        @Autowired private IResourceReindexingSvc myTarget;
    ...
    public Integer runReindexingPass() {
        if (myStorageSettings.isSchedulingDisabled()
                || !myStorageSettings.isEnableTaskPreExpandValueSets()) {
            return null;
        }
    ...
    private int runReindexJobs() {
        ...
        for (ResourceReindexJobEntity next : jobs) {
            ...
            if (next.getThresholdLow() != null
                    && next.getThresholdLow().getTime() >= next.getThresholdHigh().getTime()) {
    ...
    private int runReindexJob(ResourceReindexJobEntity theJob) {
        ...
        if (latestDate.getTime() == low.getTime()) {
            if (count == PASS_SIZE) {
                // Just in case we end up in some sort of infinite loop. This shouldn't happen, and
                // couldn't really
                // happen unless there were 10000 resources with the exact same update time down to
                // the
                // millisecond.
                ourLog.error(
        ...
        ourLog.info(
                "Completed pass of reindex JOB[{}] - Indexed {} resources in {} ({} / sec) - Have"
                        + " indexed until: {}",
                theJob.getId(),
                count,
                sw,
    ...
    private void expungeJobsMarkedAsDeleted() {
        myTxTemplate.execute(
                t -> {
                    Collection<ResourceReindexJobEntity> toDelete =
                            myReindexJobDao.findAll(PageRequest.of(0, 10), true);
                    toDelete.forEach(
                            job -> {
                ...
                (TransactionCallback<Void>)
                        theStatus -> {
                            ...
                            myResourceTableDao.updateIndexStatus(
                                    theId, BaseHapiFhirDao.INDEX_STATUS_INDEXING_FAILED);

                            Query q =
                                    myEntityManager.createQuery(
                                            "DELETE FROM ResourceTag t WHERE t.myResourceId = :id");
                            q.setParameter("id", theId);
                            q.executeUpdate();

                            q =
                                    myEntityManager.createQuery(
                                            "DELETE FROM ResourceIndexedSearchParamCoords t WHERE"
                                                    + " t.myResourcePid = :id");
                            q.setParameter("id", theId);
                            q.executeUpdate();
                            ...
                            q =
                                    myEntityManager.createQuery(
                                            "DELETE FROM ResourceIndexedSearchParamDate t WHERE"
                                                    + " t.myResourcePid = :id");
                            q.setParameter("id", theId);
                            q.executeUpdate();
                            ...
                            q =
                                    myEntityManager.createQuery(
                                            "DELETE FROM ResourceIndexedSearchParamNumber t WHERE"
                                                    + " t.myResourcePid = :id");
                            q.setParameter("id", theId);
                            q.executeUpdate();
                            ...
                            q =
                                    myEntityManager.createQuery(
                                            "DELETE FROM ResourceIndexedSearchParamQuantity t WHERE"
                                                    + " t.myResourcePid = :id");
                            q.setParameter("id", theId);
                            q.executeUpdate();
                            ...
                            q =
                                    myEntityManager.createQuery(
                                            "DELETE FROM"
                                                + " ResourceIndexedSearchParamQuantityNormalized t"
                                                + " WHERE t.myResourcePid = :id");
                            q.setParameter("id", theId);
                            q.executeUpdate();

                            q =
                                    myEntityManager.createQuery(
                                            "DELETE FROM ResourceIndexedSearchParamString t WHERE"
                                                    + " t.myResourcePid = :id");
                            q.setParameter("id", theId);
                            q.executeUpdate();
                            ...
                            q =
                                    myEntityManager.createQuery(
                                            "DELETE FROM ResourceIndexedSearchParamToken t WHERE"
                                                    + " t.myResourcePid = :id");
                            q.setParameter("id", theId);
                            q.executeUpdate();
                            ...
                            q =
                                    myEntityManager.createQuery(
                                            "DELETE FROM ResourceIndexedSearchParamUri t WHERE"
                                                    + " t.myResourcePid = :id");
                            q.setParameter("id", theId);
                            q.executeUpdate();
                            ...
                            q =
                                    myEntityManager.createQuery(
                                            "DELETE FROM ResourceLink t WHERE t.mySourceResourcePid"
                                                    + " = :id");
                            q.setParameter("id", theId);
                            q.executeUpdate();
                            ...
                            q =
                                    myEntityManager.createQuery(
                                            "DELETE FROM ResourceLink t WHERE t.myTargetResourcePid"
                                                    + " = :id");
                            q.setParameter("id", theId);
                            q.executeUpdate();
    ...
    private class ResourceReindexingTask implements Callable<Date> {
        ...
        public Date call() {
            ...
            try {
                 ...
                 */
                ourLog.info(
                        "Failed to reindex because of a version conflict. Leaving in unindexed"
                                + " state: {}",
                        e.getMessage());
                reindexFailure = null;
            }
