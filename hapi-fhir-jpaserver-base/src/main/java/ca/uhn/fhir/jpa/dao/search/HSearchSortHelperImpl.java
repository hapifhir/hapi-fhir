...
public class HSearchSortHelperImpl implements IHSearchSortHelper {
    ...
    private Map<RestSearchParameterTypeEnum, List<String>> mySortPropertyListMap =
            Map.of(
                    RestSearchParameterTypeEnum.STRING,
                            List.of(SEARCH_PARAM_ROOT + ".*.string." + IDX_STRING_LOWER),
                    RestSearchParameterTypeEnum.TOKEN,
                            List.of(
                                    String.join(
                                            ".", NESTED_SEARCH_PARAM_ROOT, "*", "token", "system"),
                                    String.join(
                                            ".", NESTED_SEARCH_PARAM_ROOT, "*", "token", "code")),
                    RestSearchParameterTypeEnum.REFERENCE,
                            List.of(SEARCH_PARAM_ROOT + ".*.reference.value"),
                    ...
                    RestSearchParameterTypeEnum.NUMBER,
                            List.of(SEARCH_PARAM_ROOT + ".*." + NUMBER_VALUE));
    ...
    public SortFinalStep getSortClauses(
        ...
        Optional<SortFinalStep> sortClauseOpt =
                getSortClause(theSortFactory, theSortParams, theResourceType);
    ...
    /**
     * Builds sort clauses for the received SortSpec by _ finding out the corresponding
     * RestSearchParameterTypeEnum for the parameter _ obtaining the list of properties to sort for
     * the found parameter type _ building the sort clauses for the found list of properties
     */
    @VisibleForTesting
    Optional<SortFinalStep> getSortClause(
            SearchSortFactory theF, SortSpec theSortSpec, String theResourceType) {
        Optional<RestSearchParameterTypeEnum> paramTypeOpt =
                getParamType(theResourceType, theSortSpec.getParamName());
        if (paramTypeOpt.isEmpty()) {
            ourLog.warn(
                    "Sprt parameter type couldn't be determined for parameter: "
                            + theSortSpec.getParamName()
                            + ". Result will not be properly sorted");
            return Optional.empty();
        }
        List<String> paramFieldNameList =
                getSortPropertyList(paramTypeOpt.get(), theSortSpec.getParamName());
        if (paramFieldNameList.isEmpty()) {
            ourLog.warn(
                    "Unable to sort by parameter '"
                            + theSortSpec.getParamName()
    ...
    @VisibleForTesting
    Optional<RestSearchParameterTypeEnum> getParamType(
            String theResourceTypeName, String theParamName) {
        ResourceSearchParams activeSearchParams =
                mySearchParamRegistry.getActiveSearchParams(theResourceTypeName);
    ...
    /**
     * Retrieves the generic property names (* instead of parameter name) from the configured map
     * and replaces the '*' segment by theParamName before returning the final property name list
     */
    @VisibleForTesting
    List<String> getSortPropertyList(
