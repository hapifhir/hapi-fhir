...
/**
 * This class is a {@link IValidationSupport Validation support} module that loads validation
 * resources (StructureDefinition, ValueSet, CodeSystem, etc.) from the resources persisted in the
 * JPA server.
 */
...
public class JpaPersistedResourceValidationSupport implements IValidationSupport {
    ...
    private static final Logger ourLog =
            LoggerFactory.getLogger(JpaPersistedResourceValidationSupport.class);
    ...
    private Cache<String, IBaseResource> myLoadCache =
            CacheFactory.build(TimeUnit.MINUTES.toMillis(1), 1000);
    ...
    public IBaseResource fetchCodeSystem(String theSystem) {
        if (TermReadSvcUtil.isLoincUnversionedCodeSystem(theSystem)) {
            Optional<IBaseResource> currentCSOpt =
                    getCodeSystemCurrentVersion(new UriType(theSystem));
    ...
    /**
     * Obtains the current version of a CodeSystem using the fact that the current version is always
     * pointed by the ForcedId for the no-versioned CS
     */
    ...
    public IBaseResource fetchValueSet(String theSystem) {
        if (TermReadSvcUtil.isLoincUnversionedValueSet(theSystem)) {
            Optional<IBaseResource> currentVSOpt =
                    getValueSetCurrentVersion(new UriType(theSystem));
    ...
    /**
     * Obtains the current version of a ValueSet using the fact that the current version is always
     * pointed by the ForcedId for the no-versioned VS
     */
    ...
    private <T extends IBaseResource> IBaseResource doFetchResource(
            @Nullable Class<T> theClass, String theUri) {
        if (theClass == null) {
            Supplier<IBaseResource>[] fetchers =
                    new Supplier[] {
        ...
        switch (resourceName) {
            case "ValueSet":
                if (localReference) {
                    ...
                    if (versionSeparator != -1) {
                        params.add(
                                ValueSet.SP_VERSION,
                                new TokenParam(theUri.substring(versionSeparator + 1)));
                        params.add(
                                ValueSet.SP_URL,
                    ...
                    if (search.isEmpty()
                        ...
                        if (versionSeparator != -1) {
                            params.add(
                                    ...
                                    new TokenParam(theUri.substring(versionSeparator + 1)));
                            params.add(
                                    "system", new UriParam(theUri.substring(0, versionSeparator)));
                        } else {
            ...
            case "StructureDefinition":
                {
                    ...
                    if (theUri.startsWith("http://hl7.org/fhir/StructureDefinition/")) {
                        String typeName =
                                theUri.substring(
                                        "http://hl7.org/fhir/StructureDefinition/".length());
            ...
            case "Questionnaire":
                {
                    ...
                    if (localReference
                            || myFhirContext
                                    .getVersion()
                                    .getVersion()
                                    .isEquivalentTo(FhirVersionEnum.DSTU2)) {
            ...
            default:
                // N.B.: this code assumes that we are searching by canonical URL and that the
                // CanonicalType in question has a URL
                SearchParameterMap params = new SearchParameterMap();
                params.setLoadSynchronousUpTo(1);
        ...
        if (size > 1) {
            ourLog.warn(
                    "Found multiple {} instances with URL search value of: {}",
                    resourceName,
                    theUri);
        }
    ...
    public void start() {
        ...
        if (myFhirContext.getVersion().getVersion().isNewerThan(FhirVersionEnum.DSTU2)) {
            myCodeSystemType =
                    myFhirContext.getResourceDefinition("CodeSystem").getImplementingClass();
        } else {
            myCodeSystemType =
                    myFhirContext.getResourceDefinition("ValueSet").getImplementingClass();
        }
    }
