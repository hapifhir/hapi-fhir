...
/**
 * Used by {@link CascadingDeleteInterceptor} to handle {@link DeleteConflictList}s in a thead-safe
 * way.
 *
 * <p>Specifically, this class spawns an inner transaction for each {@link DeleteConflictList}. This
 * class is meant to handle any potential delete collisions (ex {@link ResourceGoneException} or
 * {@link ResourceVersionConflictException}. In the former case, we swallow the Exception in the
 * inner transaction then continue. In the latter case, we retry according to the
 * RETRY_BACKOFF_PERIOD and RETRY_MAX_ATTEMPTS before giving up.
 */
public class ThreadSafeResourceDeleterSvc {
    ...
    private static final Logger ourLog =
            LoggerFactory.getLogger(ThreadSafeResourceDeleterSvc.class);
    ...
    public Integer delete(
        ...
        List<String> cascadeDeleteIdCache =
                CascadingDeleteInterceptor.getCascadedDeletesList(theRequest, true);
    ...
    private Integer handleNextSource(
        ...
        // We will retry deletes on any occurrence of ResourceVersionConflictException up to
        // RETRY_MAX_ATTEMPTS
        return myRetryTemplate.execute(
                retryContext -> {
                    String previousNewTransactionValue = null;
                    if (theRequest != null) {
                        previousNewTransactionValue =
                                (String)
                                        theRequest
                                                .getUserData()
                                                .get(REQ_DET_KEY_IN_NEW_TRANSACTION);
                    }

                    try {
                        if (retryContext.getRetryCount() > 0) {
                            ourLog.info(
                                    "Retrying delete of {} - Attempt #{}",
                                    nextSourceId,
                        ...
                        if (theRequest == null || previousNewTransactionValue != null) {
                            ...
                            theRequest
                                    .getUserData()
                                    .put(
                                            REQ_DET_KEY_IN_NEW_TRANSACTION,
                                            REQ_DET_KEY_IN_NEW_TRANSACTION);
                        ...
                        ourLog.info(
                                "{} is already deleted.  Skipping cascade delete of this resource",
                                nextSourceId);
                    } finally {
                        if (theRequest != null) {
                            theRequest
    ...
    private DaoMethodOutcome doDelete(
        ...
        return dao.delete(
                resource.getIdElement(), theConflictList, theRequest, theTransactionDetails);
    }
    ...
    private static RetryTemplate getRetryTemplate() {
        ...
        final SimpleRetryPolicy retryPolicy =
                new SimpleRetryPolicy(
                        RETRY_MAX_ATTEMPTS,
                        Collections.singletonMap(ResourceVersionConflictException.class, true));
