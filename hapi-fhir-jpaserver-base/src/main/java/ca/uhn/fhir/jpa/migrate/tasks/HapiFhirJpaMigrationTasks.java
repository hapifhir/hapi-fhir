...
public class HapiFhirJpaMigrationTasks extends BaseMigrationTasks<VersionEnum> {
    ...
    protected void init680() {
        ...
        // This migration is failing in Oracle because there is already an index created on column
        // RES_VER_PID since it is a primary key.
    ...
    protected void init660() {
...

        {
            ...
            addTagDefConstraint.put(
                    ...
                    "ALTER TABLE HFJ_TAG_DEF ADD CONSTRAINT IDX_TAGDEF_TYPESYSCODEVERUS UNIQUE"
                            + " (TAG_TYPE, TAG_CODE, TAG_SYSTEM, TAG_VERSION, TAG_USER_SELECTED)");
            addTagDefConstraint.put(
...

        {
            ...
            // drop and recreate FK_SPIDXSTR_RESOURCE since it will be useing the old
            // IDX_SP_STRING_RESID
...

        {
            ...
            final Builder.BuilderAddTableByColumns empiLink =
                    version.addTableByColumns(
                            "20230306.6", enversMpiLinkAuditTable, "PID", revColumnName);
            ...
            // N.B.  It's impossible to rename a foreign key in a Hibernate Envers audit table, and
            // the schema migration unit test will fail if we try to drop and recreate it
...

        {
            ...
            // We do this becausea doing a modifyColumn on Postgres (and possibly other RDBMS's)
            // will fail with a nasty error:
            // column "revtstmp" cannot be cast automatically to type timestamp without time zone
            // Hint: You might need to specify "USING revtstmp::timestamp without time zone".
        ...
        version.onTable("HFJ_SPIDX_COORDS")
                ...
                .online(true)
                .withColumns(
                        "HASH_IDENTITY", "SP_LATITUDE", "SP_LONGITUDE", "RES_ID", "PARTITION_ID");

        // Postgres tuning.
        String postgresTuningStatementsAll =
                ClasspathUtil.loadResource(
                        "ca/uhn/fhir/jpa/docs/database/hapifhirpostgres94-init01.sql");
        List<String> postgresTuningStatements =
                Arrays.stream(postgresTuningStatementsAll.split("\\n"))
                        .map(org.apache.commons.lang3.StringUtils::trim)
                        .filter(StringUtils::isNotBlank)
                        .filter(t -> !t.startsWith("--"))
                        .collect(Collectors.toList());
        version.executeRawSqls(
                "20230402.1", Map.of(DriverTypeEnum.POSTGRES_9_4, postgresTuningStatements));
    ...
    protected void init640_after_20230126() {
        Builder version = forVersion(VersionEnum.V6_3_0);
        { // We added this constraint when userSelected and Version were added. It is no longer
            // necessary.
            Builder.BuilderWithTableName tagDefTable = version.onTable("HFJ_TAG_DEF");
    ...
    private void init600() {
        ...
        /**
         * New indexing for the core SPIDX tables. Ensure all queries can be satisfied by the index
         * directly, either as left or right table in a hash or sort join.
         *
         * <p>new date search indexing
         *
         ...
         */
        {
            ...
            tokenTable
                    ...
                    .online(true)
                    .withColumns(
                            "HASH_IDENTITY", "SP_SYSTEM", "SP_VALUE", "RES_ID", "PARTITION_ID");
            ...
            // for joining as second table:
            {
                ...
                tokenTable.dropForeignKey(
                        "20220208.11", "FK7ULX3J1GG3V7MAQREJGC7YBC4", "HFJ_RESOURCE");
        ...
        // index must have same name that indexed FK or SchemaMigrationTest complains because H2
        // sets this index automatically
        ...
        batchInstance
                .addIndex("20220227.2", "IDX_BT2JI_CT")
                .unique(false)
                .withColumns("CREATE_TIME");

        Builder.BuilderAddTableByColumns batchChunk =
                version.addTableByColumns("20220227.3", "BT2_WORK_CHUNK", "ID");
        ...
        batchChunk
                .addIndex("20220227.4", "IDX_BT2WC_II_SEQ")
                .unique(false)
                .withColumns("INSTANCE_ID", "SEQ");
        batchChunk
        ...
        // New Index on HFJ_RESOURCE for $reindex Operation - hapi-fhir #3534
        {
            version.onTable("HFJ_RESOURCE")
                    ...
                    .online(true)
                    .withColumns(
                            "RES_TYPE", "RES_DELETED_AT", "RES_UPDATED", "PARTITION_ID", "RES_ID");

            // Drop existing Index on HFJ_RESOURCE.RES_TYPE since the new Index will meet the
            // overall Index Demand
         ...
         */
        {
            ...
            tokenTable
                    ...
                    .online(true)
                    .withColumns(
                            "HASH_NORM_PREFIX", "SP_VALUE_NORMALIZED", "RES_ID", "PARTITION_ID");
        ...
        // Update tag indexing
        {
            ...
            addResTagConstraint.put(
                    DriverTypeEnum.H2_EMBEDDED,
                    "ALTER TABLE HFJ_RES_TAG ADD CONSTRAINT IDX_RESTAG_TAGID UNIQUE (RES_ID,"
                            + " TAG_ID)");
            addResTagConstraint.put(
                    DriverTypeEnum.MARIADB_10_1,
                    "ALTER TABLE HFJ_RES_TAG ADD CONSTRAINT IDX_RESTAG_TAGID UNIQUE (RES_ID,"
            ...
            // Ugh.  Only oracle supports using IDX_TAG_DEF_TP_CD_SYS to enforce this constraint.
            // The others will create another index.
            // For Sql Server, should change the index to be unique with include columns.  Do this
            // in 6.1
            tagTable.dropIndex("20220429.8", "IDX_TAGDEF_TYPESYSCODE");
            Map<DriverTypeEnum, String> addTagDefConstraint = new HashMap<>();
            addTagDefConstraint.put(
                    DriverTypeEnum.H2_EMBEDDED,
                    "ALTER TABLE HFJ_TAG_DEF ADD CONSTRAINT IDX_TAGDEF_TYPESYSCODE UNIQUE"
                            + " (TAG_TYPE, TAG_CODE, TAG_SYSTEM)");
            addTagDefConstraint.put(
                    DriverTypeEnum.MARIADB_10_1,
                    "ALTER TABLE HFJ_TAG_DEF ADD CONSTRAINT IDX_TAGDEF_TYPESYSCODE UNIQUE"
                            + " (TAG_TYPE, TAG_CODE, TAG_SYSTEM)");
    ...
    private void replaceQuantitySPIndices(Builder theVersion) {
        {
            ...
            // for joining to other queries and sorts
            {
                ...
                // So we need to drop and recreate the constraint to drop the old RES_ID index.
                // Rename it while we're at it.  FK7ULX3J1GG3V7MAQREJGC7YBC4 was not a pretty name.
                quantityTable.dropForeignKey(
                        "20220304.18", "FKN603WJJOI1A6ASEWXBBD78BI5", "HFJ_RESOURCE");
        ...
        {
            Builder.BuilderWithTableName quantityNormTable =
                    theVersion.onTable("HFJ_SPIDX_QUANTITY_NRML");
            ...
            // for joining to other queries and sorts
            {
                ...
                // So we need to drop and recreate the constraint to drop the old RES_ID index.
                // Rename it while we're at it.  FK7ULX3J1GG3V7MAQREJGC7YBC4 was not a pretty name.
                quantityNormTable.dropForeignKey(
                        "20220304.30", "FKRCJOVMUH5KC0O6FVBLE319PYV", "HFJ_RESOURCE");
    ...
    /**
     * See https://github.com/hapifhir/hapi-fhir/issues/3237 for reasoning for these indexes. This
     * adds indexes to various tables to enhance delete-expunge performance, which deletes by PID.
     */
    private void addIndexesForDeleteExpunge(Builder theVersion) {
        ...
        theVersion
                ...
                .doNothing() // This migration was added in error, as this table already has a
                // unique constraint on RESOURCE_PID and every database creates an
                // index on anything that is unique.
                .onlyAppliesToPlatforms(NON_AUTOMATIC_FK_INDEX_PLATFORMS);
    }
    ...
    private void init570() {
        ...
        // both indexes must have same name that indexed FK or SchemaMigrationTest complains because
        // H2 sets this index automatically
    ...
    private void init550() {
        ...
        Builder.BuilderAddTableByColumns cmpToks =
                version.addTableByColumns("20210720.3", "HFJ_IDX_CMB_TOK_NU", "PID");
        ...
        cmpToks.addForeignKey("20210720.4", "FK_IDXCMBTOKNU_RES_ID")
                .toColumn("RES_ID")
                .references("HFJ_RESOURCE", "RES_ID");
        cmpToks.addIndex("20210720.5", "IDX_IDXCMBTOKNU_STR")
                .unique(false)
                .withColumns("IDX_STRING");
        ...
        cmbTokNuTable
                .addColumn("20210722.2", "PARTITION_DATE")
                .nullable()
                .type(ColumnTypeEnum.DATE_ONLY);
    ...
    private void init540() {
        ...
        blkImportJobTable
                .addColumn("ROW_PROCESSING_MODE")
                .nonNullable()
                .type(ColumnTypeEnum.STRING, 20);
    ...
    private void init530() {
        ...
        pkg.addIndex("20210109.3", "IDX_SP_QNTY_NRML_HASH")
                .unique(false)
        ...
        pkg.addIndex("20210109.6", "IDX_SP_QNTY_NRML_UPDATED")
                .unique(false)
                .withColumns("SP_UPDATED");
        ...
        nrmlTable
                .addColumn("20210111.2", "PARTITION_DATE")
                .nullable()
                .type(ColumnTypeEnum.DATE_ONLY);
    ...
    protected void init520() {
        ...
        mdmLink.addColumn("20201029.1", "GOLDEN_RESOURCE_PID")
                .nonNullable()
                .type(ColumnTypeEnum.LONG);
    ...
    protected void init510() {
        ...
        Builder.BuilderAddTableByColumns pkg =
                version.addTableByColumns("20200610.2", "NPM_PACKAGE", "PID");
        ...
        Builder.BuilderAddTableByColumns pkgVer =
                version.addTableByColumns("20200610.5", "NPM_PACKAGE_VER", "PID");
        ...
        pkgVer.addForeignKey("20200610.7", "FK_NPM_PKV_RESID")
                ...
                .references("HFJ_RESOURCE", "RES_ID");
        pkgVer.addIndex("20200610.8", "IDX_PACKVER")
                .unique(true)
                .withColumns("PACKAGE_ID", "VERSION_ID");
        ...
        pkgVerResAdd
                ...
                .references("HFJ_RESOURCE", "RES_ID");
        pkgVerResAdd
                .addIndex("20200610.13", "IDX_PACKVERRES_URL")
                .unique(false)
                .withColumns("CANONICAL_URL");

        init510_20200610();

        Builder.BuilderWithTableName pkgVerMod = version.onTable("NPM_PACKAGE_VER");
        pkgVerMod
                .modifyColumn("20200629.1", "PKG_DESC")
                .nullable()
                .withType(ColumnTypeEnum.STRING, 200);
        pkgVerMod
                .modifyColumn("20200629.2", "DESC_UPPER")
                .nullable()
                .withType(ColumnTypeEnum.STRING, 200);
        ...
        trmConceptMap
                .addIndex("20200910.3", "IDX_CONCEPT_MAP_URL")
                .unique(true)
                .withColumns("URL", "VER");
        ...
        trmValueSet
                .addIndex("20200923.4", "IDX_VALUESET_URL")
                .unique(true)
                .withColumns("URL", "VER");
        ...
        trmValueSetComp
                .addColumn("20201028.1", "SYSTEM_VER")
                .nullable()
    ...
    private void init501() { // 20200514 - present
        ...
        spidxDate
                ...
                .unique(false)
                .withColumns(
                        "HASH_IDENTITY", "SP_VALUE_LOW_DATE_ORDINAL", "SP_VALUE_HIGH_DATE_ORDINAL")
                .doNothing();
        spidxDate
        ...
        Builder.BuilderAddTableByColumns empiLink =
                version.addTableByColumns("20200517.2", "MPI_LINK", "PID");
        empiLink.addColumn("PID").nonNullable().type(ColumnTypeEnum.LONG);
        ...
        empiLink.addIndex("20200517.5", "IDX_EMPI_PERSON_TGT")
                .unique(true)
                .withColumns("PERSON_PID", "TARGET_PID");
    }
        ...
        spidxDate.addTask(
                new CalculateOrdinalDatesTask(VersionEnum.V5_0_0, "20200501.3")
                        .addCalculator(
                                "SP_VALUE_LOW_DATE_ORDINAL",
                                t ->
                                        ResourceIndexedSearchParamDate.calculateOrdinalValue(
                                                t.getDate("SP_VALUE_LOW")))
                        .addCalculator(
                                "SP_VALUE_HIGH_DATE_ORDINAL",
                                t ->
                                        ResourceIndexedSearchParamDate.calculateOrdinalValue(
                                                t.getDate("SP_VALUE_HIGH")))
                        .setColumnName(
                                "SP_VALUE_LOW_DATE_ORDINAL") // It doesn't matter which of the two
                // we choose as they will both be null.
                );
    }

    /**
     * Partway through the 4.3.0 releaase cycle we renumbered to 5.0.0 - We have a bunch of NOP
     * tasks here to avoid breakage for anyone who installed a prerelease before we made the switch
     */
    ...
    protected void init420() { // 20191015 - 20200217
        ...
        searchTable
                .addIndex("20200203.3", "IDX_SEARCH_CREATED")
                .unique(false)
                .withColumns("CREATED");
    }
    ...
    protected void init410() { // 20190815 - 20191014
        ...
        bulkExportJob
                .addIndex("20190921.5", "IDX_BLKEX_EXPTIME")
                .unique(false)
                .withColumns("EXP_TIME");
        ...
        // TermValueSetConceptDesignation
        version.startSectionWithMessage(
                "Processing bulkExportCollectionFile: TRM_VALUESET_C_DESIGNATION");
        ...
        version.onTable("HFJ_SEARCH_RESULT")
                .dropForeignKey("20190921.20", "FK_SEARCHRES_RES", "HFJ_RESOURCE");
        version.onTable("HFJ_SEARCH_RESULT")
                .dropForeignKey("20190921.21", "FK_SEARCHRES_SEARCH", "HFJ_SEARCH");
        ...
        // TermValueSetConcept
        version.startSectionWithMessage(
                "Processing bulkExportCollectionFile: TRM_VALUESET_CONCEPT");
        Builder.BuilderWithTableName termValueSetConceptTable =
                version.onTable("TRM_VALUESET_CONCEPT");
    ...
    protected void init400() { // 20190401 - 20190814
        ...
        version.startSectionWithMessage(
                "Processing table: TRM_VALUESET_CONCEPT, swapping index for unique constraint");
        termValueSetConceptTable.dropIndex("20190801.1", "IDX_VALUESET_CONCEPT_CS_CD");
        // This index has been renamed in later versions. As such, allowing failure here as some DBs
        // disallow
        ...
        Builder.BuilderWithTableName termCodeSystemVersionTable =
        ...
        reindex.addColumn("UPDATE_THRESHOLD_HIGH")
                .nonNullable()
                .type(ColumnTypeEnum.DATE_TIMESTAMP);
        ...
        // Make some columns non-nullable that were previously nullable - These are marked as
        // failure allowed, since
    ...
    private void init360() { // 20180918 - 20181112
        ...
        version.addTableRawSql("20181104.1", "HFJ_RES_REINDEX_JOB")
                .addSql(
                        DriverTypeEnum.MSSQL_2012,
                        "create table HFJ_RES_REINDEX_JOB (PID bigint not null, JOB_DELETED bit not"
                                + " null, RES_TYPE varchar(255), SUSPENDED_UNTIL datetime2,"
                                + " UPDATE_THRESHOLD_HIGH datetime2 not null, UPDATE_THRESHOLD_LOW"
                                + " datetime2, primary key (PID))")
                .addSql(
                        DriverTypeEnum.DERBY_EMBEDDED,
                        "create table HFJ_RES_REINDEX_JOB (PID bigint not null, JOB_DELETED boolean"
                                + " not null, RES_TYPE varchar(255), SUSPENDED_UNTIL timestamp,"
                                + " UPDATE_THRESHOLD_HIGH timestamp not null, UPDATE_THRESHOLD_LOW"
                                + " timestamp, primary key (PID))")
                .addSql(
                        DriverTypeEnum.MARIADB_10_1,
                        "create table HFJ_RES_REINDEX_JOB (PID bigint not null, JOB_DELETED bit not"
                            + " null, RES_TYPE varchar(255), SUSPENDED_UNTIL datetime(6),"
                            + " UPDATE_THRESHOLD_HIGH datetime(6) not null, UPDATE_THRESHOLD_LOW"
                            + " datetime(6), primary key (PID))")
                .addSql(
                        DriverTypeEnum.POSTGRES_9_4,
                        "create table HFJ_RES_REINDEX_JOB (PID int8 not null, JOB_DELETED boolean"
                                + " not null, RES_TYPE varchar(255), SUSPENDED_UNTIL timestamp,"
                                + " UPDATE_THRESHOLD_HIGH timestamp not null, UPDATE_THRESHOLD_LOW"
                                + " timestamp, primary key (PID))")
                .addSql(
                        DriverTypeEnum.MYSQL_5_7,
                        " create table HFJ_RES_REINDEX_JOB (PID bigint not null, JOB_DELETED bit"
                            + " not null, RES_TYPE varchar(255), SUSPENDED_UNTIL datetime(6),"
                            + " UPDATE_THRESHOLD_HIGH datetime(6) not null, UPDATE_THRESHOLD_LOW"
                            + " datetime(6), primary key (PID))")
                .addSql(
                        DriverTypeEnum.ORACLE_12C,
                        "create table HFJ_RES_REINDEX_JOB (PID number(19,0) not null, JOB_DELETED"
                            + " number(1,0) not null, RES_TYPE varchar2(255 char), SUSPENDED_UNTIL"
                            + " timestamp, UPDATE_THRESHOLD_HIGH timestamp not null,"
                            + " UPDATE_THRESHOLD_LOW timestamp, primary key (PID))");
    ...
    private void init350() { // 20180601 - 20180917
        ...
        forcedId.addIndex("20180827.3", "IDX_FORCEDID_TYPE_FID")
                .unique(true)
                .withColumns("RESOURCE_TYPE", "FORCED_ID");
        ...
        if (!myFlags.contains(FlagEnum.NO_MIGRATE_HASHES)) {
            ...
            spidxCoords.addTask(
                    new CalculateHashesTask(VersionEnum.V3_5_0, "20180903.5")
                            .addCalculator(
                                    "HASH_IDENTITY",
                                    t ->
        ...
        if (!myFlags.contains(FlagEnum.NO_MIGRATE_HASHES)) {
            ...
            spidxDate.addTask(
                    new CalculateHashesTask(VersionEnum.V3_5_0, "20180903.10")
                            .addCalculator(
                                    "HASH_IDENTITY",
                                    t ->
                                            BaseResourceIndexedSearchParam.calculateHashIdentity(
        ...
        if (!myFlags.contains(FlagEnum.NO_MIGRATE_HASHES)) {
            ...
            spidxNumber
                    ...
                    .doNothing();
            spidxNumber.addTask(
                    new CalculateHashesTask(VersionEnum.V3_5_0, "20180903.14")
                            .addCalculator(
                                    "HASH_IDENTITY",
                                    t ->
                                            BaseResourceIndexedSearchParam.calculateHashIdentity(
        ...
        if (!myFlags.contains(FlagEnum.NO_MIGRATE_HASHES)) {
            ...
            spidxQuantity
                    ...
                    .unique(false)
                    .withColumns("HASH_IDENTITY_SYS_UNITS", "SP_VALUE");
            spidxQuantity.addTask(
                    new CalculateHashesTask(VersionEnum.V3_5_0, "20180903.22")
                            .addCalculator(
                                    "HASH_IDENTITY",
                                    t ->
                                            BaseResourceIndexedSearchParam.calculateHashIdentity(
                            ...
                            .addCalculator(
                                    "HASH_IDENTITY_AND_UNITS",
                                    t ->
                                            ResourceIndexedSearchParamQuantity.calculateHashUnits(
                                                    new PartitionSettings(),
                                                    RequestPartitionId.defaultPartition(),
                                                    t.getResourceType(),
                                                    t.getString("SP_NAME"),
        ...
        if (!myFlags.contains(FlagEnum.NO_MIGRATE_HASHES)) {
            ...
            spidxString.addTask(
                    new CalculateHashesTask(VersionEnum.V3_5_0, "20180903.28")
        ...
        if (!myFlags.contains(FlagEnum.NO_MIGRATE_HASHES)) {
            ...
            spidxToken
                    ...
                    .doNothing();
            spidxToken.addTask(
                    new CalculateHashesTask(VersionEnum.V3_5_0, "20180903.39")
                            .setColumnName("HASH_IDENTITY")
                            .addCalculator(
                                    "HASH_IDENTITY",
                                    t ->
        ...
        if (!myFlags.contains(FlagEnum.NO_MIGRATE_HASHES)) {
            ...
            spidxUri.addTask(
                    new CalculateHashesTask(VersionEnum.V3_5_0, "20180903.44")
                            .setColumnName("HASH_IDENTITY")
                            .addCalculator(
                                    "HASH_IDENTITY",
                                    t ->
        ...
        spp.addIndex("20180903.47", "IDX_RESPARMPRESENT_HASHPRES")
                .unique(false)
                .withColumns("HASH_PRESENCE");

        ArbitrarySqlTask consolidateSearchParamPresenceIndexesTask =
                new ArbitrarySqlTask(
                        VersionEnum.V3_5_0,
                        "20180903.48",
                        "HFJ_SEARCH_PARM",
                        "Consolidate search parameter presence indexes");
        ...
        String sql =
                "SELECT HFJ_SEARCH_PARM.RES_TYPE RES_TYPE, HFJ_SEARCH_PARM.PARAM_NAME PARAM_NAME,"
                    + " HFJ_RES_PARAM_PRESENT.PID PID, HFJ_RES_PARAM_PRESENT.SP_ID SP_ID,"
                    + " HFJ_RES_PARAM_PRESENT.SP_PRESENT SP_PRESENT,"
                    + " HFJ_RES_PARAM_PRESENT.HASH_PRESENCE HASH_PRESENCE from"
                    + " HFJ_RES_PARAM_PRESENT join HFJ_SEARCH_PARM ON (HFJ_SEARCH_PARM.PID ="
                    + " HFJ_RES_PARAM_PRESENT.SP_ID) where HFJ_RES_PARAM_PRESENT.HASH_PRESENCE is"
                    + " null";
        consolidateSearchParamPresenceIndexesTask.addExecuteOnlyIfColumnExists(
                "HFJ_RES_PARAM_PRESENT", "SP_ID");
        consolidateSearchParamPresenceIndexesTask.addQuery(
                sql,
                ArbitrarySqlTask.QueryModeEnum.BATCH_UNTIL_NO_MORE,
                t -> {
        ...
        version.addTableRawSql("20180907.1", "TRM_CONCEPT_DESIG")
                .addSql(
                        DriverTypeEnum.H2_EMBEDDED,
                        "create table TRM_CONCEPT_DESIG (PID bigint not null, LANG varchar(500),"
                                + " USE_CODE varchar(500), USE_DISPLAY varchar(500), USE_SYSTEM"
                                + " varchar(500), VAL varchar(500) not null, CS_VER_PID bigint,"
                                + " CONCEPT_PID bigint, primary key (PID))")
                .addSql(
                        DriverTypeEnum.H2_EMBEDDED,
                        "alter table TRM_CONCEPT_DESIG add constraint FK_CONCEPTDESIG_CSV foreign"
                                + " key (CS_VER_PID) references TRM_CODESYSTEM_VER")
                .addSql(
                        DriverTypeEnum.H2_EMBEDDED,
                        "alter table TRM_CONCEPT_DESIG add constraint FK_CONCEPTDESIG_CONCEPT"
                                + " foreign key (CONCEPT_PID) references TRM_CONCEPT")
                .addSql(
                        DriverTypeEnum.DERBY_EMBEDDED,
                        "create table TRM_CONCEPT_DESIG (PID bigint not null, LANG varchar(500),"
                                + " USE_CODE varchar(500), USE_DISPLAY varchar(500), USE_SYSTEM"
                ...
                .addSql(
                        ...
                        "create table TRM_CONCEPT_DESIG (PID bigint not null, LANG varchar(500),"
                                + " USE_CODE varchar(500), USE_DISPLAY varchar(500), USE_SYSTEM"
                                + " varchar(500), VAL varchar(500) not null, CS_VER_PID bigint,"
                                + " CONCEPT_PID bigint, primary key (PID)) ENGINE=InnoDB")
                .addSql(
                        DriverTypeEnum.MYSQL_5_7,
                        "alter table TRM_CONCEPT_DESIG add constraint FK_CONCEPTDESIG_CSV foreign"
                                + " key (CS_VER_PID) references TRM_CODESYSTEM_VER (PID)")
                .addSql(
                        DriverTypeEnum.MYSQL_5_7,
                        "alter table TRM_CONCEPT_DESIG add constraint FK_CONCEPTDESIG_CONCEPT"
                ...
                .addSql(
                        DriverTypeEnum.ORACLE_12C,
                        "create table TRM_CONCEPT_DESIG (PID number(19,0) not null, LANG"
                            + " varchar2(500 char), USE_CODE varchar2(500 char), USE_DISPLAY"
                            + " varchar2(500 char), USE_SYSTEM varchar2(500 char), VAL varchar2(500"
                            + " char) not null, CS_VER_PID number(19,0), CONCEPT_PID number(19,0),"
                            + " primary key (PID))")
                .addSql(
                        DriverTypeEnum.ORACLE_12C,
                        "alter table TRM_CONCEPT_DESIG add constraint FK_CONCEPTDESIG_CSV foreign"
                ...
                .addSql(
                        DriverTypeEnum.POSTGRES_9_4,
                        "create table TRM_CONCEPT_DESIG (PID int8 not null, LANG varchar(500),"
                                + " USE_CODE varchar(500), USE_DISPLAY varchar(500), USE_SYSTEM"
                                + " varchar(500), VAL varchar(500) not null, CS_VER_PID int8,"
                                + " CONCEPT_PID int8, primary key (PID))")
                .addSql(
                        DriverTypeEnum.POSTGRES_9_4,
                        "alter table TRM_CONCEPT_DESIG add constraint FK_CONCEPTDESIG_CSV foreign"
                                + " key (CS_VER_PID) references TRM_CODESYSTEM_VER")
        ...
        version.addTableRawSql("20180907.2", "TRM_CONCEPT_PROPERTY")
                .addSql(
                        DriverTypeEnum.DERBY_EMBEDDED,
                        "create table TRM_CONCEPT_PROPERTY (PID bigint not null, PROP_CODESYSTEM"
                            + " varchar(500), PROP_DISPLAY varchar(500), PROP_KEY varchar(500) not"
                            + " null, PROP_TYPE integer not null, PROP_VAL varchar(500), CS_VER_PID"
                            + " bigint, CONCEPT_PID bigint, primary key (PID))")
                .addSql(
                        DriverTypeEnum.DERBY_EMBEDDED,
                        "alter table TRM_CONCEPT_PROPERTY add constraint FK_CONCEPTPROP_CSV foreign"
                                + " key (CS_VER_PID) references TRM_CODESYSTEM_VER")
                .addSql(
                        DriverTypeEnum.DERBY_EMBEDDED,
                ...
                .addSql(
                        DriverTypeEnum.ORACLE_12C,
                        "create table TRM_CONCEPT_PROPERTY (PID number(19,0) not null,"
                            + " PROP_CODESYSTEM varchar2(500 char), PROP_DISPLAY varchar2(500"
                            + " char), PROP_KEY varchar2(500 char) not null, PROP_TYPE number(10,0)"
                            + " not null, PROP_VAL varchar2(500 char), CS_VER_PID number(19,0),"
                            + " CONCEPT_PID number(19,0), primary key (PID))")
                .addSql(
                        DriverTypeEnum.ORACLE_12C,
                        "alter table TRM_CONCEPT_PROPERTY add constraint FK_CONCEPTPROP_CSV foreign"
                                + " key (CS_VER_PID) references TRM_CODESYSTEM_VER")
                ...
                .addSql(
                        DriverTypeEnum.POSTGRES_9_4,
                        "create table TRM_CONCEPT_PROPERTY (PID int8 not null, PROP_CODESYSTEM"
                            + " varchar(500), PROP_DISPLAY varchar(500), PROP_KEY varchar(500) not"
                            + " null, PROP_TYPE int4 not null, PROP_VAL varchar(500), CS_VER_PID"
                            + " int8, CONCEPT_PID int8, primary key (PID))")
                .addSql(
                        DriverTypeEnum.POSTGRES_9_4,
                        "alter table TRM_CONCEPT_PROPERTY add constraint FK_CONCEPTPROP_CSV foreign"
                                + " key (CS_VER_PID) references TRM_CODESYSTEM_VER")
                ...
                .addSql(
                        ...
                        "create table TRM_CONCEPT_PROPERTY (PID bigint not null, PROP_CODESYSTEM"
                            + " varchar(500), PROP_DISPLAY varchar(500), PROP_KEY varchar(500) not"
                            + " null, PROP_TYPE int not null, PROP_VAL varchar(500), CS_VER_PID"
                            + " bigint, CONCEPT_PID bigint, primary key (PID))")
                .addSql(
                        DriverTypeEnum.MSSQL_2012,
                        "alter table TRM_CONCEPT_PROPERTY add constraint FK_CONCEPTPROP_CSV foreign"
        ...
        version.addTableRawSql("20180907.3", "TRM_CONCEPT_MAP")
                .addSql(
                        DriverTypeEnum.DERBY_EMBEDDED,
                        "create table TRM_CONCEPT_MAP (PID bigint not null, RES_ID bigint,"
                            + " SOURCE_URL varchar(200), TARGET_URL varchar(200), URL varchar(200)"
                            + " not null, primary key (PID))")
                .addSql(
                        DriverTypeEnum.DERBY_EMBEDDED,
                        "alter table TRM_CONCEPT_MAP add constraint FK_TRMCONCEPTMAP_RES foreign"
                                + " key (RES_ID) references HFJ_RESOURCE")
                .addSql(
                        DriverTypeEnum.MYSQL_5_7,
                        "create table TRM_CONCEPT_MAP (PID bigint not null, RES_ID bigint,"
                            + " SOURCE_URL varchar(200), TARGET_URL varchar(200), URL varchar(200)"
                ...
                .addSql(
                        DriverTypeEnum.MYSQL_5_7,
                        "alter table TRM_CONCEPT_MAP add constraint IDX_CONCEPT_MAP_URL unique"
                                + " (URL)")
                .addSql(
                        DriverTypeEnum.MYSQL_5_7,
                        "alter table TRM_CONCEPT_MAP add constraint FK_TRMCONCEPTMAP_RES foreign"
                                + " key (RES_ID) references HFJ_RESOURCE (RES_ID)")
                .addSql(
                        DriverTypeEnum.ORACLE_12C,
                        "create table TRM_CONCEPT_MAP (PID number(19,0) not null, RES_ID"
                            + " number(19,0), SOURCE_URL varchar2(200 char), TARGET_URL"
                            + " varchar2(200 char), URL varchar2(200 char) not null, primary key"
                            + " (PID))")
                .addSql(
                        DriverTypeEnum.ORACLE_12C,
                        "alter table TRM_CONCEPT_MAP add constraint IDX_CONCEPT_MAP_URL unique"
                ...
                .addSql(
                        DriverTypeEnum.POSTGRES_9_4,
                        "create table TRM_CONCEPT_MAP (PID int8 not null, RES_ID int8, SOURCE_URL"
                            + " varchar(200), TARGET_URL varchar(200), URL varchar(200) not null,"
                            + " primary key (PID))")
                .addSql(
                        DriverTypeEnum.POSTGRES_9_4,
                        "alter table TRM_CONCEPT_MAP add constraint FK_TRMCONCEPTMAP_RES foreign"
        ...
        version.addTableRawSql("20180907.4", "TRM_CONCEPT_MAP_GROUP")
                .addSql(
                        DriverTypeEnum.DERBY_EMBEDDED,
                        "create table TRM_CONCEPT_MAP_GROUP (PID bigint not null, myConceptMapUrl"
                            + " varchar(255), SOURCE_URL varchar(200) not null, mySourceValueSet"
                            + " varchar(255), SOURCE_VERSION varchar(100), TARGET_URL varchar(200)"
                            + " not null, myTargetValueSet varchar(255), TARGET_VERSION"
                            + " varchar(100), CONCEPT_MAP_PID bigint not null, primary key (PID))")
                .addSql(
                        DriverTypeEnum.DERBY_EMBEDDED,
                        "alter table TRM_CONCEPT_MAP_GROUP add constraint FK_TCMGROUP_CONCEPTMAP"
                                + " foreign key (CONCEPT_MAP_PID) references TRM_CONCEPT_MAP")
                .addSql(
                        DriverTypeEnum.DERBY_EMBEDDED,
                        "create unique index IDX_CONCEPT_MAP_URL on TRM_CONCEPT_MAP (URL)")
                .addSql(
                        DriverTypeEnum.ORACLE_12C,
                        "create table TRM_CONCEPT_MAP_GROUP (PID number(19,0) not null,"
                            + " myConceptMapUrl varchar2(255 char), SOURCE_URL varchar2(200 char)"
                            + " not null, mySourceValueSet varchar2(255 char), SOURCE_VERSION"
                            + " varchar2(100 char), TARGET_URL varchar2(200 char) not null,"
                            + " myTargetValueSet varchar2(255 char), TARGET_VERSION varchar2(100"
                            + " char), CONCEPT_MAP_PID number(19,0) not null, primary key (PID))")
                .addSql(
                        DriverTypeEnum.ORACLE_12C,
                        "alter table TRM_CONCEPT_MAP_GROUP add constraint FK_TCMGROUP_CONCEPTMAP"
                                + " foreign key (CONCEPT_MAP_PID) references TRM_CONCEPT_MAP")
                ...
                .addSql(
                        DriverTypeEnum.POSTGRES_9_4,
                        "create table TRM_CONCEPT_MAP_GROUP (PID int8 not null, myConceptMapUrl"
                            + " varchar(255), SOURCE_URL varchar(200) not null, mySourceValueSet"
                            + " varchar(255), SOURCE_VERSION varchar(100), TARGET_URL varchar(200)"
                            + " not null, myTargetValueSet varchar(255), TARGET_VERSION"
                            + " varchar(100), CONCEPT_MAP_PID int8 not null, primary key (PID))")
                .addSql(
                        DriverTypeEnum.POSTGRES_9_4,
                        "alter table TRM_CONCEPT_MAP_GROUP add constraint FK_TCMGROUP_CONCEPTMAP"
                                + " foreign key (CONCEPT_MAP_PID) references TRM_CONCEPT_MAP");
        ...
        version.addTableRawSql("20180907.5", "TRM_CONCEPT_MAP_GRP_ELEMENT")
                .addSql(
                        DriverTypeEnum.DERBY_EMBEDDED,
                        "create table TRM_CONCEPT_MAP_GRP_ELEMENT (PID bigint not null, SOURCE_CODE"
                            + " varchar(500) not null, myConceptMapUrl varchar(255), SOURCE_DISPLAY"
                            + " varchar(400), mySystem varchar(255), mySystemVersion varchar(255),"
                            + " myValueSet varchar(255), CONCEPT_MAP_GROUP_PID bigint not null,"
                            + " primary key (PID))")
                .addSql(
                        DriverTypeEnum.DERBY_EMBEDDED,
                        "alter table TRM_CONCEPT_MAP_GRP_ELEMENT add constraint"
                            + " FK_TCMGELEMENT_GROUP foreign key (CONCEPT_MAP_GROUP_PID) references"
                ...
                .addSql(
                        ...
                        "alter table TRM_CONCEPT_MAP_GRP_ELEMENT add constraint"
                            + " FK_TCMGELEMENT_GROUP foreign key (CONCEPT_MAP_GROUP_PID) references"
                            + " TRM_CONCEPT_MAP_GROUP (PID)")
                .addSql(
                        DriverTypeEnum.MARIADB_10_1,
                        "create index IDX_CNCPT_MAP_GRP_CD on TRM_CONCEPT_MAP_GRP_ELEMENT"
                                + " (SOURCE_CODE)")
                .addSql(
                        DriverTypeEnum.DERBY_EMBEDDED,
                        "create index IDX_CNCPT_MAP_GRP_CD on TRM_CONCEPT_MAP_GRP_ELEMENT"
                ...
                .addSql(
                        DriverTypeEnum.POSTGRES_9_4,
                        "create table TRM_CONCEPT_MAP_GRP_ELEMENT (PID int8 not null, SOURCE_CODE"
                            + " varchar(500) not null, myConceptMapUrl varchar(255), SOURCE_DISPLAY"
                            + " varchar(400), mySystem varchar(255), mySystemVersion varchar(255),"
                            + " myValueSet varchar(255), CONCEPT_MAP_GROUP_PID int8 not null,"
                            + " primary key (PID))")
                .addSql(
                        DriverTypeEnum.POSTGRES_9_4,
                        "alter table TRM_CONCEPT_MAP_GRP_ELEMENT add constraint"
                ...
                .addSql(
                        DriverTypeEnum.ORACLE_12C,
                        "create table TRM_CONCEPT_MAP_GRP_ELEMENT (PID number(19,0) not null,"
                            + " SOURCE_CODE varchar2(500 char) not null, myConceptMapUrl"
                            + " varchar2(255 char), SOURCE_DISPLAY varchar2(400 char), mySystem"
                            + " varchar2(255 char), mySystemVersion varchar2(255 char), myValueSet"
                            + " varchar2(255 char), CONCEPT_MAP_GROUP_PID number(19,0) not null,"
                            + " primary key (PID))")
                .addSql(
                        DriverTypeEnum.ORACLE_12C,
                        "alter table TRM_CONCEPT_MAP_GRP_ELEMENT add constraint"
        ...
        version.addTableRawSql("20180907.6", "TRM_CONCEPT_MAP_GRP_ELM_TGT")
                .addSql(
                        DriverTypeEnum.DERBY_EMBEDDED,
                        "create table TRM_CONCEPT_MAP_GRP_ELM_TGT (PID bigint not null, TARGET_CODE"
                            + " varchar(500) not null, myConceptMapUrl varchar(255), TARGET_DISPLAY"
                            + " varchar(400), TARGET_EQUIVALENCE varchar(50), mySystem"
                            + " varchar(255), mySystemVersion varchar(255), myValueSet"
                            + " varchar(255), CONCEPT_MAP_GRP_ELM_PID bigint not null, primary key"
                            + " (PID))")
                .addSql(
                        DriverTypeEnum.DERBY_EMBEDDED,
                        "alter table TRM_CONCEPT_MAP_GRP_ELM_TGT add constraint"
                                + " FK_TCMGETARGET_ELEMENT foreign key (CONCEPT_MAP_GRP_ELM_PID)"
                                + " references TRM_CONCEPT_MAP_GRP_ELEMENT")
                .addSql(
                        DriverTypeEnum.DERBY_EMBEDDED,
                        "create index IDX_CNCPT_MP_GRP_ELM_TGT_CD on TRM_CONCEPT_MAP_GRP_ELM_TGT"
                                + " (TARGET_CODE)")
                .addSql(
                        DriverTypeEnum.MARIADB_10_1,
                        "create table TRM_CONCEPT_MAP_GRP_ELM_TGT (PID bigint not null, TARGET_CODE"
                ...
                .addSql(
                        DriverTypeEnum.ORACLE_12C,
                        "create table TRM_CONCEPT_MAP_GRP_ELM_TGT (PID number(19,0) not null,"
                            + " TARGET_CODE varchar2(500 char) not null, myConceptMapUrl"
                            + " varchar2(255 char), TARGET_DISPLAY varchar2(400 char),"
                            + " TARGET_EQUIVALENCE varchar2(50 char), mySystem varchar2(255 char),"
                            + " mySystemVersion varchar2(255 char), myValueSet varchar2(255 char),"
                            + " CONCEPT_MAP_GRP_ELM_PID number(19,0) not null, primary key (PID))")
                .addSql(
                        DriverTypeEnum.ORACLE_12C,
                        "alter table TRM_CONCEPT_MAP_GRP_ELM_TGT add constraint"
                                + " FK_TCMGETARGET_ELEMENT foreign key (CONCEPT_MAP_GRP_ELM_PID)"
                ...
                .addSql(
                        DriverTypeEnum.POSTGRES_9_4,
                        "create table TRM_CONCEPT_MAP_GRP_ELM_TGT (PID int8 not null, TARGET_CODE"
                            + " varchar(500) not null, myConceptMapUrl varchar(255), TARGET_DISPLAY"
                            + " varchar(400), TARGET_EQUIVALENCE varchar(50), mySystem"
                            + " varchar(255), mySystemVersion varchar(255), myValueSet"
                            + " varchar(255), CONCEPT_MAP_GRP_ELM_PID int8 not null, primary key"
                            + " (PID))")
                .addSql(
                        DriverTypeEnum.POSTGRES_9_4,
                        "alter table TRM_CONCEPT_MAP_GRP_ELM_TGT add constraint"
