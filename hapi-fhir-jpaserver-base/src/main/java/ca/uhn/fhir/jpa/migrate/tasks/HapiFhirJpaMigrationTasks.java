...
public class HapiFhirJpaMigrationTasks extends BaseMigrationTasks<VersionEnum> {
	...
	protected void init670() {
		...
		final String enversRevisionTable = "ENVERS_REVISION";

		version.addIdGenerator("20230306.1", "SEQ_REVINFO");

		// TODO:  table envers_revision already exists on fresh install
		// TODO:  should they always be in caps?
...
//		enversRevInfo.addColumn("REVTSTMP")
		enversRevInfo.addColumn("timestamp")
			.nullable()
			.type(ColumnTypeEnum.LONG);

		final Builder.BuilderAddTableByColumns empiLink = version.addTableByColumns("20230306.6", "MPI_LINK_AUD", "PID", "REV");
		empiLink.addColumn("PID").nonNullable().type(ColumnTypeEnum.LONG);
		empiLink.addColumn("REV").nonNullable().type(ColumnTypeEnum.INT);
		// TODO:  do we really need a TINYINT here as is generated?   Should we do a database-specific ALTER TABLE here?
		...
		// TODO: better FK name
		// TODO  drop the old FK
		empiLink.addForeignKey("20230306.7", "FK_REV_TO_REVINFO")
			.toColumn(revColumnName)
			.references(enversRevisionTable, revColumnName);
	}

	protected void init670_v2() {
		final Builder version = forVersion(VersionEnum.V6_6_0);

		final String revColumnName = "REV";


		// 1. Create revinfo sequence
		// 2. Drop revinfo if exists, at least for H2
		// 3. create table revinfo with tasks/flyway framework
		// 4. DB-specific ALTER TABLE to use DEFAULT nextval('seq_revinfo') primary key
		// 5. create aud table with tasks/flyway framework

		version.addIdGenerator("20230306.1", "SEQ_REVINFO");

		final Map<DriverTypeEnum, String> h2DropTableIfExists = new HashMap<>();

		h2DropTableIfExists.put(DriverTypeEnum.H2_EMBEDDED, "DROP TABLE IF EXISTS REVINFO CASCADE");
		h2DropTableIfExists.put(DriverTypeEnum.POSTGRES_9_4, "DROP TABLE IF EXISTS REVINFO CASCADE");
		// TODO:
//		revInfoUseSequence.put(DriverTypeEnum.MARIADB_10_1, "");
//		// TODO:
//		revInfoUseSequence.put(DriverTypeEnum.MSSQL_2012, "");
//		// TODO:
//		revInfoUseSequence.put(DriverTypeEnum.MYSQL_5_7, "");
//		// TODO:
//		revInfoUseSequence.put(DriverTypeEnum.ORACLE_12C, "");

//		version.executeRawSql("20230306.2", h2DropTableIfExists);

//		final Builder.BuilderAddTableByColumns enversRevInfo = version.addTableByColumns("20230306.3", "REVINFO", revColumnName);
		final Builder.BuilderAddTableByColumns enversRevInfo = version.addTableByColumns("20230306.3", "ENVERS_REVISION", revColumnName);
...
//		final Map<DriverTypeEnum, String> dropHibernateSequence = new HashMap<>();
//		dropHibernateSequence.put(DriverTypeEnum.POSTGRES_9_4, "DROP SEQUENCE IF EXISTS hibernate_sequence");
//
//		version.executeRawSql("20230306.4", dropHibernateSequence);

		final Map<DriverTypeEnum, String> revInfoUseSequence = new HashMap<>();


		revInfoUseSequence.put(DriverTypeEnum.H2_EMBEDDED, "ALTER TABLE REVINFO ALTER COLUMN rev SET DEFAULT NEXTVAL('seq_revinfo')");
		revInfoUseSequence.put(DriverTypeEnum.POSTGRES_9_4, "ALTER TABLE REVINFO ALTER COLUMN rev SET DEFAULT NEXTVAL('seq_revinfo')");
		// TODO:
//		revInfoUseSequence.put(DriverTypeEnum.MARIADB_10_1, "");
//		// TODO:
//		revInfoUseSequence.put(DriverTypeEnum.MSSQL_2012, "");
...
//		// TODO:
//		revInfoUseSequence.put(DriverTypeEnum.ORACLE_12C, "");
//		version.executeRawSql("20230306.5", revInfoUseSequence);

//		revInfoUseSequence.put(DriverTypeEnum.H2_EMBEDDED, "ALTER TABLE revinfo ALTER COLUMN rev SET INTEGER NOT NULL DEFAULT nextval('seq_revinfo') primary key)");

//		revInfoUseSequence.put(DriverTypeEnum.H2_EMBEDDED, "CREATE TABLE IF NOT EXISTS REVINFO(REV INTEGER NOT NULL DEFAULT nextval('seq_revinfo') primary key, REVTSTMP BIGINT)");
		// TODO: this doesn't work because the table is already generated
//		revInfoUseSequence.put(DriverTypeEnum.H2_EMBEDDED, "CREATE TABLE REVINFO(REV INTEGER NOT NULL DEFAULT nextval('seq_revinfo') primary key, REVTSTMP BIGINT)");
...
//		revInfoUseSequence.put(DriverTypeEnum.POSTGRES_9_4, "CREATE TABLE IF NOT EXISTS revinfo (rev INTEGER NOT NULL PRIMARY KEY, revtstmp BIGINT)");

//		version.executeRawSql("20230306.1", revInfoUseSequence);
		...
		empiLink.addForeignKey("20230306.7", "FK_REV_TO_REVINFO")
			.toColumn(revColumnName)
			.references("REVINFO", revColumnName);
	}

	protected void init670_Prototype() {
		/*
		questions:

		1. which DB's to support for sequence?
		2. can we put off supporting the other DB's?
		 */

		/*
		Notes:

		* for a fresh install, this class takes precedence over hbmddl... this means we can do away with "hibernate_sequence"
		* hibernate envers hbmddl2 auto generation is all or nothing
		* it's not possible to define a sequence on a non-entity class... so adding a sequence on reninfo will effectively make it an orphan
		* there is no code to add auto-increment to a column
		* there is a maven plugin that generates a schema.sql file, but this is only possible per dialect

		options:

		1. Write code to set up a auto-increment column on all platforms (need to write code per platform)
		2. Write code to set up the sequence for a given column and do this per platform (need to write code per platform)
		3. use the maven plugin?
		4. explicitly limit the supported DBs for auto-increemnt to postgres and H2 for this branch?  add issues to support others later and make this experimental?
		5. write raw SQL for each DB product with conditional logic
		ex:
			Map<DriverTypeEnum, String> revInfoUseSequence = new HashMap<>();
			revInfoUseSequence.put(DriverTypeEnum.H2_EMBEDDED, "ALTER TABLE HFJ_RES_TAG ADD CONSTRAINT IDX_RESTAG_TAGID UNIQUE (RES_ID, TAG_ID)");
			revInfoUseSequence.put(DriverTypeEnum.MARIADB_10_1, "ALTER TABLE HFJ_RES_TAG ADD CONSTRAINT IDX_RESTAG_TAGID UNIQUE (RES_ID, TAG_ID)");
			revInfoUseSequence.put(DriverTypeEnum.MSSQL_2012, "ALTER TABLE HFJ_RES_TAG ADD CONSTRAINT IDX_RESTAG_TAGID UNIQUE (RES_ID, TAG_ID)");
			revInfoUseSequence.put(DriverTypeEnum.MYSQL_5_7, "ALTER TABLE HFJ_RES_TAG ADD CONSTRAINT IDX_RESTAG_TAGID UNIQUE (RES_ID, TAG_ID)");
			revInfoUseSequence.put(DriverTypeEnum.ORACLE_12C, "ALTER TABLE HFJ_RES_TAG ADD CONSTRAINT IDX_RESTAG_TAGID UNIQUE (RES_ID, TAG_ID)");
			revInfoUseSequence.put(DriverTypeEnum.POSTGRES_9_4, "ALTER TABLE HFJ_RES_TAG ADD CONSTRAINT IDX_RESTAG_TAGID UNIQUE (RES_ID, TAG_ID)");
			version.executeRawSql("20220429.5", revInfoUseSequence);
		...
		// TODO:  version on column?
//		final String versionToUseRevInfo = "20230306.1";
//		final String versionToUseMpiLinkAud = "20230306.2";


		// TODO: what if there are activemq/kafka events?

		final String revColumnName = "REV";

		final Map<DriverTypeEnum, String> revInfoUseSequence = new HashMap<>();
//		REV      INTEGER NOT NULL DEFAULT nextval('seq_revinfo')
		// TODO:  should we use autoincrement or sequence here?
		revInfoUseSequence.put(DriverTypeEnum.H2_EMBEDDED, "CREATE TABLE IF NOT EXISTS REVINFO(REV INTEGER NOT NULL DEFAULT nextval('seq_revinfo') primary key, REVTSTMP BIGINT)");
//		revInfoUseSequence.put(DriverTypeEnum.H2_EMBEDDED, "CREATE TABLE IF NOT EXISTS REVINFO(REV INTEGER NOT NULL auto_increment primary key, REVTSTMP BIGINT)");
	// TODO:
//		revInfoUseSequence.put(DriverTypeEnum.MARIADB_10_1, "");
//		// TODO:
//		revInfoUseSequence.put(DriverTypeEnum.MSSQL_2012, "");
...
//		revInfoUseSequence.put(DriverTypeEnum.ORACLE_12C, "");
		...
		version.executeRawSql("20230306.2", revInfoUseSequence);

		final Map<DriverTypeEnum, String> revInfoUseSequence2 = new HashMap<>();
		// TODO: comment about why we're doing this:  namely, that envers creates a hibernate_seqeunce that's used as the implicit autoincrement sequence
		revInfoUseSequence2.put(DriverTypeEnum.POSTGRES_9_4, "CREATE SEQUENCE hibernate_sequence START WITH 1 INCREMENT BY 1 NO MINVALUE NO MAXVALUE CACHE 1");

		version.executeRawSql("20230306.3", revInfoUseSequence2);

		// TODO:  choose between sequence and auto_increment
//		final Builder.BuilderAddTableByColumns enversRevInfo = version.addTableByColumns("20230306.1", "REVINFO", revColumnName);
//
//		enversRevInfo.addColumn(revColumnName)
//			.nonNullable()
//			.type(ColumnTypeEnum.INT);
...
//		ALTER TABLE REVINFO ALTER COLUMN rev SET DEFAULT seq_revinfo.nextval;

//		final Map<DriverTypeEnum, String> revInfoUseSequence = new HashMap<>();
////		REV      INTEGER NOT NULL DEFAULT nextval('seq_revinfo')
//		revInfoUseSequence.put(DriverTypeEnum.H2_EMBEDDED, "ALTER TABLE REVINFO ALTER COLUMN rev SET DEFAULT seq_revinfo.nextval");
//		// TODO:
//		revInfoUseSequence.put(DriverTypeEnum.MARIADB_10_1, "ALTER TABLE REVINFO");
//		// TODO:
//		revInfoUseSequence.put(DriverTypeEnum.MSSQL_2012, "ALTER TABLE REVINFO");
//		// TODO:
//		revInfoUseSequence.put(DriverTypeEnum.MYSQL_5_7, "ALTER TABLE REVINFO");
//		// TODO:
//		revInfoUseSequence.put(DriverTypeEnum.ORACLE_12C, "ALTER TABLE REVINFO");
//		revInfoUseSequence.put(DriverTypeEnum.POSTGRES_9_4, "ALTER TABLE REVINFO ALTER COLUMN rev SET DEFAULT NEXTVAL('seq_revinfo')");
//		version.executeRawSql("20230306.2", revInfoUseSequence);
//
//		// TODO:  this won't work because it depends on an entity class that already has a sequence defined on it
		version.addIdGenerator("20230306.4", "SEQ_REVINFO");

				// TODO:  how to do this for multiple DBs?
		/*
		H2:
...
alter table public.revinfo
    owner to cdr;
		...
		// TODO:  how to do this for MPI_LINK across databases?
		// TODO: copy init550()?
		/*
...
(
    ...
    primary key (PID, REV),
    constraint FKLSRUJ6UMNG8VLI4E00PTWHMFY
        foreign key (REV) references PUBLIC.REVINFO
);

Postgres:

create table if not exists public.mpi_link_aud
(
    pid                 bigint  not null,
    rev                 integer not null
        constraint fklsruj6umng8vli4e00ptwhmfy
            references public.revinfo
        constraint fk_rev_to_revinfo
            references public.revinfo,
    revtype             smallint,
    created             timestamp,
    eid_match           boolean,
...
alter table public.mpi_link_aud
    owner to cdr;


		 */

		// TODO:  reorder columns to match table in DB
		final Builder.BuilderAddTableByColumns empiLink = version.addTableByColumns("20230306.5", "MPI_LINK_AUD", "PID", "REV");
		empiLink.addColumn("PID").nonNullable().type(ColumnTypeEnum.LONG);
		empiLink.addColumn("REV").nonNullable().type(ColumnTypeEnum.INT);
		// TODO:  do we really need a TINYINT here as is generated?   Should we do a database-specific ALTER TABLE here?
		empiLink.addColumn("REVTYPE").nonNullable().type(ColumnTypeEnum.INT);
...
//		dateTable.dropForeignKey("20220207.12", "FK17S70OA59RM9N61K9THJQRSQM", "HFJ_RESOURCE");
		empiLink.addForeignKey("20230306.6", "FK_REV_TO_REVINFO").toColumn(revColumnName).references("REVINFO", revColumnName);
	}
