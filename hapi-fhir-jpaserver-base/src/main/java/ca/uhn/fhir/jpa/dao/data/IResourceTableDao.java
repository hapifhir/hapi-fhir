...
public interface IResourceTableDao
        extends JpaRepository<ResourceTable, Long>, IHapiFhirJpaRepository {
    ...
    @Query(
            "SELECT t.myId FROM ResourceTable t WHERE t.myResourceType = :restype AND t.myDeleted"
                    + " IS NOT NULL")
    Slice<Long> findIdsOfDeletedResourcesOfType(
            Pageable thePageable, @Param("restype") String theResourceName);

    @Query(
            "SELECT t.myId FROM ResourceTable t WHERE t.myId = :resid AND t.myResourceType ="
                    + " :restype AND t.myDeleted IS NOT NULL")
    Slice<Long> findIdsOfDeletedResourcesOfType(
    ...
    @Query(
            "SELECT t.myResourceType as type, COUNT(t.myResourceType) as count FROM ResourceTable t"
                    + " GROUP BY t.myResourceType")
    List<Map<?, ?>> getResourceCounts();

    @Query(
            "SELECT t.myId FROM ResourceTable t WHERE t.myUpdated >= :low AND t.myUpdated <= :high"
                    + " ORDER BY t.myUpdated DESC")
     ...
     */
    @Query(
            "SELECT t.myId, t.myResourceType, t.myUpdated FROM ResourceTable t WHERE t.myUpdated >="
                    + " :low AND t.myUpdated <= :high ORDER BY t.myUpdated ASC")
    ...
    @Query(
            "SELECT t.myId, t.myResourceType, t.myUpdated FROM ResourceTable t WHERE t.myUpdated >="
                    + " :low AND t.myUpdated <= :high AND t.myPartitionIdValue IN (:partition_ids)"
                    + " ORDER BY t.myUpdated ASC")
    Slice<Object[]>
    ...
    // TODO in the future, consider sorting by pid as well so batch jobs process in the same order
    // across restarts
    @Query(
            "SELECT t.myId FROM ResourceTable t WHERE t.myUpdated >= :low AND t.myUpdated <= :high"
                    + " AND t.myPartitionIdValue = :partition_id ORDER BY t.myUpdated ASC")
    ...
    @Query(
            "SELECT t.myId FROM ResourceTable t WHERE t.myUpdated >= :low AND t.myUpdated <= :high"
                    + " AND t.myResourceType = :restype ORDER BY t.myUpdated ASC")
    Slice<Long> findIdsOfResourcesWithinUpdatedRangeOrderedFromOldest(
            Pageable thePage,
     ...
     * returned by the query). Be careful if you change this query in any way.
     */
    @Query(
            "SELECT t.myResourceType, t.myId, t.myDeleted FROM ResourceTable t WHERE t.myId IN"
                    + " (:pid)")
    ...
    @Query(
            "SELECT t.myResourceType, t.myId, t.myDeleted FROM ResourceTable t WHERE t.myId IN"
                    + " (:pid) AND t.myPartitionIdValue IN :partition_id")
    ...
    @Query(
            "SELECT t.myResourceType, t.myId, t.myDeleted FROM ResourceTable t WHERE t.myId IN"
                    + " (:pid) AND (t.myPartitionIdValue IS NULL OR t.myPartitionIdValue IN"
                    + " :partition_id)")
    ...
    @Query(
            "SELECT t.myResourceType, t.myId, t.myDeleted FROM ResourceTable t WHERE t.myId IN"
                    + " (:pid) AND t.myPartitionIdValue IS NULL")
    Collection<Object[]> findLookupFieldsByResourcePidInPartitionNull(
            @Param("pid") List<Long> thePids);
    ...
    /**
     * This query will return rows with the following values: Id (resource pid - long), ResourceType
     * (Patient, etc), version (long) Order matters!
     *
     * @param pid - list of pids to get versions for
     * @return
     */
    @Query(
            "SELECT t.myId, t.myResourceType, t.myVersion FROM ResourceTable t WHERE t.myId IN ("
                    + " :pid )")
    Collection<Object[]> getResourceVersionsForPid(@Param("pid") List<Long> pid);

    @Query(
            "SELECT t FROM ResourceTable t LEFT JOIN FETCH t.myForcedId WHERE"
                    + " t.myPartitionId.myPartitionId IS NULL AND t.myId = :pid")
    Optional<ResourceTable> readByPartitionIdNull(@Param("pid") Long theResourceId);

    @Query(
            "SELECT t FROM ResourceTable t LEFT JOIN FETCH t.myForcedId WHERE"
                    + " t.myPartitionId.myPartitionId = :partitionId AND t.myId = :pid")
    Optional<ResourceTable> readByPartitionId(
            @Param("partitionId") int thePartitionId, @Param("pid") Long theResourceId);

    @Query(
            "SELECT t FROM ResourceTable t LEFT JOIN FETCH t.myForcedId WHERE"
                    + " (t.myPartitionId.myPartitionId IS NULL OR t.myPartitionId.myPartitionId IN"
                    + " (:partitionIds)) AND t.myId = :pid")
    ...
    @Query(
            "SELECT t FROM ResourceTable t LEFT JOIN FETCH t.myForcedId WHERE"
                    + " t.myPartitionId.myPartitionId IN (:partitionIds) AND t.myId = :pid")
