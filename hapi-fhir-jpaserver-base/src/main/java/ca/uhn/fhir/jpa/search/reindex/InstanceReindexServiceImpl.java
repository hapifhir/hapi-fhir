...
public class InstanceReindexServiceImpl implements IInstanceReindexService {
    ...
    @Autowired private BaseRequestPartitionHelperSvc myPartitionHelperSvc;
    ...
    @Nonnull
    private Parameters reindexInTransaction(
        ...
        BaseHapiFhirResourceDao.invokeStoragePreShowResources(
                myInterceptorService, theRequestDetails, resource);

        ResourceIndexedSearchParams existingParamsToPopulate =
                new ResourceIndexedSearchParams(entity);
        existingParamsToPopulate.mySearchParamPresentEntities.addAll(
                entity.getSearchParamPresents());
        ...
        ReindexOutcome outcome =
                dao.reindex(
                        pid, new ReindexParameters(), theRequestDetails, new TransactionDetails());
    ...
    private Parameters reindexDryRunInTransaction(
        ...
        ISearchParamExtractor.ISearchParamFilter searchParamFilter =
        ...
        if (theParameters == null) {
            ...
            fillInParamNames(
                    entity,
                    existingParamsToPopulate.mySearchParamPresentEntities,
                    theResourceId.getResourceType());
            showAction = true;
        } else {
        ...
        String message =
                "Reindex dry-run completed in "
                        + sw
                        + ". No changes were committed to any stored data.";
        return buildIndexResponse(
                existingParamsToPopulate, newParamsToPopulate, showAction, List.of(message));
    }
    ...
    private RequestPartitionId determinePartition(
        ...
        ReadPartitionIdRequestDetails details =
                ReadPartitionIdRequestDetails.forRead(theResourceId);
    ...
    Parameters buildIndexResponse(
        ...
        String narrativeText =
                myNarrativeGenerator.generateResourceNarrative(myContextR4, parameters);
    ...
    /**
     * The {@link SearchParamPresentEntity} entity doesn't actually store the parameter names in the
     * database entity, it only stores a hash. So we brute force possible hashes here to figure out
     * the associated param names.
     */
    private void fillInParamNames(
            ResourceTable theEntity,
            Collection<SearchParamPresentEntity> theTarget,
            String theResourceName) {
        Map<Long, String> hashes = new HashMap<>();
        ResourceSearchParams searchParams =
                mySearchParamRegistry.getActiveSearchParams(theResourceName);
        for (RuntimeSearchParam next : searchParams.values()) {
            hashes.put(
                    SearchParamPresentEntity.calculateHashPresence(
                            myPartitionSettings,
                            theEntity.getPartitionId(),
                            theResourceName,
                            next.getName(),
                            true),
                    next.getName());
    ...
    private abstract static class BaseParamPopulator<T> {
        ...
        public Parameters.ParametersParameterComponent addIndexValue(
            ...
            Parameters.ParametersParameterComponent retVal =
                    theParent.addPart().setName(toPartName(theParam));
        ...
        public Parameters.ParametersParameterComponent addIndexValue(
            ...
            if (theParam.getTargetResourceId() != null) {
                retVal.addPart()
                        ...
                        .setValue(
                                new StringType(
                                        theParam.getTargetResourceType()
                                                + "/"
                                                + theParam.getTargetResourceId()));
    ...
    /**
     * Links loaded from the database have a PID link to their target, but the ones extracted from
     * the resource in memory won't have the PID. So this method strips the PIDs so that the
     * generated hashCodes and equals comparisons will actually be equal.
     */
    private static List<ResourceLink> normalizeLinks(Collection<ResourceLink> theLinks) {
        return theLinks.stream()
    ...
    private static <T> void addParams(
        ...
        for (T next : addedParams) {
            Parameters.ParametersParameterComponent parent =
                    getOrCreateSection(theParameters, theSectionName);
            if (theShowAction) {
