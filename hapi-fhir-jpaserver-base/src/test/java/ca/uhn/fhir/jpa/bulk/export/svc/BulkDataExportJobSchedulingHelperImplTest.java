...
public class BulkDataExportJobSchedulingHelperImplTest {
	...
	@Mock private TransactionTemplate myTxTemplate;
	@Mock private IJobPersistence myJpaJobPersistence;
	@Mock private BulkExportHelperService myBulkExportHelperSvc;
	@Mock private DaoRegistry myDaoRegistry;
	@Mock private IFhirResourceDao<IBaseBinary> myBinaryDao;
	@Captor private ArgumentCaptor<Date> myCutoffCaptor;
	...
	public void purgeExpiredFilesNothingToDeleteOneHourRetention() {
		...
		assertEquals(
					DateUtils.truncate(computeDateFromConfig(expectedRetentionHours), Calendar.SECOND),
					DateUtils.truncate(cutoffDate, Calendar.SECOND));
	}
	...
	public void purgeExpiredFilesSingleJobSingleBinaryOneHourRetention_NULL_reportString() {
		...
		final List<JobInstance> jobInstances =
					getJobInstances(numBinariesPerJob, StatusEnum.COMPLETED);
		...
		for (JobInstance jobInstance : jobInstances) {
				...
				verify(myBulkExportHelperSvc, never()).toId(anyString());
				verify(myBinaryDao, never())
						.delete(any(IIdType.class), any(SystemRequestDetails.class));
		...
		assertThat(
					cutoffDate,
					within(1, ChronoUnit.MINUTES, computeDateFromConfig(expectedRetentionHours)));
	}
	...
	public void purgeExpiredFilesSingleJobSingleBinaryOneHourRetention() {
		...
		for (JobInstance jobInstance : jobInstances) {
				...
				for (int index = 0; index < numBinariesPerJob; index++) {
					verify(myBulkExportHelperSvc)
								.toId(jobInstance.getInstanceId() + "-binary-" + index);
					verify(myBinaryDao)
								.delete(
										eq(toId(jobInstance.getInstanceId() + "-binary-" + index)),
	...
	private void setupTest(
				boolean theIsEnabled,
				int theRetentionHours,
				List<JobInstance> theJobInstances,
				boolean theIsEnableBinaryMocks) {
		myBulkDataExportJobSchedulingHelper =
					new BulkDataExportJobSchedulingHelperImpl(
								myDaoRegistry,
								myTxManager,
								myStorageSettings,
								myBulkExportHelperSvc,
								myJpaJobPersistence,
								myTxTemplate);
		...
		final Answer<List<JobInstance>> fetchInstancesAnswer =
					theInvocationOnMock -> {
						final TransactionCallback<List<JobInstance>> transactionCallback =
									theInvocationOnMock.getArgument(0);
						//noinspection DataFlowIssue
						return transactionCallback.doInTransaction(null);
					};

		final Answer<Void> purgeExpiredJobsAnswer =
					theInvocationOnMock -> {
						final TransactionCallback<Optional<JobInstance>> transactionCallback =
									theInvocationOnMock.getArgument(0);
						//noinspection DataFlowIssue
		...
		when(myJpaJobPersistence.fetchInstances(
								eq(Batch2JobDefinitionConstants.BULK_EXPORT),
		...
		when(myTxTemplate.execute(any()))
					.thenAnswer(fetchInstancesAnswer)
		...
		OngoingStubbing<Optional<JobInstance>> when =
					when(myJpaJobPersistence.fetchInstance(anyString()));
	...
	@Nonnull
	private static List<JobInstance> getJobInstances(
				int theNumBinaries, StatusEnum... theStatusEnums) {
		return IntStream.range(0, theStatusEnums.length)
					...
					.map(
								pair -> {
									...
									jobInstance.setReport(
												serialize(getBulkExportJobResults(instanceId, theNumBinaries)));
	...
	@Nonnull
	private static BulkExportJobResults getBulkExportJobResults(
				String theInstanceId, int theNumBinaries) {
		final BulkExportJobResults bulkExportJobResults = new BulkExportJobResults();
		bulkExportJobResults.setResourceTypeToBinaryIds(
					Map.of(
								"Patient",
