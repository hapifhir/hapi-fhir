...
import static ca.uhn.fhir.rest.server.provider.ProviderConstants.OPERATION_MERGE_PARAM_TARGET_RESOURCE;
import static ca.uhn.fhir.rest.server.provider.ProviderConstants.OPERATION_MERGE_PARAM_TARGET_RESOURCE_IDENTIFIER;
...
class MergeOperationParametersUtilTest {
	...
	@Nested
	class BuildMergeOutputParametersTest {
		...
		void testBuildOutputParameters_withMergeOperationOutcome_allComponentsPresent() {
			...
			Parameters result = (Parameters) MergeOperationParametersUtil.buildMergeOperationOutputParameters(
					ourFhirContext, mergeOutcome, myInputParameters);

			// Assert
			assertThat(result).isNotNull();

			// Verify input parameter
			assertThat(ParametersUtil.getNamedParameters(ourFhirContext, result,
			...
			OperationOutcome outcome = (OperationOutcome) ParametersUtil.getNamedParameterResource(
					ourFhirContext, result, OPERATION_MERGE_OUTPUT_PARAM_OUTCOME)
					.orElseThrow();
	...
	@Nested
	class MergeInputParamsFromParametersTest {
		...
		void testFromParameters_withAllParameters_success() {
			...
			// Act
			MergeOperationInputParameters result = MergeOperationParametersUtil.inputParamsFromParameters(ourFhirContext, parameters, RESOURCE_LIMIT, myProvenanceAgents);
	...
	@Nested
	class UndoInputParamsFromParametersTest {

		private static final int RESOURCE_LIMIT = 1000;
		private final AbstractMergeOperationInputParameterNames myParameterNames = new GenericMergeOperationInputParameterNames();

		@Test
		void testFromParameters_withReferences_success() {
			// Arrange
			Parameters parameters = new Parameters();
			parameters.addParameter().setName(OPERATION_MERGE_PARAM_SOURCE_RESOURCE).setValue(new Reference("Patient/source-123"));
			parameters.addParameter().setName(OPERATION_MERGE_PARAM_TARGET_RESOURCE).setValue(new Reference("Patient/target-456"));

			// Act
			UndoMergeOperationInputParameters result = MergeOperationParametersUtil.undoInputParametersFromParameters(
				ourFhirContext, parameters, myParameterNames, RESOURCE_LIMIT);
			...
			assertThat(result.getTargetResource())
				.isNotNull()
				.returns("Patient/target-456", r -> r.getReferenceElement().getValue());
			assertThat(result.getSourceIdentifiers()).isNull();
			assertThat(result.getTargetIdentifiers()).isNull();
			assertThat(result.getResourceLimit()).isEqualTo(RESOURCE_LIMIT);
		}
		...
		void testFromParameters_withIdentifiers_success() {
			...
			sourceIdentifier.setSystem("http://example.com/mrn");
			sourceIdentifier.setValue("12345");
			parameters.addParameter().setName(OPERATION_MERGE_PARAM_SOURCE_RESOURCE_IDENTIFIER).setValue(sourceIdentifier);

			Identifier targetIdentifier = new Identifier();
			targetIdentifier.setSystem("http://example.com/mrn");
			targetIdentifier.setValue("67890");
			parameters.addParameter().setName(OPERATION_MERGE_PARAM_TARGET_RESOURCE_IDENTIFIER).setValue(targetIdentifier);
			...
			// Assert
			assertThat(result).isNotNull();
			assertThat(result.getSourceResource()).isNull();
			assertThat(result.getTargetResource()).isNull();
			assertThat(result.getSourceIdentifiers()).hasSize(1);
			CanonicalIdentifier sourceCanonical = result.getSourceIdentifiers().get(0);
			assertThat(sourceCanonical.getSystemElement().getValue()).isEqualTo("http://example.com/mrn");
			assertThat(sourceCanonical.getValueElement().getValue()).isEqualTo("12345");
		...
		void testFromParameters_withMultipleIdentifiers_success() {
			...
			Identifier sourceIdentifier1 = new Identifier();
			sourceIdentifier1.setSystem("http://example.com/mrn");
			sourceIdentifier1.setValue("source-mrn-12345");
			parameters.addParameter().setName(OPERATION_MERGE_PARAM_SOURCE_RESOURCE_IDENTIFIER).setValue(sourceIdentifier1);

			Identifier sourceIdentifier2 = new Identifier();
			sourceIdentifier2.setSystem("http://example.com/ssn");
			sourceIdentifier2.setValue("source-ssn-999");
			parameters.addParameter().setName(OPERATION_MERGE_PARAM_SOURCE_RESOURCE_IDENTIFIER).setValue(sourceIdentifier2);

			Identifier targetIdentifier1 = new Identifier();
			targetIdentifier1.setSystem("http://example.com/mrn");
			targetIdentifier1.setValue("target-mrn-67890");
			parameters.addParameter().setName(OPERATION_MERGE_PARAM_TARGET_RESOURCE_IDENTIFIER).setValue(targetIdentifier1);

			Identifier targetIdentifier2 = new Identifier();
			targetIdentifier2.setSystem("http://example.com/ssn");
			targetIdentifier2.setValue("target-ssn-888");
			parameters.addParameter().setName(OPERATION_MERGE_PARAM_TARGET_RESOURCE_IDENTIFIER).setValue(targetIdentifier2);
			...
			assertThat(result.getTargetResource()).isNull();
			assertThat(result.getSourceIdentifiers()).hasSize(2);
			CanonicalIdentifier sourceCanonical1 = result.getSourceIdentifiers().get(0);
			assertThat(sourceCanonical1.getSystemElement().getValue()).isEqualTo("http://example.com/mrn");
			assertThat(sourceCanonical1.getValueElement().getValue()).isEqualTo("source-mrn-12345");
			CanonicalIdentifier sourceCanonical2 = result.getSourceIdentifiers().get(1);
			assertThat(sourceCanonical2.getSystemElement().getValue()).isEqualTo("http://example.com/ssn");
			assertThat(sourceCanonical2.getValueElement().getValue()).isEqualTo("source-ssn-999");
			assertThat(result.getTargetIdentifiers()).hasSize(2);
			CanonicalIdentifier targetCanonical1 = result.getTargetIdentifiers().get(0);
			assertThat(targetCanonical1.getSystemElement().getValue()).isEqualTo("http://example.com/mrn");
			assertThat(targetCanonical1.getValueElement().getValue()).isEqualTo("target-mrn-67890");
			CanonicalIdentifier targetCanonical2 = result.getTargetIdentifiers().get(1);
			assertThat(targetCanonical2.getSystemElement().getValue()).isEqualTo("http://example.com/ssn");
			assertThat(targetCanonical2.getValueElement().getValue()).isEqualTo("target-ssn-888");
			assertThat(result.getResourceLimit()).isEqualTo(RESOURCE_LIMIT);
		}

		@Test
		void testFromParameters_withBothReferencesAndIdentifiers_success() {
			// Arrange
			Parameters parameters = new Parameters();
		...
		@Test
		void testFromParameters_withNoParameters_success() {
			// Arrange
			Parameters parameters = new Parameters();
