...
public class FhirResourceDaoR4SearchOptimizedTest extends BaseJpaR4Test {
	...
	public void testFetchCountWithMultipleIndexesOnOneResource() {
		...
		List<String> ids = toUnqualifiedVersionlessIdValues(results, 0, 10, true);
		assertThat(ids, empty());
		assertEquals(201, myDatabaseBackedPagingProvider.retrieveResultList(mySrd, uuid).size().intValue());
...
//		assertEquals(200, myDatabaseBackedPagingProvider.retrieveResultList(mySrd, uuid).size().intValue());
		...
		IBundleProvider search2 = myDatabaseBackedPagingProvider.retrieveResultList(mySrd, uuid);
	...
	public void testCountEvenIfPreviousSimilarSearchDidNotRequestIt() {
		...
		assertEquals("Patient/PT00000", ids.get(0));
		assertEquals("Patient/PT00009", ids.get(9));
		assertEquals(null, myDatabaseBackedPagingProvider.retrieveResultList(mySrd, uuid).size());
		...
		assertEquals("Patient/PT00000", ids.get(0));
		assertEquals("Patient/PT00009", ids.get(9));
		assertEquals(200, myDatabaseBackedPagingProvider.retrieveResultList(mySrd, uuid2).size().intValue());
	...
	public void testFetchRightUpToActualNumberExistingThenFetchAnotherPage() {
		...
		assertNull(myDatabaseBackedPagingProvider.retrieveResultList(mySrd, uuid).size());
	...
	public void testFetchOnlySmallBatches() {
		...
		assertEquals(190, myDatabaseBackedPagingProvider.retrieveResultList(mySrd, uuid).size().intValue());
	...
	public void testSearchThatOnlyReturnsASmallResult() {
		...
		assertEquals(1, myDatabaseBackedPagingProvider.retrieveResultList(mySrd, uuid).size().intValue());
