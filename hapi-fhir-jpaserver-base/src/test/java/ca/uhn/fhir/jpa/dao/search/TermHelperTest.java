...
class TermHelperTest {
	...
	void simpleQuerySyntax_mustBeLeftUnchanged() {
		Set<String> result =
				TermHelper.makePrefixSearchTerm(Set.of("(def | efg)", "(def efg)", "ghi +(\"abc\" \"def\")"));
	...
	void isToLeftUntouchedRemovesbackslashedStarAndHypenBeforeChecking() {
		...
		assertFalse(
				TermHelper.isToLeftUntouched("\\-ab*cd-ef"), "When all stars and hyphens are backslashed or internal");
		assertFalse(
