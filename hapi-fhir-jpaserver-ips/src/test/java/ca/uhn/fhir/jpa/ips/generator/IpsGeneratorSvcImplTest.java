...
public class IpsGeneratorSvcImplTest {
	...
	private static final List<Class<? extends IBaseResource>> RESOURCE_TYPES =
				Lists.newArrayList(
	...
	public void testGenerateIps() {
		...
		Bundle outcome =
					(Bundle)
								mySvc.generateIps(
										new SystemRequestDetails(), new TokenParam("http://foo", "bar"));
		...
		assertThat(
					section.getText().getDivAsString(),
	...
	@Test
	public void testMedicationSummary_MedicationStatementWithMedicationReference()
				throws IOException {
		// Setup Patient
		...
		when(medicationStatementDao.search(any(), any()))
					.thenReturn(
								new SimpleBundleProvider(
										Lists.newArrayList(medicationStatement, medication)));

		registerRemainingResourceDaos();

		// Test
		Bundle outcome =
					(Bundle) mySvc.generateIps(new SystemRequestDetails(), new IdType(PATIENT_ID));
		...
		Composition.SectionComponent section =
					findSection(compositions, IpsSectionEnum.MEDICATION_SUMMARY);
	...
	@Nonnull
	private Composition.SectionComponent findSection(
				Composition compositions, IpsSectionEnum sectionEnum) {
		Composition.SectionComponent section =
					compositions.getSection().stream()
								.filter(
										t ->
													t.getTitle()
																.equals(
	...
	public void testMedicationSummary_DuplicateSecondaryResources() {
		myStrategy.setSectionRegistry(
					new SectionRegistry().addGlobalCustomizer(t -> t.withNoInfoGenerator(null)));
		...
		Medication medication2 =
					createSecondaryMedication(
								MEDICATION_ID); // same ID again (could happen if we span multiple pages of
		// results)
	...
	*/
	...
	public void testMedicationSummary_ResourceAppearsAsSecondaryThenPrimary() throws IOException {
		...
		ResourceMetadataKeyEnum.ENTRY_SEARCH_MODE.put(
					medicationStatement2, BundleEntrySearchModeEnum.INCLUDE);
	...
	public void testMedicalDevices_DeviceUseStatementWithDevice() throws IOException {
		...
		ResourceMetadataKeyEnum.ENTRY_SEARCH_MODE.put(
					deviceUseStatement, BundleEntrySearchModeEnum.MATCH);
		...
		when(deviceUseStatementDao.search(any(), any()))
					.thenReturn(
								new SimpleBundleProvider(Lists.newArrayList(deviceUseStatement, device)));
		...
		Composition compositions = (Composition) outcome.getEntry().get(0).getResource();
		Composition.SectionComponent section =
					findSection(compositions, IpsSectionEnum.MEDICAL_DEVICES);
	...
	public void testImmunizations() throws IOException {
		...
		ResourceMetadataKeyEnum.ENTRY_SEARCH_MODE.put(
					immunization, BundleEntrySearchModeEnum.MATCH);

		IFhirResourceDao<Immunization> deviceUseStatementDao =
					registerResourceDaoWithNoData(Immunization.class);
		when(deviceUseStatementDao.search(any(), any()))
		...
		Composition compositions = (Composition) outcome.getEntry().get(0).getResource();
		Composition.SectionComponent section =
					findSection(compositions, IpsSectionEnum.IMMUNIZATIONS);
	...
	public void testReferencesUpdatedInSecondaryInclusions() {
		...
		ResourceMetadataKeyEnum.ENTRY_SEARCH_MODE.put(
					conditionActive, BundleEntrySearchModeEnum.MATCH);
		...
		ResourceMetadataKeyEnum.ENTRY_SEARCH_MODE.put(
					conditionResolved, BundleEntrySearchModeEnum.MATCH);
		...
		MedicationStatement addedMedicationStatement =
					findEntryResource(outcome, MedicationStatement.class, 0, 1);
		...
		Composition composition = (Composition) outcome.getEntry().get(0).getResource();
		Composition.SectionComponent problemListSection =
					findSection(composition, IpsSectionEnum.PROBLEM_LIST);
		...
		Composition.SectionComponent illnessHistorySection =
					findSection(composition, IpsSectionEnum.ILLNESS_HISTORY);
		assertEquals(
	...
	private IBundleProvider bundleProviderWithAllOfType(
				Bundle theSourceData, Class<? extends IBaseResource> theType) {
		List<Resource> resources =
					theSourceData.getEntry().stream()
								.filter(
										t ->
													t.getResource() != null
																&& theType.isAssignableFrom(
																		t.getResource().getClass()))
								.map(Bundle.BundleEntryComponent::getResource)
								.collect(Collectors.toList());
		resources.forEach(
					t ->
								ResourceMetadataKeyEnum.ENTRY_SEARCH_MODE.put(
										t, BundleEntrySearchModeEnum.MATCH));
		return new SimpleBundleProvider(resources);
	}

	@SuppressWarnings("unchecked")
	@Nonnull
	private <T extends IBaseResource> IFhirResourceDao<T> registerResourceDaoWithNoData(
				@Nonnull Class<T> theType) {
	...
	private void registerResourceDaosForSmallPatientSet() {
		Bundle sourceData =
					ClasspathUtil.loadCompressedResource(
								myFhirContext, Bundle.class, "/small-patient-everything.json.gz");

		for (var nextType : IpsGeneratorSvcImplTest.RESOURCE_TYPES) {
				IFhirResourceDao dao = registerResourceDaoWithNoData(nextType);
				when(dao.search(any(), any()))
						.thenReturn(bundleProviderWithAllOfType(sourceData, nextType));
		}
	}
	...
	private static Medication createSecondaryMedication(String medicationId) {
		...
		ResourceMetadataKeyEnum.ENTRY_SEARCH_MODE.put(
					medication, BundleEntrySearchModeEnum.INCLUDE);
		return medication;
	}
	...
	private static MedicationStatement createPrimaryMedicationStatement(
		...
		ResourceMetadataKeyEnum.ENTRY_SEARCH_MODE.put(
					medicationStatement, BundleEntrySearchModeEnum.MATCH);
