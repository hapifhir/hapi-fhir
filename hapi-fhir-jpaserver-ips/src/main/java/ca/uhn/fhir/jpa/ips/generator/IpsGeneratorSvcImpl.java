...
public class IpsGeneratorSvcImpl implements IIpsGeneratorSvc {
    ...
    public IBaseBundle generateIps(RequestDetails theRequestDetails, IIdType thePatientId) {
        IBaseResource patient =
                myDaoRegistry.getResourceDao("Patient").read(thePatientId, theRequestDetails);
    ...
    public IBaseBundle generateIps(
            RequestDetails theRequestDetails, TokenParam thePatientIdentifier) {
        ...
        ValidateUtil.isTrueOrThrowInvalidRequest(
                searchResults.sizeOrThrowNpe() > 0,
                "No Patient could be found matching given identifier");
        ValidateUtil.isTrueOrThrowInvalidRequest(
                searchResults.sizeOrThrowNpe() == 1,
    ...
    private IBaseBundle generateIpsForPatient(
            RequestDetails theRequestDetails, IBaseResource thePatient) {
        ...
        globalResourcesToInclude.addResourceIfNotAlreadyPresent(
                thePatient, originalSubjectId.getValue());
        ...
        determineInclusions(
                ...
                context,
                compositionBuilder,
                globalResourcesToInclude);

        IBaseResource composition = compositionBuilder.getComposition();

        // Create the narrative for the Composition itself
        CustomThymeleafNarrativeGenerator generator =
                newNarrativeGenerator(globalResourcesToInclude);
    ...
    private void determineInclusionsForSection(
        ...
        for (String nextResourceType : theSection.getResourceTypes()) {
            ...
            Set<Include> includes =
                    myGenerationStrategy.provideResourceSearchIncludes(ipsSectionContext);
            ...
            for (int startIndex = 0; ; startIndex += CHUNK_SIZE) {
                ...
                for (IBaseResource nextCandidate : resources) {
                    ...
                    if (ResourceMetadataKeyEnum.ENTRY_SEARCH_MODE.get(nextCandidate)
                        ...
                        include =
                                myGenerationStrategy.shouldInclude(
                                        ipsSectionContext, nextCandidate);
                    }
                    ...
                    if (include) {
                        ...
                        IBaseResource previouslyExistingResource =
                                theGlobalResourcesToInclude.getResourceByOriginalId(
                                        originalResourceId);
                        ...
                        } else if (theGlobalResourcesToInclude.hasResourceWithReplacementId(
                                originalResourceId)) {
                            sectionResourcesToInclude.addResourceIfNotAlreadyPresent(
                                    nextCandidate, originalResourceId);
                        } else {
                            IIdType id =
                                    myGenerationStrategy.massageResourceId(
                                            theIpsContext, nextCandidate);
        ...
        if (sectionResourcesToInclude.isEmpty() && theSection.getNoInfoGenerator() != null) {
            IBaseResource noInfoResource =
                    theSection.getNoInfoGenerator().generate(theIpsContext.getSubjectId());
            String id = IdType.newRandomUuid().getValue();
            if (noInfoResource.getIdElement().isEmpty()) {
                noInfoResource.setId(id);
            }
            ResourceMetadataKeyEnum.ENTRY_SEARCH_MODE.put(
                    noInfoResource, BundleEntrySearchModeEnum.MATCH);
            theGlobalResourcesToInclude.addResourceIfNotAlreadyPresent(
                    noInfoResource,
        ...
        for (IBaseResource nextResource : sectionResourcesToInclude.getResources()) {
            List<ResourceReferenceInfo> references =
                    myFhirContext.newTerser().getAllResourceReferences(nextResource);
            for (ResourceReferenceInfo nextReference : references) {
                String existingReference =
                        nextReference.getResourceReference().getReferenceElement().getValue();
                if (isNotBlank(existingReference)) {
                    existingReference =
                            new IdType(existingReference).toUnqualifiedVersionless().getValue();
                    String replacement =
                            theGlobalResourcesToInclude.getIdSubstitution(existingReference);
                    ...
                    } else if (theGlobalResourcesToInclude.getResourceById(existingReference)
                            == null) {
    ...
    private void addSection(
        ...
        for (IBaseResource next : theResourcesToInclude.getResources()) {
            if (ResourceMetadataKeyEnum.ENTRY_SEARCH_MODE.get(next)
                    == BundleEntrySearchModeEnum.INCLUDE) {
                continue;
            }
        ...
        String narrative =
                createSectionNarrative(
                        theSection, theResourcesToInclude, theGlobalResourcesToInclude);
        sectionBuilder.setText("generated", narrative);
    }

    private CompositionBuilder createComposition(
            IBaseResource thePatient, IpsContext context, IBaseResource author) {
    ...
    private String determinePatientCompartmentSearchParameterName(String theResourceType) {
        ...
        Set<String> searchParams =
                resourceDef.getSearchParamsForCompartmentName("Patient").stream()
    ...
    private String createSectionNarrative(
        ...
        CustomThymeleafNarrativeGenerator generator =
                newNarrativeGenerator(theGlobalResourceCollection);

        Bundle bundle = new Bundle();
        for (IBaseResource resource : theResources.getResources()) {
            BundleEntrySearchModeEnum searchMode =
                    ResourceMetadataKeyEnum.ENTRY_SEARCH_MODE.get(resource);
    ...
    private CustomThymeleafNarrativeGenerator newNarrativeGenerator(
        ...
        CustomThymeleafNarrativeGenerator generator =
                new CustomThymeleafNarrativeGenerator(narrativePropertyFiles);
        generator.setFhirPathEvaluationContext(
                new IFhirPathEvaluationContext() {
                    @Override
                    public IBase resolveReference(
                            @Nonnull IIdType theReference, @Nullable IBase theContext) {
                        IBaseResource resource =
                                theGlobalResourceCollection.getResourceById(theReference);
                        return resource;
                    }
                });
        return generator;
    ...
    */
    ...
    private static class ResourceInclusionCollection {
         ...
         */
        public void addResourceIfNotAlreadyPresent(
                IBaseResource theResource, String theOriginalResourceId) {
            assert theOriginalResourceId.matches(
                            "([A-Z][a-z]([A-Za-z]+)/[a-zA-Z0-9._-]+)|(urn:uuid:[0-9a-z-]+)")
