...
public class IpsContext {
	...
	* @param theSubjectId The original ID for {@literal theSubject}, which may not match the
	*     current ID if {@link IIpsGenerationStrategy#massageResourceId(IpsContext, IBaseResource)}
	*     has modified it
	*/
	...
	/**
	* Returns the subject Patient resource for the IPS being generated. Note that the {@literal
	* Resource.id} value may not match the ID of the resource stored in the repository if {@link
	* IIpsGenerationStrategy#massageResourceId(IpsContext, IBaseResource)} has returned a different
	* ID. Use {@link #getSubjectId()} if you want the originally stored ID.
	*
	...
	*/
	...
	/**
	* Returns the ID of the subject for the given IPS. This value should match the ID which was
	* originally fetched from the repository.
	*/
