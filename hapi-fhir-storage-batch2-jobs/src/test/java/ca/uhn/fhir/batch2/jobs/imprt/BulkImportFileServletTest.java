...
public class BulkImportFileServletTest {
    ...
    static final String ourInput =
            "{\"resourceType\":\"Patient\", \"id\": \"A\", \"active\": true}\n"
                    + "{\"resourceType\":\"Patient\", \"id\": \"B\", \"active\": false}";

    @RegisterExtension
    private HttpServletExtension myServletExtension =
            new HttpServletExtension()
                    .withServlet(mySvc)
    ...
    private void executeBulkImportAndCheckReturnedContentType(String theUrl) throws IOException {
        ...
        try (CloseableHttpResponse response = client.execute(new HttpGet(theUrl))) {
            ...
            assertEquals(
                    BulkImportFileServlet.DEFAULT_HEADER_CONTENT_TYPE, responseHeaderContentType);
