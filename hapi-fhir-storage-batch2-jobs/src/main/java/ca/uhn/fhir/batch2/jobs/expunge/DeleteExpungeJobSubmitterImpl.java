...
public class DeleteExpungeJobSubmitterImpl implements IDeleteExpungeJobSubmitter {
    @Autowired IJobCoordinator myJobCoordinator;
    @Autowired FhirContext myFhirContext;
    @Autowired MatchUrlService myMatchUrlService;
    @Autowired IRequestPartitionHelperSvc myRequestPartitionHelperSvc;
    @Autowired JpaStorageSettings myStorageSettings;
    @Autowired IInterceptorBroadcaster myInterceptorBroadcaster;
    @Autowired UrlPartitioner myUrlPartitioner;
    ...
    public String submitJob(
        ...
        if (!myStorageSettings.canDeleteExpunge()) {
            throw new ForbiddenOperationException(
                    Msg.code(820)
                            + "Delete Expunge not allowed:  "
                            + myStorageSettings.cannotDeleteExpungeReason());
        }
        ...
        for (String url : theUrlsToDeleteExpunge) {
            ...
            CompositeInterceptorBroadcaster.doCallHooks(
                    ...
                    theRequestDetails,
                    Pointcut.STORAGE_PRE_DELETE_EXPUNGE,
                    params);
        }
        ...
        ReadPartitionIdRequestDetails details =
                ReadPartitionIdRequestDetails.forOperation(
                        null, null, ProviderConstants.OPERATION_DELETE_EXPUNGE);
