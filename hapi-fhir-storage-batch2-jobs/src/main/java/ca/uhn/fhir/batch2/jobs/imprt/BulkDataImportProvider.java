...
public class BulkDataImportProvider {
    ...
    public static final String PARAM_STORAGE_DETAIL_MAX_BATCH_RESOURCE_COUNT =
            "maxBatchResourceCount";
    ...
    public void setRequestPartitionHelperService(
     ...
     *
     * <p>Note that this is a weird operation, defined here:
     * https://github.com/smart-on-fhir/bulk-import/blob/master/import-manifest.md Per the
     * definition in that spec, this operation isn't actually a FHIR operation using a Parameters
     * resource, but instead uses an arbitrary JSON document as its payload.
     *
     * <p>We are using a Parameters resource here, so we're not actually technically compliant with
     * the spec as it is written (2022-02-15). However the text indicates that there has been a
     * suggestion to use Parameters so this feels like the right direction.
     */
    ...
    public void importByManifest(
        ...
        String inputFormat =
                ParametersUtil.getNamedParameterValueAsString(
                                myFhirCtx, theRequest, PARAM_INPUT_FORMAT)
                        .orElse("");
        ...
        if (storageDetailOpt.isPresent()) {
            ...
            String maximumBatchResourceCount =
                    ParametersUtil.getParameterPartValueAsString(
                            myFhirCtx,
                            storageDetail,
                            PARAM_STORAGE_DETAIL_MAX_BATCH_RESOURCE_COUNT);
        ...
        for (IBase input :
                ParametersUtil.getNamedParameters(
                        myFhirCtx, theRequest, BulkDataImportProvider.PARAM_INPUT)) {
            String type =
                    ParametersUtil.getParameterPartValueAsString(
        ...
        ourLog.info(
                "Requesting Bulk Import Job ($import by Manifest) with {} urls",
                typeAndUrls.size());

        Batch2JobStartResponse jobStartResponse =
        ...
        OperationOutcomeUtil.addIssue(
                ...
                response,
                "information",
                "Use the following URL to poll for job status: "
                        + createPollLocationLink(theRequestDetails, jobId),
                null,
                "informational");
        ...
        myFhirCtx
                .newJsonParser()
                .setPrettyPrint(true)
                .encodeResourceToWriter(response, theResponse.getWriter());
        theResponse.getWriter().close();
    }

    /** $import-poll-status */
    @Operation(
            name = JpaConstants.OPERATION_IMPORT_POLL_STATUS,
            manualResponse = true,
            idempotent = true)
    public void importPollStatus(
            @OperationParam(
                            name = JpaConstants.PARAM_IMPORT_POLL_STATUS_JOB_ID,
                            typeName = "string",
                            min = 0,
                            max = 1)
            ...
            case QUEUED:
                {
                    ...
                    String msg =
                            "Job was created at "
                                    + renderTime(instance.getCreateTime())
                                    + " and is in "
                                    + instance.getStatus()
                                    + " state.";
            ...
            case IN_PROGRESS:
                {
                    ...
                    String msg =
                            "Job was created at "
                                    + renderTime(instance.getCreateTime())
                                    + ", started at "
                                    + renderTime(instance.getStartTime())
                                    + " and is in "
                                    + instance.getStatus()
                                    + " state. Current completion: "
                                    + new DecimalFormat("0.0")
                                            .format(100.0 * instance.getProgress())
                                    + "% and ETA is "
    ...
    private void streamOperationOutcomeResponse(
            HttpServletResponse response, String theMessage, String theSeverity)
            throws IOException {
    ...
    public void writePollingLocationToResponseHeaders(
            ServletRequestDetails theRequestDetails, String theJobId) {
    ...
    private String createPollLocationLink(
        ...
        return serverBase
                ...
                + "?"
                + JpaConstants.PARAM_IMPORT_POLL_STATUS_JOB_ID
                + "="
                + theJobId;
    }
