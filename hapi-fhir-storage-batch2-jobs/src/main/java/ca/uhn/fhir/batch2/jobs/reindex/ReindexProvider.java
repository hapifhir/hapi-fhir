...
public class ReindexProvider {
    ...
    public IBaseParameters reindex(
            @Description(
                            "Optionally provides one ore more relative search parameter URLs (e.g."
                                + " \"Patient?active=true\" or \"Observation?\") that will be"
                                + " reindexed. Note that the URL applies to the resources as they"
                                + " are currently indexed, so you should not use a search parameter"
                                + " that needs reindexing in the URL or some resources may be"
                                + " missed. If no URLs are provided, all resources of all types"
                                + " will be reindexed.")
                    @OperationParam(
            ...
            @Description(
                            "Should search parameters be reindexed (default: "
                                    + ReindexParameters.REINDEX_SEARCH_PARAMETERS_DEFAULT_STRING
                                    + ")")
                    @OperationParam(
                            name = REINDEX_SEARCH_PARAMETERS,
                            typeName = "code",
                            min = 0,
                            max = 1)
                    IPrimitiveType<String> theReindexSearchParameters,
            @Description(
                            "Should we attempt to optimize storage for resources (default: "
            ...
            @Description(
                            "Should we attempt to optimistically lock resources being reindexed in"
                                    + " order to avoid concurrency issues (default: "
                                    + ReindexParameters.OPTIMISTIC_LOCK_DEFAULT
                                    + ")")
                    @OperationParam(
                            name = ReindexJobParameters.OPTIMISTIC_LOCK,
                            typeName = "boolean",
                            min = 0,
                            max = 1)
            ...
            if (value != null) {
                ...
                ValidateUtil.isTrueOrThrowInvalidRequest(
                        EnumUtils.isValidEnum(
                                ReindexParameters.ReindexSearchParametersEnum.class, value),
                        "Invalid "
                                + REINDEX_SEARCH_PARAMETERS
                                + " value: "
                                + UrlUtil.sanitizeUrlPart(theReindexSearchParameters.getValue()));
                params.setReindexSearchParameters(
                        ReindexParameters.ReindexSearchParametersEnum.valueOf(value));
            }
        }
        if (theOptimizeStorage != null) {
            ...
            if (value != null) {
                ...
                ValidateUtil.isTrueOrThrowInvalidRequest(
                        EnumUtils.isValidEnum(
                                ReindexParameters.OptimizeStorageModeEnum.class, value),
                        "Invalid "
                                + OPTIMIZE_STORAGE
                                + " value: "
        ...
        ReadPartitionIdRequestDetails details =
                ReadPartitionIdRequestDetails.forOperation(
                        null, null, ProviderConstants.OPERATION_REINDEX);
        ...
        ParametersUtil.addParameterToParametersString(
                myFhirContext,
                retVal,
                ProviderConstants.OPERATION_BATCH_RESPONSE_JOB_ID,
