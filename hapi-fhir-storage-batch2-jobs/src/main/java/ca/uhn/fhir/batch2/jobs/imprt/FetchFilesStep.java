...
public class FetchFilesStep
	...
	private static final List<String> ourValidContentTypes =
				Arrays.asList(
	...
	public RunOutcome run(
				@Nonnull
						StepExecutionDetails<BulkImportJobParameters, VoidModel>
									theStepExecutionDetails,
				@Nonnull IJobDataSink<NdJsonFileJson> theDataSink) {

		Integer maxBatchResourceCount =
					theStepExecutionDetails.getParameters().getMaxBatchResourceCount();
				...
				for (String nextUrl : urls) {
					...
					try (CloseableHttpResponse response = httpClient.execute(new HttpGet(nextUrl))) {
						...
						Validate.isTrue(
									...
									"Received content type \"%s\" from URL: %s. This format is not one of"
												+ " the supported content type: %s",
									contentType,
									nextUrl,
									getContentTypesString());
						if (hasMatchingSubstring(contentType, ourValidNonNdJsonContentTypes)) {
								ourLog.info(
										"Received non-NDJSON content type \"{}\" from URL: {}. It will be"
												+ " processed but it may not complete correctly if the actual"
												+ " data is not NDJSON.",
										contentType,
										nextUrl);
						}
								...
								try (LineIterator lineIterator =
									...
									while (lineIterator.hasNext()) {
										...
										if (lineCount >= maxBatchResourceCount
												...
												ourLog.info(
														"Loaded chunk {} of {} NDJSON file with {} resources"
																	+ " from URL: {}",
														chunkCount,
	...
	private CloseableHttpClient newHttpClient(
		...
		String httpBasicCredentials =
					theStepExecutionDetails.getParameters().getHttpBasicCredentials();
		if (isNotBlank(httpBasicCredentials)) {
				int colonIdx = httpBasicCredentials.indexOf(':');
				if (colonIdx == -1) {
					throw new JobExecutionFailedException(
								Msg.code(2055)
										+ "Invalid credential parameter provided. Must be in the form"
										+ " \"username:password\".");
				}
