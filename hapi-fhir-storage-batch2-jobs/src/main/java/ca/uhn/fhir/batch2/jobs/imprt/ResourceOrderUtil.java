...
public class ResourceOrderUtil {
    ...
    /**
     * This method attempts to order all resource type names so that if one resource has references
     * to another, the latter comes before the former.
     *
     * <p>So for example, "Patient" would come before "Observation" in the list because Observation
     * refers to Patient but not vice versa.
     *
     * <p>This implementation is a pretty naive bubble sort, and could probably be more efficient so
     * don't call it repeatedly.
     *
     * <p>Note that the results can't cover every reference, since some are circular (e.g.
     * MedicationStatement:part-of -> Observation:part-of -> MedicationStatement:part-of) So for
     * those cases the order is arbitrary.
     *
     * @since 6.0.0
     */
    public static List<String> getResourceOrder(FhirContext theFhirContext) {
        ...
        for (; passCount < retVal.size(); passCount++) {
            ...
            for (int i = retVal.size() - 1; i >= 0; i--) {
                ...
                RuntimeResourceDefinition typeDefinition =
                        theFhirContext.getResourceDefinition(typeAtCurrentIndex);
                List<RuntimeSearchParam> params =
                        typeDefinition.getSearchParams().stream()
                                .filter(
                                        t ->
                                                t.getParamType()
                ...
                for (RuntimeSearchParam nextParam : params) {
                    for (String targetType : nextParam.getTargets()) {
                        ...
                        if (targetIndex > i) {
                            ...
                            ourLog.debug(
                                    "Resource[{}] at index[{}] has SP[{}] with target[{}] at"
                                            + " index[{}] - moving to index[{}]",
                                    typeAtCurrentIndex,
                                    i,
                                    nextParamName,
                                    targetType,
    ...
    //							}
    //
    //							ourLog.debug("Resource[{}] at index[{}] has SP[{}] with target[{}] at index[{}] -
    // moving to index[{}]", typeAtCurrentIndex, i, nextParamName, targetType, targetIndex, i);
