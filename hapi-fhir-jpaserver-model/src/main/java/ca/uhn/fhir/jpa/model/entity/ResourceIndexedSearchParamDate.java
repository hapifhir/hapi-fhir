...
@Table(
        ...
        indexes = {
            ...
            @Index(
                    name = "IDX_SP_DATE_HASH_HIGH_V2",
                    columnList = "HASH_IDENTITY,SP_VALUE_HIGH,RES_ID,PARTITION_ID"),
            @Index(
...
public class ResourceIndexedSearchParamDate extends BaseResourceIndexedSearchParam {
    ...
    /**
     * Field which stores an integer representation of YYYYMDD as calculated by Calendar e.g.
     * 2019-01-20 -> 20190120
     */
    ...
    @Transient private transient String myOriginalValue;
    ...
    /**
     * Composite of resourceType, paramName, and partition info if configured. Combined with the
     * various date fields for a query.
     *
    ...
    private void reComputeValueHighDate(Date theHigh, String theHighString) {
        ...
        if (theHighString.length() == 4
                || theHighString.length() == 7
                || theHighString.length() == 10) {
    ...
    private boolean matchesOrdinalDateBounds(DateRangeParam range) {
        ...
        if (lowerBoundAsDateInteger != null) {
            ...
            result &=
                    (myValueLowDateOrdinal.equals(lowerBoundAsDateInteger)
                            || myValueLowDateOrdinal > lowerBoundAsDateInteger);
            result &=
                    (myValueHighDateOrdinal.equals(lowerBoundAsDateInteger)
                            || myValueHighDateOrdinal > lowerBoundAsDateInteger);
        }
        if (upperBoundAsDateInteger != null) {
            result &=
                    (myValueHighDateOrdinal.equals(upperBoundAsDateInteger)
                            || myValueHighDateOrdinal < upperBoundAsDateInteger);
            result &=
                    (myValueLowDateOrdinal.equals(upperBoundAsDateInteger)
