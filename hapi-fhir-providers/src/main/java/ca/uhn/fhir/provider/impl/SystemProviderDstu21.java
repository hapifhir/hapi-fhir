...
public class SystemProviderDstu21 extends BaseSystemProvider<Bundle, MetaDt> {
	...
	public ISearchDao getSearchDao() {
		synchronized(this) {
			if (null == mySearchDao) {
				mySearchDao = getDaoFactory().getSearchDao();
			}
		}
		return mySearchDao;
	}
	...
	public Parameters suggestKeywords(
		...
		List<ISuggestion> keywords = getSearchDao().suggestKeywords(theContext, theSearchParam, theText);
		
		Parameters retVal = new Parameters();
		for (ISuggestion next : keywords) {
			//@formatter:off
			retVal.addParameter()
