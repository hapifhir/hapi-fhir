...
public class TransactionParameter implements IParameter {
	...
	private String createParameterTypeError(Method theMethod) {
		return "Method '"
					...
					+ " but is not of type Bundle, IBaseResource, IBaseBundle, or List<"
	...
	public void initializeTypes(
		...
		if (Modifier.isInterface(theParameterType.getModifiers()) == false
		...
		} else if (theParameterType.equals(IBaseResource.class)
					|| theParameterType.equals(IBaseBundle.class)) {
				myParamStyle = ParamStyle.RESOURCE_BUNDLE;
				...
				if (theInnerCollectionType.equals(List.class) == false) {
					throw new ConfigurationException(
								Msg.code(430) + createParameterTypeError(theMethod));
				}
				if (theParameterType.equals(IResource.class) == false) {
					throw new ConfigurationException(
								Msg.code(431) + createParameterTypeError(theMethod));
				}
	...
	public Object translateQueryParametersIntoServerArgument(
		...
		IBaseResource parsedBundle =
					ResourceParameter.parseResourceFromRequest(
								theRequest, theMethodBinding, myResourceBundleType);
