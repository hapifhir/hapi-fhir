...
public class SearchParameter extends BaseQueryParameter {
    ...
    static {
        ...
        ourParamQualifiers.put(
                ...
                CollectionUtil.newSet(
                        Constants.PARAMQUALIFIER_TOKEN_TEXT,
                        Constants.PARAMQUALIFIER_MISSING,
                        EMPTY_STRING));
        ...
        ourParamQualifiers.put(
                ...
                CollectionUtil.newSet(Constants.PARAMQUALIFIER_MISSING));
    ...
    public void setType(
        ...
        if (IQueryParameterType.class.isAssignableFrom(theType)) {
            myParamBinder =
                    new QueryParameterTypeBinder(
                            (Class<? extends IQueryParameterType>) theType, myCompositeTypes);
        } else if (IQueryParameterOr.class.isAssignableFrom(theType)) {
            myParamBinder =
                    new QueryParameterOrBinder(
                            (Class<? extends IQueryParameterOr<?>>) theType, myCompositeTypes);
        } else if (IQueryParameterAnd.class.isAssignableFrom(theType)) {
            myParamBinder =
                    new QueryParameterAndBinder(
                            (Class<? extends IQueryParameterAnd<?>>) theType, myCompositeTypes);
            ...
            myParamType = RestSearchParameterTypeEnum.DATE;
        } else if (IPrimitiveType.class.isAssignableFrom(theType)
                && ReflectionUtil.isInstantiable(theType)) {
            RuntimePrimitiveDatatypeDefinition def =
                    (RuntimePrimitiveDatatypeDefinition)
                            theContext.getElementDefinition(
                                    (Class<? extends IPrimitiveType<?>>) theType);
            ...
            throw new ConfigurationException(
                    Msg.code(354)
                            + "Unsupported data type for parameter: "
                            + theType.getCanonicalName());
        }
        ...
        if (myParamType != null) {
            ...
            throw new ConfigurationException(
                    Msg.code(355) + "Unknown search parameter type: " + theType);
        }
