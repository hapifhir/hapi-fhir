...
public class ValidationResultEnrichingInterceptor {
	...
	public static final String REQUEST_VALIDATION_RESULT =
			ValidationResultEnrichingInterceptor.class.getName() + "_REQUEST_VALIDATION_RESULT";

	@SuppressWarnings("unchecked")
	@Hook(Pointcut.SERVER_OUTGOING_RESPONSE)
	public boolean addValidationResultsToOperationOutcome(
		...
		if (theResponseObject instanceof IBaseOperationOutcome) {
			...
			if (theRequestDetails != null) {
				List<ValidationResult> validationResult =
						(List<ValidationResult>) theRequestDetails.getUserData().remove(REQUEST_VALIDATION_RESULT);
	...
	@SuppressWarnings("unchecked")
	public static void addValidationResultToRequestDetails(
			@Nullable RequestDetails theRequestDetails, @Nonnull ValidationResult theValidationResult) {
		if (theRequestDetails != null) {
			List<ValidationResult> results = (List<ValidationResult>)
					theRequestDetails.getUserData().computeIfAbsent(REQUEST_VALIDATION_RESULT, t -> new ArrayList<>(2));
