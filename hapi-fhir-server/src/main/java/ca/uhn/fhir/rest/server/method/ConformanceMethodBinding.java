...
public class ConformanceMethodBinding extends BaseResourceReturningMethodBinding {
	...
	*/
	...
	/**
	* Returns the number of milliseconds to cache the generated CapabilityStatement for. Default is
	* one minute, and can be set to 0 to never cache.
	*
	...
	*/
		...
		if (cacheControlDirective.isNoCache()) conf = null;
				...
				if (conf != null) {
					...
					if (expires < System.currentTimeMillis()) {
						...
						myThreadPool.submit(
									() -> createCapabilityStatement(theRequest, theMethodParams));
					}
				}
		}
		if (conf != null) {
				...
				if (operationType != null) {
					...
					if (theRequest.getInterceptorBroadcaster() != null) {
						...
						preHandledParams.add(
									RestOperationTypeEnum.class, theRequest.getRestOperationType());
						...
						theRequest
									...
									.callHooks(
												Pointcut.SERVER_INCOMING_REQUEST_PRE_HANDLED, preHandledParams);
					}
				}
		}
	...
	private IBaseConformance createCapabilityStatement(
				RequestDetails theRequest, Object[] theMethodParams) {
		...
		if (theRequest.getInterceptorBroadcaster() != null) {
				...
				IBaseConformance outcome =
						(IBaseConformance)
									theRequest
												.getInterceptorBroadcaster()
												.callHooksAndReturnObject(
														Pointcut.SERVER_CAPABILITY_STATEMENT_GENERATED, params);
				if (outcome != null) {
					conf = outcome;
				}
		}
	...
	public IBaseConformance provideCapabilityStatement(
				RestfulServer theServer, RequestDetails theRequest) {
