    ...
    public NoiseCharacters addRange(String theRange) {
        if (!theRange.contains("-")) {
            throw new IllegalArgumentException(
                    Msg.code(350) + String.format("Invalid range %s", theRange));
        }

        String[] range = theRange.split("-");
        if (range.length < 2) {
            throw new IllegalArgumentException(
                    Msg.code(351) + String.format("Invalid range %s", theRange));
        }
    ...
    public NoiseCharacters addRange(String theLowerBound, String theUpperBound) {
        ...
        if (lower > upper) {
            throw new IllegalArgumentException(
                    Msg.code(352)
                            + String.format(
                                    "Invalid character range %s-%s", theLowerBound, theUpperBound));
        }
