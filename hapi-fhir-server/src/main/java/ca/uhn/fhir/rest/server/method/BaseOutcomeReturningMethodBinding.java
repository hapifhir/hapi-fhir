...
abstract class BaseOutcomeReturningMethodBinding extends BaseMethodBinding {
    static final org.slf4j.Logger ourLog =
            org.slf4j.LoggerFactory.getLogger(BaseOutcomeReturningMethodBinding.class);
            ...
            if (!allowVoidReturnType()) {
                throw new ConfigurationException(
                        Msg.code(367)
                                ...
                                + " is a @"
                                + theMethodAnnotation.getSimpleName()
    ...
    protected abstract void addParametersForServerRequest(
            RequestDetails theRequest, Object[] theParams);

    /**
     * Subclasses may override to allow a void method return type, which is allowable for some
     * methods (e.g. delete)
     */
    ...
    /**
     * For servers, this method will match only incoming requests that match the given operation, or
     * which have no operation in the URL if this method returns null.
     */
        ...
        switch (getRestOperationType()) {
            case CREATE:
                ...
                if (response == null
                        ...
                        || Boolean.TRUE.equals(response.getCreated())) {
    ...
    public Object invokeServer(IRestfulServer<?> theServer, RequestDetails theRequest)
        ...
        if (response != null && response.getId() != null && response.getId().hasResourceType()) {
            if (getContext().getResourceDefinition(response.getId().getResourceType()) == null) {
                throw new InternalErrorException(
                        Msg.code(369)
                                + "Server method returned invalid resource ID: "
    ...
    private Object returnResponse(
        ...
        if (!BaseResourceReturningMethodBinding.callOutgoingResponseHook(
        ...
        if (theMethodOutcome != null) {
            if (theMethodOutcome.getResource() != null) {
                operationResourceLastUpdated =
                        RestfulServerUtils.extractLastUpdatedFromResource(
                                theMethodOutcome.getResource());
            }

            responseId = theMethodOutcome.getId();
            if (responseId != null
                    && responseId.getResourceType() == null
                    && responseId.hasIdPart()) {
