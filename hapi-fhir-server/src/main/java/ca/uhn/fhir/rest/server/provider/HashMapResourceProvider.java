...
/**
 * This class is a simple implementation of the resource provider interface that uses a HashMap to
 * store all resources in memory.
 *
 * <p>This class currently supports the following FHIR operations:
 *
 * <ul>
 *   <li>Create
 *   <li>Update existing resource
 *   <li>Update non-existing resource (e.g. create with client-supplied ID)
 *   <li>Delete
 *   <li>Search by resource type with no parameters
 * </ul>
 *
...
public class HashMapResourceProvider<T extends IBaseResource> implements IResourceProvider {
    ...
    @Create
    public synchronized MethodOutcome create(
            @ResourceParam T theResource, RequestDetails theRequestDetails) {
    ...
    @Delete
    public synchronized MethodOutcome delete(
    ...
    @History
    public synchronized List<IBaseResource> historyInstance(
            @IdParam IIdType theId, RequestDetails theRequestDetails) {
    ...
    public synchronized IBundleProvider searchAll(RequestDetails theRequestDetails) {
        ...
        return new SimpleBundleProvider(allResources) {
            ...
            public List<IBaseResource> getResources(int theFromIndex, int theToIndex) {
                ...
                // Make sure that "from" isn't less than 0, "to" isn't more than the number
                // available,
                // and "from" <= "to"
    ...
    private IIdType store(
        ...
        if (theDeleted) {
            ...
            ResourceMetadataKeyEnum.ENTRY_TRANSACTION_METHOD.put(
                    theResource, BundleEntryTransactionMethodEnum.DELETE);
        } else {
            ResourceMetadataKeyEnum.DELETED_AT.put(theResource, null);
            if (theVersionIdPart > 1) {
                ResourceMetadataKeyEnum.ENTRY_TRANSACTION_METHOD.put(
                        theResource, BundleEntryTransactionMethodEnum.PUT);
            } else {
                ResourceMetadataKeyEnum.ENTRY_TRANSACTION_METHOD.put(
        ...
        if (theRequestDetails != null && theRequestDetails.getInterceptorBroadcaster() != null) {
            ...
            if (theDeleted) {
                ...
                interceptorBroadcaster.callHooks(
                        Pointcut.STORAGE_PRESTORAGE_RESOURCE_DELETED, preStorageParams);
                ...
                interceptorBroadcaster.callHooks(
                        Pointcut.STORAGE_PRECOMMIT_RESOURCE_DELETED, preCommitParams);
                ...
                HookParams preStorageParams =
                        new HookParams()
                                ...
                                .add(
                                        ...
                                        null) // we should add this if we want - but this is test
                                // usage
                                .add(TransactionDetails.class, theTransactionDetails);
                interceptorBroadcaster.callHooks(
                        Pointcut.STORAGE_PRESTORAGE_RESOURCE_CREATED, preStorageParams);
                        ...
                        new HookParams()
                                ...
                                .add(
                                        ...
                                        theTransactionDetails.getInvocationTiming(
                                                Pointcut.STORAGE_PRECOMMIT_RESOURCE_CREATED));
                interceptorBroadcaster.callHooks(
                        Pointcut.STORAGE_PRECOMMIT_RESOURCE_CREATED, preCommitParams);
    ...
    /**
     * This is a utility method that can be used to store a resource without having to use the
     * outside API. In this case, the storage happens without any interaction with interceptors,
     * etc.
     *
    ...
    /**
     * Returns an unmodifiable list containing the current version of all resources stored in this
     * provider
     *
     * @since 4.1.0
     */
    ...
    private static <T extends IBaseResource> T fireInterceptorsAndFilterAsNeeded(
        ...
        List<IBaseResource> output =
                fireInterceptorsAndFilterAsNeeded(
                        Lists.newArrayList(theResource), theRequestDetails);
        if (output.size() == 1) {
            return theResource;
        } else {
            return null;
    ...
    protected static <T extends IBaseResource>
            List<IBaseResource> fireInterceptorsAndFilterAsNeeded(
        ...
        if (theRequestDetails != null) {
            ...
            SimplePreResourceShowDetails preResourceShowDetails =
                    new SimplePreResourceShowDetails(resourcesToReturn);
