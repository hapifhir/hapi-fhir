...
public class FhirQueryRuleTester implements IAuthRuleTester {
    ...
    private boolean checkMatch(RuleTestRequest theRuleTestRequest) {
        // look for a matcher
        IAuthorizationSearchParamMatcher matcher =
                theRuleTestRequest.ruleApplier.getSearchParamMatcher();
        if (matcher == null) {
            theRuleTestRequest
                    .ruleApplier
        ...
        // this is a bit weird.
        // A tester narrows a rule -- i.e. a rule only applies if the main logic matches AND the
        // testers all match
        ...
        // we use the target type since the rule might apply to all types, a type set, or instances,
        // and that has already been checked.
        IAuthorizationSearchParamMatcher.MatchResult mr =
                matcher.match(
                        theRuleTestRequest.resource.fhirType() + "?" + myQueryParameters,
                        theRuleTestRequest.resource);
        ...
        switch (mr.match) {
            ...
            case UNSUPPORTED:
                theRuleTestRequest
                        ...
                        .getTroubleshootingLog()
                        .warn(
                                "Unsupported matcher expression {}: {}.",
                                myQueryParameters,
                                mr.unsupportedReason);
