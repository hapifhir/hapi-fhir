...
public class StandardizingInterceptor {
	...
	public void resourcePreUpdate(
		...
		ourLog.debug(
					"Standardizing on pre-update for - {}, {}, {}",
					theRequest,
					theOldResource,
					theNewResource);
	...
	private void standardize(RequestDetails theRequest, IBaseResource theResource) {
		...
		if (!theRequest.getHeaders(STANDARDIZATION_DISABLED_HEADER).isEmpty()) {
				ourLog.debug(
						"Standardization for {} is disabled via header {}",
						theResource,
						STANDARDIZATION_DISABLED_HEADER);
				return;
		}
	...
	private void standardize(
				IBaseResource theResource, Map<String, String> theRules, IFhirPath theFhirPath) {
		for (Map.Entry<String, String> rule : theRules.entrySet()) {
				...
				for (IBase v : values) {
					if (!(v instanceof IPrimitiveType)) {
						ourLog.warn(
									"Value at path {} is of type {}, which is not of primitive type -"
												+ " skipping",
									rule.getKey(),
									v.fhirType());
						continue;
	...
	private IStandardizer getStandardizer(Map.Entry<String, String> rule) {
		...
		try {
				standardizer =
						(IStandardizer)
									Class.forName(standardizerName).getDeclaredConstructor().newInstance();
		} catch (Exception e) {
				throw new RuntimeException(
						Msg.code(349)
									+ String.format("Unable to create standardizer %s", standardizerName),
						e);
		}
