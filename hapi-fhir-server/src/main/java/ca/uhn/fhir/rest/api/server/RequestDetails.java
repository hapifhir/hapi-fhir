...
public abstract class RequestDetails {
    ...
    /**
     * Returns the request resource (as provided in the request body) if it has been parsed. Note
     * that this value is only set fairly late in the processing pipeline, so it may not always be
     * set, even for operations that take a resource as input.
     *
     * @since 4.0.0
     */
    ...
    /**
     * Sets the request resource (as provided in the request body) if it has been parsed. Note that
     * this value is only set fairly late in the processing pipeline, so it may not always be set,
     * even for operations that take a resource as input.
     *
     * @since 4.0.0
     */
    ...
    /**
     * Returns the <b>conditional URL</b> if this request has one, or <code>null</code> otherwise.
     * For an update or delete method, this is the part of the URL after the <code>?</code>. For a
     * create, this is the value of the <code>If-None-Exist</code> header.
     *
     * @param theOperationType The operation type to find the conditional URL for
     * @return Returns the <b>conditional URL</b> if this request has one, or <code>null</code>
     *     otherwise
     */
    ...
    /**
     * Returns the attribute map for this request. Attributes are a place for user-supplied objects
     * of any type to be attached to an individual request. They can be used to pass information
    ...
    /**
     * Retrieves the body of the request as binary data. Either this method or {@link #getReader}
    ...
    /**
     * Returns an invoker that can be called from user code to advise the server interceptors of any
     * nested operations being invoked within operations. This invoker acts as a proxy for all
     * interceptors
     */
    ...
    /**
     * Gets the tenant ID associated with the request. Note that the tenant ID and the partition ID
     * are not the same thing - Depending on the specific partition interceptors in use, the tenant
     * ID might be used internally to derive the partition ID or it might not. Do not assume that it
    ...
    /**
     * Sets the tenant ID associated with the request. Note that the tenant ID and the partition ID
     * are not the same thing - Depending on the specific partition interceptors in use, the tenant
     * ID might be used internally to derive the partition ID or it might not. Do not assume that it
     * will be used for this purpose.
     */
            ...
            for (String next : myParameters.keySet()) {
                for (int i = 0; i < next.length(); i++) {
                    ...
                    if (nextChar == ':' || nextChar == '.') {
                        ...
                        List<String> list =
                                myUnqualifiedToQualifiedNames.computeIfAbsent(
                                        unqualified, k -> new ArrayList<>(4));
                        list.add(next);
                        break;
                    }
                }
    ...
    /**
     * Returns a map which can be used to hold any user specific data to pass it from one part of
     * the request handling chain to another. Data in this map can use any key, although user code
     * should try to use keys which are specific enough to avoid conflicts.
     *
     * <p>A new map is created for each individual request that is handled by the server, so this
     * map can be used (for example) to pass authorization details from an interceptor to the
     * resource providers, or for example to pass data from a hook method on the {@link
     * ca.uhn.fhir.interceptor.api.Pointcut#SERVER_INCOMING_REQUEST_POST_PROCESSED} to a later hook
    ...
    /**
     * Is this request a sub-request (i.e. a request within a batch or transaction)? This flag is
     * used internally by hapi-fhir-jpaserver-base, but not used in the plain server library. You
     * may use it in your client code as a hint when implementing transaction logic in the plain
     * server.
     *
    ...
    /**
     * This method may be used to modify the contents of the incoming request by hardcoding a value
     * which will be used instead of the value received by the client.
     *
     * <p>This method is useful for modifying the request body prior to parsing within interceptors.
     * It generally only has an impact when called in the {@link
     * IServerInterceptor#incomingRequestPostProcessed(RequestDetails, HttpServletRequest,
