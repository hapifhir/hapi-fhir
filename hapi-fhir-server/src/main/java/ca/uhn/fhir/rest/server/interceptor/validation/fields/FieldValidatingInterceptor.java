...
public class FieldValidatingInterceptor {
    ...
    public void resourcePreCreate(RequestDetails theRequest, IBaseResource theResource) {
        ourLog.debug(
                "Validating address on create for resource {} / request {}",
                theResource,
                theRequest);
        ...
        for (Map.Entry<String, String> e : myConfig.entrySet()) {
            ...
            for (IBase field : fields) {
                ...
                List<IPrimitiveType> values =
                        fhirPath.evaluate(field, FHIR_PATH_VALUE, IPrimitiveType.class);
    ...
    private IValidator getValidator(String theValue) {
        ...
        try {
            ...
            throw new IllegalStateException(
                    Msg.code(348) + String.format("Unable to create validator for %s", theValue),
                    e);
        }
    }
