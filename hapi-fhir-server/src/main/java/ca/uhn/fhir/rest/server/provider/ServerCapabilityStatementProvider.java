 ...
 *
 * <p>This class is version independent, but will only work on servers supporting FHIR R4+ (as this
 * was the first FHIR release where CapabilityStatement was a normative resource)
 */
public class ServerCapabilityStatementProvider
    ...
    private static final Logger ourLog =
            LoggerFactory.getLogger(ServerCapabilityStatementProvider.class);
    ...
    public IBaseConformance getServerConformance(
        ...
        IBaseConformance retVal =
                (IBaseConformance)
                        myContext.getResourceDefinition("CapabilityStatement").newInstance();

        FhirTerser terser = myContext.newTerser();

        TreeMultimap<String, String> resourceTypeToSupportedProfiles =
                getSupportedProfileMultimap(terser);
        ...
        ServletContext servletContext =
                (ServletContext)
                        (theRequest == null
                                ? null
                                : theRequest.getAttribute(RestfulServer.SERVLET_CONTEXT_ATTRIBUTE));
        String serverBase =
                configuration
        ...
        terser.addElement(
                retVal, "implementation.description", configuration.getImplementationDescription());
        ...
        Map<String, List<BaseMethodBinding>> resourceToMethods =
                configuration.collectMethodBindings();
        ...
        for (Entry<String, List<BaseMethodBinding>> nextEntry : resourceToMethods.entrySet()) {
            ...
            if (nextEntry.getKey().isEmpty() == false) {
                ...
                if (resourceNameToSharedSupertype.containsKey(resourceName)) {
                    def =
                            context.getResourceDefinition(
                                    resourceNameToSharedSupertype.get(resourceName));
                } else {
                ...
                if (mySearchParamRegistry != null) {
                    ...
                    searchParams =
                            mySearchParamRegistry.getActiveSearchParams(resourceName).makeCopy();
                ...
                if (myRestResourceRevIncludesEnabled) {
                    NavigableSet<String> resourceRevIncludes =
                            resourceNameToRevIncludes.get(resourceName);
        ...
        if (theSearchMethodBinding.getQueryName() != null) {
            ...
            if (theOperationNamesAlreadyAdded.add(queryName)) {
                ...
                theTerser.addElement(
                        operation,
                        "definition",
                        (createOperationUrl(theRequestDetails, queryName)));
            }
        }
    }
    ...
    private void populateOperation(
        ...
        if (isNotBlank(operationCanonicalUrl)) {
            ...
            theTerser.addElement(
                    theOperation, "definition", createOperationUrl(theRequestDetails, theOpName));
        }
    ...
    private TreeMultimap<String, String> getSupportedProfileMultimap(FhirTerser terser) {
        ...
        if (myValidationSupport != null) {
            List<IBaseResource> allStructureDefinitions =
                    myValidationSupport.fetchAllNonBaseStructureDefinitions();
            if (allStructureDefinitions != null) {
                for (IBaseResource next : allStructureDefinitions) {
                    ...
                    String baseDefinition =
                            defaultString(
                                    terser.getSinglePrimitiveValueOrNull(next, "baseDefinition"));
                    if ("resource".equals(kind) && isNotBlank(url)) {

                        // Don't include the base resource definitions in the supported profile list
                        // - This isn't helpful
                        if (baseDefinition.equals(
                                        "http://hl7.org/fhir/StructureDefinition/DomainResource")
                                || baseDefinition.equals(
                                        "http://hl7.org/fhir/StructureDefinition/Resource")) {
                            continue;
                        }

                        String resourceType =
                                terser.getSinglePrimitiveValueOrNull(next, "snapshot.element.path");
                        if (isBlank(resourceType)) {
                            resourceType =
                                    terser.getSinglePrimitiveValueOrNull(
                                            next, "differential.element.path");
                        }
    ...
    public IBaseResource readOperationDefinition(
        ...
        if (operationBindings != null && !operationBindings.isEmpty()) {
            return readOperationDefinitionForOperation(
                    theRequestDetails, bindings, operationBindings);
        }
    ...
    private IBaseResource readOperationDefinitionForNamedSearch(
            List<SearchMethodBinding> bindings) {
            ...
            for (IParameter nextParamUntyped : operationMethodBinding.getParameters()) {
                if (nextParamUntyped instanceof OperationParameter) {
                    ...
                    if (isNotBlank(nextParam.getDescription())
                        ...
                        terser.addElement(
                                existingParam, "documentation", nextParam.getDescription());
                    }

                    if (nextParam.getParamType() != null) {
                        String existingType =
                                terser.getSinglePrimitiveValueOrNull(existingParam, "type");
                    ...
                    terser.setElement(
                            ...
                            (nextParam.getMax() == -1
                                    ? "*"
                                    : Integer.toString(nextParam.getMax())));
                    terser.setElement(param, "name", nextParam.getName());

                    List<IBaseExtension<?, ?>> existingExampleExtensions =
                            ExtensionUtil.getExtensionsByUrl(
                                    (IBaseHasExtensions) param,
                                    HapiExtensions.EXT_OP_PARAMETER_EXAMPLE_VALUE);
                    Set<String> existingExamples =
