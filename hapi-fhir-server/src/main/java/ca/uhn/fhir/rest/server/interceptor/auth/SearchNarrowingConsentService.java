...
public class SearchNarrowingConsentService implements IConsentService {
	private static final Logger ourLog =
				LoggerFactory.getLogger(SearchNarrowingConsentService.class);
	...
	*/
	public SearchNarrowingConsentService(
				IValidationSupport theValidationSupport, FhirContext theFhirContext) {
	...
	private ConsentOutcome applyFilterForResource(
		...
		for (AllowedCodeInValueSet next : postFilteringList) {
				...
				SearchParameterAndValueSetRuleImpl.CodeMatchCount outcome =
						SearchParameterAndValueSetRuleImpl
				...
				if (outcome.isAtLeastOneUnableToValidate()) {
					myTroubleshootingLog.warn(
								"Terminology Services failed to validate value from "
										+ next.getResourceName()
										+ ":"
										+ next.getSearchParameterName()
										+ " in ValueSet "
										+ next.getValueSetUrl()
										+ " - Assuming REJECT");
