...
public class RestfulServerConfiguration implements ISearchParamRegistry {
     ...
     */
    public RestfulServerConfiguration setResourceBindings(
     ...
     */
    public RestfulServerConfiguration setImplementationDescription(
     ...
     * Typically this would be populated with an InstanceType.
     */
    ...
    public Bindings provideBindings() {
        IdentityHashMap<SearchMethodBinding, String> namedSearchMethodBindingToName =
                new IdentityHashMap<>();
        HashMap<String, List<SearchMethodBinding>> searchNameToBindings = new HashMap<>();
        IdentityHashMap<OperationMethodBinding, String> operationBindingToId =
                new IdentityHashMap<>();
        ...
        List<BaseMethodBinding> methodBindings =
                resourceToMethods.values().stream()
                        .flatMap(t -> t.stream())
        ...
        ListMultimap<String, OperationMethodBinding> nameToOperationMethodBindings =
        ...
        for (String nextName : nameToOperationMethodBindings.keySet()) {
            List<OperationMethodBinding> nextMethodBindings =
                    nameToOperationMethodBindings.get(nextName);
        ...
        return new Bindings(
                namedSearchMethodBindingToName,
                searchNameToBindings,
                operationIdToBindings,
                operationBindingToId);
    }
                        ...
                        resourceClass -> {
                            RuntimeResourceDefinition baseDefinition =
                                    getFhirContext()
                                            ...
                                            .getBaseDefinition();
                            CommonResourceSupertypeScanner scanner =
                                    resourceNameToScanner.computeIfAbsent(
                                            baseDefinition.getName(),
                                            key -> new CommonResourceSupertypeScanner());
                            scanner.register(resourceClass);
                        });

        myResourceNameToSharedSupertype =
                resourceNameToScanner.entrySet().stream()
    ...
    private void createRuntimeBinding(
        ...
        List<SearchParameter> parameters =
                theSearchMethodBinding.getParameters().stream()
        ...
        for (SearchParameter nextParameter : parameters) {
            ...
            if (StringUtils.isBlank(nextParamDescription)) {
                RuntimeResourceDefinition def =
                        getFhirContext()
                                .getResourceDefinition(theSearchMethodBinding.getResourceName());
            ...
            IIdType id =
                    getFhirContext()
                            ...
                            .setValue(
                                    "SearchParameter/"
                                            + theSearchMethodBinding.getResourceName()
                                            + "-"
                                            + nextParamName);
            String uri = null;
            ...
            Collection<String> base =
                    Collections.singletonList(theSearchMethodBinding.getResourceName());
