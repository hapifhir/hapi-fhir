...
public class AddressValidatingInterceptor {
    ...
    private static final Logger ourLog =
            LoggerFactory.getLogger(AddressValidatingInterceptor.class);
    ...
    public static final String ADDRESS_VALIDATION_DISABLED_HEADER =
            "HAPI-Address-Validation-Disabled";
    ...
    public void start(Properties theProperties) {
        ...
        Validate.notBlank(
                validatorClassName, "%s property can not be blank", PROPERTY_VALIDATOR_CLASS);
                ...
                addressValidator =
                        (IAddressValidator)
                                validatorClass
                                        ...
                                        .newInstance(theProperties);
            } catch (Exception e) {
                addressValidator =
                        (IAddressValidator) validatorClass.getDeclaredConstructor().newInstance();
            }
    ...
    public void resourcePreUpdate(
        ...
        ourLog.debug(
                "Validating address on for update {}, {}, {}",
                theOldResource,
                theNewResource,
                theRequest);
    ...
    protected void handleRequest(RequestDetails theRequest, IBaseResource theResource) {
        ...
        List<IBase> addresses =
                getAddresses(theResource, ctx).stream()
     ...
     *
     * @return Returns true if all addresses are valid, or false if there is at least one invalid
     *     address
     */
    ...
    protected boolean validateAddress(IBase theAddress, FhirContext theFhirContext) {
        ...
        try {
            AddressValidationResult validationResult =
                    getAddressValidator().isValid(theAddress, theFhirContext);
            ...
            if (validationResult.getValidatedAddress() != null) {
                theFhirContext
                        .newTerser()
                        .cloneInto(validationResult.getValidatedAddress(), theAddress, true);
            } else {
                ourLog.info(
                        "Validated address is not provided - skipping update on the target address"
                                + " instance");
            }
            ...
            IBaseExtension extension =
                    ExtensionUtil.getOrCreateExtension(theAddress, getExtensionUrl());
    ...
    private void clearPossibleDuplicatesDueToTerserCloning(
            IBase theAddress, FhirContext theFhirContext) {
    ...
    protected List<IBase> getAddresses(
            IBaseResource theResource, final FhirContext theFhirContext) {
        ...
        for (BaseRuntimeChildDefinition c : definition.getChildren()) {
            ...
            List<IBase> allValues =
                    c.getAccessor().getValues(theResource).stream()
