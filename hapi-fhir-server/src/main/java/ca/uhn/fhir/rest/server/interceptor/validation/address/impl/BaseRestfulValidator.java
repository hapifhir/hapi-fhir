...
public abstract class BaseRestfulValidator implements IAddressValidator {
    ...
    protected abstract ResponseEntity<String> getResponseEntity(
    ...
    public AddressValidationResult isValid(IBase theAddress, FhirContext theFhirContext)
        ...
        if (isError(entity)) {
            throw new AddressValidationException(
                    Msg.code(346)
                            + String.format(
                                    "Service returned an error code %s", entity.getStatusCode()));
        }
        ...
        try {
            ...
            throw new AddressValidationException(
                    Msg.code(347) + "Unable to validate the address", e);
        }
    }
