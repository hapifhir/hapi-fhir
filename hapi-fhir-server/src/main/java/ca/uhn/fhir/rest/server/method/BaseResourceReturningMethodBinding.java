...
public abstract class BaseResourceReturningMethodBinding extends BaseMethodBinding {
            ...
            if (IBaseResource.class.isAssignableFrom(theReturnResourceType)) {
                ...
                if (!Modifier.isAbstract(theReturnResourceType.getModifiers())
                    ...
                    Class<? extends IBaseResource> resourceType =
                            (Class<? extends IResource>) theReturnResourceType;
                    RuntimeResourceDefinition resourceDefinition =
                            theContext.getResourceDefinition(resourceType);
    ...
    IBaseResource createBundleFromBundleProvider(
        ...
        if (theResult.getCurrentPageOffset() != null) {
            ...
            Validate.notNull(
                    limit,
                    "IBundleProvider returned a non-null offset, but did not return a non-null page"
                            + " size");
        } else {
            offset =
                    RestfulServerUtils.tryToExtractNamedParameter(
                            theRequest, Constants.PARAM_OFFSET);
        }
        ...
        if (offset != null || !theServer.canStoreSearchResults()) {
            ...
            if ((offset != null && !isOffsetModeHistory())
                    || theResult.getCurrentPageOffset() != null) {
                // When offset query is done theResult already contains correct amount (+ their
                // includes etc.) so return everything
            ...
            if (theSearchId != null) {
                ...
                if (numTotalResults == null || numTotalResults > numToReturn) {
                    ...
                    if (isBlank(searchId)) {
                        ourLog.info(
                                "Found {} results but paging provider did not provide an ID to use"
                                        + " for paging",
                                numTotalResults);
                        searchId = null;
                    }
                ...
                if (!(next instanceof IBaseOperationOutcome)) {
                    throw new InternalErrorException(
                            Msg.code(435)
                                    ...
                                    + "] with no ID specified (IResource#setId(IdDt) must be"
                                    + " called)");
                }
            }
        }
        ...
        if (theResult.getCurrentPageOffset() != null) {
            ...
            if (isNotBlank(theResult.getNextPageId())) {
                links.setNext(
                        RestfulServerUtils.createOffsetPagingLink(
                                links,
        ...
        if (offset != null
            ...
            if (theServer.getPagingProvider() == null && offset != null) {
                // There is no paging provider at all, so assume we're querying up to all the
                // results we need every time
            ...
            case BUNDLE:
                {
                    ...
                    bundleLinks.setSelf(
                            RestfulServerUtils.createLinkSelf(
                                    theRequest.getFhirServerBase(), theRequest));
                    ...
                    if (getMethodReturnType() == MethodReturnTypeEnum.BUNDLE_RESOURCE) {
                        ...
                        bundleFactory.addRootPropertiesToBundle(
                                null, bundleLinks, count, lastUpdated);
                        ...
                        EncodingEnum linkEncoding =
                                theRequest.getParameters().containsKey(Constants.PARAM_FORMAT)
            ...
            case RESOURCE:
                {
                    ...
                    if (result.size() == 0) {
                        ...
                        throw new InternalErrorException(
                                Msg.code(437) + "Method returned multiple resources");
                    }
    ...
    public static boolean callOutgoingResponseHook(
        ...
        if (theRequest.getInterceptorBroadcaster() != null) {
            if (!theRequest
                    ...
                    .callHooks(Pointcut.SERVER_OUTGOING_RESPONSE, responseParams)) {
                return false;
            }
        }
        return true;
