...
public class SearchMethodBinding extends BaseResourceReturningMethodBinding {
	private static final org.slf4j.Logger ourLog =
				org.slf4j.LoggerFactory.getLogger(SearchMethodBinding.class);
	...
	public SearchMethodBinding(
		...
		if (theResourceProviderResourceType != null) {
				this.myResourceProviderResourceName =
						theContext.getResourceType(theResourceProviderResourceType);
		} else {
				this.myResourceProviderResourceName = null;
		}

		myRequiredParamNames =
					getQueryParameters().stream()
								.filter(t -> t.isRequired())
								.map(t -> t.getName())
								.collect(Collectors.toList());
		myOptionalParamNames =
					getQueryParameters().stream()
								.filter(t -> !t.isRequired())
								.map(t -> t.getName())
								.collect(Collectors.toList());
	...
	public MethodMatchEnum incomingServerRequestMatchesMethod(RequestDetails theRequest) {
		...
		if (theRequest.getId() != null && myIdParamIndex == null) {
				ourLog.trace(
						"Method {} doesn't match because ID is not null: {}",
						getMethod(),
						theRequest.getId());
				return MethodMatchEnum.NONE;
		}
		if (!StringUtils.equals(myCompartmentName, theRequest.getCompartmentName())) {
				ourLog.trace(
						"Method {} doesn't match because it is for compartment {} but request is"
									+ " compartment {}",
						getMethod(),
						myCompartmentName,
		...
		for (String nextRequestParam : theRequest.getParameters().keySet()) {
				...
				if (nextRequestParam.startsWith("_")
						&& !SPECIAL_SEARCH_PARAMS.contains(nextUnqualifiedRequestParam)) {
					continue;
				}
				...
				for (BaseQueryParameter nextMethodParam : getQueryParameters()) {
					...
					if (nextRequestParam.equals(nextMethodParam.getName())) {
						QualifierDetails qualifiers =
									QualifierDetails.extractQualifiersFromParameterName(nextRequestParam);
						...
						List<String> qualifiedNames =
									theRequest
												...
												.get(nextUnqualifiedRequestParam);
	...
	*/
	...
	private boolean passesWhitelistAndBlacklist(
				List<String> theQualifiedNames,
				Set<String> theQualifierWhitelist,
				Set<String> theQualifierBlacklist) {
