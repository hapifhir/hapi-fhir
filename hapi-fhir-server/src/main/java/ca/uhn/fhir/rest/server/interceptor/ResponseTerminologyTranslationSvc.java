...
public class ResponseTerminologyTranslationSvc {
	...
	public ResponseTerminologyTranslationSvc(@Nonnull IValidationSupport theValidationSupport) {
		...
		BaseRuntimeElementCompositeDefinition<?> codeableConceptDef =
					(BaseRuntimeElementCompositeDefinition<?>)
								Objects.requireNonNull(
										myFhirContext.getElementDefinition("CodeableConcept"));
		...
		myUriDefinition =
					(RuntimePrimitiveDatatypeDefinition) myFhirContext.getElementDefinition("uri");
		myCodeDefinition =
					(RuntimePrimitiveDatatypeDefinition) myFhirContext.getElementDefinition("code");
		myStringDefinition =
					(RuntimePrimitiveDatatypeDefinition) myFhirContext.getElementDefinition("string");
	}
	...
	public void addMappingSpecification(
		...
		Validate.notBlank(
					theSourceCodeSystemUrl, "theSourceCodeSystemUrl must not be null or blank");
		Validate.notBlank(
					theTargetCodeSystemUrl, "theTargetCodeSystemUrl must not be null or blank");
						...
						if (wantTargetSystem != null) {
								if (!foundSystemsToCodes.containsKey(wantTargetSystem)) {
									...
									for (String code : foundSystemsToCodes.get(nextSourceSystem)) {
										...
										codings.add(
													createCodingFromPrimitives(nextSourceSystem, code, null));
										TranslateConceptResults translateConceptResults =
													myValidationSupport.translateConcept(
																new IValidationSupport.TranslateCodeRequest(
																		codings, wantTargetSystem));
										if (translateConceptResults != null) {
												List<TranslateConceptResult> mappings =
														translateConceptResults.getResults();
												for (TranslateConceptResult nextMapping : mappings) {

													IBase newCoding =
																createCodingFromPrimitives(
