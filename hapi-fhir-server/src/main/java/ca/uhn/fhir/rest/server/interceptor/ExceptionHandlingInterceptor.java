...
public class ExceptionHandlingInterceptor {
    ...
    public Object handleException(
            RequestDetails theRequestDetails, BaseServerResponseException theException)
            throws ServletException, IOException {
        ...
        try {
            ...
            ourLog.error(
                    "HAPI-FHIR was unable to reset the output stream during exception handling. The"
                            + " root causes follows:",
                    t);
        }
    ...
    /**
     * In some edge cases, the output stream is opened already by the point at which an exception is
     * thrown. This is a failsafe to that the output stream is writeable in that case. This
     * operation retains status code and headers, but clears the buffer. Also, it strips the
     * content-encoding header if present, as the method outcome will negotiate its own.
     */
    ...
    public BaseServerResponseException preProcessOutgoingException(
        ...
        if (theException instanceof DataFormatException) {
            // Wrapping the DataFormatException as an InvalidRequestException so that it gets sent
            // back to the client as a 400 response.
        ...
        if (oo == null) {
            try {
                ...
                if (theException instanceof InternalErrorException) {
                    ...
                    ourLog.error(
                            "Failure during REST processing: " + theException.toString(),
                            theException);
                    populateDetails(ctx, theException, oo);
                }
            } catch (Exception e1) {
                ...
                throw new ServletException(
                        Msg.code(328) + "Failed to instantiate OperationOutcome resource instance",
                        e1);
            }
        } else {
    ...
    private void populateDetails(
            FhirContext theCtx, Throwable theException, IBaseOperationOutcome theOo) {
            ...
            for (Class<?> next : myReturnStackTracesForExceptionTypes) {
                if (next.isAssignableFrom(theException.getClass())) {
                    ...
                    OperationOutcomeUtil.addIssue(
                            theCtx, theOo, "error", detailsValue, null, PROCESSING);
                    return;
                }
            }
        }
        ...
        OperationOutcomeUtil.addIssue(
                theCtx, theOo, "error", theException.getMessage(), null, PROCESSING);
    }

    /**
     * If any server methods throw an exception which extends any of the given exception types, the
     * exception stack trace will be returned to the user. This can be useful for helping to
     * diagnose issues, but may not be desirable for production situations.
     *
     ...
     */
    public ExceptionHandlingInterceptor setReturnStackTracesForExceptionTypes(
            Class<?>... theExceptionTypes) {
