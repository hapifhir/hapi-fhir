...
public class RestfulServerUtils {
    static final Pattern ACCEPT_HEADER_PATTERN =
            Pattern.compile(
                    "\\s*([a-zA-Z0-9+.*/-]+)\\s*(;\\s*([a-zA-Z]+)\\s*=\\s*([a-zA-Z0-9.]+)\\s*)?(,?)");

    private static final org.slf4j.Logger ourLog =
            org.slf4j.LoggerFactory.getLogger(RestfulServerUtils.class);

    private static final HashSet<String> TEXT_ENCODE_ELEMENTS =
            new HashSet<>(Arrays.asList("*.text", "*.id", "*.meta", "*.(mandatory)"));
    private static Map<FhirVersionEnum, FhirContext> myFhirContextMap =
    ...
    public static void configureResponseParser(RequestDetails theRequestDetails, IParser parser) {
        ...
        boolean prettyPrint =
                RestfulServerUtils.prettyPrintResponse(
                        theRequestDetails.getServer(), theRequestDetails);
        ...
        if (elements != null && !summaryMode.equals(Collections.singleton(SummaryEnum.FALSE))) {
            throw new InvalidRequestException(
                    Msg.code(304)
                            ...
                            + " and "
                            + Constants.PARAM_ELEMENTS
                            + " parameters");
        }

        // _elements:exclude
        Set<String> elementsExclude =
                ElementsParameter.getElementsValueOrNull(theRequestDetails, true);
        if (elementsExclude != null) {
            parser.setDontEncodeElements(elementsExclude);
        }

        boolean summaryModeCount =
                summaryMode.contains(SummaryEnum.COUNT) && summaryMode.size() == 1;
        if (!summaryModeCount) {
    ...
    /**
     * This function will create a self link but omit any parameters passed in via the
     * excludedParameterNames list.
     */
    public static String createLinkSelfWithoutGivenParameters(
        ...
        // For POST the URL parameters get jumbled with the post body parameters so don't include
        // them, they might be huge
        if (theRequest.getRequestType() == RequestTypeEnum.GET) {
            ...
            for (String nextParamName : new TreeSet<>(parameters.keySet())) {
                if (excludedParameterNames == null
                        || !excludedParameterNames.contains(nextParamName)) {
    ...
    @Nullable
    public static EncodingEnum determineRequestEncodingNoDefault(
    ...
    public static ResponseEncoding determineResponseEncodingNoDefault(
            RequestDetails theReq, EncodingEnum thePrefer) {
        return determineResponseEncodingNoDefault(theReq, thePrefer, null);
    }

    /**
     * Try to determing the response content type, given the request Accept header and _format
     * parameter. If a value is provided to thePreferContents, we'll prefer to return that value
     * over the native FHIR value.
     */
    public static ResponseEncoding determineResponseEncodingNoDefault(
            ...
            for (String nextFormat : format) {
                ...
                if (retVal != null) {
                    return new ResponseEncoding(
                            theReq.getServer().getFhirContext(), retVal, nextFormat);
                }
            }
        }
    ...
    /**
     * Determine whether a response should be given in JSON or XML format based on the incoming
     * HttpServletRequest's <code>"_format"</code> parameter and <code>"Accept:"</code> HTTP header.
     */
    public static ResponseEncoding determineResponseEncodingWithDefault(RequestDetails theReq) {
        ResponseEncoding retVal =
                determineResponseEncodingNoDefault(
                        theReq, theReq.getServer().getDefaultResponseEncoding());
        if (retVal == null) {
            retVal =
                    new ResponseEncoding(
                            theReq.getServer().getFhirContext(),
    ...
    public static Set<SummaryEnum> determineSummaryMode(RequestDetails theRequest) {
        ...
        if (retVal == null) {
            ...
            if (narrative != null && narrative.length > 0) {
                try {
                    NarrativeModeEnum narrativeMode =
                            NarrativeModeEnum.valueOfCaseInsensitive(narrative[0]);
                    ...
                    ourLog.debug(
                            "Invalid {} parameter: {}", Constants.PARAM_NARRATIVE, narrative[0]);
                }
            }
        }
        if (retVal == null) {
    ...
    private static FhirContext getContextForVersion(
            FhirContext theContext, FhirVersionEnum theForVersion) {

        // TODO: remove once we've bumped the core lib version
        if (theContext.getVersion().getVersion() == FhirVersionEnum.R4B
                && theForVersion == FhirVersionEnum.R5) {
            return theContext;
        }
    ...
    public static Set<String> parseAcceptHeaderAndReturnHighestRankedOptions(
    ...
    public static PreferHeader parsePreferHeader(IRestfulServer<?> theServer, String theValue) {
        ...
        if (retVal.getReturn() == null
                && theServer != null
                && theServer.getDefaultPreferReturn() != null) {
    ...
    public static boolean prettyPrintResponse(
            IRestfulServerDefaults theServer, RequestDetails theRequest) {
        Map<String, String[]> requestParams = theRequest.getParameters();
    ...
    public static Object streamResponseAsResource(
            ...
            if (theResource.getIdElement() != null) {
                ...
                fullId =
                        fullyQualifyResourceIdOrReturnNull(
                                theServer, theResource, serverBase, resourceId);
            }
        }
        ...
        if (theResource instanceof IBaseBinary) {
            ...
            IBaseReference securityContext =
                    BinaryUtil.getSecurityContext(theServer.getFhirContext(), bin);
            ...
            if (shouldStreamContents(responseEncoding, bin)) {
                ...
                OutputStream outputStream =
                        response.getResponseOutputStream(theStatusCode, contentType, contentLength);
                if (bin.hasData()) {
        ...
        if (responseEncoding == null) {
            responseEncoding =
                    new ResponseEncoding(
                            theServer.getFhirContext(),
                            theServer.getDefaultResponseEncoding(),
                            null);
        }

        boolean encodingDomainResourceAsText =
                theSummaryMode.size() == 1 && theSummaryMode.contains(SummaryEnum.TEXT);
        ...
        if (lastUpdated != null && lastUpdated.isEmpty() == false) {
            response.addHeader(
                    Constants.HEADER_LAST_MODIFIED, DateUtils.formatDate(lastUpdated.getValue()));
        }
        ...
        Writer writer =
                response.getResponseWriter(theStatusCode, contentType, charset, respondGzip);

        // Interceptor call: SERVER_OUTGOING_WRITER_CREATED
        if (theServer.getInterceptorService() != null
                && theServer
                        .getInterceptorService()
                        .hasHooks(Pointcut.SERVER_OUTGOING_WRITER_CREATED)) {
            HookParams params =
                    new HookParams()
            ...
            Object newWriter =
                    theServer
                            ...
                            .callHooksAndReturnObject(
                                    Pointcut.SERVER_OUTGOING_WRITER_CREATED, params);
            if (newWriter != null) {
        ...
        if (theResource == null) {
            ...
            IParser parser =
                    getNewParser(theServer.getFhirContext(), forVersion, theRequestDetails);
    ...
    /**
     * Determines whether we should stream out Binary resource content based on the content-type.
     * Logic is: - If the binary was externalized and has not been reinflated upstream, return
     * false. - If they request octet-stream, return true; - If the content-type happens to be a
     * match, return true.
     *
     * <p>- Construct an EncodingEnum out of the contentType. If this matches the responseEncoding,
     * return true. - Otherwise, return false.
     *
     * @param theResponseEncoding the requested {@link EncodingEnum} determined by the incoming
     *     Content-Type header.
     ...
     */
    private static boolean shouldStreamContents(
            ResponseEncoding theResponseEncoding, IBaseBinary theBinary) {
        ...
        if (isBlank(contentType)) {
            ...
            return Objects.equals(
                    EncodingEnum.forContentType(contentType), theResponseEncoding.getEncoding());
        }
    }
    ...
    public static Integer tryToExtractNamedParameter(
        ...
        try {
            ...
            ourLog.debug(
                    "Failed to parse {} value '{}': {}", theParamName, retVal[0], e.toString());
            return null;
        }
    }
    ...
    public static void validateResourceListNotNull(List<? extends IBaseResource> theResourceList) {
        if (theResourceList == null) {
            throw new InternalErrorException(
                    Msg.code(306)
                            + "IBundleProvider returned a null list of resources - This is not"
                            + " allowed");
        }
    }
    ...
    public static DeleteCascadeDetails extractDeleteCascadeParameter(RequestDetails theRequest) {
        ...
        if (theRequest != null) {
            String[] cascadeParameters =
                    theRequest.getParameters().get(Constants.PARAMETER_CASCADE_DELETE);
            if (cascadeParameters != null
                    && Arrays.asList(cascadeParameters).contains(Constants.CASCADE_DELETE)) {
                mode = DeleteCascadeModeEnum.DELETE;
                String[] maxRoundsValues =
                        theRequest
                                .getParameters()
                                .get(Constants.PARAMETER_CASCADE_DELETE_MAX_ROUNDS);
                ...
                if (isNotBlank(cascadeHeader)) {
                    if (Constants.CASCADE_DELETE.equals(cascadeHeader)
                        ...
                        if (cascadeHeader.contains(";")) {
                            String remainder =
                                    cascadeHeader.substring(cascadeHeader.indexOf(';') + 1);
                            remainder = trim(remainder);
                            if (remainder.startsWith(Constants.HEADER_CASCADE_MAX_ROUNDS + "=")) {
                                String maxRoundsString =
                                        remainder.substring(
                                                Constants.HEADER_CASCADE_MAX_ROUNDS.length() + 1);
                                maxRounds = parseMaxRoundsString(maxRoundsString);
                            }
                        }
                    }
    ...
    private static Integer parseMaxRoundsString(String theMaxRoundsString) {
        ...
        if (isBlank(theMaxRoundsString)) {
            ...
            throw new InvalidRequestException(
                    Msg.code(2349)
                            + "Invalid value for "
                            + Constants.PARAMETER_CASCADE_DELETE_MAX_ROUNDS
                            + " parameter");
        }
        return maxRounds;
    }
    ...
    public static class ResponseEncoding {
        ...
        public ResponseEncoding(
                FhirContext theCtx, EncodingEnum theEncoding, String theContentType) {
            super();
            ...
            if (theContentType != null) {
                ...
                if (theContentType.equals(EncodingEnum.JSON_PLAIN_STRING)
                    ...
                    myNonLegacy =
                            ctxtEnum.isNewerThan(FhirVersionEnum.DSTU2_1)
                                    && !EncodingEnum.isLegacy(theContentType);
                }
            } else {
                FhirVersionEnum ctxtEnum = theCtx.getVersion().getVersion();
