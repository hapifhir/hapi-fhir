...
/**
 * This object contains runtime information that is gathered and relevant to a single <i>database
 * transaction</i>. This doesn't mean a FHIR transaction necessarily, but rather any operation that
 * happens within a single DB transaction (i.e. a FHIR create, read, transaction, etc.).
 *
 * <p>The intent with this class is to hold things we want to pass from operation to operation
 * within a transaction in order to avoid looking things up multiple times, etc.
 *
 * @since 5.0.0
 */
public class TransactionDetails {
	...
	*/
	...
	/**
	* A <b>Resolved Resource ID</b> is a mapping between a resource ID (e.g. "<code>Patient/ABC
	* </code>" or "<code>Observation/123</code>") and a storage ID for that resource. Resources
	* should only be placed within the TransactionDetails if they are known to exist and be valid
	* targets for other resources to link to.
	*/
	@Nullable
	...
	/**
	* A <b>Resolved Resource ID</b> is a mapping between a resource ID (e.g. "<code>Patient/ABC
	* </code>" or "<code>Observation/123</code>") and the actual persisted/resolved resource with
	* this ID.
	*/
	@Nullable
	...
	* targets for other resources to link to.
	*/
	public void addResolvedResourceId(
				IIdType theResourceId, @Nullable IResourcePersistentId thePersistentId) {
	...
	/**
	* A <b>Resolved Resource ID</b> is a mapping between a resource ID (e.g. "<code>Patient/ABC
	* </code>" or "<code>Observation/123</code>") and the actual persisted/resolved resource. This
	* version takes a {@link Supplier} which will only be fetched if the resource is actually
	* needed. This is good in cases where the resource is lazy loaded.
	*/
	public void addResolvedResource(
				IIdType theResourceId, @Nonnull Supplier<IBaseResource> theResource) {
		assert theResourceId != null;
	...
	/**
	* A <b>Resolved Resource ID</b> is a mapping between a resource ID (e.g. "<code>Patient/ABC
	* </code>" or "<code>Observation/123</code>") and the actual persisted/resolved resource.
	*/
	...
	/**
	* A <b>Resolved Conditional URL</b> is a mapping between a conditional URL (e.g. "<code>
	* Patient?identifier=foo|bar</code>" or "<code>Observation/123</code>") and a storage ID for
	* that resource. Resources should only be placed within the TransactionDetails if they are
	* known to exist and be valid targets for other resources to link to.
	*/
	public void addResolvedMatchUrl(
				FhirContext theFhirContext,
				String theConditionalUrl,
				@Nonnull IResourcePersistentId thePersistentId) {
	...
	*/
	...
	private boolean matchUrlWithDiffIdExists(
				String theConditionalUrl, @Nonnull IResourcePersistentId thePersistentId) {
		if (myResolvedMatchUrls.containsKey(theConditionalUrl)
					&& myResolvedMatchUrls.get(theConditionalUrl) != NOT_FOUND) {
				return !myResolvedMatchUrls
						.get(theConditionalUrl)
						.getId()
						.equals(thePersistentId.getId());
		}
		return false;
	}
	...
	*/
	...
	/**
	* Fetches the existing value in the user data map, or uses {@literal theSupplier} to create a
	* new object and puts that in the map, and returns it
	*/
	@SuppressWarnings("unchecked")
	...
	/**
	* This can be used by processors for FHIR transactions to defer interceptor broadcasts on
	* sub-requests if needed
	*
	* @since 5.2.0
	*/
	...
	*/
	public boolean isAcceptingDeferredInterceptorBroadcasts(Pointcut thePointcut) {
		return myDeferredInterceptorBroadcasts != null
					&& myDeferredInterceptorBroadcastPointcuts.contains(thePointcut);
	}
	...
	*/
	...
	public InterceptorInvocationTimingEnum getInvocationTiming(Pointcut thePointcut) {
		...
		return hookParams == null
					? InterceptorInvocationTimingEnum.ACTIVE
					: InterceptorInvocationTimingEnum.DEFERRED;
	}
