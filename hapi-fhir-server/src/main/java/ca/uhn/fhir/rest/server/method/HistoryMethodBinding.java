...
public class HistoryMethodBinding extends BaseResourceReturningMethodBinding {
                ...
                for (IBaseResource nextResource : retVal) {
                    if (nextResource.getIdElement() == null
                        ...
                        throw new InternalErrorException(
                                Msg.code(410)
                                        + "Server provided resource at index "
                                        + index
                        ...
                        if (versionId == null || versionId.isEmpty()) {
                            throw new InternalErrorException(
                                    Msg.code(411)
                                            + "Server provided resource at index "
                                            + index
                                            + " with no Version ID set (using"
                                            + " IResource#setId(IdDt))");
                        }
                    }
                    index++;
                }
    ...
    private static Class<? extends IBaseResource> toReturnType(
            Method theMethod, Object theProvider) {
        if (theProvider instanceof IResourceProvider) {
