	...
	public void register(Class<? extends IBaseResource> resourceClass) {
		...
		if (initialized) {
				for (int i = 0;
						i
									< Math.min(
												resourceClassesInHierarchy.size(),
												greatestSharedAncestorsDescending.size());
						i++) {
					if (greatestSharedAncestorsDescending.get(i) != resourceClassesInHierarchy.get(i)) {
						greatestSharedAncestorsDescending =
									greatestSharedAncestorsDescending.subList(0, i);
						break;
					}
				}
		} else {
	...
	*/
	public Optional<Class<? extends IBaseResource>> getLowestCommonSuperclass() {
		...
		return Optional.ofNullable(
					greatestSharedAncestorsDescending.get(
								greatestSharedAncestorsDescending.size() - 1));
	}
}
