...
/**
 * This interceptor detects when a request is coming from a browser, and automatically returns a
 * response with syntax highlighted (coloured) HTML for the response instead of just returning raw
 * XML/JSON.
 *
 * @since 1.0
 */
...
public class ResponseHighlighterInterceptor {
    ...
    /**
     * TODO: As of HAPI 1.6 (2016-06-10) this parameter has been replaced with simply requesting
     * _format=json or xml so eventually this parameter should be removed
     */
    ...
    private int format(
            String theResultBody, StringBuilder theTarget, EncodingEnum theEncodingEnum) {
            ...
            if (theEncodingEnum == EncodingEnum.JSON) {
                ...
                if (inQuote) {
                    ...
                    if (nextChar == '&'
                            ...
                            && nextChar4 == ';') {
                        theTarget.append(
                                "<span class='hlControl'>&lt;</span><span class='hlTagName'>");
                        inTag = true;
                        i += 3;
                    } else {
                        theTarget.append(nextChar);
    ...
    @Hook(
            ...
            order = InterceptorOrders.RESPONSE_HIGHLIGHTER_INTERCEPTOR)
    public boolean handleException(
         ...
         */
        Set<String> accept =
                RestfulServerUtils.parseAcceptHeaderAndReturnHighestRankedOptions(
                        theServletRequest);
    ...
    /**
     * If set to <code>true</code> (default is <code>false</code>) response will include the request
     * headers
     */
    ...
    /**
     * If set to <code>true</code> (default is <code>true</code>) response will include the response
     * headers
     */
    ...
    @Hook(
            value = Pointcut.SERVER_OUTGOING_GRAPHQL_RESPONSE,
            order = InterceptorOrders.RESPONSE_HIGHLIGHTER_INTERCEPTOR)
    ...
    public boolean outgoingResponse(
        ...
        if (!Boolean.TRUE.equals(
                theRequestDetails.getAttribute("ResponseHighlighterInterceptorHandled"))) {
    ...
    public void capabilityStatementGenerated(
        ...
        Set<String> formats =
                terser.getValues(theCapabilityStatement, "format", IPrimitiveType.class).stream()
        ...
        addFormatConditionally(
                ...
                Constants.FORMATS_HTML_JSON);
        addFormatConditionally(
                theCapabilityStatement,
                terser,
                formats,
                Constants.CT_FHIR_XML_NEW,
                Constants.FORMATS_HTML_XML);
        addFormatConditionally(
                theCapabilityStatement,
                terser,
                formats,
                Constants.CT_RDF_TURTLE,
                Constants.FORMATS_HTML_TTL);
    }
    ...
    private boolean handleOutgoingResponse(
        ...
        if (rawParamValues != null
                && rawParamValues.length > 0
                && rawParamValues[0].equals(PARAM_RAW_TRUE)) {
            ourLog.warn(
                    "Client is using non-standard/legacy  _raw parameter - Use _format=json or"
                        + " _format=xml instead, as this parmameter will be removed at some point");
            return true;
        }
        ...
        if (!force
                ...
                && (theResponseObject.getResponseResource() instanceof IBaseBinary)) {
            return true;
        }
    ...
    private void streamResponse(
        ...
        if (isNotBlank(theGraphqlResponse)) {
            ...
            if (parameters.containsKey(Constants.PARAM_FORMAT)) {
                ...
                p =
                        defaultResponseEncoding.newParser(
                                theRequestDetails.getServer().getFhirContext());
        ...
        try {
            ...
            outputBuffer.append(
                    ClasspathUtil.loadResource(
                            "ca/uhn/fhir/rest/server/interceptor/ResponseHighlighter.css"));
            outputBuffer.append("       </style>\n");
            outputBuffer.append("	</head>\n");
            outputBuffer.append("\n");
            ...
            Date startTime =
                    (Date) theServletRequest.getAttribute(RestfulServer.REQUEST_START_TIME);
            if (startTime != null) {
            ...
            InputStream jsStream =
                    ResponseHighlighterInterceptor.class.getResourceAsStream(
                            "ResponseHighlighter.js");
            String jsStr =
                    jsStream != null
                            ? IOUtils.toString(jsStream, StandardCharsets.UTF_8)
                            : "console.log('ResponseHighlighterInterceptor: javascript theResource"
                                    + " not found')";
    ...
    @VisibleForTesting
    @Nullable
    String extractNarrativeHtml(
            @Nonnull RequestDetails theRequestDetails, @Nullable IBaseResource theResource) {
        if (theResource == null) {
            return null;
        }
        ...
        if (xhtmlNode == null && "Bundle".equals(ctx.getResourceType(theResource))) {
            if ("document"
                    .equals(ctx.newTerser().getSinglePrimitiveValueOrNull(theResource, "type"))) {
                IBaseResource firstResource =
                        ctx.newTerser()
                                .getSingleValueOrNull(
                                        theResource, "entry.resource", IBaseResource.class);
                if (firstResource != null
                        && "Composition".equals(ctx.getResourceType(firstResource))) {
        ...
        if (xhtmlNode == null && "Parameters".equals(ctx.getResourceType(theResource))) {
            String firstParameterName =
                    ctx.newTerser().getSinglePrimitiveValueOrNull(theResource, "parameter.name");
            if ("Narrative".equals(firstParameterName)) {
                String firstParameterValue =
                        ctx.newTerser()
                                .getSinglePrimitiveValueOrNull(theResource, "parameter.value[x]");
    ...
    private void writeLength(HttpServletResponse theServletResponse, int theLength)
            throws IOException {
                ...
                for (String nextHeaderValue : theServletResponse.getHeaders(nextHeaderName)) {
                    ...
                    if (nextHeaderName.equalsIgnoreCase(Constants.HEADER_CONTENT_TYPE)) {
                        ...
                        if (responseEncoding != null
                                && isNotBlank(responseEncoding.getResourceContentType())) {
                            nextHeaderValue =
                                    responseEncoding.getResourceContentType() + ";charset=utf-8";
                        }
                    }
                    appendHeader(b, nextHeaderName, nextHeaderValue);
                }
    ...
    private void appendHeader(
            StringBuilder theBuilder, String theHeaderName, String theHeaderValue) {
    ...
    /**
     * If set to {@literal true} (default is {@literal true}), if the response is a FHIR resource,
     * and that resource includes a <a href="http://hl7.org/fhir/narrative.html">Narrative</div>,
     * the narrative will be rendered in the HTML response page as actual rendered HTML.
     *
     * <p>The narrative to be rendered will be sourced from one of 3 possible locations, depending
     * on the resource being returned by the server:
     *
     * <ul>
     *   <li>if the resource is a DomainResource, the narrative in Resource.text will be rendered.
     *   <li>If the resource is a document bundle, the narrative in the document Composition will be
     *       rendered.
     *   <li>If the resource is a Parameters resource, and the first parameter has the name
     *       "Narrative" and a value consisting of a string starting with "<div", that will be
     *       rendered.
     * </ul>
     *
     * <p>In all cases, the narrative is scanned to ensure that it does not contain any tags or
     * attributes that are not explicitly allowed by the FHIR specification in order to <a
     * href="http://hl7.org/fhir/narrative.html#xhtml">prevent active content</a>. If any such tags
     * or attributes are found, the narrative is not rendered and instead a warning is displayed.
     * Note that while this scanning is helpful, it does not completely mitigate the security risks
     * associated with narratives. See <a href="http://hl7.org/fhir/security.html#narrative">FHIR
     * Security: Narrative</a> for more information.
     *
    ...
    @Nullable
    private static XhtmlNode extractNarrativeFromDomainResource(
            @Nonnull IBaseResource theResource, FhirContext ctx) {
