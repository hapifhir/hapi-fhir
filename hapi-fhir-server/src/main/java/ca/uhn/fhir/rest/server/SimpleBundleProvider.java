...
public class SimpleBundleProvider implements IBundleProvider {
     ...
     * {@link #getPublished() the published date} but this method may be used to specify an
     * alternate date/time
     */
    ...
    public List<IBaseResource> getResources(int theFromIndex, int theToIndex) {
        return (List<IBaseResource>)
                myList.subList(
                        Math.min(theFromIndex, myList.size()), Math.min(theToIndex, myList.size()));
    }
    ...
    /**
     * Sets the preferred page size to be returned by {@link #preferredPageSize()}. Default is
     * <code>null</code>.
     */
    ...
    /**
     * Sets the total number of results, if this provider corresponds to a single page within a
     * larger search result
     */
