...
public abstract class BaseResourceMessage implements IResourceMessage, IModelJson {
     ...
     *
     * <p>Attributes are just a spot for user data of any kind to be added to the message for pasing
     * along the subscription processing pipeline (typically by interceptors). Values will be
     * carried from the beginning to the end.
     *
     * <p>Note that messages are designed to be passed into queueing systems and serialized as JSON.
     * As a result, only strings are currently allowed as values.
     */
     ...
     *
     * <p>Attributes are just a spot for user data of any kind to be added to the message for
     * passing along the subscription processing pipeline (typically by interceptors). Values will
     * be carried from the beginning to the end.
     *
     * <p>Note that messages are designed to be passed into queueing systems and serialized as JSON.
     * As a result, only strings are currently allowed as values.
     *
    ...
    /**
     * Adds a transaction ID to this message. This ID can be used for many purposes. For example,
     * performing tracing across asynchronous hooks, tying data together, or downstream logging
     * purposes.
     *
     * <p>One current internal implementation uses this field to tie back MDM processing results
     * (which are asynchronous) to the original transaction log that caused the MDM processing to
     * occur.
     *
     ...
     * #getMessageKey()} returns <code>null</code>.
     *
