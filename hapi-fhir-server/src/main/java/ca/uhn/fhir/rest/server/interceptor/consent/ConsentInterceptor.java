...
/**
 * The ConsentInterceptor can be used to apply arbitrary consent rules and data access policies on
 * responses from a FHIR server.
 *
...
public class ConsentInterceptor {
	...
	*/
	public ConsentInterceptor(
				IConsentService theConsentService, IConsentContextServices theContextConsentServices) {
	...
	*
	* <p>Thread safety note: This method can be called while the service is actively processing
	* requestes
	*
	...
	*/
	...
	public void interceptPreHandled(RequestDetails theRequestDetails) {
		...
		for (IConsentService nextService : myConsentService) {
				ConsentOutcome outcome =
						nextService.startOperation(theRequestDetails, myContextConsentServices);
	...
	public void interceptPreAccess(
		...
		for (int consentSvcIdx = 0; consentSvcIdx < myConsentService.size(); consentSvcIdx++) {
				...
				boolean shouldCallCanSeeResource =
						nextService.shouldProcessCanSeeResource(
									theRequestDetails, myContextConsentServices);
		...
		IdentityHashMap<IBaseResource, Boolean> authorizedResources =
					getAuthorizedResourcesMap(theRequestDetails);
		for (int resourceIdx = 0; resourceIdx < thePreResourceAccessDetails.size(); resourceIdx++) {
				...
				for (int consentSvcIdx = 0; consentSvcIdx < myConsentService.size(); consentSvcIdx++) {
					...
					ConsentOutcome outcome =
								nextService.canSeeResource(
										theRequestDetails, nextResource, myContextConsentServices);
					Validate.notNull(outcome, "Consent service returned null outcome");
					Validate.isTrue(
								outcome.getResource() == null,
								"Consent service returned a resource in its outcome. This is not permitted"
										+ " in canSeeResource(..)");
	...
	public void interceptPreShow(
		...
		for (int i = 0; i < thePreResourceShowDetails.size(); i++) {
				...
				if (resource == null
						|| authorizedResources.putIfAbsent(resource, Boolean.TRUE) != null) {
					continue;
				}

				for (IConsentService nextService : myConsentService) {
					ConsentOutcome nextOutcome =
								nextService.willSeeResource(
										theRequestDetails, resource, myContextConsentServices);
					...
					switch (nextOutcome.getStatus()) {
						...
						case REJECT:
								if (nextOutcome.getOperationOutcome() != null) {
									IBaseOperationOutcome newOperationOutcome =
												nextOutcome.getOperationOutcome();
	...
	public void interceptOutgoingResponse(
				RequestDetails theRequestDetails, ResponseDetails theResource) {
		...
		// See outer resource
		if (authorizedResources.putIfAbsent(theResource.getResponseResource(), Boolean.TRUE)
					== null) {

				for (IConsentService next : myConsentService) {
					final ConsentOutcome outcome =
								next.willSeeResource(
										theRequestDetails,
										theResource.getResponseResource(),
										myContextConsentServices);
					...
					if (theResource.getResponseResource() instanceof IBaseBundle) {
						BundleUtil.setTotal(
									...
									(IBaseBundle) theResource.getResponseResource(),
									null);
					}
									...
									for (IConsentService next : myConsentService) {
										...
										if (shouldReplaceResource) {
												...
												BaseRuntimeChildDefinition containerChildElement =
														theChildDefinitionPath.get(
																	theChildDefinitionPath.size() - 1);
												containerChildElement
														.getMutator()
														.setValue(container, replacementResource);
	...
	private IdentityHashMap<IBaseResource, Boolean> getAuthorizedResourcesMap(
				RequestDetails theRequestDetails) {
				...
				if (theParameterMap.containsKey(Constants.PARAM_SUMMARY)
					...
					throw new InvalidRequestException(
								Msg.code(2038)
										...
										+ "=count is not permitted on this server");
				}
		}
	}
	...
	public static IdentityHashMap<IBaseResource, Boolean> getAlreadySeenResourcesMap(
		...
		IdentityHashMap<IBaseResource, Boolean> alreadySeenResources =
					(IdentityHashMap<IBaseResource, Boolean>)
	...
	private static ForbiddenOperationException toForbiddenOperationException(
