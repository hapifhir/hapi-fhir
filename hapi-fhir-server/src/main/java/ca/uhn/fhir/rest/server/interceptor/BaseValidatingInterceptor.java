...
public abstract class BaseValidatingInterceptor<T> extends ValidationResultEnrichingInterceptor {
    ...
    private void addResponseIssueHeader(
            RequestDetails theRequestDetails, SingleValidationMessage theNext) {
    ...
    public BaseValidatingInterceptor<T> addValidatorModule(IValidatorModule theModule) {
        ...
        Validate.isTrue(
                myValidator == null,
                "Can not specify both a validator and validator modules. Only one needs to be"
                        + " supplied.");
    ...
    /**
     * Provides the validator to use. This can be used as an alternative to {@link
     * #addValidatorModule(IValidatorModule)}
     *
    ...
    /**
     * Fail the request by throwing an {@link UnprocessableEntityException} as a result of a
     * validation failure. Subclasses may change this behaviour by providing alternate behaviour.
     */
     ...
     * specified by {@link #setResponseOutcomeHeaderName(String)} will be added containing a JSON
     * encoded OperationOutcome resource containing the validation results.
     */
     ...
     * encoded OperationOutcome resource containing the validation results.
     */
    public void setAddResponseOutcomeHeaderOnSeverity(
            ResultSeverityEnum theAddResponseOutcomeHeaderOnSeverity) {
        myAddResponseOutcomeHeaderOnSeverity =
                theAddResponseOutcomeHeaderOnSeverity != null
                        ? theAddResponseOutcomeHeaderOnSeverity.ordinal()
                        : null;
    }

    /**
     * The maximum length for an individual header. If an individual header would be written
     * exceeding this length, the header value will be truncated.
     */
    ...
    /**
     * The maximum length for an individual header. If an individual header would be written
     * exceeding this length, the header value will be truncated. Value must be greater than 100.
     */
    ...
    /**
     * The name of the header specified by {@link
    ...
    public void setResponseOutcomeHeaderName(String theResponseOutcomeHeaderName) {
        Validate.notEmpty(
                theResponseOutcomeHeaderName,
                "theResponseOutcomeHeaderName can not be empty or null");
    ...
    /**
     * If set to <code>true</code> (default is <code>false</code>) this interceptor will exit
     * immediately and allow processing to continue if the validator throws any exceptions.
     *
     * <p>This setting is mostly useful in testing situations
     */
    ...
    /**
     * Sets the minimum severity at which an issue detected by the validator will result in a header
     * being added to the response. Default is {@link ResultSeverityEnum#INFORMATION}. Set to <code>
     * null</code> to disable this behaviour.
     *
    ...
    /**
     * Sets the minimum severity at which an issue detected by the validator will fail/reject the
     * request. Default is {@link ResultSeverityEnum#ERROR}. Set to <code>null</code> to disable
     * this behaviour.
     */
    ...
    /**
     * Sets the value to add to the response header with the name specified by {@link
     * #setResponseHeaderName(String)} when validation produces a message of severity equal to or
     * greater than {@link #setAddResponseHeaderOnSeverity(ResultSeverityEnum)}
     *
     * <p>This field allows the following substitutions:
     *
     * <table>
     * <tr>
     * <td>Name</td>
    ...
    public void setResponseHeaderValue(String theResponseHeaderValue) {
        Validate.notBlank(
                theResponseHeaderValue, "theResponseHeaderValue must not be blank or null");
        myResponseIssueHeaderValue = theResponseHeaderValue;
    }

    /**
     * Sets the header value to add when no issues are found at or exceeding the threshold specified
     * by {@link #setAddResponseHeaderOnSeverity(ResultSeverityEnum)}
     */
    ...
    /**
     * Hook for subclasses (e.g. add a tag (coding) to an incoming resource when a given severity
     * appears in the ValidationResult).
     */
    protected void postProcessResult(
            RequestDetails theRequestDetails, ValidationResult theValidationResult) {}

    /**
     * Hook for subclasses on failure (e.g. add a response header to an incoming resource upon
     * rejection).
     */
    protected void postProcessResultOnFailure(
            RequestDetails theRequestDetails, ValidationResult theValidationResult) {}
            ...
            if (!found) {
                if (isNotBlank(myResponseIssueHeaderValueNoIssues)) {
                    theRequestDetails
                            .getResponse()
                            .addHeader(
                                    myResponseIssueHeaderName, myResponseIssueHeaderValueNoIssues);
                }
            }
        }
        ...
        if (myAddResponseOutcomeHeaderOnSeverity != null) {
            ...
            if (outcome == null
                ...
                OperationOutcomeUtil.addIssue(
                        ctx, outcome, "information", "No issues detected", "", "informational");
            }
