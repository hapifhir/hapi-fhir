...
public class ReadMethodBindingTest {
	...
	@Mock private FhirContext myCtx;

	@Mock private RequestDetails myRequestDetails;

	@Mock private RuntimeResourceDefinition definition;
	...
	public void testIncomingServerRequestMatchesMethod_Read() throws NoSuchMethodException {
		...
		assertEquals(
					MethodMatchEnum.EXACT,
					binding.incomingServerRequestMatchesMethod(myRequestDetails));

		// VRead
		when(myRequestDetails.getId()).thenReturn(new IdDt("Patient/123/_history/123"));
		assertEquals(
					MethodMatchEnum.NONE, binding.incomingServerRequestMatchesMethod(myRequestDetails));
