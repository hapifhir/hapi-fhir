...
public class SearchMethodBindingTest {
    ...
    public void methodMatchesOwnParams() throws NoSuchMethodException {
        assertThat(
                getBinding("param", String.class)
                        .incomingServerRequestMatchesMethod(
                                mockSearchRequest(
                                        ImmutableMap.of("param", new String[] {"value"}))),
    ...
    public void methodMatchesChainBlacklist() throws NoSuchMethodException {
        ...
        assertThat(
                binding.incomingServerRequestMatchesMethod(
                        mockSearchRequest(
                                ImmutableMap.of(
                                        "refChainBlacklist.badChain", new String[] {"foo"}))),
                Matchers.is(MethodMatchEnum.NONE));
        assertThat(
                binding.incomingServerRequestMatchesMethod(
                        mockSearchRequest(
                                ImmutableMap.of(
                                        "refChainBlacklist.goodChain", new String[] {"foo"}))),
                Matchers.is(MethodMatchEnum.EXACT));
    }

    private SearchMethodBinding getBinding(String name, Class<?>... parameters)
    ...
    private RequestDetails mockSearchRequest(Map<String, String[]> params) {
        ...
        when(requestDetails.getUnqualifiedToQualifiedNames())
                .thenAnswer(
                        t -> {
    ...
    private static class TestResourceProvider {
        ...
        public IBaseResource paramAndTest(
                @RequiredParam(name = "param") String param,
                @OptionalParam(name = "test") String test) {
            return null;
        }
        ...
        public IBaseResource paramAndUnderscoreTest(
                @RequiredParam(name = "param") String param,
                @OptionalParam(name = "_test") String test) {
            return null;
        }
        ...
        public IBaseResource withChainBlacklist(
                @OptionalParam(
                                name = "refChainBlacklist",
                                chainWhitelist = "goodChain",
                                chainBlacklist = "badChain")
