...
public class RestfulServerUtilsTest {
	...
	public void testParsePreferReturnAndAsync() {
		PreferHeader header =
					RestfulServerUtils.parsePreferHeader(
								null, "return=OperationOutcome; respond-async");
	...
	public void testParseHandlingLenientAndReturnRepresentation_CommaSeparatd() {
		PreferHeader header =
					RestfulServerUtils.parsePreferHeader(
								null, "handling=lenient, return=representation");
		assertEquals(PreferReturnEnum.REPRESENTATION, header.getReturn());
		assertFalse(header.getRespondAsync());
		assertEquals(PreferHandlingEnum.LENIENT, header.getHanding());
	}
	...
	public void testParseCascade(
		...
		if (isNotBlank(theCascadeMaxRoundsParam)) {
				params.put(
						...
						new String[] {theCascadeMaxRoundsParam.trim()});
		}
	...
	public void testCreateSelfLinks() {
		...
		String linkSelfWithoutGivenParameters =
					RestfulServerUtils.createLinkSelfWithoutGivenParameters(
								baseUrl, servletRequestDetails, paramsToRemove);
		// Then
		assertThat(
					linkSelfWithoutGivenParameters,
					is(containsString("http://localhost:8000/$my-operation?")));
