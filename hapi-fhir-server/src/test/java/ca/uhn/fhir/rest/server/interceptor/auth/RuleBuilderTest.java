...
public class RuleBuilderTest {
	...
	/**
	* If the user creates multiple rules that allow read/write of individual instances, we will
	* collapse these into a single rule for performance
	*/
	@Test
	public void testCollapseReadInstancesIntoSingleRule() {
		...
		builder.allow()
					.read()
					.instances(
								Lists.newArrayList(new IdDt("Patient/READ-3"), new IdDt("Patient/READ-4")));
		builder.allow()
					.write()
					.instances(
								Lists.newArrayList(
										new IdDt("Patient/WRITE-3"), new IdDt("Patient/WRITE-4")));
	...
	public void testInCompartment_withMultipleReadInstances_collapsesIntoASingleRule() {
		...
		builder.allow()
					.read()
					.allResources()
					.inCompartment("Patient", new IdDt("Patient/lob1patient"));
		builder.allow()
					.read()
					.allResources()
					.inCompartment("Patient", new IdDt("Patient/lob2patient"));
		List<IAuthRule> list = builder.build();
