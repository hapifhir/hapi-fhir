...
public class DataRequirementsProvider {
		...
		private CqfMeasure createCqfMeasure(Measure measure,
			...
			for (Map.Entry<VersionedIdentifier, Pair<Library, org.hl7.fhir.dstu3.model.Library>> libraryEntry : libraryMap
				...
				if (library.getCodeSystems() != null && library.getCodeSystems().getDef() != null) {
					for (CodeSystemDef codeSystem : library.getCodeSystems().getDef()) {
						...
						TerminologyRef term = new VersionedTerminologyRef(TerminologyRef.TerminologyRefType.CODESYSTEM, name, codeId,
							version);
				...
				if (library.getValueSets() != null && library.getValueSets().getDef() != null) {
					for (ValueSetDef valueSet : library.getValueSets().getDef()) {
						...
						TerminologyRef term = new VersionedTerminologyRef(TerminologyRef.TerminologyRefType.VALUESET, name, valueSetId);
						Boolean exists = false;
			...
			Comparator<Measure.MeasureGroupPopulationComponent> populationComparator = new Comparator<Measure.MeasureGroupPopulationComponent>() {
				@Override
				public int compare(Measure.MeasureGroupPopulationComponent item, Measure.MeasureGroupPopulationComponent t1) {
			...
			// Find shared usages
			for (Map.Entry<String, List<Triple<Integer, String, String>>> entry : criteriaMap.entrySet()) {
			...
			// Otherwise, remove the shared components.
				...
				for (int i = 0; i < cqfMeasure.getGroup().size(); i++) {
					Measure.MeasureGroupComponent mgc = cqfMeasure.getGroup().get(i);
					List<Measure.MeasureGroupPopulationComponent> newMgpc = new ArrayList<Measure.MeasureGroupPopulationComponent>();
					for (int j = 0; j < mgc.getPopulation().size(); j++) {
						Measure.MeasureGroupPopulationComponent mgpc = mgc.getPopulation().get(j);
