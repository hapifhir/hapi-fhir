...
public class JdbcUtils {
    ...
    public static Set<String> getIndexNames(
            DriverTypeEnum.ConnectionProperties theConnectionProperties, String theTableName)
            throws SQLException {
                    ...
                    .execute(
                            t -> {
                                ...
                                try {
                                    ...
                                    ResultSet indexes =
                                            getIndexInfo(theTableName, connection, metadata, false);
                                    Set<String> indexNames = new HashSet<>();
                                    while (indexes.next()) {
                                        ourLog.debug(
                                                "*** Next index: {}",
                                                new ColumnMapRowMapper().mapRow(indexes, 0));
                                        String indexName = indexes.getString("INDEX_NAME");
                                        indexNames.add(indexName);
                                    }

                                    indexes =
                                            getIndexInfo(theTableName, connection, metadata, true);
                                    while (indexes.next()) {
                                        ourLog.debug(
                                                "*** Next index: {}",
                                                new ColumnMapRowMapper().mapRow(indexes, 0));
                                    ...
                                    indexNames =
                                            indexNames.stream()
                                                    .filter(Objects::nonNull) // filter out the
                                                    // nulls first
                                                    .map(s -> s.toUpperCase(Locale.US)) // then
                                                    // convert
                                                    // the
                                                    // non-null entries to upper case
                                        ...
                                        switch (dataType) {
                                            ...
                                            case Types.BOOLEAN:
                                                return new ColumnType(
                                                        ColumnTypeEnum.BOOLEAN, length);
                                            case Types.VARCHAR:
                                                return new ColumnType(
                                                        ColumnTypeEnum.STRING, length);
                                            case Types.NUMERIC:
                                            case Types.BIGINT:
                                            case Types.DECIMAL:
                                            ...
                                            case Types.TIMESTAMP_WITH_TIMEZONE:
                                                return new ColumnType(
                                                        ColumnTypeEnum.DATE_TIMESTAMP, length);
                                            case Types.BLOB:
                                                return new ColumnType(ColumnTypeEnum.BLOB, length);
                                            case Types.LONGVARBINARY:
                                                if (DriverTypeEnum.MYSQL_5_7.equals(
                                                        theConnectionProperties.getDriverType())) {
                                                    // See git
                                                    return new ColumnType(
                                                            ColumnTypeEnum.BLOB, length);
                                                } else {
                                            ...
                                            case Types.VARBINARY:
                                                if (DriverTypeEnum.MSSQL_2012.equals(
                                                        theConnectionProperties.getDriverType())) {
                                                    // MS SQLServer seems to be mapping BLOB to
                                                    // VARBINARY under the covers, so we need to
                    ...
                    t -> {
                        ...
                        try {
                            ...
                            if (theTableName != null) {
                                ...
                                parentTables.addAll(
                                        JdbcUtils.getTableNames(theConnectionProperties));
                            }
                    ...
                    .execute(
                            t -> {
                                ...
                                try {
                                    ...
                                    String foreignTable =
                                            massageIdentifier(metadata, theForeignTable);
                    ...
                    .execute(
                            t -> {
                                try {
                                    ...
                                    Dialect dialect =
                                            dialectResolver.resolveDialect(
                                    ...
                                    if (dialect.supportsSequences()) {
                                                ...
                                                new ExtractionContext.EmptyExtractionContext() {
                                                    ...
                                                    public JdbcEnvironment getJdbcEnvironment() {
                                                        return new JdbcEnvironment() {
                                                            ...
                                                            @Override
                                                            public QualifiedObjectNameFormatter
                                        ...
                                        Iterable<SequenceInformation> sequences =
                                                sequenceInformationExtractor.extractMetadata(
                                                        extractionContext);
                                        for (SequenceInformation next : sequences) {
                                            sequenceNames.add(
                                                    next.getSequenceName()
                                                            .getSequenceName()
                                                            .getText());
                                        }
                                    }
                            ...
                            t -> {
                                ...
                                try {
                                    ...
                                    while (tables.next()) {
                                        ...
                                        if (SchemaMigrator.HAPI_FHIR_MIGRATION_TABLENAME
                            ...
                            t -> {
                                ...
                                try {
                                    ...
                                    while (tables.next()) {
                                        ...
                                        if (theColumnName.equalsIgnoreCase(
                                    ...
                                    throw new IllegalStateException(
                                            Msg.code(42) + "Did not find column " + theColumnName);
                                } catch (SQLException e) {
    ...
    private static String massageIdentifier(DatabaseMetaData theMetadata, String theCatalog)
            throws SQLException {
