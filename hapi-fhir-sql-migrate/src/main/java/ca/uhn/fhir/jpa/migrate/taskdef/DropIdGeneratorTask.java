...
public class DropIdGeneratorTask extends BaseTask {
    ...
    public DropIdGeneratorTask(
            String theProductVersion, String theSchemaVersion, String theGeneratorName) {
    ...
    public void doExecute() throws SQLException {
        ...
        if (isNotBlank(sql)) {
            Set<String> sequenceNames =
                    JdbcUtils.getSequenceNames(getConnectionProperties()).stream()
            ...
            if (!sequenceNames.contains(myGeneratorName.toLowerCase())) {
                logInfo(
                        ourLog,
                        "Sequence {} does not exist - No action performed",
                        myGeneratorName);
                return;
            }
