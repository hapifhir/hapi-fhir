					...
					getTxTemplate()
								.execute(
										t -> {
												...
												logInfo(
														ourLog,
														"Finding up to {} rows in {} that requires"
																	+ " calculations, using query: {}",
														myBatchSize,
														getTableName(),
														sql);
		...
		RejectedExecutionHandler rejectedExecutionHandler =
					new RejectedExecutionHandler() {
						...
						public void rejectedExecution(
								...
								logInfo(
										ourLog,
										"Note: Executor queue is full ({} elements), waiting for a slot to"
													+ " become available!",
										executorQueue.size());
									...
									.execute(
												t -> {
													...
													for (Map<String, Object> nextRow : theRows) {
														...
														MandatoryKeyMap<String, Object> nextRowMandatoryKeyMap =
																	new MandatoryKeyMap<>(nextRow);

														// Apply calculators
														for (Map.Entry<
																				String,
																				Function<
																						MandatoryKeyMap<String, Object>,
																						Object>>
																	nextCalculatorEntry :
																				myCalculators.entrySet()) {
																String nextColumn = nextCalculatorEntry.getKey();
																Function<MandatoryKeyMap<String, Object>, Object>
																		nextCalculator =
																					nextCalculatorEntry.getValue();
																Object value =
																		nextCalculator.apply(
																					nextRowMandatoryKeyMap);
														...
														for (Map.Entry<String, Object> nextNewValueEntry :
																	newValues.entrySet()) {
																if (arguments.size() > 0) {
																	sqlBuilder.append(", ");
																}
																sqlBuilder
																		.append(nextNewValueEntry.getKey())
																		.append(" = ?");
