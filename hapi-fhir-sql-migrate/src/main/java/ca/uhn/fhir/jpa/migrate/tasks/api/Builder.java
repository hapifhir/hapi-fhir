...
public class Builder {
	...
	public InitializeSchemaTask initializeSchema(
		...
		InitializeSchemaTask task =
					new InitializeSchemaTask(myRelease, theVersion, theSchemaInitializationProvider);
		mySink.addTask(task);
		return task;
	}
	...
	public Builder executeRawSql(
				String theVersion, DriverTypeEnum theDriver, @Language("SQL") String theSql) {
		mySink.addTask(new ExecuteRawSqlTask(myRelease, theVersion).addSql(theDriver, theSql));
		return this;
	}

	/**
	* Builder method to define a raw SQL execution migration that needs to take place against
	* multiple database types, and the SQL they need to use is not equal. Provide a map of driver
	* types to SQL statements.
	*
	...
	*/
	...
	/**
	* Builder method to define a raw SQL execution migration that needs to take place against
	* multiple database types, and the SQL they need to use is not equal, and there are multiple
	* sql commands for a given database. Provide a map of driver types to list of SQL statements.
	*
	* @param theVersion The version of the migration.
	* @param theDriverToSqls Map of driver types to list of SQL statements.
	*/
	public Builder executeRawSqls(
				String theVersion, Map<DriverTypeEnum, List<String>> theDriverToSqls) {
		ExecuteRawSqlTask executeRawSqlTask = new ExecuteRawSqlTask(myRelease, theVersion);
		theDriverToSqls.entrySet().stream()
					.forEach(
								entry -> {
									entry.getValue()
												.forEach(sql -> executeRawSqlTask.addSql(entry.getKey(), sql));
								});
	...
	public void dropIdGenerator(String theVersion, String theIdGeneratorName) {
		DropIdGeneratorTask task =
					new DropIdGeneratorTask(myRelease, theVersion, theIdGeneratorName);
		addTask(task);
	}
	...
	public static class BuilderWithTableName implements BaseMigrationTasks.IAcceptsTasks {
		...
		private DropIndexTask dropIndexOptional(
					boolean theDoNothing, String theVersion, String theIndexName) {
		...
		/**
			* @deprecated Do not rename indexes - It is too hard to figure out what happened if
		...
		/**
			...
			*     something goes wrong
			*/
		public void renameIndexStub(
					String theVersion, String theOldIndexName, String theNewIndexName) {
				renameIndexOptional(true, theVersion, theOldIndexName, theNewIndexName);
		}

		private void renameIndexOptional(
					boolean theDoNothing,
					String theVersion,
		...
		public BuilderWithTableName.BuilderAddIndexWithName addIndex(
		...
		public BuilderWithTableName.BuilderAddColumnWithName addColumn(
				...
				return new BuilderWithTableName.BuilderAddColumnWithName(
						myRelease, theVersion, theColumnName, this);
		}
		...
		public BuilderWithTableName.BuilderModifyColumnWithName modifyColumn(
					String theVersion, String theColumnName) {
				return new BuilderWithTableName.BuilderModifyColumnWithName(theVersion, theColumnName);
		}

		public BuilderWithTableName.BuilderAddForeignKey addForeignKey(
		...
		public BuilderWithTableName renameColumn(
					String theVersion, String theOldName, String theNewName) {
		...
		/**
			...
			* @param isOkayIfNeitherColumnExists Setting this to true means that it's not an error if
			*     neither column exists
			* @param theDeleteTargetColumnFirstIfBothExist Setting this to true causes the migrator to
			*     be ok with the target column existing. It will make sure that there is no data in the
			*     column with the new name, then delete it if so in order to make room for the renamed
			*     column. If there is data it will still bomb out.
			*/
		public BuilderWithTableName renameColumn(
					String theVersion,
					String theOldName,
		...
		public class BuilderAddIndexWithName {
				...
				public BuilderWithTableName.BuilderAddIndexWithName.BuilderAddIndexUnique unique(
						boolean theUnique) {
					return new BuilderWithTableName.BuilderAddIndexWithName.BuilderAddIndexUnique(
								myVersion, theUnique);
				}
				...
				public class BuilderAddIndexUnique {
					...
					private AddIndexTask withColumnsOptional(
								boolean theDoNothing, String... theColumnNames) {
		...
		public class BuilderModifyColumnWithName {
				...
				public BuilderWithTableName.BuilderModifyColumnWithName
									.BuilderModifyColumnWithNameAndNullable
						nullable() {
					return new BuilderWithTableName.BuilderModifyColumnWithName
								.BuilderModifyColumnWithNameAndNullable(myVersion, true);
				}
				...
				public BuilderWithTableName.BuilderModifyColumnWithName
						...
						nonNullable() {
					return new BuilderWithTableName.BuilderModifyColumnWithName
								.BuilderModifyColumnWithNameAndNullable(myVersion, false);
				}
				...
				public class BuilderModifyColumnWithNameAndNullable {
					...
					public BuilderModifyColumnWithNameAndNullable(
								String theVersion, boolean theNullable) {
						myVersion = theVersion;
						...
						if (theColumnType == ColumnTypeEnum.STRING) {
								if (theLength == null || theLength == 0) {
									throw new IllegalArgumentException(
												Msg.code(52)
														+ "Can not specify length 0 for column of type "
														+ theColumnType);
								}
						} else {
								if (theLength != null) {
									throw new IllegalArgumentException(
												Msg.code(53)
														+ "Can not specify length for column of type "
														+ theColumnType);
								}
						}
		...
		public class BuilderAddForeignKey {
				...
				public class BuilderAddForeignKeyToColumn
						extends BuilderWithTableName.BuilderModifyColumnWithName {
					public BuilderAddForeignKeyToColumn(String theVersion, String theColumnName) {
						super(theVersion, theColumnName);
					}

					public BuilderCompleteTask references(
								String theForeignTable, String theForeignColumn) {
		...
		public static class BuilderAddColumnWithName {
				...
				public BuilderWithTableName.BuilderAddColumnWithName.BuilderAddColumnWithNameNullable
						nullable() {
					return new BuilderWithTableName.BuilderAddColumnWithName
								.BuilderAddColumnWithNameNullable(myRelease, myVersion, true);
				}

				public BuilderWithTableName.BuilderAddColumnWithName.BuilderAddColumnWithNameNullable
						nonNullable() {
					return new BuilderWithTableName.BuilderAddColumnWithName
								.BuilderAddColumnWithNameNullable(myRelease, myVersion, false);
				}
				...
				public class BuilderAddColumnWithNameNullable {
					...
					public BuilderAddColumnWithNameNullable(
								String theRelease, String theVersion, boolean theNullable) {
						myRelease = theRelease;
						myVersion = theVersion;
						myNullable = theNullable;
					}
	...
	public class BuilderAddTableRawSql {
		...
		public BuilderAddTableRawSql addSql(
					DriverTypeEnum theDriverTypeEnum, @Language("SQL") String theSql) {
	...
	public class BuilderAddTableByColumns extends BuilderWithTableName
