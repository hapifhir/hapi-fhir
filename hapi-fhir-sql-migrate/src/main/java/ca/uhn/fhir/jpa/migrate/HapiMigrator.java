...
public class HapiMigrator {
    ...
    public HapiMigrator(
            String theMigrationTableName, DataSource theDataSource, DriverTypeEnum theDriverType) {
        myDriverType = theDriverType;
        myDataSource = theDataSource;
        myHapiMigrationStorageSvc =
                new HapiMigrationStorageSvc(
                        new HapiMigrationDao(theDataSource, theDriverType, theMigrationTableName));
    }
    ...
    private void executeTask(BaseTask theTask, MigrationResult theMigrationResult) {
        ...
        try {
            if (isDryRun()) {
                ourLog.info(
                        "Dry run {} {}", theTask.getMigrationVersion(), theTask.getDescription());
            } else {
                ourLog.info(
                        "Executing {} {}", theTask.getMigrationVersion(), theTask.getDescription());
            }
            preExecute(theTask);
            theTask.execute();
    ...
    private void postExecute(BaseTask theNext, StopWatch theStopWatch, boolean theSuccess) {
        myHapiMigrationStorageSvc.saveTask(
                theNext, Math.toIntExact(theStopWatch.getMillis()), theSuccess);
    }

    public void addTasks(Iterable<BaseTask> theMigrationTasks) {
        if (HapiSystemProperties.isUnitTestModeEnabled()) {
            // Tests only need to initialize the schemas. No need to run all the migrations for
            // every test.
