...
public class ModifyColumnTask extends BaseTableColumnTypeTask {
    ...
    public void doExecute() throws SQLException {
        ...
        try {
            existingType =
                    JdbcUtils.getColumnType(
                            getConnectionProperties(), getTableName(), getColumnName());
        ...
        if (alreadyOfCorrectType && alreadyCorrectNullable) {
            logInfo(
                    ourLog,
                    "Column {} on table {} is already of type {} and has nullable {} - No action"
                            + " performed",
                    getColumnName(),
                    getTableName(),
                    existingType,
        ...
        switch (getDriverType()) {
            ...
            default:
                throw new IllegalStateException(
                        Msg.code(67) + "Dont know how to handle " + getDriverType());
        }
        ...
        logInfo(
                ourLog,
                "Updating column {} on table {} to type {}",
                getColumnName(),
                getTableName(),
                type);
        if (sql != null) {
        ...
        if (sqlNotNull != null) {
            logInfo(
                    ourLog,
                    "Updating column {} on table {} to not null",
                    getColumnName(),
                    getTableName());
    ...
    private boolean isColumnNullable(String tableName, String columnName) throws SQLException {
        boolean result =
                JdbcUtils.isColumnNullable(getConnectionProperties(), tableName, columnName);
        // Oracle sometimes stores the NULLABLE property in a Constraint, so override the result if
                                ...
                                .execute(
                                        t ->
                                                getConnectionProperties()
                                                        .newJdbcTemplate()
                                                        .query(
                                                                findNullableConstraintSql,
                                                                params,
                                                                new ColumnMapRowMapper()));
                // If this query returns a row then the existence of that row indicates that a NOT
                // NULL constraint exists
                // on this Column and we must override whatever result was previously calculated and
                // set it to false
