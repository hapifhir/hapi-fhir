...
public class AddIndexTask extends BaseTableTask {
    ...
    public void validate() {
        ...
        Validate.isTrue(
                myColumns.size() > 0,
                "Columns not specified for AddIndexTask "
                        + myIndexName
                        + " on table "
                        + getTableName());
    ...
    public void doExecute() throws SQLException {
        ...
        if (indexNames.contains(myIndexName)) {
            logInfo(
                    ourLog,
                    "Index {} already exists on table {} - No action performed",
                    myIndexName,
                    getTableName());
            return;
        }
    ...
    String generateSql() {
        ...
        // Should we do this non-transactionally?  Avoids a write-lock, but introduces weird failure
        // modes.
