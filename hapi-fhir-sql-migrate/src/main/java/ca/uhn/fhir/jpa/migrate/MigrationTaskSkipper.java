...
public class MigrationTaskSkipper {
    ...
    public static void setDoNothingOnSkippedTasks(
            Collection<BaseTask> theTasks, String theSkipVersions) {
        if (isBlank(theSkipVersions) || theTasks.isEmpty()) {
            return;
        }
        Set<String> skippedVersionSet =
                Stream.of(theSkipVersions.split(","))
                        .map(String::trim)
                        // TODO KHS filter out all characters that aren't numbers, periods and
                        // underscores
                        .map(s -> s.replace("'", ""))
                        .map(s -> s.replace("\"", ""))
