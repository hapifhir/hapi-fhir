...
public class RenameColumnTask extends BaseTableTask {
    ...
    public void setDeleteTargetColumnFirstIfBothExist(
    ...
    public void validate() {
        super.validate();
        setDescription(
                "Rename column " + myOldName + " to " + myNewName + " on table " + getTableName());
    }
    ...
    public void doExecute() throws SQLException {
                        ...
                        getConnectionProperties()
                                ...
                                .execute(
                                        t -> {
                                            ...
                                            return jdbcTemplate
                                                    .query(sql, new ColumnMapRowMapper())
                                                    .size();
                                        });
                if (rowsWithData != null && rowsWithData > 0) {
                    throw new SQLException(
                            Msg.code(54)
                                    ...
                                    + " to "
                                    + myNewName
                                    + " because both columns exist and data exists in "
                                    + myNewName);
                }

                if (getDriverType().equals(DriverTypeEnum.MYSQL_5_7) || mySimulateMySQLForTest) {
                    // Some DBs such as MYSQL require that foreign keys depending on the column be
                    // explicitly dropped before the column itself is dropped.
                    logInfo(
                            ourLog,
                            "Table {} has columns {} and {} - Going to drop any foreign keys"
                                    + " depending on column {} before renaming",
                            getTableName(),
                            myOldName,
                            myNewName,
                            myNewName);
                    Set<String> foreignKeys =
                            JdbcUtils.getForeignKeysForColumn(
                                    getConnectionProperties(), myNewName, getTableName());
                    if (foreignKeys != null) {
                        for (String foreignKey : foreignKeys) {
                            List<String> dropFkSqls =
                                    DropForeignKeyTask.generateSql(
                                            getTableName(), foreignKey, getDriverType());
        ...
        try {
            ...
            existingType =
                    getSqlType(
                            existingColumnType.getColumnTypeEnum(), existingColumnType.getLength());
            notNull =
                    JdbcUtils.isColumnNullable(getConnectionProperties(), getTableName(), myOldName)
                            ? " null "
                            : " not null";
