...
public class DropIndexTest extends BaseTest {
    ...
    @MethodSource("data")
    public void testIndexAlreadyExists(Supplier<TestDatabaseDetails> theTestDatabaseDetails)
            throws SQLException {
        ...
        assertThat(
                ...
                contains("IDX_DIFINDEX"));
    }

    @ParameterizedTest(name = "{index}: {0}")
    @MethodSource("data")
    public void testIndexDoesntAlreadyExist(Supplier<TestDatabaseDetails> theTestDatabaseDetails)
            throws SQLException {
        before(theTestDatabaseDetails);
    ...
    @ParameterizedTest(name = "{index}: {0}")
    @MethodSource("data")
    public void testConstraintAlreadyExists(Supplier<TestDatabaseDetails> theTestDatabaseDetails)
            throws SQLException {
        before(theTestDatabaseDetails);
    ...
    @ParameterizedTest(name = "{index}: {0}")
    @MethodSource("data")
    public void testConstraintDoesntAlreadyExist(
    ...
    @ParameterizedTest(name = "{index}: {0}")
    @MethodSource("data")
    public void testDropConstraintIndex(Supplier<TestDatabaseDetails> theTestDatabaseDetails)
            throws SQLException {
        before(theTestDatabaseDetails);

        executeSql(
                "create table SOMETABLE (PID bigint not null, TEXTCOL varchar(255), TEXTCOL2"
                        + " varchar(255))");
    ...
    public class OnlineNoLocks {
        ...
        public void noAffectOffUnique(DriverTypeEnum theDriver) throws SQLException {
            ...
            switch (theDriver) {
                ...
                case MARIADB_10_1:
                    assertThat(
                            mySql,
                            equalTo(asList("alter table SOMETABLE drop index `IDX_ANINDEX`")));
                    break;
                case H2_EMBEDDED:
                ...
                case DERBY_EMBEDDED:
                    assertThat(
                            mySql,
                            equalTo(asList("alter table SOMETABLE drop constraint IDX_ANINDEX")));
                    break;
                case ORACLE_12C:
                    assertThat(mySql, equalTo(asList("drop index IDX_ANINDEX")));
                    break;
                ...
                case POSTGRES_9_4:
                    assertThat(
                            ...
                            equalTo(
                                    asList(
                                            "alter table SOMETABLE drop constraint if exists"
                                                    + " IDX_ANINDEX cascade",
                ...
                case COCKROACHDB_21_1:
                    assertThat(
                            mySql,
                            equalTo(asList("drop index if exists SOMETABLE@IDX_ANINDEX cascade")));
                    break;
            }
        }
        ...
        public void noAffectOffNotUnique(DriverTypeEnum theDriver) throws SQLException {
            ...
            switch (theDriver) {
                ...
                case MARIADB_10_1:
                    assertThat(
                            mySql, equalTo(asList("alter table SOMETABLE drop index IDX_ANINDEX")));
                    break;
                case H2_EMBEDDED:
                    assertThat(mySql, equalTo(asList("drop index IDX_ANINDEX")));
                    break;
            ...
            switch (theDriver) {
                ...
                case MSSQL_2012:
                    assertThat(
                            ...
                            equalTo(
                                    asList(
                                            "drop index IDX_ANINDEX on SOMETABLE WITH (ONLINE ="
                                                    + " ON)")));
                    break;
                case POSTGRES_9_4:
                    assertThat(
                            ...
                            equalTo(
                                    asList(
                                            "alter table SOMETABLE drop constraint if exists"
                                                    + " IDX_ANINDEX cascade",
                                            "drop index CONCURRENTLY if exists IDX_ANINDEX"
                                                    + " cascade")));
                    break;
                case COCKROACHDB_21_1:
                    assertThat(
