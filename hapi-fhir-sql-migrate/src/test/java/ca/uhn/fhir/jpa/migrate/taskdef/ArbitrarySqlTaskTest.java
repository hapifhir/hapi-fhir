...
public class ArbitrarySqlTaskTest extends BaseTest {
	...
	public void test350MigrateSearchParams(Supplier<TestDatabaseDetails> theTestDatabaseDetails) {
		...
		executeSql(
					"create table HFJ_SEARCH_PARM (PID bigint not null, RES_TYPE varchar(255),"
								+ " PARAM_NAME varchar(255))");
		executeSql(
					"insert into HFJ_SEARCH_PARM (PID, RES_TYPE, PARAM_NAME) values (1, 'Patient',"
								+ " 'identifier')");
		executeSql(
					"insert into HFJ_SEARCH_PARM (PID, RES_TYPE, PARAM_NAME) values (2, 'Patient',"
								+ " 'family')");
		executeSql(
					"create table HFJ_RES_PARAM_PRESENT (PID bigint, SP_ID bigint, SP_PRESENT boolean,"
								+ " HASH_PRESENT bigint)");
		executeSql(
					"insert into HFJ_RES_PARAM_PRESENT (PID, SP_ID, SP_PRESENT, HASH_PRESENT) values"
								+ " (100, 1, true, null)");
		executeSql(
					"insert into HFJ_RES_PARAM_PRESENT (PID, SP_ID, SP_PRESENT, HASH_PRESENT) values"
								+ " (101, 2, true, null)");

		ArbitrarySqlTask task =
		...
		String sql =
					"SELECT HFJ_SEARCH_PARM.RES_TYPE RES_TYPE, HFJ_SEARCH_PARM.PARAM_NAME PARAM_NAME,"
						...
						+ " HFJ_RES_PARAM_PRESENT.HASH_PRESENT HASH_PRESENT from HFJ_RES_PARAM_PRESENT"
		...
		task.addQuery(
					...
					t -> {
						...
						Long hash =
									(long)
												((paramName + resType + present)
														.hashCode()); // Note: not the real hash algorithm
						task.executeSql(
									"HFJ_RES_PARAM_PRESENT",
									"update HFJ_RES_PARAM_PRESENT set HASH_PRESENT = ? where PID = ?",
									hash,
									pid);
					});
		...
		List<Map<String, Object>> rows =
					executeQuery("select * from HFJ_RES_PARAM_PRESENT order by PID asc");
	...
	public void testExecuteOnlyIfTableExists(Supplier<TestDatabaseDetails> theTestDatabaseDetails) {
		...
		task.addQuery(
					...
					t -> {
						task.executeSql(
									"HFJ_RES_PARAM_PRESENT",
									"update HFJ_RES_PARAM_PRESENT set FOOFOOOFOO = null");
					});
		task.setExecuteOnlyIfTableExists("hfj_search_parm");
	...
	public void testUpdateTask(Supplier<TestDatabaseDetails> theTestDatabaseDetails) {
		...
		executeSql(
					"create table TEST_UPDATE_TASK (PID bigint not null, RES_TYPE varchar(255),"
								+ " PARAM_NAME varchar(255))");
		executeSql(
					"insert into TEST_UPDATE_TASK (PID, RES_TYPE, PARAM_NAME) values (1, 'Patient',"
								+ " 'identifier')");
		...
		migrator.forVersion(VersionEnum.V3_5_0)
					.addTableRawSql("1", "A")
	...
	public void testArbitrarySql(Supplier<TestDatabaseDetails> theTestDatabaseDetails) {
		before(theTestDatabaseDetails);
		ourLog.info(
					"Starting testArbitrarySql for {}", theTestDatabaseDetails.get().getDriverType());
		...
		executeSql(
					"insert into TEST_UPDATE_TASK (PID, RES_TYPE, PARAM_NAME) values (1, 'Patient',"
								+ " 'identifier')");
		executeSql(
					"insert into TEST_UPDATE_TASK (PID, RES_TYPE, PARAM_NAME) values (1, 'Encounter',"
								+ " 'identifier')");
		...
		migrator.forVersion(VersionEnum.V3_5_0)
					.executeRawSql(
								"1",
								getDriverType(),
								"delete from TEST_UPDATE_TASK where RES_TYPE = 'Patient'")
					.executeRawSql(
								"2",
								getDriverType(),
								"delete from TEST_UPDATE_TASK where RES_TYPE = 'Encounter'");
