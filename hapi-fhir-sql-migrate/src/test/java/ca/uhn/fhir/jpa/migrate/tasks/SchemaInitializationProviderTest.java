...
public class SchemaInitializationProviderTest {
    ...
    public void testParseSqlFileIntoIndividualStatements() {
        ...
        svc.parseSqlFileIntoIndividualStatements(
                DriverTypeEnum.POSTGRES_9_4, listToPopulate, input);
        ...
        assertThat(
                ...
                contains(
                        "create sequence foo",
                        "alter table if exists CDR_XACT_LOG_STEP add constraint"
                                + " FK_XACTLOGSTEP_XACTLOG foreign key (LOG_PID) references"
                                + " CDR_XACT_LOG",
                        "ALTER TABLE hfj_resource SET (autovacuum_vacuum_scale_factor = 0.01)",
                        "ALTER TABLE hfj_forced_id SET (autovacuum_vacuum_scale_factor = 0.01)"));
    }
