...
public class DropForeignKeyTaskTest extends BaseTest {
	...
	@MethodSource("data")
	public void testDropForeignKey(Supplier<TestDatabaseDetails> theTestDatabaseDetails)
				throws SQLException {
		before(theTestDatabaseDetails);

		executeSql(
					"create table PARENT (PID bigint not null, TEXTCOL varchar(255), primary key"
								+ " (PID))");
		executeSql("create table CHILD (PID bigint not null, PARENTREF bigint)");
		executeSql(
					"alter table CHILD add constraint FK_MOM foreign key (PARENTREF) references"
								+ " PARENT(PID)");

		assertThat(
					JdbcUtils.getForeignKeys(getConnectionProperties(), "PARENT", "CHILD"), hasSize(1));
