...
public class AddIndexTaskTest extends BaseTest {
    ...
    @MethodSource("data")
    public void testUniqueConstraintAlreadyExists(
    ...
    @ParameterizedTest(name = "{index}: {0}")
    @MethodSource("data")
    public void testUniqueIndexAlreadyExists(Supplier<TestDatabaseDetails> theTestDatabaseDetails)
            throws SQLException {
        before(theTestDatabaseDetails);
    ...
    @ParameterizedTest(name = "{index}: {0}")
    @MethodSource("data")
    public void testNonUniqueIndexAlreadyExists(
            Supplier<TestDatabaseDetails> theTestDatabaseDetails) throws SQLException {
        before(theTestDatabaseDetails);
    ...
    public class SqlFeatures {
        ...
        public class IncludeColumns {
            ...
            public void testIncludeColumns() {
                ...
                assertEquals(
                        "create index IDX_ANINDEX on SOMETABLE(PID, TEXTCOL) INCLUDE (FOO1, FOO2)",
                        mySql);
        ...
        public class OnlineNoLocks {
            ...
            @Mock private MetadataSource mockMetadataSource;
            ...
            public void platformSyntaxWhenOn(DriverTypeEnum theDriver) {
                ...
                switch (theDriver) {
                    ...
                    case COCKROACHDB_21_1:
                        assertEquals(
                                "create index CONCURRENTLY IDX_ANINDEX on SOMETABLE(PID, TEXTCOL)",
                                mySql);
                        break;
                    case ORACLE_12C:
                        assertEquals(
                    ...
                    case MSSQL_2012:
                        assertEquals(
                                "create index IDX_ANINDEX on SOMETABLE(PID, TEXTCOL) WITH (ONLINE ="
                                        + " ON)",
                                mySql);
                        break;
                    default:
            ...
            public void offForUnsupportedVersionsOfOracleServer(boolean theSupportedFlag) {
                ...
                if (theSupportedFlag) {
                    assertEquals(
                            "create index IDX_ANINDEX on SOMETABLE(PID, TEXTCOL) ONLINE DEFERRED"
                                    + " INVALIDATION",
                            mySql);
                } else {
                    assertEquals("create index IDX_ANINDEX on SOMETABLE(PID, TEXTCOL)", mySql);
