...
public class DropTableTest extends BaseTest {
    ...
    @MethodSource("data")
    public void testDropExistingTable(Supplier<TestDatabaseDetails> theTestDatabaseDetails)
            throws SQLException {
    ...
    @ParameterizedTest(name = "{index}: {0}")
    @MethodSource("data")
    public void testDropTableWithForeignKey(Supplier<TestDatabaseDetails> theTestDatabaseDetails)
            throws SQLException {
        before(theTestDatabaseDetails);

        executeSql(
                "create table FOREIGNTABLE (PID bigint not null, TEXTCOL varchar(255), primary key"
                        + " (PID))");
        executeSql(
                "create table SOMETABLE (PID bigint not null, REMOTEPID bigint not null, primary"
                        + " key (PID))");
        executeSql(
                "alter table SOMETABLE add constraint FK_MYFK foreign key (REMOTEPID) references"
                        + " FOREIGNTABLE");
    ...
    @ParameterizedTest(name = "{index}: {0}")
    @MethodSource("data")
    public void testDropNonExistingTable(Supplier<TestDatabaseDetails> theTestDatabaseDetails)
            throws SQLException {
        before(theTestDatabaseDetails);
    ...
    @ParameterizedTest(name = "{index}: {0}")
    @MethodSource("data")
    public void testHapiMigrationResult(Supplier<TestDatabaseDetails> theTestDatabaseDetails)
            throws SQLException {
        before(theTestDatabaseDetails);
