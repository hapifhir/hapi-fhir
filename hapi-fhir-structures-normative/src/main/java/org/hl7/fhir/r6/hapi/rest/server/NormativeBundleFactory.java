 ...
 * #L%
 */
package org.hl7.fhir.r6.hapi.rest.server;
...
public class NormativeBundleFactory implements IVersionSpecificBundleFactory {
	private String myBase;
	private Bundle myBundle;
	private FhirContext myContext;

	public NormativeBundleFactory(FhirContext theContext) {
	...
	public void addResourcesToBundle(
		...
		for (IBaseResource next : theResult) {
			...
			Bundle.BundleEntryComponent entry = myBundle.addEntry().setResource((Resource) next);
		...
		for (IAnyResource next : includedResources) {
			Bundle.BundleEntryComponent entry = myBundle.addEntry();
	...
	private boolean hasLink(String theLinkType, Bundle theBundle) {
		for (Bundle.BundleLinkComponent next : theBundle.getLinkList()) {
			if (theLinkType.equals(next.getRelationElement().getCode())) {
				return true;
			}
		}
		return false;
	...
	private IIdType populateBundleEntryFullUrl(IBaseResource next, Bundle.BundleEntryComponent entry) {
	...
	public List<IBaseResource> toListOfResources() {
		...
		for (Bundle.BundleEntryComponent next : myBundle.getEntryList()) {
