...
public class CacheFactory {
	...
	private static synchronized <K, V> CacheProvider<K, V> getCacheProvider() {
		...
		throw new RuntimeException(
					Msg.code(2200)
								+ "No Cache Service Providers found. Choose between"
								+ " hapi-fhir-caching-caffeine (Default) and hapi-fhir-caching-guava"
								+ " (Android)");
	}
	...
	public static <K, V> LoadingCache<K, V> build(
				long theTimeoutMillis, CacheLoader<K, V> theCacheLoader) {
