...
public class Dstu2BundleFactoryTest {
    private static final org.slf4j.Logger ourLog =
            org.slf4j.LoggerFactory.getLogger(Dstu2BundleFactoryTest.class);
    ...
    public void setUp() throws Exception {
        ...
        diagnosticReport.setResult(
                Arrays.asList(new ResourceReferenceDt[] {new ResourceReferenceDt(observation)}));
        ...
        observation.setPerformer(
                Arrays.asList(new ResourceReferenceDt[] {new ResourceReferenceDt(practitioner)}));
    ...
    @Test
    public void whenIncludeIsAsterisk_bundle_shouldContainAllReferencedResources()
            throws Exception {
        Bundle bundle =
                makeBundle(BundleInclusionRule.BASED_ON_INCLUDES, includes(new String("*")));
    ...
    @Test
    public void whenIncludeIsDiagnosticReportSubject_bundle_shouldIncludePatient()
            throws Exception {
    ...
    @Test
    public void
            whenAChainedResourceIsIncludedAndItsParentIsAlsoIncluded_bundle_shouldContainTheChainedResource()
                    throws Exception {
        Bundle bundle =
                makeBundle(
                        BundleInclusionRule.BASED_ON_INCLUDES,
                        includes(
                                DiagnosticReport.INCLUDE_RESULT.getValue(),
                                Observation.INCLUDE_SPECIMEN.getValue()));
    ...
    @Test
    public void
            whenAChainedResourceIsIncludedButItsParentIsNot_bundle_shouldNotContainTheChainedResource()
                    throws Exception {
        Bundle bundle =
                makeBundle(
                        BundleInclusionRule.BASED_ON_INCLUDES,
                        includes(Observation.INCLUDE_SPECIMEN.getValue()));
    ...
    @Test
    public void whenBundleInclusionRuleSetToResourcePresence_bundle_shouldContainAllResources()
            throws Exception {
    ...
    Bundle makeBundle(BundleInclusionRule theBundleInclusionRule, Set<Include> theIncludes) {
        myBundleFactory.addResourcesToBundle(
                myResourceList, null, "http://foo", theBundleInclusionRule, theIncludes);
    ...
    private <T extends IResource> int numberOfEntriesOfType(
            Bundle theBundle, Class<T> theResourceClass) {
        int count = 0;
    ...
    private <T extends IResource> List<T> getResourcesOfType(
            Bundle theBundle, Class<T> theResourceClass) {
