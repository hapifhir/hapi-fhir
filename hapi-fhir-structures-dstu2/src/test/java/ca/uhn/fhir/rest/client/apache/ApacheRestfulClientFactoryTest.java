...
public class ApacheRestfulClientFactoryTest {
    ...
    public void testValidatateBase() {
        ...
        try {
            factory.validateServerBase(
                    "http://127.0.0.1:22225",
                    factory.getHttpClient("http://foo"),
                    (BaseClient) ctx.newRestfulGenericClient("http://foo"));
            fail();
        } catch (FhirClientConnectionException e) {
            assertEquals(
                    Msg.code(1357)
                            + "Failed to retrieve the server metadata statement during client"
                            + " initialization. URL used was http://127.0.0.1:22225metadata",
                    e.getMessage());
        }
    }
    ...
    @SetSystemProperty(key = "http.proxyHost", value = "hostFromSystemProperty")
    @SetSystemProperty(key = "http.proxyPort", value = "1234")
    public void testGetNativeHttpClientWithSystemAndProvidedProxy()
            throws IOException, HttpException {
        HttpRoute httpRoute =
                executeRequestAndCaptureHttpRoute(f -> f.setProxy("providedProxy", 0));
    ...
    private HttpRoute executeRequestAndCaptureHttpRoute(
            Consumer<ApacheRestfulClientFactory> factoryConsumer)
            throws IOException, HttpException {
