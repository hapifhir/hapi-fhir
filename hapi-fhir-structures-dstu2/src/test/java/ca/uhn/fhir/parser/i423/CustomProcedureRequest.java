...
public class CustomProcedureRequest extends ProcedureRequest {
    ...
    /** scheduled */
    @Child(
            name = FIELD_SCHEDULED,
            min = 0,
            max = 1,
            order = Child.REPLACE_PARENT,
            type = {CustomTimingDt.class})
    @Description(
            shortDefinition = "When procedure should occur",
            formalDefinition =
                    "The timing schedule for the proposed or ordered procedure. The Schedule data"
                        + " type allows many different expressions.  E.g. \"Every 8 hours\";"
    ...
    public CustomTimingDt _getScheduled() {
        if (ourScheduled == null) ourScheduled = new CustomTimingDt();
        return ourScheduled;
    }
