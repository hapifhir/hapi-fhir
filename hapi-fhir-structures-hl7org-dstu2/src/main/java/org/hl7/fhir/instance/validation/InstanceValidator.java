...
//      System.out.println(res.literalPath+" : "+b.toString());
  ...
  private void validateResource(List<ValidationMessage> errors, WrapperElement element, StructureDefinition profile, boolean needsId, NodeStack stack) throws Exception {
    ...
    boolean ok = true;
    if (element.isXml()) {
      ok = rule(errors, "invalid", element.line(), element.col(), "/", FormatUtilities.FHIR_NS.equals(element.getNamespace()), "Namespace mismatch - expected '"+FormatUtilities.FHIR_NS+"', found '"+element.getNamespace()+"'"); 
    }
      if (ok) {
