 ...
 *
 * <p>Note: This class is safe to extend, but it is important to note that the same instance of
 * {@link Conformance} is always returned unless {@link #setCache(boolean)} is called with a value
 * of <code>false</code>. This means that if you are adding anything to the returned conformance
 * instance on each call you should call <code>setCache(false)</code> in your provider constructor.
 */
public class ServerConformanceProvider extends BaseServerCapabilityStatementProvider
	...
	/**
	* No-arg constructor and seetter so that the ServerConfirmanceProvider can be Spring-wired with
	* the RestfulService avoiding the potential reference cycle that would happen.
	*/
	...
	*/
	...
	private Map<String, List<BaseMethodBinding>> collectMethodBindings(
		...
		Map<String, List<BaseMethodBinding>> resourceToMethods =
					new TreeMap<String, List<BaseMethodBinding>>();
		for (ResourceBinding next :
					getServerConfiguration(theRequestDetails).getResourceBindings()) {
	...
	* As this is a mandatory element, the value should not be null (although this is not enforced).
	* The value defaults to "Not provided" but may be set to null, which will cause this element to
	* be omitted.
	*/
	...
	public Conformance getServerConformance(
		...
		retVal.setAcceptUnknown(
					UnknownContentCode
								.EXTENSIONS); // TODO: make this configurable - this is a fairly big effort
		// since the parser
		...
		Map<String, List<BaseMethodBinding>> resourceToMethods =
					collectMethodBindings(theRequestDetails);
				...
				if (nextEntry.getKey().isEmpty() == false) {
					...
					ServletContext servletContext =
								(ServletContext)
										(theRequest == null
													? null
													: theRequest.getAttribute(
																RestfulServer.SERVLET_CONTEXT_ATTRIBUTE));
					String serverBase =
								serverConfiguration
										.getServerAddressStrategy()
										.determineServerBase(servletContext, theRequest);
					...
					for (BaseMethodBinding nextMethodBinding : nextEntry.getValue()) {
						...
						Collections.sort(
									resource.getInteraction(),
									new Comparator<ResourceInteractionComponent>() {
										@Override
										public int compare(
													ResourceInteractionComponent theO1,
													ResourceInteractionComponent theO2) {
	...
	private void handleSearchMethodBinding(
				ConformanceRestResourceComponent resource,
				RuntimeResourceDefinition def,
				...
				for (SearchParameter nextParameter : searchParameters) {
					...
					*/
					...
					for (Class<? extends IBaseResource> nextTarget : nextParameter.getDeclaredTypes()) {
						RuntimeResourceDefinition targetDef =
									getServerConfiguration(theRequestDetails)
												.getFhirContext()
	...
	public OperationDefinition readOperationDefinition(
				@IdParam IdType theId, RequestDetails theRequestDetails) {
		if (theId == null || theId.hasIdPart() == false) {
				throw new ResourceNotFoundException(Msg.code(1986) + theId);
		}
		List<OperationMethodBinding> sharedDescriptions =
					getServerConfiguration(theRequestDetails)
								.provideBindings()
		...
		for (OperationMethodBinding sharedDescription : sharedDescriptions) {
				...
				for (IParameter nextParamUntyped : sharedDescription.getParameters()) {
					if (nextParamUntyped instanceof OperationParameter) {
						...
						param.setMax(
									nextParam.getMax() == -1 ? "*" : Integer.toString(nextParam.getMax()));
	...
	/**
	* Sets the cache property (default is true). If set to true, the same response will be returned
	* for each invocation.
	*
	...
	*/
	...
	private void sortSearchParameters(List<SearchParameter> searchParameters) {
		Collections.sort(
					searchParameters,
					new Comparator<SearchParameter>() {
						@Override
