...
import org.hl7.fhir.dstu2.model.Conformance.ConformanceRestResourceSearchParamComponent;
import org.hl7.fhir.dstu2.model.Conformance.ConformanceStatementKind;
import org.hl7.fhir.dstu2.model.Conformance.ResourceInteractionComponent;
import org.hl7.fhir.dstu2.model.Conformance.RestfulConformanceMode;
...
public class ServerConformanceProvider extends BaseServerCapabilityStatementProvider
	...
	public Conformance getServerConformance(HttpServletRequest theRequest, RequestDetails theRequestDetails) {
		...
		retVal.setAcceptUnknown(
				UnknownContentCode
						.EXTENSIONS); // TODO: make this configurable - this is a fairly big effort since the parser
				...
				for (BaseMethodBinding nextMethodBinding : nextEntry.getValue()) {
					...
					if (nextMethodBinding instanceof SearchMethodBinding) {
						handleSearchMethodBinding(
								resource, def, includes, (SearchMethodBinding) nextMethodBinding, theRequestDetails);
			...
			for (SearchParameter nextParameter : searchParameters) {
				...
				if (nextParameter.getParamType() != null) {
					param.getTypeElement()
							.setValueAsString(nextParameter.getParamType().getCode());
				}
				for (Class<? extends IBaseResource> nextTarget : nextParameter.getDeclaredTypes()) {
					RuntimeResourceDefinition targetDef = getServerConfiguration(theRequestDetails)
							.getFhirContext()
	...
	public OperationDefinition readOperationDefinition(@IdParam IdType theId, RequestDetails theRequestDetails) {
		...
		List<OperationMethodBinding> sharedDescriptions = getServerConfiguration(theRequestDetails)
				.provideBindings()
