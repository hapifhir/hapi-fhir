...
public class GenericClientDstu2Hl7OrgTest {
	...
	public void testTransactionWithListOfResources() throws Exception {
		...
		org.hl7.fhir.dstu2.model.Bundle requestBundle =
				ourCtx.newJsonParser().parseResource(org.hl7.fhir.dstu2.model.Bundle.class, requestString);
		assertEquals(2, requestBundle.getEntry().size());
		assertEquals(
				"POST", requestBundle.getEntry().get(0).getRequest().getMethod().name());
		assertEquals(
				"PUT", requestBundle.getEntry().get(1).getRequest().getMethod().name());
		assertEquals(
				"http://example.com/Patient/2", requestBundle.getEntry().get(1).getFullUrl());
