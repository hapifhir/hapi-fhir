...
* Calls to fetch or validate codes, such as `validateCode(...)` and `lookupCode(...)` will first test each module in the chain using the`isCodeSystemSupported(...)` or `isValueSetSupported(...)` methods (depending on whether a ValueSet URL is present in the method parameters) and will invoke any methods in the chain which return that they can handle the given CodeSystem/ValueSet. When the caller named a version, that version is part of the question, so a module which holds specific versions, and holds this one only at a different version, is skipped rather than answering for it. The first non-null value returned by a method in the chain that can support it will be returned to the caller.
* A call to validate a code may name a code system version. Support is tested with that version: `isCodeSystemSupported(...)` receives it as a parameter of its own, and for a module which overrides only the signature without a version, the default implementation asks with the version packed into the URL as `url|version`, and if the module does not support that, with the URL on its own, so a module which recognises only the plain URL still takes part. The modules supplied with HAPI FHIR which hold specific versions answer only for the version named. A copy of the code system stored without a version does not answer for a named version, though a module which ships a single built-in definition of a code system, such as `CommonCodeSystemsTerminologyService` for UCUM and the other common code systems, answers for any version. If no module can answer for the named version but the code system itself is known, the result is an error stating that the version could not be found, as the HL7 validator reports it. A module which is asked, but does not override the `validateCode(...)` method that takes a `ValidateCodeRequest`, answers from whichever version it treats as current, because the default implementation of that method drops the version and calls the older signature.
