...
|-------|---------------|-----------|--------------|----------|
| Legacy only | `TokenIndexStrategy.of(EnumSet.of(LEGACY), LEGACY)` | `HFJ_SPIDX_TOKEN` | `HFJ_SPIDX_TOKEN` | Default. No migration in progress. |
| Write both, query legacy | `TokenIndexStrategy.of(EnumSet.of(LEGACY, COMPRESSED), LEGACY)` | Both tables | `HFJ_SPIDX_TOKEN` | Phase 1: Start populating compressed tables while maintaining query compatibility. |
| Write both, query compressed | `TokenIndexStrategy.of(EnumSet.of(LEGACY, COMPRESSED), COMPRESSED)` | Both tables | Compressed tables | Phase 2: Switch queries to compressed tables while still writing to both. |
| Compressed only | `TokenIndexStrategy.of(EnumSet.of(COMPRESSED), COMPRESSED)` | Compressed tables | Compressed tables | Migration complete. Can be used directly for fresh databases. |
...
## Migrating Existing Data to the Compressed Token Strategy

A fresh database can start directly with the **Compressed only** strategy. An existing database, however, already has token data in the `HFJ_SPIDX_TOKEN` table, and the compressed tables are only populated for resources that are written or updated after a strategy that writes to them is enabled. The following phased process back-fills the compressed tables and switches over without downtime or incomplete search results:

1. **Start writing to the compressed tables.** Set the strategy to **Write both, query legacy** (`TokenIndexStrategy.of(EnumSet.of(LEGACY, COMPRESSED), LEGACY)`). From this point, every newly created or updated resource is indexed into both the legacy and compressed tables. Searches are still served from `HFJ_SPIDX_TOKEN`.

2. **Back-fill existing resources.** Run a `$reindex` operation across all resources to populate the compressed tables for data that existed before step 1. The compressed tables are not complete until this job finishes, so reads must not be pointed at them before then.

3. **Read from the compressed tables.** Set the strategy to **Write both, query compressed** (`TokenIndexStrategy.of(EnumSet.of(LEGACY, COMPRESSED), COMPRESSED)`). Because writes still go to both tables, both indexes stay in sync, so this change can be rolled out across a cluster with no downtime: nodes that have not yet picked up the new configuration keep reading from `HFJ_SPIDX_TOKEN` and still return correct results. This phase also lets you validate the compressed tables under production load, and revert to step 1 in case of any issues.

4. **Stop writing to the legacy table.** Once the compressed tables are confirmed to be serving searches correctly, set the strategy to **Compressed only** (`TokenIndexStrategy.of(EnumSet.of(COMPRESSED), COMPRESSED)`). The `HFJ_SPIDX_TOKEN` table is no longer read from or written to, and its rows can be removed (for example by truncating the table) to reclaim storage.
...
If [Index Identifier Of Type](/hapi-fhir/apidocs/hapi-fhir-jpaserver-model/ca/uhn/fhir/jpa/model/entity/StorageSettings.html#setIndexIdentifierOfType(boolean)) is enabled on the server (it is disabled by default), `:of-type` token search parameters are always routed to the `HFJ_SPIDX2_TOKEN_IDENTIFIER` table, regardless of the `setIdentifierTokenSearchParams` setting above. 
This is because `:of-type` lookups rely on the `TYPE_HASH_SYS_AND_VALUE` column, which is part of the `HFJ_SPIDX2_TOKEN_IDENTIFIER` schema only, so this table is the sole index able to satisfy such queries.
