...
public class GenericClientExample {
	...
	public static void patchFhir() {
		...
		// Invoke the patch
		MethodOutcome outcome = client.patch().withFhirPatch(patch).withId("Patient/123").execute();
	...
	public static void fluentSearch() {
		...
		IGenericClient client =
					ctx.newRestfulGenericClient("http://fhir.healthintersections.com.au/open");
		{
				...
				MethodOutcome outcome2 =
						client.create()
									...
									.where(
												Patient.IDENTIFIER
														.exactly()
														.systemAndIdentifier("system", "00001"))
									.execute();
		...
		}
		{
				...
				for (OperationOutcome.OperationOutcomeIssueComponent nextIssue : oo.getIssue()) {
					if (nextIssue.getSeverity().ordinal()
								>= OperationOutcome.IssueSeverity.ERROR.ordinal()) {
		...
		}
		{
				...
				client.delete().resourceConditionalByUrl("Patient?identifier=system%7C00001").execute();
		...
		}
		{
				...
				response = client.search().byUrl(searchUrl).returnBundle(Bundle.class).execute();
				...
				response =
						client.search()
									...
									.returnBundle(Bundle.class)
									.elementsSubset(
												"identifier", "name") // only include the identifier and name
									.execute();
		...
		}
		{
				...
				Patient patient =
						client.read().resource(Patient.class).withIdAndVersion("123", "888").execute();
				// END SNIPPET: vread
		}
		{
	...
	public static void history() {
		IGenericClient client = FhirContext.forDstu2().newRestfulGenericClient("");
		{
				...
				Bundle response = client.history().onServer().returnBundle(Bundle.class).execute();
	...
	private static void operationHttpGet() {
		...
		FhirContext ctx = FhirContext.forDstu2();
		IGenericClient client =
					ctx.newRestfulGenericClient("http://fhir-dev.healthintersections.com.au/open");
	...
	private static void operation() {
		...
		System.out.println(
					ctx.newXmlParser().setPrettyPrint(true).encodeResourceToString(responseBundle));
