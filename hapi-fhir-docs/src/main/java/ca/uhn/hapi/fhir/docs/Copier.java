...
public class Copier {
	...
	public static void main(String[] args) {
		...
		while (queued.size() > 0) {
				...
				for (IBaseResource nextQueued : new ArrayList<>(queued)) {
					...
					for (ResourceReferenceInfo nextRefInfo :
								ctx.newTerser().getAllResourceReferences(nextQueued)) {
						String nextRef =
									nextRefInfo.getResourceReference().getReferenceElement().getValue();
	...
	private static void copy(
				...
				Set<String> theSent) {
		Bundle received =
					theSource.search().forResource(theResType).returnBundle(Bundle.class).execute();
	...
	private static void copy(
		...
		for (Bundle.BundleEntryComponent nextEntry : theReceived.getEntry()) {
				...
				for (ResourceReferenceInfo nextRefInfo :
						theCtx.newTerser().getAllResourceReferences(nextResource)) {
