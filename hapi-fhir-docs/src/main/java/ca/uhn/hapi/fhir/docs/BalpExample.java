...
public class BalpExample {
	...
	public class ExampleBalpAuditContextServices implements IBalpAuditContextServices {
		...
		/**
			* Here we are just hard-coding a simple display name. In a real implementation we should
			* use the actual identity of the requesting client.
			*/
		@Nonnull
		@Override
		...
		/**
			* Here we are just hard-coding a simple display name. In a real implementation we should
			* use the actual identity of the requesting user.
			*/
		@Nonnull
		@Override
	...
	public class MyServer extends RestfulServer {
		...
		protected void initialize() throws ServletException {
				...
				List<Object> clientInterceptors =
						List.of(
									// We'll register an auth interceptor against the sink FHIR client so
									// that
									// credentials get passed to the target server. Of course in a real
									// implementation
				...
				IBalpAuditEventSink eventSink =
						new AsyncMemoryQueueBackedFhirClientBalpSink(
									fhirContext, targetUrl, clientInterceptors);
		}
	}
	// END SNIPPET: server
}
