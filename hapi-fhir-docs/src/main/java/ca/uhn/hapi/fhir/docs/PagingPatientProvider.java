...
public class PagingPatientProvider implements IResourceProvider {
    ...
    public IBundleProvider search(@RequiredParam(name = Patient.SP_FAMILY) StringParam theFamily) {
        ...
        /**
         * First, we'll search the database for a set of database row IDs that match the given
         * search criteria. That way we can keep just the row IDs around, and load the actual
         * resources on demand later as the client pages through them.
         */
        final List<Long> matchingResourceIds = null; // <-- implement this

        /**
         * Return a bundle provider which can page through the IDs and return the resources that go
         * with them.
         */
