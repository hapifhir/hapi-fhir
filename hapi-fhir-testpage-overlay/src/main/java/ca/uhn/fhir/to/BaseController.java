...
public class BaseController {
	...
	private static final org.slf4j.Logger ourLog =
				org.slf4j.LoggerFactory.getLogger(BaseController.class);
	@Autowired protected TesterConfig myConfig;
	...
	@Autowired private ITemplateEngine myTemplateEngine;
		...
		for (int i = 0; i < str.length(); i++) {
				...
				if (!inParams) {
					if (nextChar == '?') {
						inParams = true;
						b.append(
									"</span><wbr /><span class='hlControl'>?</span><span"
												+ " class='hlTagName'>");
					} else {
	...
	protected RuntimeResourceDefinition getResourceType(
				HomeRequest theRequest, HttpServletRequest theReq) throws ServletException {
	...
	private CapabilityStatement loadAndAddConf(
		...
		GenericClient client =
					theRequest.newClient(
								theServletRequest, getContext(theRequest), myConfig, interceptor);
		...
		try {
				Class<? extends IBaseConformance> type =
						(Class<? extends IBaseConformance>)
				...
				theModel.put(
						"errorMsg",
						toDisplayError("Failed to load conformance statement, error was: " + ex, ex));
				fetchedCapabilityStatement = ctx.getResourceDefinition(name).newInstance();
		}

		theModel.put(
					"jsonEncodedConf",
					getContext(theRequest)
								.newJsonParser()
								.encodeResourceToString(fetchedCapabilityStatement));

		org.hl7.fhir.r5.model.CapabilityStatement capabilityStatement =
					getVersionCanonicalizer(theRequest)
								.capabilityStatementToCanonical(fetchedCapabilityStatement);
		...
		for (org.hl7.fhir.r5.model.CapabilityStatement.CapabilityStatementRestComponent nextRest :
				...
				for (org.hl7.fhir.r5.model.CapabilityStatement.CapabilityStatementRestResourceComponent
						nextResource : nextRest.getResource()) {
					List<org.hl7.fhir.r5.model.Extension> exts =
								nextResource.getExtensionsByUrl(RESOURCE_COUNT_EXT_URL);
					if (exts != null && exts.size() > 0) {
						Number nextCount =
									((org.hl7.fhir.r5.model.DecimalType) (exts.get(0).getValue()))
												.getValueAsNumber();
		...
		if (total > 0) {
				for (org.hl7.fhir.r5.model.CapabilityStatement.CapabilityStatementRestComponent
						nextRest : capabilityStatement.getRest()) {
					Collections.sort(
								nextRest.getResource(),
								(theO1, theO2) -> {
									org.hl7.fhir.r5.model.DecimalType count1 =
												new org.hl7.fhir.r5.model.DecimalType();
									List<org.hl7.fhir.r5.model.Extension> count1exts =
												theO1.getExtensionsByUrl(RESOURCE_COUNT_EXT_URL);
									if (count1exts != null && count1exts.size() > 0) {
										count1 =
													(org.hl7.fhir.r5.model.DecimalType)
																count1exts.get(0).getValue();
									}
									org.hl7.fhir.r5.model.DecimalType count2 =
												new org.hl7.fhir.r5.model.DecimalType();
									List<org.hl7.fhir.r5.model.Extension> count2exts =
												theO2.getExtensionsByUrl(RESOURCE_COUNT_EXT_URL);
	...
	private String parseNarrative(FhirContext theContext, IBaseResource theResult)
				throws Exception {
		String retVal = null;
					...
					IBaseResource firstResource =
								theContext
										...
										.getSingleValueOrNull(
													bundle, "Bundle.entry.resource", IBaseResource.class);
					if (firstResource != null
								&& "Composition".equals(theContext.getResourceType(firstResource))) {
						IBaseXhtml html =
									theContext
												.newTerser()
												.getSingleValueOrNull(
														firstResource, "text.div", IBaseXhtml.class);
						if (html != null) {
	...
	protected void processAndAddLastClientInvocation(
		...
		try {
				...
				if (lastResponse != null) {
					resultStatus =
								"HTTP " + lastResponse.getStatus() + " " + lastResponse.getStatusInfo();
				...
				Header[] requestHeaders =
						lastRequest != null
									? applyHeaderFilters(lastRequest.getAllHeaders())
									: new Header[0];
				Header[] responseHeaders =
						lastResponse != null
									? applyHeaderFilters(lastResponse.getAllHeaders())
									: new Header[0];
				...
				theModelMap.put(
						"errorMsg", toDisplayError("Error during processing: " + e.getMessage(), e));
		}
	}
