...
public class TesterConfig {
	public static final String SYSPROP_FORCE_SERVERS =
				"ca.uhn.fhir.to.TesterConfig_SYSPROP_FORCE_SERVERS";
	private static final org.slf4j.Logger ourLog =
				org.slf4j.LoggerFactory.getLogger(TesterConfig.class);
	...
	public void build() {
		for (ServerBuilder next : myServerBuilders) {
				...
				Validate.notNull(
						next.myVersion,
						"Found invalid server configuration - No FHIR version supplied");
				Validate.notBlank(
						next.myBaseUrl, "Found invalid server configuration - No base URL supplied");
				...
				myServerIdToTypeToOperationNameToInclusionChecker.put(
						next.myId, next.myOperationNameToInclusionChecker);
	...
	/**
	* If set to {@literal true} (default is true) the server will refuse to load URLs in response
	* payloads that refer to third party servers (e.g. paging URLs etc)
	*/
	...
	*/
	...
	public boolean isSearchResultRowInteractionEnabled(
		...
		IInclusionChecker inclusionChecker =
					interactionNamesToInclusionCheckers.getOrDefault(interaction, id -> false);
	...
	public interface IServerBuilderStep5 {
		...
		/**
			* If this is set, Thymeleaf UI templates will be run in debug mode, meaning no caching
			* between executions. This is helpful if you want to make live changes to the template.
			*
			* @since 6.4.0
			*/
		...
		ServerBuilder withSearchResultRowOperation(
					String theOperationName, IInclusionChecker theInclusionChecker);

		/**
			* Use this method to enable/disable the interaction buttons on the search result rows
			* table. By default {@link RestOperationTypeEnum#READ} and {@link
			* RestOperationTypeEnum#UPDATE} are already enabled, and they are currently the only
	...
	public class ServerBuilder
		...
		private final Map<String, IInclusionChecker> myOperationNameToInclusionChecker =
					new LinkedHashMap<>();
		private final Map<RestOperationTypeEnum, IInclusionChecker>
					mySearchResultRowInteractionEnabled = new LinkedHashMap<>();
		...
		@Override
		public ServerBuilder withSearchResultRowOperation(
					String theOperationName, IInclusionChecker theResourceType) {
