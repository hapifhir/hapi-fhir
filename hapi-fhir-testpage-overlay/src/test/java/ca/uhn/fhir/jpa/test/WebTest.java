...
public class WebTest {
	...
	public void before() throws Exception {
		...
		String baseUrl =
					"http://localhost:" + JettyUtil.getPortForStartedServer(ourOverlayServer) + "/";
	...
	public void testInvokeCustomOperation() throws IOException {
		...
		HtmlPage summaryPage =
					controlsCell.getElementsByTagName("button").stream()
		...
		assertThat(
					summaryPage.asNormalizedText(),
					containsString("Result Narrative\t\nHELLO WORLD DOCUMENT"));
	}
	...
	public void testInvokeCustomOperation_Validate() throws IOException {
		...
		assertThat(
					summaryPage.asNormalizedText(),
					containsString("\"diagnostics\": \"VALIDATION FAILURE\""));
	}
	...
	private static class MyServletContextHandler extends ServletContextHandler {
		...
		public MyServletContextHandler() {
				super();
				_scontext =
						new ContextHandler.Context() {
								@Override
								public URL getResource(String thePath) {
									File parent =
												new File("hapi-fhir-testpage-overlay/src/main/webapp")
														.getAbsoluteFile();
									if (!parent.exists()) {
