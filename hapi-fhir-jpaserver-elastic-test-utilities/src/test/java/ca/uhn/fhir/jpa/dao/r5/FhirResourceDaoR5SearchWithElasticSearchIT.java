...
import ca.uhn.fhir.jpa.config.TestR5ConfigWithElasticHSearch;
...
@RequiresDocker
@ContextHierarchy({
	@ContextConfiguration(classes = TestR5ConfigWithElasticHSearch.class),
...
@TestExecutionListeners(listeners = {
	...
	, FhirResourceDaoR5SearchWithElasticSearchIT.TestDirtiesContextTestExecutionListener.class
})
public class FhirResourceDaoR5SearchWithElasticSearchIT extends BaseJpaTest implements ITestDataBuilder {
	public static final String URL_MY_CODE_SYSTEM = "http://example.com/my_code_system";
	public static final String URL_MY_VALUE_SET = "http://example.com/my_value_set";
	private static final org.slf4j.Logger ourLog = org.slf4j.LoggerFactory.getLogger(FhirResourceDaoR5SearchWithElasticSearchIT.class);
	...
	public void testResourceTextSearch() {
		...
		obs1.setValue(new Quantity(123));
		obs1.getNoteFirstRep().setText("obs1");
		obs1.getText().setDivAsString("<div xmlns=\"http://www.w3.org/1999/xhtml\">My custom search term. zoop 123\n" + "          example with only plain text</div>");
		...
		obs2.setStatus(Enumerations.ObservationStatus.FINAL);
		obs2.setValue(new Quantity(81));
		obs2.getText().setDivAsString("<div xmlns=\"http://www.w3.org/1999/xhtml\">My custom search term.\n" + "          example with only plain text</div>");
		...
		map = new SearchParameterMap();
		map.add(Constants.PARAM_TEXT, new StringParam("zoop 123"));
		assertThat(toUnqualifiedVersionlessIdValues(myObservationDao.search(map))).containsExactlyInAnyOrder(toValues(id1));

		map = new SearchParameterMap();
		map.add(Constants.PARAM_TEXT, new StringParam("123"));
		assertThat(toUnqualifiedVersionlessIdValues(myObservationDao.search(map))).containsExactlyInAnyOrder(toValues(id1));

		map = new SearchParameterMap();
		map.add(Constants.PARAM_TEXT, new StringParam("custom"));
		assertThat(toUnqualifiedVersionlessIdValues(myObservationDao.search(map))).containsExactlyInAnyOrder(toValues(id1, id2));
	}
	...
	public void testResourceCodeTokenSearch() {
		...
		}
		{
			...
			obs2b.setStatus(Enumerations.ObservationStatus.FINAL);
		...
		}
		{
			...
			obs3.setStatus(Enumerations.ObservationStatus.FINAL);
	...
	public void testResourceReferenceSearchForCanonicalReferences() {
		String questionnaireCanonicalUrl = "https://test.fhir.org/R5/Questionnaire/xl-5000-q";
...
//		{
//			Observation obs4 = new Observation();
//			obs4.setStatus(Enumerations.ObservationStatus.FINAL);
...
//			obs5.setStatus(Enumerations.ObservationStatus.FINAL);
...
//			obs6.setStatus(Enumerations.ObservationStatus.FINAL);
...
//	}
	...
	public void testExpandWithIsAInExternalValueSet() {
		...
		include.addFilter().setOp(Enumerations.FilterOperator.ISA).setValue("childAA").setProperty("concept");
