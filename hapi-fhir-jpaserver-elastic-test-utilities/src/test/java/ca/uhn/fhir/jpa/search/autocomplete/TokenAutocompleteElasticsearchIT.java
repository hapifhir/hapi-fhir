...
public class TokenAutocompleteElasticsearchIT extends BaseJpaTest {
    ...
    @Autowired IFhirSystemDao<?, ?> mySystemDao;
    @Autowired IResourceReindexingSvc myResourceReindexingSvc;
    @Autowired private IBulkDataExportJobSchedulingHelper myBulkDataScheduleHelper;
    @Autowired ITestDataBuilder myDataBuilder;

    // a few different codes
    static final Coding mean_blood_pressure =
            new Coding("http://loinc.org", "8478-0", "Mean blood pressure");
    static final Coding gram_positive_culture =
            new Coding(
                    "http://loinc.org",
                    "88262-1",
    ...
    public void testAutocompleteByCodeDisplay() {
        ...
        createObservationWithCode(
                new Coding("http://loinc.org", "787-2", "MCV [Entitic volume] by Automated count"));
        createObservationWithCode(
                new Coding("http://loinc.org", "786-4", "MCHC [Mass/volume] by Automated count"));
        createObservationWithCode(
                new Coding("http://loinc.org", "785-6", "MCH [Entitic mass] by Automated count"));
        ...
        createObservationWithCode(
                new Coding("http://loinc.org", "59032-3", "Lactate [Mass/volume] in Blood"));
        createObservationWithCode(
                new Coding(
        ...
        assertThat(
                "finds blood pressure",
                codes,
                Matchers.hasItem(matchingSystemAndCode(mean_blood_pressure)));
        ...
        assertThat(
                "empty finds most common first",
                codes.get(0),
                matchingSystemAndCode(gram_positive_culture));
        assertThat(
                "empty finds most common first",
                codes.get(1),
                matchingSystemAndCode(mean_blood_pressure));

        codes = autocompleteSearch("Observation", "code", null, "88262-1");
        assertThat(
                "matches by code value",
                codes,
                Matchers.hasItem(matchingSystemAndCode(gram_positive_culture)));

        codes = autocompleteSearch("Observation", "code", null, "8826");
        assertThat(
                "matches by code prefix",
                codes,
                Matchers.hasItem(matchingSystemAndCode(gram_positive_culture)));
    ...
    /**
     * If an observation has multiple codes, make sure searching by text only matches the right
     * code.
     */
    @Test
    public void testAutocompleteDistinguishesMultipleCodes() {
        ...
        createObservationWithCode(
                erythrocyte_by_volume, mean_blood_pressure, gram_positive_culture);
        createObservationWithCode(gram_positive_culture);
        createObservationWithCode(mean_blood_pressure);
                ...
                .execute(
                        s -> {
                            TokenAutocompleteSearch tokenAutocompleteSearch =
                                    new TokenAutocompleteSearch(
                                            myFhirCtx,
                                            myStorageSettings,
                                            Search.session(myEntityManager));
                            return tokenAutocompleteSearch.search(
                                    theResourceType, theSPName, theSearchText, theModifier, 30);
                        });
    }
    ...
    private Matcher<TokenAutocompleteHit> matchingSystemAndCode(IBaseCoding theCoding) {
        return new TypeSafeDiagnosingMatcher<TokenAutocompleteHit>() {
            private final String mySystemAndCode =
                    theCoding.getSystem() + "|" + theCoding.getCode();

            @Override
            protected boolean matchesSafely(
                    TokenAutocompleteHit item, Description mismatchDescription) {
