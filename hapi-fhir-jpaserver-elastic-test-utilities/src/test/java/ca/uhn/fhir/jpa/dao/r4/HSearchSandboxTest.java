...
public class HSearchSandboxTest extends BaseJpaTest {
    ...
    @Autowired private ITestDataBuilder myTestDataBuilder;

    @Autowired private IResourceReindexingSvc myResourceReindexingSvc;
    ...
    public class NotNestedObjectQueries {
        /**
         * Show that when there is only one and clause with "or" entries, we can add the shoulds at
         * the top level
         */
        @Test
        public void searchModelingMultipleAndWithOneOringClauseTest() {
            ...
            runInTransaction(
                    () -> {
                        ...
                        SearchResult<ResourceTable> result =
                                searchSession
        ...
        /**
         * Shows that when there is multiple "and" clause with "or" entries, we need to group each
         * one in a "must" clause to be able to add a minimumShouldMatchNumber(1); to each group
         */
        @Test
    ...
    public class NestedObjectQueries {
        ...
        public void searchModelingAndMultipleAndWithOneOringClauseTest() {
            IIdType myResourceId =
                    myTestDataBuilder.createObservation(
                            myTestDataBuilder.withElementAt(
                                    "valueQuantity",
                                    myTestDataBuilder.withPrimitiveAttribute("value", 0.6)
                                    //			myTestDataBuilder.withPrimitiveAttribute("system",
                                    // UCUM_CODESYSTEM_URL),
                                                                                    ...
                                                                                    .nest(
                                                                                            f.bool()
                                                                                                    .must(
                                                                                                            f.range()
                                                                                                                    .field(
                                                                                                                            "nsp.value-quantity.quantity.value")
                                                                                                                    .lessThan(
                                                                                                                            0.7))
                                                                                                    .should(
                                                                                                            f.range()
        ...
        public void searchModelingMultipleAndWithMultipleOrClausesTest() {
            ...
            //
            //								.must(f.bool(p -> {
            //									p.should(f.range().field("nsp.value-quantity.quantity.value").between(0.475,
            // 0.525));
            //									p.should(f.range().field("nsp.value-quantity.quantity.value").between(0.57,
            // 0.63));
            //									p.minimumShouldMatchNumber(1);
            //								}))
            //
            //								.must(f.bool(p -> {
            //									p.should(f.range().field("nsp.value-quantity.quantity.value").between(0.2,
            // 0.8));
            //									p.should(f.range().field("nsp.value-quantity.quantity.value").between(0.7,
            // 0.9));
            //									p.minimumShouldMatchNumber(1);
            //								}))
            //
     ...
     * to try to achieve the clean query structure modeled by previous tests, but using generic
     * methods
     */
    @Nested
    public class FragmentedCodeNotNested {
        ...
        public void searchModelingMultipleAndOneOrClauseTest() {
            ...
            theQuantityAndOrTerms.add(
                    Lists.newArrayList(
                            new QuantityParam().setValue(0.5), new QuantityParam().setValue(0.6)));
                    ...
                    () -> {
                        ...
                        SearchResult<ResourceTable> result =
                                searchSession
                                        ...
                                        .where(
                                                f -> {
                                                    TestPredBuilder builder =
                                                            new TestPredBuilder(f);
                                                    return builder.buildAndOrPredicates(
                                                            paramName, theQuantityAndOrTerms);
                                                })
                                        .fetchAll();
                        long totalHitCount = result.total().hitCount();
                        //			List<ResourceTable> hits = result.hits();
        ...
        public void searchModelingMultipleAndMultipleOrClauseTest() {
            ...
            theQuantityAndOrTerms.add(
                    Lists.newArrayList(
                            new QuantityParam().setValue(0.9), new QuantityParam().setValue(0.6)));
    ...
    private static class TestPredBuilder {
        ...
        public PredicateFinalStep buildAndOrPredicates(
            ...
            // we need to know if there is more than one "and" predicate (outer list) with more than
            // one "or" predicate (inner list)
            ...
            topBool.must(
                    myPredicateFactory.match().field("myResourceType").matching("Observation"));
            ...
            for (List<IQueryParameterType> andTerm : theAndOrTerms) {
                ...
                if (maxOrPredicateSize <= 1) {
                    ...
                    // so all of them need to be separated in groups with a
                    // minimumShouldMatchNumber(1)
                    activeBool.must(
                            myPredicateFactory.bool(
                                    p -> {
                                        for (IQueryParameterType orTerm : andTerm) {
        ...
        private void addQuantityOrClauses(
            ...
            String fieldPath =
                    NESTED_SEARCH_PARAM_ROOT + "." + theSearchParamName + "." + INDEX_TYPE_QUANTITY;

            QuantityParam qtyParam = QuantityParam.toQuantityParam(theParamType);
            ParamPrefixEnum activePrefix =
                    qtyParam.getPrefix() == null ? ParamPrefixEnum.EQUAL : qtyParam.getPrefix();
        ...
        private void addQuantityTerms(
            ...
            PredicateFinalStep rangePred =
                    getPrefixedRangePredicate(theActivePrefix, theQtyParam, valueFieldPath);
            ...
            if (isNotBlank(theQtyParam.getSystem())) {
                addFieldPredicate(
                        theIsMust,
                        theTopBool,
                        theFieldPath + "." + QTY_SYSTEM,
                        theQtyParam.getSystem());
            }
            ...
            if (isNotBlank(theQtyParam.getUnits())) {
                addFieldPredicate(
                        theIsMust,
                        theTopBool,
                        theFieldPath + "." + QTY_CODE,
                        theQtyParam.getUnits());
            }
        }
        ...
        private void addMustOrShould(
                boolean theIsMust,
                BooleanPredicateClausesStep<?> theTopBool,
                PredicateFinalStep thePredicate) {
            if (theIsMust) {
        ...
        private PredicateFinalStep getPrefixedRangePredicate(
            ...
            switch (thePrefix) {
                ...
                case LESSTHAN_OR_EQUALS:
                    ...
                    // NOT_EQUAL: searches for resource quantity not between passed param value +/-
                    // 5%
                case NOT_EQUAL:
