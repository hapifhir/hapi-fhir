...
public class PointcutLatch implements IAnonymousInterceptor, IPointcutLatch {
    ...
    public void setExpectedCount(int theCount, boolean theExactMatch) {
        if (myPointcutLatchSession.get() != null) {
            ...
            throw new PointcutLatchException(
                    Msg.code(1480)
                            + "setExpectedCount() called before previous awaitExpected() completed."
                            + " Previous set stack:\n"
                            + previousStack,
                    myName);
        }
    ...
    public List<HookParams> awaitExpectedWithTimeout(int theTimeoutSecond)
        ...
        try {
            if (isSet()) {
                ...
                throw new PointcutLatchException(
                        "awaitExpected() called before setExpected() called.", myName);
            }
        } finally {
            clear();
        }
    ...
    public void invoke(IPointcut thePointcut, HookParams theArgs) {
        ...
        try {
            ...
            if (session == null) {
                throw new PointcutLatchException(
                        Msg.code(1485)
                                + "invoke() called outside of setExpectedCount() .."
                                + " awaitExpected().  Probably got more invocations than expected"
                                + " or clear() was called before invoke().",
                        myName,
                        theArgs);
            }
    ...
    public static <T> T getInvocationParameterOfType(
            List<HookParams> theHookParams, Class<T> theType) {
    ...
    public static Object getLatchInvocationParameter(List<HookParams> theHookParams, int index) {
        ...
        Validate.isTrue(
                arg.values().size() == 1, "Expected pointcut to be invoked with 1 argument");
