...
public class TlsAuthenticationTestHelper implements AfterEachCallback {
	...
	public TlsAuthenticationTestHelper() {
		...
		myTrustStoreInfo =
					new TrustStoreInfo(
								TRUSTSTORE_RESOURCE_PATH, TRUSTSTORE_STOREPASS, TRUSTSTORE_ALIAS);
		myTlsAuthentication =
					new TlsAuthentication(Optional.of(myKeystoreInfo), Optional.of(myTrustStoreInfo));
	}
	...
	public String[] createBaseRequestGeneratingCommandArgs(
				...
				String theUrlFlag,
				boolean theAddTls,
				BaseRestServerHelper theRestServerHelper) {
	...
	public File createTlsAuthenticationFile() {
		try {
				...
				try (FileWriter inputFileWriter =
						new FileWriter(inputFile, StandardCharsets.UTF_8, false)) {
					IOUtils.write(json.toString(), inputFileWriter);
				}
				return inputFile;
		} catch (Exception e) {
				throw new RuntimeException(
						Msg.code(2122) + "Failed to load test TLS authentication file", e);
		}
	}
}
