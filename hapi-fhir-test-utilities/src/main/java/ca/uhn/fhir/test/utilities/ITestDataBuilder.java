...
/**
 * This is an experiment to see if we can make test data creation for storage unit tests a bit more
 * readable.
 */
...
public interface ITestDataBuilder {
    ...
    static void __setPrimitiveChild(
        ...
        BaseRuntimeElementCompositeDefinition def =
                (BaseRuntimeElementCompositeDefinition)
                        theFhirContext.getElementDefinition(theTarget.getClass());
    ...
    default ICreationArgument withFamily(String theFamily) {
        return t -> {
            ...
            BaseRuntimeElementCompositeDefinition<?> humanNameDef =
                    (BaseRuntimeElementCompositeDefinition<?>)
                            getFhirContext().getElementDefinition("HumanName");
            ...
            BaseRuntimeElementCompositeDefinition resourceDef =
                    (BaseRuntimeElementCompositeDefinition)
                            getFhirContext().getElementDefinition(t.getClass());
    ...
    default ICreationArgument withEffectiveDate(String theDate) {
        return t ->
                __setPrimitiveChild(getFhirContext(), t, "effectiveDateTime", "dateTime", theDate);
    }
    ...
    default ICreationArgument withIdentifier(String theSystem, String theValue) {
        return t -> {
            IPrimitiveType<?> system =
                    (IPrimitiveType<?>) getFhirContext().getElementDefinition("uri").newInstance();
            ...
            BaseRuntimeElementCompositeDefinition<?> identifierDef =
                    (BaseRuntimeElementCompositeDefinition<?>)
                            getFhirContext().getElementDefinition("Identifier");
            ...
            RuntimeResourceDefinition resourceDef =
                    getFhirContext()
                            .getResourceDefinition((Class<? extends IBaseResource>) t.getClass());
    ...
    default ICreationArgument withLastUpdated(String theIsoDate) {
        return t ->
                ((IBaseResource) t)
                        .getMeta()
                        .setLastUpdated(new InstantType(theIsoDate).getValue());
    }
    ...
    default IIdType createResource(String theResourceType, ICreationArgument... theModifiers) {
        ...
        if (ourLog.isDebugEnabled()) {
            ourLog.debug(
                    "Creating {}",
                    getFhirContext().newJsonParser().encodeResourceToString(resource));
        }
    ...
    default <T extends IBaseResource> T buildResource(
            String theResourceType, ICreationArgument... theModifiers) {
    ...
    @Nonnull
    default ICreationArgument withReference(
            String theReferenceName, @Nullable IIdType theReferenceValue) {
        return t -> {
            if (theReferenceValue != null && theReferenceValue.getValue() != null) {
                ...
                RuntimeResourceDefinition resourceDef =
                        getFhirContext().getResourceDefinition((IBaseResource) t);
    ...
    default <E extends IBase> ICreationArgument withElementAt(
            String thePath, Consumer<E>... theModifiers) {
        return t -> {
            FhirTerser terser = getFhirContext().newTerser();
    ...
    default ICreationArgument withQuantityAtPath(
            String thePath, Number theValue, String theSystem, String theCode) {
    ...
    default ICreationArgument withObservationCode(
            @Nullable String theSystem, @Nullable String theCode) {
