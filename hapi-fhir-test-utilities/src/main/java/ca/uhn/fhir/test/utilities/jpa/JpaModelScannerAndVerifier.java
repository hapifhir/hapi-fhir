...
/**
 * This class is only used at build-time. It scans the various Hibernate entity classes and enforces
 * various rules (appropriate table names, no duplicate names, etc.)
 */
public class JpaModelScannerAndVerifier {
    ...
    private static final org.slf4j.Logger ourLog =
            org.slf4j.LoggerFactory.getLogger(JpaModelScannerAndVerifier.class);
    // Exceptions set because H2 sets indexes for FKs automatically so this index had to be called
    // as the target FK field
    // it is indexing to avoid SchemaMigrationTest to complain about the extra index (which doesn't
    // exist in H2)
    ...
    public void scanEntities(String... thePackageNames) throws IOException, ClassNotFoundException {
        ...
        for (String packageName : thePackageNames) {
            ImmutableSet<ClassPath.ClassInfo> classes =
                    ClassPath.from(JpaModelScannerAndVerifier.class.getClassLoader())
    ...
    private void scanClass(Set<String> theNames, Class<?> theClazz) {
        ...
        if (table != null) {
            ...
            // This is the length for MySQL per
            ...
            for (UniqueConstraint nextIndex : table.uniqueConstraints()) {
                int indexLength =
                        calculateIndexLength(
                                nextIndex.columnNames(), columnNameToLength, nextIndex.name());
                if (indexLength > maxIndexLength) {
                    throw new IllegalStateException(
                            Msg.code(1624)
                                    + "Index '"
                                    + nextIndex.name()
                                    + "' is too long. Length is "
                                    + indexLength
                                    + " and must not exceed "
                                    + maxIndexLength
                                    + " which is the maximum MySQL length");
                }
            }
        }
    }
    ...
    private void scanClassOrSuperclass(
            ...
            Map<String, Integer> columnNameToLength) {
        ...
        for (Field nextField : theClazz.getDeclaredFields()) {
                ...
                if (Long.class.equals(nextField.getType())) {
                    ...
                    if (generatedValue != null) {
                        Validate.notBlank(
                                ...
                                "Field has no @GeneratedValue.generator(): %s",
                                nextField);
                        ...
                        GenericGenerator genericGenerator =
                                nextField.getAnnotation(GenericGenerator.class);
                        SequenceGenerator sequenceGenerator =
                                nextField.getAnnotation(SequenceGenerator.class);
                        ...
                        if (genericGenerator != null) {
                            ...
                            assertEquals(
                                    generatedValue.generator(), sequenceGenerator.sequenceName());
                        }
                    }
                }
            }
            ...
            if (!isTransient) {
                ...
                boolean isOtherSideOfOneToManyMapping =
                        oneToMany != null && isNotBlank(oneToMany.mappedBy());
                boolean isOtherSideOfOneToOneMapping =
                        oneToOne != null && isNotBlank(oneToOne.mappedBy());
                boolean isField =
                        nextField.getAnnotation(
                                        org.hibernate.search.mapper.pojo.mapping.definition
                                                .annotation.FullTextField.class)
                                != null;
                isField |=
                        nextField.getAnnotation(
                                        org.hibernate.search.mapper.pojo.mapping.definition
                                                .annotation.GenericField.class)
                                != null;
                isField |=
                        nextField.getAnnotation(
                                        org.hibernate.search.mapper.pojo.mapping.definition
                                                .annotation.ScaledNumberField.class)
                                != null;
                Validate.isTrue(
                        hasEmbedded
                                || hasColumn
                ...
                if (columnName != null) {
                    if (nextField.getType().isAssignableFrom(String.class)) {
                        // MySQL treats each char as the max possible byte count in UTF-8 for its
                        // calculations
    ...
    private void scan(
        ...
        if (table != null) {
            ...
            for (UniqueConstraint nextConstraint : table.uniqueConstraints()) {
                ...
                Validate.isTrue(
                        ...
                        nextConstraint.name() + " must start with IDX_");
            }
            for (Index nextConstraint : table.indexes()) {
                ...
                Validate.isTrue(
                        ...
                        nextConstraint.name()
                                + " must start with IDX_ or FK_ (last one when indexing a FK"
                                + " column)");
            }
        }
        ...
        if (joinColumn != null) {
            ...
            if (theIsSuperClass) {
                Validate.isTrue(
                        ...
                        "Foreign key on "
                                + theAnnotatedElement
                                + " has a name() and should not as it is a superclass");
            } else {
                Validate.notNull(fk);
                Validate.isTrue(
                        isNotBlank(fk.name()),
                        "Foreign key on " + theAnnotatedElement + " has no name()");
        ...
        if (column != null) {
            ...
            Validate.isTrue(
                    column.unique() == false,
                    "Should not use unique attribute on column (use named @UniqueConstraint"
                            + " instead) on "
                            + theAnnotatedElement);
            ...
            if (field.getType().equals(String.class)) {
                if (!hasLob) {
                    if (!theIsView && column.length() == 255) {
                        throw new IllegalStateException(
                                Msg.code(1626)
                                        + "Field does not have an explicit maximum length"
                                        + " specified: "
                                        + field);
                    }
                    if (column.length() > MAX_COL_LENGTH) {
                        throw new IllegalStateException(
                                Msg.code(1627) + "Field is too long: " + field);
                    }
                }
                ...
                if (size != null) {
                    if (size.max() > MAX_COL_LENGTH) {
                        throw new IllegalStateException(
                                Msg.code(1628) + "Field is too long: " + field);
                    }
                }
                ...
                if (length != null) {
                    if (length.max() > MAX_COL_LENGTH) {
                        throw new IllegalStateException(
                                Msg.code(1629) + "Field is too long: " + field);
                    }
                }
            }
        }
        ...
        if (theColumnName.startsWith("_")) {
            throw new IllegalArgumentException(
                    Msg.code(2272)
                            + "Column name "
                            + theColumnName
                            + " starts with an '_' (underscore). This is not permitted for oracle"
                            + " field names. Found on "
                            + theElement);
        }
    }
    ...
    private static int calculateIndexLength(
            ...
            Map<String, Integer> theColumnNameToLength,
            String theIndexName) {
        int retVal = 0;
