...
public class CascadingDeleteInterceptorMultiThreadTest {
	...
	@Autowired DelayListener myDelayListener;
	...
	public void before() throws Exception {
		...
		if (ourServer == null) {
				...
				GenericWebApplicationContext ourWebApplicationContext =
	...
	private boolean deletePractitioner(CloseableHttpClient theCloseableHttpClient)
		...
		try (CloseableHttpResponse response = theCloseableHttpClient.execute(delete)) {
				if (response.getStatusLine().getStatusCode() != 200) {
					ourLog.error(
								"Unexpected status on practitioner delete = "
	...
	private boolean deleteOrganization(CloseableHttpClient theCloseableHttpClient)
		...
		try (CloseableHttpResponse response = theCloseableHttpClient.execute(delete)) {
				if (response.getStatusLine().getStatusCode() != 200) {
					ourLog.error(
								"Unexpected status on organization delete = "
										+ response.getStatusLine().getStatusCode());
					return false;
				}
				String deleteResponse =
