...
public class Batch2JobInstanceRepositoryTest extends BaseJpaR4Test {
    ...
    @Autowired IBatch2JobInstanceRepository myBatch2JobInstanceRepository;

    @ParameterizedTest
    @CsvSource({
        "QUEUED, FAILED, QUEUED, true, normal transition",
        "IN_PROGRESS, FAILED, QUEUED IN_PROGRESS ERRORED, true, normal transition with multiple"
                + " prior",
    ...
    void updateInstance_toState_fromState_whenAllowed(
        ...
        Set<StatusEnum> theAllowedPriorStates =
                Arrays.stream(theAllowedPriorStatesString.trim().split(" +"))
        ...
        int changeCount =
                runInTransaction(
                        () ->
                                myBatch2JobInstanceRepository.updateInstanceStatusIfIn(
                                        jobId, theTargetState, theAllowedPriorStates));

        // then
        Batch2JobInstanceEntity readBack =
                runInTransaction(() -> myBatch2JobInstanceRepository.findById(jobId).orElseThrow());
