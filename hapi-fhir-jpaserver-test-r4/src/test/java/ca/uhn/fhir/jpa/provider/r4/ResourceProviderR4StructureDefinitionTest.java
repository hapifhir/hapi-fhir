...
public class ResourceProviderR4StructureDefinitionTest extends BaseResourceProviderR4Test {
	...
	private static final Logger ourLog =
				LoggerFactory.getLogger(ResourceProviderR4StructureDefinitionTest.class);

	@Test
	public void testSearchAllStructureDefinitions() throws IOException {
		StructureDefinition sd =
					loadResource(myFhirContext, StructureDefinition.class, "/r4/sd-david-dhtest7.json");
		...
		assertEquals(
					"dhtest7", response.getEntry().get(0).getResource().getIdElement().getIdPart());
	}
		...
		StructureDefinition response =
					myClient.operation()
								...
								.withParameter(
										...
										"url",
										new StringType(
													"http://example.com/fhir/StructureDefinition/patient-1a-extensions"))
								.returnResourceType(StructureDefinition.class)
								.execute();
		assertEquals(54, response.getSnapshot().getElement().size());
	}
