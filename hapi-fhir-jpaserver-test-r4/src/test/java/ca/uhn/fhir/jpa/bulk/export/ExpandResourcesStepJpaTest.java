...
public class ExpandResourcesStepJpaTest extends BaseJpaR4Test {
	...
	public void testBulkExportExpandResourcesStep(
		...
		resourceList.setIds(ids.stream()
				.map(t -> new BatchResourceId().setResourceType("Patient").setId(Long.toString(t)))
				.toList());
	...
	public void testPostFetchFiltering() {
		...
		for (int i = 0; i < 5; i++) {
			...
			Long matchingId =
					myPatientDao.create(matchingPatient, mySrd).getId().getIdPartAsLong();
		...
		ResourceIdList resourceList = new ResourceIdList();
		resourceList.setResourceType("Patient");
		resourceList.setIds(allIds.stream()
				.map(t -> new BatchResourceId().setResourceType("Patient").setId(Long.toString(t)))
				.toList());
		...
		// Verify
		verify(mySink, times(1)).accept(myWorkChunkCaptor.capture());
		List<Long> resourceIds = myWorkChunkCaptor.getValue().getStringifiedResources().stream()
				.map(t -> myFhirContext
						.newJsonParser()
						.parseResource(t)
						.getIdElement()
