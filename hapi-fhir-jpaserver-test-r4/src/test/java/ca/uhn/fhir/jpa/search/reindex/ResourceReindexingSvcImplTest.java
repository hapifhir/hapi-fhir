...
public class ResourceReindexingSvcImplTest {
	...
	@Mock private IForcedIdDao myForcedIdDao;
	@Mock private IResourceReindexJobDao myReindexJobDao;
	@Mock private IResourceTableDao myResourceTableDao;
	@Mock private IFhirResourceDao myResourceDao;
	@Captor private ArgumentCaptor<Long> myIdCaptor;
	@Captor private ArgumentCaptor<PageRequest> myPageRequestCaptor;
	@Captor private ArgumentCaptor<String> myTypeCaptor;
	@Captor private ArgumentCaptor<Date> myLowCaptor;
	@Captor private ArgumentCaptor<Date> myHighCaptor;
	private ResourceReindexJobEntity mySingleJob;
	@Mock private ISearchParamRegistry mySearchParamRegistry;
	@Mock private TransactionStatus myTxStatus;

	@InjectMocks
	private final ResourceReindexer myResourceReindexer = new ResourceReindexer(ourFhirContext);

	@InjectMocks private final ResourceReindexingSvcImpl mySvc = new ResourceReindexingSvcImpl();
	...
	public void testReindexPassOnlyReturnsValuesAtLowThreshold() {
		...
		// Run the second pass, which should index no resources (meaning it's time to mark as
		// deleted)
		mySvc.forceReindexingPass();
		verify(myResourceTableDao, never())
					.findIdsOfResourcesWithinUpdatedRangeOrderedFromOldest(any(), any(), any(), any());
		verify(myReindexJobDao, never()).markAsDeletedById(any());
		verify(myResourceTableDao, times(1))
					.findIdsOfResourcesWithinUpdatedRangeOrderedFromOldest(
								myPageRequestCaptor.capture(),
								myLowCaptor.capture(),
								myHighCaptor.capture());
		...
		verify(myReindexJobDao, times(1))
					.setThresholdLow(eq(123L), eq(new Date((40 * DateUtils.MILLIS_PER_DAY) + 1L)));
	}
	...
	public void testMarkAsDeletedIfNothingIndexed() {
		...
		when(myResourceTableDao.findIdsOfResourcesWithinUpdatedRangeOrderedFromOldest(
								any(), any(), any()))
					.thenReturn(new SliceImpl<>(values));
	...
	public void testReindexPassAllResources() {
		...
		verify(myReindexJobDao, times(1))
					.setThresholdLow(myIdCaptor.capture(), myLowCaptor.capture());
	...
	public void testReindexPassPatients() {
		...
		String[] resourceTypes = new String[] {"Patient", "Patient", "Patient", "Patient"};
		...
		when(myDaoRegistry.getResourceDao(eq("Patient"))).thenReturn(myResourceDao);
		when(myDaoRegistry.getResourceDao(eq(Patient.class))).thenReturn(myResourceDao);
		when(myResourceDao.readByPid(any(), anyBoolean()))
					.thenAnswer(
								t -> {
	...
	public void testReindexDeletedResource() {
		...
		when(myResourceTableDao.findIdsOfResourcesWithinUpdatedRangeOrderedFromOldest(
					...
					.thenReturn(new SliceImpl<>(values));
		// Mock fetching resources
		long[] updatedTimes = new long[] {10 * DateUtils.MILLIS_PER_DAY};
		String[] resourceTypes =
					new String[] {
						"Patient",
					};
		List<IBaseResource> resources = Arrays.asList(new Patient().setId("Patient/0/_history/1"));
		mockWhenResourceTableFindById(updatedTimes, resourceTypes);
		when(myDaoRegistry.getResourceDao(eq("Patient"))).thenReturn(myResourceDao);
		...
		verify(myResourceTableDao, times(1))
					.updateIndexStatus(eq(0L), eq(BaseHapiFhirDao.INDEX_STATUS_INDEXING_FAILED));
	}
	...
	public void testReindexThrowsError() {
		...
		when(myResourceTableDao.findIdsOfResourcesWithinUpdatedRangeOrderedFromOldest(
								...
								myHighCaptor.capture()))
					.thenReturn(new SliceImpl<>(values));
		when(myResourceTableDao.findById(anyLong()))
					.thenThrow(new NullPointerException("A MESSAGE"));
	...
	private void mockWhenResourceTableFindById(long[] theUpdatedTimes, String[] theResourceTypes) {
		when(myResourceTableDao.findById(any()))
					.thenAnswer(
								t -> {
	...
	private void mockFetchFourResources() {
		...
		String[] resourceTypes = new String[] {"Patient", "Patient", "Observation", "Observation"};
		List<IBaseResource> resources =
					Arrays.asList(
								new Patient().setId("Patient/0/_history/1"),
								new Patient().setId("Patient/1/_history/1"),
