...
public abstract class BaseComboParamsR4Test extends BaseJpaR4Test {
    ...
    public void before() throws Exception {
        ...
        when(myInterceptorBroadcaster.callHooks(
                ...
                .thenAnswer(
                        t -> {
                            ...
                            myMessages.add(
                                    "INFO "
                                            + params.get(StorageProcessingMessage.class)
                                                    .getMessage());
                            return null;
                        });
        when(myInterceptorBroadcaster.callHooks(
        ...
        when(myInterceptorBroadcaster.callHooks(
                        eq(Pointcut.JPA_PERFTRACE_SEARCH_REUSING_CACHED),
    ...
    public void after() throws Exception {
        ...
        myStorageSettings.setUniqueIndexesEnabled(
                new JpaStorageSettings().isUniqueIndexesEnabled());
        myStorageSettings.setReindexThreadCount(new JpaStorageSettings().getReindexThreadCount());

        ResourceReindexingSvcImpl svc =
                SpringObjectCaster.getTargetObject(
                        myResourceReindexingSvc, ResourceReindexingSvcImpl.class);
        svc.initExecutor();
    }
