...
public class FhirResourceDaoR4SearchSqlTest extends BaseJpaR4Test {
    ...
    private static final Logger ourLog =
            LoggerFactory.getLogger(FhirResourceDaoR4SearchSqlTest.class);
    ...
    public void testSingleRegularSearchParam() {
        ...
        SearchParameterMap map =
                SearchParameterMap.newSynchronous(Patient.SP_NAME, new StringParam("FOO"));
        ...
        String sql =
                myCaptureQueriesListener
                        ...
                        .getSql(false, false);
        assertEquals(
                "SELECT t0.RES_ID FROM HFJ_SPIDX_STRING t0 WHERE ((t0.HASH_NORM_PREFIX = ?) AND"
                        + " (t0.SP_VALUE_NORMALIZED LIKE ?))",
                sql);
    }
    ...
    public void testTwoRegularSearchParams() {
        ...
        String sql =
                myCaptureQueriesListener
                        ...
                        .getSql(false, false);
        assertEquals(
                "SELECT t1.RES_ID FROM HFJ_RESOURCE t1 INNER JOIN HFJ_SPIDX_STRING t0 ON (t1.RES_ID"
                    + " = t0.RES_ID) INNER JOIN HFJ_SPIDX_TOKEN t2 ON (t1.RES_ID = t2.RES_ID) WHERE"
                    + " (((t0.HASH_NORM_PREFIX = ?) AND (t0.SP_VALUE_NORMALIZED LIKE ?)) AND"
                    + " (t2.HASH_SYS_AND_VALUE = ?))",
                sql);
    }
    ...
    public void testSearchByProfile_VersionedMode() {
        ...
        String sql =
                myCaptureQueriesListener
                        ...
                        .getSql(false, false);
        assertEquals(
                "SELECT t0.RES_ID FROM HFJ_RESOURCE t0 INNER JOIN HFJ_RES_TAG t1 ON (t0.RES_ID ="
                    + " t1.RES_ID) INNER JOIN HFJ_TAG_DEF t2 ON (t1.TAG_ID = t2.TAG_ID) WHERE"
                    + " (((t0.RES_TYPE = ?) AND (t0.RES_DELETED_AT IS NULL)) AND ((t2.TAG_TYPE = ?)"
                    + " AND (t2.TAG_SYSTEM = ?) AND (t2.TAG_CODE = ?)))",
                sql);
    ...
    public void testSearchByProfile_InlineMode() {
        ...
        boolean reindexParamCache =
                myStorageSettings.isMarkResourcesForReindexingUponSearchParameterChange();
        myStorageSettings.setMarkResourcesForReindexingUponSearchParameterChange(false);

        // 		SearchParameter searchParameter =
        // FhirResourceDaoR4TagsTest.createSearchParamForInlineResourceProfile();
        SearchParameter searchParameter =
                FhirResourceDaoR4TagsInlineTest.createSearchParameterForInlineProfile();
        ourLog.debug(
                "SearchParam:\n{}",
                myFhirContext
                        .newJsonParser()
