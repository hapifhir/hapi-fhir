...
public class SearchCoordinatorSvcImplTest extends BaseJpaR4Test {
    ...
    @Autowired private ISearchCoordinatorSvc mySearchCoordinator;

    @Autowired private ISearchCacheSvc myDatabaseCacheSvc;
    ...
    public void testDeleteDontMarkPreviouslyMarkedSearchesAsDeleted() {
        ...
        runInTransaction(
                () -> {
                    for (int i = 0; i < 20; i++) {
                        ...
                        // Add a bunch of search results to a few (enough that it will take multiple
                        // passes)
                        if (i < 3) {
                            for (int j = 0; j < 10; j++) {
        ...
        runInTransaction(
                () -> {
                    // We should delete up to 10, but 3 don't get deleted since they have too many
                    // results to delete in one pass
        ...
        runInTransaction(
                () -> {
                    // Once again we attempt to delete 10, but the first 3 don't get deleted and
                    // still remain
                    // (total is 6 because 3 weren't deleted, and they blocked another 3 that might
                    // have been)
