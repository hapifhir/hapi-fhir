...
public class FhirResourceDaoR4TagsTest extends BaseResourceProviderR4Test {
	...
	private static final org.slf4j.Logger ourLog =
				org.slf4j.LoggerFactory.getLogger(FhirResourceDaoR4TagsTest.class);
	...
	public void testDeleteResourceWithTags_NonVersionedTags() {
		...
		assertThat(
					toTags(meta).toString(),
					toTags(meta),
					containsInAnyOrder("http://tag1|vtag1|dtag1", "http://tag2|vtag2|dtag2"));
		...
		assertThat(
					toProfiles(patient).toString(),
					toProfiles(patient),
					containsInAnyOrder("http://profile3"));
		assertThat(
					toTags(patient).toString(),
					toTags(patient),
					containsInAnyOrder("http://tag1|vtag1|dtag1", "http://tag2|vtag2|dtag2"));
	...
	public void testStoreAndRetrieveNonVersionedTags_Read() {
		...
		patient = myPatientDao.read(new IdType("Patient/A"), mySrd);
		assertThat(
					toProfiles(patient).toString(), toProfiles(patient), contains("http://profile2"));
		assertThat(
					toTags(patient).toString(),
					toTags(patient),
					containsInAnyOrder("http://tag1|vtag1|dtag1", "http://tag2|vtag2|dtag2"));
	...
	public void testStoreAndRetrieveVersionedTags_VRead() {
		...
		assertThat(
					toProfiles(patient).toString(), toProfiles(patient), contains("http://profile1"));
		assertThat(
					toTags(patient).toString(), toTags(patient), contains("http://tag1|vtag1|dtag1"));
	...
	public void testInlineTags_StoreAndRetrieve() {
		...
		assertThat(
					...
					contains("http://sec1|vsec1|dsec1"));
		...
		assertThat(
					toTags(patient).toString(),
					toTags(patient),
					containsInAnyOrder("http://tag2|vtag2|dtag2"));
		assertThat(
					toSecurityLabels(patient).toString(),
					toSecurityLabels(patient),
	...
	public void testInlineTags_Search_Tag() {
		...
		ourLog.debug(
					...
					myFhirContext
								.newJsonParser()
								.setPrettyPrint(true)
								.encodeResourceToString(searchParameter));
					...
					myClient.search()
								...
								.where(
										new TokenClientParam("_tag")
													.exactly()
													.systemAndCode("http://tag1", "vtag1"))
								.returnBundle(Bundle.class)
								.execute();
		assertThat(
					toUnqualifiedVersionlessIdValues(outcome),
					containsInAnyOrder("Patient/A", "Patient/B"));
	...
	@Test
	public void
				testMetaDelete_TagStorageModeNonVersioned_ShouldShowRemainingTagsInGetAllResources() {
	...
	public void testInlineTags_Search_Security() {
		...
		Bundle outcome =
					myClient.search()
								.forResource("Patient")
								.where(
										new TokenClientParam("_security")
													.exactly()
													.systemAndCode("http://sec1", "vsec1"))
								.returnBundle(Bundle.class)
								.execute();
		assertThat(
					toUnqualifiedVersionlessIdValues(outcome),
	...
	public static SearchParameter createSecuritySearchParameter(FhirContext fhirContext) {
		...
		for (String next :
					fhirContext.getResourceTypes().stream().sorted().collect(Collectors.toList())) {
	...
	static List<String> toTags(Meta meta) {
		return meta.getTag().stream()
					.map(t -> t.getSystem() + "|" + t.getCode() + "|" + t.getDisplay())
					.collect(Collectors.toList());
	}

	@Nonnull
	static List<String> toSecurityLabels(Meta meta) {
		return meta.getSecurity().stream()
					.map(t -> t.getSystem() + "|" + t.getCode() + "|" + t.getDisplay())
					.collect(Collectors.toList());
	}
	...
	public static SearchParameter createSearchParamForInlineResourceProfile() {
		...
		for (String next :
					FhirContext.forR4Cached().getResourceTypes().stream()
								.sorted()
								.collect(Collectors.toList())) {
				searchParameter.addBase(next);
		}
