...
public class MultitenantBatchOperationR4Test extends BaseMultitenantResourceProviderR4Test {
    private static final Logger ourLog =
            LoggerFactory.getLogger(MultitenantBatchOperationR4Test.class);
    ...
    public void after() throws Exception {
        ...
        myStorageSettings.setDeleteExpungeEnabled(
                new JpaStorageSettings().isDeleteExpungeEnabled());
    ...
    public void testReindexEverything() {
        ReindexTestHelper reindexTestHelper =
                new ReindexTestHelper(myFhirContext, myDaoRegistry, mySearchParamRegistry);
        ...
        StringType jobId =
                (StringType)
                        response.getParameterValue(
                                ProviderConstants.OPERATION_REINDEX_RESPONSE_JOB_ID);

        myBatch2JobHelper.awaitJobCompletion(jobId.getValue());

        ourLog.info(
                "Search params: {}",
                mySearchParamRegistry.getActiveSearchParams("Observation").getSearchParamNames());
        logAllTokenIndexes();
    ...
    public void testReindexByUrl() {
        ...
        IIdType obsFinalA =
                doCreateResource(
                        reindexTestHelper.buildObservationWithAlleleExtension(
                                Observation.ObservationStatus.FINAL));
        IIdType obsCancelledA =
                doCreateResource(
                        reindexTestHelper.buildObservationWithAlleleExtension(
        ...
        Parameters input = new Parameters();
        input.addParameter(
                ProviderConstants.OPERATION_REINDEX_PARAM_URL, "Observation?status=final");
