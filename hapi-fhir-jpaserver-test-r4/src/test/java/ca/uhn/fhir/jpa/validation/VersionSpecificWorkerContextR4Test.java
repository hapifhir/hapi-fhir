...
public class VersionSpecificWorkerContextR4Test extends BaseResourceProviderR4Test {
	...
	static {
		// override resource cache expireAfterWrite timeout to emulate cache expiration
		HapiSystemProperties.setTestValidationResourceCachesMs(1);
	}

	@AfterAll
	static void afterAll() {
		// reset validation resource cache expireAfterWrite timeout to default value configured in BaseJpaTest
	...
	public class ValidateWithCachePositiveTest {
		@BeforeEach
		public void before() {
			ValidationSupportChain chain = new ValidationSupportChain(myValidationSupport);
