...
public class PagingMultinodeProviderR4Test extends BaseResourceProviderR4Test {
	...
	/**
	* @param theUseCacheBoolean - true if we're using offset search, false if we're using paging id
	*/
	@ParameterizedTest
	...
	public void testSearch(boolean theUseCacheBoolean) {
		...
		found = myClient.loadPage().next(found).cacheControl(directive).execute();
		assertThat(toUnqualifiedVersionlessIdValues(found), contains(resultsPages[index++]));
		...
		while (index > 0) {
				...
				found = myClient.loadPage().previous(found).cacheControl(directive).execute();
				assertThat(toUnqualifiedVersionlessIdValues(found), contains(resultsPages[--index]));
		}
	}
}
