...
public class ReindexTestHelper {
	...
	public static final String EYECOLOUR_EXTENSION_URL =
				"http://hl7.org/fhir/StructureDefinition/patient-eyeColour";
	...
	private Patient buildPatientWithEyeColourExtension(boolean theActive) {
		...
		p.addExtension()
					.setUrl(EYECOLOUR_EXTENSION_URL)
					.setValue(new StringType(TEST_EYECOLOUR_VALUE));
	...
	@Nonnull
	public Observation buildObservationWithAlleleExtension(
				Observation.ObservationStatus theStatus) {
	...
	public List<String> getEyeColourPatientIds(String theCode, String theIdentifier) {
		...
		ourLog.info(
					"Searching for Patients with url {}", map.toNormalizedQueryString(myFhirContext));
	...
	public List<String> getAlleleObservationIds(String theCode, String theIdentifier) {
		...
		ourLog.info(
					"Searching for Observations with url {}",
	...
	public List<String> getAlleleObservationIds(IGenericClient theClient) {
		IBaseBundle result =
					theClient
								...
								.where(
										new StringClientParam(ALLELE_SP_CODE)
													.matches()
													.value(TEST_ALLELE_VALUE))
