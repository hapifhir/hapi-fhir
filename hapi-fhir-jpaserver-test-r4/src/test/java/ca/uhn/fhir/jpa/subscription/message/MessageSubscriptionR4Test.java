...
public class MessageSubscriptionR4Test extends BaseSubscriptionsR4Test {
	@Autowired private SubscriptionChannelFactory myChannelFactory;
	...
	public void beforeRegisterRestHookListener() {
		...
		IChannelReceiver receiver =
					myChannelFactory.newMatchingReceivingChannel(
								"my-queue-name", new ChannelConsumerSettings());
	...
	public void testUpdateResourceRetainCorrectMetaTagsThroughDelivery() throws Exception {
		...
		ourLog.info(
					getFhirContext()
								.newJsonParser()
								.setPrettyPrint(true)
								.encodeResourceToString(receivedPatient));
		assertThat(receivedTagList.size(), is(equalTo(3)));
		List<String> actual =
					receivedTagList.stream()
								.map(t -> t.getCode())
								.sorted()
