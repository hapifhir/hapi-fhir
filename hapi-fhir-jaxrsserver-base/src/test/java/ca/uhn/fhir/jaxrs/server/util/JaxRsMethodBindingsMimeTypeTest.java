...
public class JaxRsMethodBindingsMimeTypeTest {
	...
	public void testFindMethodsFor2ProvidersWithMethods() {
		assertThat(new TestJaxRsDummyPatientProviderR4().getBindings().getBinding(RestOperationTypeEnum.SEARCH_TYPE, "").getBindingKey()).contains(AbstractDummyPatientProvider.class.getName());
		assertThat(new TestJaxRsDummyPatientProviderR4MimeType().getBindings().getBinding(RestOperationTypeEnum.SEARCH_TYPE, "").getBindingKey()).contains(AbstractDummyPatientProvider.class.getName());
	}
