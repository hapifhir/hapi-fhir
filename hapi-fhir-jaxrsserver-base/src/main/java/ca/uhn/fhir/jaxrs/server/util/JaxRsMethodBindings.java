...
public class JaxRsMethodBindings {
    ...
    private static final ConcurrentHashMap<Class<?>, JaxRsMethodBindings> classBindings =
            new ConcurrentHashMap<Class<?>, JaxRsMethodBindings>();

    /** Static collection of operationBindings mapped to a class */
    private ConcurrentHashMap<RestOperationTypeEnum, ConcurrentHashMap<String, BaseMethodBinding>>
            operationBindings =
                    new ConcurrentHashMap<
                            RestOperationTypeEnum, ConcurrentHashMap<String, BaseMethodBinding>>();
     ...
     */
    public JaxRsMethodBindings(
            AbstractJaxRsProvider theProvider,
            Class<? extends AbstractJaxRsProvider> theProviderClass) {
        List<Method> declaredMethodsForCurrentProvider =
                ReflectionUtil.getDeclaredMethods(theProviderClass);
        declaredMethodsForCurrentProvider.addAll(
                ReflectionUtil.getDeclaredMethods(theProviderClass.getSuperclass()));
        for (final Method m : declaredMethodsForCurrentProvider) {
            final BaseMethodBinding foundMethodBinding =
                    BaseMethodBinding.bindMethod(m, theProvider.getFhirContext(), theProvider);
    ...
    private void addMethodBinding(String key, BaseMethodBinding binding) {
        ConcurrentHashMap<String, BaseMethodBinding> mapByOperation =
                getMapForOperation(binding.getRestOperationType());
        if (mapByOperation.containsKey(key)) {
            throw new IllegalArgumentException(
                    Msg.code(597)
                            + "Multiple Search Method Bindings Found : "
                            + mapByOperation.get(key)
                            + " -- "
                            + binding.getMethod());
        }
    ...
    /**
     * Get the map for the given operation type. If no map exists for this operation type, create a
     * new hashmap for this operation type and add it to the operation bindings.
     *
    ...
    private ConcurrentHashMap<String, BaseMethodBinding> getMapForOperation(
        ...
        if (result == null) {
            operationBindings.putIfAbsent(
                    operationType, new ConcurrentHashMap<String, BaseMethodBinding>());
    ...
    /**
     * Get the method bindings for the given class. If this class is not yet contained in the
     * classBindings, they will be added for this class
     *
     ...
     */
    public static JaxRsMethodBindings getMethodBindings(
            AbstractJaxRsProvider theProvider,
            Class<? extends AbstractJaxRsProvider> theProviderClass) {
        if (!getClassBindings().containsKey(theProviderClass)) {
            JaxRsMethodBindings foundBindings =
                    new JaxRsMethodBindings(theProvider, theProviderClass);
