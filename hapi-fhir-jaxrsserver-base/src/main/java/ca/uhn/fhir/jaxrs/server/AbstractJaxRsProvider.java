...
public abstract class AbstractJaxRsProvider implements IRestfulServerDefaults {
    ...
    /** the http headers */
    @Context private HttpHeaders myHeaders;

    /** the uri info */
    @Context private UriInfo myUriInfo;

    /**
     * Default is DSTU2. Use {@link AbstractJaxRsProvider#AbstractJaxRsProvider(FhirContext)} to
     * specify a DSTU3 context.
     */
    ...
    /**
     * This method returns the server base, including the resource path. {@link UriInfo#getBaseUri()
     * UriInfo#getBaseUri()}
     *
    ...
    public String getBaseForServer() {
        ...
        return StringUtils.isNotBlank(url) && url.endsWith("/")
    ...
    /**
     * Default: an empty list of interceptors (Interceptors are not yet supported in the JAX-RS
     * server). Please get in touch if you'd like to help!
     *
    ...
    public Map<String, String[]> getParameters() {
        ...
        for (final Entry<String, List<String>> paramEntry : queryParameters.entrySet()) {
            params.put(
                    paramEntry.getKey(),
                    paramEntry.getValue().toArray(new String[paramEntry.getValue().size()]));
        }
        return params;
    }
    ...
    public Builder getRequest(
            final RequestTypeEnum requestType, final RestOperationTypeEnum restOperation) {
    ...
    public Builder getRequest(
        ...
        return new JaxRsRequest.Builder(
                ...
                requestType,
                restOperation,
                myUriInfo.getRequestUri().toString(),
                theResourceName);
    }

    /**
     * This method returns the default server address strategy. The default strategy return the base
     * uri for the request {@link AbstractJaxRsProvider#getBaseForRequest() getBaseForRequest()}
     *
     * @return
     */
        ...
        if (theException instanceof JaxRsResponseException) {
            return new JaxRsExceptionInterceptor()
                    .convertExceptionIntoResponse(
                            theRequest, (JaxRsResponseException) theException);
        } else {
            return new JaxRsExceptionInterceptor()
                    .convertExceptionIntoResponse(
                            theRequest,
