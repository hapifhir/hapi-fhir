 ...
 * {@link CapabilityStatement} is always returned unless {@link #setCache(boolean)} is called with a
 * value of <code>false</code>. This means that if you are adding anything to the returned
 * conformance instance on each call you should call <code>setCache(false)</code> in your provider
 * constructor.
 */
public class ServerCapabilityStatementProvider extends BaseServerCapabilityStatementProvider
	...
	private static final org.slf4j.Logger ourLog =
				org.slf4j.LoggerFactory.getLogger(ServerCapabilityStatementProvider.class);
	private String myPublisher = "Not provided";

	/**
	* No-arg constructor and setter so that the ServerConformanceProvider can be Spring-wired with
	...
	*/
	...
	public CapabilityStatement getServerConformance(
		...
		retVal.setAcceptUnknown(
					UnknownContentCode
								.EXTENSIONS); // TODO: make this configurable - this is a fairly big
		...
		for (Entry<String, List<BaseMethodBinding>> nextEntry : resourceToMethods.entrySet()) {
				...
				if (nextEntry.getKey().isEmpty() == false) {
					...
					// Map<String, CapabilityStatement.RestResourceSearchParam> nameToSearchParam = new
					// HashMap<String,
					...
					for (BaseMethodBinding nextMethodBinding : nextEntry.getValue()) {
						...
						if (nextMethodBinding instanceof SearchMethodBinding) {
								...
								if (methodBinding.getQueryName() != null) {
									String queryName =
												bindings.getNamedSearchMethodBindingToName().get(methodBinding);
									if (operationNames.add(queryName)) {
										rest.addOperation()
													.setName(methodBinding.getQueryName())
													.setDefinition(
																new Reference("OperationDefinition/" + queryName));
									}
								} else {
									handleNamelessSearchMethodBinding(
												rest,
												resource,
												resourceName,
												def,
								...
								if (operationNames.add(opName)) {
									...
									rest.addOperation()
												.setName(methodBinding.getName().substring(1))
												.setDefinition(new Reference("OperationDefinition/" + opName));
								}
						}
	...
	private void handleNamelessSearchMethodBinding(
				CapabilityStatementRestComponent rest,
		...
		if (!searchParameters.isEmpty()) {
				...
				// query.addUndeclaredExtension(false, ExtensionConstants.QUERY_RETURN_TYPE, new
				// CodeDt(resourceName));
				// for (String nextInclude : searchMethodBinding.getIncludes()) {
				// query.addUndeclaredExtension(false, ExtensionConstants.QUERY_ALLOWED_INCLUDE, new
				// StringDt(nextInclude));
				// }
				// }
				...
				for (SearchParameter nextParameter : searchParameters) {
					...
					*/
					...
					//					for (String nextWhitelist : new
					// TreeSet<String>(nextParameter.getQualifierWhitelist())) {
	...
	public OperationDefinition readOperationDefinition(
		...
		List<OperationMethodBinding> operationBindings =
					bindings.getOperationIdToBindings().get(theId.getIdPart());
	...
	private OperationDefinition readOperationDefinitionForNamedSearch(
		...
		for (SearchMethodBinding binding : bindings) {
				...
				for (IParameter nextParamUntyped : binding.getParameters()) {
					if (nextParamUntyped instanceof SearchParameter) {
						...
						param.getSearchTypeElement()
									.setValueAsString(nextParam.getParamType().getCode());
	...
	private OperationDefinition readOperationDefinitionForOperation(
				List<OperationMethodBinding> bindings) {
		OperationDefinition op = new OperationDefinition();
		op.setStatus(PublicationStatus.ACTIVE);
				...
				for (IParameter nextParamUntyped : sharedDescription.getParameters()) {
					if (nextParamUntyped instanceof OperationParameter) {
						...
						if (nextParam.getSearchParamType() != null) {
								param.getSearchTypeElement()
										.setValueAsString(nextParam.getSearchParamType());
						}
