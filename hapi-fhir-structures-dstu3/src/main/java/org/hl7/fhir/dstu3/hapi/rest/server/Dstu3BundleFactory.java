	...
	public void addResourcesToBundle(
		...
		for (IBaseResource next : theResult) {
					...
					switch (theBundleType) {
						...
						case TRANSACTION_RESPONSE:
								...
								if (isNotBlank(id.getVersionIdPart())) {
									entry.getResponse()
												.setEtag(RestfulServerUtils.createEtag(id.getVersionIdPart()));
								}
								break;
					}
				}
				...
				BundleEntrySearchModeEnum searchMode =
						ResourceMetadataKeyEnum.ENTRY_SEARCH_MODE.get(nextAsResource);
				if (searchMode != null) {
	...
	public void addRootPropertiesToBundle(
		...
		if (myBundle.getMeta().getLastUpdated() == null && theLastUpdated != null) {
				myBundle.getMeta()
						...
						.setValueAsString(theLastUpdated.getValueAsString());
		}
