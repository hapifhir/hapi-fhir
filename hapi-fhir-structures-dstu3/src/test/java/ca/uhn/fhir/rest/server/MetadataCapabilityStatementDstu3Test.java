...
public class MetadataCapabilityStatementDstu3Test {
		...
		try {
				...
				assertEquals(
						"<OperationOutcome xmlns=\"http://hl7.org/fhir\"><issue><severity"
									...
									+ Msg.code(388)
									+ "/metadata request must use HTTP GET\"/></issue></OperationOutcome>",
						output);
		} finally {
				IOUtils.closeQuietly(status.getEntity().getContent());
		}
	...
	public void testResponseContainsBaseUrl() throws Exception {
		...
		HttpRequestBase httpPost =
					new HttpGet("http://localhost:" + ourPort + "/metadata?_format=json");
		CloseableHttpResponse status = ourClient.execute(httpPost);
		try {
				output = IOUtils.toString(status.getEntity().getContent(), StandardCharsets.UTF_8);
				assertEquals(200, status.getStatusLine().getStatusCode());
				...
				CapabilityStatement cs =
						ourCtx.newJsonParser().parseResource(CapabilityStatement.class, output);
	...
	public static void beforeClass() throws Exception {
		...
		ourServlet.setServerConformanceProvider(
					new ServerCapabilityStatementProvider(ourServlet).setCache(false));
