...
public class SearchClientDstu3Test {
	...
	public void testSortForDstu3() throws Exception {
		...
		assertThat(capt.getAllValues().get(idx++).getURI().toString()).isEqualTo("http://localhost/fhir/Bundle?_sort=param1");

		client.search(new SortSpec("param1", SortOrderEnum.ASC).setChain(new SortSpec("param2", SortOrderEnum.DESC)));
		assertThat(capt.getAllValues().get(idx++).getURI().toString()).isEqualTo("http://localhost/fhir/Bundle?_sort=param1%2C-param2");
	}
	...
	public void testSearchWithPrimitiveTypes() throws Exception {
		...
		try {
			...
			assertThat(UrlUtil.unescape(capt.getAllValues().get(idx++).getURI().toString())).isEqualTo("http://localhost/fhir/Bundle?stringParam=STRING1&stringTypeParam=STRING2&dateParam=1970-10-04T10:23:55.986-04:00&calParam=1970-10-04T10:23:55.986-04:00");

			client.search(null, null, null, null);
			assertThat(UrlUtil.unescape(capt.getAllValues().get(idx++).getURI().toString())).isEqualTo("http://localhost/fhir/Bundle");
		} finally {
