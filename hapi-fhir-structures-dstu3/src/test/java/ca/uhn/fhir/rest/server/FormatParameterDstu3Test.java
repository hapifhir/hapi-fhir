...
public class FormatParameterDstu3Test {
	...
	private static final String VALUE_XML =
				"<Patient xmlns=\"http://hl7.org/fhir\"><id value=\"p1ReadId\"/><meta><profile"
						+ " value=\"http://foo_profile\"/></meta><identifier><value"
						+ " value=\"p1ReadValue\"/></identifier></Patient>";
	private static final String VALUE_JSON =
				"{\"resourceType\":\"Patient\",\"id\":\"p1ReadId\",\"meta\":{\"profile\":[\"http://foo_profile\"]},\"identifier\":[{\"value\":\"p1ReadValue\"}]}";
	private static CloseableHttpClient ourClient;
	private static FhirContext ourCtx = FhirContext.forDstu3();
	private static final org.slf4j.Logger ourLog =
				org.slf4j.LoggerFactory.getLogger(FormatParameterDstu3Test.class);
	...
	public void testFormatApplicationXml() throws Exception {
		...
		HttpGet httpGet =
					new HttpGet("http://localhost:" + ourPort + "/Patient/123?_format=application/xml");
		CloseableHttpResponse status = ourClient.execute(httpGet);
		try {
				String responseContent = IOUtils.toString(status.getEntity().getContent());
				ourLog.info(responseContent);
	...
	public void testFormatApplicationXmlFhir() throws Exception {
		...
		HttpGet httpGet =
					new HttpGet(
								"http://localhost:"
										+ ourPort
										+ "/Patient/123?_format=application/xml%2Bfhir");
		CloseableHttpResponse status = ourClient.execute(httpGet);
		try {
				String responseContent = IOUtils.toString(status.getEntity().getContent());
				ourLog.info(responseContent);
	...
	public void testFormatApplicationXmlFhirUnescaped() throws Exception {
		...
		HttpGet httpGet =
					new HttpGet(
								"http://localhost:"
										+ ourPort
										+ "/Patient/123?_format=application/xml+fhir");
		CloseableHttpResponse status = ourClient.execute(httpGet);
		try {
				String responseContent = IOUtils.toString(status.getEntity().getContent());
				ourLog.info(responseContent);
	...
	public void testFormatApplicationJson() throws Exception {
		...
		HttpGet httpGet =
					new HttpGet(
								"http://localhost:" + ourPort + "/Patient/123?_format=application/json");
		CloseableHttpResponse status = ourClient.execute(httpGet);
		try {
				String responseContent = IOUtils.toString(status.getEntity().getContent());
				ourLog.info(responseContent);
	...
	public void testFormatApplicationJsonFhir() throws Exception {
		...
		HttpGet httpGet =
					new HttpGet(
								"http://localhost:"
										+ ourPort
										+ "/Patient/123?_format=application/json%2Bfhir");
		CloseableHttpResponse status = ourClient.execute(httpGet);
		try {
				String responseContent = IOUtils.toString(status.getEntity().getContent());
				ourLog.info(responseContent);
	...
	public void testFormatApplicationJsonFhirUnescaped() throws Exception {
		...
		HttpGet httpGet =
					new HttpGet(
								"http://localhost:"
										+ ourPort
										+ "/Patient/123?_format=application/json+fhir");
		CloseableHttpResponse status = ourClient.execute(httpGet);
		try {
				String responseContent = IOUtils.toString(status.getEntity().getContent());
				ourLog.info(responseContent);
