		...
		if (msgNode == null) {
				...
				if (TokenTypes.NUM_INT == numberNode.getType()) {
					...
					if (ourCache.containsKey(code)) {
						log(
									theAst.getLineNo(),
									"Two different exception messages call Msg.code("
												+ code
												+ ").  \n"
												+ "Each thrown exception must call Msg.code() with a different"
												+ " code. \n"
												+ "Previously found at: "
												+ ourCache.get(code));
					} else {
						String location =
									getFilePath()
												+ ":"
												+ instantiation.getLineNo()
												+ ":"
												+ instantiation.getColumnNo()
												+ "("
												+ code
												+ ")";
