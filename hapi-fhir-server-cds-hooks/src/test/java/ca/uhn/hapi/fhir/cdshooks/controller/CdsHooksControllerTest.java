...
public class CdsHooksControllerTest {
	...
	public void before() {
		myMockMvc = MockMvcBuilders.standaloneSetup(new CdsHooksController(myCdsHooksRegistry))
			// Jackson 3: MappingJackson2HttpMessageConverter → JacksonJsonHttpMessageConverter
			// Constructor accepting ObjectMapper is identical — rename only.
			.setMessageConverters(new Jackson3JsonHttpMessageConverter(myObjectMapper))
			.build();
	}
	...
	void testEmptyCardsResponse() throws Exception {
		...
		final Function<CdsServiceRequestJson, CdsServiceResponseJson> serviceFunction =
			(CdsServiceRequestJson theCdsServiceRequestJson) -> new CdsServiceResponseJson();
	...
	public static String prettyJson(String theInput) {
		// JsonUtil.deserialize returns tools.jackson.databind.JsonNode once hapi-fhir
		// itself is migrated to Jackson 3. The import at the top of this file must
		// match whichever Jackson version JsonUtil is compiled against.
