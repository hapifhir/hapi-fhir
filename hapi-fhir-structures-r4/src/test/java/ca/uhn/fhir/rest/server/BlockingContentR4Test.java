...
public class BlockingContentR4Test {
	...
	@RegisterExtension
	public static RestfulServerExtension ourServerRule =
				new RestfulServerExtension(ourCtx).withIdleTimeout(5000);
	...
	public void beforeEach() {
		myErrorCaptureInterceptor =
					(thePointcut, theArgs) ->
								myServerException = theArgs.get(BaseServerResponseException.class);
		ourServerRule.registerAnonymousInterceptor(
					Pointcut.SERVER_HANDLE_EXCEPTION, myErrorCaptureInterceptor);
	}
	...
	/**
	* This test is just verifying that a client that is using an HTTP 100 continue followed by a
	* network timeout generates a useful error message.
	*/
	@Test
	public void testCreateWith100Continue() throws Exception {
		...
		try (CloseableHttpClient client = builder.build()) {
				...
				HttpEntity entity =
						new InputStreamEntity(inputStream, ContentType.parse("application/fhir+json")) {
								@Override
