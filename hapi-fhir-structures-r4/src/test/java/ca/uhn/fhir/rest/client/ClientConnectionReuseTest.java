...
public class ClientConnectionReuseTest {
	...
	public void testRead(IGenericClient theClient) {
		...
		for (int i = 0; i < reps; i++) {
				ourServer.getFhirClient().read().resource(Patient.class).withId("Patient/P").execute();
		}
	...
	public void testCreate(IGenericClient theClient) {
		...
		int reps = 100;
		for (int i = 0; i < reps; i++) {
				ourServer.getFhirClient().create().resource(new Patient().setActive(true)).execute();
		}
		ourLog.info("Invoked {} counts in {}", reps, sw);
	...
	public static List<IGenericClient> clients() {
		...
		additionalRequestHeadersInterceptor.addHeaderValue(
					Constants.HEADER_ACCEPT_ENCODING, Constants.ENCODING_GZIP);
