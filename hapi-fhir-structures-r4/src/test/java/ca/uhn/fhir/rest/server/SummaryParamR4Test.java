...
public class SummaryParamR4Test {
    ...
    private static final org.slf4j.Logger ourLog =
            org.slf4j.LoggerFactory.getLogger(SummaryParamR4Test.class);
    ...
    public void testReadSummaryText() throws Exception {
        ...
        try (CloseableHttpResponse status = ourClient.execute(httpGet)) {
            ...
            assertEquals(
                    "<div xmlns=\"http://www.w3.org/1999/xhtml\">THE DIV</div>", responseContent);
    ...
    public void testReadSummaryTrue() throws Exception {
        String url =
                "http://localhost:" + ourPort + "/Patient/1?_summary=" + SummaryEnum.TRUE.getCode();
        verifyXmlAndJson(
                url,
                Patient.class,
                patient -> {
    ...
    public void testSearchSummaryData() throws Exception {
        String url =
                "http://localhost:" + ourPort + "/Patient?_summary=" + SummaryEnum.DATA.getCode();
        verifyXmlAndJson(
                url,
                bundle -> {
                    String responseContent = ourCtx.newXmlParser().encodeResourceToString(bundle);
    ...
    public void testSearchSummaryText() throws Exception {
        String url =
                "http://localhost:" + ourPort + "/Patient?_summary=" + SummaryEnum.TEXT.getCode();
        verifyXmlAndJson(
                url,
                bundle -> {
                    String responseContent = ourCtx.newXmlParser().encodeResourceToString(bundle);
    ...
    public void testSearchSummaryTrue() throws Exception {
        String url =
                "http://localhost:" + ourPort + "/Patient?_summary=" + SummaryEnum.TRUE.getCode();
        verifyXmlAndJson(
                url,
                bundle -> {
                    String responseContent = ourCtx.newXmlParser().encodeResourceToString(bundle);
    ...
    public void testSearchSummaryWithTextAndOthers() throws Exception {
        ...
        try (CloseableHttpResponse status = ourClient.execute(new HttpGet(url))) {
            ...
            assertThat(
                    responseContent,
                    containsString("Can not combine _summary=text with other values for _summary"));
        }
    }
    ...
    public static void beforeClass() throws Exception {
        ...
        servlet.setResourceProviders(
                new DummyPatientResourceProvider(), new DummyMedicationRequestProvider());
