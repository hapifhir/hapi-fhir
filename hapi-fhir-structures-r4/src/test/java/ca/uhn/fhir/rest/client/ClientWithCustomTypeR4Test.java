...
public class ClientWithCustomTypeR4Test {
    ...
    public void testReadCustomType() throws Exception {
        ...
        // @formatter:off
        MyPatientWithExtensions value =
                client.read().resource(MyPatientWithExtensions.class).withId("123").execute();
        // @formatter:on
    ...
    public void testSearchWithGenericReturnType() throws Exception {
        ...
        ctx.setDefaultTypeForProfile(
                ExtendedPatient.HTTP_FOO_PROFILES_PROFILE, ExtendedPatient.class);
        ...
        List<IBaseResource> response =
                client.getPatientByDobWithGenericResourceReturnType(
                        new DateParam(ParamPrefixEnum.GREATERTHAN_OR_EQUALS, "2011-01-02"));

        assertEquals(
                "http://foo/Patient?birthdate=ge2011-01-02", capt.getValue().getURI().toString());
