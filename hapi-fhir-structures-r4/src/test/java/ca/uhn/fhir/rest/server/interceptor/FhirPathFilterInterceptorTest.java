...
public class FhirPathFilterInterceptorTest {
    ...
    private static final Logger ourLog =
            LoggerFactory.getLogger(FhirPathFilterInterceptorTest.class);
    ...
    public void testUnfilteredResponse_WithResponseHighlightingInterceptor() throws IOException {
        ...
        HttpGet request =
                new HttpGet(myPatientId.getValue() + "?_format=" + Constants.FORMATS_HTML_JSON);
        try (CloseableHttpResponse response = myHttpClient.execute(request)) {
            String responseText =
                    IOUtils.toString(response.getEntity().getContent(), Charsets.UTF_8);
            ourLog.info("Response:\n{}", responseText);
            assertThat(
                    responseText,
                    containsString(
                            "<span class='hlTagName'>&quot;system&quot;</span>: <span"
                                    + " class='hlQuot'>&quot;http://identifiers/1&quot;"));
            assertThat(
                    responseText,
                    containsString(
                            "<span class='hlTagName'>&quot;given&quot;</span>: <span"
                                    + " class='hlControl'>[</span> <span"
                                    + " class='hlTagName'>&quot;Homer&quot;</span><span"
                                    + " class='hlControl'>,</span> <span"
                                    + " class='hlTagName'>&quot;Jay&quot;</span> ]</div>"));
        }
    }
            ...
            assertThat(
                    ...
                    containsString(
                            Msg.code(327)
                                    ...
                                    + Msg.code(255)
                                    + "org.hl7.fhir.exceptions.PathEngineException: Error"
                                    + " evaluating FHIRPath expression: left operand to * can only"
                                    + " have 1 value, but has 8 values (@char 1)"));
        }
    }
    ...
    public void testFilteredResponseBundle() throws IOException {
        ...
        HttpGet request =
                new HttpGet(
                        myBaseUrl
                                + "/Patient?_fhirpath=Bundle.entry.resource.as(Patient).name&_pretty=true");
        try (CloseableHttpResponse response = myHttpClient.execute(request)) {
            String responseText =
                    IOUtils.toString(response.getEntity().getContent(), Charsets.UTF_8);
            ourLog.info("Response:\n{}", responseText);
    ...
    public void testFilteredResponse_WithResponseHighlightingInterceptor() throws IOException {
        ...
        HttpGet request =
                new HttpGet(
                        myPatientId
                                + "?_fhirpath=Patient.identifier&_format="
    ...
    private void createPatient() {
        ...
        myPatientId =
                myClient.create()
                        ...
                        .execute()
                        .getId()
                        .withServerBase(myBaseUrl, "Patient");
    }
}
