...
public class ExceptionInterceptorMethodTest {
    ...
    public void testThrowUnprocessableEntityException() throws Exception {
        ...
        when(myInterceptor.incomingRequestPreProcessed(
        ...
        HttpGet httpGet =
                new HttpGet(
                        "http://localhost:"
                                + ourPort
                                + "/Patient?_query=throwUnprocessableEntityException");
        try (CloseableHttpResponse status = ourClient.execute(httpGet)) {
            ourLog.info(IOUtils.toString(status.getEntity().getContent(), Charsets.UTF_8));
            assertEquals(422, status.getStatusLine().getStatusCode());
        }

        ArgumentCaptor<BaseServerResponseException> captor =
                ArgumentCaptor.forClass(BaseServerResponseException.class);
