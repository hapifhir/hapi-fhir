...
public class ReadR4Test {
    private static final org.slf4j.Logger ourLog =
            org.slf4j.LoggerFactory.getLogger(ReadR4Test.class);
    ...
    public void testRead() throws Exception {
        ...
        HttpGet httpGet =
                new HttpGet("http://localhost:" + myPort + "/Patient/2?_format=xml&_pretty=true");
    ...
    public void testReadUsingPlainProvider() throws Exception {
        myRestfulServerExtension
                .getRestfulServer()
                .registerProvider(new PlainGenericPatientProvider());
    ...
    public void testInvalidQueryParamsInRead() throws Exception {
        ...
        httpGet =
                new HttpGet(
                        "http://localhost:"
                                + myPort
                                + "/Patient/2?_contained=both&_format=xml&_pretty=true");
        ...
        try (InputStream inputStream = status.getEntity().getContent()) {
            ...
            assertThat(
                    ...
                    stringContainsInOrder(
                            ...
                            "  <diagnostics value=\""
                                    + Msg.code(384)
                                    + "Invalid query parameter(s) for this request:"
                                    + " &quot;[_contained]&quot;\"/>",
                            " </issue>",
                            "</OperationOutcome>"));
        }
        ...
        httpGet =
                new HttpGet(
                        "http://localhost:"
                                + myPort
                                + "/Patient/2?_containedType=contained&_format=xml&_pretty=true");
        status = ourClient.execute(httpGet);
        try (InputStream inputStream = status.getEntity().getContent()) {
            assertEquals(400, status.getStatusLine().getStatusCode());
        ...
        httpGet =
                new HttpGet(
                        "http://localhost:"
                                + myPort
                                + "/Patient/2?_count=10&_format=xml&_pretty=true");
        status = ourClient.execute(httpGet);
        try (InputStream inputStream = status.getEntity().getContent()) {
            assertEquals(400, status.getStatusLine().getStatusCode());
        ...
        httpGet =
                new HttpGet(
                        "http://localhost:"
                                + myPort
                                + "/Patient/2?_include=Patient:organization&_format=xml&_pretty=true");
        status = ourClient.execute(httpGet);
        try (InputStream inputStream = status.getEntity().getContent()) {
            assertEquals(400, status.getStatusLine().getStatusCode());
        ...
        httpGet =
                new HttpGet(
                        "http://localhost:"
                                + myPort
                                + "/Patient/2?_revinclude=Provenance:target&_format=xml&_pretty=true");
        status = ourClient.execute(httpGet);
        try (InputStream inputStream = status.getEntity().getContent()) {
            assertEquals(400, status.getStatusLine().getStatusCode());
        ...
        httpGet =
                new HttpGet(
                        "http://localhost:"
                                + myPort
                                + "/Patient/2?_sort=family&_format=xml&_pretty=true");
        status = ourClient.execute(httpGet);
        try (InputStream inputStream = status.getEntity().getContent()) {
            assertEquals(400, status.getStatusLine().getStatusCode());
        ...
        httpGet =
                new HttpGet(
                        "http://localhost:"
                                + myPort
                                + "/Patient/2?_total=accurate&_format=xml&_pretty=true");
        status = ourClient.execute(httpGet);
        try (InputStream inputStream = status.getEntity().getContent()) {
            assertEquals(400, status.getStatusLine().getStatusCode());
