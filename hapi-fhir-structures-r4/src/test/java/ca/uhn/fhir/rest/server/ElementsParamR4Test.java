...
public class ElementsParamR4Test {
    ...
    private static final org.slf4j.Logger ourLog =
            org.slf4j.LoggerFactory.getLogger(ElementsParamR4Test.class);
    ...
    public void testElementsOnChoiceWithGenericName() throws IOException {
        ...
        verifyXmlAndJson(
                ...
                bundle -> {
                    ...
                    assertEquals(
                            "222", obs.getValueQuantity().getValueElement().getValueAsString());
    ...
    public void testInvalidInclude() throws IOException {
        ...
        httpGet =
                new HttpGet(
                        ("http://localhost:"
                                        + ourPort
                                        + "/Procedure?_include=*&_elements=DiagnosticReport:foo")
                                + "&_pretty=true&_format="
                                + encodingEnum.getFormatContentType());
    ...
    private void createObservationWithQuantity() {
        ...
        ourNextObservation.setValue(
                new Quantity().setValue(222).setCode("mg").setSystem("http://unitsofmeasure.org"));
    }
    ...
    /**
     * By default the elements apply only to the focal resource in a search and not any included
     * resources
     */
    @Test
    ...
    public void testMultiResourceElementsOnExtension() throws IOException {
        ...
        verifyXmlAndJson(
                ...
                bundle -> {
                    ...
                    assertEquals(
                            "mg",
                            ((Quantity) procedure.getExtension().get(0).getValue()).getCode());
                });
    ...
    /**
     * A search on procedure, with only resource specific elements filters that are specifically on
     * other resources but Procedure, should not affect the output of the procedure resource.
     */
    @Test
    ...
    @Test
    public void testMultiResourceElementsFilterWithMetadataExcludedStandardMode()
            throws IOException {
        ...
        verifyXmlAndJson(
                ...
                bundle -> {
                    ...
                    assertEquals(
                            ...
                            procedure.getReasonCode().get(0).getCoding().get(0).getCode());
                    assertEquals(
                            null, procedure.getReasonCode().get(0).getCoding().get(0).getSystem());
                    assertEquals(
                            null, procedure.getReasonCode().get(0).getCoding().get(0).getDisplay());
                    assertEquals(0, procedure.getUsedCode().size());
                });
    }
    ...
    private <T extends IBaseResource> void verifyXmlAndJson(
            String theUri, Class<T> theType, Consumer<T> theVerifier) throws IOException {
        EncodingEnum encodingEnum;
        HttpGet httpGet;
        ...
        httpGet =
                new HttpGet(
                        theUri + "&_pretty=true&_format=" + encodingEnum.getFormatContentType());
        try (CloseableHttpResponse status = ourClient.execute(httpGet)) {
            String responseContent =
                    IOUtils.toString(status.getEntity().getContent(), Charsets.UTF_8);
            ourLog.info(responseContent);
