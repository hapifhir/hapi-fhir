...
public class ExceptionHandlingInterceptorTest {
    ...
    public void testInternalError() throws Exception {
        ...
        {
            HttpGet httpGet =
                    new HttpGet(ourServer.getBaseUrl() + "/Patient?throwInternalError=aaa");
    ...
    public void ExceptionHandlingInterceptor_HandlesFailure_WhenWriting() throws IOException {
        ...
        // Given: We have an interceptor which causes a failure after the response output stream has
        // been started.
        ProblemGeneratingInterceptor interceptor = new ProblemGeneratingInterceptor();
        ourServer.registerInterceptor(interceptor);

        // When: We make a request to the server, triggering this exception to be thrown on an
        ...
        // Then: This should still return an OperationOutcome, and not explode with an HTML
        // IllegalState response.
            ...
            HttpGet httpGet =
                    new HttpGet(
                            ourServer.getBaseUrl()
                                    + "/Patient?throwInternalError=aaa&_format=true");
            HttpResponse status = ourClient.execute(httpGet);
            String responseContent =
                    IOUtils.toString(status.getEntity().getContent(), Charsets.UTF_8);
            IOUtils.closeQuietly(status.getEntity().getContent());
    ...
    public static class DummyPatientResourceProvider implements IResourceProvider {
        ...
        @Search
        public List<Patient> throwInternalError(
                @RequiredParam(name = "throwInternalError") StringParam theParam) {
        ...
        public List<Patient> throwUnprocessableEntityWithMultipleMessages(
                @RequiredParam(name = "throwUnprocessableEntityWithMultipleMessages")
