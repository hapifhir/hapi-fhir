...
public class CqlMeasureEvaluationR4ImmunizationTest extends BaseCrR4Test {
    ...
    private static final String MY_IMMUNIZATION_CQL_RESOURCES =
            "ca/uhn/fhir/cr/r4/immunization/Measure_Library_Ontario_ImmunizationStatus.json";
    private static final String MY_VALUE_SETS =
            "ca/uhn/fhir/cr/r4/immunization/Terminology_ValueSets.json";
    ...
    protected void assertMeasureScore(MeasureReport theReport, double theExpectedScore) {
        ...
        double actualScore =
                theReport.getGroupFirstRep().getMeasureScore().getValue().doubleValue();
        assertEquals(theExpectedScore, actualScore, epsilon);
    }

    // evaluates a Measure to produce one certain MeasureReport
    protected MeasureReport evaluateMeasureByMeasure(
            String theMeasureId, String thePractitionerRef, String thePatientRef) {
    ...
    public void test_Immunization_Ontario_Schedule() throws IOException {
        ...
        // when
        MeasureReport reportBasic =
                evaluateMeasureByMeasure("ImmunizationStatusRoutine", null, null);
        MeasureReport reportByPractitioner =
                evaluateMeasureByMeasure(
                        "ImmunizationStatusRoutine",
                        "Practitioner/ImmunizationStatus-practitioner-3",
                        null);
        MeasureReport reportIndividualImmunized =
                evaluateMeasureByMeasure(
                        "ImmunizationStatusRoutine", null, "ImmunizationStatus-1-year-patient-1");
        MeasureReport reportIndividualNotImmunized =
                evaluateMeasureByMeasure(
                        "ImmunizationStatusRoutine", null, "ImmunizationStatus-1-year-patient-2");

        // then
        assertMeasureScore(
                reportBasic, 0.3442623); // 21 out of 61 patients are fully immunized on 2022-09-16
        assertMeasureScore(
                reportByPractitioner,
                0.23077); // 3 out of 13 patients are fully immunized on 2022-09-16
        assertMeasureScore(
                reportIndividualImmunized, 1.0); // the patient is fully immunized on on 2022-09-16
        assertMeasureScore(
                reportIndividualNotImmunized,
                0.0); // the patient is not fully immunized on 2022-09-16
    }
}
