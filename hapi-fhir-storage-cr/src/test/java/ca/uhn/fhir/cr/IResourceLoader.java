 ...
 * Resources. This is used primarily to set up integration tests for clinical reasoning operations
 * since they often require big bundles of content, such as Libraries, ValueSets, Measures, and so
 * on.
 */
        ...
        switch (theEncoding.toLowerCase()) {
            ...
            default:
                throw new IllegalArgumentException(
                        String.format(
                                "Expected encoding xml, or json.  %s is not a valid encoding",
                                theEncoding));
        }
        ...
        try {
            if (theLocation.startsWith(File.separator)) {
                ...
                org.springframework.core.io.Resource resource =
                        resourceLoader.getResource(theLocation);
                is = resource.getInputStream();
            }
            return IOUtils.toString(is, StandardCharsets.UTF_8);
        } catch (Exception e) {
            throw new RuntimeException(
                    String.format("Error loading resource from %s", theLocation), e);
        }
    }
