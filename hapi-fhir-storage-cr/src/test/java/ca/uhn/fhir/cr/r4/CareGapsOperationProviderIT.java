...
/**
 * End to end test for care gaps functionality Scenario is that we have a Provider that is
 * transmitting data to a Payer to validate that no gaps in care exist (a "gap in care" means that a
 * Patient is not conformant with best practices for a given pathology). Specifically, for this
 * test, we're checking to ensure that a Patient has had the appropriate colorectal cancer
 * screenings.
 *
 * <p>So, it's expected that the Payer already has the relevant quality measure content loaded. The
 * first two steps here are initializing the Payer by loading Measure content, and by setting up a
 * reporting Organization resource (IOW, the Payer's identify to associate with the care-gaps
 * report).
 *
 * <p>The next step is for the Provider to submit data to the Payer for review. That's the submit
 * data operation.
 *
 * <p>After that, the Provider can invoke $care-gaps to check for any issues, which are reported.
 *
 * <p>The Provider can then resolve those issues, submit additional data, and then check to see if
 * the gaps are closed.
 *
 * <p>1. Initialize Payer with Measure content 2. Initialize Payer with Organization info 3.
 * Provider submits Patient data 4. Provider invokes care-gaps (and discovers issues) 5. (not
 * included in test, since it's done out of bad) Provider closes gap (by having the Procedure done
 * on the Patient). 6. Provider submits additional Patient data 7. Provider invokes care-gaps (and
 * discovers issues are closed).
 */
