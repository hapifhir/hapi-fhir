...
/**
 * This class listens for changes to Library resources and invalidates the CodeCache. The CodeCache
 * is used in CQL evaluatuon to speed up the measure operations. If underlying values change in the
 * library then cache requires updating.
 */
public class ElmCacheResourceChangeListener implements IResourceChangeListener {
	...
	private void invalidateCacheById(IIdType theId) {
		...
		try {
				...
				ourLog.debug(
						"Failed to locate resource {} to look up name and version. Clearing all"
									+ " libraries from cache.",
		...
		this.myGlobalLibraryCache.remove(
					new VersionedIdentifier().withId(name).withVersion(version));
	}
}
