...
public class HapiFhirRepository implements Repository {
    ...
    @Override
    public <T extends IBaseResource> MethodOutcome create(
            T theResource, Map<String, String> theHeaders) {
        var details = startWith(myRequestDetails).addHeaders(theHeaders).create();
    ...
    @Override
    public <T extends IBaseResource> MethodOutcome update(
            T theResource, Map<String, String> theHeaders) {
        var details = startWith(myRequestDetails).addHeaders(theHeaders).create();
    ...
    private <B extends IBaseBundle> B createBundle(
        ...
        var linkSelf =
                RestfulServerUtils.createLinkSelf(
                        theRequestDetails.getFhirServerBase(), theRequestDetails);
        ...
        var offset =
                RestfulServerUtils.tryToExtractNamedParameter(
                        theRequestDetails, Constants.PARAM_PAGINGOFFSET);
        ...
        var linkEncoding =
                theRequestDetails.getParameters().containsKey(Constants.PARAM_FORMAT)
    ...
    // repository action"?
    @Override
    public <B extends IBaseBundle> B link(
            Class<B> theBundleType, String theUrl, Map<String, String> theHeaders) {
        var details = startWith(myRequestDetails).addHeaders(theHeaders).create();
        ...
        if (pagingProvider == null) {
            throw new InvalidRequestException(
                    Msg.code(2312) + "This server does not support paging");
        }
