...
/**
 * Simulate FhirDal operations until that's fully baked. This interface is trying to be small and
 * have a reasonable path forwards towards model independence in the future. The method overloads
 * with "RequestDetails" will eventually go away once we're able to make that a cross-cutting
 * concern. There are some ramifications to not using the "RequestDetails" such as not firing hooks
 * on the server, so the overloads with that parameter should generally be preferred for the
 * short-term.
 */
public interface IDaoRegistryUser {
    ...
    /**
     * Get the class of the given Resource. FHIR version aware. For example, if the server is
     * running in DSTU3 mode this will return the DSTU3 Library class when invoked with "Library".
     *
    ...
    /**
     * Reads a Resource with the given Id from the local server. Throws an error if the Resource is
     * not present
     *
     ...
     */
    @SuppressWarnings("unchecked")
    default <T extends IBaseResource> DaoMethodOutcome create(
            T theResource, RequestDetails requestDetails) {
        checkNotNull(theResource);
     ...
     */
    @SuppressWarnings("unchecked")
    default <T extends IBaseResource> DaoMethodOutcome update(
            T theResource, RequestDetails requestDetails) {
        checkNotNull(theResource);
    ...
    default DaoMethodOutcome delete(IIdType theIdType, RequestDetails requestDetails) {
        ...
        return getDaoRegistry()
                .getResourceDao(theIdType.getResourceType())
                .delete(theIdType, requestDetails);
    }
     ...
     */
    @SuppressWarnings("unchecked")
    default <T extends IBaseBundle> T transaction(
            T theTransaction, RequestDetails theRequestDetails) {
        checkNotNull(theTransaction);
    ...
    default <T extends IBaseResource> Iterable<IBaseResource> search(
        ...
        var provider =
                getDaoRegistry()
                        .getResourceDao(theResourceClass)
                        .search(theSearchMap, theRequestDetails);
