...
public class PlanDefinitionOperationsProvider {
	...
	@Autowired IPlanDefinitionProcessorFactory myDstu3PlanDefinitionServiceFactory;

	/**
	* Implements the <a href=
	* "http://www.hl7.org/fhir/plandefinition-operation-apply.html">$apply</a> operation found in
	* the <a href="http://www.hl7.org/fhir/clinicalreasoning-module.html">FHIR Clinical Reasoning
	* Module</a>. This implementation aims to be compatible with the <a
	* href="https://build.fhir.org/ig/HL7/cqf-recommendations/OperationDefinition-cpg-plandefinition-apply.html">
	* CPG IG</a>.
	*
	* @param theId The id of the PlanDefinition to apply
	* @param theCanonical The canonical identifier for the PlanDefinition to apply (optionally
	*     version-specific)
	...
	* @param theUserType The type of user initiating the request, e.g. patient, healthcare
	...
	* @param theUserTaskContext The task the system user is performing, e.g. laboratory results
	*     review, medication list review, etc. This information can be used to tailor decision
	...
	* @param theParameters Any input parameters defined in libraries referenced by the
	*     PlanDefinition.
	* @param theData Data to be made available to the PlanDefinition evaluation.
	* @param theDataEndpoint An endpoint to use to access data referenced by retrieve operations in
	*     libraries referenced by the PlanDefinition.
	* @param theContentEndpoint An endpoint to use to access content (i.e. libraries) referenced by
	*     the PlanDefinition.
	* @param theTerminologyEndpoint An endpoint to use to access terminology (i.e. valuesets,
	*     codesystems, and membership testing) referenced by the PlanDefinition.
	...
	*/
	@Operation(
				name = ProviderConstants.CR_OPERATION_APPLY,
				idempotent = true,
				type = PlanDefinition.class)
