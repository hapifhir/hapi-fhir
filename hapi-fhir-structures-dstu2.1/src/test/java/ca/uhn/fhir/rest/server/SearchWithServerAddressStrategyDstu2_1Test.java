...
public class SearchWithServerAddressStrategyDstu2_1Test {
	...
	public void testApacheProxyAddressStrategy() throws Exception {
		...
		assertThat(responseContent, containsString("<given value=\"FAMILY\""));
		assertThat(
					responseContent, containsString("<fullUrl value=\"http://foo.com/Patient/1\"/>"));
		...
		assertThat(responseContent, containsString("<given value=\"FAMILY\""));
		assertThat(
					responseContent, containsString("<fullUrl value=\"https://foo.com/Patient/1\"/>"));
	...
	public void testHardcodedAddressStrategy() throws Exception {
		ourServlet.setServerAddressStrategy(
					new HardcodedServerAddressStrategy("http://example.com/fhir/base"));
		...
		assertThat(
					responseContent,
					containsString("<fullUrl value=\"http://example.com/fhir/base/Patient/1\"/>"));
	}
	...
	public static class DummyPatientResourceProvider implements IResourceProvider {
		...
		public List<Patient> searchByIdentifier() {
				...
				retVal.add(
						(Patient) new Patient().addName(new HumanName().addGiven("FAMILY")).setId("1"));
				retVal.add(
						(Patient) new Patient().addName(new HumanName().addGiven("FAMILY")).setId("2"));
				return retVal;
		}
		// @formatter:on
