...
public class PatchClientDstu2_1Test {
	public interface IClientType extends IRestfulClient {
		...
		MethodOutcome patch(
					@IdParam IdType theId, @ResourceParam String theBody, PatchTypeEnum thePatchType);
	}
	...
	public void testJsonPatchAnnotation() throws Exception {
		...
		MethodOutcome outcome =
					client.patch(new IdType("Patient/123"), "{}", PatchTypeEnum.JSON_PATCH);
	...
	public void testJsonPatchFluent() throws Exception {
		...
		//		assertEquals("PATCH", capt.getAllValues().get(0).getMethod());
		//		assertEquals("http://example.com/fhir/Patient/123",
		// capt.getAllValues().get(0).getURI().toASCIIString());
	...
	private ArgumentCaptor<HttpUriRequest> prepareResponse()
