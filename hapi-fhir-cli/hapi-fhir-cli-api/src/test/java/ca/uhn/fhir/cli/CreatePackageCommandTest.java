...
public class CreatePackageCommandTest extends BaseTest {
	...
	public void stop() {
		try {
			...
			throw new InternalErrorException(
					"Failed to delete temporary directory \"" + myWorkDirectory.getAbsolutePath() + "\"", e);
		}
	}
	...
	public void testCreatePackage() throws IOException {
		...
		List<String> allFiles =
				FileUtils.listFiles(myExtractDirectory, TrueFileFilter.INSTANCE, TrueFileFilter.INSTANCE).stream()
						.map(t -> t.getPath())
						.sorted()
		...
		// Verify package.json
		String packageJsonContents = IOUtils.toString(
				new FileInputStream(new File(myExtractDirectory, "package/package.json")), Charsets.UTF_8);
