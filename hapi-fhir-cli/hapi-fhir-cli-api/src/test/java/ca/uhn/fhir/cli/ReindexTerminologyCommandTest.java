...
class ReindexTerminologyCommandTest {
	...
	// Deliberately not registered - we manually run this later because App startup resets the
	// logging.
	...
	public void testProviderMethodInvoked(boolean theIncludeTls) {
		...
		assertThat(
					...
					Matchers.not(hasItem(eventWithMessageContains("FAILURE"))));
	}
	...
	public void testNoTargetThrows(boolean theIncludeTls) {
		...
		try {
				App.main(
						myTlsAuthenticationTestHelper.createBaseRequestGeneratingCommandArgs(
									new String[] {
										ReindexTerminologyCommand.REINDEX_TERMINOLOGY, "-v", "r4"
									},
									null,
									theIncludeTls,
									myRestServerR4Helper));
		...
		assertThat(
					...
					hasItem(
								eventWithMessageContains(
										"Internal error. Command result unknown. Check system logs for"
													+ " details")));
	}
	...
	public void testHandleServiceError(boolean theIncludeTls) {
		...
		ParametersUtil.addParameterToParametersBoolean(
					myContext, retVal, RESP_PARAM_SUCCESS, false);
