...
public class HapiClearMigrationLockCommandTest extends ConsoleOutputCapturingBaseTest {
    ...
    public void testClearNonExistingLockIncorrectLock() throws IOException {
        ...
        HapiMigrationDao dao =
                new HapiMigrationDao(
                        connectionData.connectionProperties.getDataSource(),
    ...
    public void testClearNonExistingLockNoLocks() throws IOException {
        ConnectionData connectionData =
                createSchemaAndMigrate("test_migrate_clear_nonexisting_lock");
        HapiMigrationDao dao =
                new HapiMigrationDao(
                        connectionData.connectionProperties.getDataSource(),
                        DriverTypeEnum.H2_EMBEDDED,
        ...
        assertThat(
                getConsoleOutput(),
                containsString("Did not successfully remove lock entry. [uuid=" + lockUUID + "]"));
    }
    ...
    public void testMigrateAndClearExistingLock() throws IOException, SQLException {
        ...
        assertThat(
                getConsoleOutput(),
                containsString("Successfully removed lock entry. [uuid=" + lockUUID + "]"));
    }

    private record ConnectionData(
            DriverTypeEnum.ConnectionProperties connectionProperties, String url) {}
                ...
                .execute(
                        t -> {
                            ...
                            jdbcTemplate.execute(
                                    "insert into HFJ_RESOURCE (RES_DELETED_AT, RES_VERSION,"
                                        + " FORCED_ID_PID, HAS_TAGS, RES_PUBLISHED, RES_UPDATED,"
                                        + " SP_HAS_LINKS, HASH_SHA256, SP_INDEX_STATUS,"
                                        + " RES_LANGUAGE, SP_CMPSTR_UNIQ_PRESENT,"
                                        + " SP_COORDS_PRESENT, SP_DATE_PRESENT, SP_NUMBER_PRESENT,"
                                        + " SP_QUANTITY_PRESENT, SP_STRING_PRESENT,"
                                        + " SP_TOKEN_PRESENT, SP_URI_PRESENT, RES_PROFILE,"
                                        + " RES_TYPE, RES_VER, RES_ID) values (?, ?, ?, ?, ?, ?, ?,"
                                        + " ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
                                    new AbstractLobCreatingPreparedStatementCallback(
                                            new DefaultLobHandler()) {
                                        @Override
                                        protected void setValues(
                                                PreparedStatement thePs, LobCreator theLobCreator)
                                                throws SQLException {
                            ...
                            jdbcTemplate.execute(
                                    "insert into HFJ_RES_VER (RES_DELETED_AT, RES_VERSION,"
                                        + " FORCED_ID_PID, HAS_TAGS, RES_PUBLISHED, RES_UPDATED,"
                                        + " RES_ENCODING, RES_TEXT, RES_ID, RES_TYPE, RES_VER, PID)"
                                        + " values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
                                    new AbstractLobCreatingPreparedStatementCallback(
                                            new DefaultLobHandler()) {
                                        @Override
                            ...
                            jdbcTemplate.execute(
                                    "insert into HFJ_SPIDX_STRING (SP_MISSING, SP_NAME, RES_ID,"
                                        + " RES_TYPE, SP_UPDATED, SP_VALUE_EXACT,"
                                        + " SP_VALUE_NORMALIZED, SP_ID) values (?, ?, ?, ?, ?, ?,"
                                        + " ?, ?)",
                                    new AbstractLobCreatingPreparedStatementCallback(
                                            new DefaultLobHandler()) {
                                        @Override
                            ...
                            jdbcTemplate.execute(
                                    "insert into HFJ_SPIDX_TOKEN (SP_MISSING, SP_NAME, RES_ID,"
                                            + " RES_TYPE, SP_UPDATED, SP_SYSTEM, SP_VALUE, SP_ID)"
                                            + " values (?, ?, ?, ?, ?, ?, ?, ?)",
                                    new AbstractLobCreatingPreparedStatementCallback(
                                            new DefaultLobHandler()) {
                                        @Override
                            ...
                            jdbcTemplate.execute(
                                    "insert into HFJ_SPIDX_DATE (SP_MISSING, SP_NAME, RES_ID,"
                                            + " RES_TYPE, SP_UPDATED, SP_VALUE_HIGH, SP_VALUE_LOW,"
                                            + " SP_ID) values (?, ?, ?, ?, ?, ?, ?, ?)",
                                    new AbstractLobCreatingPreparedStatementCallback(
                                            new DefaultLobHandler()) {
                                        @Override
                                        protected void setValues(
                                            ...
                                            thePs.setTimestamp(
                                                    5, new Timestamp(System.currentTimeMillis()));
                                            thePs.setTimestamp(
                                                    6, new Timestamp(1000000000L)); // value high
                                            thePs.setTimestamp(
                                                    7, new Timestamp(1000000000L)); // value low
                                            thePs.setLong(8, 1L);
                                        }
                                    });
    ...
    private void executeSqlStatements(
            DriverTypeEnum.ConnectionProperties theConnectionProperties, String theInitSql)
            throws IOException {
        String script =
                IOUtils.toString(
                        HapiClearMigrationLockCommandTest.class.getResourceAsStream(theInitSql),
                        Charsets.UTF_8);
