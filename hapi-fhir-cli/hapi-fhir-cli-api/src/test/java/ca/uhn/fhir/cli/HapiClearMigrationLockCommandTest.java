...
public class HapiClearMigrationLockCommandTest extends ConsoleOutputCapturingBaseTest {
	...
	public void testClearNonExistingLockIncorrectLock() throws IOException {
		...
		HapiMigrationDao dao = new HapiMigrationDao(
				connectionData.connectionProperties.getDataSource(), DriverTypeEnum.H2_EMBEDDED, "FLY_HFJ_MIGRATION");
		...
		try {
			...
			assertThat(
					e.getMessage(),
					containsString("HAPI-2152: Internal error: on unlocking, a competing lock was found"));
		}
	}
	...
	public void testClearNonExistingLockNoLocks() throws IOException {
		...
		assertThat(
				getConsoleOutput(), containsString("Did not successfully remove lock entry. [uuid=" + lockUUID + "]"));
	}
	...
	private void executeSqlStatements(DriverTypeEnum.ConnectionProperties theConnectionProperties, String theInitSql)
		...
		String script = IOUtils.toString(
				HapiClearMigrationLockCommandTest.class.getResourceAsStream(theInitSql), Charsets.UTF_8);
