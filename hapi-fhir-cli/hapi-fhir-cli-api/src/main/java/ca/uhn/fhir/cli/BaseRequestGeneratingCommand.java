...
public abstract class BaseRequestGeneratingCommand extends BaseCommand {
    ...
    protected Options getSomeOptions(
            Collection<BaseRequestGeneratingCommandOptions> theExcludeOptions) {
    ...
    protected IGenericClient newClientWithBaseUrl(
        ...
        IGenericClient client =
                super.newClientWithBaseUrl(
                        theCommandLine,
                        theBaseUrl,
                        theBasicAuthOptionName,
                        theBearerTokenOptionName,
                        theTlsAuthOptionName);
        registerHeaderPassthrough(theCommandLine, client);

        return client;
    }

    private void registerHeaderPassthrough(CommandLine theCommandLine, IGenericClient theClient)
            throws ParseException {
        if (theCommandLine.hasOption(HEADER_PASSTHROUGH)) {
            theClient.registerInterceptor(
                    new AdditionalRequestHeadersInterceptor(
                            getAndParseOptionHeadersPassthrough(
                                    theCommandLine, HEADER_PASSTHROUGH)));
        }
    }
    ...
    private void addHeaderPassthroughOption(Options theOptions) {
        addOptionalOption(
                ...
                "If specified, this argument specifies headers to include in the generated"
                        + " request");
    }
    ...
    private Map<String, List<String>> getAndParseOptionHeadersPassthrough(
        ...
        for (String nextOptionValue : theCommandLine.getOptionValues(theOptionName)) {
            Pair<String, String> nextHeader =
                    parseNameValueParameter(":", theOptionName, nextOptionValue);
            headersMap
