...
public class WebsocketSubscribeCommand extends BaseCommand {
    private static final org.slf4j.Logger LOG_RECV =
            org.slf4j.LoggerFactory.getLogger("websocket.RECV");
    private static final org.slf4j.Logger LOG_SEND =
            org.slf4j.LoggerFactory.getLogger("websocket.SEND");
    // TODO: Don't use qualified names for loggers in HAPI CLI.
    private static final org.slf4j.Logger ourLog =
            org.slf4j.LoggerFactory.getLogger(WebsocketSubscribeCommand.class);

    private boolean myQuit;

    @Override
    public String getCommandDescription() {
        return "Opens a websocket client connection to a server for the purposes of binding to a"
                + " Subscription, and then outputs the results";
    }
    ...
    public Options getOptions() {
        ...
        options.addOption(
                "t", "target", true, "Target URL, e.g. \"ws://fhirtest.uhn.ca/websocket/dstu2\"");
    ...
    public void run(CommandLine theCommandLine) throws ParseException {
        ...
        if (isBlank(target) || (!target.startsWith("ws://") && !target.startsWith("wss://"))) {
            throw new ParseException(
                    Msg.code(1536)
                            + "Target (-t) needs to be in the form \"ws://foo\" or \"wss://foo\"");
        }
