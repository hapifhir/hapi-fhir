...
public class CreatePackageCommand extends BaseCommand {
	...
	public Options getOptions() {
		...
		addRequiredOption(
					...
					NAME_OPT,
					"Package Name",
					"The name/id of the package, e.g. \"com.example.fhir.myapp\"");
		addRequiredOption(
					options,
					null,
					VERSION_OPT,
		...
		addOptionalOption(
					options, null, DESCRIPTION_OPT, "Description", "A description for this package");
		addOptionalOption(
					options,
					null,
					INCLUDE_PACKAGE_OPT,
		...
		addOptionalOption(
					options,
					null,
					TARGET_DIRECTORY_OPT,
					"Directory",
					"The directory in which to place the final package");
		addOptionalOption(
					options,
					null,
					DEPENDENCY_OPT,
					"name:version",
					"Include this dependency, in the form \"name:version\"");
	...
	public void run(CommandLine theCommandLine) throws ParseException, ExecutionException {
		...
		if (isBlank(myPackageName)) {
				throw new ParseException(
						Msg.code(1546) + "No package name supplied (--" + NAME_OPT + ")");
		}
		...
		if (isBlank(myPackageVersion)) {
				throw new ParseException(
						Msg.code(1548) + "No package version supplied (--" + VERSION_OPT + ")");
		}
		if (!NpmPackage.isValidVersion(myPackageVersion)) {
				throw new ParseException(
						Msg.code(1549) + "Invalid package version: " + myPackageVersion);
		}
		...
		if (dependencies != null) {
				for (String nextDependencyString : dependencies) {
					...
					if (colonIdx == -1) {
						throw new ParseException(
									Msg.code(1550) + "Invalid dependency spec: " + nextDependencyString);
					}
		...
		File targetFile =
					new File(
								new File(targetDirectory), myPackageName + "-" + myPackageVersion + ".tgz");
	...
	public void addFiles(String[] thePackageValues, String theFolder)
		...
		if (thePackageValues != null) {
				for (String nextPackageValue : thePackageValues) {
					if (!nextPackageValue.contains("/")) {
						throw new ParseException(
									Msg.code(1552) + "Invalid file expression: " + nextPackageValue);
					}
					...
					Collection<File> files =
								FileUtils.listFiles(new File(path), filter, FalseFileFilter.INSTANCE);
					...
					for (File next : files) {
						...
						try {
								String contents =
										IOUtils.toString(new FileInputStream(next), StandardCharsets.UTF_8);
								contentBytes = contents.getBytes(StandardCharsets.UTF_8);
								type =
										EncodingEnum.detectEncoding(contents)
													.newParser(myFhirCtx)
