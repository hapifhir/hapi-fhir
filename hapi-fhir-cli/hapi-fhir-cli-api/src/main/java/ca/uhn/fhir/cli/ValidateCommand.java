...
public class ValidateCommand extends BaseCommand {
    ...
    private static final org.slf4j.Logger ourLog =
            org.slf4j.LoggerFactory.getLogger(ValidateCommand.class);
    ...
    public Options getOptions() {
        ...
        addOptionalOption(
                retVal, source, "n", "file", "filename", "The name of the file to validate");
        addOptionalOption(retVal, source, "d", "data", "text", "The text to validate");
        retVal.addOptionGroup(source);

        retVal.addOption(
                "p", "profile", false, "Validate using Profiles (StructureDefinition / ValueSet)");
        retVal.addOption(
                "r",
                "fetch-remote",
                false,
                "Allow fetching remote resources (in other words, if a resource being validated"
                    + " refers to an external StructureDefinition, Questionnaire, etc. this flag"
                    + " allows the validator to access the internet to try and fetch this"
                    + " resource)");
        addOptionalOption(
                retVal,
                "l",
                "fetch-local",
                "filename",
                "Fetch a profile locally and use it if referenced");
        addOptionalOption(
                retVal,
                null,
                "igpack",
    ...
    public void run(CommandLine theCommandLine) throws ParseException {
        ...
        if (isNotBlank(fileName) && isNotBlank(contents)) {
            throw new ParseException(
                    Msg.code(1616) + "Can not supply both a file (-n) and data (-d)");
        }
        if (isBlank(fileName) && isBlank(contents)) {
            throw new ParseException(
                    Msg.code(1617) + "Must supply either a file (-n) or data (-d)");
        }
        ...
        if (isNotBlank(fileName)) {
            ...
            ourLog.info(
                    "Fully read - Size is {}", FileHelper.getFileSizeDisplay(contents.length()));
        }

        ca.uhn.fhir.rest.api.EncodingEnum enc =
                ca.uhn.fhir.rest.api.EncodingEnum.detectEncodingNoDefault(defaultString(contents));
        if (enc == null) {
            throw new ParseException(
                    Msg.code(1619) + "Could not detect encoding (json/xml) of contents");
        }
        ...
        if (theCommandLine.hasOption("p")) {
            switch (ctx.getVersion().getVersion()) {
                case DSTU2:
                    {
                        ...
                        ValidationSupportChain validationSupportChain =
                                ValidationSupportChainCreator.getValidationSupportChainDstu2(
                                        ctx, theCommandLine);
                ...
                case R4:
                    {
                        ...
                        val.registerValidatorModule(instanceValidator);
                        ValidationSupportChain validationSupportChain =
                                ValidationSupportChainCreator.getValidationSupportChainR4(
                                        ctx, theCommandLine);
                ...
                default:
                    throw new ParseException(
                            Msg.code(1620)
                                    + "Profile validation (-p) is not supported for this FHIR"
                                    + " version");
            }
        }
        ...
        for (SingleValidationMessage next : results.getMessages()) {
            ...
            String[] message =
                    WordUtils.wrap(next.getMessage(), 80 - leftWidth, "\n", true).split("\\n");
