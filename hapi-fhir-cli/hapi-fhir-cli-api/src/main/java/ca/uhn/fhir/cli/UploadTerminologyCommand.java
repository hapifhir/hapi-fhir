...
public class UploadTerminologyCommand extends BaseRequestGeneratingCommand {
    ...
    private static final org.slf4j.Logger ourLog =
            org.slf4j.LoggerFactory.getLogger(UploadTerminologyCommand.class);
    ...
    public String getCommandDescription() {
        return "Uploads a terminology package (e.g. a SNOMED CT ZIP file or a custom terminology"
                + " bundle) to a server, using the appropriate operation.";
    }
    ...
    public Options getOptions() {
        ...
        addOptionalOption(
                options, "m", "mode", true, "The upload mode: SNAPSHOT (default), ADD, REMOVE");
        addOptionalOption(
                options,
                "s",
                "size",
    ...
    private void invokeOperation(
        ...
        boolean isDeltaOperation =
                theOperationName.equals(JpaConstants.OPERATION_APPLY_CODESYSTEM_DELTA_ADD)
                        || theOperationName.equals(
                                JpaConstants.OPERATION_APPLY_CODESYSTEM_DELTA_REMOVE);
        ...
        try {
            for (String nextDataFile : theDatafile) {
                ...
                try (FileInputStream fileInputStream = new FileInputStream(dataFile)) {
                    boolean isFhirType =
                            nextDataFile.endsWith(".json") || nextDataFile.endsWith(".xml");
                    if (nextDataFile.endsWith(".csv")
                            || nextDataFile.endsWith(".properties")
                            || isFhirType) {
                        ...
                        if (isDeltaOperation && isFhirType) {
                            ...
                            if (encoding == null) {
                                throw new ParseException(
                                        Msg.code(1541)
                                                + "Could not detect FHIR encoding for file: "
                                                + nextDataFile);
                            }
                            ...
                            ParametersUtil.addParameterToParameters(
                                    myFhirCtx,
                                    inputParameters,
                                    TerminologyUploaderProvider.PARAM_CODESYSTEM,
                                    resource);
                            ...
                            CountingInputStream countingInputStream =
                                    new CountingInputStream(fileInputStream);
                        ...
                        addFileToRequestBundle(
                                inputParameters, fileName, IOUtils.toByteArray(fileInputStream));

                    } else {

                        throw new ParseException(
                                Msg.code(1542) + "Don't know how to handle file: " + nextDataFile);
                    }
                }
            }
            zipOutputStream.flush();
        ...
        if (ourLog.isDebugEnabled() || HapiSystemProperties.isTestModeEnabled()) {
            ourLog.debug(
                    ...
                    myFhirCtx
                            .newJsonParser()
                            .setPrettyPrint(true)
                            .encodeResourceToString(inputParameters));
        }
    ...
    private void addFileToRequestBundle(
            IBaseParameters theInputParameters, String theFileName, byte[] theBytes) {
        ...
        if (bytes.length > ourTransferSizeLimit) {
            ourLog.info(
                    "File size is greater than {} - Going to use a local file reference instead of"
                        + " a direct HTTP transfer. Note that this will only work when executing"
                        + " this command on the same server as the FHIR server itself.",
    ...
    private String getContentType(String theSuffix) {
        ...
        ourLog.debug(
                "File suffix given was {} and contentType is {}, defaulting to content type"
                        + " text/plain",
                theSuffix,
                retVal);
        return retVal;
