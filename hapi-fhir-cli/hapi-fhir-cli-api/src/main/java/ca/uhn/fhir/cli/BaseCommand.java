...
public abstract class BaseCommand implements Comparable<BaseCommand> {
    ...
    protected static final String TLS_AUTH_PARAM_DESC =
            "If specified, this parameter supplies a path and filename for a json authentication"
                    + " file that will be used to authenticate HTTPS requests.";
    ...
    protected static final String BASIC_AUTH_PARAM_DESC =
            "If specified, this parameter supplies a username and password (in the format"
                    + " \"username:password\") to include in an HTTP Basic Auth header. The value"
                    + " \"PROMPT\" may also be used to specify that an interactive prompt should"
    ...
    protected static final String BEARER_TOKEN_PARAM_DESC =
            "If specified, this parameter supplies a Bearer Token to supply with the request. The"
                + " value \"PROMPT\" may also be used to specify that an interactive prompt should"
                + " request a Bearer Token from the user.";
    ...
    protected static final String FHIR_VERSION_PARAM_DESC =
            "The FHIR version being used. Valid values: ";
    protected static final String VERBOSE_LOGGING_PARAM = "l";
    protected static final String VERBOSE_LOGGING_PARAM_LONGOPT = "logging";
    protected static final String VERBOSE_LOGGING_PARAM_DESC =
            "If specified, verbose logging will be used.";
    ...
    protected void addBasicAuthOption(Options theOptions) {
        addOptionalOption(
                ...
                BASIC_AUTH_PARAM_NAME,
                BASIC_AUTH_PARAM_DESC);
        addOptionalOption(
                theOptions,
                null,
                BEARER_TOKEN_PARAM_LONGOPT,
                BEARER_TOKEN_PARAM_NAME,
    ...
    protected void addThreadCountOption(Options theOptions) {
        addOptionalOption(
                ...
                THREAD_COUNT,
                "count",
                "If specified, this argument specifies the number of worker threads used (default"
                        + " is "
    ...
    protected void addHttpsAuthOption(Options theOptions) {
        addOptionalOption(
                theOptions, null, TLS_AUTH_PARAM_LONGOPT, TLS_AUTH_PARAM_NAME, TLS_AUTH_PARAM_DESC);
    }
    ...
    protected void addFhirVersionOption(Options theOptions) {
        String versions =
                Arrays.stream(FhirVersionEnum.values())
    ...
    private void addOption(
        ...
        Option option =
                createOption(theRequired, theOpt, theLongOpt, theHasArgument, theDescription);
        ...
        if (isNotBlank(theOpt)) {
            ...
            if (theOptionGroup != null
                    && theOptionGroup.getOptions().stream()
                            .anyMatch(t -> theOpt.equals(t.getOpt()))) {
        ...
        if (isNotBlank(theLongOpt)) {
            ...
            if (theOptionGroup != null
                    && theOptionGroup.getOptions().stream()
                            .anyMatch(t -> theLongOpt.equals(t.getLongOpt()))) {
    ...
    protected void addVerboseLoggingOption(Options theOptions) {
        addOptionalOption(
                theOptions,
                VERBOSE_LOGGING_PARAM,
                VERBOSE_LOGGING_PARAM_LONGOPT,
                false,
                VERBOSE_LOGGING_PARAM_DESC);
    }
    ...
    private void downloadFileFromInternet(CloseableHttpResponse result, File localFile)
            throws IOException {
     ...
     */
    protected String getAndParseOptionBasicAuthHeader(
        ...
        if (theCommandLine.hasOption(theOptionName)) {
            ...
            basicAuthHeaderValue =
                    Constants.HEADER_AUTHORIZATION_VALPREFIX_BASIC + base64EncodedBasicAuth;
        }
    ...
    public <T extends Enum> T getAndParseOptionEnum(
        ...
        try {
            ...
            throw new ParseException(
                    Msg.code(1575) + "Invalid option \"" + val + "\" for option -" + theOption);
        }
    }
    ...
    public Integer getAndParsePositiveIntegerParam(CommandLine theCommandLine, String theName)
            throws ParseException {
        int minimum = 1;
        return doGetAndParseIntegerParam(theCommandLine, theName, minimum);
    }
    ...
    @Nullable
    private Integer doGetAndParseIntegerParam(
            CommandLine theCommandLine, String theName, int minimum) throws ParseException {
    ...
    protected int getThreadCount(CommandLine theCommandLine) throws ParseException {
        Integer parallelismThreadCount =
                getAndParsePositiveIntegerParam(theCommandLine, THREAD_COUNT);
        parallelismThreadCount =
                ObjectUtils.defaultIfNull(parallelismThreadCount, DEFAULT_THREAD_COUNT);
    ...
    protected Collection<File> loadFile(String theSpecUrl, String theFilepath, boolean theCacheFile)
        ...
        if (isNotBlank(theFilepath)) {
            ...
            File inputFile =
                    new File(
                            cacheDir,
                            "examples-json-" + getFhirContext().getVersion().getVersion() + ".zip");

            Date cacheExpiryDate = DateUtils.addHours(new Date(), -12);

            if (!inputFile.exists()
                    | (theCacheFile && FileUtils.isFileOlder(inputFile, cacheExpiryDate))) {

                File exampleFileDownloading =
                        new File(
                                cacheDir,
                                "examples-json-"
    ...
    protected IGenericClient newClient(
        ...
        if (isBlank(baseUrl)) {
            throw new ParseException(
                    Msg.code(1579) + "No target server (-" + BASE_URL_PARAM + ") specified.");
        } else if (!baseUrl.startsWith("http") && !baseUrl.startsWith("file")) {
            throw new ParseException(
                    Msg.code(1580)
                            + "Invalid target server specified, must begin with 'http' or 'file'.");
        }
    ...
    protected IGenericClient newClientWithBaseUrl(
        ...
        Optional<TlsAuthentication> tlsConfig =
                createTlsConfig(theCommandLine, theTlsAuthOptionName);
        ...
        String basicAuthHeaderValue =
                getAndParseOptionBasicAuthHeader(theCommandLine, theBasicAuthOptionName);
        if (isNotBlank(basicAuthHeaderValue)) {
            retVal.registerInterceptor(
                    new SimpleRequestHeaderInterceptor(
                            Constants.HEADER_AUTHORIZATION, basicAuthHeaderValue));
        }

        if (isNotBlank(theBearerTokenOptionName)) {
            String bearerToken =
                    getAndParseBearerTokenAuthHeader(theCommandLine, theBearerTokenOptionName);
            if (isNotBlank(bearerToken)) {
                retVal.registerInterceptor(
                        new SimpleRequestHeaderInterceptor(
                                Constants.HEADER_AUTHORIZATION,
                                Constants.HEADER_AUTHORIZATION_VALPREFIX_BEARER + bearerToken));
            }
        }
    ...
    private Optional<TlsAuthentication> createTlsConfig(
            CommandLine theCommandLine, String theTlsAuthOptionName) {
        ...
        try (FileReader fileReader = new FileReader(httpAuthFilePath)) {
            ...
            throw new RuntimeException(
                    Msg.code(2253) + "Could not create TLS configuration options", e);
        }
    }
    ...
    private String getAndParseBearerTokenAuthHeader(
            CommandLine theCommandLine, String theBearerTokenOptionName) throws ParseException {
    ...
    protected FhirVersionEnum parseFhirVersion(CommandLine theCommandLine) throws ParseException {
        ...
        if (isBlank(version)) {
            throw new ParseException(
                    Msg.code(1581) + "Missing required option: -" + FHIR_VERSION_PARAM);
        }
        try {
