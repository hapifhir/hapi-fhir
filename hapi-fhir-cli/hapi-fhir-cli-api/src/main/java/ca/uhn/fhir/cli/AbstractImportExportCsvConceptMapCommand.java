...
public abstract class AbstractImportExportCsvConceptMapCommand
    ...
    protected static final String FILE_PARAM_DESC =
            "The path and filename of the CSV file to be imported/exported (e.g. ./input.csv,"
                    + " ./output.csv, etc.).";
    ...
    protected Collection<Object> getFilterOutVersions() {
        return Sets.newHashSet(
                FhirVersionEnum.DSTU2_1, FhirVersionEnum.DSTU2_HL7ORG, FhirVersionEnum.DSTU2);
    }
    ...
    public void run(CommandLine theCommandLine) throws ParseException, ExecutionException {
        ...
        if (isBlank(targetServer)) {
            throw new ParseException(
                    Msg.code(1583) + "No target server (-" + BASE_URL_PARAM + ") specified.");
        ...
        if (isBlank(conceptMapUrl)) {
            throw new ParseException(
                    Msg.code(1585) + "No ConceptMap URL (" + CONCEPTMAP_URL_PARAM + ") specified.");
        } else {
        ...
        if (fhirVersion != FhirVersionEnum.DSTU3 && fhirVersion != FhirVersionEnum.R4) {
            throw new ParseException(
                    Msg.code(1587)
                            + "This command does not support FHIR version "
                            + fhirVersion
                            + ".");
        }
