...
public class BulkImportCommand extends BaseCommand {
	...
	public Options getOptions() {
		...
		addOptionalOption(
					...
					SOURCE_BASE,
					"base url",
					"The URL to advertise as the base URL for accessing the files (i.e. this is the"
						+ " address that this command will declare that it is listening on). If not"
						+ " present, the server will default to \"http://localhost:[port]\" which will"
						+ " only work if the server is on the same host.");
		addRequiredOption(
					options,
					null,
					SOURCE_DIRECTORY,
					"directory",
					"The source directory. This directory will be scanned for files with an extensions"
						+ " of .json, .ndjson, .json.gz and .ndjson.gz, and any files in this directory"
						+ " will be assumed to be NDJSON and uploaded. This command will read the first"
						+ " resource from each file to verify its resource type, and will assume that"
						+ " all resources in the file are of the same type.");
		addRequiredOption(
					options, null, TARGET_BASE, "base url", "The base URL of the target FHIR server.");
	...
	public void run(CommandLine theCommandLine) throws ParseException, ExecutionException {
		...
		IBaseResource outcome =
					client.operation()
								...
								.returnResourceType(
										myFhirCtx
													.getResourceDefinition("OperationOutcome")
		...
		ourLog.info(
					"Bulk import is now running. Do not terminate this command until all files have"
								+ " been uploaded.");
				...
				try {
					response =
								client.operation()
										...
										.withSearchParameter(
													Parameters.class, "_jobId", new StringParam(jobId))
										.returnMethodOutcome()
										.execute();
	...
	private IBaseParameters createRequest(
				String theBaseUrl, List<String> theIndexes, List<String> theResourceTypes) {
		...
		ParametersUtil.addParameterToParameters(
					ctx,
					retVal,
					BulkDataImportProvider.PARAM_INPUT_FORMAT,
					"code",
		...
		IBase storageDetail =
					ParametersUtil.addParameterToParameters(
								ctx, retVal, BulkDataImportProvider.PARAM_STORAGE_DETAIL);
		...
		for (int i = 0; i < theIndexes.size(); i++) {
				IBase input =
						ParametersUtil.addParameterToParameters(
									ctx, retVal, BulkDataImportProvider.PARAM_INPUT);
				ParametersUtil.addPartCode(
						ctx, input, BulkDataImportProvider.PARAM_INPUT_TYPE, theResourceTypes.get(i));
	...
	private List<String> startServer(int thePort, List<File> files) {
		...
		for (File t : files) {
				...
				BulkImportFileServlet.IFileSupplier fileSupplier =
						new BulkImportFileServlet.IFileSupplier() {
								@Override
	...
	private void scanDirectoryForJsonFiles(
				String baseDirectory, List<String> types, List<File> files) {
		try {
				File directory = new File(baseDirectory);
				final String[] extensions = new String[] {".json", ".ndjson", ".json.gz", ".ndjson.gz"};
				final IOFileFilter filter =
						FileFileFilter.INSTANCE.and(
									new SuffixFileFilter(extensions, IOCase.INSENSITIVE));
