...
/**
 * Intended for use with the HapiFhir CLI only. <br>
 * <br>
 * A Restful Factory to create clients, requests and responses based on the Apache httpclient
 * library. This class supports HTTP and HTTPS protocol and attempts to use the same client whenever
 * possible. The method {@link HapiFhirCliRestfulClientFactory#useHttp()} should be used if the
 * protocol needs to be changed to HTTP. Similarly, the method {@link
 * HapiFhirCliRestfulClientFactory#useHttps(TlsAuthentication)} should be used if the protocol needs
 * to be changed to HTTPS or if new TLS credentials are required for a client request.
 */
public class HapiFhirCliRestfulClientFactory extends RestfulClientFactory {
    ...
    public HapiFhirCliRestfulClientFactory(
            FhirContext theFhirContext, TlsAuthentication theTlsAuthentication) {
