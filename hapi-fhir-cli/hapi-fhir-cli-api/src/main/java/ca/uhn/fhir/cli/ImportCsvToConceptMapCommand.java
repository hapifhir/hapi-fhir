...
public class ImportCsvToConceptMapCommand extends AbstractImportExportCsvConceptMapCommand {
    ...
    protected static final String CONCEPTMAP_STATUS_PARAM_DESC =
            "The status of the ConceptMap resource to be imported/exported (i.e."
                    + " ConceptMap.status).";
    ...
    public Options getOptions() {
        ...
        // </editor-fold>
        addRequiredOption(
                options, FILE_PARAM, FILE_PARAM_LONGOPT, FILE_PARAM_NAME, FILE_PARAM_DESC);
    ...
    protected void parseAdditionalParameters(CommandLine theCommandLine) throws ParseException {
        ...
        if (isBlank(sourceValueSet)) {
            ...
            ourLog.info(
                    "Specified source value set (i.e. ConceptMap.sourceUri): {}", sourceValueSet);
        }
        ...
        if (isBlank(targetValueSet)) {
            ...
            ourLog.info(
                    "Specified target value set (i.e. ConceptMap.targetUri): {}", targetValueSet);
        }

        status = theCommandLine.getOptionValue(CONCEPTMAP_STATUS_PARAM);
        if (isBlank(status)) {
            throw new ParseException(
                    Msg.code(2132) + "No status (" + CONCEPTMAP_STATUS_PARAM + ") specified.");
        }
    }
        ...
        if (fhirVersion == FhirVersionEnum.DSTU3) {
            ...
            MethodOutcome methodOutcome =
                    client.update()
                            ...
                            .conditional()
                            .where(
                                    org.hl7.fhir.dstu3.model.ConceptMap.URL
                                            .matches()
                                            .value(conceptMapUrl))
                            .execute();
    ...
    private org.hl7.fhir.dstu3.model.ConceptMap convertCsvToConceptMapDstu3()
    ...
    private ConceptMap convertCsvToConceptMapR4() throws ExecutionException {
        ...
        try (Reader reader = getBufferedReader();
            ...
            Map<
                            ...
                            Map<
                                    TemporaryConceptMapGroupElement,
                                    Set<TemporaryConceptMapGroupElementTarget>>>
            ...
            for (Map.Entry<
                ...
                if (temporaryConceptMapGroup.hasSourceVersion()) {
                    conceptMapGroupComponent.setSourceVersion(
                            temporaryConceptMapGroup.getSourceVersion());
                }
                ...
                if (temporaryConceptMapGroup.hasTargetVersion()) {
                    conceptMapGroupComponent.setTargetVersion(
                            temporaryConceptMapGroup.getTargetVersion());
                }
                ...
                for (Map.Entry<
                    ...
                    if (temporaryConceptMapGroupElement.hasDisplay()) {
                        sourceElementComponent.setDisplay(
                                temporaryConceptMapGroupElement.getDisplay());
                    }

                    targetSet = elementEntry.getValue();
                    for (TemporaryConceptMapGroupElementTarget
                            temporaryConceptMapGroupElementTarget : targetSet) {

                        targetElementComponent = new TargetElementComponent();

                        if (temporaryConceptMapGroupElementTarget.hasCode()) {
                            targetElementComponent.setCode(
                                    temporaryConceptMapGroupElementTarget.getCode());
                        }

                        if (temporaryConceptMapGroupElementTarget.hasDisplay()) {
                            targetElementComponent.setDisplay(
                                    temporaryConceptMapGroupElementTarget.getDisplay());
                        }
                        ...
                        if (temporaryConceptMapGroupElementTarget.hasComment()) {
                            targetElementComponent.setComment(
                                    temporaryConceptMapGroupElementTarget.getComment());
                        }
