...
public class ExportConceptMapToCsvCommand extends AbstractImportExportCsvConceptMapCommand {
	...
	private void searchForConceptMapByUrl() throws ExecutionException {
		ourLog.info(
					"Searching for ConceptMap with specified URL (i.e. ConceptMap.url): {}",
					conceptMapUrl);
		if (fhirVersion == FhirVersionEnum.DSTU3) {
				...
				if (response.hasEntry()) {
					ourLog.info(
								"Found ConceptMap with specified URL (i.e. ConceptMap.url): {}",
								conceptMapUrl);
					org.hl7.fhir.dstu3.model.ConceptMap conceptMap =
								(org.hl7.fhir.dstu3.model.ConceptMap)
										response.getEntryFirstRep().getResource();
					convertConceptMapToCsv(conceptMap);
				} else {
					ourLog.info(
								"No ConceptMap exists with specified URL (i.e. ConceptMap.url): {}",
								conceptMapUrl);
				}
	...
	private void convertConceptMapToCsv(org.hl7.fhir.dstu3.model.ConceptMap theConceptMap)
		...
		try {
				convertConceptMapToCsv(
						(ConceptMap) VersionConvertorFactory_30_40.convertResource(theConceptMap));
