...
public class MdmSubmitSvcImpl implements IMdmSubmitSvc {
	...
	@Override
	@Transactional
	public long submitAllSourceTypesToMdm(
				@Nullable String theCriteria, @Nonnull RequestDetails theRequestDetails) {
		long submittedCount =
					myMdmSettings.getMdmRules().getMdmTypes().stream()
								.mapToLong(
										type ->
													submitSourceResourceTypeToMdm(
																type, theCriteria, theRequestDetails))
								.sum();
	...
	public long submitSourceResourceTypeToMdm(
		...
		SearchParameterMap spMap =
					myMdmSearchParamSvc.getSearchParameterMapFromCriteria(
								theSourceResourceType, theCriteria);
		spMap.setLoadSynchronous(true);
		spMap.setCount(myBufferSize);
		ISearchBuilder searchBuilder =
					myMdmSearchParamSvc.generateSearchBuilderForType(theSourceResourceType);
	...
	private long submitAllMatchingResourcesToMdmChannel(
		...
		try (IResultIterator query =
					theSearchBuilder.createQuery(
								theSpMap, searchRuntimeDetails, null, theRequestPartitionId)) {
				...
				throw new InternalErrorException(
						Msg.code(749)
									+ "Failure while attempting to query resources for "
	...
	/**
	* Given a collection of IResourcePersistentId objects, and a search builder, load the
	* IBaseResources and submit them to the MDM channel for processing.
	*
	* @param theSearchBuilder the related DAO search builder.
	* @param thePidsToSubmit The collection of PIDs whos resources you want to submit for MDM
	*     processing.
	...
	*/
	private long loadPidsAndSubmitToMdmChannel(
		...
		theSearchBuilder.loadResourcesByPid(
					thePidsToSubmit, Collections.emptyList(), resourcesToSubmit, false, null);
		ourLog.info("Submitting {} resources to MDM", resourcesToSubmit.size());
		resourcesToSubmit.forEach(
					resource -> myMdmChannelSubmitterSvc.submitResourceToMdmChannel(resource));
		return resourcesToSubmit.size();
	}

	@Override
	@Transactional
	public long submitPractitionerTypeToMdm(
				@Nullable String theCriteria, @Nonnull RequestDetails theRequestDetails) {
	...
	@Override
	@Transactional
	public long submitPatientTypeToMdm(
				@Nullable String theCriteria, @Nonnull RequestDetails theRequestDetails) {
