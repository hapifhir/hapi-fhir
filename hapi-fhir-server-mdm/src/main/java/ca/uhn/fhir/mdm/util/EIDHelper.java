...
public final class EIDHelper {
	...
	public CanonicalEID createHapiEid() {
		return new CanonicalEID(
					MdmConstants.HAPI_ENTERPRISE_IDENTIFIER_SYSTEM, UUID.randomUUID().toString(), null);
	}

	/**
	* Given an {@link IAnyResource} representing a type supported by MDM, retrieve their
	* externally-assigned EID, represented as a {@link CanonicalEID}
	*
	* @param theResource the resource to extract the EID from.
	* @return An optional {@link CanonicalEID} representing the external EID. Absent if the EID is
	*     not present.
	*/
	...
	/**
	* Given an {@link IAnyResource} representing a type supported by MDM, retrieve their
	* internally-assigned EID, represented as a {@link CanonicalEID}
	*
	* @param theResource the resource to extract the EID from.
	* @return An optional {@link CanonicalEID} representing the internal EID. Absent if the EID is
	*     not present.
	*/
	...
	/**
	* Determines whether two lists of {@link CanonicalEID} have any intersection. Two resources are
	* considered a match if a single {@link CanonicalEID} matches between the two collections.
	*
	...
	*/
	public boolean eidMatchExists(
				List<CanonicalEID> theFirstResourceEids, List<CanonicalEID> theSecondResourceEids) {
		List<String> collect =
					theFirstResourceEids.stream()
								.map(CanonicalEID::getValue)
								.collect(Collectors.toList());
		List<String> collect1 =
					theSecondResourceEids.stream()
								.map(CanonicalEID::getValue)
								.collect(Collectors.toList());
	...
	/**
	* An incoming resource is a potential duplicate if it matches a source resource that has a
	* golden resource with an official EID, but the incoming resource also has an EID that does not
	* match.
	*/
	public boolean hasEidOverlap(
