 ...
 * records. It does so by calling individual comparators, and returning a vector based on the
 * combination of field comparators that matched.
 */
@Service
public class MdmResourceMatcherSvc {
    ...
    private void addFieldMatchers() {
        if (myMdmRulesJson == null) {
            throw new ConfigurationException(
                    Msg.code(1521)
                            + "Failed to load MDM Rules.  If MDM is enabled, then MDM rules must be"
        ...
        for (MdmFieldMatchJson matchFieldJson : myMdmRulesJson.getMatchFields()) {
            myFieldMatchers.add(
                    new MdmResourceFieldMatcher(
                            myFhirContext, myMatcherFactory, matchFieldJson, myMdmRulesJson));
        }
    }

    /**
     * Given two {@link IBaseResource}s, perform all comparisons on them to determine an {@link
     * MdmMatchResultEnum}, indicating to what level the two resources are considered to be
     * matching.
     *
    ...
    public MdmMatchOutcome getMatchResult(
            IBaseResource theLeftResource, IBaseResource theRightResource) {
     ...
     * generated by the results of each of the given comparator matches. For example. start with a
     * binary representation of the value 0 for long: 0000 first_name matches, so the value `1` is
     * bitwise-ORed to the current value (0) in right-most position. `0001`
     *
     * <p>Next, we look at the second field comparator, and see if it matches. If it does, we
     * left-shift 1 by the index of the comparator, in this case also 1. `0010`
     *
     * <p>Then, we bitwise-or it with the current retval: 0001|0010 = 0011 The binary string is now
     * `0011`, which when you return it as a long becomes `3`.
     */
    private MdmMatchOutcome getMatchOutcome(
        ...
        // TODO GGG MDM: This grabs ALL comparators, not just the ones we care about (e.g. the ones
        // for Medication)
        ...
        for (int i = 0; i < myFieldMatchers.size(); ++i) {
            ...
            ourLog.trace(
                    ...
                    fieldComparator.getName(),
                    resourceType);
            MdmMatchEvaluation matchEvaluation =
                    fieldComparator.match(theLeftResource, theRightResource);
