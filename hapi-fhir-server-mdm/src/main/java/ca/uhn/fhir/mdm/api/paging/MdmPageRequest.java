 ...
 * itself into a standard {@link PageRequest} for spring data to use. The reason we don't use
 * PageRequest natively is because it is concerned with `pages` and `counts`, but we are using
 * `offset` and `count` which requires some minor translation.
 */
public class MdmPageRequest {
	...
	public MdmPageRequest(
		...
		myCount =
					theCount == null
								? theDefaultPageSize
								: Math.min(theCount.getValue(), theMaximumPageSize);
