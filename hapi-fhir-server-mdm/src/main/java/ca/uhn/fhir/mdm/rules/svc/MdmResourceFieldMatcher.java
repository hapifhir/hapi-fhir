...
/**
 * This class is responsible for performing matching between raw-typed values of a left record and a
 * right record.
 */
public class MdmResourceFieldMatcher {
     ...
     *
     * <p>In this implementation, it determines whether a given field matches between two resources.
     * Internally this is evaluated using FhirPath. If any of the elements of theLeftResource match
     * any of the elements of theRightResource, will return true. Otherwise, false.
     *
    ...
    private MdmMatchEvaluation match(IBase theLeftValue, IBase theRightValue) {
        ...
        if (matcher != null) {
            boolean isMatches =
                    matcher.matches(theLeftValue, theRightValue, myMdmFieldMatchJson.getMatcher());
        ...
        throw new InternalErrorException(
                Msg.code(1522)
                        + "Field Match "
                        + myName
                        + " has neither a matcher nor a similarity.");
    }
    ...
    private void validate(IBaseResource theResource) {
        ...
        if (ALL_RESOURCE_SEARCH_PARAM_TYPE.equals(myResourceType)) {
            boolean isMdmType =
                    myMdmRulesJson.getMdmTypes().stream()
                            .anyMatch(mdmType -> mdmType.equalsIgnoreCase(resourceType));
            Validate.isTrue(
                    isMdmType,
