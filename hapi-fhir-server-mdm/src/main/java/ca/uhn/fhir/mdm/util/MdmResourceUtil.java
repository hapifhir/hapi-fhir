...
public final class MdmResourceUtil {
    ...
    public static boolean isMdmAllowed(IBaseResource theBaseResource) {
        return theBaseResource
                        .getMeta()
                        .getTag(MdmConstants.SYSTEM_MDM_MANAGED, MdmConstants.CODE_NO_MDM_MANAGED)
                == null;
    }

    /**
     * Checks for the presence of the MDM-managed tag, indicating the MDM system has ownership of
     * this golden resource's links.
     *
    ...
    private static boolean resourceHasTag(
            IBaseResource theBaseResource, String theSystem, String theCode) {
    ...
    private static boolean resourceHasTagWithSystem(
            IBaseResource theBaseResource, @Nonnull String theSystem) {
        if (theBaseResource == null) {
            return false;
        }
        return theBaseResource.getMeta().getTag().stream()
                .anyMatch(tag -> theSystem.equalsIgnoreCase(tag.getSystem()));
    }
    ...
    /**
     * Sets the MDM-managed tag, indicating the MDM system has ownership of this Resource. No
     * changes are made if resource is already managed by MDM.
     *
    ...
    /**
     * WARNING: This code may _look_ like it replaces in place a code of a tag, but this DOES NOT
     * ACTUALLY WORK!. In reality what will happen is a secondary tag will be created with the same
     * system. the only way to actually remove a tag from a resource is by calling dao.removeTag().
     * This logic here is for the case where our representation of the resource still happens to
     * contain a reference to a tag, to make sure it isn't double-added.
     */
    @Nonnull
    private static IBaseResource setTagOnResource(
        ...
        Optional<? extends IBaseCoding> tagWithSystem =
                getTagWithSystem(theGoldenResource, theSystem);
