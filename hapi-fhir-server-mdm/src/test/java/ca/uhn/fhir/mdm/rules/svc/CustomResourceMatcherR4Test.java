...
public class CustomResourceMatcherR4Test extends BaseR4Test {
	...
	public void testExactNameAnyOrder() {
		MdmResourceMatcherSvc nameAnyOrderMatcher =
					buildMatcher(buildNameRules(MatchTypeEnum.NAME_ANY_ORDER, true));

		assertMatch(
					MdmMatchResultEnum.MATCH, nameAnyOrderMatcher.match(ourJohnHenry, ourJohnHenry));
		assertMatch(
					MdmMatchResultEnum.MATCH, nameAnyOrderMatcher.match(ourJohnHenry, ourHenryJohn));
		assertMatch(
					MdmMatchResultEnum.NO_MATCH, nameAnyOrderMatcher.match(ourJohnHenry, ourHenryJOHN));
		assertMatch(
					MdmMatchResultEnum.NO_MATCH, nameAnyOrderMatcher.match(ourJohnHenry, ourJohnHENRY));
		assertMatch(
					MdmMatchResultEnum.NO_MATCH, nameAnyOrderMatcher.match(ourJohnHenry, ourJaneHenry));
		assertMatch(
					MdmMatchResultEnum.NO_MATCH, nameAnyOrderMatcher.match(ourJohnHenry, ourJohnSmith));
		assertMatch(
					MdmMatchResultEnum.MATCH,
					nameAnyOrderMatcher.match(ourJohnHenry, ourJohnBillyHenry));
		assertMatch(
					MdmMatchResultEnum.MATCH,
					nameAnyOrderMatcher.match(ourJohnHenry, ourBillyJohnHenry));
	}

	@Test
	public void testNormalizedNameAnyOrder() {
		MdmResourceMatcherSvc nameAnyOrderMatcher =
					buildMatcher(buildNameRules(MatchTypeEnum.NAME_ANY_ORDER, false));
	...
	public void testExactNameFirstAndLast() {
		...
		assertMatchResult(
					...
					false,
					false,
					nameAnyOrderMatcher.match(ourJohnHenry, ourJohnHenry));
		assertMatch(
					MdmMatchResultEnum.NO_MATCH, nameAnyOrderMatcher.match(ourJohnHenry, ourHenryJohn));
		assertMatch(
					MdmMatchResultEnum.NO_MATCH, nameAnyOrderMatcher.match(ourJohnHenry, ourHenryJOHN));
	...
	private MdmRulesJson buildNameRules(MatchTypeEnum theAlgorithm, boolean theExact) {
		...
		MdmFieldMatchJson nameAnyOrderFieldMatch =
					new MdmFieldMatchJson()
