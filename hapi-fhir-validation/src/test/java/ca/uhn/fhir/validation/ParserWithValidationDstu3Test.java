...
public class ParserWithValidationDstu3Test {
    private static final Logger ourLog =
            LoggerFactory.getLogger(ParserWithValidationDstu3Test.class);
    ...
    public void testActivityDefinitionElementsOrder() {
        ...
        {
            IValidationContext<IBaseResource> validationCtx =
                    ValidationContext.forText(ourCtx, origContent, null);
        ...
        {
            IValidationContext<IBaseResource> validationCtx =
                    ValidationContext.forText(ourCtx, content, null);
            new FhirInstanceValidator(validationSupport).validateResource(validationCtx);
            ValidationResult result = validationCtx.toResult();
            for (SingleValidationMessage msg : result.getMessages()) {
                ourLog.info("{}", msg);
    ...
    public void testChildOrderWithChoiceTypeXml() {
        final String origContent =
                "<ActivityDefinition xmlns=\"http://hl7.org/fhir\"><id value=\"x1\"/><url"
                        + " value=\"http://testing.org\"/><status value=\"draft\"/><timingDateTime"
                        + " value=\"2011-02-03\"/></ActivityDefinition>";
    ...
    public void testConceptMapElementsOrderXml() {
        final String origContent =
                "<ConceptMap xmlns=\"http://hl7.org/fhir\"><id value=\"x1\"/><url"
                        + " value=\"http://testing.org\"/><status value=\"draft\"/><sourceUri"
                        + " value=\"http://url1\"/></ConceptMap>";
        final IParser parser = ourCtx.newXmlParser();
        IValidationSupport validationSupport = getValidationSupport();
