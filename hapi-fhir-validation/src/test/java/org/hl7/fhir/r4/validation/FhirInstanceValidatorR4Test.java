...
public class FhirInstanceValidatorR4Test extends BaseTest {
	...
	public void testValidateResourceContainingProfileDeclarationDoesntResolve() {
		...
		assertEquals("Unable to locate profile http://foo/structuredefinition/myprofile", errors.get(0).getMessage());
		assertEquals(ResultSeverityEnum.ERROR, errors.get(0).getSeverity());
		assertEquals("StructureDefinition reference \"http://foo/structuredefinition/myprofile\" could not be resolved", errors.get(1).getMessage());
		assertEquals(ResultSeverityEnum.WARNING, errors.get(1).getSeverity());
	}
