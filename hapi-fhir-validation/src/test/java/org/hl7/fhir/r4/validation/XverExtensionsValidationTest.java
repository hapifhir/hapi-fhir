...
public class XverExtensionsValidationTest {
    ...
    @Test
    public void validation_XverExtensionsAndCachingValidationSupport_ReturnsNoErrors()
            throws IOException {
        ...
        assertEquals(
                0, validationResult.getMessages().stream().filter(errorMessagePredicate()).count());
    }
    ...
    public void validation_InvalidExtensionValue_ReturnsError() throws IOException {
        ...
        assertEquals(
                1, validationResult.getMessages().stream().filter(errorMessagePredicate()).count());
        SingleValidationMessage errorMessage =
        ...
        med_req =
                ourCtx.newJsonParser()
                        .parseResource(
                                ...
                                loadResource("/r4/amz/medication-request-amz.json"));
        return med_req;
    }
    ...
    private IValidationSupport getValidationSupport() throws IOException {
        ...
        npmPackageSupport.loadPackageFromClasspath(
                "classpath:package/hl7.fhir.xver-extensions-0.0.11.tgz");

        return new ValidationSupportChain(
                new DefaultProfileValidationSupport(ourCtx), npmPackageSupport);
    }
}
