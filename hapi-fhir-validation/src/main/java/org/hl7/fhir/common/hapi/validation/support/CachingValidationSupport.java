...
public class CachingValidationSupport extends BaseValidationSupportWrapper
	...
	*/
	...
	public CachingValidationSupport(
				IValidationSupport theWrap, boolean theIsEnabledValidationForCodingsLogicalAnd) {
	...
	*/
	...
	public List<IBaseResource> fetchAllConformanceResources() {
		...
		return loadFromCacheWithAsyncRefresh(
					myCache, key, t -> super.fetchAllConformanceResources());
	}
	...
	public <T extends IBaseResource> List<T> fetchAllStructureDefinitions() {
		...
		return loadFromCacheWithAsyncRefresh(
					myCache, key, t -> super.fetchAllStructureDefinitions());
	}
	...
	public <T extends IBaseResource> List<T> fetchAllNonBaseStructureDefinitions() {
		...
		return loadFromCacheWithAsyncRefresh(
					myCache, key, t -> super.fetchAllNonBaseStructureDefinitions());
	}

	@Override
	public IBaseResource fetchCodeSystem(String theSystem) {
		return loadFromCache(
					myCache, "fetchCodeSystem " + theSystem, t -> super.fetchCodeSystem(theSystem));
	}
	...
	public IBaseResource fetchStructureDefinition(String theUrl) {
		return loadFromCache(
					myCache,
					"fetchStructureDefinition " + theUrl,
					t -> super.fetchStructureDefinition(theUrl));
	}

	@Override
	public byte[] fetchBinary(String theBinaryKey) {
		return loadFromCache(
					myCache, "fetchBinary " + theBinaryKey, t -> super.fetchBinary(theBinaryKey));
	}
	...
	public boolean isCodeSystemSupported(
		...
		Boolean retVal =
					loadFromCacheReentrantSafe(
								myCache,
								key,
								t -> super.isCodeSystemSupported(theValidationSupportContext, theSystem));
		assert retVal != null;
		return retVal;
	}
	...
	public LookupCodeResult lookupCode(
		...
		String key =
					"lookupCode "
								...
								+ theCode
								+ " "
								+ defaultIfBlank(theDisplayLanguage, "NO_LANG");
		return loadFromCache(
					myLookupCodeCache,
					key,
					t ->
	...
	public IValidationSupport.CodeValidationResult validateCodeInValueSet(
		...
		Optional<String> valueSetUrl =
					urlChild.getAccessor().getValues(theValueSet).stream()
		...
		if (valueSetUrl.isPresent()) {
				String key =
						"validateCodeInValueSet "
									+ theValidationOptions.toString()
									+ " "
									+ defaultString(theCodeSystem)
									+ " "
									+ defaultString(theCode)
									+ " "
									+ defaultString(theDisplay)
									+ " "
									+ valueSetUrl.get();
	...
	public TranslateConceptResults translateConcept(TranslateCodeRequest theRequest) {
		return loadFromCache(
					myTranslateCodeCache, theRequest, k -> super.translateConcept(theRequest));
	}
	...
	private <S, T> T loadFromCache(Cache<S, Object> theCache, S theKey, Function<S, T> theLoader) {
		...
		Function<S, Optional<T>> loaderWrapper =
					key -> Optional.ofNullable(theLoader.apply(theKey));
	...
	/**
	* The Caffeine cache uses ConcurrentHashMap which is not reentrant, so if we get unlucky and
	* the hashtable needs to grow at the same time as we are in a reentrant cache lookup, the
	* thread will deadlock. Use this method in place of loadFromCache in situations where a cache
	* lookup calls another cache lookup within its lambda
	*/
	@Nullable
	private <S, T> T loadFromCacheReentrantSafe(
				Cache<S, Object> theCache, S theKey, Function<S, T> theLoader) {
	...
	private <S, T> T loadFromCacheWithAsyncRefresh(
				Cache<S, Object> theCache, S theKey, Function<S, T> theLoader) {
