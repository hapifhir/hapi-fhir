...
/**
 * This {@link IValidationSupport validation support module} can be used to validate codes against
 * common CodeSystems that are commonly used, but are not distriuted with the FHIR specification for
 * various reasons (size, complexity, etc.).
 *
 * <p>See <a
 * href="https://hapifhir.io/hapi-fhir/docs/validation/validation_support_modules.html#CommonCodeSystemsTerminologyService">CommonCodeSystemsTerminologyService</a>
 * in the HAPI FHIR documentation for details about what is and isn't covered by this class.
 */
...
public class CommonCodeSystemsTerminologyService implements IValidationSupport {
	...
	public static final String ALL_LANGUAGES_VALUESET_URL =
				"http://hl7.org/fhir/ValueSet/all-languages";
	private static final String USPS_CODESYSTEM_URL = "https://www.usps.com/";
	private static final String USPS_VALUESET_URL =
				"http://hl7.org/fhir/us/core/ValueSet/us-core-usps-state";
	private static final Logger ourLog =
				LoggerFactory.getLogger(CommonCodeSystemsTerminologyService.class);
	private static final Map<String, String> USPS_CODES =
				Collections.unmodifiableMap(buildUspsCodes());
	private static final Map<String, String> ISO_4217_CODES =
				Collections.unmodifiableMap(buildIso4217Codes());
	private static final Map<String, String> ISO_3166_CODES =
				Collections.unmodifiableMap(buildIso3166Codes());
	...
	public CodeValidationResult validateCodeInValueSet(
		...
		return validateCode(
					theValidationSupportContext, theOptions, theCodeSystem, theCode, theDisplay, url);
	}
	...
	public CodeValidationResult validateCode(
		...
		switch (defaultString(theValueSetUrl)) {
				...
				case LANGUAGES_VALUESET_URL:
					...
					if (languagesVs == null) {
						...
						myLanguagesVs =
									myVersionCanonicalizer.valueSetToValidatorCanonical(languagesVs);
					}
		...
		if (handlerMap != null) {
				...
				return new CodeValidationResult()
						.setSeverity(IssueSeverity.ERROR)
						.setMessage(
									"Code \"" + theCode + "\" is not in system: " + USPS_CODESYSTEM_URL);
		}
	...
	private LookupCodeResult lookupLanguageCode(String theCode) {
		...
		if (hasRegionAndCodeSegments) {
				...
				language =
						myLanguagesLanugageMap.get(
									theCode.substring(0, langRegionSeparatorIndex).toLowerCase());
				region =
						myLanguagesRegionMap.get(
									theCode.substring(langRegionSeparatorIndex + 1).toUpperCase());

				if (language == null || region == null) {
					// In case the user provides both a language and a region, they must both be valid
					// for the lookup to succeed.
					ourLog.warn(
								"Couldn't find a valid bcp47 language-region combination from code: {}",
								theCode);
	...
	private LookupCodeResult buildLookupResultForLanguageAndRegion(
				@Nonnull String theOriginalCode,
				@Nonnull String theLanguage,
				@Nonnull String theRegion) {
	...
	private void initializeBcp47LanguageMap() {
		...
		String input =
					ClasspathUtil.loadResource(
								"org/hl7/fhir/common/hapi/validation/support/registry.json");
		ArrayNode map;
		try {
				...
				case DSTU3:
					normalized =
								VersionConvertorFactory_30_40.convertResource(
										retVal, new BaseAdvisor_30_40(false));
					break;
				case R4:
					normalized = retVal;
					break;
				case R4B:
					Resource normalized50 =
								VersionConvertorFactory_40_50.convertResource(
										retVal, new BaseAdvisor_40_50(false));
					normalized =
								VersionConvertorFactory_43_50.convertResource(
										normalized50, new BaseAdvisor_43_50());
					break;
				case R5:
					normalized =
								VersionConvertorFactory_40_50.convertResource(
	...
	public static String getValueSetUrl(
				FhirContext theFhirContext, @Nonnull IBaseResource theValueSet) {
		String url;
	...
	public static String getCodeSystemUrl(
				@Nonnull FhirContext theFhirContext, @Nonnull IBaseResource theCodeSystem) {
		String url;
		FhirVersionEnum structureFhirVersionEnum =
					getFhirVersionEnum(theFhirContext, theCodeSystem);
		switch (structureFhirVersionEnum) {
				case R4:
					{
	...
	public static String getValueSetVersion(
				@Nonnull FhirContext theFhirContext, @Nonnull IBaseResource theValueSet) {
		String version;
