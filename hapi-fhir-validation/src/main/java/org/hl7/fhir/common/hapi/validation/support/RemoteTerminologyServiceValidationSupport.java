...
/**
 * This class is an implementation of {@link IValidationSupport} that fetches validation codes from
 * a remote FHIR based terminology server. It will invoke the FHIR <a
 * href="http://hl7.org/fhir/valueset-operation-validate-code.html">ValueSet/$validate-code</a>
 * operation in order to validate codes.
 */
public class RemoteTerminologyServiceValidationSupport extends BaseValidationSupport
        implements IValidationSupport {
    private static final Logger ourLog =
            LoggerFactory.getLogger(RemoteTerminologyServiceValidationSupport.class);
    ...
    public CodeValidationResult validateCodeInValueSet(
        ...
        String valueSetUrl =
                DefaultProfileValidationSupport.getConformanceResourceUrl(myCtx, valueSet);
    ...
    private String extractCodeSystemForCode(ValueSet theValueSet, String theCode) {
        ...
        // when component has more than one include, their codeSystem(s) could be different, so we
        // need to make sure
        ...
        // at this point codeSystem couldn't be extracted for a multi-include ValueSet. Just on case
        // it was
        // because the format was not well handled, let's allow to watch the VS by an easy logging
        // change
        ourLog.trace(
                "CodeSystem couldn't be extracted for code: {} for ValueSet: {}",
                theCode,
                theValueSet.getId());
        return null;
    }
            ...
            case R4:
                ...
                if (!StringUtils.isEmpty(theSystem)) {
                    ParametersUtil.addParameterToParametersString(
                            fhirContext, params, "system", theSystem);
                }
                if (!StringUtils.isEmpty(theDisplayLanguage)) {
                    ParametersUtil.addParameterToParametersString(
                            fhirContext, params, "language", theDisplayLanguage);
                }
                    ...
                    switch (fhirVersion) {
                        case DSTU3:
                            return generateLookupCodeResultDSTU3(
                                    theCode,
                                    theSystem,
                                    (org.hl7.fhir.dstu3.model.Parameters) outcome);
                        case R4:
    ...
    private LookupCodeResult generateLookupCodeResultDSTU3(
        ...
        // NOTE: I wanted to put all of this logic into the IValidationSupport Class, but it
        // would've required adding
        //       several new dependencies on version-specific libraries and that is explicitly
        // forbidden (see comment in POM).
        ...
        for (org.hl7.fhir.dstu3.model.Parameters.ParametersParameterComponent parameterComponent :
            ...
            switch (parameterComponent.getName()) {
                case "property":
                    org.hl7.fhir.dstu3.model.Property part =
                            parameterComponent.getChildByName("part");
                    // The assumption here is that we may only have 2 elements in this part, and if
                    // so, these 2 will be saved
                ...
                case "designation":
                    ...
                    for (org.hl7.fhir.dstu3.model.Parameters.ParametersParameterComponent
                            designationComponent : parameterComponent.getPart()) {
    ...
    public TranslateConceptResults translateConcept(TranslateCodeRequest theRequest) {
        ...
        IBaseParameters params =
                RemoteTerminologyUtil.buildTranslateInputParameters(fhirContext, theRequest);
    ...
    protected CodeValidationResult invokeRemoteValidateCode(
        ...
        Validate.isTrue(
                resultValues.size() == 1,
                "Response contained %d 'result' values",
                resultValues.size());
    ...
    protected IBaseParameters buildValidateCodeInputParameters(
        ...
        if (theValueSet == null && theValueSetUrl == null) {
            ParametersUtil.addParameterToParametersUri(
                    getFhirContext(), params, "url", theCodeSystem);
            ParametersUtil.addParameterToParametersString(
                    getFhirContext(), params, "code", theCode);
            if (isNotBlank(theDisplay)) {
                ParametersUtil.addParameterToParametersString(
                        getFhirContext(), params, "display", theDisplay);
            }
            return params;
        }
        ...
        if (theValueSet != null) {
            ParametersUtil.addParameterToParameters(
                    getFhirContext(), params, "valueSet", theValueSet);
        }
        return params;
    }
     ...
     *
     * <p>Note that this method is not thread-safe and should only be called prior to this module
     * being used.
     *
