 ...
 *
 * <p>This module currently supports the following FHIR versions:
 *
 * <ul>
 *   <li>DSTU3
 *   <li>R4
...
public class SnapshotGeneratingValidationSupport implements IValidationSupport {
	private static final Logger ourLog =
				LoggerFactory.getLogger(SnapshotGeneratingValidationSupport.class);
	...
	public IBaseResource generateSnapshot(
		...
		try {
				...
				if (theValidationSupportContext.getCurrentlyGeneratingSnapshots().contains(inputUrl)) {
					ourLog.warn(
								"Detected circular dependency, already generating snapshot for: {}",
								inputUrl);
					return theInput;
				}
				...
				if (isBlank(baseDefinition)) {
					throw new PreconditionFailedException(
								Msg.code(704)
										...
										+ inputCanonical.getIdElement().getId()
										+ ", url="
										+ inputCanonical.getUrl()
										+ "] has no base");
				}
				...
				IBaseResource base =
						theValidationSupportContext
									...
									.fetchStructureDefinition(baseDefinition);
				if (base == null) {
					throw new PreconditionFailedException(
								Msg.code(705) + "Unknown base definition: " + baseDefinition);
				}
				...
				IWorkerContext context =
						new VersionSpecificWorkerContextWrapper(
									theValidationSupportContext, myVersionCanonicalizer);
				ProfileUtilities profileUtilities =
						new ProfileUtilities(context, messages, profileKnowledgeProvider);
				profileUtilities.generateSnapshot(
						baseCanonical, inputCanonical, theUrl, theWebUrl, theProfileName);
				...
				switch (getFhirVersionEnum(
					...
					case DSTU3:
						...
						break;
					case R4:
						org.hl7.fhir.r4.model.StructureDefinition generatedR4 =
					...
					case R4B:
						...
						break;
					case R5:
						org.hl7.fhir.r5.model.StructureDefinition generatedR5 =
