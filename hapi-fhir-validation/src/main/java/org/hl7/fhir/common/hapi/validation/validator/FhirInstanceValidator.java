...
public class FhirInstanceValidator extends BaseValidatorBridge implements IInstanceValidatorModule {
     ...
     * Constructor
     *
     * <p>Uses {@link DefaultProfileValidationSupport} for {@link IValidationSupport validation
     * support}
     */
    ...
    public FhirInstanceValidator(IValidationSupport theValidationSupport) {
        if (theValidationSupport.getFhirContext().getVersion().getVersion()
                == FhirVersionEnum.DSTU2) {
            myValidationSupport =
                    new HapiToHl7OrgDstu2ValidatingSupportWrapper(theValidationSupport);
        } else {
    ...
    /**
     * Every element in a resource or data type includes an optional <it>extension</it> child
     * element which is identified by it's {@code url attribute}. There exists a number of
     * predefined extension urls or extension domains:
     *
     * <ul>
     *   <li>any url which contains {@code example.org}, {@code nema.org}, or {@code acme.com}.
     *   <li>any url which starts with {@code http://hl7.org/fhir/StructureDefinition/}.
     * </ul>
     *
     * It is possible to extend this list of known extension by defining custom extensions: Any url
     * which starts which one of the elements in the list of custom extension domains is considered
     * as known.
     *
     * <p>Any unknown extension domain will result in an information message when validating a
     * resource.
     */
     ...
     *
     * <p>The FHIR Instance Validator has a number of checks for best practices in terms of FHIR
     * usage. If this setting is set to {@link BestPracticeWarningLevel#Error}, any resource data
     * which does not meet these best practices will be reported at the ERROR level. If this setting
     * is set to {@link BestPracticeWarningLevel#Ignore}, best practice guielines will be ignored.
     *
     ...
     * will be reported at this level.
     *
     * <p>The FHIR Instance Validator has a number of checks for best practices in terms of FHIR
     * usage. If this setting is set to {@link BestPracticeWarningLevel#Error}, any resource data
     * which does not meet these best practices will be reported at the ERROR level. If this setting
    ...
    /**
     * Returns the {@link IValidationSupport validation support} in use by this validator. Default
     * is an instance of DefaultProfileValidationSupport if the no-arguments constructor for this
     * object was used.
     */
    ...
    /**
     * Sets the {@link IValidationSupport validation support} in use by this validator. Default is
     * an instance of DefaultProfileValidationSupport if the no-arguments constructor for this
     * object was used.
     */
    ...
    /**
     * If set to {@literal true} (default is true) extensions which are not known to the validator
     * (e.g. because they have not been explicitly declared in a profile) will be validated but will
     * not cause an error.
     */
    ...
    /**
     * Clear any cached data held by the validator or any of its internal stores. This is mostly
     * intended for unit tests, but could be used for production uses too.
     */
