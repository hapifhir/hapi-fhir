...
public class FHIRPathResourceGeneratorR4<T extends Resource> {
	...
	private void handlePrimitiveNode(ExpressionNode fhirPath) {
		...
		RuntimePrimitiveDatatypeDefinition primitiveTarget =
				(RuntimePrimitiveDatatypeDefinition) childDefinition.getChildByName(fhirPath.getName());
	...
	private void handleWhereFunctionParam(ExpressionNode param) {
		...
		if (matchingNodes.size() == 0) {
			if (unlabeledNodes.size() == 0) {
				...
				ICompositeType compositeNode =
						compositeTarget.newInstance(latestTier.childDefinition.getInstanceConstructorArguments());
