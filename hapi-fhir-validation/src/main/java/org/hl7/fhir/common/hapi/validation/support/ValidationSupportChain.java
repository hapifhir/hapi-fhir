...
public class ValidationSupportChain implements IValidationSupport {
			...
			if (translations != null) {
				...
				if (!translations.isEmpty()) {
					if (ourLog.isDebugEnabled()) {
						ourLog.debug("{} found {} concept translation{} for {}",
							next.getName(),
							translations.size(),
	...
	public boolean isValueSetSupported(ValidationSupportContext theValidationSupportContext, String theValueSetUrl) {
		for (IValidationSupport next : myChain) {
			...
			if (retVal) {
				if (ourLog.isDebugEnabled()) {
					ourLog.debug("ValueSet {} found in {}",
						theValueSetUrl,
						next.getName());
				}
				return true;
	...
	public IBaseResource generateSnapshot(
		...
		for (IValidationSupport next : myChain) {
			...
			if (retVal != null) {
				if (ourLog.isDebugEnabled()) {
					ourLog.debug("Profile snapshot for {} generated by {}",
						theInput.getIdElement(),
						next.getName());
				}
				return retVal;
			}
	...
	public ValueSetExpansionOutcome expandValueSet(
		...
		for (IValidationSupport next : myChain) {
			...
			if (expanded != null) {
				if (ourLog.isDebugEnabled()) {
					ourLog.debug("ValueSet {} expanded by {}",
						theValueSetToExpand.getIdElement(),
						next.getName());
				}
				return expanded;
			}
	...
	public IBaseResource fetchCodeSystem(String theSystem) {
		for (IValidationSupport next : myChain) {
			...
			if (retVal != null) {
				if (ourLog.isDebugEnabled()) {
					ourLog.debug("CodeSystem {} with System {} fetched by {}",
						retVal.getIdElement(),
						theSystem,
						next.getName());
				}
	...
	public IBaseResource fetchValueSet(String theUrl) {
		for (IValidationSupport next : myChain) {
			...
			if (retVal != null) {
				if (ourLog.isDebugEnabled()) {
					ourLog.debug("ValueSet {} with URL {} fetched by {}",
						retVal.getIdElement(),
						theUrl,
						next.getName());
				}
	...
	public <T extends IBaseResource> T fetchResource(Class<T> theClass, String theUri) {
		for (IValidationSupport next : myChain) {
			...
			if (retVal != null) {
				if (ourLog.isDebugEnabled()) {
					ourLog.debug("Resource {} with URI {} fetched by {}",
						retVal.getIdElement(),
						theUri,
						next.getName());
				}
	...
	public byte[] fetchBinary(String key) {
		for (IValidationSupport next : myChain) {
			...
			if (retVal != null) {
				if (ourLog.isDebugEnabled()) {
					ourLog.debug("Binary with key {} fetched by {}",
						key,
						next.getName());
				}
				return retVal;
	...
	public IBaseResource fetchStructureDefinition(String theUrl) {
		for (IValidationSupport next : myChain) {
			...
			if (retVal != null) {
				if (ourLog.isDebugEnabled()) {
					ourLog.debug("StructureDefinition with URL {} fetched by {}",
						theUrl,
						next.getName());
				}
				return retVal;
	...
	public boolean isCodeSystemSupported(ValidationSupportContext theValidationSupportContext, String theSystem) {
		for (IValidationSupport next : myChain) {
			if (next.isCodeSystemSupported(theValidationSupportContext, theSystem)) {
				if (ourLog.isDebugEnabled()) {
					ourLog.debug("CodeSystem with System {} is supported by {}",
						theSystem,
						next.getName());
				}
				return true;
	...
	public CodeValidationResult validateCode(
		...
		for (IValidationSupport next : myChain) {
			if ((isBlank(theValueSetUrl) && next.isCodeSystemSupported(theValidationSupportContext, theCodeSystem))
				...
				if (retVal != null) {
					if (ourLog.isDebugEnabled()) {
						ourLog.debug("Code {}|{} '{}' in ValueSet {} validated by {}",
							theCodeSystem,
							theCode,
							theDisplay,
							theValueSetUrl,
	...
	public LookupCodeResult lookupCode(
		...
		for (IValidationSupport next : myChain) {
			if (next.isCodeSystemSupported(theValidationSupportContext, theSystem)) {
				LookupCodeResult lookupCodeResult = next.lookupCode(theValidationSupportContext, theSystem, theCode, theDisplayLanguage);
				if (ourLog.isDebugEnabled()) {
					ourLog.debug("Code {}|{}{} {} by {}",
						theSystem,
						theCode,
