...
public class ValidationSupportChain implements IValidationSupport {
     ...
     *
     * <p>Note that this method is not thread-safe. All validation support modules should be added
     * prior to use.
     *
     * @param theValidationSupport The validation support. Must not be null, and must have a {@link
     *     #getFhirContext() FhirContext} that is configured for the same FHIR version as other
    ...
    public void addValidationSupport(int theIndex, IValidationSupport theValidationSupport) {
        ...
        if (existingFhirContext != null) {
            ...
            if (!existingVersion.equals(newVersion)) {
                String message =
                        "Trying to add validation support of version "
                                + newVersion
                                + " to chain with "
                                + myChain.size()
                                + " entries of version "
    ...
    /**
     * Removes an item from the chain. Note that this method is mostly intended for testing.
     * Removing items from the chain while validation is actually occurring is not an expected use
     * case for this class.
     */
    ...
    public boolean isRemoteTerminologyServiceConfigured() {
        if (myChain != null) {
            Optional<IValidationSupport> remoteTerminologyService =
                    myChain.stream()
            ...
            if (allStructureDefinitions != null) {
                for (IBaseResource next : allStructureDefinitions) {
                    ...
                    IPrimitiveType<?> urlType =
                            getFhirContext()
                                    .newTerser()
                                    .getSingleValueOrNull(next, "url", IPrimitiveType.class);
                    if (urlType == null
    ...
    public CodeValidationResult validateCode(
        ...
        for (IValidationSupport next : myChain) {
            if ((isBlank(theValueSetUrl)
                            && next.isCodeSystemSupported(
                                    theValidationSupportContext, theCodeSystem))
                    || (isNotBlank(theValueSetUrl)
                            && next.isValueSetSupported(
