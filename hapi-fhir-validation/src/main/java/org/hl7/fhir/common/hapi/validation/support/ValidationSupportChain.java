...
public class ValidationSupportChain implements IValidationSupport {
	...
	public IBaseResource fetchCodeSystem(String theSystem) {
		...
		if (retVal.getValue() == null) {
			CanonicalReference requested =
				CanonicalReference.parse(theSystem);

			if (requested.url().startsWith(OID_URN_PREFIX)) {
				IBaseResource aliasResult =
					fetchCodeSystemByIdentifier(
				...
				if (aliasResult != null) {
					ourLog.info(
						"Resolved CodeSystem identifier alias {}",
						requested.asString());
	...
	@Nullable
	private IBaseResource fetchCodeSystemByCanonicalUrl(
			String theSystem) {
		for (IValidationSupport next : myChain) {
			IBaseResource outcome;
			if (next instanceof ValidationSupportChain nestedChain) {
				outcome = nestedChain.fetchCodeSystemByCanonicalUrl(
					theSystem);
			} else {
				outcome = next.fetchCodeSystem(
					theSystem);
			}
			if (outcome != null) {
				ourLog.debug(
					"CodeSystem {} fetched by canonical URL from {}",
					theSystem,
					next.getName());
	...
	public IBaseResource fetchCodeSystemByIdentifier(
		...
		for (IValidationSupport next : myChain) {
			IBaseResource outcome = next.fetchCodeSystemByIdentifier(
							theIdentifierSystem,
							theIdentifierValue,
							theVersion);
