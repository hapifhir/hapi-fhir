...
public class DefaultProfileValidationSupportNpmStrategy extends NpmPackageValidationSupport {
    private static final Logger ourLog =
            LoggerFactory.getLogger(DefaultProfileValidationSupportNpmStrategy.class);
    ...
    public DefaultProfileValidationSupportNpmStrategy(@Nonnull FhirContext theFhirContext) {
        ...
        try {
            ...
            loadPackageFromClasspath(
                    "org/hl7/fhir/r5/packages/hl7.fhir.uv.extensions.r5-1.0.0.tgz");
            loadPackageFromClasspath("org/hl7/fhir/r5/packages/hl7.terminology-5.1.0.tgz");
        } catch (IOException e) {
            throw new ConfigurationException(
                    Msg.code(2333)
                            + "Failed to load required validation resources. Make sure that the"
                            + " appropriate hapi-fhir-validation-resources-VER JAR is on the"
                            + " classpath",
                    e);
        }
