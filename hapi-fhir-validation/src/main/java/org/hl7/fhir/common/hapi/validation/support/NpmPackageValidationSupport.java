...
/**
 * This interceptor loads and parses FHIR NPM Conformance Packages, and makes the artifacts foudn
 * within them available to the FHIR validator.
 *
 * @since 5.5.0
 */
public class NpmPackageValidationSupport extends PrePopulatedValidationSupport {
    ...
    /**
     * Load an NPM package using a classpath specification, e.g. <code>
     * /path/to/resource/my_package.tgz</code>. The classpath spec can optionally be prefixed with
     * the string <code>classpath:</code>
     *
    ...
    private void loadResourcesFromPackage(NpmPackage thePackage) {
        ...
        for (String nextFile : packageFolder.listFiles()) {
            if (nextFile.toLowerCase(Locale.US).endsWith(".json")) {
                String input =
                        new String(
                                packageFolder.getContent().get(nextFile), StandardCharsets.UTF_8);
