...
class JobQuerySvc {
                ...
                .orElseThrow(
                        () ->
                                new ResourceNotFoundException(
                                        Msg.code(2040)
                                                + "Unknown instance ID: "
                                                + UrlUtil.escapeUrlParam(theInstanceId)
    ...
    public List<JobInstance> fetchRecentInstances(int theCount, int theStart) {
        return massageInstancesForUserAccess(
                myJobPersistence.fetchRecentInstances(theCount, theStart));
    }

    private List<JobInstance> massageInstancesForUserAccess(
            List<JobInstance> theFetchRecentInstances) {
    ...
    /** Scans a model object for fields marked as {@link PasswordField} and nulls them */
    ...
    public List<JobInstance> getInstancesByJobDefinitionIdAndEndedStatus(
        ...
        if (theEnded == null) {
            return myJobPersistence.fetchInstancesByJobDefinitionId(
                    theJobDefinitionId, theCount, theStart);
        }
        ...
        return getInstancesByJobDefinitionAndStatuses(
                theJobDefinitionId, requestedStatus, theCount, theStart);
    }
