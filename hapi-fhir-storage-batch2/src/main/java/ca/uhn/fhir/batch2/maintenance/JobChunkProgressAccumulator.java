...
/**
 * While performing cleanup, the cleanup job loads all work chunks to examine their status. This
 * bean collects the counts that are found, so that they can be reused for maintenance jobs without
...
public class JobChunkProgressAccumulator {
	...
	private final Multimap<String, ChunkStatusCountValue> myInstanceIdToChunkStatuses =
	...
	private Collection<ChunkStatusCountValue> getChunkStatuses(String theInstanceId) {
		Collection<ChunkStatusCountValue> chunkStatuses =
					myInstanceIdToChunkStatuses.get(theInstanceId);
	...
	public void addChunk(WorkChunk theChunk) {
		...
		// Note: If chunks are being written while we're executing, we may see the same chunk twice.
		// This
		...
		if (myConsumedInstanceAndChunkIds.add(instanceId + " " + chunkId)) {
				...
				myInstanceIdToChunkStatuses.put(
						instanceId,
						new ChunkStatusCountValue(
									chunkId, theChunk.getTargetStepId(), theChunk.getStatus()));
		}
	}
