...
public class JobStepExecutor<PT extends IModelJson, IT extends IModelJson, OT extends IModelJson> {
    ...
    public void executeStep() {
        ...
        if (stepExecutorOutput.getDataSink().firstStepProducedNothing()) {
            ourLog.info(
                    "First step of job myInstance {} produced no work chunks, marking as completed"
                            + " and setting end date",
                    myInstanceId);
    ...
    private void handleFastTracking(BaseDataSink<PT, IT, OT> theDataSink) {
        if (theDataSink.getWorkChunkCount() <= 1) {
            ourLog.debug(
                    "Gated job {} step {} produced exactly one chunk:  Triggering a maintenance"
                            + " pass.",
                    myDefinition.getJobDefinitionId(),
                    myCursor.currentStep.getStepId());
            // wipmb 6.8 either delete fast-tracking, or narrow this call to just this instance and
            // step
            // This runs full maintenance for EVERY job as each chunk completes in a fast tracked
            // job.  That's a LOT of work.
