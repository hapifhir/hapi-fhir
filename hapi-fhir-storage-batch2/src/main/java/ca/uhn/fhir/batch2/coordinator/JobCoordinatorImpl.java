...
public class JobCoordinatorImpl implements IJobCoordinator {
    ...
    public Batch2JobStartResponse startInstance(
        ...
        if (theStartRequest.isUseCache()) {
            FetchJobInstancesRequest request =
                    new FetchJobInstancesRequest(
                            ...
                            theStartRequest.getParameters(),
                            getStatesThatTriggerCache());
                        ...
                        .orElseThrow(
                                () ->
                                        new IllegalArgumentException(
                                                Msg.code(2063)
                                                        + "Unknown job definition ID: "
                                                        + theStartRequest.getJobDefinitionId()));

        myJobParameterJsonValidator.validateJobParameters(
                theRequestDetails, theStartRequest, jobDefinition);
        ...
        IJobPersistence.CreateResult instanceAndFirstChunk =
                myTransactionService
                        ...
                        .execute(
                                () ->
                                        myJobPersistence.onCreateWithFirstChunk(
                                                jobDefinition, theStartRequest.getParameters()));

        JobWorkNotification workNotification =
                JobWorkNotification.firstStepNotification(
                        jobDefinition,
                        instanceAndFirstChunk.jobInstanceId,
                        instanceAndFirstChunk.workChunkId);
    ...
    /**
     * Cache will be used if an identical job is QUEUED or IN_PROGRESS. Otherwise a new one will
     * kickoff.
     */
    ...
    @Override
    public List<JobInstance> getJobInstancesByJobDefinitionId(
    ...
    // wipmb For 6.8 - Clarify this interface. We currently return a JobOperationResultJson, and
    // don't throw ResourceNotFoundException
    @Override
