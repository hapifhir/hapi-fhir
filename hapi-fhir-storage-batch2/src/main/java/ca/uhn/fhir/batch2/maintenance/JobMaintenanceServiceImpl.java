...
/**
 * This class performs regular polls of the stored jobs in order to perform maintenance. This
 * includes two major functions.
 *
 * <p>First, we calculate statistics and delete expired tasks. This class does the following things:
 *
 * <ul>
 *   <li>For instances that are IN_PROGRESS, calculates throughput and percent complete
 *   <li>For instances that are IN_PROGRESS where all chunks are COMPLETE, marks instance as
 *       COMPLETE
 *   <li>For instances that are COMPLETE, purges chunk data
 *   <li>For instances that are IN_PROGRESS where at least one chunk is FAILED, marks instance as
 *       FAILED and propagates the error message to the instance, and purges chunk data
 *   <li>For instances that are IN_PROGRESS with an error message set where no chunks are ERRORED or
 *       FAILED, clears the error message in the instance (meaning presumably there was an error but
 *       it cleared)
 *   <li>For instances that are IN_PROGRESS and isCancelled flag is set marks them as ERRORED and
 *       indicating the current running step if any
 *   <li>For instances that are COMPLETE or FAILED and are old, delete them entirely
 * </ul>
 *
 * <p>Second, we check for any job instances where the job is configured to have gated execution.
 * For these instances, we check if the current step is complete (all chunks are in COMPLETE status)
 * and trigger the next step.
 *
 * <p>The maintenance pass is run once per minute. However if a gated job is fast-tracking (i.e.
 * every step produced exactly one chunk, then the maintenance task will be triggered earlier than
 * scheduled by the step executor.
 */
public class JobMaintenanceServiceImpl implements IJobMaintenanceService, IHasScheduledJobs {
    ...
    public void scheduleJobs(ISchedulerService theSchedulerService) {
        mySchedulerService.scheduleClusteredJob(
                myScheduledJobFrequencyMillis, buildJobDefinition());
    }
    ...
    private boolean runMaintenanceDirectlyWithTimeout() {
        if (getQueueLength() > 0) {
            ourLog.debug(
                    "There are already {} threads waiting to run a maintenance pass.  Ignoring"
                            + " request.",
                    getQueueLength());
            return false;
        }
        ...
        try {
            ourLog.debug(
                    "There is no clustered scheduling service.  Requesting semaphore to run"
                            + " maintenance pass directly.");
            // Some unit test, esp. the Loinc terminology tests, depend on this maintenance pass
            // being run shortly after it is requested
            ...
            throw new RuntimeException(
                    Msg.code(2134) + "Timed out waiting to run a maintenance pass", e);
        } finally {
        ...
        for (int page = 0; ; page++) {
            ...
            for (JobInstance instance : instances) {
                ...
                if (myJobDefinitionRegistry
                        .getJobDefinition(
                                instance.getJobDefinitionId(), instance.getJobDefinitionVersion())
                        .isPresent()) {
    ...
    public static class JobMaintenanceScheduledJob implements HapiJob {
        @Autowired private IJobMaintenanceService myTarget;
