...
public class StepExecutionDetails<PT extends IModelJson, IT extends IModelJson> {
	...
	/**
	* Returns the data associated with this step execution. This method should never be called
	* during the first step of a job, or in a reduction step, and will never return <code>null
	* </code> during any other steps.
	*
	...
	*/
	...
	/**
	* Returns true if there's a workchunk to store data to. If false, failures and data storage go
	* straight to the jobinstance instead
	*
	* @return - true if there's a workchunk in the db to store to. false if the output goes to the
	*     jobinstance instead
	*/
