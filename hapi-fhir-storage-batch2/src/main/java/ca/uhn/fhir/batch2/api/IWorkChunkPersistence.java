...
/**
 * Work Chunk api, implementing the WorkChunk state machine. Test specification is in {@link
 * ca.uhn.hapi.fhir.batch2.test.AbstractIJobPersistenceSpecificationTest}. Note on transaction
 * boundaries: these are messy - some methods expect an existing transaction and are marked with
 * {@code @Transactional(propagation = Propagation.MANDATORY)}, some will create a tx as needed and
 * are marked {@code @Transactional(propagation = Propagation.REQUIRED)}, and some run in a NEW
 * transaction and are not marked on the interface, but on the implementors instead. We had a bug
 * where interface methods marked {@code @Transactional(propagation = Propagation.REQUIRES_NEW)}
 * were starting two (2!) transactions because of our synchronized wrapper.
 *
...
public interface IWorkChunkPersistence {
    ...
    /**
     * Stores a chunk of work for later retrieval. The first state event, as the chunk is created.
     * This method should be atomic and should only return when the chunk has been successfully
     * stored in the database. Chunk should be stored with a status of {@link
    ...
    /**
     * On arrival at a worker. The second state event, as the worker starts processing. Transition
     * to {@link WorkChunkStatusEnum#IN_PROGRESS} if unless not in QUEUED or ERRORRED state.
     *
     * @param theChunkId The ID from {@link #onWorkChunkCreate}
     * @return The WorkChunk or empty if no chunk exists, or not in a runnable state (QUEUED or
     *     ERRORRED)
     */
    @Transactional(propagation = Propagation.REQUIRED)
    ...
    /**
     * A retryable error. Transition to {@link WorkChunkStatusEnum#ERRORED} unless max-retries
     * passed, then transition to {@link WorkChunkStatusEnum#FAILED}.
     *
    ...
    /**
     * An unrecoverable error. Transition to {@link WorkChunkStatusEnum#FAILED}
     *
    ...
    /**
     * Report success and complete the chunk. Transition to {@link WorkChunkStatusEnum#COMPLETED}
     *
     ...
     * @return - an iterator for fetching work chunks wipmb replace with a stream and a consumer in
     *     6.8
     */
