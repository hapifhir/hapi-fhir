...
public class JobInstanceProcessor {
	...
	public JobInstanceProcessor(
		...
		myJobInstanceProgressCalculator =
					new JobInstanceProgressCalculator(
								theJobPersistence, theProgressAccumulator, theJobDefinitionRegistry);
	...
	private boolean handleCancellation(JobInstance theInstance) {
		if (theInstance.isPendingCancellationRequest()) {
				...
				return myJobPersistence.updateInstance(
						theInstance.getInstanceId(),
						instance -> {
								boolean changed =
										myJobInstanceStatusUpdater.updateInstanceStatus(
													instance, StatusEnum.CANCELLED);
								if (changed) {
	...
	private void cleanupInstance(JobInstance theInstance) {
		switch (theInstance.getStatus()) {
				case QUEUED:
					...
					break;
				case FINALIZE:
					// If we're in FINALIZE, the reduction step is working, so we should stay out of the
					// way until it
				...
				case CANCELLED:
					...
					// wipmb For 6.8 - Are we deliberately not purging chunks for cancelled jobs?  This
					// is a very complicated way to say that.
					return;
		}
	...
	private void triggerGatedExecutions(JobInstance theInstance) {
		...
		JobWorkCursor<?, ?, ?> jobWorkCursor =
					JobWorkCursor.fromJobDefinitionAndRequestedStepId(
								jobDefinition, theInstance.getCurrentGatedStepId());

		// final step
		if (jobWorkCursor.isFinalStep() && !jobWorkCursor.isReductionStep()) {
				ourLog.debug(
						"Job instance {} is in final step and it's not a reducer step",
						theInstance.getInstanceId());
				return;
		}
		...
		boolean shouldAdvance =
					myJobPersistence.canAdvanceInstanceToNextStep(instanceId, currentStepId);
		if (shouldAdvance) {
				String nextStepId = jobWorkCursor.nextStep.getStepId();
				ourLog.info(
						"All processing is complete for gated execution of instance {} step {}."
									+ " Proceeding to step {}",
						instanceId,
						currentStepId,
						nextStepId);
				...
				if (jobWorkCursor.nextStep.isReductionStep()) {
					JobWorkCursor<?, ?, ?> nextJobWorkCursor =
								JobWorkCursor.fromJobDefinitionAndRequestedStepId(
	...
	private void processChunksForNextSteps(JobInstance theInstance, String nextStepId) {
		...
		List<String> queuedChunksForNextStep =
					myProgressAccumulator.getChunkIdsWithStatus(
								instanceId, nextStepId, WorkChunkStatusEnum.QUEUED);
		int totalChunksForNextStep =
					myProgressAccumulator.getTotalChunkCountForInstanceAndStep(instanceId, nextStepId);
		if (totalChunksForNextStep != queuedChunksForNextStep.size()) {
				ourLog.debug(
						"Total ProgressAccumulator QUEUED chunk count does not match QUEUED chunk size!"
									+ " [instanceId={}, stepId={}, totalChunks={}, queuedChunks={}]",
						instanceId,
						nextStepId,
						totalChunksForNextStep,
						queuedChunksForNextStep.size());
		}
		// Note on sequence: we don't have XA transactions, and are talking to two stores (JPA +
		// Queue)
		// Sequence: 1 - So we run the query to minimize the work overlapping.
		List<String> chunksToSubmit =
					myJobPersistence.fetchAllChunkIdsForStepWithStatus(
								instanceId, nextStepId, WorkChunkStatusEnum.QUEUED);
		...
		if (!changed) {
				...
				ourLog.warn(
						"Skipping gate advance to {} for instance {} - already advanced.",
						nextStepId,
						instanceId);
				return;
		}
		...
		for (String nextChunkId : chunksToSubmit) {
				JobWorkNotification workNotification =
						new JobWorkNotification(theInstance, nextStepId, nextChunkId);
				myBatchJobSender.sendWorkChannelMessage(workNotification);
		}
		ourLog.debug(
					"Submitted a batch of chunks for processing. [chunkCount={}, instanceId={},"
								+ " stepId={}]",
					chunksToSubmit.size(),
					instanceId,
					nextStepId);
