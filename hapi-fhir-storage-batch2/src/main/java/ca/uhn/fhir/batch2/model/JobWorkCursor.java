 ...
 * job definition. It holds information required to execute the current step and send data to the
 * next step.
 *
...
public class JobWorkCursor<PT extends IModelJson, IT extends IModelJson, OT extends IModelJson> {
    ...
    private void validate() {
        ...
        // Note that if it is not the first step, it can have VoidModel as it's input type--not all
        // steps require input from
        // the previous step
        if (nextStep != null) {
    ...
    public static <PT extends IModelJson>
            JobWorkCursor<PT, ?, ?> fromJobDefinitionAndRequestedStepId(
