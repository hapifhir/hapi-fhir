...
public class JobDefinitionRegistry {
	...
	private final Map<String, NavigableMap<Integer, JobDefinition<?>>> myJobDefinitions =
	...
	*/
	public synchronized <PT extends IModelJson> boolean addJobDefinitionIfNotRegistered(
		...
		Optional<JobDefinition<?>> orig =
					getJobDefinition(
								theDefinition.getJobDefinitionId(),
	...
	public synchronized <PT extends IModelJson> void addJobDefinition(
				@Nonnull JobDefinition<PT> theDefinition) {
	...
	public Optional<JobDefinition<?>> getLatestJobDefinition(@Nonnull String theJobDefinitionId) {
		NavigableMap<Integer, JobDefinition<?>> versionMap =
					myJobDefinitions.get(theJobDefinitionId);
	...
	*/
	public JobDefinition<?> getJobDefinitionOrThrowException(
				String theJobDefinitionId, int theJobDefinitionVersion) {
		Optional<JobDefinition<?>> opt =
					getJobDefinition(theJobDefinitionId, theJobDefinitionVersion);
		if (opt.isEmpty()) {
				String msg =
						"Unknown job definition ID["
									+ theJobDefinitionId
									+ "] version["
									+ theJobDefinitionVersion
									+ "]";
				ourLog.warn(msg);
	...
	*/
	...
	public <T extends IModelJson> JobDefinition<T> getJobDefinitionOrThrowException(
				JobInstance theJobInstance) {
		return (JobDefinition<T>)
					getJobDefinitionOrThrowException(
								theJobInstance.getJobDefinitionId(),
								theJobInstance.getJobDefinitionVersion());
	}
