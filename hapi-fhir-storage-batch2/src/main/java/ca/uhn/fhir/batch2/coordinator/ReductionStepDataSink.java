	...
	public void accept(WorkChunkData<OT> theData) {
		...
		JobInstanceProgressCalculator myJobInstanceProgressCalculator =
					new JobInstanceProgressCalculator(
								myJobPersistence, progressAccumulator, myJobDefinitionRegistry);

		InstanceProgress progress =
					myJobInstanceProgressCalculator.calculateInstanceProgress(instanceId);
		boolean changed =
					myJobPersistence.updateInstance(
								...
								instance -> {
									...
									if (instance.getReport() != null) {
										...
										ourLog.error(
													"Report has already been set. Now it is being overwritten."
																+ " Last in will win!");
									}
		...
		if (!changed) {
				...
				throw new JobExecutionFailedException(
						Msg.code(2097) + ("No instance found with Id " + instanceId));
		}
	}
