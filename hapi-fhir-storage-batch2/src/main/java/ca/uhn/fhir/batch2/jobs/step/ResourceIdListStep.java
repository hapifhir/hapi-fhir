...
public class ResourceIdListStep<PT extends PartitionedJobParameters, IT extends ChunkRangeJson>
	...
	public RunOutcome run(
		...
		while (true) {
				IResourcePidList nextChunk =
						myIdChunkProducer.fetchResourceIdsPage(
									nextStart,
									end,
									pageSize,
									requestPartitionId,
				...
				if (nextChunk.getLastDate().getTime() == previousLastTime) {
					ourLog.info(
								"Matching final timestamp of {}, loading is completed",
								new Date(previousLastTime));
					break;
				}
	...
	private void submitWorkChunk(
		...
		ResourceIdListWorkChunkJson data =
					new ResourceIdListWorkChunkJson(theTypedPids, theRequestPartitionId);
