...
class WorkChannelMessageHandler implements MessageHandler {
    ...
    class MessageProcess {
        ...
        Optional<MessageProcess> validateChunkId() {
            ...
            if (myChunkId == null) {
                ourLog.error(
                        "Received work notification with null chunkId: {}", myWorkNotification);
        ...
        Optional<MessageProcess> loadJobDefinitionOrThrow() {
            ...
            // Failing to load a job definition probably means this is an old process during
            // upgrade.
            // Retry those until this node is killed/restarted.
            myJobDefinition =
                    myJobDefinitionRegistry.getJobDefinitionOrThrowException(
                            jobDefinitionId, jobDefinitionVersion);
            return Optional.of(this);
        }
        ...
        Optional<MessageProcess> updateChunkStatusAndValidate() {
            return myJobPersistence
                    ...
                    .or(
                            () -> {
                                ourLog.error(
                                        "Unable to find chunk with ID {} - Aborting.  {}",
                                        myChunkId,
                                        myWorkNotification);
                                return Optional.empty();
                            })
        ...
        Optional<MessageProcess> updateAndValidateJobStatus() {
            ...
            switch (myJobInstance.getStatus()) {
                ...
                default:
                    ...
                    ourLog.info(
                            "Skipping chunk {} because job instance is {}",
                            myChunkId,
                            myJobInstance.getStatus());
                    return Optional.empty();
            }
        ...
        public Optional<MessageProcess> buildStepExecutor() {
            this.myStepExector =
                    myJobStepExecutorFactory.newJobStepExecutor(
                            this.myJobInstance, this.myWorkChunk, this.myCursor);
    ...
    private void handleWorkChannelMessage(JobWorkNotificationJsonMessage theMessage) {
        ...
        // 1. Normal execution.  We validate, load, update statuses, all in a tx.  Then we process
        // the chunk.
        // 2. Discard chunk.  If some validation fails (e.g. no chunk with that id), we log and
        // discard the chunk.
        //    Probably a db rollback, with a stale queue.
        // 3. Fail and retry.  If we throw an exception out of here, Spring will put the queue
        // message back, and redeliver later.
        //
        ...
        executeInTxRollbackWhenEmpty(
                        () ->
                                (
                                // Use a chain of Optional flatMap to handle all the setup
                                // short-circuit exits cleanly.
                                Optional.of(new MessageProcess(workNotification))
                                        // validate and load info
                                        .flatMap(MessageProcess::validateChunkId)
                                        // no job definition should be retried - we must be a stale
                                        // process encountering a new job definition.
                ...
                .execute(
                        theTransactionStatus -> {
                            ...
                            if (setupProcessing.isEmpty()) {
                                // If any setup failed, roll back the chunk and instance status
                                // changes.
