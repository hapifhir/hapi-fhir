...
public class LoadIdsStepTest {
    ...
    @Mock private IBatch2DaoSvc myBatch2DaoSvc;

    @Mock private IJobDataSink<ResourceIdListWorkChunkJson> mySink;
    ...
    @Captor private ArgumentCaptor<ResourceIdListWorkChunkJson> myChunkIdsCaptor;
    ...
    public void testGenerateSteps() {
        ...
        StepExecutionDetails<PartitionedUrlListJobParameters, PartitionedUrlChunkRangeJson>
                details = new StepExecutionDetails<>(parameters, range, jobInstance, chunkId);

        // First Execution

        when(myBatch2DaoSvc.fetchResourceIdsPage(
                        eq(DATE_1), eq(DATE_END), eq(DEFAULT_PAGE_SIZE), isNull(), isNull()))
                .thenReturn(createIdChunk(0L, 20000L, DATE_2));
        when(myBatch2DaoSvc.fetchResourceIdsPage(
                        eq(DATE_2), eq(DATE_END), eq(DEFAULT_PAGE_SIZE), isNull(), isNull()))
                .thenReturn(createIdChunk(20000L, 40000L, DATE_3));
        when(myBatch2DaoSvc.fetchResourceIdsPage(
                        eq(DATE_3), eq(DATE_END), eq(DEFAULT_PAGE_SIZE), isNull(), isNull()))
                .thenReturn(createIdChunk(40000L, 40040L, DATE_4));
        when(myBatch2DaoSvc.fetchResourceIdsPage(
                        eq(DATE_4), eq(DATE_END), eq(DEFAULT_PAGE_SIZE), isNull(), isNull()))
