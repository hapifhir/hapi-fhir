 ...
 *
 * <p><b>Definition:</b> A measured amount (or an amount that can potentially be measured). Note
 * that measured amounts include amounts that are not precisely quantified, including amounts
 * involving arbitrary units and floating currencies
 *
 * <p><b>Requirements:</b> Need to able to capture all sorts of measured values, even if the
 * measured value are not precisely quantified. Values include exact measures such as 3.51g,
 * customary units such as 3 tablets, and currencies such as $100.32USD
 */
...
public class QuantityDt extends BaseQuantityDt implements ICompositeDatatype {
    ...
    /**
     * @deprecated Use {@link #setUnit(String)} instead - Quantity.units was renamed to
     *     Quantity.unit in DSTU2
     */
    @Deprecated
    @Override
    ...
    /**
     * @deprecated Use {@link #getUnitElement()} - Quantity.units was renamed to Quantity.unit in
     *     DSTU2
     */
    @Deprecated
    @Override
    ...
    @Description(
            ...
            formalDefinition =
                    "How the value should be understood and represented - whether the actual value"
                            + " is greater or less than the stated value due to measurement issues;"
                            + " e.g. if the comparator is \"<\" , then the real value is < stated"
                            + " value")
    ...
    public boolean isEmpty() {
        return super.isBaseEmpty()
                && ca.uhn.fhir.util.ElementUtil.isEmpty(
                        myValue, myComparator, myUnit, mySystem, myCode);
    }

    @Override
    public <T extends IElement> List<T> getAllPopulatedChildElementsOfType(Class<T> theType) {
        return ca.uhn.fhir.util.ElementUtil.allPopulatedChildElements(
                theType, myValue, myComparator, myUnit, mySystem, myCode);
    }

    /**
     * Gets the value(s) for <b>value</b> (). creating it if it does not exist. Will not return
     * <code>null</code>.
     *
    ...
    /**
     * Gets the value(s) for <b>comparator</b> (). creating it if it does not exist. Will not return
     * <code>null</code>.
     *
    ...
    public BoundCodeDt<QuantityComparatorEnum> getComparatorElement() {
        if (myComparator == null) {
            myComparator =
                    new BoundCodeDt<QuantityComparatorEnum>(QuantityComparatorEnum.VALUESET_BINDER);
        }
        return myComparator;
    }
    ...
    public QuantityDt setComparator(QuantityComparatorEnum theValue) {
        setComparator(
                new BoundCodeDt<QuantityComparatorEnum>(
                        QuantityComparatorEnum.VALUESET_BINDER, theValue));
    ...
    /**
     * Gets the value(s) for <b>unit</b> (). creating it if it does not exist. Will not return
     * <code>null</code>.
     *
     * <p><b>Definition:</b> A human-readable form of the unit
     */
    ...
    /**
     * Gets the value(s) for <b>system</b> (). creating it if it does not exist. Will not return
     * <code>null</code>.
     *
    ...
    /**
     * Gets the value(s) for <b>code</b> (). creating it if it does not exist. Will not return
     * <code>null</code>.
     *
     * <p><b>Definition:</b> A computer processable form of the unit in some unit representation
     * system
     */
    @Override
