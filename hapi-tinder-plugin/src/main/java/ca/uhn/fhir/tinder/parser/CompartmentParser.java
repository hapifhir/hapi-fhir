...
public class CompartmentParser {
	...
	private static final org.slf4j.Logger ourLog =
				org.slf4j.LoggerFactory.getLogger(CompartmentParser.class);
	...
	public void parse() throws Exception {
		...
		if (nextRes == null) {
				throw new MojoFailureException(
						Msg.code(178) + "Unknown base resource name: " + resName);
		}
		...
		for (int i = 0;
					i < file.getElementsByTagName("Worksheet").getLength() && resourcesSheet == null;
					i++) {
				resourcesSheet = (Element) file.getElementsByTagName("Worksheet").item(i);
				if (!"resources"
						.equals(
									resourcesSheet.getAttributeNS(
		...
		if (resourcesSheet == null) {
				throw new Exception(
						Msg.code(180)
									+ "Failed to find worksheet with name 'Data Elements' in spreadsheet: "
									+ resName);
		}
		...
		for (int i = 1; i < cells.getLength(); i++) {
				...
				for (String nextName : namesSplit) {
					...
					if (sp == null) {
						throw new MojoFailureException(
									Msg.code(181)
												+ "Can't find child named "
												+ parts[0]
												+ " - Valid names: "
												+ element.getSearchParameterNames());
					}
