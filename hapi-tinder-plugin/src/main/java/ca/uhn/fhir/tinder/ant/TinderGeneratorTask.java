 ...
 *
 * <p>Generates either source or resource files for each selected resource or composite data type.
 * One file is generated for each selected entity. The files are generated using a Velocity template
 * that can be taken from inside the hapi-timder-plugin project or can be located in other projects
 *
 * <p>The following Ant task properties are used
 *
 * <p>
 *
	...
	public void execute() throws BuildException {
		...
		try {
				...
				if (targetFile != null) {
					...
					OutputStreamWriter targetWriter =
					...
					*/
					VelocityEngine v =
								VelocityHelper.configureVelocityEngine(
										templateFile, velocityPath, velocityProperties);
					...
					*/
					...
					ctx.put(
								"isRi",
								BaseStructureSpreadsheetParser.determineVersionEnum(version).isRi());
					ctx.put("hash", "#");
				...
				e.printStackTrace();
				throw new BuildException(
						Msg.code(142) + "Error processing " + getTaskName() + " task.", e);
		} finally {
				cleanup();
		}
	}
		...
		if (null == version) {
				throw new BuildException(
						Msg.code(143)
									+ "The "
									+ this.getTaskName()
									+ " task requires \"version\" attribute.");
		}
		if (null == template) {
		...
		if (null == projectHome) {
				throw new BuildException(
						Msg.code(148)
									+ "The "
									+ this.getTaskName()
									+ " task requires \"projectHome\" attribute.");
		}
	}
