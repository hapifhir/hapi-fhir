...
public abstract class BaseStructureSpreadsheetParser extends BaseStructureParser {
	...
	public void parse() throws Exception {
		...
		if (getVersion().equals("dstu2")) {
				...
				String path =
						ctx.getVersion().getPathToSchemaDefinitions().replace("/schema", "/valueset")
									+ "/valuesets.xml";

				InputStream valuesetText =
						BaseStructureSpreadsheetParser.class.getResourceAsStream(path);
				Bundle bundle =
						ctx.newXmlParser()
									.parseResource(Bundle.class, new InputStreamReader(valuesetText));
		...
		for (InputStream nextInputStream : getInputStreams()) {
				...
				Element dataElementsSheet =
						findSheetByName(spreadsheetName, "Data Elements", file, true);
					...
					while (element.getElementParentName() != null) {
						...
						for (BaseElement child : children) {
								if (!child.equals(blockCopy)
									...
									((ResourceBlockCopy) blockCopy)
												.setReferencedBlock((ResourceBlock) child);
									break refLoop;
								}
						}
						element = parent;
	...
	private Element findSheetByName(
				String spreadsheetName, String wantedName, Document file, boolean theFailIfNotFound)
				throws Exception {
		Element retVal = null;
		for (int i = 0;
					i < file.getElementsByTagName("Worksheet").getLength() && retVal == null;
					i++) {
				retVal = (Element) file.getElementsByTagName("Worksheet").item(i);
				if (!wantedName.equals(
						retVal.getAttributeNS(
		...
		if (retVal == null && theFailIfNotFound) {
				throw new Exception(
						Msg.code(166)
									+ "Failed to find worksheet with name '"
									+ wantedName
									+ "' in spreadsheet: "
									+ spreadsheetName);
		}
	...
	private void parseParameters(Document theFile, BaseRootType theResource)
		...
		for (int i = 0; i < sheets.getLength(); i++) {
				...
				String name =
						sheet.getAttributeNS("urn:schemas-microsoft-com:office:spreadsheet", "Name");
				if ("Search".equals(name)) {
					...
					for (SearchParameter nextCompositeParam : compositeParams) {
						...
						if (nextCompositeParam.getPath().indexOf('&') == -1) {
								throw new MojoExecutionException(
										Msg.code(168)
													...
													+ nextCompositeParam.getName()
													+ " has path with no '&': "
													+ nextCompositeParam.getPath());
						}

						String[] parts = nextCompositeParam.getPath().split("\\&");
						List<List<SearchParameter>> compositeOf =
									new ArrayList<List<SearchParameter>>();
						...
						for (String nextPart : parts) {
								...
								possibleMatches.add(
										theResource.getName() + "." + nextPart.replace("[x]", "-[x]"));
								possibleMatches.add(nextPart.replace("-[x]", "[x]"));
								possibleMatches.add(
										theResource.getName() + "." + nextPart.replace("-[x]", "[x]"));
								...
								if (part.isEmpty()) {
									throw new MojoExecutionException(
												Msg.code(169)
														+ "Composite param "
														+ nextCompositeParam.getName()
														+ " has path that doesn't seem to correspond to any"
														+ " other params: "
														+ nextPart);
								}
						}
						...
						for (SearchParameter part1 : compositeOf.get(0)) {
								for (SearchParameter part2 : compositeOf.get(1)) {
									SearchParameter composite =
												new SearchParameter(getVersion(), theResource.getName());
									...
									composite.setCompositeOf(
												Arrays.asList(part1.getName(), part2.getName()));
									composite.setCompositeTypes(
												Arrays.asList(
														WordUtils.capitalize(part1.getType()),
														WordUtils.capitalize(part2.getType())));
								}
						}
					}
				}
	...
	private void parseFirstRow(Element theDefRow) {
		...
		if (myColModifier == -1) {
				throw new IllegalArgumentException(
						Msg.code(171) + "Unable to determine column: modifier");
		}
		if (myColCard == -1) {
		...
		if (myColBinding == -1) {
				throw new IllegalArgumentException(
						Msg.code(174) + "Unable to determine column: binding");
		}
		if (myColDefinition == -1) {
				throw new IllegalArgumentException(
						Msg.code(175) + "Unable to determine column: definition");
		}
		if (myColRequirements == -1) {
				throw new IllegalArgumentException(
						Msg.code(176) + "Unable to determine column: requirements");
		}
		if (myColV2Mapping == -1) {
				throw new IllegalArgumentException(
						Msg.code(177) + "Unable to determine column: v2 mapping");
		}
	}

	private void parseBasicElements(
				Element theRowXml, BaseElement theTarget, String theTypeOrNull) {
