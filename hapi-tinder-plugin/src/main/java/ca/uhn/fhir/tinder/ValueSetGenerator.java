...
public class ValueSetGenerator {
    ...
    private static final org.slf4j.Logger ourLog =
            org.slf4j.LoggerFactory.getLogger(ValueSetGenerator.class);
                ...
                if ("dstu".equals(myVersion)) {
                    ...
                    ca.uhn.fhir.model.dstu2.resource.ValueSet nextVs =
                            (ca.uhn.fhir.model.dstu2.resource.ValueSet)
                                    newXmlParser.parseResource(
                                            ca.uhn.fhir.model.dstu2.resource.ValueSet.class, vs);
                    tm = parseValueSet(nextVs);
                }
                if (tm != null) {
    ...
    private ValueSetTm parseValueSet(ca.uhn.fhir.model.dstu2.resource.ValueSet nextVs) {
        ...
        ourLog.debug(
                "Parsing ValueSetTm #{} - {} - {} concepts total",
                myValueSetCount++,
                nextVs.getName(),
                myConceptCount);
        ...
        //			ourLog.info("ValueSet " + nextVs.getName() + " has no codes, not going to generate any
        // code for it");
        //			return null;
        //		}
        ...
        if (vs.getName().endsWith(" Codes")) {
            myValueSets.put(
                    vs.getName().substring(0, vs.getName().length() - 6).replace(" ", ""), vs);
        }
    ...
    public void write(
            Collection<ValueSetTm> theValueSets, File theOutputDirectory, String thePackageBase)
            throws IOException {
    ...
    public void write(
            ...
            Collection<ValueSetTm> theValueSets,
    ...
    private void write(
        ...
        VelocityEngine v =
                VelocityHelper.configureVelocityEngine(
                        myTemplateFile, myVelocityPath, myVelocityProperties);
    ...
    public void writeMarkedValueSets(File theOutputDirectory, String thePackageBase)
            throws MojoFailureException {
        writeMarkedValueSets(TargetType.SOURCE, theOutputDirectory, thePackageBase);
    }

    public void writeMarkedValueSets(
            TargetType theTarget, File theOutputDirectory, String thePackageBase)
            throws MojoFailureException {
        try {
