...
public abstract class BaseBlockingQueueSubscribableChannelDstu3Test
		extends BaseSubscriptionDstu3Test {
	public static final ChannelConsumerSettings CONSUMER_OPTIONS =
				new ChannelConsumerSettings().setConcurrentConsumers(1);
	protected static final List<Observation> ourCreatedObservations =
				Collections.synchronizedList(Lists.newArrayList());
	protected static final List<Observation> ourUpdatedObservations =
				Collections.synchronizedList(Lists.newArrayList());
	protected static final List<String> ourContentTypes =
				Collections.synchronizedList(new ArrayList<>());
	private static final Logger ourLog =
				LoggerFactory.getLogger(SubscriptionMatchingSubscriberTest.class);

	// Caused by: java.lang.IllegalStateException: Unable to register mock bean
	// org.springframework.messaging.MessageHandler expected a single matching bean to replace but
	// found [subscriptionActivatingSubscriber, subscriptionDeliveringEmailSubscriber,
	// subscriptionDeliveringRestHookSubscriber, subscriptionMatchingSubscriber,
	...
	protected final PointcutLatch mySubscriptionMatchingPost =
				new PointcutLatch(Pointcut.SUBSCRIPTION_AFTER_PERSISTED_RESOURCE_CHECKED);
	protected final PointcutLatch mySubscriptionActivatedPost =
				new PointcutLatch(Pointcut.SUBSCRIPTION_AFTER_ACTIVE_SUBSCRIPTION_REGISTERED);
	protected final PointcutLatch mySubscriptionAfterDelivery =
				new PointcutLatch(Pointcut.SUBSCRIPTION_AFTER_DELIVERY);
	protected final PointcutLatch mySubscriptionResourceMatched =
				new PointcutLatch(Pointcut.SUBSCRIPTION_RESOURCE_MATCHED);
	protected final PointcutLatch mySubscriptionResourceNotMatched =
				new PointcutLatch(Pointcut.SUBSCRIPTION_RESOURCE_DID_NOT_MATCH_ANY_SUBSCRIPTIONS);
	...
	@Autowired protected StorageSettings myStorageSettings;
	...
	@Autowired SubscriptionChannelFactory mySubscriptionChannelFactory;
	@Autowired IInterceptorService myInterceptorRegistry;
	...
	public void beforeReset() {
		...
		ourSubscribableChannel =
					mySubscriptionChannelFactory.newDeliveryReceivingChannel(
								mySubscriptionDeliveryChannelNamer.nameFromSubscription(
		...
		myInterceptorRegistry.registerAnonymousInterceptor(
					Pointcut.SUBSCRIPTION_AFTER_DELIVERY, mySubscriptionAfterDelivery);
		myInterceptorRegistry.registerAnonymousInterceptor(
					Pointcut.SUBSCRIPTION_RESOURCE_MATCHED, mySubscriptionResourceMatched);
		myInterceptorRegistry.registerAnonymousInterceptor(
	...
	public <T extends IBaseResource> T sendResource(
				T theResource, RequestPartitionId theRequestPartitionId) throws InterruptedException {
	...
	protected Subscription sendSubscription(
				Subscription theSubscription, RequestPartitionId theRequestPartitionId, Boolean mockDao)
	...
	protected Observation sendObservation(
				String code, String system, RequestPartitionId theRequestPartitionId)
				throws InterruptedException {
	...
	public static class ObservationListener implements IResourceProvider, IPointcutLatch {
		...
		public MethodOutcome create(
				...
				ourContentTypes.add(
						theRequest
									.getHeader(ca.uhn.fhir.rest.api.Constants.HEADER_CONTENT_TYPE)
									.replaceAll(";.*", ""));
	...
	public static void startListenerServer() throws Exception {
		...
		// Preload structure definitions so the load doesn't happen during the test (first load can
		// be a little slow)
