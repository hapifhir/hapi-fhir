...
public class SubscriptionMatchingSubscriberTest
		extends BaseBlockingQueueSubscribableChannelDstu3Test {
	private final IFhirResourceDao<Subscription> myMockSubscriptionDao =
				Mockito.mock(IFhirResourceDao.class);
	...
	public void afterEach() {
		myStorageSettings.setCrossPartitionSubscriptionEnabled(
					new JpaStorageSettings().isCrossPartitionSubscriptionEnabled());
	}
	...
	public void testRestHookSubscription_NoResourceTypeInPayloadId() throws Exception {
		...
		Subscription subscription =
					makeActiveSubscription(
								"Observation?", "application/fhir+xml", ourListenerServerBase);
	...
	public void testCriteriaStarOnly() throws InterruptedException {
		...
		Subscription subscription3 =
					makeActiveSubscription(criteria3, payload, ourListenerServerBase);
	...
	public void testSubscriptionAndResourceOnTheSamePartitionMatch() throws InterruptedException {
		...
		Subscription subscription =
					makeActiveSubscription(criteria, payload, ourListenerServerBase);
	...
	@Test
	public void testSubscriptionAndResourceOnTheSamePartitionMatchPart2()
	...
	@Test
	public void testSubscriptionAndResourceOnDiffPartitionNotMatchPart2()
				throws InterruptedException {
		myPartitionSettings.setPartitioningEnabled(true);
		String payload = "application/fhir+json";
	...
	@Test
	public void testSubscriptionOnDefaultPartitionAndResourceOnDiffPartitionNotMatch()
				throws InterruptedException {
		myPartitionSettings.setPartitioningEnabled(true);
		String payload = "application/fhir+json";
	...
	@Test
	public void testSubscriptionOnAPartitionAndResourceOnDefaultPartitionNotMatch()
				throws InterruptedException {
		myPartitionSettings.setPartitioningEnabled(true);
		String payload = "application/fhir+json";
	...
	@Test
	public void testSubscriptionOnOnePartitionMatchResourceOnMultiplePartitions()
				throws InterruptedException {
		myPartitionSettings.setPartitioningEnabled(true);
		String payload = "application/fhir+json";
	...
	@Test
	public void testSubscriptionOnOnePartitionDoNotMatchResourceOnMultiplePartitions()
				throws InterruptedException {
		myPartitionSettings.setPartitioningEnabled(true);
		String payload = "application/fhir+json";
	...
	@Test
	public void testCrossPartitionSubscriptionForResourceOnTheSamePartitionMatch()
				throws InterruptedException {
		myPartitionSettings.setPartitioningEnabled(true);
	...
	@Test
	public void testCrossPartitionSubscriptionForResourceOnDifferentPartitionMatch()
				throws InterruptedException {
		myPartitionSettings.setPartitioningEnabled(true);
		myStorageSettings.setCrossPartitionSubscriptionEnabled(true);
		String payload = "application/fhir+json";
	...
	@Test
	public void testCrossPartitionSubscriptionForMultipleResourceOnDifferentPartitionMatch()
				throws InterruptedException {
		myPartitionSettings.setPartitioningEnabled(true);
		myStorageSettings.setCrossPartitionSubscriptionEnabled(true);
		String payload = "application/fhir+json";
	...
	private void mockSubscriptionRead(
				RequestPartitionId theRequestPartitionId, Subscription subscription) {
		Subscription modifiedSubscription = subscription.copy();
		// the original partition info was the request info, but we need the actual storage
		// partition.
		modifiedSubscription.setUserData(Constants.RESOURCE_PARTITION_ID, theRequestPartitionId);
		when(myMockSubscriptionDao.read(eq(subscription.getIdElement()), any(), eq(true)))
					.thenReturn(modifiedSubscription);
	}
	...
	public class TestDeleteMessages {
		...
		@Mock CanonicalSubscription myNonDeleteCanonicalSubscription;
		@Mock SubscriptionCriteriaParser.SubscriptionCriteria mySubscriptionCriteria;
		@Mock SubscriptionMatchDeliverer mySubscriptionMatchDeliverer;
		@InjectMocks SubscriptionMatchingSubscriber subscriber;

		@Test
		public void testAreNotIgnored() {

				when(message.getOperationType())
						.thenReturn(BaseResourceModifiedMessage.OperationTypeEnum.DELETE);
				when(myInterceptorBroadcaster.callHooks(
									eq(Pointcut.SUBSCRIPTION_BEFORE_PERSISTED_RESOURCE_CHECKED),
									any(HookParams.class)))
						.thenReturn(true);
				when(mySubscriptionRegistry.getAll()).thenReturn(Collections.emptyList());

				subscriber.matchActiveSubscriptionsAndDeliver(message);

				verify(myInterceptorBroadcaster)
						.callHooks(
									eq(Pointcut.SUBSCRIPTION_BEFORE_PERSISTED_RESOURCE_CHECKED),
									any(HookParams.class));
				verify(myInterceptorBroadcaster)
						.callHooks(
									eq(Pointcut.SUBSCRIPTION_AFTER_PERSISTED_RESOURCE_CHECKED),
									any(HookParams.class));
		}
		...
		public void matchActiveSubscriptionsChecksSendDeleteMessagesExtensionFlag() {
				...
				when(mySubscriptionRegistry.getAllNonTopicSubscriptions())
						.thenReturn(Collections.singletonList(myActiveSubscription));
		...
		public void testMultipleSubscriptionsDoNotEarlyReturn() {
				...
				when(mySubscriptionRegistry.getAllNonTopicSubscriptions())
						.thenReturn(List.of(myNonDeleteSubscription, myActiveSubscription));
				when(myActiveSubscription.getSubscription()).thenReturn(myCanonicalSubscription);
				when(myActiveSubscription.getCriteria()).thenReturn(mySubscriptionCriteria);
				when(myActiveSubscription.getId()).thenReturn("Patient/123");
				when(myNonDeleteSubscription.getSubscription())
						.thenReturn(myNonDeleteCanonicalSubscription);
