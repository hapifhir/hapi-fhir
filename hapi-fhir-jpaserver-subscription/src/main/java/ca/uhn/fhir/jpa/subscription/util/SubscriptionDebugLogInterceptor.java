 ...
 *
 * <p>This interceptor loges each step in the processing pipeline with a different event code, using
 * the event codes itemized in {@link EventCodeEnum}. By default these are each placed in a logger
 * with a different name (e.g. <code>
 * ca.uhn.fhir.jpa.subscription.util.SubscriptionDebugLogInterceptor.SUBS20</code> in order to
 * facilitate fine-grained logging controls where some codes are omitted and some are not.
 *
 * <p>A custom log factory can also be passed in, in which case the logging creation may use another
 * strategy.
 *
 * @see EventCodeEnum
 * @since 3.7.0
