...
public class SubscriptionSubmitInterceptorLoader {
	private static final Logger ourLog =
				LoggerFactory.getLogger(SubscriptionSubmitInterceptorLoader.class);

	@Autowired private SubscriptionMatcherInterceptor mySubscriptionMatcherInterceptor;
	@Autowired private SubscriptionValidatingInterceptor mySubscriptionValidatingInterceptor;
	...
	public void start() {
		...
		if (supportedSubscriptionTypes.isEmpty()) {
				ourLog.info(
						"Subscriptions are disabled on this server.  Subscriptions will not be"
									+ " activated and incoming resources will not be matched against"
									+ " subscriptions.");
		} else {
		...
		if (mySubscriptionTopicValidatingInterceptor != null
					&& !mySubscriptionTopicValidatingInterceptorRegistered) {
