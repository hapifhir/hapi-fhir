...
public class SubscriptionActivatingSubscriber implements MessageHandler {
    ...
    /**
     * Note: This is synchronized because this is called both by matching channel messages as well
     * as from Subscription Loader (which periodically refreshes from the DB to make sure nothing
     * got missed). If these two mechanisms try to activate the same subscription at the same time
     * they can get a constraint error.
     */
    public synchronized boolean activateSubscriptionIfRequired(
            final IBaseResource theSubscription) {
    ...
    private boolean activateSubscription(final IBaseResource theSubscription) {
        ...
        try {
            ...
            SubscriptionUtil.setStatus(
                    myFhirContext, subscription, SubscriptionConstants.ACTIVE_STATUS);
            ...
            SubscriptionUtil.setStatus(
                    myFhirContext, subscription, SubscriptionConstants.ERROR_STATUS);
