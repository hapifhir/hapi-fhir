...
public class SubscriptionMatcherInterceptor implements IResourceModifiedConsumer {
	private static final Logger ourLog =
				LoggerFactory.getLogger(SubscriptionMatcherInterceptor.class);
	...
	public void startIfNeeded() {
		...
		if (myMatchingChannel == null) {
				myMatchingChannel =
						mySubscriptionChannelFactory.newMatchingSendingChannel(
									SubscriptionMatchingSubscriber.SUBSCRIPTION_MATCHING_CHANNEL_NAME,
	...
	public void resourceCreated(IBaseResource theResource, RequestDetails theRequest) {
		...
		submitResourceModified(
					theResource, ResourceModifiedMessage.OperationTypeEnum.CREATE, theRequest);
	}
	...
	public void resourceDeleted(IBaseResource theResource, RequestDetails theRequest) {
		startIfNeeded();
		submitResourceModified(
					theResource, ResourceModifiedMessage.OperationTypeEnum.DELETE, theRequest);
	}
	...
	public void resourceUpdated(
		...
		if (!myStorageSettings.isTriggerSubscriptionsForNonVersioningChanges()) {
				if (theOldResource != null && theNewResource != null) {
					...
					if (isNotBlank(oldVersion)
								&& isNotBlank(newVersion)
		...
		submitResourceModified(
					theNewResource, ResourceModifiedMessage.OperationTypeEnum.UPDATE, theRequest);
	}
	...
	public void submitResourceModified(
		...
		// Even though the resource is being written, the subscription will be interacting with it
		// by effectively "reading" it so we set the RequestPartitionId as a read request
		RequestPartitionId requestPartitionId =
					myRequestPartitionHelperSvc.determineReadPartitionForRequestForRead(
								theRequest,
								theNewResource.getIdElement().getResourceType(),
								theNewResource.getIdElement());
		ResourceModifiedMessage msg =
		...
		boolean outcome =
					CompositeInterceptorBroadcaster.doCallHooks(
								...
								theRequest,
								Pointcut.SUBSCRIPTION_RESOURCE_MODIFIED,
								params);
		if (!outcome) {
				return;
		}
	...
	protected void sendToProcessingChannel(final ResourceModifiedMessage theMessage) {
		...
		Validate.notNull(
					myMatchingChannel,
					"A SubscriptionMatcherInterceptor has been registered without calling start() on"
								+ " it.");
