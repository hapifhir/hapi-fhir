 ...
 * supported and in R4 via the subscription backport spec <a
 * href="http://build.fhir.org/ig/HL7/fhir-subscription-backport-ig/components.html">Subscription
 * Backport</a>. In all versions, it is possible for a FHIR Repository to submit topic subscription
 * notifications triggered by some arbitrary "business event". In R5 and R4B most subscription topic
 * notifications will be triggered by a SubscriptionTopic match. However, in the R4 backport, the
 * SubscriptionTopic is not supported and the SubscriptionTopicDispatcher service is provided to
 * generate those notifications instead. Any custom java extension to the FHIR repository
 * can @Autowire this service to send topic notifications to all Subscription resources subscribed
 * to that topic.
 */
public class SubscriptionTopicDispatcher {
     ...
     * @param theResources The list of resources to deliver. The first resource will be the primary
     *     "focus" resource per the Subscription documentation. This list should _not_ include the
     *     SubscriptionStatus. The SubscriptionStatus will be added as the first element to the
     *     delivered bundle. The reason for this is that the SubscriptionStatus needs to reference
     *     the subscription ID, which is not known until the bundle is delivered.
     * @param theRequestType The type of request that led to this dispatch. This determines the
     *     request type of the bundle entries
    ...
    public int dispatch(
            ...
            RestOperationTypeEnum theRequestType) {
        SubscriptionTopicDispatchRequest subscriptionTopicDispatchRequest =
     ...
     * Deliver a Subscription topic notification to all subscriptions for the given topic.
     *
     * @param theSubscriptionTopicDispatchRequest contains the topic URL, the list of resources to
     *     deliver, and the request type
     * @return The number of subscription notifications that were successfully queued for delivery
     */
    public int dispatch(SubscriptionTopicDispatchRequest theSubscriptionTopicDispatchRequest) {
        ...
        List<ActiveSubscription> topicSubscriptions =
                mySubscriptionRegistry.getTopicSubscriptionsByTopic(
                        theSubscriptionTopicDispatchRequest.getTopicUrl());
        if (!topicSubscriptions.isEmpty()) {
            for (ActiveSubscription activeSubscription : topicSubscriptions) {
                boolean success =
                        matchFiltersAndDeliver(
                                theSubscriptionTopicDispatchRequest, activeSubscription);
                if (success) {
                    count++;
                }
            }
    ...
    private boolean matchFiltersAndDeliver(
        ...
        if (resources.size() > 0) {
            ...
            if (topicSubscription.hasFilters()) {
                ...
                if (!SubscriptionTopicFilterUtil.matchFilters(
                        firstResource,
                        resourceType,
                        subscriptionTopicFilterMatcher,
                        topicSubscription)) {
                    return false;
                }
            }
        ...
        IBaseBundle bundlePayload =
                mySubscriptionTopicPayloadBuilder.buildPayload(
                        resources,
                        theActiveSubscription,
                        topicUrl,
                        theSubscriptionTopicDispatchRequest.getRequestType());
        ...
        return mySubscriptionMatchDeliverer.deliverPayload(
                ...
                theSubscriptionTopicDispatchRequest.getInMemoryMatchResult());
    }
}
