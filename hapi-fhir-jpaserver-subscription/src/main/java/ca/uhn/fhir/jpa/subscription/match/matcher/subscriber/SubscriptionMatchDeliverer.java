...
public class SubscriptionMatchDeliverer {
	...
	public boolean deliverPayload(
		...
		if (thePayload != null) {
				subscriptionDeliveryRequest =
						new SubscriptionDeliveryRequest(thePayload, theMsg, theActiveSubscription);
		} else {
				subscriptionDeliveryRequest =
						new SubscriptionDeliveryRequest(
									theMsg.getPayloadId(myFhirContext), theMsg, theActiveSubscription);
		}
		ResourceDeliveryMessage deliveryMsg =
					buildResourceDeliveryMessage(subscriptionDeliveryRequest);
	...
	public boolean deliverPayload(
		...
		return sendToDeliveryChannel(
					subscriptionDeliveryRequest.getActiveSubscription(),
	...
	private boolean sendToDeliveryChannel(
		...
		MessageChannel deliveryChannel =
					mySubscriptionChannelRegistry.getDeliverySenderChannel(
		...
		if (deliveryChannel != null) {
				...
				ourLog.warn(
						"Do not have delivery channel for subscription {}",
						theActiveSubscription.getId());
		}
		return retVal;
	}

	private ResourceDeliveryMessage buildResourceDeliveryMessage(
				@Nonnull SubscriptionDeliveryRequest theRequest) {
	...
	private boolean callHooks(
		...
		if (!myInterceptorBroadcaster.callHooks(Pointcut.SUBSCRIPTION_RESOURCE_MATCHED, params)) {
				ourLog.info(
						"Interceptor has decided to abort processing of subscription {}",
						theActiveSubscription.getId());
				return false;
		}
		return true;
	...
	private void trySendToDeliveryChannel(
		...
		try {
				...
				throw new RuntimeException(
						Msg.code(7) + "Failed to send message to Delivery Channel", e);
		}
	}
}
