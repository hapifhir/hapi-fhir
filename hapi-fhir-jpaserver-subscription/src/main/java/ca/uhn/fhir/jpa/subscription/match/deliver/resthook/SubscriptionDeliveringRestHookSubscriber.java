...
public class SubscriptionDeliveringRestHookSubscriber extends BaseSubscriptionDeliverySubscriber {
    private static final Logger ourLog =
            LoggerFactory.getLogger(SubscriptionDeliveringRestHookSubscriber.class);
    ...
    protected void doDelivery(
        ...
        if (theSubscription.isTopicSubscription()) {
            ...
            operation =
                    createDeliveryRequestTransaction(
                            theSubscription, theClient, thePayloadResource);
    ...
    private IClientExecutable<?, ?> createDeliveryRequestTransaction(
            ...
            IBaseResource thePayloadResource) {
        IBaseBundle bundle =
                createDeliveryBundleForPayloadSearchCriteria(theSubscription, thePayloadResource);
        return theClient.transaction().withBundle(bundle);
    }

    private IClientExecutable<?, ?> createDeliveryRequestTopic(
            IBaseBundle theBundle, IGenericClient theClient) {
        return theClient.transaction().withBundle(theBundle);
    }

    public IBaseResource getResource(
            IIdType thePayloadId, RequestPartitionId thePartitionId, boolean theDeletedOK)
            throws ResourceGoneException {
        RuntimeResourceDefinition resourceDef =
                myFhirContext.getResourceDefinition(thePayloadId.getResourceType());
    ...
    /**
     * Perform operations on the payload based on various subscription extension settings such as
     * deliver latest version, delete and/or strip version id.
     *
     * @param theMsg
        ...
        BundleUtil.processEntries(
                ...
                entry -> {
                    ...
                    if (entryResource != null) {
                        // SubscriptionStatus is a "virtual" resource type that is not stored in the
                        // repository
                        if (!"SubscriptionStatus"
                                .equals(myFhirContext.getResourceType(entryResource))) {
                            IBaseResource updatedResource =
                                    getAndMassageResource(theMsg, entryResource, theSubscription);
    ...
    private IBaseResource getAndMassageResource(
        ...
        if (thePayloadResource == null
                || theSubscription.getRestHookDetails().isDeliverLatestVersion()) {
            ...
            try {
                if (payloadId != null) {
                    boolean deletedOK =
                            theMsg.getOperationType()
                                    == BaseResourceModifiedMessage.OperationTypeEnum.DELETE;
                    thePayloadResource =
                            getResource(payloadId, theMsg.getRequestPartitionId(), deletedOK);
                } else {
                    return null;
                }
    ...
    public void handleMessage(ResourceDeliveryMessage theMessage) throws MessagingException {
        ...
        if (!getInterceptorBroadcaster()
                .callHooks(Pointcut.SUBSCRIPTION_BEFORE_REST_HOOK_DELIVERY, params)) {
            return;
        }
        ...
        params =
                new HookParams()
                        ...
                        .add(ResourceDeliveryMessage.class, theMessage);
        if (!getInterceptorBroadcaster()
                .callHooks(Pointcut.SUBSCRIPTION_AFTER_REST_HOOK_DELIVERY, params)) {
    ...
    protected void sendNotification(ResourceDeliveryMessage theMsg) {
        ...
        IHttpClient client =
                myFhirContext
                        .getRestfulClientFactory()
                        .getHttpClient(url, params, "", RequestTypeEnum.POST, headers);
        IHttpRequest request = client.createParamRequest(myFhirContext, params, null);
        try {
            IHttpResponse response = request.execute();
            // close connection in order to return a possible cached connection to the connection
            // pool
            response.close();
        } catch (IOException e) {
            ourLog.error(
                    "Error trying to reach {}: {}",
                    theMsg.getSubscription().getEndpointUrl(),
                    e.toString());
