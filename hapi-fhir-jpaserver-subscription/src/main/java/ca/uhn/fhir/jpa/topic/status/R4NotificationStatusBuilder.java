...
public class R4NotificationStatusBuilder implements INotificationStatusBuilder<Parameters> {
	...
	public Parameters buildNotificationStatus(
		...
		// See
		// http://build.fhir.org/ig/HL7/fhir-subscription-backport-ig/Parameters-r4-notification-status.json.html
		// and
		...
		parameters.addParameter(
					"status", new CodeType(Subscription.SubscriptionStatus.ACTIVE.toCode()));
		parameters.addParameter(
					"type",
					new CodeType(
								SubscriptionStatus.SubscriptionNotificationType.EVENTNOTIFICATION
										.toCode()));
		...
		notificationEvent
					...
					.setValue(new StringType(eventNumber.toString()));
