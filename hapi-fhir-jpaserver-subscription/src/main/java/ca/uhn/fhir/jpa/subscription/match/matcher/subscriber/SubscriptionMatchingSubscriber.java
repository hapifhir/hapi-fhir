...
public class SubscriptionMatchingSubscriber implements MessageHandler {
    ...
    @Autowired private ISubscriptionMatcher mySubscriptionMatcher;
    ...
    @Autowired private SubscriptionMatchDeliverer mySubscriptionMatchDeliverer;
    ...
    public void matchActiveSubscriptionsAndDeliver(ResourceModifiedMessage theMsg) {
        ...
        if (!myInterceptorBroadcaster.callHooks(
                Pointcut.SUBSCRIPTION_BEFORE_PERSISTED_RESOURCE_CHECKED, params)) {
            return;
        }
        ...
        try {
            ...
            myInterceptorBroadcaster.callHooks(
                    Pointcut.SUBSCRIPTION_AFTER_PERSISTED_RESOURCE_CHECKED, params);
        }
    }
    ...
    private void doMatchActiveSubscriptionsAndDeliver(ResourceModifiedMessage theMsg) {
        ...
        Collection<ActiveSubscription> subscriptions =
                mySubscriptionRegistry.getAllNonTopicSubscriptions();
        ...
        for (ActiveSubscription nextActiveSubscription : subscriptions) {
            anySubscriptionsMatchedResource |=
                    processSubscription(theMsg, resourceId, nextActiveSubscription);
        }
        ...
        if (!anySubscriptionsMatchedResource) {
            ...
            HookParams params = new HookParams().add(ResourceModifiedMessage.class, theMsg);
            myInterceptorBroadcaster.callHooks(
                    Pointcut.SUBSCRIPTION_RESOURCE_DID_NOT_MATCH_ANY_SUBSCRIPTIONS, params);
        }
    }

    /**
     * Returns true if subscription matched, and processing completed successfully, and the message
     * was sent to the delivery channel. False otherwise.
     */
    private boolean processSubscription(
        ...
        if (theActiveSubscription.getCriteria().getType()
                == SubscriptionCriteriaParser.TypeEnum.SEARCH_EXPRESSION) {
            matchResult =
                    mySubscriptionMatcher.match(theActiveSubscription.getSubscription(), theMsg);
        ...
        return mySubscriptionMatchDeliverer.deliverPayload(
                payload, theMsg, theActiveSubscription, matchResult);
    }
    ...
    private boolean resourceTypeIsAppropriateForSubscription(
        ...
        SubscriptionCriteriaParser.SubscriptionCriteria criteria =
                theActiveSubscription.getCriteria();
        ...
        ourLog.trace(
                "Checking subscription {} for {} with criteria {}",
                subscriptionId,
                resourceType,
                criteria);
        ...
        switch (criteria.getType()) {
            ...
            case MULTITYPE_EXPRESSION:
                ...
                ourLog.trace(
                        "Subscription {} applicable resource type check: {}",
                        subscriptionId,
                        contains);
                return contains;
            case STARTYPE_EXPRESSION:
                ...
                ourLog.trace(
                        "Subscription {} start resource type check: {}", subscriptionId, match);
                return match;
        }
    }
}
