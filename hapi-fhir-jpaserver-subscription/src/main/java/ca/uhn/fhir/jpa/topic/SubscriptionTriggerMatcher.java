...
public class SubscriptionTriggerMatcher {
	...
	private InMemoryMatchResult match(
		...
		InMemoryMatchResult previousMatches =
					InMemoryMatchResult.fromBoolean(previousCriteria == null);
		InMemoryMatchResult currentMatches =
					InMemoryMatchResult.fromBoolean(currentCriteria == null);

		// WIP STR5 implement fhirPathCriteria per
		// https://build.fhir.org/subscriptiontopic.html#fhirpath-criteria
		...
		if (previousCriteria != null) {
				if (myOperation == ResourceModifiedMessage.OperationTypeEnum.UPDATE
					...
					Optional<IBaseResource> oPreviousVersion =
								myPreviousVersionReader.readPreviousVersion(myResource);
					if (oPreviousVersion.isPresent()) {
						previousMatches = matchResource(oPreviousVersion.get(), previousCriteria);
					} else {
						ourLog.warn(
									"Resource {} has a version of 1, which should not be the case for a"
	...
	private InMemoryMatchResult matchResource(IBaseResource theResource, String theCriteria) {
		InMemoryMatchResult result =
					mySubscriptionTopicSupport
								...
								.match(theCriteria, theResource, mySrd);
		if (!result.supported()) {
				ourLog.warn(
