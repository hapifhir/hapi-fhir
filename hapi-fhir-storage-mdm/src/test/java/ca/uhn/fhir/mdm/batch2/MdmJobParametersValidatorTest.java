...
class MdmClearJobParametersValidatorTest {
    ...
    @BeforeEach
    public void before() {
        myMdmClearJobParametersValidator =
                new MdmClearJobParametersValidator(myDaoRegistry, myMdmSettings);
    }
    ...
    public void testNoResourceType() {
        ...
        assertThat(result, hasSize(1));
        assertEquals(
                "Mdm Clear Job Parameters must define at least one resource type", result.get(0));
    }

    @Test
    public void testInvalidResourceType() {
        // setup
        MdmClearJobParameters parameters =
                new MdmClearJobParameters().addResourceType("Immunization");
        when(myMdmSettings.isEnabled()).thenReturn(true);
        ...
        assertEquals(
                "Resource type 'Immunization' is not supported on this server.", result.get(0));
