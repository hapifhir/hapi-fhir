...
public class MdmClearStep
    ...
    @Autowired HapiTransactionService myHapiTransactionService;
    @Autowired DaoRegistry myDaoRegistry;
    @Autowired IIdHelperService myIdHelperService;
    @Autowired FhirContext myFhirContext;
    @Autowired IMdmLinkDao myMdmLinkSvc;

    @Nonnull
    @Override
    public RunOutcome run(
            @Nonnull
                    StepExecutionDetails<MdmClearJobParameters, ResourceIdListWorkChunkJson>
    ...
    class MdmClearJob implements TransactionCallback<Void> {
        ...
        private void performWork(List<JpaPid> thePersistentIds) {
            ...
            ourLog.trace(
                    "Deleted {} mdm links in {}",
                    thePersistentIds.size(),
                    StopWatch.formatMillis(sw.getMillis()));

            // We know the list is not empty, and that all resource types are the same, so just use
            // the first one
            ...
            dao.deletePidList(
                    ProviderConstants.OPERATION_MDM_CLEAR,
                    thePersistentIds,
                    conflicts,
                    myRequestDetails);
            DeleteConflictUtil.validateDeleteConflictsEmptyOrThrowException(
                    myFhirContext, conflicts);
            ourLog.trace(
                    "Deleted {} golden resources in {}",
                    thePersistentIds.size(),
                    StopWatch.formatMillis(sw.getMillis()));
            ...
            ourLog.info(
                    "Finished removing {} golden resources in {} - {}/sec - Instance[{}] Chunk[{}]",
                    thePersistentIds.size(),
                    sw,
    ...
    @VisibleForTesting
    public static void setClearCompletionCallbackForUnitTest(
            Runnable theClearCompletionCallbackForUnitTest) {
