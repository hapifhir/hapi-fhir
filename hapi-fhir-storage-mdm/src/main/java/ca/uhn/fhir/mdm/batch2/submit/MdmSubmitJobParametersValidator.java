...
public class MdmSubmitJobParametersValidator
    ...
    public List<String> validate(
            RequestDetails theRequestDetails, @Nonnull MdmSubmitJobParameters theParameters) {
        ...
        for (PartitionedUrl partitionedUrl : theParameters.getPartitionedUrls()) {
            ...
            RuntimeResourceDefinition resourceDefinition =
                    myFhirContext.getResourceDefinition(resourceType);
            validateTypeIsUsedByMdm(errorMsgs, resourceType);
            validateAllSearchParametersApplyToResourceType(
                    errorMsgs, partitionedUrl, resourceType, resourceDefinition);
        }
        return errorMsgs;
    }
    ...
    private void validateAllSearchParametersApplyToResourceType(
        ...
        try {
            ...
            String errorMsg =
                    String.format(
                            "Search parameter %s is not recognized for resource type %s. Source"
                                    + " error is %s",
    ...
    private void validateTypeIsUsedByMdm(List<String> errorMsgs, String resourceType) {
        if (!myMdmSettings.isSupportedMdmType(resourceType)) {
            errorMsgs.add(
                    "Resource type "
                            + resourceType
                            + " is not supported by MDM. Check your MDM settings");
        }
    }
