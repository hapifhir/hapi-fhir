...
public abstract class BaseMdmR4Test extends BaseJpaR4Test {
    ...
    @Autowired protected IFhirResourceDao<Patient> myPatientDao;
    @Autowired protected IFhirResourceDao<Organization> myOrganizationDao;
    @Autowired protected IFhirResourceDao<Medication> myMedicationDao;
    @Autowired protected IFhirResourceDao<Practitioner> myPractitionerDao;
    @Autowired protected IFhirResourceDao<Observation> myObservationDao;
    @Autowired protected MdmResourceMatcherSvc myMdmResourceMatcherSvc;
    @Autowired protected IMdmLinkDao<JpaPid, MdmLink> myMdmLinkDao;
    @Autowired protected MdmLinkDaoSvc<JpaPid, MdmLink> myMdmLinkDaoSvc;
    @Autowired protected IIdHelperService<JpaPid> myIdHelperService;
    @Autowired protected MdmSettings myMdmSettings;
    @Autowired protected MdmMatchLinkSvc myMdmMatchLinkSvc;
    @Autowired protected EIDHelper myEIDHelper;
    ...
    @Autowired protected IPartitionLookupSvc myPartitionLookupSvc;
    ...
    @Nonnull
    protected Patient buildPatientWithNameIdAndBirthday(
            String theGivenName, String theId, Date theBirthday) {
    ...
    @Nonnull
    protected Practitioner buildPractitionerWithNameIdAndBirthday(
            String theGivenName, String theId, Date theBirthday) {
    ...
    @Deprecated
    @Nonnull
    protected Patient buildSourcePatientWithNameIdAndBirthday(
            String theGivenName, String theId, Date theBirthday) {
        Patient patient = new Patient();
    ...
    protected <T extends IAnyResource> T getGoldenResourceFromTargetResource(T theBaseResource) {
        ...
        Optional<MdmLink> matchedLinkForTargetPid =
    ...
    protected <T extends IBaseResource> T getTargetResourceFromMdmLink(
            IMdmLink theMdmLink, String theResourceType) {
    ...
    protected Practitioner createPractitionerAndUpdateLinks(Practitioner thePractitioner) {
        ...
        thePractitioner.setId(daoMethodOutcome.getId());
        myMdmMatchLinkSvc.updateMdmLinksForMdmSource(
                thePractitioner, createContextForCreate("Practitioner"));
        return thePractitioner;
    }
    ...
    protected Practitioner createPractitionerAndUpdateLinksOnPartition(
        ...
        DaoMethodOutcome daoMethodOutcome =
                myPractitionerDao.create(thePractitioner, systemRequestDetails);
        thePractitioner.setId(daoMethodOutcome.getId());
    ...
    private Matcher<IAnyResource> wrapMatcherInTransaction(
            Supplier<Matcher<IAnyResource>> theFunction) {
        return new Matcher<IAnyResource>() {
            ...
            public void describeMismatch(Object actual, Description mismatchDescription) {
                runInTransaction(
                        () -> theFunction.get().describeMismatch(actual, mismatchDescription));
            }
    ...
    protected Matcher<IAnyResource> sameGoldenResourceAs(IAnyResource... theBaseResource) {
        return wrapMatcherInTransaction(
                () ->
                        IsSameGoldenResourceAs.sameGoldenResourceAs(
                                myIdHelperService, myMdmLinkDaoSvc, theBaseResource));
    }

    protected Matcher<IAnyResource> linkedTo(IAnyResource... theBaseResource) {
        return wrapMatcherInTransaction(
                () -> IsLinkedTo.linkedTo(myIdHelperService, myMdmLinkDaoSvc, theBaseResource));
    }

    protected Matcher<IAnyResource> possibleLinkedTo(IAnyResource... theBaseResource) {
        return wrapMatcherInTransaction(
                () ->
                        IsPossibleLinkedTo.possibleLinkedTo(
                                myIdHelperService, myMdmLinkDaoSvc, theBaseResource));
    }
    ...
    protected Matcher<IAnyResource> possibleMatchWith(IAnyResource... theBaseResource) {
        return wrapMatcherInTransaction(
                () ->
                        IsPossibleMatchWith.possibleMatchWith(
                                myIdHelperService, myMdmLinkDaoSvc, theBaseResource));
    }
    ...
    protected Matcher<IAnyResource> possibleDuplicateOf(IAnyResource... theBaseResource) {
        return wrapMatcherInTransaction(
                () ->
                        IsPossibleDuplicateOf.possibleDuplicateOf(
                                myIdHelperService, myMdmLinkDaoSvc, theBaseResource));
    }
    ...
    protected Matcher<IAnyResource> matchedToAGoldenResource() {
        return wrapMatcherInTransaction(
                () ->
                        IsMatchedToAGoldenResource.matchedToAGoldenResource(
                                myIdHelperService, myMdmLinkDaoSvc));
    }
    ...
    protected MdmLink createResourcesAndBuildTestMDMLink() {
        ...
        mdmLink.setGoldenResourcePersistenceId(
                runInTransaction(
                        () ->
                                myIdHelperService.getPidOrNull(
                                        RequestPartitionId.allPartitions(), sourcePatient)));
        mdmLink.setSourcePersistenceId(
                runInTransaction(
                        () ->
                                myIdHelperService.getPidOrNull(
                                        RequestPartitionId.allPartitions(), patient)));
        return mdmLink;
    }
    ...
    private <T> void assertFields(Function<MdmLink, T> theAccessor, T... theExpectedValues) {
        ...
        for (int i = 0; i < links.size(); ++i) {
            assertEquals(
                    theExpectedValues[i],
                    theAccessor.apply(links.get(i)),
                    "Value at index " + i + " was not equal");
        }
    }
    ...
    protected MdmLink createGoldenPatientAndLinkToSourcePatient(
        ...
        mdmLink.setGoldenResourcePersistenceId(
                runInTransaction(
                        () ->
                                myIdHelperService.getPidOrNull(
                                        RequestPartitionId.allPartitions(), goldenPatient)));
        mdmLink.setSourcePersistenceId(
                runInTransaction(
                        () ->
                                myIdHelperService.getPidOrNull(
