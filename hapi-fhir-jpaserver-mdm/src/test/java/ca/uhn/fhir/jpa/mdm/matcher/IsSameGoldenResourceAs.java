...
public class IsSameGoldenResourceAs extends BaseGoldenResourceMatcher {
	...
	protected boolean matchesSafely(IAnyResource theIncomingResource) {
		...
		boolean allToCheckAreSame =
				goldenResourcePidsToMatch.stream().allMatch(pid -> pid.equals(goldenResourcePidsToMatch.get(0)));
		if (!allToCheckAreSame) {
			throw new IllegalStateException(
					"You wanted to do a source resource comparison, but the pool of source resources you submitted for checking don't match! We won't even check the incoming source resource against them.");
		}
	...
	public void describeTo(Description theDescription) {
		theDescription.appendText(String.format(
				" %s linked to source resource %s/%s", myTargetType, myTargetType, goldenResourcePidsToMatch));
	}
	...
	protected void describeMismatchSafely(IAnyResource item, Description mismatchDescription) {
		...
		mismatchDescription.appendText(
				String.format(" was actually linked to %s/%s", myTargetType, incomingGoldenResourcePid));
	}

	public static Matcher<IAnyResource> sameGoldenResourceAs(
			IIdHelperService theIdHelperService, MdmLinkDaoSvc theMdmLinkDaoSvc, IAnyResource... theBaseResource) {
