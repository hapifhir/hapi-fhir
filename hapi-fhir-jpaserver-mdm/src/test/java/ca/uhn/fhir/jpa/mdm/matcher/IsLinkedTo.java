...
/**
 * A Matcher which allows us to check that a target patient/practitioner at a given link level. is
 * linked to a set of patients/practitioners via a golden resource.
 */
public class IsLinkedTo extends BaseGoldenResourceMatcher {
	...
	protected boolean matchesSafely(IAnyResource theIncomingResource) {
		...
		// OK, lets grab all the golden resource pids of the resources passed in via the
		// constructor.
		...
		// The resources are linked if all golden resource pids match the incoming golden resource
		// pid.
