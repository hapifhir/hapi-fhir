...
public class MdmHelperR4 extends BaseMdmHelper {
    ...
    public OutcomeAndLogMessageWrapper createWithLatch(IBaseResource theResource)
            throws InterruptedException {
        return createWithLatch(theResource, true);
    }

    public OutcomeAndLogMessageWrapper createWithLatch(
            IBaseResource theBaseResource, boolean isExternalHttpRequest)
            throws InterruptedException {
        myAfterMdmLatch.setExpectedCount(1);
        DaoMethodOutcome daoMethodOutcome =
                doCreateResource(theBaseResource, isExternalHttpRequest);
    ...
    public OutcomeAndLogMessageWrapper updateWithLatch(IBaseResource theIBaseResource)
            throws InterruptedException {
        return updateWithLatch(theIBaseResource, true);
    }

    public OutcomeAndLogMessageWrapper updateWithLatch(
            IBaseResource theIBaseResource, boolean isExternalHttpRequest)
            throws InterruptedException {
        myAfterMdmLatch.setExpectedCount(1);
        DaoMethodOutcome daoMethodOutcome =
                doUpdateResource(theIBaseResource, isExternalHttpRequest);
        List<HookParams> hookParams = myAfterMdmLatch.awaitExpected();
        return new OutcomeAndLogMessageWrapper(daoMethodOutcome, hookParams);
    }

    public DaoMethodOutcome doCreateResource(
            IBaseResource theResource, boolean isExternalHttpRequest) {
    ...
    public DaoMethodOutcome doUpdateResource(
            IBaseResource theResource, boolean isExternalHttpRequest) {
        String resourceType = myFhirContext.getResourceType(theResource);
        IFhirResourceDao<IBaseResource> dao = myDaoRegistry.getResourceDao(resourceType);
    ...
    /**
     * OutcomeAndLogMessageWrapper is a simple wrapper class which is _excellent_. It allows us to
     * skip the fact that java doesn't allow multiple returns, and wraps both the Method Outcome of
     * the DAO, _and_ the TransactionLogMessages that were passed to the pointcut by the MDM module.
     */
    public class OutcomeAndLogMessageWrapper {
        ...
        public OutcomeAndLogMessageWrapper(
                DaoMethodOutcome theDaoMethodOutcome, List<HookParams> theHookParams) {
        ...
        public TransactionLogMessages getLogMessages() {
            return PointcutLatch.getInvocationParameterOfType(
                    myHookParams, TransactionLogMessages.class);
        }
        ...
        public ResourceOperationMessage getResourceOperationMessage() {
            return PointcutLatch.getInvocationParameterOfType(
                    myHookParams, ResourceOperationMessage.class);
        }
    }
}
