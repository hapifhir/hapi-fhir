 ...
 *
 * <p>This rule is to be used whenever you want to have the MdmInterceptor loaded, and be able to
 * execute creates/updates/deletes while being assured that all MDM work has been done before
 * exiting. Provides two types of method:
 *
 * <p>1. doUpdate/doCreate. These methods do not wait for Asynchronous MDM work to be done. Use
 * these when you are expecting the calls to fail, as those hooks will never be called.
 *
 * <p>2. createWithLatch/updateWithLatch. These methods will await the MDM hooks, which are only
 * triggered post-MDM processing You should use these when you are expecting successful processing
 * of the resource, and need to wait for async MDM linking work to be done.
 *
 * <p>Note: all create/update functions take an optional isExternalHttpRequest boolean, to make it
