...
public class IsPossibleDuplicateOf extends BaseGoldenResourceMatcher {
    ...
    /**
     * Matcher with tells us if there is an MdmLink with between these two resources that are
     * considered POSSIBLE DUPLICATE. For use only on GoldenResource.
     */
    private IResourcePersistentId incomingGoldenResourcePid;

    protected IsPossibleDuplicateOf(
            IIdHelperService theIdHelperService,
            MdmLinkDaoSvc theMdmLinkDaoSvc,
    ...
    protected boolean matchesSafely(IAnyResource theIncomingResource) {
        ...
        List<IResourcePersistentId> goldenResourcePidsToMatch =
                myBaseResources.stream()
                        .map(this::getMatchedResourcePidFromResource)
                        .collect(Collectors.toList());

        // Returns true if there is a POSSIBLE_DUPLICATE between the incoming resource, and all of
        // the resources passed in via the constructor.
    ...
    public void describeTo(Description theDescription) {
        theDescription.appendText(
                "Resource was not duplicate of Resource/" + incomingGoldenResourcePid);
    }
