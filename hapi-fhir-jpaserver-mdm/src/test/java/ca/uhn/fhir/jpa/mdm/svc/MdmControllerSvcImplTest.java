...
public class MdmControllerSvcImplTest extends BaseLinkR4Test {
	...
	@Autowired IMdmControllerSvc myMdmControllerSvc;

	@SpyBean @Autowired IRequestPartitionHelperSvc myRequestPartitionHelperSvc;

	@Autowired private IInterceptorService myInterceptorService;
	@Autowired private Batch2JobHelper myBatch2JobHelper;
	@Autowired private MdmSettings myMdmSettings;
	private UserRequestRetryVersionConflictsInterceptor
				myUserRequestRetryVersionConflictsInterceptor;
	private final RequestTenantPartitionInterceptor myPartitionInterceptor =
				new RequestTenantPartitionInterceptor();
	...
	public void testSurvivorshipIsCalledOnMatchingToTheSameGoldenResource() {
		...
		MdmPageRequest pageRequest =
					new MdmPageRequest((Integer) null, null, DEFAULT_PAGE_SIZE, MAX_PAGE_SIZE);
		MdmQuerySearchParameters params =
					new MdmQuerySearchParameters(pageRequest)
	...
	public void testMdmDuplicateGoldenResource() {
		...
		assertEquals(
					patient.getIdElement().getResourceType() + "/" + patient.getIdElement().getIdPart(),
					resultPage.getContent().get(0).getSourceId());
	...
	public void testMdmClearWithProvidedResources() {
		...
		Parameters.ParametersParameterComponent parameter =
					((Parameters) clearJob).getParameter("jobId");
	...
	public void testMdmClearWithWriteConflict() {
		...
		IBaseParameters clearJob = myMdmControllerSvc.submitMdmClearJob(urls, batchSize, details);
		String jobId =
					((StringType) ((Parameters) clearJob).getParameterValue("jobId"))
								.getValueAsString();
		myBatch2JobHelper.awaitJobCompletion(jobId);
	...
	private class PartitionIdMatcher implements ArgumentMatcher<RequestPartitionId> {
		...
		public boolean matches(RequestPartitionId theRequestPartitionId) {
				return myRequestPartitionId
						.getPartitionIds()
						.equals(theRequestPartitionId.getPartitionIds());
		}
	}
}
