...
public class MdmLinkCreateSvcImpl implements IMdmLinkCreateSvc {
    ...
    public IAnyResource createLink(
        ...
        // check if the golden resource and the source resource are in the same partition, throw
        // error if not
        ...
        IMdmLink mdmLink =
                myMdmLinkDaoSvc.getOrCreateMdmLinkByGoldenResourceAndSourceResource(
                        theGoldenResource, theSourceResource);
    ...
    private void validateCreateLinkRequest(
        ...
        if (!MdmResourceUtil.isMdmManaged(theGoldenRecord)) {
            throw new InvalidRequestException(
                    Msg.code(758) + myMessageHelper.getMessageForUnmanagedResource());
        }
