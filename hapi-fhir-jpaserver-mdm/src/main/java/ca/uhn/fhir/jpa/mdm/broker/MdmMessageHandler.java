...
public class MdmMessageHandler implements MessageHandler {
    ...
    @Autowired private MdmMatchLinkSvc myMdmMatchLinkSvc;
    ...
    @Autowired private IMdmModelConverterSvc myModelConverter;
    ...
    private void matchMdmAndUpdateLinks(
            IBaseResource theSourceResource, ResourceModifiedMessage theMsg) {
        ...
        if (myInterceptorBroadcaster.hasHooks(Pointcut.MDM_BEFORE_PERSISTED_RESOURCE_CHECKED)) {
            ...
            myInterceptorBroadcaster.callHooks(
                    Pointcut.MDM_BEFORE_PERSISTED_RESOURCE_CHECKED, params);
        }
        ...
        try {
            switch (theMsg.getOperationType()) {
                ...
                default:
                    ourLog.trace(
                            "Not processing modified message for {}", theMsg.getOperationType());
            }
        } catch (Exception e) {
            ...
            HookParams params =
                    new HookParams()
                            ...
                            .add(
                                    TransactionLogMessages.class,
                                    mdmContext.getTransactionLogMessages())
                            .add(MdmLinkEvent.class, buildLinkChangeEvent(mdmContext));

            myInterceptorBroadcaster.callHooks(
                    Pointcut.MDM_AFTER_PERSISTED_RESOURCE_CHECKED, params);
        }
    }

    private MdmTransactionContext createMdmContext(
            ResourceModifiedMessage theMsg, String theResourceType) {
        ...
        switch (theMsg.getOperationType()) {
            ...
            default:
                ourLog.trace(
                        "Not creating an MdmTransactionContext for {}", theMsg.getOperationType());
    ...
    private void validateResourceType(String theResourceType) {
        if (!myMdmSettings.isSupportedMdmType(theResourceType)) {
            throw new IllegalStateException(
                    Msg.code(735)
                            + "Unsupported resource type submitted to MDM matching queue: "
                            + theResourceType);
        }
    }

    private void handleCreateResource(
            IBaseResource theResource, MdmTransactionContext theMdmTransactionContext) {
        myMdmMatchLinkSvc.updateMdmLinksForMdmSource(
                (IAnyResource) theResource, theMdmTransactionContext);
    }
    ...
    private void log(
            MdmTransactionContext theMdmContext, String theMessage, Exception theException) {
    ...
    private MdmLinkEvent buildLinkChangeEvent(MdmTransactionContext theMdmContext) {
        ...
        theMdmContext.getMdmLinks().stream()
                .forEach(
                        l -> {
    ...
    private ResourceOperationMessage getOutgoingMessage(ResourceModifiedMessage theMsg) {
        ...
        ResourceOperationMessage outgoingMsg =
                new ResourceOperationMessage(
                        myFhirContext, targetResource, theMsg.getOperationType());
