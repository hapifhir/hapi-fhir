...
public class MdmLinkDaoSvc<P extends IResourcePersistentId, M extends IMdmLink<P>> {
	...
	@Autowired private IMdmLinkDao<P, M> myMdmLinkDao;
	@Autowired private MdmLinkFactory<M> myMdmLinkFactory;
	@Autowired private IIdHelperService<P> myIdHelperService;
	...
	public M createOrUpdateLinkEntity(
		...
		if (partitionId != null && partitionId.getFirstPartitionIdOrNull() != null) {
				mdmLink.setPartitionId(
						new PartitionablePartitionId(
									partitionId.getFirstPartitionIdOrNull(),
									partitionId.getPartitionDate()));
		}
	...
	public M getOrCreateMdmLinkByGoldenResourceAndSourceResource(
		...
		P sourceResourcePid =
					myIdHelperService.getPidOrNull(
								RequestPartitionId.allPartitions(), theSourceResource);
		Optional<M> oExisting =
					getLinkByGoldenResourcePidAndSourceResourcePid(
								goldenResourcePid, sourceResourcePid);
	...
	/**
	* Given a golden resource Pid and source Pid, return the mdm link that matches these criterias
	* if exists
	*
	...
	*/
	...
	public Optional<M> getLinkByGoldenResourcePidAndSourceResourcePid(
		...
		return getLinkByGoldenResourcePidAndSourceResourcePid(
					myIdHelperService.newPid(theGoldenResourcePid),
					myIdHelperService.newPid(theSourceResourcePid));
	}
	...
	*/
	public Optional<M> getLinkByGoldenResourcePidAndSourceResourcePid(
				P theGoldenResourcePid, P theSourceResourcePid) {
	...
	*/
	public List<M> getMdmLinksBySourcePidAndMatchResult(
	...
	/**
	* Given a source Pid, return its Matched {@link IMdmLink}. There can only ever be at most one
	* of these, but its possible the source has no matches, and may return an empty optional.
	*
	...
	*/
	...
	/**
	* Given an IBaseResource, return its Matched {@link IMdmLink}. There can only ever be at most
	* one of these, but its possible the source has no matches, and may return an empty optional.
	*
	* @param theSourceResource The IBaseResource representing the source you wish to find the
	*     matching link for.
	* @return the {@link IMdmLink} that contains the Match information for the source.
	*/
	...
	@Nonnull
	private Optional<M> getMdmLinkWithMatchResult(
				IBaseResource theSourceResource, MdmMatchResultEnum theMatchResult) {
		P pid =
					myIdHelperService.getPidOrNull(
								RequestPartitionId.allPartitions(), theSourceResource);
		if (pid == null) {
	...
	/**
	* Given a golden resource a source and a match result, return the matching {@link IMdmLink}, if
	* it exists.
	*
	...
	*/
	public Optional<M> getMdmLinksByGoldenResourcePidSourcePidAndMatchResult(
		...
		return getMdmLinksByGoldenResourcePidSourcePidAndMatchResult(
					myIdHelperService.newPid(theGoldenResourcePid),
					myIdHelperService.newPid(theSourcePid),
					theMatchResult);
	}
	...
	/**
	* Get all {@link IMdmLink} which have {@link MdmMatchResultEnum#POSSIBLE_DUPLICATE} as their
	* match result.
	*
	...
	*/
	...
	/**
	* Delete a given {@link IMdmLink}. Note that this does not clear out the Golden resource. It is
	* a simple entity delete.
	*
	...
	* {@link IMdmLink}
	*
	* @param theGoldenResource The {@link IBaseResource} Golden Resource who's links you would like
	*     to retrieve.
	* @return A list of all {@link IMdmLink} entities in which theGoldenResource is the source
	*     Golden Resource
	*/
	@Transactional
	...
	*/
	...
	/**
	* Given a source {@link IBaseResource}, return all {@link IMdmLink} entities in which this
	* source is the source of the relationship. This will show you all links for a given
	...
	*/
	...
	/**
	* Finds all {@link IMdmLink} entities in which theGoldenResource's PID is the source of the
	* relationship.
	*
	...
	*/
	...
	/**
	* Factory delegation method, whenever you need a new MdmLink, use this factory method. //TODO
	* Should we make the constructor private for MdmLink? or work out some way to ensure they can
	* only be instantiated via factory.
	*
	...
	*/
	...
	@Transactional
	public List<MdmLinkWithRevision<M>> findMdmLinkHistory(
