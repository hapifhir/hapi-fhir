 ...
 * updateMdmLinksForMdmSource and the underlying MDM system will take care of matching it to a
 * GoldenResource, or creating a new GoldenResource if a suitable one was not found.
 */
@Service
public class MdmMatchLinkSvc {
    ...
    @Autowired private IMdmLinkSvc myMdmLinkSvc;
    @Autowired private MdmGoldenResourceFindingSvc myMdmGoldenResourceFindingSvc;
    @Autowired private GoldenResourceHelper myGoldenResourceHelper;
    @Autowired private MdmEidUpdateService myEidUpdateService;

    /**
     * Given an MDM source (consisting of any supported MDM type), find a suitable Golden Resource
     * candidate for them, or create one if one does not exist. Performs matching based on rules
     * defined in mdm-rules.json. Does nothing if resource is determined to be not managed by MDM.
     *
     * @param theResource the incoming MDM source, which can be any supported MDM type.
     * @param theMdmTransactionContext
     * @return an {@link TransactionLogMessages} which contains all informational messages related
     *     to MDM processing of this resource.
     */
    @Transactional
    ...
    private MdmTransactionContext doMdmUpdate(
        ...
        CandidateList candidateList =
                myMdmGoldenResourceFindingSvc.findGoldenResourceCandidates(theResource);

        if (candidateList.isEmpty()) {
            handleMdmWithNoCandidates(theResource, theMdmTransactionContext);
        } else if (candidateList.exactlyOneMatch()) {
            handleMdmWithSingleCandidate(
                    theResource, candidateList.getOnlyMatch(), theMdmTransactionContext);
        } else {
    ...
    private void handleMdmWithMultipleCandidates(
            ...
            CandidateList theCandidateList,
            MdmTransactionContext theMdmTransactionContext) {
        MatchedGoldenResourceCandidate firstMatch = theCandidateList.getFirstMatch();
        IResourcePersistentId<?> sampleGoldenResourcePid =
                firstMatch.getCandidateGoldenResourcePid();
        boolean allSameGoldenResource =
                theCandidateList.stream()
                        .allMatch(
                                candidate ->
                                        candidate
                                                .getCandidateGoldenResourcePid()
                                                .equals(sampleGoldenResourcePid));

        if (allSameGoldenResource) {
            log(
                    theMdmTransactionContext,
                    "MDM received multiple match candidates, but they are all linked to the same"
                            + " Golden Resource.");
            handleMdmWithSingleCandidate(theResource, firstMatch, theMdmTransactionContext);
        } else {
            log(
                    theMdmTransactionContext,
                    "MDM received multiple match candidates, that were linked to different Golden"
                            + " Resources. Setting POSSIBLE_DUPLICATES and POSSIBLE_MATCHES.");
            ...
            goldenResources
                    .subList(1, goldenResources.size())
                    .forEach(
    ...
    private List<IAnyResource> createPossibleMatches(
        ...
        for (MatchedGoldenResourceCandidate matchedGoldenResourceCandidate :
                theCandidateList.getCandidates()) {
            IAnyResource goldenResource =
                    myMdmGoldenResourceFindingSvc
    ...
    private void handleMdmWithNoCandidates(
        ...
        log(
                theMdmTransactionContext,
                String.format(
                        "There were no matched candidates for MDM, creating a new %s Golden"
                                + " Resource.",
    ...
    private void handleMdmCreate(
        ...
        if (myGoldenResourceHelper.isPotentialDuplicate(goldenResource, theTargetResource)) {
            log(
                    theMdmTransactionContext,
                    "Duplicate detected based on the fact that both resources have different"
                            + " external EIDs.");
            IAnyResource newGoldenResource =
                    myGoldenResourceHelper.createGoldenResourceFromMdmSourceResource(
    ...
    private void handleMdmWithSingleCandidate(
            ...
            MatchedGoldenResourceCandidate theGoldenResourceCandidate,
            MdmTransactionContext theMdmTransactionContext) {
        if (theMdmTransactionContext
                .getRestOperation()
                .equals(MdmTransactionContext.OperationType.UPDATE_RESOURCE)) {
            log(theMdmTransactionContext, "MDM has narrowed down to one candidate for matching.");
            myEidUpdateService.handleMdmUpdate(
                    theResource, theGoldenResourceCandidate, theMdmTransactionContext);
        } else {
