...
public class MdmCandidateSearchSvc {
	...
	@Autowired private MdmCandidateSearchCriteriaBuilderSvc myMdmCandidateSearchCriteriaBuilderSvc;
	@Autowired private CandidateSearcher myCandidateSearcher;

	public MdmCandidateSearchSvc() {}

	/**
	* Given a source resource, search for all resources that are considered an MDM match based on
	* defined MDM rules.
	*
	...
	* @param theRequestPartitionId the {@link RequestPartitionId} representation of the partitions
	*     we are limited to when attempting to match
	...
	*/
	...
	public Collection<IAnyResource> findCandidates(
		...
		// If there are zero MdmResourceSearchParamJson, we end up only making a single search,
		// otherwise we
		...
		if (theResource.getIdElement().getIdPart() != null) {
				if (matchedPidsToResources.remove(
									myIdHelperService.getPidOrNull(
												RequestPartitionId.allPartitions(), theResource))
						!= null) {
					ourLog.debug(
	...
	private boolean isSearchParamForResource(
				String theResourceType, MdmResourceSearchParamJson resourceSearchParam) {
		String resourceType = resourceSearchParam.getResourceType();
		return resourceType.equals(theResourceType)
					|| resourceType.equalsIgnoreCase(ALL_RESOURCE_SEARCH_PARAM_TYPE);
	}
	...
	*/
	...
	private void searchForIdsAndAddToMap(
				...
				RequestPartitionId theRequestPartitionId) {
		// 1.
		Optional<String> oResourceCriteria =
					myMdmCandidateSearchCriteriaBuilderSvc.buildResourceQueryString(
		...
		Optional<IBundleProvider> bundleProvider =
					myCandidateSearcher.search(
								theResourceType, resourceCriteria, theRequestPartitionId);
		if (!bundleProvider.isPresent()) {
				throw new TooManyCandidatesException(
						Msg.code(762)
									+ "More than "
									+ myMdmSettings.getCandidateSearchLimit()
									+ " candidate matches found for "
									+ resourceCriteria
									+ ".  Aborting mdm matching.");
		}
		...
		resources.forEach(
					resource ->
								theMatchedPidsToResources.put(
										myIdHelperService.getPidOrNull(
													RequestPartitionId.allPartitions(), resource),
	...
	private List<String> buildFilterQuery(
		...
		return Collections.unmodifiableList(
					theFilterSearchParams.stream()
								.filter(spFilterJson -> paramIsOnCorrectType(theResourceType, spFilterJson))
								.map(this::convertToQueryString)
								.collect(Collectors.toList()));
	}

	private boolean paramIsOnCorrectType(
				String theResourceType, MdmFilterSearchParamJson spFilterJson) {
