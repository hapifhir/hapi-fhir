...
public class MdmLinkSvcImpl implements IMdmLinkSvc {
    ...
    public void updateLink(
        ...
        validateRequestIsLegal(
                theGoldenResource, theSourceResource, matchResultEnum, theLinkSource);
    ...
    private boolean goldenResourceLinkedAsNoMatch(
            IAnyResource theGoldenResource, IAnyResource theSourceResource) {
        IResourcePersistentId goldenResourceId =
                myIdHelperService.getPidOrThrowException(theGoldenResource);
        IResourcePersistentId sourceId =
                myIdHelperService.getPidOrThrowException(theSourceResource);
    ...
    private void validateRequestIsLegal(
        ...
        if (oExistingLink.isPresent()
                && systemIsAttemptingToModifyManualLink(theLinkSource, oExistingLink.get())) {
            throw new InternalErrorException(
                    Msg.code(760)
                            + "MDM system is not allowed to modify links on manually created"
                            + " links");
        }
        ...
        if (systemIsAttemptingToAddNoMatch(theLinkSource, theMatchResult)) {
            throw new InternalErrorException(
                    Msg.code(761)
                            + "MDM system is not allowed to automatically NO_MATCH a resource");
        }
    }

    /**
     * Helper function which detects when the MDM system is attempting to add a NO_MATCH link, which
     * is not allowed.
     */
    private boolean systemIsAttemptingToAddNoMatch(
            MdmLinkSourceEnum theLinkSource, MdmMatchResultEnum theMatchResult) {
        return theLinkSource == MdmLinkSourceEnum.AUTO
                && theMatchResult == MdmMatchResultEnum.NO_MATCH;
    }

    /**
     * Helper function to let us catch when System MDM rules are attempting to override a manually
     * defined link.
     */
        ...
        if (theGoldenResource.getIdElement().getIdPart() == null
            ...
            return myMdmLinkDaoSvc.getLinkByGoldenResourcePidAndSourceResourcePid(
                    myIdHelperService.getPidOrNull(
                            RequestPartitionId.allPartitions(), theGoldenResource),
                    myIdHelperService.getPidOrNull(
                            RequestPartitionId.allPartitions(), theCandidate));
        }
    }
