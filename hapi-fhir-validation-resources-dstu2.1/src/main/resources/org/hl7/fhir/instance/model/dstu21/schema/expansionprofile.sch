...
<sch:schema xmlns:sch="http://purl.oclc.org/dsdl/schematron" queryBinding="xslt2">
  ...
  <sch:pattern>
    ...
    <sch:rule context="//f:ExpansionProfile">
      ...
      <sch:assert test="exists(f:codeSystems.include) or exists(f:codeSystems.exclude) or (exists(f:includeDesignations) and f:includeDesignations='true') orexists(f:excludeNested) orexists(f:codeSystem.excludeNotForUI) orexists(f:codeSystem.excludePostCoordinated) orexists(f:codeSystem.displayLanguage) orexists(f:codeSystem.limitedExpansion)">exp-5: SHALL have at least one of– codeSystems.include– codeSystems.exclude– includeDesignations (as true)– excludeNested– excludeNotForUI– excludePostCoordinated– displayLanguage– limitedExpansion</sch:assert>
    </sch:rule>
