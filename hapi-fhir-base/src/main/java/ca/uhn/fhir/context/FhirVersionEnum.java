...
public enum FhirVersionEnum {
	...
	*/
	...
	public IFhirVersion getVersionImplementation() {
		if (!isPresentOnClasspath()) {
				throw new IllegalStateException(
						Msg.code(1709) + "Version " + name() + " is not present on classpath");
		}
		if (myVersionImplementation == null) {
				try {
					...
					throw new InternalErrorException(
								Msg.code(1710) + "Failed to instantiate FHIR version " + name(), e);
				}
		}
	...
	/**
	* Creates a new FhirContext for this FHIR version, or returns a previously created one if one
	* exists. This method uses {@link FhirContext#forCached(FhirVersionEnum)} to return a cached
	* instance.
	*/
	...
	/**
	* This class attempts to read the FHIR version from the actual model classes in order to supply
	* an accurate version string even over time
	*/
	...
	/**
	* Returns the {@link FhirVersionEnum} which corresponds to a specific version of FHIR. Partial
	* version strings (e.g. "3.0") are acceptable. This method will also accept version names such
	* as "DSTU2", "STU3", "R5", etc.
	*
