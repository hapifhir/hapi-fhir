...
public class ApacheEncoder implements IPhoneticEncoder {
    ...
    public String encode(String theString) {
        try {
            // If the string contains a space, encode alpha parts separately so, for example,
            // numbers are preserved in address lines.
    ...
    private String encodeStringWithSpaces(String theString) throws EncoderException {
        ...
        for (String part : theString.split("[\\s\\W]+")) {
            if (StringUtils.isAlpha(part)) {
                ...
                // Once we hit a non-alpha part, encode all the alpha parts together as a single
                // string
    ...
    private StringJoiner encodeAlphaParts(StringJoiner theJoiner, StringJoiner theAlphaJoiner)
