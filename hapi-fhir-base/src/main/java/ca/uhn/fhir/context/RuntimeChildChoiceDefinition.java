...
public class RuntimeChildChoiceDefinition extends BaseRuntimeDeclaredChildDefinition {
     ...
     * Constructor
     *
     * <p>For extension, if myChoiceTypes will be set some other way
     */
    ...
    public BaseRuntimeElementDefinition<?> getChildByName(String theName) {
        assert myNameToChildDefinition.containsKey(theName)
                : "Can't find child '"
                        + theName
                        + "' in names: "
                        + myNameToChildDefinition.keySet();
            ...
            if (IBaseResource.class.isAssignableFrom(next)) {
                ...
                if (nextDef instanceof IRuntimeDatatypeDefinition) {
                    IRuntimeDatatypeDefinition nextDefDatatype =
                            (IRuntimeDatatypeDefinition) nextDef;
                    if (nextDefDatatype.getProfileOf() != null
                            && !IPrimitiveType.class.isAssignableFrom(next)) {
                        ...
                        BaseRuntimeElementDefinition<?> elementDef =
                                theClassToElementDefinitions.get(profileType);
                        elementName =
                                getElementName() + StringUtils.capitalize(elementDef.getName());
                    }
                }
                if (nextDefForChoice != null) {
                    elementName =
                            getElementName() + StringUtils.capitalize(nextDefForChoice.getName());
                }
            }
