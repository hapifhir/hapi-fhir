...
public class RuntimeResourceDefinition
    ...
    private Map<String, RuntimeSearchParam> myNameToSearchParam =
            new LinkedHashMap<String, RuntimeSearchParam>();
     ...
     *
     * <p>If the definition has no parent, returns <code>this</code>
     */
    ...
    public void sealAndInitialize(
        ...
        ArrayList<RuntimeSearchParam> searchParams =
                new ArrayList<RuntimeSearchParam>(myNameToSearchParam.values());
        Collections.sort(
                searchParams,
                new Comparator<RuntimeSearchParam>() {
                    @Override
            ...
            if (next.getProvidesMembershipInCompartments() != null) {
                for (String nextCompartment : next.getProvidesMembershipInCompartments()) {
                    ...
                    if (nextCompartment.startsWith("Base FHIR compartment definition for ")) {
                        nextCompartment =
                                nextCompartment.substring(
                                        "Base FHIR compartment definition for ".length());
                    }
                    ...
                    for (RuntimeSearchParam nextAlternate : searchParams) {
                        String nextAlternatePath =
                                massagePathForCompartmentSimilarity(nextAlternate.getPath());
        ...
        do {
            ...
            if (IBaseResource.class.isAssignableFrom(target)
                    && target.getAnnotation(ResourceDef.class) != null) {
                myBaseType = (Class<? extends IBaseResource>) target;
            }
                ...
                if (!IDomainResource.class.isAssignableFrom(getImplementingClass())) {
                    throw new ConfigurationException(
                            Msg.code(1733)
                                    + "Class \""
                                    + getImplementingClass()
                                    + "\" is invalid. This resource type is not a DomainResource,"
                                    + " it must not have extensions");
                }
            }
        }
    }
