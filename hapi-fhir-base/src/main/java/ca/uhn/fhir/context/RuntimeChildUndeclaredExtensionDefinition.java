...
public class RuntimeChildUndeclaredExtensionDefinition extends BaseRuntimeChildDefinition {
    ...
    private void addReferenceBinding(
        ...
        BaseRuntimeElementDefinition<?> def =
                findResourceReferenceDefinition(theClassToElementDefinitions);
        ...
        if (!value.equals(VALUE_RESOURCE)) {
            myDatatypeToAttributeName.put(
                    theContext.getVersion().getResourceReferenceType(), value);
    ...
    void sealAndInitialize(
            ...
            Map<Class<? extends IBase>, BaseRuntimeElementDefinition<?>>
                    theClassToElementDefinitions) {
        Map<String, BaseRuntimeElementDefinition<?>> datatypeAttributeNameToDefinition =
                new HashMap<>();
        ...
        for (BaseRuntimeElementDefinition<?> next : theClassToElementDefinitions.values()) {
            if (next instanceof IRuntimeDatatypeDefinition) {
                ...
                if (datatypeAttributeNameToDefinition.containsKey(attrName)) {
                    BaseRuntimeElementDefinition<?> existing =
                            datatypeAttributeNameToDefinition.get(attrName);
                    // We do allow built-in standard types to override extension types with the same
                    // element name,
                    // e.g. how EnumerationType extends CodeType but both serialize to "code". In
                    // this case,
                    // CodeType should win. If we aren't in a situation like that, there is a
                    // problem with the
                    ...
                    if (!existing.isStandardType()) {
                        throw new ConfigurationException(
                                Msg.code(1734)
                                        ...
                                        + attrName
                                        + ". Found ["
                                        + existing.getImplementingClass().getName()
                                        + "] and ["
