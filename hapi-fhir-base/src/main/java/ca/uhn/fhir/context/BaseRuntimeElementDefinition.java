...
public abstract class BaseRuntimeElementDefinition<T extends IBase> {
    ...
    private Map<Class<?>, Constructor<T>> myConstructors =
    ...
    private List<RuntimeChildDeclaredExtensionDefinition> myExtensionsNonModifier =
            new ArrayList<>();
            ...
            if (retVal == null) {
                throw new ConfigurationException(
                        Msg.code(1695)
                                ...
                                + " has no constructor with a single argument of type "
                                + argumentType);
            }
    ...
    public RuntimeChildDeclaredExtensionDefinition getDeclaredExtension(
        ...
        if (definition == null && StringUtils.isNotBlank(serverBaseUrl)) {
            for (final Map.Entry<String, RuntimeChildDeclaredExtensionDefinition> entry :
                    myUrlToExtension.entrySet()) {
                final String key =
                        (!UrlUtil.isValid(entry.getKey()) && StringUtils.isNotBlank(serverBaseUrl))
    ...
    /**
     * @return Returns the runtime name for this resource (i.e. the name that will be used in
     *     encoded messages)
     */
    ...
    void sealAndInitialize(
        ...
        do {
            BaseRuntimeElementDefinition<?> parentDefinition =
                    theClassToElementDefinitions.get(parent);
