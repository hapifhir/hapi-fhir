...
public class RuntimeSearchParam {
    ...
    public RuntimeSearchParam(
        ...
        myStatus = theStatus;
        if (theProvidesMembershipInCompartments != null
                && !theProvidesMembershipInCompartments.isEmpty()) {
            myProvidesMembershipInCompartments =
                    Collections.unmodifiableSet(theProvidesMembershipInCompartments);
        } else {
    ...
    public RuntimeSearchParam addExtension(String theKey, IBaseExtension<?, ?> theValue) {
        List<IBaseExtension<?, ?>> valuesList =
                myExtensions.computeIfAbsent(theKey, k -> new ArrayList<>());
    ...
    /**
     * Does this search parameter have an uplift refchain definition for the given code? See the
     * HAPI FHIR documentation for a description of how uplift refchains work.
     *
     * @since 6.6.0
     */
    ...
    /**
     * Returns a set of all codes associated with uplift refchains for this search parameter. See
     * the HAPI FHIR documentation for a description of how uplift refchains work.
     *
     * @since 6.6.0
     */
    ...
    /**
     * Does this search parameter have any uplift refchain definitions? See the HAPI FHIR
     * documentation for a description of how uplift refchains work.
     *
     * @since 6.6.0
     */
    ...
    /**
     * This method tests whether a given FHIRPath expression <i>could</i> possibly apply to the
     * given resource type.
     *
    ...
    private static boolean fhirPathExpressionStartsWith(
            String theResourceName, String thePath, int theStartingIndex) {
        if (thePath.startsWith(theResourceName, theStartingIndex)
                && thePath.length() > theResourceName.length()) {
