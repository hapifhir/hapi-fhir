...
public abstract class BaseRuntimeElementCompositeDefinition<T extends IBase>
            ...
            for (Field next : nextClass.getDeclaredFields()) {
                if (fields.add(next)) {
                    ScannedField scannedField =
                            new ScannedField(next, theImplementingClass, fieldIndexInClass == 0);
        ...
        if (retVal == null) {
            throw new DataFormatException(
                    Msg.code(1700)
                            ...
                            + getName()
                            + " - Valid names are: "
                            + new TreeSet<String>(myNameToChild.keySet()));
        }
        return retVal;
    }
    ...
    private void scanCompositeElementForChildren() {
        ...
        if (forcedOrder != null) {
             ...
             */
            TreeMap<Integer, BaseRuntimeDeclaredChildDefinition> newOrderToExtensionDef =
                    new TreeMap<>();
            int unknownCount = 0;
    ...
    private void scanCompositeElementForChildren(
        ...
        for (ScannedField next : myScannedFields) {
            ...
            Description descriptionAnnotation =
                    ModelScanner.pullAnnotation(next.getField(), Description.class);
            ...
            if (order == Child.REPLACE_PARENT) {
                ...
                if (extensionAttr != null) {
                    ...
                    for (Entry<Integer, BaseRuntimeDeclaredChildDefinition> nextEntry :
                            orderMap.entrySet()) {
                    ...
                    if (order == Child.REPLACE_PARENT) {
                        throw new ConfigurationException(
                                Msg.code(1701)
                                        ...
                                        + " has order() of REPLACE_PARENT ("
                                        + Child.REPLACE_PARENT
                                        + ") but no parent element with extension URL "
                                        + extensionAttr.url()
                    ...
                    if (order == Child.REPLACE_PARENT) {
                        throw new ConfigurationException(
                                Msg.code(1702)
                                        ...
                                        + " has order() of REPLACE_PARENT ("
                                        + Child.REPLACE_PARENT
                                        + ") but no parent element with name "
                                        + elementName
                                        + " could be found on type "
                                        + nextField.getDeclaringClass().getSimpleName());
                    }
            ...
            if (orderMap.containsKey(order)) {
                throw new ConfigurationException(
                        Msg.code(1704)
                                + "Detected duplicate field order '"
                                + childAnnotation.order()
                                + "' for element named '"
                                + elementName
                                + "' in type '"
            ...
            if (childAnnotation.name().equals("extension")
                ...
                def =
                        new RuntimeChildExtension(
                                nextField,
                                childAnnotation.name(),
                                childAnnotation,
                 ...
                 */
                def =
                        new RuntimeChildContainedResources(
                 ...
                 */
                def =
                        new RuntimeChildDirectResource(
                                nextField, childAnnotation, descriptionAnnotation, elementName);
            } else {
                ...
                if (extensionAttr == null
                    ...
                    def =
                            new RuntimeChildChoiceDefinition(
                                    nextField,
                                    elementName,
                                    childAnnotation,
                                    descriptionAnnotation,
                    ...
                    def =
                            new RuntimeChildDeclaredExtensionDefinition(
                                    nextField,
                                    childAnnotation,
                                    descriptionAnnotation,
                                    extensionAttr,
                    ...
                    for (Class<? extends IElement> nextType : childAnnotation.type()) {
                        if (IBaseReference.class.isAssignableFrom(nextType)) {
                            refTypesList.add(
                                    myContext.getVersion().getVersion().isRi()
                                            ? IAnyResource.class
                                            : IResource.class);
                            continue;
                            ...
                            throw new ConfigurationException(
                                    Msg.code(1706)
                                            ...
                                            + nextField.getName()
                                            + "' in class '"
                                            + nextField.getDeclaringClass().getCanonicalName()
                                            + "' is of type "
                                            + BaseResourceReferenceDt.class
                                            + " but contains a non-resource type: "
                                            + nextType.getCanonicalName());
                        }
                        refTypesList.add((Class<? extends IBaseResource>) nextType);
                    }
                    def =
                            new RuntimeChildResourceDefinition(
                                    nextField,
                                    elementName,
                                    childAnnotation,
                                    descriptionAnnotation,
                    ...
                    def =
                            new RuntimeChildResourceBlockDefinition(
                                    myContext,
                                    nextField,
                                    childAnnotation,
                                    descriptionAnnotation,
                    ...
                    if (IPrimitiveType.class.isAssignableFrom(nextElementType)) {
                        if (nextElementType.equals(BoundCodeDt.class)) {
                            IValueSetEnumBinder<Enum<?>> binder =
                                    ModelScanner.getBoundCodeBinder(nextField);
                            Class<? extends Enum<?>> enumType =
                                    ModelScanner.determineEnumTypeForBoundField(nextField);
                            def =
                                    new RuntimeChildPrimitiveBoundCodeDatatypeDefinition(
                                            nextField,
                                            elementName,
                                            childAnnotation,
                                            descriptionAnnotation,
                            ...
                            Class<? extends Enum<?>> binderType =
                                    ModelScanner.determineEnumTypeForBoundField(nextField);
                            def =
                                    new RuntimeChildPrimitiveEnumerationDatatypeDefinition(
                                            nextField,
                                            elementName,
                                            childAnnotation,
                                            descriptionAnnotation,
                        ...
                        if (IBoundCodeableConcept.class.isAssignableFrom(nextElementType)) {
                            ...
                            Class<? extends Enum<?>> enumType =
                                    ModelScanner.determineEnumTypeForBoundField(nextField);
                            def =
                                    new RuntimeChildCompositeBoundDatatypeDefinition(
                                            nextField,
                                            elementName,
                                            childAnnotation,
                                            descriptionAnnotation,
                            ...
                            def =
                                    new RuntimeChildNarrativeDefinition(
                                            nextField,
                                            elementName,
                                            childAnnotation,
                                            descriptionAnnotation,
                            ...
                            def =
                                    new RuntimeChildCompositeDatatypeDefinition(
                                            nextField,
                                            elementName,
                                            childAnnotation,
                                            descriptionAnnotation,
    ...
    void sealAndInitialize(
        ...
        synchronized (myContext) {
            ...
            if (getExtensionsModifier().isEmpty() == false) {
                children.addAll(
                        findIndex(children, "modifierExtension", true), getExtensionsModifier());
            }
    ...
    private static class ScannedField {
        ...
        ScannedField(Field theField, Class<?> theClass, boolean theFirstFieldInNewClass) {
            ...
            if (childAnnotation == null) {
                ourLog.trace(
                        "Ignoring non @Child field {} on target type {}",
                        theField.getName(),
                        theClass);
                return;
            }
            if (Modifier.isFinal(theField.getModifiers())) {
                ourLog.trace(
                        "Ignoring constant {} on target type {}", theField.getName(), theClass);
                return;
            }
    ...
    private static int findIndex(
        ...
        for (ListIterator<BaseRuntimeChildDefinition> iter = theChildren.listIterator();
                iter.hasNext(); ) {
