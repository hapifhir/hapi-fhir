...
/**
 * This interface is a version-independent representation of the various functions that can be
 * provided by validation and terminology services.
 *
 * <p>This interface is invoked directly by internal parts of the HAPI FHIR API, including the
 * Validator and the FHIRPath evaluator. It is used to supply artifacts required for validation
 * (e.g. StructureDefinition resources, ValueSet resources, etc.) and also to provide terminology
 * functions such as code validation, ValueSet expansion, etc.
 *
 * <p>Implementations are not required to implement all of the functions in this interface; in fact
 * it is expected that most won't. Any methods which are not implemented may simply return <code>
 * null</code> and calling code is expected to be able to handle this. Generally, a series of
 * implementations of this interface will be joined together using the <a
 * href="https://hapifhir.io/hapi-fhir/apidocs/hapi-fhir-validation/org/hl7/fhir/common/hapi/validation/ValidationSupportChain2.html">ValidationSupportChain</a>
 * class.
 *
 * <p>See <a
 * href="https://hapifhir.io/hapi-fhir/docs/validation/validation_support_modules.html">Validation
 * Support Modules</a> for information on how to assemble and configure implementations of this
 * interface. See also the <code>org.hl7.fhir.common.hapi.validation.support</code> <a
 * href="https://hapifhir.io/hapi-fhir/apidocs/hapi-fhir-validation/org/hl7/fhir/common/hapi/validation/package-summary.html">package
 * summary</a> in the <code>hapi-fhir-validation</code> module for many implementations of this
 * interface.
 *
 * @since 5.0.0
 */
public interface IValidationSupport {
	...
	*     method. This is convenient in cases where the operation needs to make calls to other
	*     method in the support chain, so that they can be passed through the entire chain.
	*     Implementations of this interface may always safely ignore this parameter.
	* @param theExpansionOptions If provided (can be <code>null</code>), contains options
	...
	*/
	...
	default ValueSetExpansionOutcome expandValueSet(
		...
		Validate.notBlank(
					theValueSetUrlToExpand, "theValueSetUrlToExpand must not be null or blank");
		IBaseResource valueSet = fetchValueSet(theValueSetUrlToExpand);
		if (valueSet == null) {
				throw new ResourceNotFoundException(
						Msg.code(2024)
									+ "Unknown ValueSet: "
									+ UrlUtil.escapeUrlParam(theValueSetUrlToExpand));
		}
	...
	* This method may return null if it doesn't make sense for a given module.
	*/
	@Nullable
				...
				for (T next : retVal) {
					String url =
								defaultString(
										getFhirContext()
													.newTerser()
													.getSinglePrimitiveValueOrNull(next, "url"));
					if (url.startsWith("http://hl7.org/fhir/StructureDefinition/")) {
						String lastPart =
									url.substring("http://hl7.org/fhir/StructureDefinition/".length());
	...
	*
	* <p>Note: Since 5.3.0, {@literal theClass} can be {@literal null}
	*
	* @param theClass The type of the resource to load, or <code>null</code> to return any resource
	*     with the given canonical URI
	* @param theUri The resource URI
	* @return Returns the resource, or <code>null</code> if no resource with the given URI can be
	*     found
	*/
	...
	default <T extends IBaseResource> T fetchResource(@Nullable Class<T> theClass, String theUri) {
		...
		if (theClass == null) {
				Supplier<IBaseResource>[] sources =
						new Supplier[] {
	...
	* called to check codes which are found in "example" binding fields (e.g. <code>
	* Observation.code</code>) in the default profile.
	*
	* @param theValidationSupportContext The validation support module will be passed in to this
	*     method. This is convenient in cases where the operation needs to make calls to other
	*     method in the support chain, so that they can be passed through the entire chain.
	...
	* @param theDisplayLanguage to filter out the designation by the display language. To return
	*     all designation, set this value to <code>null</code>.
	*/
	@Nullable
	...
	*/
	...
	/**
	* Returns <code>true</code> if the given valueset can be validated by the given validation
	* support module
	*
	* @param theValidationSupportContext The validation support module will be passed in to this
	*     method. This is convenient in cases where the operation needs to make calls to other
	...
	*/
	...
	/**
	* This method clears any temporary caches within the validation support. It is mainly intended
	* for unit tests, but could be used in non-test scenarios as well.
	*/
	...
	class CodingConceptProperty extends BaseConceptProperty {
		...
		/**
			...
			* @param theName The name
			*/
		public CodingConceptProperty(
					String theName, String theCodeSystem, String theCode, String theDisplay) {
				super(theName);
	...
	class CodeValidationResult {
		...
		public LookupCodeResult asLookupCodeResult(
					String theSearchedForSystem, String theSearchedForCode) {
		...
		/**
			* Sets an issue severity as a string code. Value must be the name of one of the enum values
			* in {@link IssueSeverity}. Value is case-insensitive.
			*/
	...
	class LookupCodeResult {
		...
		public IBaseParameters toParameters(
				...
				if (isNotBlank(getCodeSystemDisplayName())) {
					ParametersUtil.addParameterToParametersString(
								theContext, retVal, "name", getCodeSystemDisplayName());
				}
				if (isNotBlank(getCodeSystemVersion())) {
					ParametersUtil.addParameterToParametersString(
								theContext, retVal, "version", getCodeSystemVersion());
				}
				ParametersUtil.addParameterToParametersString(
						theContext, retVal, "display", getCodeDisplay());
				ParametersUtil.addParameterToParametersBoolean(
						theContext, retVal, "abstract", isCodeIsAbstract());
				...
				if (myProperties != null) {
					...
					for (IValidationSupport.BaseConceptProperty next : myProperties) {
						...
						IBase property =
									ParametersUtil.addParameterToParameters(theContext, retVal, "property");
						ParametersUtil.addPartCode(
									theContext, property, "code", next.getPropertyName());

						if (next instanceof IValidationSupport.StringConceptProperty) {
								IValidationSupport.StringConceptProperty prop =
										(IValidationSupport.StringConceptProperty) next;
								ParametersUtil.addPartString(
										theContext, property, "value", prop.getValue());
						} else if (next instanceof IValidationSupport.CodingConceptProperty) {
								IValidationSupport.CodingConceptProperty prop =
										(IValidationSupport.CodingConceptProperty) next;
					...
					for (ConceptDesignation next : myDesignations) {
						...
						IBase property =
									ParametersUtil.addParameterToParameters(
												theContext, retVal, "designation");
						ParametersUtil.addPartCode(
									theContext, property, "language", next.getLanguage());
						ParametersUtil.addPartCoding(
									theContext,
									property,
									"use",
		...
		public static LookupCodeResult notFound(
					String theSearchedForSystem, String theSearchedForCode) {
	...
	*
	* <p>If true, validation for codings will return a positive result if all codings are valid. If
	* false, validation for codings will return a positive result if there is any coding that is
	* valid.
	*
	* @return if the application has configured validation to use logical AND, as opposed to
	*     logical OR, which is the default
	*/
