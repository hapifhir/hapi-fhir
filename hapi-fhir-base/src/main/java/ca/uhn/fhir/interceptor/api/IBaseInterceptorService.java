...
public interface IBaseInterceptorService<POINTCUT extends IPointcut>
	...
	/**
	* Register an interceptor. This method has no effect if the given interceptor is already
	* registered.
	*
	* @param theInterceptor The interceptor to register
	* @return Returns <code>true</code> if at least one valid hook method was found on this
	*     interceptor
	*/
	boolean registerInterceptor(Object theInterceptor);

	/**
	* Unregister an interceptor. This method has no effect if the given interceptor is not already
	* registered.
	*
	...
	*/
	...
	/**
	* Unregisters all interceptors that are indicated by the given callback function returning
	* <code>true</code>
	*/
	void unregisterInterceptorsIf(Predicate<Object> theShouldUnregisterFunction);

	/**
	* Unregisters all anonymous interceptors (i.e. all interceptors registered with <code>
	* registerAnonymousInterceptor</code>)
	*/
