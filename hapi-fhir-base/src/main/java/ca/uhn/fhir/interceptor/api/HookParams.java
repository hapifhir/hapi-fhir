...
public class HookParams {
    ...
    private <T> HookParams doAdd(Class<T> theType, Object theParam) {
        Validate.isTrue(
                theType.equals(Supplier.class) == false, "Can not add parameters of type Supplier");
    ...
    /**
     * Returns an unmodifiable multimap of the params, where the key is the param type and the value
    ...
    public ListMultimap<Class<?>, Object> getParamsForType() {
        ...
        myParams.entries()
                .forEach(entry -> retVal.put(entry.getKey(), unwrapValue(entry.getValue())));
