...
public abstract class BaseNarrativeGenerator implements INarrativeGenerator {
    ...
    @Nullable
    private INarrativeTemplate selectTemplate(
            FhirContext theFhirContext, IBaseResource theResource) {
        ...
        if (templates.isEmpty()) {
            ...
            if (templates.size() > 1) {
                Logs.getNarrativeGenerationTroubleshootingLog()
                        .debug(
                                "Multiple templates match for resource of type {} - Picking first"
                                        + " from: {}",
    ...
    protected List<INarrativeTemplate> getTemplateForElement(
    ...
    private boolean applyTemplate(
        ...
        List<IBase> targets =
                findElementsInResourceRequiringNarratives(theFhirContext, theResource, contextPath);
        for (IBase nextTargetContext : targets) {

            // Extract [element].text of type Narrative
            INarrative nextTargetNarrative =
                    getOrCreateNarrativeChildElement(theFhirContext, nextTargetContext);
    ...
    private INarrative getOrCreateNarrativeChildElement(
            FhirContext theFhirContext, IBase nextTargetContext) {
        BaseRuntimeElementCompositeDefinition<?> targetElementDef =
    ...
    private boolean templateDoesntApplyToResource(
            INarrativeTemplate theTemplate, IBaseResource theResource) {
        boolean retVal = false;
        if (theTemplate.getAppliesToProfiles() != null
                && !theTemplate.getAppliesToProfiles().isEmpty()) {
            Set<String> resourceProfiles =
                    ...
                    if (i + 3 < theResult.length()) {
                        ...
                        char char4 =
                                Character.toLowerCase(
                                        (i + 4 < theResult.length())
                                                ? theResult.charAt(i + 4)
                                                : ' ');
