...
public class NarrativeTemplateManifest implements INarrativeTemplateManifest {
    ...
    public List<INarrativeTemplate> getTemplateByResourceName(
        ...
        return getFromMap(
                theStyles, theResourceName.toUpperCase(), myResourceTypeToTemplate, theProfiles);
    }
    ...
    public List<INarrativeTemplate> getTemplateByFragmentName(
        ...
        return getFromMap(
                theStyles, theFragmentName, myFragmentNameToTemplate, Collections.emptyList());
    }
        ...
        if (retVal.isEmpty()) {
            ...
            retVal =
                    getFromMap(
                            ...
                            myDatatypeToTemplate,
                            Collections.emptyList());
        }
        return retVal;
    }

    public static NarrativeTemplateManifest forManifestFileLocation(
            String... thePropertyFilePaths) {
        return forManifestFileLocation(Arrays.asList(thePropertyFilePaths));
    }

    public static NarrativeTemplateManifest forManifestFileLocation(
            Collection<String> thePropertyFilePaths) {
    ...
    public static NarrativeTemplateManifest forManifestFileContents(
            Collection<String> theResources) {
        try {
    ...
    private static Collection<NarrativeTemplate> loadProperties(String theManifestText)
            throws IOException {
        ...
        for (Object nextKeyObj : file.keySet()) {
            ...
            Validate.isTrue(
                    StringUtils.countMatches(nextKey, ".") == 1,
                    "Invalid narrative property file key: %s",
                    nextKey);
            ...
            NarrativeTemplate nextTemplate =
                    nameToTemplate.computeIfAbsent(
                            name, t -> new NarrativeTemplate().setTemplateName(name));

            if (nextKey.endsWith(".class")) {
                String className = file.getProperty(nextKey);
                if (isNotBlank(className)) {
                    try {
                        nextTemplate.addAppliesToClass(
                                (Class<? extends IBase>) Class.forName(className));
                ...
                throw new ConfigurationException(
                        Msg.code(1868)
                                ...
                                + " '.profile', '.resourceType', '.dataType', '.style',"
                                + " '.contextPath', '.narrative', '.title'");
            }
        }
    ...
    private static void parseValuesAndAddToMap(
            String resourceType, Consumer<String> addAppliesToResourceType) {
    ...
    static String loadResource(String theName) {
        if (theName.startsWith("classpath:")) {
            ...
            if (file.exists() == false) {
                throw new InternalErrorException(
                        Msg.code(1870) + "File not found: " + file.getAbsolutePath());
            }
            ...
            throw new InternalErrorException(
                    Msg.code(1871)
                            ...
                            + theName
                            + "' (must start with classpath: or file: )");
        }
    }
    ...
    private static <T> List<INarrativeTemplate> getFromMap(
        ...
        return theMap.get(theKey).stream()
                ...
                .filter(
                        t ->
                                theProfiles.isEmpty()
                                        || t.getAppliesToProfiles().stream()
                                                .anyMatch(theProfiles::contains))
                .collect(Collectors.toList());
    }
}
