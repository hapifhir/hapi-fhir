...
public abstract class BaseResourceReferenceDt extends BaseIdentifiableElement
        implements IBaseDatatype, IBaseReference {

    private static final long serialVersionUID = 1L;

    private static final org.slf4j.Logger ourLog =
            org.slf4j.LoggerFactory.getLogger(BaseResourceReferenceDt.class);
     ...
     * method will return the resource instance only if it has previously been loaded using {@link
     * #loadResource(IRestfulClient)} or it was contained within the resource containing this
     * resource.
     *
     * <p>See the FHIR specification section on <a
     * href="http://www.hl7.org/implement/standards/fhir/references.html#id">contained resources</a>
     ...
     * method invokes the HTTP client to retrieve the resource unless it has already been loaded, or
     * was a contained resource in which case it is simply returned.
     */
    public IBaseResource loadResource(IRestfulClient theClient) {
        ...
        if (resourceId == null || isBlank(resourceId.getValue())) {
            throw new IllegalStateException(
                    Msg.code(1905) + "Reference has no resource ID defined");
        }
        if (isBlank(resourceId.getBaseUrl()) || isBlank(resourceId.getResourceType())) {
            throw new IllegalStateException(
                    Msg.code(1906)
                            + "Reference is not complete (must be in the form [baseUrl]/[resource"
                            + " type]/[resource ID]) - Reference is: "
                            + resourceId.getValue());
        }
    ...
    public String toString() {
        org.apache.commons.lang3.builder.ToStringBuilder b =
