 ...
 * representation, and uses a String instead. If you need to work with XML as StAX events, you can
 * use the {@link XmlUtil#parse(String)} and {@link XmlUtil#encode(List)} methods to do so.
 */
...
public class XhtmlDt extends BasePrimitive<String> {
	...
	*/
	...
	/**
	* Note that as of HAPI FHIR 3.1.0, this method no longer uses the StAX XMLEvent type as the XML
	* representation, and uses a String instead. If you need to work with XML as StAX events, you
	* can use the {@link XmlUtil#parse(String)} and {@link XmlUtil#encode(List)} methods to do so.
	*/
	@Override
	...
	*
	* <p><b>Formatting note:</b> The text will be trimmed {@link String#trim()}. If the text does
	* not start with an HTML tag (generally this would be a div tag), a div tag will be
	* automatically placed surrounding the text.
	*
	* <p>Also note that if the parsed text contains any entities (&amp;foo;) which are not a part
	* of the entities defined in core XML (e.g. &amp;sect; which is valid in XHTML 1.0 but not in
	* XML 1.0) they will be parsed and converted to their equivalent unicode character.
	*/
	@Override
				...
				if (firstTagEnd != -1) {
					if (firstSlash > firstTagEnd) {
						...
						if (!firstTag.contains(" xmlns")) {
								value =
										value.substring(0, firstTagEnd)
													+ DECL_XMLNS
													+ value.substring(firstTagEnd);
						}
					}
				}
		}
