 ...
 *
 * <p>Note on equality: This class uses {@link #getValue() value} and the {@link #isRecurse()
 * recurse} properties to test equality. Prior to HAPI 1.2 (and FHIR DSTU2) the recurse property did
 * not exist, so this may merit consideration when upgrading servers.
 *
 * <p>Note on thread safety: This class is not thread safe.
 */
public class Include implements Serializable {
	...
	*
	* @param theValue The <code>_include</code> value, e.g. "Patient:name"
	*/
	...
	*
	* @param theValue The <code>_include</code> value, e.g. "Patient:name"
	* @param theIterate Should the include recurse
	*/
	...
	*/
	...
	/**
	* Returns the portion of the string after the second colon, or null if there are not two colons
	* in the value.
	*/
	...
	*/
	...
	public void setValue(String theValue) {
		if (myImmutable) {
				throw new IllegalStateException(
						Msg.code(1888) + "Can not change the value of this include");
		}
	...
	/**
	* Creates and returns a new copy of this Include with the given type. The following table shows
	...
	* @return A new copy of the include. Note that if this include is {@link #toLocked() locked},
	...
	*/
	public Include withType(String theResourceType) {
		...
		if (isBlank(paramType) || isBlank(paramName)) {
				throw new IllegalStateException(
						Msg.code(1889)
									+ "This include does not contain a value in the format"
