...
public abstract class BaseDateTimeDt extends BasePrimitive<Date> {
    ...
    public BaseDateTimeDt(Date theDate, TemporalPrecisionEnum thePrecision) {
        ...
        if (isPrecisionAllowed(thePrecision) == false) {
            throw new DataFormatException(
                    Msg.code(1880)
                            + "Invalid date/time string (datatype "
     ...
     *
     * <p>Note that this method returns the same value as {@link #getNanos()} but with less
     * precision.
     */
     ...
     *
     * <p>Note that this method returns the same value as {@link #getMillis()} but with more
     * precision.
     */
    ...
    /**
     * Returns the TimeZone associated with this dateTime's value. May return <code>null</code> if
     * no timezone was supplied.
     */
    ...
    /**
     * To be implemented by subclasses to indicate whether the given precision is allowed by this
     * type
     */
    ...
    protected Date parse(String theValue) throws DataFormatException {
        ...
        if (length > 4) {
            ...
            if (length > 7) {
                ...
                cal.set(
                        Calendar.DATE,
                        1); // for some reason getActualMaximum works incorrectly if date isn't set
                int actualMaximum = cal.getActualMaximum(Calendar.DAY_OF_MONTH);
                cal.set(
                        Calendar.DAY_OF_MONTH,
                        parseInt(value, value.substring(8, 10), 1, actualMaximum));
    ...
    private int parseInt(
            String theValue, String theSubstring, int theLowerBound, int theUpperBound) {
        int retVal = 0;
        try {
     ...
     *
     * <p>Note that this method sets the same value as {@link #setNanos(long)} but with less
     * precision.
     */
     ...
     *
     * <p>Note that this method sets the same value as {@link #setMillis(int)} but with more
     * precision.
     */
     ...
     */
    public BaseDateTimeDt setPrecision(TemporalPrecisionEnum thePrecision)
            throws DataFormatException {
    ...
    private BaseDateTimeDt setTimeZone(String theWholeValue, String theValue) {
        ...
        if (isBlank(theValue)) {
            ...
            throwBadDateFormat(
                    theWholeValue,
                    "Timezone offset must be in the form \"Z\", \"-HH:mm\", or \"+HH:mm\"");
     ...
     * default precision for this datatype (unless the precision is already set), as well as the
     * local timezone as determined by the local operating system. Both of these properties may be
    ...
    /**
     * Sets the value for this type using the given Java Date object as the time, and using the
     * specified precision, as well as the local timezone as determined by the local operating
     ...
     * @throws DataFormatException
     */
    public void setValue(Date theValue, TemporalPrecisionEnum thePrecision)
            throws DataFormatException {
        if (getTimeZone() == null) {
    ...
    private void throwBadDateFormat(String theValue) {
        throw new DataFormatException(
                Msg.code(1882) + "Invalid date/time format: \"" + theValue + "\"");
    }
     ...
     *
     * <p><b>Note on time zones:</b> This method renders the value using the time zone that is
     * contained within the value. For example, if this date object contains the value
     * "2012-01-05T12:00:00-08:00", the human display will be rendered as "12:00:00" even if the
     * application is being executed on a system in a different time zone. If this behaviour is not
     * what you want, use {@link #toHumanDisplayLocalTimezone()} instead.
     */
    ...
    /**
     * Returns a human readable version of this date/time using the system local format, converted
     * to the local timezone if neccesary.
     *
     * @see #toHumanDisplay() for a method which does not convert the time to the local timezone
     *     before rendering it.
     */
        ...
        if (theValue < theMinimum || theValue > theMaximum) {
            throw new IllegalArgumentException(
                    Msg.code(1884)
                            + "Value "
                            + theValue
                            + " is not between allowable range: "
                            + theMinimum
                            + " - "
                            + theMaximum);
        }
    ...
    private void validatePrecisionAndThrowDataFormatException(
            String theValue, TemporalPrecisionEnum thePrecision) {
        if (isPrecisionAllowed(thePrecision) == false) {
            throw new DataFormatException(
                    Msg.code(1885)
                            + "Invalid date/time string (datatype "
