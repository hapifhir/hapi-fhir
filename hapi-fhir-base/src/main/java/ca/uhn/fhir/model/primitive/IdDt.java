...
/**
 * Represents the FHIR ID type. This is the actual resource ID, meaning the ID that will be used in
 * RESTful URLs, Resource References, etc. to represent a specific instance of a resource.
 *
 * <p>
 *
 * <p><b>Description</b>: A whole number in the range 0 to 2^64-1 (optionally represented in hex), a
 * uuid, an oid, or any other combination of lowercase letters, numerals, "-" and ".", with a length
 * limit of 36 characters.
 *
 * <p>regex: [a-z-Z0-9\-\.]{1,36}
 */
...
public class IdDt extends UriDt implements /*IPrimitiveDatatype<String>, */ IIdType {
	...
	/**
	* Create a new ID, using a BigDecimal input. Uses {@link BigDecimal#toPlainString()} to
	...
	*/
	...
	/**
	* Create a new ID using a string. This String may contain a simple ID (e.g. "1234") or it may
	* contain a complete URL (http://example.com/fhir/Patient/1234).
	*
	* <p>
	*
	* <p><b>Description</b>: A whole number in the range 0 to 2^64-1 (optionally represented in
	* hex), a uuid, an oid, or any other combination of lowercase letters, numerals, "-" and ".",
	...
	*/
	...
	public IdDt(IdDt theIdDt) {
		this(
					theIdDt.myBaseUrl,
					theIdDt.myResourceType,
					theIdDt.myUnqualifiedId,
					theIdDt.myUnqualifiedVersionId);
	}
	...
	public void applyTo(IBaseResource theResouce) {
		if (theResouce == null) {
				...
				throw new IllegalArgumentException(
						Msg.code(1876)
									+ "Unknown resource class type, does not implement IResource or extend"
									+ " Resource");
		}
	}

	/**
	* @deprecated Use {@link #getIdPartAsBigDecimal()} instead (this method was deprocated because
	...
	*/
	...
	/**
	* Returns true if this IdDt matches the given IdDt in terms of resource type and ID, but
	* ignores the URL base
	*/
	...
	/**
	* Returns the portion of this resource ID which corresponds to the server base URL. For example
	* given the resource ID <code>http://example.com/fhir/Patient/123</code> the base URL would be
	* <code>http://example.com/fhir</code>.
	*
	* <p>This method may return null if the ID contains no base (e.g. "Patient/123")
	*/
	@Override
	...
	*/
	...
	/**
	* Returns the unqualified portion of this ID as a big decimal, or <code>null</code> if the
	* value is null
	*
	...
	*/
	...
	/**
	* Returns the unqualified portion of this ID as a {@link Long}, or <code>null</code> if the
	* value is null
	*
				...
				if (idIndex <= 0) {
					...
					if (typeIndex == -1) {
						...
						if (typeIndex >= idIndex) {
								...
								// 'foo' was the id but we're making that the resource type. Nullify the id
								// part because we don't have an id.
								// Also set null value to the super.setValue() and enable
								// myHaveComponentParts so it forces getValue() to properly
								// recreate the url
	...
	*/
	...
	/**
	* Returns <code>true</code> if this ID contains an absolute URL (in other words, a URL starting
	* with "http://" or "https://"
	*/
	@Override
	...
	/**
	* Returns <code>true</code> if the ID is a local reference (in other words, it begins with the
	* '#' character)
	*/
	@Override
	...
	/**
	* Copies the value from the given IdDt to <code>this</code> IdDt. It is generally not neccesary
	* to use this method but it is provided for consistency with the rest of the API.
	*
	* @deprecated
	*/
	...
	@Override
	public IIdType setParts(
				String theBaseUrl, String theResourceType, String theIdPart, String theVersionIdPart) {
		if (isNotBlank(theVersionIdPart)) {
				Validate.notBlank(
						theResourceType,
						"If theVersionIdPart is populated, theResourceType and theIdPart must be"
									+ " populated");
				Validate.notBlank(
						theIdPart,
						"If theVersionIdPart is populated, theResourceType and theIdPart must be"
		...
		if (isNotBlank(theBaseUrl) && isNotBlank(theIdPart)) {
				Validate.notBlank(
						theResourceType,
						"If theBaseUrl is populated and theIdPart is populated, theResourceType must be"
									+ " populated");
		}
	...
	/**
	* Returns a new IdDt containing this IdDt's values but with no server base URL if one is
	* present in this IdDt. For example, if this IdDt contains the ID "http://foo/Patient/1", this
	* method will return a new IdDt containing ID "Patient/1".
	*/
	@Override
	...
	* (which will only be used if the ID does not already contain those respective parts).
	* Essentially, because IdDt can contain either a complete URL or a partial one (or even jut a
	* simple ID), this method may be used to translate into a complete URL.
	*
	...
	*/
	...
	/**
	* Creates a new instance of this ID which is identical, but refers to the specific version of
	* this resource ID noted by theVersion.
	*
	* @param theVersion The actual version string, e.g. "1". If theVersion is blank or null,
	*     returns the same as {@link #toVersionless()}}
	* @return A new instance of IdDt which is identical, but refers to the specific version of this
	*     resource ID noted by theVersion.
	*/
	@Override
	...
	/**
	* Construct a new ID with with form "urn:uuid:[UUID]" where [UUID] is a new, randomly created
