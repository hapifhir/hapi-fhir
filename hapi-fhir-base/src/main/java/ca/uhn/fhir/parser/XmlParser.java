...
/**
 * This class is the FHIR XML parser/encoder. Users should not interact with this class directly,
 * but should use {@link FhirContext#newXmlParser()} to get an instance.
 */
public class XmlParser extends BaseParser {

    static final String FHIR_NS = "http://hl7.org/fhir";
    private static final org.slf4j.Logger ourLog =
            org.slf4j.LoggerFactory.getLogger(XmlParser.class);
    private boolean myPrettyPrint;

    /**
     * Do not use this constructor, the recommended way to obtain a new instance of the XML parser
     * is to invoke {@link FhirContext#newXmlParser()}.
     *
    ...
    private XMLEventReader createStreamReader(Reader theReader) {
        try {
            ...
            throw new ConfigurationException(
                    Msg.code(1848) + "Failed to initialize STaX event factory", e1);
    ...
    @Override
    public void doEncodeResourceToWriter(
        ...
        try {
            ...
            throw new ConfigurationException(
                    Msg.code(1850) + "Failed to initialize STaX event factory", e);
        }
    }
            ...
            while (streamReader.hasNext()) {
                ...
                try {
                        ...
                        case XMLStreamConstants.START_ELEMENT:
                            {
                                ...
                                if ("extension".equals(elem.getName().getLocalPart())) {
                                    ...
                                    if (urlAttr == null || isBlank(urlAttr.getValue())) {
                                        getErrorHandler()
                                                .missingRequiredElement(
                                                        new ParseLocation()
                                                                .setParentElementName("extension"),
                                                        "url");
                                        url = null;
                                    } else {
                                    ...
                                    parserState.enteringNewElementExtension(
                                            elem, url, false, getServerBaseUrl());
                                } else if ("modifierExtension"
                                        .equals(elem.getName().getLocalPart())) {
                                    Attribute urlAttr = elem.getAttributeByName(new QName("url"));
                                    String url;
                                    if (urlAttr == null || isBlank(urlAttr.getValue())) {
                                        getErrorHandler()
                                    ...
                                    parserState.enteringNewElementExtension(
                                            elem, url, true, getServerBaseUrl());
                                } else {
                                ...
                                for (Iterator<Attribute> attributes = elem.getAttributes();
                                        attributes.hasNext(); ) {
                    ...
                    throw new DataFormatException(
                            Msg.code(1851)
                                    + "DataFormatException at ["
                                    + nextEvent.getLocation().toString()
                                    + "]: "
                                    + e.getMessage(),
                            e);
                }
                ...
                case RESOURCE:
                    {
                        ...
                        encodeResourceToXmlStreamWriter(
                                resource, theEventWriter, theIncludedResource, theEncodeContext);
                ...
                case UNDECL_EXT:
                    {
                        throw new IllegalStateException(
                                Msg.code(1853) + "state should not happen: " + childDef.getName());
                    }
            }
    ...
    private void encodeResourceToXmlStreamWriter(
        ...
        encodeResourceToXmlStreamWriter(
                theResource, theEventWriter, theIncludedResource, resourceId, theEncodeContext);
    }
    ...
    private void encodeResourceToXmlStreamWriter(
        ...
        if (resDef == null) {
            throw new ConfigurationException(
                    Msg.code(1854) + "Unknown resource type: " + theResource.getClass());
        }
        ...
        if (theResource instanceof IAnyResource) {
            ...
            if (super.shouldEncodeResourceMeta(resource)
                    && ElementUtil.isEmpty(versionIdPart, updated, securityLabels, tags, profiles)
                            == false) {
                ...
                if (updated != null) {
                    if (shouldEncodePath(resource, "meta.lastUpdated")) {
                        writeOptionalTagWithValue(
                                theEventWriter, "lastUpdated", updated.getValueAsString());
                    }
                }
    ...
    private void encodeUndeclaredExtensions(
        ...
        for (IBaseExtension<?, ?> next : theExtensions) {
            if (next == null
                    || (ElementUtil.isEmpty(next.getValue()) && next.getExtension().isEmpty())) {
                continue;
            }
            ...
            // child extensions
            encodeExtensionsIfPresent(
                    theResource, theEventWriter, next, theIncludedResource, theEncodeContext);
                ...
                case XMLStreamConstants.START_ELEMENT:
                    ...
                    if (firstElement) {
                        if (StringUtils.isBlank(se.getName().getPrefix())) {
                            ...
                            theEventWriter.writeStartElement(
                                    prefix, se.getName().getLocalPart(), namespaceURI);
                    ...
                    for (Iterator<?> attrIter = se.getAttributes(); attrIter.hasNext(); ) {
                        ...
                        if (isBlank(next.getName().getNamespaceURI())) {
                            ...
                            theEventWriter.writeAttribute(
                                    next.getName().getPrefix(),
                                    next.getName().getNamespaceURI(),
                                    next.getName().getLocalPart(),
                                    next.getValue());
                        }
                    }
                    break;
    ...
    private <T extends IBaseResource> T parseResource(
            Class<T> theResourceType, XMLEventReader theStreamReader) {
    ...
    /**
     * This is just to work around the fact that casting
     * java.util.List<ca.uhn.fhir.model.api.ExtensionDt> to java.util.List<? extends
     * org.hl7.fhir.instance.model.api.IBaseExtension<?, ?>> seems to be rejected by the compiler
     * some of the time.
     */
    private <Q extends IBaseExtension<?, ?>> List<IBaseExtension<?, ?>> toBaseExtensionList(
            final List<Q> theList) {
        List<IBaseExtension<?, ?>> retVal = new ArrayList<IBaseExtension<?, ?>>(theList.size());
        retVal.addAll(theList);
        return retVal;
    }

    private void writeCommentsPost(XMLStreamWriter theEventWriter, IBase theElement)
            throws XMLStreamException {
    ...
    private void writeCommentsPre(XMLStreamWriter theEventWriter, IBase theElement)
            throws XMLStreamException {
        if (theElement != null && theElement.hasFormatComment()) {
    ...
    private void writeOptionalTagWithValue(
            XMLStreamWriter theEventWriter, String theName, String theValue)
            throws XMLStreamException {
