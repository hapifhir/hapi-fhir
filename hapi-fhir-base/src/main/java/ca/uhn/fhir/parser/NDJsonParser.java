 ...
 * but should use {@link FhirContext#newNDJsonParser()} to get an instance.
 */
public class NDJsonParser extends BaseParser {

    private static final org.slf4j.Logger ourLog =
            org.slf4j.LoggerFactory.getLogger(NDJsonParser.class);

    private IParser myJsonParser;
    private FhirContext myFhirContext;

    /**
     * Do not use this constructor, the recommended way to obtain a new instance of the NDJSON
     * parser is to invoke {@link FhirContext#newNDJsonParser()}.
     *
    ...
    protected void doEncodeResourceToWriter(
        ...
        if (!(IBaseBundle.class.isAssignableFrom(theResource.getClass()))) {
            throw new IllegalArgumentException(
                    Msg.code(1833)
                            + "NDJsonParser can only encode Bundle types.  Received "
                            + theResource.getClass().getName());
        }

        // Ok, convert the bundle to a list of resources.
        List<IBaseResource> theBundleResources =
                BundleUtil.toListOfResources(myFhirContext, (IBaseBundle) theResource);
    ...
    public <T extends IBaseResource> T doParseResource(Class<T> theResourceType, Reader theReader)
        ...
        if ((theResourceType != null) && (!(IBaseBundle.class.isAssignableFrom(theResourceType)))) {
            throw new DataFormatException(
                    Msg.code(1834)
                            + "NDJsonParser can only parse to Bundle types.  Received "
