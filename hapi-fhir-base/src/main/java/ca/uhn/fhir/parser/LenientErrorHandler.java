 ...
 * exceptions:
 *
 * <p>The {@link #invalidValue(ca.uhn.fhir.parser.IParserErrorHandler.IParseLocation, String,
 * String)} method will throw a {@link DataFormatException} by default since ignoring this type of
 * error can lead to data loss (since invalid values are silently ignored). See {@link
 ...
 *     <p>The {@link
 *     #extensionContainsValueAndNestedExtensions(ca.uhn.fhir.parser.IParserErrorHandler.IParseLocation)}
 *     method will throw a {@link DataFormatException} by default since ignoring this type of error
 *     will allow malformed resouces to be created and result in errors when attempts to read,
 *     update or delete the resource in the future. See {@link #setErrorOnInvalidExtension(boolean)}
 *     for information on this.
 */
public class LenientErrorHandler extends ParseErrorHandler implements IParserErrorHandler {

    private static final org.slf4j.Logger ourLog =
            org.slf4j.LoggerFactory.getLogger(LenientErrorHandler.class);
     ...
     * default, invalid attribute values cause this error handler to throw a {@link
     * DataFormatException} (unlike other methods in this class which default to simply logging
     * errors).
     *
     * <p>Note that empty values (e.g. <code>""</code>) will not lead to an error when this is set
     * to <code>true</code>, only invalid values (e.g. a gender code of <code>foo</code>)
     *
    ...
    /**
     * If set to <code>false</code> (default is <code>true</code>) invalid extensions will be
     * logged. By default, invalid resource extensions cause this error handler to throw a {@link
     * DataFormatException} (unlike other methods in this class which default to simply logging
     * errors).
     *
    ...
    /**
     * If this method is called, both invalid resource extensions and invalid attribute values will
     * set to simply logging errors.
     */
    ...
    public void unknownAttribute(IParseLocation theLocation, String theElementName) {
        if (myLogErrors) {
            ourLog.warn(
                    "{}Unknown attribute '{}' found while parsing",
                    describeLocation(theLocation),
                    theElementName);
        }
    }
    ...
    public void unknownElement(IParseLocation theLocation, String theElementName) {
        if (myLogErrors) {
            ourLog.warn(
                    "{}Unknown element '{}' found while parsing",
                    describeLocation(theLocation),
                    theElementName);
        }
    }
    ...
    public void unknownReference(IParseLocation theLocation, String theReference) {
        if (myLogErrors) {
            ourLog.warn(
                    "{}Resource has invalid reference: {}",
                    describeLocation(theLocation),
                    theReference);
        }
    }
