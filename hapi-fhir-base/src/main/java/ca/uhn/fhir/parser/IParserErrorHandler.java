...
public interface IParserErrorHandler {
    ...
    /**
     * Invoked when a contained resource is parsed that has no ID specified (and is therefore
     * invalid)
     *
     * @param theLocation The location in the document. WILL ALWAYS BE NULL currently, as this is
     *     not yet implemented, but this parameter is included so that locations can be added in the
    ...
    /**
     * Invoked if the wrong type of element is found while parsing JSON. For example if a given
     * element is expected to be a JSON Object and is a JSON array
     *
     * @param theLocation The location in the document. Note that this may be <code>null</code> as
     *     the ParseLocation feature is experimental. Use with caution, as the API may change.
     ...
     * @param theExpectedScalarType If theExpectedValueType is {@link ValueType#SCALAR}, this is the
     *     specific scalar type expected. Otherwise this parameter will be null.
     * @param theFoundValueType The datatype that was found at this location
     * @param theFoundScalarType If theFoundValueType is {@link ValueType#SCALAR}, this is the
     *     specific scalar type found. Otherwise this parameter will be null.
     * @since 2.2
     */
    ...
    /**
     * The parser detected an attribute value that was invalid (such as: empty "" values are not
     * permitted)
     *
     * @param theLocation The location in the document. Note that this may be <code>null</code> as
     *     the ParseLocation feature is experimental. Use with caution, as the API may change.
    ...
    /**
     * Invoked when an element repetition (e.g. a second repetition of something) is found for a
     * field which is non-repeating.
     *
     * @param theLocation The location in the document. Note that this may be <code>null</code> as
     *     the ParseLocation feature is experimental. Use with caution, as the API may change.
     * @param theElementName The name of the element that was found.
     ...
     * because it is a local reference to an unknown contained resource)
     *
     * @param theLocation The location in the document. Note that this may be <code>null</code> as
     *     the ParseLocation feature is experimental. Use with caution, as the API may change.
     ...
     *
     * @param theLoc The location in the document. Note that this may be <code>null</code> as the
     *     ParseLocation feature is experimental. Use with caution, as the API may change.
     */
    void extensionContainsValueAndNestedExtensions(IParseLocation theLocation);

    /**
     * For now this is an empty interface. Error handling methods include a parameter of this type
     * which will currently always be set to null. This interface is included here so that locations
     * can be added to the API in a future release without changing the API.
     */
    interface IParseLocation {

        /**
         * Returns the name of the parent element (the element containing the element currently
         * being parsed)
         *
         * @since 2.1
         */
