...
/**
 * This class is the FHIR JSON parser/encoder. Users should not interact with this class directly,
 * but should use {@link FhirContext#newJsonParser()} to get an instance.
 */
public class JsonParser extends BaseParser implements IJsonLikeParser {

    private static final org.slf4j.Logger ourLog =
            org.slf4j.LoggerFactory.getLogger(JsonParser.HeldExtension.class);
    ...
    /**
     * Do not use this constructor, the recommended way to obtain a new instance of the JSON parser
     * is to invoke {@link FhirContext#newJsonParser()}.
     *
                ...
                if (encodeExtension == null) {
                    encodeExtension =
                            isEncodeExtension(
                                    ...
                                    theContainingElement);
                }

                if (encodeExtension) {
                    HeldExtension extension =
                            new HeldExtension(next, theIsModifier, theChildElem, theParent);
    ...
    // private void assertObjectOfType(JsonLikeValue theResourceTypeObj, Object theValueType, String
    // thePosition) {
    // if (theResourceTypeObj == null) {
    // throw new DataFormatException(Msg.code(1836) + "Invalid JSON content detected, missing
    // required element: '" + thePosition + "'");
    // }
    //
    // if (theResourceTypeObj.getValueType() != theValueType) {
    // throw new DataFormatException(Msg.code(1837) + "Invalid content of element " + thePosition +
    // ", expected " + theValueType);
    // }
    // }

    private void beginArray(BaseJsonLikeWriter theEventWriter, String arrayName)
            throws IOException {
        theEventWriter.beginArray(arrayName);
    }

    private void beginObject(BaseJsonLikeWriter theEventWriter, String arrayName)
            throws IOException {
    ...
    public <T extends IBaseResource> T doParseResource(
            Class<T> theResourceType, JsonLikeStructure theJsonStructure) {
        ...
        if (resourceTypeObj == null
                || !resourceTypeObj.isString()
                || isBlank(resourceTypeObj.getAsString())) {
        ...
        ParserState<? extends IBaseResource> state =
                ParserState.getPreResourceInstance(
                        this, theResourceType, getContext(), true, getErrorHandler());
        ...
        switch (theChildDef.getChildType()) {
            case ID_DATATYPE:
                {
                    ...
                    String encodedValue =
                            "id".equals(theChildName) ? value.getIdPart() : value.getValue();
                    ...
                    if (value instanceof IBaseIntegerDatatype) {
                        if (theChildName != null) {
                            write(
                                    theEventWriter,
                                    theChildName,
                                    ((IBaseIntegerDatatype) value).getValue());
                        } else {
                        ...
                        decimalValue =
                                new BigDecimal(decimalValue.toString()) {
                        ...
                        if (theChildName != null) {
                            write(
                                    theEventWriter,
                                    theChildName,
                                    ((IBaseBooleanDatatype) value).getValue());
                        } else {
            ...
            case CONTAINED_RESOURCES:
                {
                    List<IBaseResource> containedResources =
                            getContainedResources().getContainedResources();
            ...
            default:
                throw new IllegalStateException(
                        Msg.code(1839)
                                + "Should not have this state here: "
    ...
    private void encodeCompositeElementChildrenToStreamWriter(
        ...
        Iterable<CompositeChildElement> compositeChildElements =
                super.compositeChildIterator(
                        theElement, theContainedResource, theParent, theEncodeContext);
        for (CompositeChildElement nextChildElem : compositeChildElements) {
            ...
            if (nextChild instanceof RuntimeChildNarrativeDefinition) {
                ...
                if (gen != null) {
                    ...
                    if (narr != null && narr.isEmpty()) {
                        ...
                        if (!narr.isEmpty()) {
                            RuntimeChildNarrativeDefinition child =
                                    (RuntimeChildNarrativeDefinition) nextChild;
                            String childName =
                                    nextChild.getChildNameByDatatype(child.getDatatype());
            ...
            values =
                    preProcessValues(
                            nextChild, theResource, values, nextChildElem, theEncodeContext);
            ...
            for (IBase nextValue : values) {
                ...
                BaseParser.ChildNameAndDef childNameAndDef =
                        super.getChildNameAndDef(nextChild, nextValue);
                ...
                if (primitive) {
                    if (nextValue instanceof ISupportsUndeclaredExtensions) {
                        List<ExtensionDt> ext =
                                ((ISupportsUndeclaredExtensions) nextValue)
                        ...
                        if (nextValue instanceof IBaseHasModifierExtensions) {
                            IBaseHasModifierExtensions element =
                                    (IBaseHasModifierExtensions) nextValue;
                            List<? extends IBaseExtension<?, ?>> ext =
                                    element.getModifierExtension();
                            force |=
                                    addToHeldExtensions(
                                            valueIdx,
                                            ext,
                    ...
                    if (nextValue.hasFormatComment()) {
                        force |=
                                addToHeldComments(
                                        valueIdx, nextValue.getFormatCommentsPre(), comments);
                        force |=
                                addToHeldComments(
                                        valueIdx, nextValue.getFormatCommentsPost(), comments);
                    }
                ...
                if (currentChildName == null || !currentChildName.equals(nextChildSpecificName)) {
                    ...
                    BaseRuntimeChildDefinition replacedParentDefinition =
                            nextChild.getReplacedParentDefinition();
    ...
    @Override
    public void encodeResourceToJsonLikeWriter(
        ...
        if (theResource.getStructureFhirVersionEnum() != getContext().getVersion().getVersion()) {
            throw new IllegalArgumentException(
                    Msg.code(1840)
                            + "This parser is for FHIR version "
                            + getContext().getVersion().getVersion()
                            + " - Can not encode a structure for version "
    ...
    private void parseMetaForDSTU2(
        ...
        List<Map.Entry<ResourceMetadataKeyEnum<?>, Object>> extensionMetadataKeys =
                getExtensionMetadataKeys(resource);

        if (super.shouldEncodeResourceMeta(resource)
                        && (ElementUtil.isEmpty(
                                        versionIdPart, updated, securityLabels, tags, profiles)
                                == false)
            ...
            if (tags != null && tags.isEmpty() == false) {
                ...
                for (Tag tag : tags) {
                    ...
                    // wipmb should we be writing the new properties here?  There must be another
                    // path.
                    theEventWriter.endObject();
                }
                theEventWriter.endArray();
    ...
    /**
     * This is useful only for the two cases where extensions are encoded as direct children (e.g.
     * not in some object called _name): resource extensions, and extension extensions
     */
    private void extractAndWriteExtensionsAsDirectChild(
        ...
        if (theElementDef != null) {
            extractDeclaredExtensions(
                    theElement, theElementDef, extensions, modifierExtensions, theChildElem);
        }
        ...
        if (theElement instanceof ISupportsUndeclaredExtensions) {
            ...
            if (theElement instanceof IBaseHasExtensions) {
                ...
                for (IBaseExtension<?, ?> next : ext) {
                    ...
                    if (encodeExtension == null) {
                        encodeExtension =
                                isEncodeExtension(
                                        theParent, theEncodeContext, theContainedResource, element);
                    }
                    if (encodeExtension) {
                        HeldExtension extension =
                                new HeldExtension(next, false, theChildElem, theParent);
            ...
            if (theElement instanceof IBaseHasModifierExtensions) {
                ...
                for (IBaseExtension<?, ?> next : ext) {
                    ...
                    HeldExtension extension =
                            new HeldExtension(next, true, theChildElem, theParent);
    ...
    private boolean isEncodeExtension(
        ...
        if (runtimeElementDefinition instanceof BaseRuntimeElementCompositeDefinition) {
            ...
            CompositeChildElement c =
                    new CompositeChildElement(theParent, childDef, theEncodeContext);
    ...
    private BaseJsonLikeArray grabJsonArray(
            BaseJsonLikeObject theObject, String nextName, String thePosition) {
    ...
    private void parseAlternates(
        ...
        if (theAlternateVal.isArray()) {
            ...
            if (array.size() > 1) {
                throw new DataFormatException(
                        Msg.code(1842)
                                + "Unexpected array of length "
                                + array.size()
                                + " (expected 0 or 1) for element: "
                                + theElementName);
            }
            if (array.size() == 0) {
                return;
    ...
    private void parseChildren(
        ...
        if (theJsonVal.isArray()) {
            ...
            BaseJsonLikeArray nextAlternateArray =
                    BaseJsonLikeValue.asArray(alternateVal); // could be null
            ...
            if (!theInArray && theState.elementIsRepeating(theName)) {
                getErrorHandler()
                        .incorrectJsonType(
                                null, theName, ValueType.ARRAY, null, ValueType.OBJECT, null);
            }
            ...
            if (theState.isPreResource()) {
                ...
                if (resType == null || !resType.isString()) {
                    throw new DataFormatException(
                            Msg.code(1843)
                                    + "Missing required element 'resourceType' from JSON resource"
    ...
    private void parseExtension(
            ParserState<?> theState, BaseJsonLikeArray theValues, boolean theIsModifier) {
        int allUnderscoreNames = 0;
        int handledUnderscoreNames = 0;
            ...
            if (null == jsonElement || !(jsonElement.isScalar())) {
                ...
                getErrorHandler()
                        .missingRequiredElement(
                                new ParseLocation().setParentElementName(parentElementName), "url");
                url = null;
            } else {
                        ...
                        if (nextValue != null) {
                            if (nextValue.isObject()) {
                                ...
                                if (nextExtObj.get(nextName) == null) {
                                    theState.enteringNewElement(null, nextName);
                                    parseAlternates(
                                            nextValue, theState, alternateName, alternateName);
                                    theState.endingElement();
                                }
                            } else {
                                getErrorHandler()
        ...
        if ("Bundle".equals(def.getName())) {
            ...
            if (entries != null) {
                for (IBase nextEntry : entries) {
                    ...
                    // TODO: should emit a warning and maybe notify the error handler if the
                    // resource ID doesn't match the
                    // fullUrl idPart
                    BaseRuntimeChildDefinition fullUrlChild = entryDef.getChildByName("fullUrl");
                    if (fullUrlChild == null) {
                        continue; // TODO: remove this once the data model in tinder plugin catches
                        // up to 1.2
                    }
                    ...
                    if (fullUrl != null && !fullUrl.isEmpty()) {
                        ...
                        if (value.isEmpty() == false) {
                            List<IBase> entryResources =
                                    entryDef.getChildByName("resource")
                                            .getAccessor()
                                            .getValues(nextEntry);
                            if (entryResources != null && entryResources.size() > 0) {
                                ...
                                if (isNotBlank(versionId)
                                        && res.getIdElement().hasVersionIdPart() == false) {
    ...
    @Override
    public IBaseResource parseResource(JsonLikeStructure theJsonLikeStructure)
            throws DataFormatException {
    ...
    private void write(BaseJsonLikeWriter theEventWriter, String theChildName, Boolean theValue)
            throws IOException {
        if (theValue != null) {
    ...
    // /*
    // * Extension children which are arrays are sub-extensions. Any other value type should be
    // treated as a value.
    // */
    ...
    private void write(
            BaseJsonLikeWriter theEventWriter, String theChildName, BigDecimal theDecimalValue)
            throws IOException {
        theEventWriter.write(theChildName, theDecimalValue);
    }

    private void write(BaseJsonLikeWriter theEventWriter, String theChildName, Integer theValue)
            throws IOException {
        theEventWriter.write(theChildName, theValue);
    }

    private void writeCommentsPreAndPost(IBase theNextValue, BaseJsonLikeWriter theEventWriter)
            throws IOException {
    ...
    private void writeOptionalTagWithTextNode(
            BaseJsonLikeWriter theEventWriter,
            String theElementName,
            IPrimitiveDatatype<?> thePrimitive)
            throws IOException {
    ...
    private void writeOptionalTagWithTextNode(
            BaseJsonLikeWriter theEventWriter, String theElementName, String theValue)
            throws IOException {
    ...
    private static void write(BaseJsonLikeWriter theWriter, String theName, String theValue)
            throws IOException {
    ...
    private class HeldExtension implements Comparable<HeldExtension> {
        ...
        public HeldExtension(
                RuntimeChildDeclaredExtensionDefinition theDef,
                IBase theValue,
                CompositeChildElement theChildElem) {
            assert theDef != null;
        ...
        public int compareTo(HeldExtension theArg0) {
            ...
            String url2 =
                    theArg0.myDef != null
                            ? theArg0.myDef.getExtensionUrl()
                            : theArg0.myUndeclaredExtension.getUrl();
        ...
        private void managePrimitiveExtension(
                ...
                boolean theContainedResource)
                throws IOException {
            if (def.getChildType().equals(ID_DATATYPE)
                    || def.getChildType().equals(PRIMITIVE_DATATYPE)) {
                ...
                if (def != null) {
                    extractDeclaredExtensions(
                            theValue, def, extensions, modifierExtensions, myParent);
                }
                boolean haveContent = false;
        ...
        public void write(
            ...
            if (myUndeclaredExtension != null) {
                ...
                BaseRuntimeElementDefinition<?> def =
                        myDef.getChildElementDefinitionByDatatype(myValue.getClass());
                ...
                if (!noValue) {
                    ...
                    if (childDef == null) {
                        throw new ConfigurationException(
                                Msg.code(1844)
                                        + "Unable to encode extension, unrecognized child element"
                                        + " type: "
