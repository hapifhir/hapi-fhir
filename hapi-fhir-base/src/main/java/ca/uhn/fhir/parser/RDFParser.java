...
/**
 * This class is the FHIR RDF parser/encoder. Users should not interact with this class directly,
 * but should use {@link FhirContext#newRDFParser()} to get an instance.
 */
public class RDFParser extends BaseParser {
	...
	private static final org.slf4j.Logger logger =
				org.slf4j.LoggerFactory.getLogger(RDFParser.class);
	...
	/**
	* Do not use this constructor, the recommended way to obtain a new instance of the RDF parser
	* is to invoke {@link FhirContext#newRDFParser()}.
	*
	* @param parserErrorHandler the Parser Error Handler
	*/
	public RDFParser(
				final FhirContext context,
				final IParserErrorHandler parserErrorHandler,
				final Lang lang) {
	...
	/**
	* Writes the provided resource to the writer. This should only be called for the top-level
	* resource being encoded.
	*
	...
	*/
	...
	protected void doEncodeResourceToWriter(
		...
		encodeResourceToRDFStreamWriter(
					resource, rdfModel, false, resourceId, encodeContext, true, null);
	...
	*/
	@Override
	protected <T extends IBaseResource> T doParseResource(
				final Class<T> resourceType, final Reader reader) throws DataFormatException {
	...
	private Resource encodeResourceToRDFStreamWriter(
		...
		if (resDef == null) {
				throw new ConfigurationException(
						Msg.code(1845) + "Unknown resource type: " + resource.getClass());
		}
		...
		if (parentResource == null) {
				if (!resource.getIdElement().toUnqualified().hasIdPart()) {
					...
					String resourceUri =
								IRIs.resolve(uriBase, resource.getIdElement().toUnqualified().toString())
										.toString();
		...
		if (rootResource) {
				parentResource.addProperty(
						rdfModel.createProperty(FHIR_NS + NODE_ROLE),
						rdfModel.createProperty(FHIR_NS + TREE_ROOT));
		}
	...
	*/
	...
	/**
	* Utility method to create a blank node with a fhir:value predicate accepting a specific data
	* type and index
	*
	* @param rdfModel Model to create node within
	...
	*/
	private Resource createFhirValueBlankNode(
		...
		Resource fhirValueBlankNodeResource =
					rdfModel.createResource()
								.addProperty(
										rdfModel.createProperty(FHIR_NS + VALUE),
	...
	*/
	private String constructPredicateName(
				...
				BaseRuntimeChildDefinition definition,
				String childName,
				IBase parentElement) {
		...
		if (definition instanceof BaseRuntimeDeclaredChildDefinition) {
				BaseRuntimeDeclaredChildDefinition declaredDef =
						(BaseRuntimeDeclaredChildDefinition) definition;
				...
				if (declaringClass != resource.getClass()) {
					...
					if (IBaseBackboneElement.class.isAssignableFrom(declaringClass)
						...
						if (classToFhirTypeMap.containsKey(declaringClass)) {
								...
								try {
									IBase elem =
												(IBase) declaringClass.getDeclaredConstructor().newInstance();
									property = elem.fhirType();
									classToFhirTypeMap.put(declaringClass, property);
								} catch (Exception ex) {
									logger.debug(
												"Error instantiating an "
									...
									if (StringUtils.isNotBlank(encodedValue)) {
										...
										if (element != null) {
												XSDDatatype dataType =
														getXSDDataTypeForFhirType(
																	element.fhirType(), encodedValue);
									...
									if (value != null) {
										String propertyName =
													constructPredicateName(
																...
																parentElement);
										XSDDatatype dataType =
													getXSDDataTypeForFhirType(pd.fhirType(), value);
										Resource valueResource =
													this.createFhirValueBlankNode(
																rdfModel, value, dataType, cardinalityIndex);
													...
													for (IBaseExtension extension :
														...
														extensionResource.addProperty(
																	...
																	rdfModel.createTypedLiteral(
																				i, XSDDatatype.XSDinteger));
														valueResource.addProperty(
																	rdfModel.createProperty(
																				FHIR_NS + ELEMENT_EXTENSION),
																	extensionResource);
														encodeCompositeElementToStreamWriter(
																	resource,
																	extension,
										...
										rdfResource.addProperty(
													rdfModel.createProperty(propertyName), valueResource);
									}
								}
								break;
						}
					...
					case COMPOSITE_DATATYPE:
						{
								...
								if (element instanceof IBaseResource) {
									...
									IIdType resourceId =
												processResourceID((IBaseResource) element, encodeContext);
									if (resourceId != null) {
								...
								if (idString != null) {
									rdfResource.addProperty(
												rdfModel.createProperty(idPredicate),
												createFhirValueBlankNode(rdfModel, idString));
								}
								rdfModel =
										encodeCompositeElementToStreamWriter(
													resource,
					...
					case CONTAINED_RESOURCES:
						{
								if (element != null) {
									...
									rdfResource.addProperty(
												rdfModel.createProperty(FHIR_NS + DOMAIN_RESOURCE_CONTAINED),
												containedResource);
					...
					case RESOURCE:
						{
								...
								IIdType resourceId = processResourceID(resource, encodeContext);
								encodeResourceToRDFStreamWriter(
										resource, rdfModel, false, resourceId, encodeContext, false, null);
					...
					default:
						{
								throw new IllegalStateException(
										Msg.code(1847)
													+ "Unexpected node - should not happen: "
													+ childDef.getName());
						}
				}
		} finally {
	...
	*/
	...
	private IIdType processResourceID(
				final IBaseResource resource, final EncodeContext encodeContext) {
		...
		if (!super.shouldEncodeResourceId(resource, encodeContext)) {
				resourceId = null;
		} else if (encodeContext.getResourcePath().size() == 1
					&& super.getEncodeForceResourceId() != null) {
	...
	private Model encodeExtension(
		...
		String extensionPredicateName =
					constructPredicateName(resource, extDef, extDef.getElementName(), null);
	...
	private Model encodeCompositeElementToStreamWriter(
		...
		for (CompositeChildElement nextChildElem :
				...
				if (nextChild instanceof RuntimeChildNarrativeDefinition) {
					...
					if (gen != null) {
						...
						if (narrative.isEmpty()) {
								...
								String propertyName =
										constructPredicateName(
													resource, child, child.getElementName(), element);
								rdfResource.addProperty(
										rdfModel.createProperty(propertyName), childResource);
				...
				if (nextChild instanceof RuntimeChildDirectResource) {
					...
					String propertyName =
								constructPredicateName(
										resource, nextChild, nextChild.getElementName(), element);
				...
				if (nextChild instanceof RuntimeChildContainedResources) {
					...
					List<? extends IBase> values = nextChild.getAccessor().getValues(element);
					values =
								super.preProcessValues(
										nextChild, resource, values, nextChildElem, encodeContext);
					...
					for (IBase nextValue : values) {
						...
						ChildNameAndDef childNameAndDef =
						...
						if (extensionUrl != null && !childName.equals(EXTENSION)) {
								...
								rdfModel =
										encodeExtension(
													...
													encodeContext,
													cardinalityIndex);
						} else if (!(nextChild instanceof RuntimeChildNarrativeDefinition)
									|| !containedResource) {

								// If the child is not a value type, create a child object (blank node) for
								// subordinate predicates to be attached to
								if (childDef.getChildType() != PRIMITIVE_DATATYPE
									...
									String propertyName =
												constructPredicateName(
														resource, nextChild, childName, nextValue);
									rdfResource.addProperty(
												rdfModel.createProperty(propertyName), childResource);
									if (cardinalityIndex != null && cardinalityIndex > -1) {
										childResource.addProperty(
	...
	private <T extends IBaseResource> T parseResource(Class<T> resourceType, Model rdfModel) {
		...
		ParserState<T> parserState =
					ParserState.getPreResourceInstance(
								this, resourceType, getContext(), true, getErrorHandler());
		return parseRootResource(rdfModel, parserState, resourceType);
	}

	private <T> T parseRootResource(
				Model rdfModel, ParserState<T> parserState, Class<T> resourceType) {
		logger.trace("Entering parseRootResource with state: {}", parserState);

		StmtIterator rootStatementIterator =
					rdfModel.listStatements(
								null,
								rdfModel.getProperty(FHIR_NS + NODE_ROLE),
								rdfModel.getProperty(FHIR_NS + TREE_ROOT));
		...
		while (rootStatementIterator.hasNext()) {
				...
				// If a resourceType is not provided via the server framework, discern it based on the
				// rdf:type Arc
				...
				RuntimeResourceDefinition definition =
						getContext().getResourceDefinition(fhirTypeString);
	...
	private <T> void parseResource(
				ParserState<T> parserState, String resourceType, RDFNode rootNode) {
				...
				for (Statement statement : statements) {
					...
					if (predicateAttributeName != null) {
						if (predicateAttributeName.equals(MODIFIER_EXTENSION)) {
								...
								processStatementObject(
										parserState, predicateAttributeName, statement.getObject());
						}
					}
				}
	...
	private String extractAttributeNameFromPredicate(Statement statement) {
		...
		if (predicateObjectAttribute.contains(".")) {
				predicateAttributeName =
						predicateObjectAttribute.substring(
									predicateObjectAttribute.lastIndexOf(".") + 1);
		} else {
	...
	private <T> void processExtension(
				ParserState<T> parserState, RDFNode statementObject, boolean isModifier) {
		logger.trace("Entering processExtension with state: {}", parserState);
		Resource resource = statementObject.asResource();
		Statement urlProperty =
					resource.getProperty(resource.getModel().createProperty(FHIR_NS + EXTENSION_URL));
		...
		for (Statement statement : extensionStatements) {
				...
				if (propertyUri.contains("Extension.value")) {
					...
					if (target.getChildType().equals(ID_DATATYPE)
						...
						extensionValueResource =
									statement
												.getObject()
												.asResource()
												.getProperty(
														resource.getModel().createProperty(FHIR_NS + VALUE))
												.getObject()
												.asLiteral();
					} else {
		...
		// Some extensions don't have their own values - they then have more extensions inside of
		// them
		...
		private int getFhirIndex(Resource resource) {
				if (resource.hasProperty(resource.getModel().createProperty(FHIR_NS + FHIR_INDEX))) {
					return resource.getProperty(
										resource.getModel().createProperty(FHIR_NS + FHIR_INDEX))
								.getInt();
				}
				return -1;
		}
