...
public class ValidationResult {
    ...
    public ValidationResult(FhirContext theCtx, List<SingleValidationMessage> theMessages) {
        ...
        for (SingleValidationMessage next : myMessages) {
            if (next.getSeverity() == null
                    || next.getSeverity().ordinal() > ResultSeverityEnum.WARNING.ordinal()) {
                successful = false;
                break;
            }
        }
     ...
     * {@link ResultSeverityEnum#ERROR} or {@link ResultSeverityEnum#FATAL}. A validation is still
     * considered successful if it only has issues at level {@link ResultSeverityEnum#WARNING} or
     * lower.
     *
     ...
     *     {@link #toOperationOutcome()} is identical to this method, but has a more suitable name
     *     so this method will be removed at some point.
     */
    @Deprecated
    ...
    public void populateOperationOutcome(IBaseOperationOutcome theOperationOutcome) {
        for (SingleValidationMessage next : myMessages) {
            ...
            if (isNotBlank(next.getLocationString())) {
                ...
                location =
                        "Line[" + next.getLocationLine() + "] Col[" + next.getLocationCol() + "]";
            } else {
                location = null;
            }
        ...
        if (myMessages.isEmpty()) {
            String message =
                    myCtx.getLocalizer().getMessage(ValidationResult.class, "noIssuesDetected");
            OperationOutcomeUtil.addIssue(
                    myCtx, theOperationOutcome, "information", message, null, "informational");
        }
    }
