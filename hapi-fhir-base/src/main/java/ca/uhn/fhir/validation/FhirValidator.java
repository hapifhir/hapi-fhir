...
/**
 * Resource validator, which checks resources for compliance against various validation schemes
 * (schemas, schematrons, profiles, etc.)
 *
 * <p>To obtain a resource validator, call {@link FhirContext#newValidator()}
 *
 * <p><b>Thread safety note:</b> This class is thread safe, so you may register or unregister
 * validator modules at any time. Individual modules are not guaranteed to be thread safe however.
 * Reconfigure them with caution.
 */
public class FhirValidator {
    ...
    /**
     * Constructor (this should not be called directly, but rather {@link
     * FhirContext#newValidator()} should be called to obtain an instance of {@link FhirValidator})
     */
     ...
     */
    public synchronized FhirValidator setValidateAgainstStandardSchema(
            boolean theValidateAgainstStandardSchema) {
        addOrRemoveValidator(
                theValidateAgainstStandardSchema,
                SchemaBaseValidator.class,
    ...
    /**
     * Add a new validator module to this validator. You may register as many modules as you like at
     * any time.
     *
    ...
    /**
     * Removes a validator module from this validator. You may register as many modules as you like,
    ...
    public synchronized void unregisterValidatorModule(IValidatorModule theValidator) {
        Validate.notNull(theValidator, "theValidator must not be null");
        ArrayList<IValidatorModule> newValidators =
                new ArrayList<IValidatorModule>(myValidators.size() + 1);
        newValidators.addAll(myValidators);
    ...
    /**
     * Validates a resource instance returning a {@link ValidationResult} which contains the
     * results.
     *
    ...
    public ValidationResult validateWithResult(
            IBaseResource theResource, ValidationOptions theOptions) {
        Validate.notNull(theResource, "theResource must not be null");
        IValidationContext<IBaseResource> validationContext =
    ...
    private ValidationResult validateBundleEntriesConcurrently(
        ...
        List<IBaseResource> entries =
                BundleUtil.toListOfResources(
                        myContext, (IBaseBundle) theValidationContext.getResource());
        // Async validation tasks
        List<ConcurrentValidationTask> validationTasks =
                        ...
                        .mapToObj(
                                index -> {
                                    ...
                                    String entryPathPrefix =
                                            String.format(
                                                    "Bundle.entry[%d].resource.ofType(%s)",
                                                    index, resourceToValidate.fhirType());
                                    Future<ValidationResult> future =
                                            myExecutorService.submit(
                                                    () -> {
                                                        IValidationContext<IBaseResource>
    ...
    static List<SingleValidationMessage> buildValidationMessages(
            List<ConcurrentValidationTask> validationTasks) {
                                ...
                                .map(
                                        message -> {
                                            ...
                                            if (dotIndex >= 0) {
                                                ...
                                                if (isBlank(bundleEntryPathPrefix)
                                                        || isBlank(locationString)) {
                                            ...
                                            message.setLocationString(
                                                    bundleEntryPathPrefix + currentPath);
                                            return message;
                                        })
                                .collect(Collectors.toList());
    ...
    private ValidationResult invokeValidationCompletedHooks(
            IBaseResource theResourceParsed,
            String theResourceRaw,
            ...
            if (myInterceptorBroadcaster.hasHooks(Pointcut.VALIDATION_COMPLETED)) {
                ...
                Object newResult =
                        myInterceptorBroadcaster.callHooksAndReturnObject(
                                Pointcut.VALIDATION_COMPLETED, params);
                if (newResult != null) {
    ...
    /**
     * Optionally supplies an interceptor broadcaster that will be used to invoke validation related
     * Pointcut events
     *
     * @since 5.5.0
     */
    ...
    /**
     * If this is true, bundles will be validated in parallel threads. The bundle structure itself
     * will not be validated, only the resources in its entries.
     */
    ...
    /**
     * If this is true, any resource that has contained resources will first be deep-copied and then
     * the contained resources remove from the copy and this copy without contained resources will
     * be validated.
     */
     ...
     * be validated.
     */
    public FhirValidator setSkipContainedReferenceValidation(
            boolean theSkipContainedReferenceValidation) {
