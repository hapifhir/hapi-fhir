 ...
 * task duration, throughput, estimated task completion, etc.
 *
 * <p>
 *
 * <p><b>Thread Safety Note: </b> StopWatch is not intended to be thread safe.
 *
 * @since HAPI FHIR 3.3.0
 */
public class StopWatch {

	// TODO KHS it is risky for this to be a static field.  Safer to make it non-static, but that
	// will require
	...
	*/
	...
	/**
	* Finish the counter on the current task (which was started by calling {@link
	* #startTask(String)}. This method has no effect if no task is currently started so it's ok to
	...
	*/
	...
	/**
	* Returns a nice human-readable display of the time taken per operation. Note that this may not
	* actually output the number of milliseconds if the time taken per operation was very long
	* (over 10 seconds)
	*
	...
	*/
	...
	/**
	* Determine the current throughput per unit of time (specified in theUnit) assuming that
	* theNumOperations operations have happened.
	*
	* <p>For example, if this stopwatch has 2 seconds elapsed, and this method is called for
	* theNumOperations=30 and TimeUnit=SECONDS, this method will return 15
	*
	...
	*/
	...
	/**
	* Given an amount of something completed so far, and a total amount, calculates how long it
	* will take for something to complete
	*
	...
	* @return A formatted amount of time
	*/
	public static String formatEstimatedTimeRemaining(
				double theCompleteToDate, double theTotal, double millis) {
	...
	*
	* <p><b>Thread Safety Note: </b> This method is not threadsafe! Do not use subtasks in a
	...
	*/
	...
	/**
	* Formats value in an appropriate format. See {@link #formatMillis(long)}} for a description of
	* the format
	*
	* @see #formatMillis(long)
	*/
	...
	*/
	public static double getThroughput(
				long theNumOperations, long theMillisElapsed, TimeUnit theUnit) {
	...
	/**
	* Formats a number of milliseconds for display (e.g. in a log file), tailoring the output to
	* how big the value actually is.
	*
	* <p>Example outputs:
	*
	* <ul>
	...
	public static String formatMillis(double theMillis) {
		...
		if (theMillis > 0.0 && theMillis < 1.0) {
				...
				appendRightAlignedNumber(
						...
						"",
						2,
						((millisAsLong % DateUtils.MILLIS_PER_DAY) / DateUtils.MILLIS_PER_HOUR));
				appendRightAlignedNumber(
						buf,
						":",
						2,
						((millisAsLong % DateUtils.MILLIS_PER_HOUR) / DateUtils.MILLIS_PER_MINUTE));
				appendRightAlignedNumber(
						buf,
						":",
						2,
						((millisAsLong % DateUtils.MILLIS_PER_MINUTE) / DateUtils.MILLIS_PER_SECOND));
