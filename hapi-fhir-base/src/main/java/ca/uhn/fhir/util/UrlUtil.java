...
public class UrlUtil {
    ...
    private static final Escaper PARAMETER_ESCAPER =
    ...
    /**
     * Resolve a relative URL - THIS METHOD WILL NOT FAIL but will log a warning and return
     * theEndpoint if the input is invalid.
     */
    ...
    public static String constructRelativeUrl(
            String theParentExtensionUrl, String theExtensionUrl) {
    ...
    /**
     * Given a FHIR resource URL, extracts the associated resource type. Supported formats include
     * the following inputs, all of which will return {@literal Patient}. If no resource type can be
     * determined, {@literal null} will be returned.
     *
     * <ul>
     *   <li>Patient
     *   <li>Patient?
     ...
     */
    @Nullable
    public static String determineResourceTypeInResourceUrl(
            FhirContext theFhirContext, String theUrl) {
        if (theUrl == null) {
            return null;
        }
     ...
     *
     * <p>This method is intended to be applied to an individual parameter name or value. For
     * example, if you are creating the URL <code>http://example.com/fhir/Patient?key=føø</code> it
     * would be appropriate to pass the string "føø" to this method, but not appropriate to pass the
     * entire URL since characters such as "/" and "?" would also be escaped.
     */
    ...
    /**
     * Applies the same encodong as {@link #escapeUrlParam(String)} but against all values in a
     * collection
     */
    public static List<String> escapeUrlParams(@Nonnull Collection<String> theUnescaped) {
        return theUnescaped.stream()
    ...
    /**
     * Normalizes canonical URLs for comparison. Trailing "/" is stripped, and any version
     * identifiers or fragment hash is removed
     */
                ...
                if (retVal.getResourceType() == null) {
                    ...
                    if (nextSubstring.equals(Constants.URL_TOKEN_HISTORY)) {
                        ...
                        throw new InvalidRequestException(
                                Msg.code(1742) + "Invalid FHIR resource URL: " + url);
                    }
                }
                if (nextChar == '?') {
                    if (url.length() > idx + 1) {
    ...
    /**
     * Applies the same logic as {@link #sanitizeUrlPart(CharSequence)} but against an array,
     * returning an array with the same strings as the input but with sanitization applied
     */
        ...
        for (int i = 0; i < theString.length(); i++) {
            ...
            if (nextChar == '%' || (nextChar == '+' && shouldEscapePlus)) {
                try {
                    ...
                    throw new Error(
                            Msg.code(1743) + "UTF-8 not supported, this shouldn't happen", e);
                }
            }
        }
        return theString;
    ...
    public static List<NameValuePair> translateMatchUrl(String theMatchUrl) {
        ...
        final String[] replacementList =
                new String[] {"%2B", "%7C", "=%3E%3D", "=%3C%3D", "=%3E", "=%3C"};
        ...
        // One issue that has happened before is people putting a "+" sign into an email address in
        // a match URL
        // and having that turn into a " ". Since spaces are never appropriate for email addresses,
        // let's just
