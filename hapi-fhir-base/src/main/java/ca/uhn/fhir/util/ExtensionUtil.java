...
public class ExtensionUtil {
     ...
     * @throws IllegalArgumentException IllegalArgumentException is thrown in case resource doesn't
     *     support extensions
     */
    ...
    private static IBaseHasExtensions validateExtensionSupport(IBase theBase) {
        if (!(theBase instanceof IBaseHasExtensions)) {
            throw new IllegalArgumentException(
                    Msg.code(1747)
                            + String.format(
                                    "Expected instance that supports extensions, but got %s",
                                    theBase));
        }
    ...
    public static boolean hasExtension(
            IBase theBase, String theExtensionUrl, String theExtensionValue) {
     ...
     * @return Returns the first available extension with the specified URL, or null if such
    ...
    /**
     * Given a resource or other structure that can have direct extensions, pulls out any extensions
     * that have the given theExtensionUrl and a primitive value type, and returns a list of the
     * string version of the extension values.
     */
    public static List<String> getExtensionPrimitiveValues(
            IBaseHasExtensions theBase, String theExtensionUrl) {
        List<String> values =
                theBase.getExtension().stream()
     ...
     * @return Returns all extension with the specified URL, or an empty list if such extensions do
     *     not exist
     */
     ...
     *     not exist
     */
    public static List<IBaseExtension<?, ?>> getExtensionsByUrl(
            IBaseHasExtensions theBase, String theExtensionUrl) {
        Predicate<IBaseExtension<?, ?>> urlEqualityPredicate =
                e -> theExtensionUrl.equals(e.getUrl());
    ...
    public static void setExtension(
            FhirContext theFhirContext, IBaseExtension<?, ?> theExtension, String theValue) {
     ...
     * @param theFhirContext The context containing FHIR resource definitions
     */
    public static void setExtensionAsString(
            FhirContext theFhirContext, IBase theBase, String theUrl, String theValue) {
    ...
    public static boolean equals(
            IBaseExtension<?, ?> theLeftExtension, IBaseExtension<?, ?> theRightExtension) {
        return TerserUtil.equals(theLeftExtension, theRightExtension);
    }

    /**
     * Given an extension, looks for the first child extension with the given URL of {@literal
     * theChildExtensionUrl} and a primitive datatype value, and returns the String version of that
     * value. E.g. if the value is a FHIR boolean, it would return the string "true" or "false. If
     * the extension has no value, or the value is not a primitive datatype, or the URL is not
     * found, the method will return {@literal null}.
     *
    ...
    public static <D, T extends IBaseExtension<T, D>> String extractChildPrimitiveExtensionValue(
        ...
        Optional<T> codeExtension =
                theExtension.getExtension().stream()
