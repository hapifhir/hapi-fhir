...
public class SubscriptionUtil {
    ...
    private static void populatePrimitiveValue(
        ...
        if (entries.size() == 0) {
            BaseRuntimeElementDefinition<?> statusElement =
                    statusChild.getChildByName(theChildName);
            instance =
                    (IPrimitiveType<?>)
                            statusElement.newInstance(
                                    statusChild.getInstanceConstructorArguments());
    ...
    public static void setReason(
            FhirContext theContext, IBaseResource theSubscription, String theMessage) {
        populatePrimitiveValue(theContext, theSubscription, "reason", theMessage);
    }

    public static void setStatus(
            FhirContext theContext, IBaseResource theSubscription, String theStatus) {
    ...
    public static boolean isCrossPartition(IBaseResource theSubscription) {
        if (theSubscription instanceof IBaseHasExtensions) {
            ...
            if (Objects.nonNull(extension)) {
                try {
                    IBaseBooleanDatatype booleanDatatype =
                            (IBaseBooleanDatatype) (extension.getValue());
