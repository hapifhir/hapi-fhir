...
public class ReflectionUtil {
    ...
    private static final ConcurrentHashMap<String, Object> ourFhirServerVersions =
            new ConcurrentHashMap<>();
    private static final org.slf4j.Logger ourLog =
            org.slf4j.LoggerFactory.getLogger(ReflectionUtil.class);
    ...
    /**
     * Returns all methods declared against {@literal theClazz}. This method returns a predictable
     * order, which is sorted by method name and then by parameters.
     *
     * <p>This method does not include superclass methods (see {@link #getDeclaredMethods(Class,
     * boolean)} if you want to include those.
     */
    ...
    public static List<Method> getDeclaredMethods(
            Class<?> theClazz, boolean theIncludeMethodsFromSuperclasses) {
    ...
    private static void populateDeclaredMethodsMap(
            Class<?> theClazz,
            HashMap<String, Method> foundMethods,
            boolean theIncludeMethodsFromSuperclasses) {
        ...
        if (theIncludeMethodsFromSuperclasses && !theClazz.getSuperclass().equals(Object.class)) {
            populateDeclaredMethodsMap(
                    theClazz.getSuperclass(), foundMethods, theIncludeMethodsFromSuperclasses);
        }
    }

    /**
     * Returns a description like <code>startsWith params(java.lang.String, int) returns(boolean)
     * </code>. The format is chosen in order to provide a predictable and useful sorting order.
     */
    ...
    public static Class<?> getGenericCollectionTypeOfMethodParameter(
            Method theMethod, int theParamIndex) {
        Type genericParameterType = theMethod.getGenericParameterTypes()[theParamIndex];
        if (Class.class.equals(genericParameterType)
                || Class.class.equals(genericParameterType.getClass())) {
            return null;
        }
    ...
    /**
     * Instantiate a class by no-arg constructor, throw {@link ConfigurationException} if we fail to
     * do so
     */
    public static <T> T newInstance(Class<T> theType) {
        ...
        try {
            ...
            throw new ConfigurationException(
                    Msg.code(1784) + "Failed to instantiate " + theType.getName(), e);
        }
    }

    public static <T> T newInstance(
            Class<T> theType, Class<?> theArgumentType, Object theArgument) {
        Validate.notNull(theType, "theType must not be null");
        try {
            ...
            throw new ConfigurationException(
                    Msg.code(1785) + "Failed to instantiate " + theType.getName(), e);
        }
    }

    public static Object newInstanceOfFhirServerType(String theType) {
        String errorMessage =
                "Unable to instantiate server framework. Please make sure that hapi-fhir-server"
                        + " library is on your classpath!";
        String wantedType = "ca.uhn.fhir.rest.api.server.IFhirVersionServer";
        return newInstanceOfType(
                theType, theType, errorMessage, wantedType, new Class[0], new Object[0]);
    }
    ...
    private static Object newInstanceOfType(
        ...
        if (fhirServerVersion == null) {
            try {
                ...
                fhirServerVersion =
                        type.getConstructor(theParameterArgTypes).newInstance(theConstructorArgs);
            } catch (Exception e) {
    ...
    public static <T> T newInstanceOrReturnNull(String theClassName, Class<T> theType) {
        return newInstanceOrReturnNull(
                theClassName, theType, EMPTY_CLASS_ARRAY, EMPTY_OBJECT_ARRAY);
    }
    ...
    public static <T> T newInstance(
        ...
        try {
            ...
            if (!theType.isAssignableFrom(clazz)) {
                throw new ConfigurationException(
                        Msg.code(1787) + theClassName + " is not assignable to " + theType);
            }
