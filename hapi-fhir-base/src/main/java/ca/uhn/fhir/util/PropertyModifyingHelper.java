...
/**
 * Helper class for handling updates of the instances that support property modification via <code>
 * setProperty</code> and <code>getProperty</code> methods.
 */
public class PropertyModifyingHelper {
    ...
    public PropertyModifyingHelper(FhirContext theFhirContext, IBase theBase) {
        if (findGetPropertyMethod(theBase) == null) {
            throw new IllegalArgumentException(
                    Msg.code(1771)
                            + "Specified base instance does not support property retrieval.");
        }
        myBase = theBase;
    ...
    /**
     * Gets all non-blank fields as a single string joined with the delimiter provided by {@link
     * #getDelimiter()}
     *
     ...
     * @return Returns property value converted to string. In case of multiple values, they are
     *     joined with the specified delimiter.
     */
    ...
    /**
     * Sets property or adds to a collection of properties with the specified name from the provided
     * base class.
     *
     * @param thePropertyName Name of the property to set or add element to in case property is a
     *     collection
     */
    public void set(String thePropertyName, String theValue) {
        ...
        try {
            ...
            Method setPropertyMethod =
                    findSetPropertyMethod(myBase, int.class, String.class, value.getClass());
            ...
            throw new IllegalStateException(
                    Msg.code(1772)
                            + String.format(
                                    "Unable to set property %s on %s", thePropertyName, myBase),
                    e);
        }
    }
    ...
    public List<String> getMultiple(String thePropertyName) {
        ...
        try {
            values =
                    (Object[])
                            getPropertyMethod.invoke(
                                    myBase, thePropertyName.hashCode(), thePropertyName, true);
        } catch (Exception e) {
            throw new IllegalStateException(
                    Msg.code(1773)
                            + String.format(
                                    "Instance %s does not supply property %s",
                                    myBase, thePropertyName),
                    e);
        }
