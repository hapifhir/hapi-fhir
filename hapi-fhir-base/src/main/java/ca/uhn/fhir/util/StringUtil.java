...
public class StringUtil {
	...
	/**
	* If a string ends with a given character, remove that character from the end of the string (as
	* many times as it occurs at the end)
	*/
	public static String chompCharacter(String theInput, char theCharacter) {
		String retVal = theInput;
		while (retVal != null
					&& retVal.length() > 0
					&& retVal.charAt(retVal.length() - 1) == theCharacter) {
	...
	*
	* @param theString String to get the prefix from
	* @param theCodePointCount Length of the prefix in code points
	* @return Returns the string prefix of the specified number of codepoints.
	*/
