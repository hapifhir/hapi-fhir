...
/**
 * This class can be used to build a Bundle resource to be used as a FHIR transaction. Convenience
 * methods provide support for setting various bundle fields and working with bundle parts such as
 * metadata and entry (method and search).
 *
 * <p>
 *
 * <p>This is not yet complete, and doesn't support all FHIR features. <b>USE WITH CAUTION</b> as
 * the API may change.
 *
 * @since 5.1.0
 */
public class BundleBuilder {
	...
	*/
	public BundleBuilder setBundleField(String theFieldName, String theFieldValue) {
		...
		IPrimitiveType<?> type =
					(IPrimitiveType<?>)
								typeChild
										.getChildByName(theFieldName)
										.newInstance(typeChild.getInstanceConstructorArguments());
	...
	* @param theFieldValue Value of the field to be set.
	*/
	public BundleBuilder setSearchField(
				IBase theSearch, String theFieldName, String theFieldValue) {
	...
	*/
	public PatchBuilder addTransactionFhirPatchEntry(IIdType theTarget, IBaseParameters thePatch) {
		...
		IPrimitiveType<?> url =
					addAndPopulateTransactionBundleEntryRequest(
								thePatch,
								theTarget.getValue(),
	...
	/**
	* Adds a FHIRPatch patch bundle to the transaction. This method is intended for conditional
	* PATCH operations. If you know the ID of the resource you wish to patch, use {@link
	* #addTransactionFhirPatchEntry(IIdType, IBaseParameters)} instead.
	*
	* @param thePatch The FHIRPath Parameters resource
	...
	*/
	public PatchBuilder addTransactionFhirPatchEntry(IBaseParameters thePatch) {
		IPrimitiveType<?> url =
					addAndPopulateTransactionBundleEntryRequest(thePatch, null, null, "PATCH");

		return new PatchBuilder(url);
	}

	/**
	* Adds an entry containing an update (PUT) request. Also sets the Bundle.type value to
	* "transaction" if it is not already set.
	*
	...
	*/
	public UpdateBuilder addTransactionUpdateEntry(IBaseResource theResource) {
		...
		IPrimitiveType<?> url =
					addAndPopulateTransactionBundleEntryRequest(theResource, fullUrl, requestUrl, verb);
	...
	private IPrimitiveType<?> addAndPopulateTransactionBundleEntryRequest(
		...
		IPrimitiveType<?> method =
					(IPrimitiveType<?>)
								myEntryRequestMethodDef.newInstance(
										myEntryRequestMethodChild.getInstanceConstructorArguments());
	...
	/**
	* Adds an entry containing an create (POST) request. Also sets the Bundle.type value to
	* "transaction" if it is not already set.
	*
	...
	*/
	...
	/**
	* Adds an entry containing a delete (DELETE) request. Also sets the Bundle.type value to
	* "transaction" if it is not already set.
	*
	* <p>Note that the resource is only used to extract its ID and type, and the body of the
	* resource is not included in the entry,
	*
	...
	*/
	public DeleteBuilder addTransactionDeleteEntry(String theResourceType, String theIdPart) {
		...
		String deleteUrl =
					idDt.toUnqualifiedVersionless().withResourceType(theResourceType).getValue();
	...
	*/
	...
	/**
	* Creates new search instance for the specified entry. Note that this method does not work for
	* DSTU2 model classes, it will only work on DSTU3+.
	*
	...
	*/
	...
	private void addToBase(
				IBase theBase,
				String theSearchChildName,
				IBase theValue,
				BaseRuntimeElementDefinition mySearchDef) {
		BaseRuntimeChildDefinition defn = mySearchDef.getChildByName(theSearchChildName);
		Validate.notNull(
					defn, "Unable to get child definition %s from %s", theSearchChildName, theBase);
	...
	*/
	public <T> IPrimitiveType<T> newPrimitive(String theTypeName) {
		BaseRuntimeElementDefinition primitiveDefinition =
					myContext.getElementDefinition(theTypeName);
	...
	* must be an actual valid value for this field or a {@link
	* ca.uhn.fhir.parser.DataFormatException} will be thrown.
	*/
	...
	*/
	...
	public abstract class BaseOperationBuilder {
		...
		/**
			...
			*
			* <p>Calling this method has no effect at all, it is only provided for easy method chaning
			* if you want to build your bundle as a single fluent call.
			*
			* @since 6.3.0
			*/
