...
public class BinaryUtil {
	...
	/**
	* Fetches the base64Binary value of Binary.data (or Binary.content on versions of FHIR before
	* R4), creating it if it does not already exist.
	*/
	@SuppressWarnings("unchecked")
	public static IPrimitiveType<byte[]> getOrCreateData(
				FhirContext theContext, IBaseBinary theBinary) {
		...
		BaseRuntimeChildDefinition entryChild =
					AttachmentUtil.getChild(theContext, theBinary, elementName);
		...
		return entries.stream()
					...
					.orElseGet(
								() -> {
									IPrimitiveType<byte[]> binary =
												AttachmentUtil.newPrimitive(theContext, "base64Binary", null);
	...
	public static void setData(
				FhirContext theCtx, IBaseBinary theBinary, byte[] theBytes, String theContentType) {
		getOrCreateData(theCtx, theBinary).setValue(theBytes);

		String elementName = "contentType";
		BaseRuntimeChildDefinition entryChild =
					AttachmentUtil.getChild(theCtx, theBinary, elementName);
		List<IBase> entries = entryChild.getAccessor().getValues(theBinary);
		IPrimitiveType<String> contentTypeElement =
					entries.stream()
								.map(t -> (IPrimitiveType<String>) t)
								.findFirst()
								.orElseGet(
										() -> {
												IPrimitiveType<String> stringType =
														AttachmentUtil.newPrimitive(theCtx, "code", null);
