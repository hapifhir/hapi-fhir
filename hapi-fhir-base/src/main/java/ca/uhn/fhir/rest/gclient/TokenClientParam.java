...
public class TokenClientParam extends BaseClientParam implements IParam {
	...
	public IMatches exactly() {
		return new IMatches() {
				...
				@Override
				public ICriterion<TokenClientParam> systemAndIdentifier(
				...
				public ICriterion<?> systemAndValues(String theSystem, String... theValues) {
					return new TokenCriterion(
								getParamName(), defaultString(theSystem), convertToList(theValues));
				}
	...
	/**
	* Create a search criterion that matches against the given system value but does not specify a
	* code. This means that any code/identifier with the given system should match.
	*
	* <p>Use {@link #exactly()} if you want to specify a code.
	*/
	...
	public interface IMatches {
		/**
			* Creates a search criterion that matches against the given code, with no code system
			* specified
			*
		...
		/**
			...
			* codes (this will be used to form a comma-separated OR query) with any system value. The
			* URL form of this method will create a parameter like <code>parameter=code1,code2</code>
			*
		...
		/**
			* Creates a search criterion that matches a given system with a collection of possible
			* codes (this will be used to form a comma-separated OR query) with the given <code>
			* Coding.system</code> and <code>Coding.value</code> values.
			*
			* <p>The URL form of this method will create a parameter like <code>
			* parameter=system1|code1,system2|code2</code>
			*
			* @param theCodings The codings
			*/
		ICriterion<?> codings(IBaseCoding... theCodings);

		/**
			* Creates a search criterion that matches against the given identifier (system and code if
			* both are present, or whatever is present)
			*
			* @param theIdentifier The identifier
			* @return A criterion
			*/
		...
		/**
			* Creates a search criterion that matches against the given identifier, with no system
			* specified
			*
			* @param theIdentifier The identifier
			* @return A criterion
		...
		/**
			...
			* (system and code if both are present, or whatever is present). In the query URL that is
			* generated, identifiers will be joined with a ',' to create an OR query.
			*
			* @param theIdentifiers The identifier
			* @return A criterion
			*/
		...
		/**
			...
			*
			* @param theSystem The code system (should be a URI)
