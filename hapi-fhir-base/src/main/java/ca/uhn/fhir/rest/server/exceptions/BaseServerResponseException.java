 ...
 * exceptions which are subclasses of this exception type, and RESTful server methods should also
 * only call subclasses of this exception type.
 *
 * <p>HAPI provides a number of subclasses of BaseServerResponseException, and each one corresponds
 * to a specific HTTP status code. For example, if a IResourceProvider method throws {@link
 * ResourceNotFoundException}, this is a signal to the server that an <code>HTTP 404</code> should
 * be returned to the client.
 *
 * <p><b>See:</b> A complete list of available exceptions is in the <a
 * href="./package-summary.html">package summary</a>. If an exception doesn't exist for a condition
 * you want to represent, let us know by filing an <a
 * href="https://github.com/hapifhir/hapi-fhir/issues">issue in our tracker</a>. You may also use
 * {@link UnclassifiedServerFailureException} to represent any error code you want.
 */
public abstract class BaseServerResponseException extends RuntimeException {
	...
	static {
		...
		registerExceptionType(
					MethodNotAllowedException.STATUS_CODE, MethodNotAllowedException.class);
		registerExceptionType(
					NotImplementedOperationException.STATUS_CODE,
					NotImplementedOperationException.class);
		registerExceptionType(NotModifiedException.STATUS_CODE, NotModifiedException.class);
		registerExceptionType(
					ResourceNotFoundException.STATUS_CODE, ResourceNotFoundException.class);
		registerExceptionType(ResourceGoneException.STATUS_CODE, ResourceGoneException.class);
		registerExceptionType(
					PreconditionFailedException.STATUS_CODE, PreconditionFailedException.class);
		registerExceptionType(
					ResourceVersionConflictException.STATUS_CODE,
					ResourceVersionConflictException.class);
		registerExceptionType(
					UnprocessableEntityException.STATUS_CODE, UnprocessableEntityException.class);
		registerExceptionType(
					ForbiddenOperationException.STATUS_CODE, ForbiddenOperationException.class);
	}
	...
	public BaseServerResponseException(int theStatusCode, String... theMessages) {
		...
		if (theMessages != null && theMessages.length > 1) {
				myAdditionalMessages =
						Arrays.asList(
									Arrays.copyOfRange(theMessages, 1, theMessages.length, String[].class));
		}
	}
	...
	* @param theStatusCode The HTTP status code corresponding to this problem
	* @param theMessage The message
	* @param theBaseOperationOutcome An BaseOperationOutcome resource to return to the calling
	*     client (in a server) or the BaseOperationOutcome that was returned from the server (in a
	*     client)
	*/
	...
	*/
	...
	/**
	* This flag can be used to signal to server infrastructure that the message supplied to this
	* exception (ie to the constructor) is considered trusted and is safe to return to the calling
	* client.
	*/
	...
	*/
	...
	/**
	* Returns the {@link IBaseOperationOutcome} resource if any which was supplied in the response,
	* or <code>null</code>
	*/
	...
	*
	* @param theBaseOperationOutcome The BaseOperationOutcome resource Sets the
	*     BaseOperationOutcome resource associated with this exception. In server implementations,
	*     this is the OperartionOutcome resource to include with the HTTP response. In client
	...
	*/
	...
	/**
	* In a RESTful client, this method will be populated with the body of the HTTP respone if one
	* was provided by the server, or <code>null</code> otherwise.
	*
	* <p>In a restful server, this method is currently ignored.
	*/
	...
	/**
	* This method is currently only called internally by HAPI, it should not be called by user
	* code.
	*/
	...
	/**
	* Returns a map containing any headers which should be added to the outgoing response. This
	* methos creates the map if none exists, so it will never return <code>null</code>
	*
	* @since 2.0 (note that this method existed in previous versions of HAPI but the method
	*     signature has been changed from <code>Map&lt;String, String[]&gt;</code> to <code>
	...
	*/
	...
	/**
	* In a RESTful client, this method will be populated with the HTTP status code that was
	* returned with the HTTP response.
	*
	* <p>In a restful server, this method is currently ignored.
	*/
	...
	*/
	...
	static void registerExceptionType(
				int theStatusCode, Class<? extends BaseServerResponseException> theType) {
		if (ourStatusCodeToExceptionType.containsKey(theStatusCode)) {
				throw new Error(
						Msg.code(1918)
									+ "Can not register "
									...
									+ theStatusCode
									+ " because "
									+ ourStatusCodeToExceptionType.get(theStatusCode)
									+ " already registers that code");
		}
