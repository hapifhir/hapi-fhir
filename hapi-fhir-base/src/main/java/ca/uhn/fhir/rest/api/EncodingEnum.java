...
public enum EncodingEnum {
	...
	static {
		...
		for (EncodingEnum next : values()) {
				 ...
				 * See #346
				 */
				ourContentTypeToEncoding.put(
						next.myResourceContentTypeNonLegacy.replace('+', ' '), next);
				ourContentTypeToEncoding.put(next.myResourceContentTypeLegacy.replace('+', ' '), next);
				ourContentTypeToEncodingLegacy.put(
						next.myResourceContentTypeLegacy.replace('+', ' '), next);
		}

		// Add before we add the lenient ones
		ourContentTypeToEncodingStrict =
					Collections.unmodifiableMap(new HashMap<>(ourContentTypeToEncoding));
		...
		ourContentTypeToEncodingLegacy =
					Collections.unmodifiableMap(ourContentTypeToEncodingLegacy);
	}
	...
	EncodingEnum(
				String theResourceContentTypeLegacy,
				String theResourceContentType,
				String theFormatContentType) {
	...
	/**
	* Returns <code>xml</code> or <code>json</code> as used on the <code>_format</code> search
	* parameter
	*/
	...
	/**
	* Returns the encoding for a given content type, or <code>null</code> if no encoding is found.
	*
	* <p><b>This method is lenient!</b> Things like "application/xml" will return {@link
	* EncodingEnum#XML} even if the "+fhir" part is missing from the expected content type. Also,
	* spaces are treated as a plus (i.e. "application/fhir json" will be treated as
	* "application/fhir+json" in order to account for unescaped spaces in URL parameters)
	*/
	...
	* Returns the encoding for a given content type, or <code>null</code> if no encoding is found.
	*
	* <p><b>This method is NOT lenient!</b> Things like "application/xml" will return <code>null
	* </code>
	*
