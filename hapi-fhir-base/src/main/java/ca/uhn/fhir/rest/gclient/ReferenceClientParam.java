...
public class ReferenceClientParam extends BaseClientParam implements IParam {
     ...
     * </pre>
     */
    public ICriterion<ReferenceClientParam> hasChainedProperty(
            String theResourceType, ICriterion<?> theCriterion) {
        return new ReferenceChainCriterion(getParamName(), theResourceType, theCriterion);
    }

    /**
     * Match the referenced resource if the resource has the given ID (this can be the logical ID or
    ...
    /**
     * Match the referenced resource if the resource has ANY of the given IDs (this is an OR search,
     * not an AND search), (this can be the logical ID or the absolute URL of the resource). Note
     * that to specify an AND search, simply add a subsequent {@link IQuery#where(ICriterion) where}
    ...
    private static class ReferenceChainCriterion
            implements ICriterion<ReferenceClientParam>, ICriterionInternal {
        ...
        ReferenceChainCriterion(
                String theParamName, String theResourceType, ICriterion<?> theWrappedCriterion) {
        ...
        public String getParameterName() {
            return myParamName
                    + myResourceTypeQualifier
                    + "."
                    + myWrappedCriterion.getParameterName();
        }
