...
public class DateParam extends BaseParamWithPrefix<DateParam>
    ...
    /**
     * Returns <code>true</code> if no date/time is specified. Note that this method does not check
     * the comparator, so a QualifiedDateParam with only a comparator and no date/time is considered
     * empty.
     */
    ...
    /**
     * Sets the value of the param to the given date (sets to the {@link TemporalPrecisionEnum#MILLI
     * millisecond} precision, and will be encoded using the system local time zone).
     */
    ...
    /**
     * Accepts values with or without a prefix (e.g. <code>gt2011-01-01</code> and <code>2011-01-01
     * </code>). If no prefix is provided in the given value, the {@link #getPrefix() existing
     * prefix} is preserved
     */
    ...
    public void setValuesAsQueryTokens(
        ...
        if (theParameters.size() == 1) {
            ...
            throw new InvalidRequestException(
                    Msg.code(1939)
                            + "This server does not support multi-valued dates for this parameter: "
                            + theParameters);
        }
    }
