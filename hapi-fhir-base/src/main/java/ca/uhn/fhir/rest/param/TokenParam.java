...
public class TokenParam extends BaseParam /*implements IQueryParameterType*/ {
	...
	/**
	* Constructor which copies the {@link InternalCodingDt#getSystemElement() system} and {@link
	* InternalCodingDt#getCodeElement() code} from a {@link InternalCodingDt} instance and adds it
	* as a parameter
	*
	...
	*/
	...
	/**
	* Constructor which copies the {@link BaseIdentifierDt#getSystemElement() system} and {@link
	* BaseIdentifierDt#getValueElement() value} from a {@link BaseIdentifierDt} instance and adds
	* it as a parameter
	*
	...
	*/
	...
	/**
	* Construct a {@link TokenParam} from the {@link IBaseCoding#getSystem()} () system} and {@link
	* IBaseCoding#getCode()} () code} of a {@link IBaseCoding} instance.
	*
	...
	*/
	...
	public TokenParam(String theSystem, String theValue, boolean theText) {
		if (theText && isNotBlank(theSystem)) {
				throw new IllegalArgumentException(
						Msg.code(1938)
									+ "theSystem can not be non-blank if theText is true (:text searches do"
									+ " not include a system). In other words, set the first parameter to"
									+ " null for a text search");
		}
		setSystem(theSystem);
		setValue(theValue);
		setText(theText);
	...
	/**
	* Returns the system for this token. Note that if a {@link #getModifier()} is being used, the
	* entire value of the parameter will be placed in {@link #getValue() value} and this method
	* will return <code>null</code>.
	*
	* <p
	* Also note that this value may be <code>null</code> or <code>""</code> (empty string)
	* and that each of these have a different meaning. When a token is passed on a URL and it has
	* no vertical bar (often meaning "return values that match the given code in any codesystem")
	* this method will return <code>null</code>. When a token is passed on a URL and it has a
	* vetical bar but nothing before the bar (often meaning "return values that match the given
	...
	*/
	...
	public boolean isEmpty() {
		return StringUtils.isBlank(mySystem)
					&& StringUtils.isBlank(myValue)
					&& getMissing() == null;
	}
