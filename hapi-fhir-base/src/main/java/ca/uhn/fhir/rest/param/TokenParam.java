...
public class TokenParam extends BaseParam /*implements IQueryParameterType*/ {
     ...
     *
     * <p
     * Also note that this value may be <code>null</code> or <code>""</code> (empty
     * string) and that each of these have a different meaning. When a token is passed on a URL and
     * it has no vertical bar (often meaning "return values that match the given code in any
     * codesystem") this method will return <code>null</code>. When a token is passed on a URL and
     * it has a vetical bar but nothing before the bar (often meaning "return values that match the
