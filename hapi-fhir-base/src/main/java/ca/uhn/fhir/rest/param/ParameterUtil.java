...
public class ParameterUtil {
     ...
     * href="http://www.hl7.org/implement/standards/fhir/search.html#escaping">FHIR Specification
     * Escaping Section</a>
     */
        ...
        if (index != null) {
            ...
            if (isRi == usesHapiId) {
                throw new ConfigurationException(
                        Msg.code(1936)
                                + "Method uses the wrong Id datatype (IdDt / IdType) for the given"
                                + " context FHIR version: "
                                + theMethod.toString());
            }
        }
        return index;
    ...
    public static IQueryParameterOr<?> singleton(
            final IQueryParameterType theParam, final String theParamName) {
        return new IQueryParameterOr<IQueryParameterType>() {
            ...
            public void setValuesAsQueryTokens(
                ...
                theParam.setValueAsQueryToken(
                        theContext,
                        theParamName,
                        theParameters.getQualifier(),
                        theParameters.get(0));
            }
        };
    }

    static List<String> splitParameterString(
            String theInput, char theDelimiter, boolean theUnescapeComponents) {
    ...
    /**
     * Returns true if the value is :iterate or :recurse (the former name of :iterate) for an
     * _include parameter
     */
