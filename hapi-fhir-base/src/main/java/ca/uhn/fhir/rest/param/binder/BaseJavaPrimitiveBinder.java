...
abstract class BaseJavaPrimitiveBinder<T> implements IParamBinder<T> {
    ...
    @Override
    public List<IQueryParameterOr<?>> encode(FhirContext theContext, T theString)
        ...
        List<?> retValList =
                Collections.singletonList(ParameterUtil.singleton(new StringParam(retVal), null));
        ...
        if (theParams.size() > 1 || theParams.get(0).size() > 1) {
            throw new InvalidRequestException(
                    Msg.code(1955)
                            ...
                            + theName
                            + "'. This server is not configured to allow multiple (AND) values for"
                            + " this param.");
        }
