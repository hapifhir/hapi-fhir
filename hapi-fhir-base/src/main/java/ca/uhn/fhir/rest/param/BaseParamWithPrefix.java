...
public abstract class BaseParamWithPrefix<T extends BaseParam> extends BaseParam {
    ...
    private static final org.slf4j.Logger ourLog =
            org.slf4j.LoggerFactory.getLogger(BaseParamWithPrefix.class);
    ...
    /**
     * Eg. if this is invoked with "gt2012-11-02", sets the prefix to GREATER_THAN and returns
     * "2012-11-02"
     */
    String extractPrefixAndReturnRest(String theString) {
        ...
        if (offset > 0 && theString.length() == offset) {
            throw new DataFormatException(
                    Msg.code(1940) + MSG_PREFIX_INVALID_FORMAT + "\"" + theString + "\"");
        }
        ...
        if (!isBlank(prefix)) {
            ...
            if (myPrefix == null) {
                ...
                switch (prefix) {
                    ...
                    default:
                        throw new DataFormatException(
                                Msg.code(1941) + "Invalid prefix: \"" + prefix + "\"");
                }
                ourLog.warn(
                        "Date parameter has legacy prefix '{}' which has been removed from FHIR."
