...
public class NumericParamRangeUtilTest {
	...
	void testSignificantFigures(String theNumber, int theExpectedSigDigs) {
		...
		System.out.printf("%1$10s %2$2s %3$2s %4$2s %n", theNumber, bd.scale(), bd.precision(), theExpectedSigDigs);
	...
	private static Stream<Arguments> provideRangeParameters() {
		return Stream.of(
				...
				//				Search for all the observations with a value of 5.4(+/-0.05) mg where mg is understood as a UCUM
				// unit (system/code)
				...
				//				Search for all the observations with a value of 0.0054(+/-0.000005) g where g is understood as a
				// UCUM unit (system/code)
