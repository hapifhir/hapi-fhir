...
public class NumericParamRangeUtilTest {
	...
	void testObtainedRange(String theNumber, Pair<Double, Double> theExpectedRange) {
		...
		System.out.printf(
					"%1$10s %2$2s  %3$2s :: %4$8f %5$8f :: %6$8f %7$8f%n",
					theNumber,
					bd.scale(),
					bd.precision(),
					theExpectedRange.getLeft(),
					theExpectedRange.getRight(),
					range.getLeft(),
					range.getRight());
	...
	private static Stream<Arguments> provideRangeParameters() {
		return Stream.of(
					...
					Arguments.of(
								"100.00",
								Pair.of(99.995d, 100.005d)), // 100 +/- 0.005  (0.5e-2 = 0.5e(3-5) )
					...
					//				Search for all the observations with a value of 5.4(+/-0.05) mg where mg is
					// understood as a UCUM unit (system/code)
					...
					//				Search for all the observations with a value of 0.0054(+/-0.000005) g where g
					// is understood as a UCUM unit (system/code)
					Arguments.of(
								"5.40e-3",
								Pair.of(0.005395d, 0.005405d)) // 0.0054 +/- 0.000005 (=0.5e5 =0.5e(-3-2) )
					);
	}
