...
public class CoordCalculatorTest {
	...
	public void testCHINToUHN() {
		GeoPoint result = CoordCalculator.findTarget(
				CoordCalculatorTestUtil.LATITUDE_CHIN,
				CoordCalculatorTestUtil.LONGITUDE_CHIN,
				CoordCalculatorTestUtil.BEARING_CHIN_TO_UHN,
				CoordCalculatorTestUtil.DISTANCE_KM_CHIN_TO_UHN);
	...
	@Test
	public void testBox() {
		GeoBoundingBox box = CoordCalculator.getBox(
				CoordCalculatorTestUtil.LATITUDE_CHIN, CoordCalculatorTestUtil.LONGITUDE_CHIN, 1.0);
		double expectedLatitudeDelta = 0.0090;
		assertEquals(
				CoordCalculatorTestUtil.LATITUDE_CHIN - expectedLatitudeDelta,
		...
		assertEquals(
				CoordCalculatorTestUtil.LONGITUDE_CHIN - expectedLongitudeDelta,
	...
	public void testOnPrimeMeridian() {
		...
		assertEquals(
				meridianLongitide + expectedLongitudeDelta, box.bottomRight().longitude(), 0.0001);
	}

	@Test
	public void testOnAntiMeridian() {
		GeoBoundingBox box = CoordCalculator.getBox(
				CoordCalculatorTestUtil.LATITUDE_TAVEUNI, CoordCalculatorTestUtil.LONGITIDE_TAVEUNI, 100.0);
		double expectedLatitudeDelta = 0.90;
		assertEquals(
				CoordCalculatorTestUtil.LATITUDE_TAVEUNI - expectedLatitudeDelta,
				box.bottomRight().latitude(),
				0.01);
		assertEquals(
				CoordCalculatorTestUtil.LATITUDE_TAVEUNI + expectedLatitudeDelta,
		...
		assertEquals(
				CoordCalculatorTestUtil.LONGITIDE_TAVEUNI - expectedLongitudeDelta,
				box.topLeft().longitude(),
				0.01);
		// This case wraps
		assertEquals(
				CoordCalculatorTestUtil.LONGITIDE_TAVEUNI + expectedLongitudeDelta - 360.0,
				box.bottomRight().longitude(),
				0.01);
	}
}
