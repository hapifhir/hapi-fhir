...
// TODO: Is there a better home for this test? It can't live in hapi-fhir-server since we need a
// real FhirContext for the compartment checks.
@MockitoSettings
class FhirQueryRuleImplTest implements ITestDataBuilder {

	private final TestRuleApplier myMockRuleApplier =
				new TestRuleApplier() {
					@Override
	...
	LogbackCaptureTestExtension myLogCapture =
				new LogbackCaptureTestExtension(myMockRuleApplier.getTroubleshootingLog().getName());
	...
	@Mock private IAuthorizationSearchParamMatcher myMatcher;
	...
	public class MatchingLogic {
		...
		@Test
		public void
					inCompartmentwithQueryFilter_resourceNotInCompartmentButMatchFilter_noVerdict() {
				// given
				withPatientWithNameAndId();
				// create patient for observation to point to so that the observation isn't in our main
				// patient compartment
				...
				myRule =
						new RuleBuilder()
									...
									.inCompartmentWithFilter(
												"patient",
												myPatient.getIdElement().withResourceType("Patient"),
												"code=28521000087105")
									.andThen()
									.build()
									.get(0);
		...
		public void allowIdwithQueryFilter_matchesIdAndFilter_allow() {
				...
				myRule =
						new RuleBuilder()
									.allow()
									.read()
									.instance(myPatient.getIdElement())
	...
	private void stubMatcherCall(
				String expectedQuery,
				IBaseResource theTargetResource,
				IAuthorizationSearchParamMatcher.MatchResult theStubResult) {
		when(myMatcher.match(
								ArgumentMatchers.eq(expectedQuery),
								ArgumentMatchers.same(theTargetResource)))
	...
	public class MisconfigurationChecks {
		/**
			* in case an unsupported perm snuck through the front door. Each scope provides positive
			* perm, so unsupported means we can't vote yes. Abstain.
			*/
		@Test
				...
				myRule =
						new RuleBuilder()
									...
									.inCompartmentWithFilter(
												"patient",
												myPatient.getIdElement().withResourceType("Patient"),
												"unsupported.chain=smi")
									.andThen()
									.build()
									.get(0);
				stubMatcherCall(
						"Patient?unsupported.chain=smi",
						myPatient,
						buildUnsupported("I'm broken unsupported chain XXX"));
		...
		public void givenDenyRule_whenUnsupportedQuery_reject() {
				...
				myRule =
						new RuleBuilder()
									...
									.resourcesOfType("Patient")
									.inCompartmentWithFilter(
												"patient", myPatientId, "unsupported.chain=smi")
									.andThen()
									.build()
									.get(0);
				stubMatcherCall(
		...
		/**
			* for backwards compatibility, if the IRuleApplier doesn't provide a matcher service, log a
			* warning, and return no verdict.
			*/
		@Test
	...
	private void withObservationWithSubjectAndCode(IIdType theIdElement) {
		...
		myObservation =
					buildResource(
								"Observation",
								withObservationCode(snomedUriString, insulin2hCode),
								withSubject(theIdElement));
	}
