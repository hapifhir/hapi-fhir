...
public class Icd10CmLoaderTest {
    ...
    public static void beforeClass() throws IOException, SAXException {
        StringReader reader =
                new StringReader(ClasspathUtil.loadResource("icd/icd10cm_tabular_2021.xml"));
    ...
    public void testLoadIcd10CmCheckRootConcepts() {
        ...
        List<String> conceptNames =
                rootConcepts.stream().map(t -> t.getCode()).collect(Collectors.toList());
        assertThat(
                conceptNames.toString(),
                conceptNames,
                Matchers.contains("A00", "A01", "H40", "R40"));
    }
    ...
    public void testLoadIcd10CmCheckChildCode() {
        ...
        assertEquals(
                "Cholera due to Vibrio cholerae 01, biovar cholerae", firstChildCode.getDisplay());
        List<String> conceptNames =
                rootConcepts.get(0).getChildCodes().stream()
                        .map(t -> t.getCode())
                        .collect(Collectors.toList());
        assertThat(
                conceptNames.toString(),
                conceptNames,
                Matchers.contains("A00.0", "A00.1", "A00.9"));
    }
    ...
    public void testLoadIcd10CmCheckExtendedChildCode() {
        ...
        List<String> conceptNames =
                rootConcepts.get(2).getChildCodes().get(0).getChildCodes().stream()
                        .map(t -> t.getCode())
                        .collect(Collectors.toList());
        assertThat(
                conceptNames.toString(),
