...
public class CalculateHashesTest extends BaseTest {
    ...
    public void testCreateHashes(Supplier<TestDatabaseDetails> theTestDatabaseDetails) {
        ...
        executeSql(
                "create table HFJ_SPIDX_TOKEN (SP_ID bigint not null, SP_MISSING boolean, SP_NAME"
                        + " varchar(100) not null, RES_ID bigint, RES_TYPE varchar(255) not null,"
                        + " SP_UPDATED timestamp, HASH_IDENTITY bigint, HASH_SYS bigint,"
                        + " HASH_SYS_AND_VALUE bigint, HASH_VALUE bigint, SP_SYSTEM varchar(200),"
                        + " SP_VALUE varchar(200), primary key (SP_ID))");
        executeSql(
                "insert into HFJ_SPIDX_TOKEN (SP_MISSING, SP_NAME, RES_ID, RES_TYPE, SP_UPDATED,"
                    + " SP_SYSTEM, SP_VALUE, SP_ID) values (false, 'identifier', 999, 'Patient',"
                    + " '2018-09-03 07:44:49.196', 'urn:oid:1.2.410.100110.10.41308301',"
                    + " '88888888', 1)");
        executeSql(
                "insert into HFJ_SPIDX_TOKEN (SP_MISSING, SP_NAME, RES_ID, RES_TYPE, SP_UPDATED,"
                    + " SP_SYSTEM, SP_VALUE, SP_ID) values (false, 'identifier', 999, 'Patient',"
                        ...
                        t -> {
                            ...
                            map =
                                    jdbcTemplate.queryForMap(
                                            "select * from HFJ_SPIDX_TOKEN where SP_ID = 1");
                            ...
                            map =
                                    jdbcTemplate.queryForMap(
                                            "select * from HFJ_SPIDX_TOKEN where SP_ID = 2");
                            assertEquals(7001889285610424179L, map.get("HASH_IDENTITY"));
                            assertEquals(2686400398917843456L, map.get("HASH_SYS"));
    ...
    public void testCreateHashesLargeNumber(Supplier<TestDatabaseDetails> theTestDatabaseDetails) {
        ...
        for (int i = 0; i < 777; i++) {
            executeSql(
                    "insert into HFJ_SPIDX_TOKEN (SP_MISSING, SP_NAME, RES_ID, RES_TYPE,"
                        + " SP_UPDATED, SP_SYSTEM, SP_VALUE, SP_ID) values (false, 'identifier',"
                        + " 999, 'Patient', '2018-09-03 07:44:49.196',"
                        + " 'urn:oid:1.2.410.100110.10.41308301', '8888888"
                            + i
                            + "', "
                            + i
                            + ")");
                        ...
                        .execute(
                                t -> {
                                    JdbcTemplate jdbcTemplate =
                                            getConnectionProperties().newJdbcTemplate();
                                    return jdbcTemplate.queryForObject(
                                            "SELECT count(*) FROM HFJ_SPIDX_TOKEN WHERE HASH_VALUE"
                                                    + " IS NULL",
                                            Long.class);
                                });
