...
public class SearchQueryBuilderDialectMySqlTest extends BaseSearchQueryBuilderDialectTest {
    ...
    public void testAddSortStringNoNullOrder() {
        ...
        //		assertTrue(generatedSql.getSql().endsWith("ORDER BY CASE WHEN t1.SP_VALUE_NORMALIZED IS
        // NULL THEN 1 ELSE 0 END ASC, t1.SP_VALUE_NORMALIZED ASC limit ?"));
    ...
    private GeneratedSql buildSqlWithStringSort(
            Boolean theAscending, OrderObject.NullOrder theNullOrder) {
        ...
        when(mySqlObjectFactory.stringIndexTable(any()))
                .thenReturn(new StringPredicateBuilder(searchQueryBuilder));

        BaseJoiningPredicateBuilder firstPredicateBuilder =
                searchQueryBuilder.getOrCreateFirstPredicateBuilder();
        StringPredicateBuilder sortPredicateBuilder =
                searchQueryBuilder.addStringPredicateBuilder(
                        firstPredicateBuilder.getResourceIdColumn());

        Condition hashIdentityPredicate =
                sortPredicateBuilder.createHashIdentityPredicate("patient", "family");
        searchQueryBuilder.addPredicate(hashIdentityPredicate);
        if (theNullOrder == null) {
            searchQueryBuilder.addSortString(
                    sortPredicateBuilder.getColumnValueNormalized(), theAscending);
        } else {
            searchQueryBuilder.addSortString(
                    sortPredicateBuilder.getColumnValueNormalized(),
                    theAscending,
                    theNullOrder,
                    false);
        }
    ...
    public void testAddSortStringWithNullOrder() {
        ...
        //		assertTrue(generatedSql.getSql().endsWith("ORDER BY CASE WHEN t1.SP_VALUE_NORMALIZED IS
        // NULL THEN 1 ELSE 0 END DESC, t1.SP_VALUE_NORMALIZED DESC limit ?"));
        assertTrue(generatedSql.getSql().endsWith("ORDER BY t1.SP_VALUE_NORMALIZED DESC limit ?"));
    ...
    public void testAddSortDateNoNullOrder() {
        ...
        //		assertTrue(generatedSql.getSql().endsWith("ORDER BY CASE WHEN t1.SP_VALUE_LOW IS NULL
        // THEN 1 ELSE 0 END ASC, t1.SP_VALUE_LOW ASC limit ?"));
        assertTrue(generatedSql.getSql().endsWith("ORDER BY t1.SP_VALUE_LOW ASC limit ?"));
    ...
    private GeneratedSql buildSqlWithDateSort(
            Boolean theAscending, OrderObject.NullOrder theNullOrder) {
        SearchQueryBuilder searchQueryBuilder = createSearchQueryBuilder();
        when(mySqlObjectFactory.resourceTable(any()))
                .thenReturn(new ResourceTablePredicateBuilder(searchQueryBuilder));
        ...
        DatePredicateBuilder sortPredicateBuilder =
                searchQueryBuilder.addDatePredicateBuilder(
                        firstPredicateBuilder.getResourceIdColumn());

        Condition hashIdentityPredicate =
                sortPredicateBuilder.createHashIdentityPredicate("patient", "birthdate");
        searchQueryBuilder.addPredicate(hashIdentityPredicate);
        if (theNullOrder == null) {
            searchQueryBuilder.addSortDate(sortPredicateBuilder.getColumnValueLow(), theAscending);
        } else {
            searchQueryBuilder.addSortDate(
    ...
    public void testAddSortDateWithNullOrder() {
        ...
        //		assertTrue(generatedSql.getSql().endsWith("ORDER BY CASE WHEN t1.SP_VALUE_LOW IS NULL
        // THEN 1 ELSE 0 END DESC, t1.SP_VALUE_LOW DESC limit ?"));
        assertTrue(generatedSql.getSql().endsWith("ORDER BY t1.SP_VALUE_LOW DESC limit ?"));
