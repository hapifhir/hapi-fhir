...
public class SearchQueryBuilderDialectPostgresTest extends BaseSearchQueryBuilderDialectTest {
	...
	/** Make sure we're using integers and not strings as bind variables for ordinals */
	@Test
	public void testOrdinalSearchesUseIntegerParameters() {
		...
		when(mySqlObjectFactory.dateIndexTable(any()))
					.thenReturn(new DatePredicateBuilder(searchQueryBuilder));

		DatePredicateBuilder datePredicateBuilder =
					searchQueryBuilder.addDatePredicateBuilder(null);
		datePredicateBuilder.setStorageSettingsForUnitTest(storageSettings);

		Condition datePredicate =
					datePredicateBuilder.createPredicateDateWithoutIdentityPredicate(
								new DateParam("2022"), SearchFilterParser.CompareOperation.eq);
		Condition comboPredicate =
					datePredicateBuilder.combineWithHashIdentityPredicate(
								"Observation", "date", datePredicate);
		...
		assertEquals(
					"SELECT t0.RES_ID FROM HFJ_SPIDX_DATE t0 WHERE ((t0.HASH_IDENTITY = ?) AND"
						+ " ((t0.SP_VALUE_LOW_DATE_ORDINAL >= ?) AND (t0.SP_VALUE_HIGH_DATE_ORDINAL <="
						+ " ?))) limit ?",
					sql);
