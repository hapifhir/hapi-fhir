 ...
 *
 * <p>Intended to be nested in a context that provides a ITestDataBuilder.Support and TestDaoSearch.
 */
public abstract class CompositeSearchParameterTestCases implements ITestDataBuilder.WithSupport {
     ...
     *
     * <p>JPA currently reuses the extracted values from each sub-param. This works fine for
     * non-repeating elements, but can fail for repeating elements like Observation.component. For
     * the composites defined on Observation.component, both sub-params must match on the specific
     * component (eg systolic vs diasystolic readings), and JPA doesn't track down to the element
     * level.
     */
    ...
    void searchCodeQuantity_onSameComponent_found() {
        ...
        myTestDaoSearch.assertSearchFinds(
                ...
                "Observation?component-code-value-quantity=8480-6$60",
                id1);
    }
    ...
    void searchCodeQuantity_differentComponents_notFound() {
        ...
        List<String> ids =
                myTestDaoSearch.searchForIds(
                        "Observation?component-code-value-quantity=8480-6$100");
        assertThat(
                "Search for the value from one component, but the code from the other, so it"
                        + " shouldn't match",
                ids,
                empty());
    }
    ...
    void searchCodeCode_differentComponents_notFound() {
        createObservation(
                ...
                withObservationComponent(
                        withCodingAt("code.coding", SYSTEM_LOINC_ORG, CODE_3421_5, null),
                        withCodingAt(
                                "valueCodeableConcept.coding", SYSTEM_LOINC_ORG, "another-code")));

        List<String> ids =
                myTestDaoSearch.searchForIds(
                        "Observation?component-code-value-concept=8480-6$another-code");
        assertThat(
                "Search for the value from one component, but the code from the other, so it"
                        + " shouldn't match",
                ids,
    ...
    void searchCodeDate_onSameResource_found() {
        ...
        myTestDaoSearch.assertSearchFinds(
                "search matches both sps in composite",
                "Observation?code-value-date=8480-6$lt2021",
                id1);
    }
    ...
    void searchCodeString_onSameResource_found() {
        ...
        myTestDaoSearch.assertSearchFinds(
                "token code + string prefix matches",
                "Observation?code-value-string=8480-6$ABC",
                id1);
    }

    /**
     * Create a goofy SP to test composite component support for uri and number.
     *
     * <p>Note - JPA doesn't support this either yet. Just piggyback on this flag.
     */
    @EnabledIf("isCorrelatedSupported")
    @Test
    void searchUriNumber_onSameResource_found() {
        ...
        SearchParameter searchParameter =
                createCompositeSearchParameter("uri-number-compound-test", "RiskAssessment");
        searchParameter.addComponent(
                componentFrom(
                        "http://hl7.org/fhir/SearchParameter/Resource-source", "meta.source"));
        searchParameter.addComponent(
                componentFrom(
        ...
        // verify config
        myTestDaoSearch.assertSearchFinds(
                "simple uri search works",
                "RiskAssessment?_source=https://example.com/ourSource",
                raId);
        myTestDaoSearch.assertSearchFinds(
                "simple number search works", "RiskAssessment?probability=0.02", raId);
        ...
        myTestDaoSearch.assertSearchFinds(
                "combo uri + number",
                "RiskAssessment?_source=https://example.com/ourSource&probability=0.02",
                raId);
    }
    ...
    void testComboSearch_withTokenAndNumber_returnsMatchingResources(Extension theExtension) {
        ...
        SearchParameter searchParameter =
                createCompositeSearchParameter("token-number-combo-test", "RiskAssessment");
        searchParameter.addComponent(
                componentFrom(
        ...
        riskAssessmentNonMatch.setMethod(
                new CodeableConcept(new Coding(null, "NOT_FOUND_CODE", null)));
        riskAssessmentNonMatch.addPrediction(
                new RiskAssessment.RiskAssessmentPredictionComponent()
                        .setProbability(new DecimalType(0.03)));
        doCreateResource(riskAssessmentNonMatch);

        // verify combo query
        myTestDaoSearch.assertSearchFinds(
                "combo uri + number", "RiskAssessment?method=BRCAPRO&probability=0.02", raId);
        myTestDaoSearch.assertSearchNotFound(
    ...
    void testComboSearch_withUriAndString_returnsMatchingResources(Extension theExtension) {
        ...
        SearchParameter searchParameter =
                createCompositeSearchParameter("uri-string-combo-test", "Device");
        searchParameter.addComponent(
                componentFrom("http://hl7.org/fhir/SearchParameter/Device-url", "Device"));
        searchParameter.addComponent(
                componentFrom("http://hl7.org/fhir/SearchParameter/Device-model", "Device"));
        searchParameter.setExtension(List.of(theExtension));
        doCreateResource(searchParameter);
    ...
    private static SearchParameter createCompositeSearchParameter(
            String theCodeValue, String theBase) {
        ...
        return Stream.of(
                Arguments.of(
                        new Extension(
                                ...
                                new BooleanType(
                                        false))), // composite SP of type combo with non-unique
                // index
                Arguments.of(
                        new Extension(
                                HapiExtensions.EXT_SP_UNIQUE,
                                new BooleanType(
                                        true))) // composite SP of type combo with non-unique index
                );
    }
}
