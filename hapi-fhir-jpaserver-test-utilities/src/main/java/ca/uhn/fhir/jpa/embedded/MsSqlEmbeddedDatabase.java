 ...
 *
 * @see <a href="https://www.testcontainers.org/modules/databases/mssqlserver/">MS SQL Server
 *     TestContainer</a>
 */
public class MsSqlEmbeddedDatabase extends JpaEmbeddedDatabase {
	...
	public void enableConstraints() {
		...
		for (String tableName : getAllTableNames()) {
				sql.add(
						String.format(
									"ALTER TABLE \"%s\" WITH CHECK CHECK CONSTRAINT ALL;", tableName));
		}
		executeSqlAsBatch(sql);
	}
	...
	private void dropSequences() {
		List<String> sql = new ArrayList<>();
		List<Map<String, Object>> queryResults =
					query("SELECT name FROM SYS.SEQUENCES WHERE is_ms_shipped = 'false'");
		for (Map<String, Object> row : queryResults) {
	...
	private List<String> getAllTableNames() {
		...
		List<Map<String, Object>> queryResults =
					query("SELECT name FROM SYS.TABLES WHERE is_ms_shipped = 'false'");
		for (Map<String, Object> row : queryResults) {
