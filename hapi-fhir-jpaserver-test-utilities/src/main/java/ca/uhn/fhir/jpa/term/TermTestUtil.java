...
public final class TermTestUtil {
	...
	public static void addLoincMandatoryFilesAndSinglePartLinkToZip(ZipCollectionBuilder theFiles)
				throws IOException {
	...
	public static void addLoincMandatoryFilesAndConsumerNameAndLinguisticVariants(
				ZipCollectionBuilder theFiles) throws IOException {
		addBaseLoincMandatoryFilesToZip(theFiles, true);
		theFiles.addFileZip("/loinc/", "loincupload_singlepartlink.properties");
		theFiles.addFileZip("/loinc/", LOINC_PART_LINK_FILE_DEFAULT.getCode());
		...
		theFiles.addFileZip(
					"/loinc/",
					LOINC_LINGUISTIC_VARIANTS_PATH_DEFAULT.getCode() + "zhCN5LinguisticVariant.csv");
		theFiles.addFileZip(
					"/loinc/",
					LOINC_LINGUISTIC_VARIANTS_PATH_DEFAULT.getCode() + "deAT24LinguisticVariant.csv");
		theFiles.addFileZip(
					"/loinc/",
					LOINC_LINGUISTIC_VARIANTS_PATH_DEFAULT.getCode() + "frCA8LinguisticVariant.csv");
	}

	public static void addLoincMandatoryFilesToZip(ZipCollectionBuilder theFiles)
				throws IOException {
		addBaseLoincMandatoryFilesToZip(theFiles, true);
	...
	public static void addLoincMandatoryFilesWithoutTop2000ToZip(ZipCollectionBuilder theFiles)
				throws IOException {
	...
	static void addBaseLoincMandatoryFilesToZip(
				ZipCollectionBuilder theFiles, Boolean theIncludeTop2000) throws IOException {
		...
		theFiles.addFileZip(
					"/loinc/", LOINC_IEEE_MEDICAL_DEVICE_CODE_MAPPING_TABLE_FILE_DEFAULT.getCode());
		theFiles.addFileZip("/loinc/", LOINC_IMAGING_DOCUMENT_CODES_FILE_DEFAULT.getCode());
		if (theIncludeTop2000) {
				theFiles.addFileZip(
						"/loinc/", LOINC_TOP2000_COMMON_LAB_RESULTS_SI_FILE_DEFAULT.getCode());
				theFiles.addFileZip(
						"/loinc/", LOINC_TOP2000_COMMON_LAB_RESULTS_US_FILE_DEFAULT.getCode());
		}
	}

	static void verifyConsumerName(
				Collection<TermConceptDesignation> designationList, String theConsumerName) {
	...
	static void verifyLinguisticVariant(
		...
		verifyDesignation(
					longCommonNameDes, ITermLoaderSvc.LOINC_URI, "LONG_COMMON_NAME", theLongCommonName);
		verifyDesignation(
	...
	private static void verifyDesignation(
				TermConceptDesignation theDesignation,
				String theUseSystem,
				String theUseCode,
				String theValue) {
