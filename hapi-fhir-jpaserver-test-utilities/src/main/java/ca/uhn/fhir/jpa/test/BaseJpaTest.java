...
public abstract class BaseJpaTest extends BaseTest {
    ...
    private static final org.slf4j.Logger ourLog =
            org.slf4j.LoggerFactory.getLogger(BaseJpaTest.class);
    ...
    @Autowired protected JpaStorageSettings myStorageSettings = new JpaStorageSettings();
    @Autowired protected DatabaseBackedPagingProvider myDatabaseBackedPagingProvider;
    @Autowired protected IInterceptorService myInterceptorRegistry;
    @Autowired protected CircularQueueCaptureQueriesListener myCaptureQueriesListener;
    @Autowired protected ISearchResultCacheSvc mySearchResultCacheSvc;
    @Autowired protected ITermCodeSystemDao myTermCodeSystemDao;
    @Autowired protected ITermCodeSystemVersionDao myTermCodeSystemVersionDao;
    @Autowired protected ISearchCacheSvc mySearchCacheSvc;
    @Autowired protected IPartitionLookupSvc myPartitionConfigSvc;
    ...
    @Autowired protected IResourceLinkDao myResourceLinkDao;
    @Autowired protected IResourceIndexedSearchParamTokenDao myResourceIndexedSearchParamTokenDao;
    @Autowired protected IResourceIndexedSearchParamNumberDao myResourceIndexedSearchParamNumberDao;
    @Autowired protected IResourceIndexedSearchParamUriDao myResourceIndexedSearchParamUriDao;
    @Autowired protected IResourceIndexedSearchParamStringDao myResourceIndexedSearchParamStringDao;
    @Autowired protected IResourceIndexedSearchParamDateDao myResourceIndexedSearchParamDateDao;
    @Autowired protected IResourceIndexedSearchParamCoordsDao myResourceIndexedSearchParamCoordsDao;
    ...
    @Autowired protected ITermConceptDao myTermConceptDao;
    @Autowired protected ITermValueSetConceptDao myTermValueSetConceptDao;
    @Autowired protected ITermValueSetDao myTermValueSetDao;
    @Autowired protected ITermConceptDesignationDao myTermConceptDesignationDao;
    @Autowired protected ITermConceptPropertyDao myTermConceptPropertyDao;
    ...
    @Autowired private FhirInstanceValidator myFhirInstanceValidator;
    @Autowired private IResourceTableDao myResourceTableDao;
    @Autowired private IResourceTagDao myResourceTagDao;
    @Autowired private IResourceHistoryTableDao myResourceHistoryTableDao;
    ...
    protected static void purgeDatabase(
        ...
        for (int count = 0; ; count++) {
            try {
                theSystemDao.expunge(
                        new ExpungeOptions().setExpungeEverything(true),
    ...
    public static void waitForSize(
            int theTarget, Callable<Number> theCallable, Callable<String> theFailureMessage)
            throws Exception {
    ...
    public static void waitForSize(
            ...
            Callable<Number> theCallable,
            Callable<String> theFailureMessage)
            throws Exception {
        await().alias(
                        "Waiting for size "
                                + theTarget
                                + ". Current size is "
                                + theCallable.call().intValue()
                                + ": "
                                + theFailureMessage.call())
    ...
    protected <T extends IBaseResource> T loadResourceFromClasspath(
            Class<T> type, String resourceName) throws IOException {
    ...
    public void afterPerformCleanup() {
        ...
        if (myJpaPersistedValidationSupport != null) {
            ProxyUtil.getSingletonTarget(
                            myJpaPersistedValidationSupport,
                            JpaPersistedResourceValidationSupport.class)
                    .clearCaches();
        }
    ...
    public void afterValidateNoTransaction() {
        ...
        if (txManager instanceof JpaTransactionManager) {
            ...
            SessionFactory sessionFactory =
                    (SessionFactory) hibernateTxManager.getEntityManagerFactory();
    ...
    public void beforeInitMocks() throws Exception {
        ...
        when(mySrd.getHeaders(eq(JpaConstants.HEADER_META_SNAPSHOT_MODE)))
    ...
    protected CountDownLatch registerLatchHookInterceptor(int theCount, Pointcut theLatchPointcut) {
        ...
        myInterceptorRegistry.registerAnonymousInterceptor(
                theLatchPointcut, Integer.MAX_VALUE, interceptor);
        ...
        return runInTransaction(
                () -> {
                    ...
                    ourLog.info(
                            ...
                            resources.stream()
                                    .map(TermConceptDesignation::toString)
                                    .collect(Collectors.joining("\n * ")));
                    return resources.size();
                });
    }
        ...
        return runInTransaction(
                () -> {
                    ...
                    ourLog.info(
                            "Concept Designations:\n * {}",
                            resources.stream()
                                    .map(TermConceptProperty::toString)
                                    .collect(Collectors.joining("\n * ")));
                    return resources.size();
                });
    }
        ...
        return runInTransaction(
                () -> {
                    ...
                    ourLog.info(
                            "Concepts:\n * {}",
                            resources.stream()
                                    .map(TermValueSetConcept::toString)
                                    .collect(Collectors.joining("\n * ")));
                    return resources.size();
                });
    }
    ...
    protected void logAllStringIndexes(String... theParamNames) {
        String messageSuffix =
                theParamNames.length > 0 ? " containing " + Arrays.asList(theParamNames) : "";
        runInTransaction(
                () -> {
                    String message =
                            myResourceIndexedSearchParamStringDao.findAll().stream()
                                    .filter(
                                            t ->
                                                    theParamNames.length == 0
                                                            ? true
                                                            : Arrays.asList(theParamNames)
                                                                    .contains(t.getParamName()))
                                    .map(t -> t.toString())
                                    .collect(Collectors.joining("\n * "));
    ...
    protected void logAllResourceTags() {
        runInTransaction(
                () -> {
                    ourLog.info(
                            "Token tags:\n * {}",
                            myResourceTagDao.findAll().stream()
                                    .map(t -> t.toString())
                                    .collect(Collectors.joining("\n * ")));
                });
    }
    ...
    protected List<String> toUnqualifiedVersionlessIdValues(
        ...
        if (theFirstCall) {
            ...
            bundleProvider =
                    myDatabaseBackedPagingProvider.retrieveResultList(null, theFound.getUuid());
        }
    ...
    protected List<String> toUnqualifiedVersionlessIdValues(
            List<? extends IBaseResource> theFound) {
        List<String> retVal = new ArrayList<>();
        for (IBaseResource next : theFound) {
            retVal.add(next.getIdElement().toUnqualifiedVersionless().getValue());
        }
    ...
    protected TermValueSetConcept assertTermValueSetContainsConceptAndIsInDeclaredOrder(
        ...
        if (theDesignationCount != null) {
            stream =
                    stream.filter(
                            concept -> concept.getDesignations().size() == theDesignationCount);
        }
        ...
        if (!first.isPresent()) {
            String failureMessage =
                    String.format(
                            "Expanded ValueSet %s did not contain concept [%s|%s|%s] with [%d]"
                                    + " designations",
                            theValueSet.getId(),
                            theSystem,
                            theCode,
            ...
            assertEquals(
                    termValueSetConcept.getOrder(),
                    theValueSet.getConcepts().indexOf(termValueSetConcept));
    ...
    protected TermValueSetConceptDesignation assertTermConceptContainsDesignation(
        ...
        if (theLanguage != null) {
            stream =
                    stream.filter(
                            designation -> theLanguage.equalsIgnoreCase(designation.getLanguage()));
        }
        if (theUseSystem != null) {
            stream =
                    stream.filter(
                            designation ->
                                    theUseSystem.equalsIgnoreCase(designation.getUseSystem()));
        }
        if (theUseCode != null) {
            stream =
                    stream.filter(
                            designation -> theUseCode.equalsIgnoreCase(designation.getUseCode()));
        }
        if (theUseDisplay != null) {
            stream =
                    stream.filter(
                            designation ->
                                    theUseDisplay.equalsIgnoreCase(designation.getUseDisplay()));
        }
        if (theDesignationValue != null) {
            stream =
                    stream.filter(
                            designation ->
                                    theDesignationValue.equalsIgnoreCase(designation.getValue()));
        }
    ...
    /**
     * Asserts that the resource with {@literal theId} does not exist (i.e. not that it exists but
     * that it was deleted, but rather that the ID doesn't exist at all). This can be used to test
     * that a resource was expunged.
     */
