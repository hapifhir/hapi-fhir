...
public class SearchParamRegistryImplTest {
	...
	static {
		...
		ourBuiltinPatientSearchParamCount =
					ReadOnlySearchParamCache.fromFhirContext(
										ourFhirContext, new SearchParameterCanonicalizer(ourFhirContext))
	...
	@Autowired SearchParamRegistryImpl mySearchParamRegistry;
	@Autowired private ResourceChangeListenerRegistryImpl myResourceChangeListenerRegistry;
	...
	@MockBean private SearchParamExtractorService mySearchParamExtractorService;
	@MockBean private IndexedSearchParamExtractor myIndexedSearchParamExtractor;
	...
	public void before() {
		myAnswerCount = 0;
		when(myResourceVersionSvc.getVersionMap(anyString(), any()))
					.thenReturn(ourResourceVersionMap);
		...
		assertEquals(
					TEST_SEARCH_PARAMS,
					myResourceChangeListenerRegistry.getResourceVersionCacheSizeForUnitTest());
		assertDbCalled();
		assertEquals(
	...
	void handleInit() {
		...
		SearchParameter goodSearchParam =
					buildSearchParameter(Enumerations.PublicationStatus.ACTIVE);
	...
	private void assertPatientSearchParamSize(int theExpectedSize) {
		assertEquals(
					theExpectedSize, mySearchParamRegistry.getActiveSearchParams("Patient").size());
	}
	...
	public void testGetActiveSearchParamsRetries() {
		...
		assertEquals(
					ourBuiltInSearchParams.getSearchParamMap("Patient").size(),
					activeSearchParams.size());
	}
	...
	public void testUpliftRefchains() {
		...
		upliftRefChain.addExtension(
					...
					new StringType("element1"));
		Extension upliftRefChain2 =
					sp.addExtension().setUrl(HapiExtensions.EXTENSION_SEARCHPARAM_UPLIFT_REFCHAIN);
		...
		ResourceVersionMap newResourceVersionMap =
					ResourceVersionMap.fromResourceTableEntities(newEntities);
		when(myResourceVersionSvc.getVersionMap(anyString(), any()))
					.thenReturn(newResourceVersionMap);
		when(mySearchParamProvider.search(any())).thenReturn(new SimpleBundleProvider(sp));

		mySearchParamRegistry.forceRefresh();

		RuntimeSearchParam canonicalSp =
	...
	private void resetMock(
				Enumerations.PublicationStatus theStatus, List<ResourceTable> theNewEntities) {
		ResourceVersionMap resourceVersionMap =
					ResourceVersionMap.fromResourceTableEntities(theNewEntities);
		...
		when(mySearchParamProvider.search(any()))
					.thenReturn(new SimpleBundleProvider(buildSearchParameter(theStatus)));
	}
	...
	static class SpringConfig {
		...
		InMemoryResourceMatcher inMemoryResourceMatcher() {
				...
				when(retval.canBeEvaluatedInMemory(any(), any()))
