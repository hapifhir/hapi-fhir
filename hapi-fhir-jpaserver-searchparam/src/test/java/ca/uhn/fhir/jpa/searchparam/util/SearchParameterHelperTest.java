...
class SearchParameterHelperTest {
	...
	@Mock private SearchParameterCanonicalizer mockedCanonicalizer;
	@Mock private IBaseResource mockedSearchParam;
	...
	void whenParamNonCanonicalizableReturnsEmpty() {
		...
		Optional<SearchParameterMap> result =
					myTestedHelper.buildSearchParameterMapFromCanonical(mockedSearchParam);
	...
	void whenParamCanonicalizableReturnsFromCanonical() {
		...
		when(mockedCanonicalizer.canonicalizeSearchParameter(mockedSearchParam))
					.thenReturn(mockedRuntimeSearchParam);
