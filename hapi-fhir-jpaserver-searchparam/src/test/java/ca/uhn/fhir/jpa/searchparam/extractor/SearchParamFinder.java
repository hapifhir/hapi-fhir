...
public class SearchParamFinder {
            ...
            for (RuntimeSearchParam nextSp : nextResDef.getSearchParams()) {
                ...
                if (nextSp.getParamType() == theWantType) {
                    for (String nextPath : nextSp.getPathsSplit()) {
                        ...
                        List<String> pathsPart =
                                new ArrayList<>(Arrays.asList(nextPath.split("\\.")));
        ...
        if (theDef == null) {
            ...
            try {
                ...
                throw new IllegalArgumentException(
                        "Unknown resource: " + theFullPath + " for " + theSearchParam);
            }
            traverse(
                    theCtx,
                    theSearchParam,
            ...
            if (child != null) {
                ...
                Validate.notNull(
                        subChild, "Unknown child: " + nextName + " in path: " + theFullPath);
                ...
                BaseRuntimeElementCompositeDefinition def =
                        (BaseRuntimeElementCompositeDefinition) subChild;
                traverse(
                        theCtx,
                        theSearchParam,
                        theFullPath,
            ...
            if (choiceChild != null) {
                for (String nextChildName : choiceChild.getValidChildNames()) {
                    if (nextChildName.startsWith(nextName)) {
                        BaseRuntimeElementDefinition<?> subChild =
                                choiceChild.getChildByName(nextChildName);
                        if (subChild instanceof RuntimePrimitiveDatatypeDefinition) {
                            assert thePathsPart.size() == 1;
                            ourLog.info(
                                    "Found {} - {} - {} - {}",
    ...
    private static void handle(
            FhirContext theCtx, RuntimeSearchParam theSearchParam, String theName) {
