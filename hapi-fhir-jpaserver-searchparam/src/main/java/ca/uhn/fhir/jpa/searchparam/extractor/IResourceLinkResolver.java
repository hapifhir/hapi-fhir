...
public interface IResourceLinkResolver {
    ...
    /**
     * This method resolves the target of a reference found within a resource that is being
     * created/updated. We do this so that we can create indexed links between resources, and so
     * that we can validate that the target actually exists in cases where we need to check that.
     *
     * <p>This method returns an {@link IResourceLookup} to avoid needing to resolve the entire
     * resource.
     *
