...
public enum JpaParamUtil {
    ...
    public static IQueryParameterAnd<?> parseQueryParams(
        ...
        switch (paramType) {
            ...
            case DATE:
                binder =
                        new QueryParameterAndBinder(
                                DateAndListParam.class, Collections.emptyList());
                break;
            case NUMBER:
                binder =
                        new QueryParameterAndBinder(
                                NumberAndListParam.class, Collections.emptyList());
                break;
            case QUANTITY:
                binder =
                        new QueryParameterAndBinder(
                                QuantityAndListParam.class, Collections.emptyList());
                break;
            case REFERENCE:
                binder =
                        new QueryParameterAndBinder(
                                ReferenceAndListParam.class, Collections.emptyList());
                break;
            case STRING:
                binder =
                        new QueryParameterAndBinder(
                                StringAndListParam.class, Collections.emptyList());
                break;
            case TOKEN:
                binder =
                        new QueryParameterAndBinder(
                                TokenAndListParam.class, Collections.emptyList());
                break;
            case URI:
                binder =
                        new QueryParameterAndBinder(UriAndListParam.class, Collections.emptyList());
                break;
            case HAS:
                binder =
                        new QueryParameterAndBinder(HasAndListParam.class, Collections.emptyList());
                break;
            case SPECIAL:
                binder =
                        new QueryParameterAndBinder(
                                SpecialAndListParam.class, Collections.emptyList());
                break;
            default:
                throw new IllegalArgumentException(
                        Msg.code(497)
                                + "Parameter '"
                                + theUnqualifiedParamName
                                + "' has type "
                                + paramType
    ...
    public static IQueryParameterAnd<?> parseQueryParams(
        ...
        if (paramType == RestSearchParameterTypeEnum.COMPOSITE) {
            ...
            List<RuntimeSearchParam> compositeList =
                    resolveComponentParameters(theSearchParamRegistry, theParamDef);
            ...
            CompositeAndListParam<IQueryParameterType, IQueryParameterType> cp =
                    new CompositeAndListParam(
    ...
    public static List<RuntimeSearchParam> resolveCompositeComponentsDeclaredOrder(
        ...
        for (RuntimeSearchParam.Component next : components) {
            ...
            RuntimeSearchParam componentParam =
                    theSearchParamRegistry.getActiveSearchParamByUrl(url);
            if (componentParam == null) {
                throw new InternalErrorException(
                        Msg.code(499) + "Can not find SearchParameter: " + url);
            }
