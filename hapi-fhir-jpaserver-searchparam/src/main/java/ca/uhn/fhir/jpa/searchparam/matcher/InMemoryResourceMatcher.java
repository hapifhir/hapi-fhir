...
public class InMemoryResourceMatcher {
    ...
    public static final Set<String> UNSUPPORTED_PARAMETER_NAMES =
            Sets.newHashSet(Constants.PARAM_HAS);
    private static final org.slf4j.Logger ourLog =
            LoggerFactory.getLogger(InMemoryResourceMatcher.class);
    ...
    @Autowired SearchParamExtractorService mySearchParamExtractorService;
    @Autowired IndexedSearchParamExtractor myIndexedSearchParamExtractor;
    ...
    /**
     * Lazy loads a {@link IValidationSupport} implementation just-in-time. If no suitable bean is
     * available, or if a {@link ca.uhn.fhir.context.ConfigurationException} is thrown, matching can
     * proceed, but the qualifiers that depend on the validation support will be disabled.
     *
    ...
    private IValidationSupport getValidationSupportOrNull() {
        if (validationSupportState == ValidationSupportInitializationState.NOT_INITIALIZED) {
            try {
                ...
                // We couldn't get a validation support bean, and we don't want to waste cycles
                // trying again
                ourLog.warn(
                        Msg.code(2100)
                                + "No bean satisfying IValidationSupport could be initialized."
                                + " Qualifiers dependent on IValidationSupport will not be"
                                + " supported.");
    ...
    /**
     * @deprecated Use {@link #match(String, IBaseResource, ResourceIndexedSearchParams,
     *     RequestDetails)}
     */
    @Deprecated
    ...
    /**
     * This method is called in two different scenarios. With a null theResource, it determines
     * whether database matching might be required. Otherwise, it tries to perform the match
     * in-memory, returning UNSUPPORTED if it's not possible.
     *
     * <p>Note that there will be cases where it returns UNSUPPORTED with a null resource, but when
     * a non-null resource it returns supported and no match. This is because an earlier parameter
     * may be matchable in-memory in which case processing stops and we never get to the parameter
     * that would have required a database call.
     *
     * @param theIndexedSearchParams If the search params have already been calculated for the given
     *     resource, they can be passed in. Passing in {@literal null} is also fine, in which case
     *     they will be calculated for the resource. It can be preferable to pass in {@literal null}
     *     unless you already actually had to calculate the indexes for another reason, since we can
     *     be efficient here and only calculate the params that are actually relevant for the given
     *     search expression.
     */
    public InMemoryMatchResult match(
            String theCriteria,
        ...
        try {
            searchParameterMap =
                    myMatchUrlService.translateMatchUrl(theCriteria, resourceDefinition);
        ...
        if (theIndexedSearchParams != null) {
            ...
            relevantSearchParams =
                    myIndexedSearchParamExtractor.extractIndexedSearchParams(
                            theResource, theRequestDetails, filter);
        }
    ...
    public InMemoryMatchResult match(
        ...
        for (Map.Entry<String, List<List<IQueryParameterType>>> entry :
    ...
    private InMemoryMatchResult matchIdsWithAndOr(
        ...
        RuntimeSearchParam paramDef =
                mySearchParamRegistry.getActiveSearchParam(resourceName, theParamName);
        ...
        switch (theParamName) {
            ...
            case Constants.PARAM_SOURCE:
                return InMemoryMatchResult.fromBoolean(
                        matchSourcesAndOr(theAndOrParams, theResource));
            case Constants.PARAM_TAG:
                return InMemoryMatchResult.fromBoolean(
                        matchTagsOrSecurityAndOr(theAndOrParams, theResource, true));
            case Constants.PARAM_SECURITY:
                return InMemoryMatchResult.fromBoolean(
                        matchTagsOrSecurityAndOr(theAndOrParams, theResource, false));
            case Constants.PARAM_PROFILE:
                return InMemoryMatchResult.fromBoolean(
                        matchProfilesAndOr(theAndOrParams, theResource));
            default:
    ...
    private InMemoryMatchResult checkForUnsupportedParameters(
        ...
        for (List<IQueryParameterType> orParams : theAndOrParams) {
            ...
            if (orParams.size() > 0) {
                // The params in each OR list all share the same qualifier, prefix, etc., so we only
                // need to check the first one
                InMemoryMatchResult checkUnsupportedResult =
                        checkOneParameterForUnsupportedModifiers(
                                theParamName, theParamDef, orParams.get(0));
                if (!checkUnsupportedResult.supported()) {
                    return checkUnsupportedResult;
                }
            }
        ...
        if (hasChain(theParam)) {
            checkUnsupportedResult =
                    InMemoryMatchResult.unsupportedFromParameterAndReason(
                            theParamName + "." + ((ReferenceParam) theParam).getChain(),
                            InMemoryMatchResult.CHAIN);
        }

        if (checkUnsupportedResult.supported()) {
            checkUnsupportedResult =
                    checkUnsupportedQualifiers(theParamName, theParamDef, theParam);
        }
    ...
    private boolean matchProfilesAndOr(
            List<List<IQueryParameterType>> theAndOrParams, IBaseResource theResource) {
        if (theResource == null) {
            return true;
        }
    ...
    private boolean matchProfilesOr(
            List<IQueryParameterType> theOrParams, IBaseResource theResource) {
    ...
    private boolean matchProfile(IQueryParameterType theProfileParam, IBaseResource theResource) {
        ...
        if (isBlank(paramProfileValue)) {
            ...
            return theResource.getMeta().getProfile().stream()
                    ...
                    .anyMatch(
                            profileValue ->
                                    profileValue != null && profileValue.equals(paramProfileValue));
        }
    }

    private boolean matchSourcesAndOr(
            List<List<IQueryParameterType>> theAndOrParams, IBaseResource theResource) {
        if (theResource == null) {
            return true;
        }
    ...
    private boolean matchSourcesOr(
            List<IQueryParameterType> theOrParams, IBaseResource theResource) {
    ...
    private boolean matchSource(IQueryParameterType theSourceParam, IBaseResource theResource) {
        SourceParam paramSource =
                new SourceParam(theSourceParam.getValueAsQueryToken(myFhirContext));
        SourceParam resourceSource =
                new SourceParam(MetaUtil.getSource(myFhirContext, theResource.getMeta()));
    ...
    private boolean matchTagsOrSecurityAndOr(
        ...
        return theAndOrParams.stream()
                .allMatch(nextAnd -> matchTagsOrSecurityOr(nextAnd, theResource, theTag));
    }
    ...
    private boolean matchTagsOrSecurityOr(
        ...
        return theOrParams.stream()
                .anyMatch(param -> matchTagOrSecurity(param, theResource, theTag));
    }

    private boolean matchTagOrSecurity(
            IQueryParameterType theParam, IBaseResource theResource, boolean theTag) {
    ...
    private boolean matchIdsOr(List<IQueryParameterType> theOrParams, IBaseResource theResource) {
        return theOrParams.stream()
                .anyMatch(
                        param ->
                                param instanceof StringParam
                                        && matchId(
                                                ((StringParam) param).getValue(),
    ...
    private boolean matchParams(
        ...
        // negative tests like :not and :not-in must not match any or-clause, so we invert the
        // quantifier.
        if (isNegativeTest) {
    ...
    private boolean isNegative(
            RuntimeSearchParam theParamDef, List<? extends IQueryParameterType> theOrList) {
    ...
    private boolean matchParam(
        ...
        if (theParamDef.getParamType().equals(RestSearchParameterTypeEnum.TOKEN)) {
            ...
            return theSearchParams.matchParam(
                    theStorageSettings, theResourceName, theParamName, theParamDef, theToken);
        }
    }

    /**
     * Checks whether a query parameter of type token matches one of the search parameters of an
     * in-memory resource. The :not modifier is supported. The :in and :not-in qualifiers are
     * supported only if a bean implementing IValidationSupport is available. Any other qualifier
     * will be ignored and the match will be treated as unqualified.
     *
    ...
    private boolean systemContainsCode(
            TokenParam theQueryParam, ResourceIndexedSearchParamToken theSearchParamToken) {
        IValidationSupport validationSupport = getValidationSupportOrNull();
        if (validationSupport == null) {
            ourLog.error(
                    Msg.code(2096)
                            + "Attempting to evaluate an unsupported qualifier. This should not"
                            + " happen.");
            return false;
        }
        ...
        if (theParamDef != null && theParam instanceof BaseParamWithPrefix) {
            ...
            if (!supportedPrefix(prefix, paramType)) {
                return InMemoryMatchResult.unsupportedFromParameterAndReason(
                        theParamName,
                        String.format(
                                "The prefix %s is not supported for param type %s",
                                prefix, paramType));
            }
        }
        return InMemoryMatchResult.successfulMatch();
    ...
    private boolean supportedPrefix(
            ParamPrefixEnum theParam, RestSearchParameterTypeEnum theParamType) {
    ...
    private InMemoryMatchResult checkUnsupportedQualifiers(
        ...
        if (hasQualifiers(theParam) && !supportedQualifier(theParamDef, theParam)) {
            return InMemoryMatchResult.unsupportedFromParameterAndReason(
                    theParamName + theParam.getQueryParameterQualifier(),
                    InMemoryMatchResult.QUALIFIER);
        }
        return InMemoryMatchResult.successfulMatch();
    }

    private boolean supportedQualifier(
            RuntimeSearchParam theParamDef, IQueryParameterType theParam) {
        ...
        switch (theParamDef.getParamType()) {
            case TOKEN:
                ...
                switch (tokenParam.getModifier()) {
                    case IN:
                    case NOT_IN:
                        // Support for these qualifiers is dependent on an implementation of
                        // IValidationSupport being available to delegate the check to
