...
/**
 * This class fully and unabashedly stolen from Hibernate search 5.11.4 FINAL's implementation as it
 * was stripped in HS6
 */
public class GeopointNormalizer {
    ...
    public static double normalizeLongitudeInclusive(double longitude) {
        if ((longitude < -(LONGITUDE_DEGREE_RANGE / 2))
                || (longitude > (LONGITUDE_DEGREE_RANGE / 2))) {
            double _longitude;
            // shift 180 and normalize full circle turn
            _longitude = ((longitude + (LONGITUDE_DEGREE_RANGE / 2)) % WHOLE_CIRCLE_DEGREE_RANGE);
            // as Java % is not a math modulus we may have negative numbers so the unshift is sign
            // dependant
            if (_longitude < 0) {
    ...
    public static double normalizeLatitude(double latitude) {
        if (latitude > LATITUDE_DEGREE_MAX || latitude < LATITUDE_DEGREE_MIN) {
            ...
            double _latitude =
                    Math.abs(
                            (latitude + (LATITUDE_DEGREE_RANGE / 2)) % (WHOLE_CIRCLE_DEGREE_RANGE));
