...
public class SearchParamExtractorService {
    private static final org.slf4j.Logger ourLog =
            org.slf4j.LoggerFactory.getLogger(SearchParamExtractorService.class);
     ...
     * I.e. for all search parameters defined for a given resource type, it extracts the associated
     * indexes and populates {@literal theParams}.
     */
    public void extractFromResource(
            RequestPartitionId theRequestPartitionId,
            RequestDetails theRequestDetails,
        ...
        extractSearchIndexParameters(
                theRequestDetails, normalParams, theResource, theSearchParamFilter);
        ...
        // Do this after, because we add to strings during both string and token processing, and
        // contained resource if any
        ...
        if (myStorageSettings.getIndexMissingFields() == StorageSettings.IndexEnabledEnum.ENABLED) {
            ...
            Map<String, Boolean> presenceMap =
                    getReferenceSearchParamPresenceMap(theEntity, theNewParams);
            presenceMap.forEach(
                    (key, value) -> {
            ...
            theNewParams.findMissingSearchParams(
                    myPartitionSettings, myStorageSettings, theEntity, activeSearchParams);
        }
    ...
    private Map<String, Boolean> getReferenceSearchParamPresenceMap(
        ...
        ResourceSearchParams activeSearchParams =
                mySearchParamRegistry.getActiveSearchParams(entity.getResourceType());
        activeSearchParams
    ...
    /**
     * Extract search parameter indexes for contained resources. E.g. if we are storing a Patient
     * with a contained Organization, we might extract a String index on the Patient with
     * paramName="organization.name" and value="Org Name"
     */
    private void extractSearchIndexParametersForContainedResources(
        ...
        Collection<IBaseResource> containedResources =
                terser.getAllEmbeddedResources(theResource, false);

        // Extract search parameters
        IChainedSearchParameterExtractionStrategy strategy =
                new IChainedSearchParameterExtractionStrategy() {
                    @Nonnull
                    @Override
                    public ISearchParamExtractor.ISearchParamFilter getSearchParamFilter(
                        ...
                        // be to make this configurable, perhaps with an optional extension you
                        // could
    ...
    /**
     * Extract search parameter indexes for uplifted refchains. E.g. if we are storing a Patient
     * with reference to an Organization and the "Patient:organization" SearchParameter declares an
     * uplifted refchain on the "name" SearchParameter, we might extract a String index on the
     * Patient with paramName="organization.name" and value="Org Name"
     */
        ...
        IChainedSearchParameterExtractionStrategy strategy =
                new IChainedSearchParameterExtractionStrategy() {
                    ...
                    public ISearchParamExtractor.ISearchParamFilter getSearchParamFilter(
                        ...
                        RuntimeSearchParam searchParam =
                                mySearchParamRegistry.getActiveSearchParam(
                                        theEntity.getResourceType(), searchParamName);
                    ...
                    public IBaseResource fetchResourceAtPath(@Nonnull PathAndRef thePathAndRef) {
                        ...
                        IBaseResource resolvedResource =
                                theTransactionDetails.getResolvedResource(reference);
                        ...
                        if (resolvedResource == null
                            ...
                            RequestPartitionId targetRequestPartitionId =
                                    determineResolverPartitionId(theRequestPartitionId);
                            resolvedResource =
                                    myResourceLinkResolver.loadTargetResource(
                            ...
                            if (resolvedResource != null) {
                                ...
                                theTransactionDetails.addResolvedResource(
                                        thePathAndRef.getRef().getReferenceElement(),
                                        resolvedResource);
                            }
                        }
    ...
    /**
     * Extract indexes for contained references as well as for uplifted refchains. These two types
     * of indexes are both similar special cases. Normally we handle chained searches
     * ("Patient?organization.name=Foo") using a join from the {@link ResourceLink} table (for the
     * "organization" part) to the {@link ResourceIndexedSearchParamString} table (for the "name"
     * part). But for both contained resource indexes and uplifted refchains we use only the {@link
     * ResourceIndexedSearchParamString} table to handle the entire "organization.name" part, or the
     * other similar tables for token, number, etc.
     *
     * @see #extractSearchIndexParametersForContainedResources(RequestDetails,
     *     ResourceIndexedSearchParams, IBaseResource, ResourceTable,
     *     ISearchParamExtractor.SearchParamSet)
     * @see #extractSearchIndexParametersForUpliftedRefchains(RequestDetails,
     *     ResourceIndexedSearchParams, ResourceTable, RequestPartitionId, TransactionDetails,
     *     ISearchParamExtractor.SearchParamSet)
     */
    private void extractSearchIndexParametersForTargetResources(
        ...
        for (PathAndRef nextPathAndRef : theIndexedReferences) {
            ...
            if (spnamePrefix == null
                    || (nextPathAndRef.getRef() == null && nextPathAndRef.getResource() == null))
                continue;
            ...
            IBaseResource targetResource =
                    theTargetIndexingStrategy.fetchResourceAtPath(nextPathAndRef);
            if (targetResource == null) continue;

            // 3.2.1 if we've already processed this resource upstream, do not process it again, to
            ...
            // this
            if (theRecurse) {
                HashSet<IBaseResource> nextAlreadySeenResources =
                        new HashSet<>(theAlreadySeenResources);
                nextAlreadySeenResources.add(targetResource);

                ISearchParamExtractor.SearchParamSet<PathAndRef> indexedReferences =
                        mySearchParamExtractor.extractResourceLinks(
    ...
    private IBaseResource findContainedResource(
            Collection<IBaseResource> resources, IBaseReference reference) {
    ...
    private void mergeParams(
            ResourceIndexedSearchParams theSrcParams, ResourceIndexedSearchParams theTargetParams) {
    ...
    void extractSearchIndexParameters(
        ...
        if (myStorageSettings
            ...
            ISearchParamExtractor.SearchParamSet<ResourceIndexedSearchParamQuantityNormalized>
                    quantitiesNormalized =
    ...
    private void populateResourceTables(
            ResourceIndexedSearchParams theParams, ResourceTable theEntity) {
    ...
    private void extractResourceLinks(
        ...
        if (LogicalReferenceHelper.isLogicalReference(myStorageSettings, nextId) || canonical) {
            ...
            ResourceLink resourceLink =
                    ResourceLink.forLogicalReference(
                            thePathAndRef.getPath(), theEntity, value, transactionDate);
        ...
        if (isBlank(typeString)) {
            String msg =
                    "Invalid resource reference found at path["
                            + path
                            + "] - Does not contain resource type - "
                            + nextId.getValue();
        ...
        try {
            ...
            String msg =
                    "Invalid resource reference found at path["
                            + path
                            + "] - Resource type is unknown or not supported on this server - "
                            + nextId.getValue();
            if (theFailOnInvalidReference) {
        ...
        JpaPid resolvedTargetId =
                (JpaPid) theTransactionDetails.getResolvedResourceId(referenceElement);
        ResourceLink resourceLink;
    ...
    private Optional<ResourceLink> findMatchingResourceLink(
        ...
        for (ResourceLink resourceLink : resourceLinks) {
            ...
            boolean hasMatchingResourceType =
                    StringUtils.equals(
                            resourceLink.getTargetResourceType(),
                            referenceElement.getResourceType());

            boolean hasMatchingResourceId =
                    StringUtils.equals(
                            resourceLink.getTargetResourceId(), referenceElement.getIdPart());
            ...
            boolean hasMatchingResourceVersion =
                    myContext.getParserOptions().isStripVersionsFromReferences()
                            ...
                            || referenceElement
                                    ...
                                    .equals(resourceLink.getTargetResourceVersion());
    ...
    private void extractResourceLinksForContainedResources(
        ...
        for (PathAndRef nextPathAndRef : referencedSearchParamSet) {
            ...
            IBaseResource containedResource =
                    findContainedResource(theContainedResources, nextPathAndRef.getRef());
            ...
            if (myPartitionSettings.getAllowReferencesAcrossPartitions()
                ...
                if (CompositeInterceptorBroadcaster.hasHooks(
                    ...
                    CrossPartitionReferenceDetails referenceDetails =
                            new CrossPartitionReferenceDetails(
                                    theRequestPartitionId,
                                    theSourceResourceName,
                                    thePathAndRef,
                                    theRequest,
    ...
    private RequestPartitionId determineResolverPartitionId(
            @Nonnull RequestPartitionId theRequestPartitionId) {
    ...
    private void populateResourceTableForComboParams(
            Collection<? extends IResourceIndexComboSearchParameter> theParams,
            ResourceTable theResourceTable) {
    ...
    private ISearchParamExtractor.SearchParamSet<ResourceIndexedSearchParamDate>
    ...
    private ISearchParamExtractor.SearchParamSet<ResourceIndexedSearchParamNumber>
    ...
    private ISearchParamExtractor.SearchParamSet<ResourceIndexedSearchParamQuantity>
    ...
    private ISearchParamExtractor.SearchParamSet<ResourceIndexedSearchParamQuantityNormalized>
            extractSearchParamQuantityNormalized(
                    IBaseResource theResource,
                    ISearchParamExtractor.ISearchParamFilter theSearchParamFilter) {
        return mySearchParamExtractor.extractSearchParamQuantityNormalized(
                theResource, theSearchParamFilter);
    }

    private ISearchParamExtractor.SearchParamSet<ResourceIndexedSearchParamString>
    ...
    private ISearchParamExtractor.SearchParamSet<BaseResourceIndexedSearchParam>
    ...
    private ISearchParamExtractor.SearchParamSet<ResourceIndexedSearchParamUri>
    ...
    private ISearchParamExtractor.SearchParamSet<ResourceIndexedSearchParamComposite>
            extractSearchParamComposites(
                    IBaseResource theResource,
                    ISearchParamExtractor.ISearchParamFilter theSearchParamFilter) {
        return mySearchParamExtractor.extractSearchParamComposites(
                theResource, theSearchParamFilter);
    }
    ...
    public List<String> extractParamValuesAsStrings(
        ...
        return mySearchParamExtractor.extractParamValuesAsStrings(
                theActiveSearchParam, theResource);
    }
     ...
     * #extractSearchIndexParametersForTargetResources(RequestDetails, ResourceIndexedSearchParams,
     * ResourceTable, Collection, IChainedSearchParameterExtractionStrategy,
     * ISearchParamExtractor.SearchParamSet, boolean, boolean)} in order to use that method for
     * extracting chained search parameter indexes both from contained resources and from uplifted
     * refchains.
     */
    private interface IChainedSearchParameterExtractionStrategy {

        /**
         * Which search parameters should be indexed for the resource target at the given path. In
         * other words if thePathAndRef contains "Patient/123", then we could return a filter that
         * only lets the "name" and "gender" search params through if we only want those two
         * parameters to be indexed for the resolved Patient resource with that ID.
         */
        @Nonnull
        ISearchParamExtractor.ISearchParamFilter getSearchParamFilter(
                @Nonnull PathAndRef thePathAndRef);

        /**
         * Actually fetch the resource at the given path, or return {@literal null} if none can be
         * found.
         */
        @Nullable
