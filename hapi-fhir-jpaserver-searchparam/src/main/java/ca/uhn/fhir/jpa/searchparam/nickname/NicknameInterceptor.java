...
public class NicknameInterceptor {
    ...
    public void expandNicknames(SearchParameterMap theSearchParameterMap) {
        for (Map.Entry<String, List<List<IQueryParameterType>>> set :
                theSearchParameterMap.entrySet()) {
    ...
    /**
     * If a Parameter is a string parameter, and it has been set to expand Nicknames, perform the
     * expansion.
     */
    private void expandAnyNicknameParameters(
            String theParamName, List<IQueryParameterType> orList) {
        ...
        for (IQueryParameterType iQueryParameterType : orList) {
            if (iQueryParameterType instanceof StringParam) {
                ...
                if (stringParam.isNicknameExpand()) {
                    ourLog.debug(
                            "Found a nickname parameter to expand: {} {}",
                            theParamName,
                            stringParam);
                    ...
                    ourLog.debug(
                            "Parameter has been expanded to: {} {}",
                            theParamName,
