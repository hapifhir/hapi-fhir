...
public class InMemoryMatchResult {
	...
	/**
	* if mySupported is false, then the parameter responsible for in-memory search not being
	* supported
	*/
	...
	/**
	* Only used by CompositeInMemoryDaoSubscriptionMatcher to track whether we had to go out to the
	* database to resolve the match.
	*/
	...
	public static InMemoryMatchResult and(
	...
	public static InMemoryMatchResult or(
				InMemoryMatchResult theLeft, InMemoryMatchResult theRight) {
		if (theLeft == null) {
				return theRight;
		}
