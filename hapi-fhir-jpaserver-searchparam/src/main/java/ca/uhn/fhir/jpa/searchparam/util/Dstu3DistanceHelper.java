 ...
 * search for near-distance search parameters and if we find any, remove them from the list of
 * search parameters and store it in a dedicated field in {@link SearchParameterMap}. This is so
 * that when the "near" search parameter is processed, we have access to this near-distance value.
 * This requires at most one near-distance parameter. If more are found, we throw an {@link
 * IllegalArgumentException}.
 */
public class Dstu3DistanceHelper {
	public static void setNearDistance(
				Class<? extends IBaseResource> theResourceType, SearchParameterMap theParams) {
	...
	private static ReferenceParam getChainedLocationNearDistanceParam(
				List<List<IQueryParameterType>> theParamAndList) {
	...
	private static QuantityParam getNearDistanceParam(
				List<List<IQueryParameterType>> theParamAndList) {
