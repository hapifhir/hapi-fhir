...
public class ReadOnlySearchParamCache {
    ...
    public Stream<RuntimeSearchParam> getSearchParamStream() {
        return myResourceNameToSpNameToSp.values().stream()
    ...
    public static ReadOnlySearchParamCache fromFhirContext(
        ...
        if (theFhirContext.getVersion().getVersion() == FhirVersionEnum.R4) {
            IBaseBundle allSearchParameterBundle =
                    (IBaseBundle)
                            theFhirContext
                                    .newJsonParser()
                                    .parseResource(
                                            ClasspathUtil.loadResourceAsStream(
                                                    "org/hl7/fhir/r4/model/sp/search-parameters.json"));
            ...
            IBaseBundle allSearchParameterBundle =
                    (IBaseBundle)
                            theFhirContext
                                    ...
                                    .parseResource(
                                            ClasspathUtil.loadResourceAsStream(
                                                    "org/hl7/fhir/r4b/model/sp/search-parameters.xml"));
            searchParams = BundleUtil.toListOfResources(theFhirContext, allSearchParameterBundle);
        } else if (theFhirContext.getVersion().getVersion() == FhirVersionEnum.R5) {
            searchParams =
                    FhirContext.forR5Cached().getValidationSupport().fetchAllSearchParameters();
        }
            ...
            if (nextCanonical != null) {
                ...
                for (String nextResourceName : base) {
                    ResourceSearchParams resourceSearchParams =
                            retVal.myResourceNameToSpNameToSp.computeIfAbsent(
                                    nextResourceName,
                                    t -> new ResourceSearchParams(nextResourceName));
        ...
        for (String resourceName : resourceNames) {
            RuntimeResourceDefinition nextResDef =
                    theFhirContext.getResourceDefinition(resourceName);
    ...
    public static boolean searchParamMatchesAtLeastOnePattern(
        ...
        for (String nextPattern : theSearchParamPatterns) {
            ...
            Validate.notBlank(
                    resourceType, "No resource type specified in pattern: %s", nextPattern);
            Validate.notBlank(
                    searchParamName, "No param name specified in pattern: %s", nextPattern);
