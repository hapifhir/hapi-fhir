...
public final class ResourceIndexedSearchParams {
    ...
    public final Collection<ResourceIndexedSearchParamQuantity> myQuantityParams =
            new ArrayList<>();
    public final Collection<ResourceIndexedSearchParamQuantityNormalized>
            myQuantityNormalizedParams = new ArrayList<>();
    ...
    private static final Set<String> myIgnoredParams =
            Set.of(Constants.PARAM_TEXT, Constants.PARAM_CONTENT);
    ...
    public void populateResourceTableSearchParamsPresentFlags(ResourceTable theEntity) {
        ...
        theEntity.setParamsQuantityNormalizedPopulated(
                myQuantityNormalizedParams.isEmpty() == false);
    ...
    public void updateSpnamePrefixForIndexOnUpliftedChain(
            String theContainingType, String theSpnamePrefix) {
        updateSpnamePrefixForIndexOnUpliftedChain(
                theContainingType, myNumberParams, theSpnamePrefix);
        updateSpnamePrefixForIndexOnUpliftedChain(
                theContainingType, myQuantityParams, theSpnamePrefix);
        updateSpnamePrefixForIndexOnUpliftedChain(
                theContainingType, myQuantityNormalizedParams, theSpnamePrefix);
        ...
        updateSpnamePrefixForIndexOnUpliftedChain(
                theContainingType, myTokenParams, theSpnamePrefix);
        updateSpnamePrefixForIndexOnUpliftedChain(
                theContainingType, myStringParams, theSpnamePrefix);
        updateSpnamePrefixForIndexOnUpliftedChain(
                theContainingType, myCoordsParams, theSpnamePrefix);
    }

    public void updateSpnamePrefixForLinksOnContainedResource(String theSpNamePrefix) {
        for (ResourceLink param : myLinks) {
            // The resource link already has the resource type of the contained resource at the head
            // of the path.
            // We need to replace this with the name of the containing type, and extend the search
            // path.
    ...
    private void setUpdatedTime(
            Collection<? extends BaseResourceIndexedSearchParam> theParams, Date theUpdateTime) {
    ...
    public boolean matchResourceLinks(
            ...
            String theParamPath) {
        return matchResourceLinks(
                new StorageSettings(), theResourceName, theParamName, theParam, theParamPath);
    }
    ...
    public boolean matchResourceLinks(
        ...
        for (String nextPath : theParamPaths) {
            if (matchResourceLinks(
                    theStorageSettings, theResourceName, theParamName, theParam, nextPath)) {
                return true;
            }
        }
        return false;
    ...
    public boolean matchResourceLinks(
        ...
        Predicate<ResourceLink> namedParamPredicate =
                resourceLink ->
                        searchParameterPathMatches(
                                        theResourceName, resourceLink, theParamName, theParamPath)
    ...
    private boolean resourceIdMatches(
            StorageSettings theStorageSettings,
            ResourceLink theResourceLink,
            ReferenceParam theReference) {
                ...
                if (!haveParam) {
                    ...
                    switch (type) {
                        ...
                        case URI:
                            ...
                            break;
                        case SPECIAL:
                            if (BaseSearchParamExtractor.COORDS_INDEX_PATHS.contains(
                                    searchParam.getPath())) {
