...
public class ResourceChangeListenerCache implements IResourceChangeListenerCache {
	...
	*/
	...
	public boolean matches(IBaseResource theResource) {
		...
		if (!result.supported()) {
				// This should never happen since we enforce only in-memory SearchParamMaps at
				// registration time
				throw new IllegalStateException(
						Msg.code(483)
									+ "Search Parameter Map "
									+ mySearchParameterMap
									+ " cannot be processed in-memory: "
	...
	private ResourceChangeResult refreshCacheAndNotifyListenersWithRetry() {
		Retrier<ResourceChangeResult> refreshCacheRetrier =
					new Retrier<>(
								() -> {
									synchronized (this) {
										return myResourceChangeListenerCacheRefresher
													.refreshCacheAndNotifyListener(this);
									}
								},
								MAX_RETRIES);
