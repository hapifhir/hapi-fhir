...
public abstract class BaseHttpClient implements IHttpClient {
    ...
    private void addHeaderIfNoneExist(IHttpRequest result) {
        if (myIfNoneExistParams != null) {
            ...
            BaseHttpClientInvocation.appendExtraParamsWithQuestionMark(
                    myIfNoneExistParams, b, b.indexOf("?") == -1);
    ...
    public void addHeadersToRequest(
            IHttpRequest theHttpRequest, EncodingEnum theEncoding, FhirContext theContext) {
        if (myHeaders != null) {
        ...
        theHttpRequest.addHeader(
                "User-Agent", HttpClientUtil.createUserAgentString(theContext, "apache"));
    ...
    public IHttpRequest createByteRequest(
        ...
        retVal.addHeader(
                Constants.HEADER_CONTENT_TYPE, theContentType + Constants.HEADER_SUFFIX_CT_UTF_8);
        return retVal;
    }
