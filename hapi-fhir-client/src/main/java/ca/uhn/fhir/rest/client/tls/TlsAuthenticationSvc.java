		...
		try {
				...
				if (theTlsAuthentication.getKeyStoreInfo().isPresent()) {
					...
					contextBuilder.loadKeyMaterial(
								keyStore, keyStoreInfo.getKeyPass(), privateKeyStrategy);
				}
		...
		try {
				...
				if (PathType.RESOURCE.equals(theStoreInfo.getPathType())) {
					try (InputStream inputStream =
								TlsAuthenticationSvc.class.getResourceAsStream(
										theStoreInfo.getFilePath())) {
		...
		try {
				...
				if (!theTrustStoreInfo.isPresent()) {
					trustManagerFactory.init(
								(KeyStore) null); // Load Trust Manager Factory with default Java truststore
				} else {
				...
				throw new TlsAuthenticationException(
						Msg.code(2104) + "Could not find X509TrustManager");
		} catch (Exception e) {
				throw new TlsAuthenticationException(
						Msg.code(2105) + "Failed to create X509TrustManager");
		}
	}

	public static HostnameVerifier createHostnameVerifier(
				Optional<TrustStoreInfo> theTrustStoreInfo) {
		return theTrustStoreInfo.isPresent()
