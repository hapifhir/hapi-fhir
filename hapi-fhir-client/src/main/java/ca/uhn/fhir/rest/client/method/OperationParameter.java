...
public class OperationParameter implements IParameter {
	...
	public OperationParameter(
				FhirContext theCtx, String theOperationName, OperationParam theOperationParam) {
		this(
					theCtx,
					theOperationName,
					theOperationParam.name(),
					theOperationParam.min(),
					theOperationParam.max());
	}
	...
	public void initializeTypes(
				...
				if (IDatatype.class.isAssignableFrom(theParameterType)) {
					throw new ConfigurationException(
								Msg.code(1408)
										...
										+ getContext().getVersion().getVersion().name()
										+ " in method: "
										+ theMethod.toString());
				}
		}
		...
		if (!myParameterType.equals(IBase.class) && !myParameterType.equals(String.class)) {
				if (IBaseResource.class.isAssignableFrom(myParameterType)
						&& myParameterType.isInterface()) {
					...
					throw new ConfigurationException(
								Msg.code(1409)
										+ "Invalid type for @OperationParam: "
										+ myParameterType.getName());
				}
		}
	}
	...
	public static void throwInvalidMode(String paramValues) {
		throw new InvalidRequestException(
					Msg.code(1410) + "Invalid mode value: \"" + paramValues + "\"");
	}
