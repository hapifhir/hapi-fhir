 ...
 * and the header value are specified by the calling code.
 *
 * @see AdditionalRequestHeadersInterceptor for a more advanced version of this interceptor which
...
public class SimpleRequestHeaderInterceptor implements IClientInterceptor {
     ...
     *
     * @param theCompleteHeader The complete header, e.g. "<code>Authorization: Bearer af09ufe90efh
    ...
    public SimpleRequestHeaderInterceptor(String theCompleteHeader) {
        ...
        if (colonIdx != -1) {
            ...
            setHeaderValue(
                    theCompleteHeader.substring(colonIdx + 1, theCompleteHeader.length()).trim());
        } else {
