...
class SinceOrAtParameter extends SearchParameter {
	...
	//	@Override
	//	public void translateClientArgumentIntoQueryArgument(FhirContext theContext, Object
	// theSourceClientArgument, Map<String, List<String>> theTargetQueryArguments, IBaseResource
	// theTargetResource) throws InternalErrorException {
	//		if (theSourceClientArgument != null) {
	//			InstantDt since = ParameterUtil.toInstant(theSourceClientArgument);
	//			if (since.isEmpty() == false) {
	//				theTargetQueryArguments.put(myParamName,
	...
	//						throw new InvalidRequestException(Msg.code(1465) + "Invalid " + Constants.PARAM_SINCE +
	// " value: " + sinceParams[0]);
	//					}
	//				}
	//			}
	...
	//
	//	@Override
	//	public void initializeTypes(Method theMethod, Class<? extends Collection<?>>
	// theOuterCollectionType, Class<? extends Collection<?>> theInnerCollectionType, Class<?>
	// theParameterType) {
	//		if (theOuterCollectionType != null) {
	//			throw new ConfigurationException(Msg.code(1466) + "Method '" + theMethod.getName() + "' in
	// type '" + "' is annotated with @" + myAnnotationType.getName() + " but can not be of
	// collection type");
	//		}
	//		if (ParameterUtil.getBindableInstantTypes().contains(theParameterType)) {
	//			myType = theParameterType;
	//		} else {
	//			throw new ConfigurationException(Msg.code(1467) + "Method '" + theMethod.getName() + "' in
	// type '" + "' is annotated with @" + myAnnotationType.getName() + " but is an invalid type,
	// must be one of: " + ParameterUtil.getBindableInstantTypes());
	//		}
	//	}
