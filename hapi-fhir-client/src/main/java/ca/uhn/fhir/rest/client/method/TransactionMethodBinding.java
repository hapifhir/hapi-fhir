...
public class TransactionMethodBinding extends BaseResourceReturningMethodBinding {
					...
					if (myTransactionParamIndex != -1) {
						throw new ConfigurationException(
									Msg.code(1418)
												...
												+ TransactionParam.class
												+ " annotation, exactly one is required for @"
												+ Transaction.class
												+ " methods");
					}
		...
		if (myTransactionParamIndex == -1) {
				throw new ConfigurationException(
						Msg.code(1419)
									...
									+ "' in type "
									+ theMethod.getDeclaringClass().getCanonicalName()
									+ " does not have a parameter annotated with the @"
									+ TransactionParam.class
									+ " annotation");
		}
	}
	...
	public static BaseHttpClientInvocation createTransactionInvocation(
				IBaseBundle theBundle, FhirContext theContext) {
	...
	public static BaseHttpClientInvocation createTransactionInvocation(
				String theRawBundle, FhirContext theContext) {
