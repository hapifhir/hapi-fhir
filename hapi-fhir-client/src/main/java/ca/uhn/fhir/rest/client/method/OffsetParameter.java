...
public class OffsetParameter implements IParameter {
	...
	public void translateClientArgumentIntoQueryArgument(
		...
		if (theSourceClientArgument != null) {
			...
			if (since.isEmpty() == false) {
				theTargetQueryArguments.put(
						Constants.PARAM_OFFSET, Collections.singletonList(since.getValueAsString()));
			}
		}
	}
	...
	public void initializeTypes(
		...
		if (theOuterCollectionType != null) {
			throw new ConfigurationException(Msg.code(1476) + "Method '" + theMethod.getName() + "' in type '"
		...
		if (!ParameterUtil.isBindableIntegerType(theParameterType)) {
			throw new ConfigurationException(Msg.code(1477) + "Method '" + theMethod.getName() + "' in type '"
					+ theMethod.getDeclaringClass().getCanonicalName() + "' is annotated with @"
					+ Offset.class.getName() + " but type '" + theParameterType
					+ "' is an invalid type, must be Integer or IntegerType");
		}
	}
}
