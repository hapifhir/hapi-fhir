...
public class ResourceProviderDstu3StructureDefinitionTest extends BaseResourceProviderDstu3Test {
	...
	public void testSnapshotWithResourceParameter() throws IOException {
		StructureDefinition sd =
				loadResourceFromClasspath(StructureDefinition.class, "/dstu3/profile-differential-patient-dstu3.json");

		StructureDefinition response = myClient.operation()
	...
	public void testSnapshotWithUrl() throws IOException {
		...
		StructureDefinition response = myClient.operation()
				...
				.named(JpaConstants.OPERATION_SNAPSHOT)
				.withParameter(
						Parameters.class, "url", new StringType("http://hl7.org/fhir/StructureDefinition/MyPatient421"))
				.returnResourceType(StructureDefinition.class)
				.execute();
		assertEquals(54, response.getSnapshot().getElement().size());
	}
		...
		try {
			myClient.operation()
					...
					.withParameter(
							Parameters.class,
							"url",
							new StringType("http://hl7.org/fhir/StructureDefinition/MyPatient421"))
					.returnResourceType(StructureDefinition.class)
					.execute();
		} catch (InvalidRequestException e) {
			assertEquals(
					"HTTP 400 Bad Request: " + Msg.code(1770)
	...
	public void testSnapshotWithInvalidUrl() {
		try {
			myClient.operation()
					...
					.named(JpaConstants.OPERATION_SNAPSHOT)
					.withParameter(
							Parameters.class, "url", new StringType("http://hl7.org/fhir/StructureDefinition/FOO"))
					.returnResourceType(StructureDefinition.class)
					.execute();
		} catch (ResourceNotFoundException e) {
			assertEquals(
					"HTTP 404 Not Found: " + Msg.code(1162)
							+ "No StructureDefiniton found with url = 'http://hl7.org/fhir/StructureDefinition/FOO'",
					e.getMessage());
		}
	}
}
