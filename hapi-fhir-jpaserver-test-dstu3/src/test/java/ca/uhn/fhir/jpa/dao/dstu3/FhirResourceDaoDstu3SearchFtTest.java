...
public class FhirResourceDaoDstu3SearchFtTest extends BaseJpaDstu3Test {
    ...
    public void testCodeTextSearch() {
        ...
        map.add(
                Observation.SP_CODE,
                new TokenParam(null, "systolic").setModifier(TokenParamModifier.TEXT));
        assertThat(
                toUnqualifiedVersionlessIdValues(myObservationDao.search(map)),
                containsInAnyOrder(toValues(id1)));

        //		map = new SearchParameterMap();
        //		map.add(Observation.SP_CODE, new TokenParam(null,
        // "blood").setModifier(TokenParamModifier.TEXT));
    ...
    public void testSearchAndReindex() {
        ...
        assertThat(
                toUnqualifiedVersionlessIdValues(myPatientDao.search(map)),
                contains(toValues(pId1)));
    ...
    public void testEverythingInstanceWithContentFilter() {
        ...
        IIdType obsId3 =
                myObservationDao.create(obs3, mockSrd()).getId().toUnqualifiedVersionless();
        ...
        actual =
                toUnqualifiedVersionlessIdValues(
                        myPatientDao.patientInstanceEverything(
                                request, mockSrd(), everythingParams, ptId1));
        ...
        actual =
                toUnqualifiedVersionlessIdValues(
                        myPatientDao.patientInstanceEverything(
                                request, mockSrd(), new PatientEverythingParameters(), ptId1));
        ...
        IIdType obsId4 =
                myObservationDao.create(obs4, mockSrd()).getId().toUnqualifiedVersionless();
    ...
    public void testEverythingTypeWithContentFilter() {
        ...
        actual =
                toUnqualifiedVersionlessIdValues(
                        myPatientDao.patientTypeEverything(
                                request, mockSrd(), everythingParams, null));
        assertThat(actual, containsInAnyOrder(toValues(ptId1, obsId1, devId1)));
        ...
        actual =
                toUnqualifiedVersionlessIdValues(
                        myPatientDao.patientTypeEverything(
                                request, mockSrd(), new PatientEverythingParameters(), null));
        assertThat(
                actual, containsInAnyOrder(toValues(ptId1, obsId1, obsId2, devId1, ptId2, obsId3)));
    ...
    /**
     * When processing transactions, we do two passes. Make sure we don't update the lucene index
     * twice since that would be inefficient
     */
    @Test
    public void testSearchDontReindexForUpdateWithIndexDisabled() {
        ...
        patient.addName().addGiven("NAMEAAA");
        final IIdType pId1 =
                myPatientDao.create(patient, mockSrd()).getId().toUnqualifiedVersionless();
        ...
        assertDoesNotContainAnyOf(
                toUnqualifiedVersionlessIdValues(myPatientDao.search(map)), Arrays.asList(idArray));
