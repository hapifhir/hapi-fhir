...
public class NpmDstu3Test extends BaseJpaDstu3Test {
    ...
    @Autowired public PackageInstallerSvcImpl igInstaller;
    ...
    public void installDstu3Package() throws Exception {
        ...
        ValueSet vs =
                (ValueSet)
                        myNpmJpaValidationSupport.fetchValueSet("http://fhir.de/ValueSet/ifa/pzn");
        assertEquals("http://fhir.de/ValueSet/ifa/pzn", vs.getUrl());

        CodeSystem cs =
                (CodeSystem)
                        myNpmJpaValidationSupport.fetchCodeSystem(
        ...
        MethodOutcome result =
                myConditionDao.validate(
                        condition, null, null, null, ValidationModeEnum.CREATE, null, mySrd);
        ...
        assertThat(
                ...
                containsString(
                        "Condition.subject: minimum required = 1, but only found 0 (from"
                                + " http://fhir.de/StructureDefinition/condition-de-basis/0.2"));
    }
