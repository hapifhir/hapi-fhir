...
public abstract class BaseResourceCacheSynchronizer implements IResourceChangeListener {
	private static final Logger ourLog =
				LoggerFactory.getLogger(BaseResourceCacheSynchronizer.class);
	...
	public void registerListener() {
		if (myDaoRegistry.getResourceDaoOrNull(myResourceName) == null) {
				ourLog.info(
						"No resource DAO found for resource type {}, not registering listener",
						myResourceName);
				return;
		}
	...
	private int doSyncResources() {
		...
		synchronized (mySyncResourcesLock) {
				...
				IBundleProvider resourceBundleList =
						getResourceDao().search(mySearchParameterMap, mySystemRequestDetails);
	...
	public void handleInit(Collection<IIdType> theResourceIds) {
		if (!resourceDaoExists()) {
				ourLog.warn(
						"The resource type {} is enabled on this server, but there is no {} DAO"
									+ " configured.",
						myResourceName,
						myResourceName);
				return;
	...
	public void handleChange(IResourceChangeEvent theResourceChangeEvent) {
		// For now ignore the contents of theResourceChangeEvent.  In the future, consider updating
		// the registry based on
