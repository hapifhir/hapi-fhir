 ...
 * AuditEvent resources that are conformant to the BALP profile in response to events in a FHIR
 * server. See <a href="https://hapifhir.io/hapi-fhir/security/balp_interceptor.html">BALP
 * Interceptor</a> in the HAPI FHIR documentation for more information.
 *
 * @since 6.6.0
 */
public class BalpAuditCaptureInterceptor {
     ...
     * Constructor
     *
     * @param theAuditEventSink This service is the target for generated AuditEvent resources. The
     *     {@link BalpAuditCaptureInterceptor} does not actually store AuditEvents, it simply
     *     generates them when appropriate and passes them to the sink service. The sink service
     *     might store them locally, transmit them to a remote repository, or even simply log them
     *     to a syslog.
     * @param theContextServices This service supplies details to the BALP about the context of a
     *     given request. For example, in order to generate a conformant AuditEvent resource, this
     *     interceptor needs to determine the identity of the user and the client from the {@link
     *     ca.uhn.fhir.rest.api.server.RequestDetails} object.
     */
    ...
    void hookStoragePreShowResources(
            IPreResourceShowDetails theDetails, ServletRequestDetails theRequestDetails) {
    ...
    private void handleCreateUpdateDelete(
        ...
        Set<String> patientCompartmentOwners =
                determinePatientCompartmentOwnersForResources(
                        List.of(theResource), theRequestDetails);
        if (patientCompartmentOwners.isEmpty()) {
            AuditEvent auditEvent =
                    createAuditEventBasicCreateUpdateDelete(
                            theRequestDetails, theResource, theBasicProfile);
            ...
            AuditEvent auditEvent =
                    createAuditEventPatientCreateUpdateDelete(
                            theRequestDetails,
                            theResource,
                            patientCompartmentOwners,
                            thePatientProfile);
            myAuditEventSink.recordAuditEvent(auditEvent);
        }
    }

    private void handleReadOrVRead(
            IPreResourceShowDetails theDetails, ServletRequestDetails theRequestDetails) {
        Validate.isTrue(
                theDetails.size() == 1,
                "Unexpected number of results for read: %d",
                theDetails.size());
        IBaseResource resource = theDetails.getResource(0);
        if (resource != null) {
            String dataResourceId =
                    myContextServices.massageResourceIdForStorage(
            ...
            for (String patientId : patientIds) {
                AuditEvent auditEvent =
                        createAuditEventPatientRead(theRequestDetails, dataResourceId, patientId);
                myAuditEventSink.recordAuditEvent(auditEvent);
            }
            ...
            if (patientIds.isEmpty()) {
                AuditEvent auditEvent =
                        createAuditEventBasicRead(theRequestDetails, dataResourceId);
                myAuditEventSink.recordAuditEvent(auditEvent);
            }
        }
    }
    ...
    private void handleSearch(
        ...
        Set<String> compartmentOwners =
                determinePatientCompartmentOwnersForResources(resources, theRequestDetails);

        if (!compartmentOwners.isEmpty()) {
            AuditEvent auditEvent =
                    createAuditEventPatientQuery(theRequestDetails, compartmentOwners);
            myAuditEventSink.recordAuditEvent(auditEvent);
        } else {
    ...
    private AuditEvent createAuditEventPatientCreateUpdateDelete(
            ...
            BalpProfileEnum theProfile) {
        AuditEvent retVal =
                createAuditEventCommonCreate(theRequestDetails, theResource, theProfile);
    ...
    private AuditEvent createAuditEventPatientRead(
        ...
        AuditEvent auditEvent =
                createAuditEventCommonRead(theRequestDetails, dataResourceId, profile);
