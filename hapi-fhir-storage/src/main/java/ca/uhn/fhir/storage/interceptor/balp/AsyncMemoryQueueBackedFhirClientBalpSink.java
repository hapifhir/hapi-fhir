...
/**
 * This implementation of the {@link IBalpAuditEventSink} transmits audit events to a FHIR endpoint
 * for creation, using a standard fhir <i>create</i> event. The target server FHIR version does not
 * need to match the FHIR version of the AuditEvent source, events will be converted automatically
 * prior to sending.
 *
 * <p>This sink transmits events asynchronously using an in-memory queue. This means that in the
 * event of a server shutdown or unavailability of the target server <b>data could be lost</b>.
 */
public class AsyncMemoryQueueBackedFhirClientBalpSink extends FhirClientBalpSink
    ...
    private static final Logger ourLog =
            LoggerFactory.getLogger(AsyncMemoryQueueBackedFhirClientBalpSink.class);
     ...
     *
     * @param theFhirContext The FhirContext instance. This context must be for the FHIR Version
     *     supported by the target/sink server (as opposed to the FHIR Version supported by the
     *     audit source).
     * @param theTargetBaseUrl The FHIR server base URL for the target/sink server to receive audit
     *     events.
     */
     ...
     * @param theTargetBaseUrl The FHIR server base URL for the target/sink server to receive audit
     *     events.
     * @param theClientInterceptors An optional list of interceptors to register against the client.
     *     May be {@literal null}.
     */
    public AsyncMemoryQueueBackedFhirClientBalpSink(
    ...
    public AsyncMemoryQueueBackedFhirClientBalpSink(IGenericClient theClient) {
        ...
        myThreadPool =
                ThreadPoolUtil.newThreadPool(
                        1,
                        1,
                        "BalpClientSink-" + ourNextThreadId.getAndIncrement() + "-",
                        Integer.MAX_VALUE);
    }
    ...
    private class TransmitterTask implements Runnable {
        ...
        public void run() {
            ...
            try {
                ...
                ourLog.error(
                        "Failed to transmit AuditEvent items to target. Will re-attempt {} failed"
                                + " events once. Error: {}",
                        queue.length,
                        e.toString());
            }
