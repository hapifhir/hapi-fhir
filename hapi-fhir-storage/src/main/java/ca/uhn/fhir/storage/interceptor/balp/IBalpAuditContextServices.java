...
/**
 * This interface is intended to be implemented in order to supply implementation strategy details
 * to the {@link BalpAuditCaptureInterceptor}.
 *
 * @since 6.6.0
 */
public interface IBalpAuditContextServices {
     ...
     *
     * @see #getNetworkAddressType(RequestDetails) If this method is returning an adress type that
     *     is not an IP address, you must also oerride this method and return the correct code.
     */
    default String getNetworkAddress(RequestDetails theRequestDetails) {
        String remoteAddr = null;
        if (theRequestDetails instanceof ServletRequestDetails) {
            remoteAddr =
                    ((ServletRequestDetails) theRequestDetails).getServletRequest().getRemoteAddr();
        }
        return remoteAddr;
    }
     ...
     */
    default AuditEvent.AuditEventAgentNetworkType getNetworkAddressType(
            RequestDetails theRequestDetails) {
        return BalpConstants.AUDIT_EVENT_AGENT_NETWORK_TYPE_IP_ADDRESS;
    }

    /**
     * Turns an entity resource ID from an {@link IIdType} to a String. The default implementation
     * injects the server's base URL into the ID in order to create fully qualified URLs for
     * resource references within BALP events.
     */
    @Nonnull
