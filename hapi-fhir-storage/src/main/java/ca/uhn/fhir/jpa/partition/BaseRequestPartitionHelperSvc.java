...
public abstract class BaseRequestPartitionHelperSvc implements IRequestPartitionHelperSvc {
	...
	/**
	* Invoke the {@link Pointcut#STORAGE_PARTITION_IDENTIFY_READ} interceptor pointcut to determine
	* the tenant for a read request.
	*
	* <p>If no interceptors are registered with a hook for {@link
	* Pointcut#STORAGE_PARTITION_IDENTIFY_READ}, return {@link RequestPartitionId#allPartitions()}
	* instead.
	*/
	@Nonnull
	@Override
	public RequestPartitionId determineReadPartitionForRequest(
		...
		if (myPartitionSettings.isPartitioningEnabled()) {
				...
				// TODO GGG eventually, theRequest will not be allowed to be null here, and we will pass
				// through SystemRequestDetails instead.
				...
				if (requestDetails instanceof SystemRequestDetails
					...
					requestPartitionId =
								getSystemRequestPartitionId(
										(SystemRequestDetails) requestDetails, nonPartitionableResource);
				} else if ((requestDetails instanceof SystemRequestDetails)
						&& nonPartitionableResource) {
				...
				validateRequestPartitionNotNull(
						requestPartitionId, Pointcut.STORAGE_PARTITION_IDENTIFY_READ);

				return validateNormalizeAndNotifyHooksForRead(
						requestPartitionId, requestDetails, resourceType);
		}
	...
	@Override
	public RequestPartitionId determineGenericPartitionForRequest(
		...
		if (retVal == null) {
				if (hasHooks(
					...
					if (retVal != null) {
						retVal =
									validateNormalizeAndNotifyHooksForRead(retVal, theRequestDetails, null);
					}
				}
		}
	...
	/**
	* For system requests, read partition from tenant ID if present, otherwise set to DEFAULT. If
	* the resource they are attempting to partition is non-partitionable scream in the logs and set
	* the partition to DEFAULT.
	*/
	private RequestPartitionId getSystemRequestPartitionId(
		...
		if (theNonPartitionableResource && !requestPartitionId.isDefaultPartition()) {
				throw new InternalErrorException(
						Msg.code(1315)
									+ "System call is attempting to write a non-partitionable resource to a"
									+ " partition! This is a bug!");
		}
		return requestPartitionId;
	}

	/**
	* Determine the partition for a System Call (defined by the fact that the request is of type
	* SystemRequestDetails)
	*
	* <p>1. If the tenant ID is set to the constant for all partitions, return all partitions 2. If
	* there is a tenant ID set in the request, use it. 3. Otherwise, return the Default Partition.
	*
	...
	*/
	@Nonnull
	private RequestPartitionId getSystemRequestPartitionId(
				@Nonnull SystemRequestDetails theRequest) {
	...
	/**
	* Invoke the {@link Pointcut#STORAGE_PARTITION_IDENTIFY_CREATE} interceptor pointcut to
	* determine the tenant for a create request.
	*/
	@Nonnull
	@Override
	public RequestPartitionId determineCreatePartitionForRequest(
		...
		if (myPartitionSettings.isPartitioningEnabled()) {
				boolean nonPartitionableResource =
						myNonPartitionableResourceNames.contains(theResourceType);

				// TODO GGG eventually, theRequest will not be allowed to be null here, and we will pass
				// through SystemRequestDetails instead.
				if ((theRequest == null || theRequest instanceof SystemRequestDetails)
						&& nonPartitionableResource) {
					return RequestPartitionId.defaultPartition();
				}
				...
				if (theRequest instanceof SystemRequestDetails
					...
					requestPartitionId =
								getSystemRequestPartitionId(
										(SystemRequestDetails) theRequest, nonPartitionableResource);
				} else {
					if (hasHooks(
								Pointcut.STORAGE_PARTITION_IDENTIFY_ANY,
								myInterceptorBroadcaster,
						...
						// This is an external Request (e.g. ServletRequestDetails) so we want to figure
						// out the partition via interceptor.
					...
					// If the interceptors haven't selected a partition, and its a non-partitionable
					// resource anyhow, send to DEFAULT
				...
				return validateNormalizeAndNotifyHooksForRead(
						requestPartitionId, theRequest, theResourceType);
		}
	...
	*
	* <p>If the partition has an ID but not a name, the name is resolved.
	*
	* <p>If the partition has both, they are validated to ensure that they correspond.
	*/
	@Nonnull
	...
	public void validateHasPartitionPermissions(
		...
		if (myInterceptorBroadcaster.hasHooks(Pointcut.STORAGE_PARTITION_SELECTED)) {
				...
				doCallHooks(
						myInterceptorBroadcaster,
						theRequest,
						Pointcut.STORAGE_PARTITION_SELECTED,
						params);
		}
	}
	...
	protected abstract RequestPartitionId validateAndNormalizePartitionIds(
				RequestPartitionId theRequestPartitionId);

	protected abstract RequestPartitionId validateAndNormalizePartitionNames(
				RequestPartitionId theRequestPartitionId);
		...
		if ((theRequestPartitionId.hasPartitionIds()
					...
					|| (theRequestPartitionId.hasPartitionNames()
								&& !theRequestPartitionId
										.getPartitionNames()
										.contains(JpaConstants.DEFAULT_PARTITION_NAME))) {
	...
	private void validateRequestPartitionNotNull(
				RequestPartitionId theRequestPartitionId, Pointcut theThePointcut) {
	...
	private void validateSinglePartitionIdOrNameForCreate(@Nullable List<?> thePartitionIds) {
		if (thePartitionIds != null && thePartitionIds.size() != 1) {
				throw new InternalErrorException(
						Msg.code(1320)
									+ "RequestPartitionId must contain a single partition for create"
									+ " operations, found: "
									+ thePartitionIds);
		}
	}
