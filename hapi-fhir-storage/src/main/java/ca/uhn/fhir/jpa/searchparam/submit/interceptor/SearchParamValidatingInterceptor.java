...
public class SearchParamValidatingInterceptor {
    ...
    public void validateSearchParamOnCreate(
        ...
        RuntimeSearchParam runtimeSearchParam =
                mySearchParameterCanonicalizer.canonicalizeSearchParameter(theResource);
    ...
    private void validateSearchParamOnCreateAndUpdate(RuntimeSearchParam theRuntimeSearchParam) {
        ...
        List<IBaseExtension<?, ?>> refChainExtensions =
                theRuntimeSearchParam.getExtensions(
                        HapiExtensions.EXTENSION_SEARCHPARAM_UPLIFT_REFCHAIN);
        for (IBaseExtension<?, ?> nextExtension : refChainExtensions) {
            List<? extends IBaseExtension> codeExtensions =
                    nextExtension.getExtension().stream()
    ...
    private void validateStandardSpOnCreate(
            RequestDetails theRequestDetails, SearchParameterMap searchParameterMap) {
        List<IResourcePersistentId> persistedIdList =
                getDao().searchForIds(searchParameterMap, theRequestDetails);
        if (isNotEmpty(persistedIdList)) {
            throw new UnprocessableEntityException(
                    Msg.code(2196)
                            + "Can't process submitted SearchParameter as it is overlapping an"
                            + " existing one.");
        }
    }
    ...
    private boolean isNewSearchParam(
            RuntimeSearchParam theSearchParam, Set<String> theExistingIds) {
        return theExistingIds.stream()
                .noneMatch(
                        resId ->
                                resId.substring(resId.indexOf("/") + 1)
    ...
    private void validateStandardSpOnUpdate(
        ...
        List<IResourcePersistentId> pidList =
                getDao().searchForIds(searchParameterMap, theRequestDetails);
        if (isNotEmpty(pidList)) {
            Set<String> resolvedResourceIds =
                    myIdHelperService.translatePidsToFhirResourceIds(new HashSet<>(pidList));
    ...
    private void throwDuplicateError() {
        throw new UnprocessableEntityException(
                Msg.code(2125)
                        + "Can't process submitted SearchParameter as it is overlapping an existing"
                        + " one.");
    }
    ...
    @Autowired
    public void setSearchParameterCanonicalizer(
            SearchParameterCanonicalizer theSearchParameterCanonicalizer) {
