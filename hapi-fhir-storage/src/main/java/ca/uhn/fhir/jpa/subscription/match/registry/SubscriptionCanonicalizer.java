...
public class SubscriptionCanonicalizer {
		...
		switch (myFhirContext.getVersion().getVersion()) {
				...
				default:
					throw new ConfigurationException(
								Msg.code(556)
										+ "Subscription not supported for version: "
	...
	private CanonicalSubscription canonicalizeDstu2(IBaseResource theSubscription) {
		...
		try {
				retVal.setStatus(
						org.hl7.fhir.r4.model.Subscription.SubscriptionStatus.fromCode(
	...
	private boolean extractDeleteExtensionDstu2(
				ca.uhn.fhir.model.dstu2.resource.Subscription theSubscription) {
		return theSubscription
					.getChannel()
					.getUndeclaredExtensionsByUrl(EX_SEND_DELETE_MESSAGES)
					.stream()
	...
	*/
	...
	private CanonicalSubscription canonicalizeDstu3(IBaseResource theSubscription) {
		...
		CanonicalSubscription retVal = new CanonicalSubscription();
		try {
				org.hl7.fhir.dstu3.model.Subscription.SubscriptionStatus status =
						subscription.getStatus();
				if (status != null) {
					retVal.setStatus(
								org.hl7.fhir.r4.model.Subscription.SubscriptionStatus.fromCode(
				...
				retVal.setPayloadSearchCriteria(
						getExtensionString(
									subscription, HapiExtensions.EXT_SUBSCRIPTION_PAYLOAD_SEARCH_CRITERIA));
				retVal.setTags(extractTags(subscription));
				retVal.setCrossPartitionEnabled(SubscriptionUtil.isCrossPartition(theSubscription));
					...
					try {
						...
						subjectTemplate =
									channel.getExtensionString(
												HapiExtensions.EXT_SUBSCRIPTION_SUBJECT_TEMPLATE);
					} catch (FHIRException theE) {
						throw new ConfigurationException(
									Msg.code(558)
												+ "Failed to extract subscription extension(s): "
												+ theE.getMessage(),
									theE);
					}
				...
				if (retVal.getChannelType() == CanonicalSubscriptionChannelType.RESTHOOK) {
					...
					retVal.getRestHookDetails()
								.setStripVersionId(Boolean.parseBoolean(stripVersionIds));
					retVal.getRestHookDetails()
								.setDeliverLatestVersion(Boolean.parseBoolean(deliverLatestVersion));
				}
		...
		try {
				switch (theSubscription.getStructureFhirVersionEnum()) {
					...
					case R5:
						{
								// TODO KHS fix
								// org.hl7.fhir.r4b.model.BaseResource.getStructureFhirVersionEnum() for R4B
	...
	private CanonicalSubscription canonicalizeR4(IBaseResource theSubscription) {
		...
		for (org.hl7.fhir.r4.model.CanonicalType next : profiles) {
				if (SubscriptionConstants.SUBSCRIPTION_TOPIC_PROFILE_URL.equals(
		...
		if (retVal.isTopicSubscription()) {
				...
				if (channel.hasExtension(
						SubscriptionConstants.SUBSCRIPTION_TOPIC_CHANNEL_TIMEOUT_URL)) {
					org.hl7.fhir.r4.model.Extension timeoutExtension =
								channel.getExtensionByUrl(
										SubscriptionConstants.SUBSCRIPTION_TOPIC_CHANNEL_TIMEOUT_URL);
				...
				if (channel.hasExtension(SubscriptionConstants.SUBSCRIPTION_TOPIC_CHANNEL_MAX_COUNT)) {
					org.hl7.fhir.r4.model.Extension timeoutExtension =
								channel.getExtensionByUrl(
										SubscriptionConstants.SUBSCRIPTION_TOPIC_CHANNEL_MAX_COUNT);
				...
				if (channel.getPayloadElement()
					...
					org.hl7.fhir.r4.model.Extension timeoutExtension =
								channel.getPayloadElement()
										.getExtensionByUrl(
													SubscriptionConstants
																.SUBSCRIPTION_TOPIC_CHANNEL_PAYLOAD_CONTENT);
					topicSubscription.setContent(
								org.hl7.fhir.r5.model.Subscription.SubscriptionPayloadContent.fromCode(
		...
		if (topicExts.size() > 0) {
				...
				if (!"EventDefinition".equals(ref.getReferenceElement().getResourceType())) {
					throw new PreconditionFailedException(
								Msg.code(563) + "Topic reference must be an EventDefinition");
				}
		}
	...
	private CanonicalSubscription canonicalizeR4B(IBaseResource theSubscription) {
		...
		setPartitionIdOnReturnValue(theSubscription, retVal);
		org.hl7.fhir.r4b.model.Subscription.SubscriptionChannelComponent channel =
					subscription.getChannel();
		retVal.setHeaders(channel.getHeader());
		retVal.setChannelExtensions(extractExtension(subscription));
		retVal.setIdElement(subscription.getIdElement());
		...
		if (retVal.getChannelType() == CanonicalSubscriptionChannelType.EMAIL) {
				...
				try {
					...
					subjectTemplate =
								getExtensionString(
										subscription, HapiExtensions.EXT_SUBSCRIPTION_SUBJECT_TEMPLATE);
				} catch (FHIRException theE) {
					throw new ConfigurationException(
								Msg.code(564)
										+ "Failed to extract subscription extension(s): "
		...
		if (topicExts.size() > 0) {
				...
				if (!"EventDefinition".equals(ref.getReferenceElement().getResourceType())) {
					throw new PreconditionFailedException(
								Msg.code(566) + "Topic reference must be an EventDefinition");
				}
		}

		org.hl7.fhir.r4b.model.Extension extension =
					channel.getExtensionByUrl(EX_SEND_DELETE_MESSAGES);
	...
	private CanonicalSubscription canonicalizeR5(IBaseResource theSubscription) {
		...
		if (topicExts.size() > 0) {
				...
				if (!"EventDefinition".equals(ref.getReferenceElement().getResourceType())) {
					throw new PreconditionFailedException(
								Msg.code(2325) + "Topic reference must be an EventDefinition");
				}
		}
		...
		if (status != null) {
				switch (status) {
					case REQUESTED:
						retVal.setStatus(
									org.hl7.fhir.r4.model.Subscription.SubscriptionStatus.REQUESTED);
						break;
					case ACTIVE:
	...
	private void setPartitionIdOnReturnValue(
				IBaseResource theSubscription, CanonicalSubscription retVal) {
	...
	public CanonicalSubscriptionChannelType getChannelType(IBaseResource theSubscription) {
		...
		switch (myFhirContext.getVersion().getVersion()) {
				...
				case R4:
					{
						org.hl7.fhir.r4.model.Subscription.SubscriptionChannelType type =
									((org.hl7.fhir.r4.model.Subscription) theSubscription)
												.getChannel()
												.getType();
						if (type != null) {
				...
				case R5:
					{
						...
						CanonicalSubscriptionChannelType code =
									CanonicalSubscriptionChannelType.fromCode(
												nextTypeCode.getSystem(), nextTypeCode.getCode());
						if (code != null) {
								retVal = code;
						}
						break;
				...
				default:
					throw new IllegalStateException(
								Msg.code(2326)
										+ "Unsupported Subscription FHIR version: "
										+ myFhirContext.getVersion().getVersion());
		}
	...
	public String getCriteria(IBaseResource theSubscription) {
		...
		switch (myFhirContext.getVersion().getVersion()) {
				...
				default:
					throw new IllegalStateException(
								Msg.code(2327)
										+ "Subscription criteria is not supported for FHIR version: "
										+ myFhirContext.getVersion().getVersion());
		}
	...
	public void setMatchingStrategyTag(
				...
				@Nullable SubscriptionMatchingStrategy theStrategy) {
		...
		meta.getTag().stream()
					.filter(
								t ->
										HapiExtensions.EXT_SUBSCRIPTION_MATCHING_STRATEGY.equals(
													t.getSystem()))
					.forEach(
								t -> {
									t.setCode(null);
		...
		if (theStrategy == SubscriptionMatchingStrategy.DATABASE) {
				...
				throw new IllegalStateException(
						Msg.code(567)
									+ "Unknown "
									+ SubscriptionMatchingStrategy.class.getSimpleName()
									+ ": "
									+ theStrategy);
		}
		meta.addTag()
	...
	public String getSubscriptionStatus(IBaseResource theSubscription) {
		final IPrimitiveType<?> status =
					myFhirContext
								.newTerser()
								.getSingleValueOrNull(
										theSubscription,
										SubscriptionConstants.SUBSCRIPTION_STATUS,
