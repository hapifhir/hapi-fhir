 ...
 *
 * <p>The API is super simplistic, and caches are all 1-minute, max 10000 entries for starters. We
 * could definitely add nuance to this, which will be much easier now that this is being
 * centralized. Some logging/monitoring would be good too.
 */
...
public class MemoryCacheService {
    ...
    private void populateCaches() {
        for (CacheEnum next : CacheEnum.values()) {
            ...
            Cache<Object, Object> nextCache =
                    CacheFactory.build(SECONDS.toMillis(timeoutSeconds), maximumSize);
    ...
    /**
     * Fetch an item from the cache if it exists, and use the loading function to obtain it
     * otherwise.
     *
     * <p>This method will put the value into the cache using {@link #putAfterCommit(CacheEnum,
     * Object, Object)}.
     */
    public <K, T> T getThenPutAfterCommit(
            CacheEnum theCache, K theKey, Function<K, T> theSupplier) {
        assert theCache.getKeyType().isAssignableFrom(theKey.getClass());
    ...
    /**
     * This method registers a transaction synchronization that puts an entry in the cache if and
     * when the current database transaction successfully commits. If the transaction is rolled
     * back, the key+value passed into this method will not be added to the cache.
     *
     * <p>This is useful for situations where you want to store something that has been resolved in
     * the DB during the current transaction, but it's not yet guaranteed that this item will
     * successfully save to the DB. Use this method in that case in order to avoid cache poisoning.
     */
    ...
    public enum CacheEnum {
        ...
        /** Key type: {@literal Long} Value type: {@literal Optional<String>} */
