...
public class PartitionRunner {
    ...
    /**
     * Constructor - Use this constructor and provide a {@link RequestDetails} and {@link
     * HapiTransactionService} if you want each individual callable task to be performed in a
     * managed transaction.
     */
    public PartitionRunner(
    ...
    public void runInPartitionedThreads(
            ...
            Consumer<List<IResourcePersistentId>> partitionConsumer) {
    ...
    private List<Callable<Void>> buildCallableTasks(
        ...
        if (myBatchSize > theResourceIds.size()) {
            ourLog.info(
                    "Splitting batch job of {} entries into chunks of {}",
                    theResourceIds.size(),
                    myBatchSize);
        } else {
    ...
    private ExecutorService buildExecutor(int numberOfTasks) {
        ...
        RejectedExecutionHandler rejectedExecutionHandler =
                (theRunnable, theExecutor) -> {
                    ourLog.info(
                            "Note: "
                                    + myThreadPrefix
                                    + " executor queue is full ({} elements), waiting for a slot to"
                                    + " become available!",
                            executorQueue.size());
