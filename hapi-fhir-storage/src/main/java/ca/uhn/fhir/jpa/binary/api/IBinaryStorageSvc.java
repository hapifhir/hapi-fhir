...
public interface IBinaryStorageSvc {
     ...
     * The default is {@link Long#MAX_VALUE}
     */
    long getMaximumBinarySize();

    /**
     * Given a blob ID, return true if it is valid for the underlying storage mechanism, false
     * otherwise.
     *
    ...
    /**
     * Gets the minimum number of bytes that will be stored. Binary content smaller * than this
     * threshold may be inlined even if a binary storage service * is active.
     */
    int getMinimumBinarySize();

    /**
     * Sets the minimum number of bytes that will be stored. Binary content smaller than this
     * threshold may be inlined even if a binary storage service is active.
     */
    ...
    /**
     * Generate a new blob ID that will be passed to {@link #storeBlob(IIdType, String, String,
     * InputStream)} later
     */
    String newBlobId();
     ...
     *
     * @param theResourceId The resource ID that owns this blob. Note that it should not be possible
     *     to retrieve a blob without both the resource ID and the blob ID being correct.
     * @param theBlobIdOrNull If set, forces
     * @param theContentType The content type to associate with this blob
     * @param theInputStream An InputStream to read from. This method should close the stream when
     *     it has been fully consumed.
     * @return Returns details about the stored data
     * @deprecated Use {@link #storeBlob(IIdType theResourceId, String theBlobIdOrNull, String
     *     theContentType, InputStream theInputStream, RequestDetails theRequestDetails)} instead.
     *     This method will be removed because it doesn't receive the 'theRequestDetails' parameter
     *     it needs to forward to the pointcut)
     */
    ...
    /**
     * @return Returns <code>true</code> if the blob was found and written, of <code>false</code> if
     *     the blob was not found (i.e. it was expunged or the ID was invalid)
     */
    boolean writeBlob(IIdType theResourceId, String theBlobId, OutputStream theOutputStream)
            throws IOException;
    ...
    /**
     * Fetch the byte[] contents of a given Binary resource's `data` element. If the data is a
     * standard base64encoded string that is embedded, return it. Otherwise, attempt to load the
     * externalized binary blob via the the externalized binary storage service.
     *
