...
/**
 * Thrown when a partition-split transaction partially fails: one or more partition sub-bundles committed
 * before a later one failed, so it cannot be rolled back atomically. Carries the committed response entries
 * per sub-bundle so the caller can report or manually revert them.
 */
...
public class PartitionedTransactionPartialFailureException extends BaseServerResponseException {
	...
	private static int deriveStatusCode(Throwable theCause) {
		...
		return Constants.STATUS_HTTP_500_INTERNAL_ERROR;
	}
