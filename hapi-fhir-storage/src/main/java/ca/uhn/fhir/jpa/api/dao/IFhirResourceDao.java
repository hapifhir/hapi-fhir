...
public interface IFhirResourceDao<T extends IBaseResource> extends IDao {
    ...
    /**
     * Create a resource - Note that this variant of the method does not take in a {@link
     * RequestDetails} and therefore can not fire any interceptors.
     *
    ...
    /**
     * @param thePerformIndexing Use with caution! If you set this to false, you need to manually
     *     perform indexing or your resources won't be indexed and searches won't work.
     * @param theRequestDetails The request details including permissions and partitioning
     *     information
     */
    ...
    /**
     * Delete a resource - Note that this variant of the method does not take in a {@link
     * RequestDetails} and therefore can not fire any interceptors.
     *
    ...
    /**
     * This method does not throw an exception if there are delete conflicts, but populates them in
     * the provided list
     */
    ...
    default <P extends IResourcePersistentId> DeleteMethodOutcome deletePidList(
        ...
        return deletePidList(
                theUrl, theResourceIds, theDeleteConflicts, theRequest, new TransactionDetails());
    }

    /**
     * Delete a list of resource Pids
     *
     * <p>CAUTION: This list does not throw an exception if there are delete conflicts. It should
     * always be followed by a call to
     * DeleteConflictUtil.validateDeleteConflictsEmptyOrThrowException(fhirContext, conflicts); to
     * actually throw the exception. The reason this method doesn't do that itself is that it is
     * expected to be called repeatedly where an earlier conflict can be removed in a subsequent
     * pass.
     *
    ...
    ExpungeOutcome expunge(
            IIdType theIIdType, ExpungeOptions theExpungeOptions, RequestDetails theRequest);

    <P extends IResourcePersistentId> void expunge(
     ...
     *     information
     */
    <MT extends IBaseMetaType> MT metaAddOperation(
     ...
     *     information
     */
    <MT extends IBaseMetaType> MT metaDeleteOperation(
     ...
     *     information
     */
    <MT extends IBaseMetaType> MT metaGetOperation(
            Class<MT> theType, IIdType theId, RequestDetails theRequestDetails);
    ...
    /**
     * Read a resource - Note that this variant of the method does not take in a {@link
     * RequestDetails} and therefore can not fire any interceptors.
     *
    ...
    /**
     * Should deleted resources be returned successfully. This should be false for a normal FHIR
     * read.
     *
     * @throws ResourceNotFoundException If the ID is not known to the server
     * @throws ResourceGoneException If the resource has been deleted and theDeletedOk is true
    ...
    /**
     * Read an entity from the database, and return it. Note that here we're talking about whatever
     * the native database representation is, not the parsed {@link IBaseResource} instance.
     *
    ...
    /**
     * Updates index tables associated with the given resource. Does not create a new version or
     * update the resource's update time.
     *
     ...
     * Search for IDs for processing a match URLs, etc.
     *
     * @param theConditionalOperationTargetOrNull If we're searching for IDs in order to satisfy a
     *     conditional create/update, this is the resource being searched for
     * @since 5.5.0
     */
    default <T extends IResourcePersistentId> List<T> searchForIds(
            SearchParameterMap theParams,
     ...
     * {@link IQueryParameterType} instances of the appropriate type for the given param
     *
    ...
    /**
     * Update a resource - Note that this variant of the method does not take in a {@link
     * RequestDetails} and therefore can not fire any interceptors.
     *
    ...
    /**
     * @param theForceUpdateVersion Create a new version with the same contents as the current
     *     version even if the content hasn't changed (this is mostly useful for resources mapping
     *     to external content such as external code systems)
     */
    DaoMethodOutcome update(
            T theResource,
            String theMatchUrl,
     ...
     * @param theRequestDetails The request details including permissions and partitioning
     *     information
     * @return MethodOutcome even if the resource fails validation it should still successfully
     *     return with a response status of 200
     */
