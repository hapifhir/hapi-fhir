...
public final class PatientEverythingParameters {
    @Description(
            formalDefinition =
                    "Results from this method are returned across multiple pages. This parameter"
                            + " controls the size of those pages.")
    private IPrimitiveType<Integer> myCount;

    @Description(
            formalDefinition =
                    "Results from this method are returned across multiple pages. This parameter"
                            + " controls the offset when fetching a page.")
    ...
    @Description(
            shortDefinition =
                    "Filter the resources to return only resources matching the given _content"
                        + " filter (note that this filter is applied only to results which link to"
                        + " the given patient, not to the patient itself or to supporting resources"
                        + " linked to by the matched resources)")
    private StringAndListParam myContent;

    @Description(
            shortDefinition =
                    "Filter the resources to return only resources matching the given _text filter"
                            + " (note that this filter is applied only to results which link to the"
                            + " given patient, not to the patient itself or to supporting resources"
                            + " linked to by the matched resources)")
    private StringAndListParam myNarrative;

    @Description(
            shortDefinition =
                    "Filter the resources to return only resources matching the given _filter"
                        + " filter (note that this filter is applied only to results which link to"
                        + " the given patient, not to the patient itself or to supporting resources"
                        + " linked to by the matched resources)")
    ...
    @Description(
            shortDefinition =
                    "Filter the resources to return only resources matching the given _type filter"
                            + " (note that this filter is applied only to results which link to the"
                            + " given patient, not to the patient itself or to supporting resources"
                            + " linked to by the matched resources)")
