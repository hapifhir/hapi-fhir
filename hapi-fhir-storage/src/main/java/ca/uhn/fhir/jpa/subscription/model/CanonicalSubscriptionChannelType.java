...
public enum CanonicalSubscriptionChannelType {
	...
	*/
	...
	public static CanonicalSubscriptionChannelType fromCode(
				@Nullable String theSystem, @Nonnull String codeString) throws FHIRException {
		...
		throw new FHIRException(
					Msg.code(569) + "Unknown SubscriptionChannelType code '" + codeString + "'");
	}
	...
	public String getDefinition() {
		switch (this) {
				case RESTHOOK:
					return "The channel is executed by making a post to the URI. If a payload is"
								+ " included, the URL is interpreted as the service base, and an update"
								+ " (PUT) is made.";
				case WEBSOCKET:
					return "The channel is executed by sending a packet across a web socket connection"
								+ " maintained by the client. The URL identifies the websocket, and the"
								+ " client binds to this URL.";
				case EMAIL:
					return "The channel is executed by sending an email to the email addressed in the"
								+ " URI (which must be a mailto:).";
				case SMS:
					return "The channel is executed by sending an SMS message to the phone number"
								+ " identified in the URL (tel:).";
				case MESSAGE:
					return "The channel is executed by sending a message (e.g. a Bundle with a"
								+ " MessageHeader resource etc.) to the application identified in the"
								+ " URI.";
				case NULL:
				default:
					return "?";
