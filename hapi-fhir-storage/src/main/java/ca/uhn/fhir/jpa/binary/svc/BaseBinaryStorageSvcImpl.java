...
public abstract class BaseBinaryStorageSvcImpl implements IBinaryStorageSvc {
    ...
    @Deprecated(
            since =
                    "6.6.0 - Maintained for interface backwards compatibility. Note that invokes"
                            + " interceptor pointcut with empty parameters",
            forRemoval = true)
    ...
    protected String provideIdForNewBlob(
        ...
        String blobPrefixFromHooksOrNull =
                callBlobIdPointcut(theBytes, theRequestDetails, theContentType);
        String blobIdPrefixFromHooks =
                blobPrefixFromHooksOrNull == null ? "" : blobPrefixFromHooksOrNull;
     ...
     */
    @Nullable
    private String callBlobIdPointcut(
        ...
        IBaseBinary binary =
                BinaryUtil.newBinary(myFhirContext)
                        .setContent(theBytes)
    ...
    public byte[] fetchDataBlobFromBinary(IBaseBinary theBaseBinary) throws IOException {
        IPrimitiveType<byte[]> dataElement =
                BinaryUtil.getOrCreateData(myFhirContext, theBaseBinary);
        ...
        if (value == null) {
            ...
            if (attachmentId.isPresent()) {
                ...
                throw new InternalErrorException(
                        Msg.code(1344)
                                + "Unable to load binary blob data for "
                                + theBaseBinary.getIdElement());
            }
        }
        return value;
    }
    ...
    @VisibleForTesting
    public void setInterceptorBroadcasterForTests(
