...
public class GraphQLProvider {
     ...
     *
     * @param theStorageServices The storage services (this object will be used to retrieve various
     *     resources as required by the GraphQL engine)
     */
            ...
            case DSTU3:
                {
                    ...
                    org.hl7.fhir.dstu3.hapi.ctx.HapiWorkerContext workerContext =
                            new org.hl7.fhir.dstu3.hapi.ctx.HapiWorkerContext(
                                    theFhirContext, validationSupport);
                    myEngineFactory =
                            () -> new org.hl7.fhir.dstu3.utils.GraphQLEngine(workerContext);
                    break;
                }
            case R4:
                {
                    ...
                    org.hl7.fhir.r4.hapi.ctx.HapiWorkerContext workerContext =
                            new org.hl7.fhir.r4.hapi.ctx.HapiWorkerContext(
                                    theFhirContext, validationSupport);
            ...
            case R4B:
                {
                    ...
                    org.hl7.fhir.r4b.hapi.ctx.HapiWorkerContext workerContext =
                            new org.hl7.fhir.r4b.hapi.ctx.HapiWorkerContext(
                                    theFhirContext, validationSupport);
            ...
            case R5:
                {
                    ...
                    org.hl7.fhir.r5.hapi.ctx.HapiWorkerContext workerContext =
                            new org.hl7.fhir.r5.hapi.ctx.HapiWorkerContext(
                                    theFhirContext, validationSupport);
            ...
            default:
                {
                    throw new UnsupportedOperationException(
                            Msg.code(1143)
                                    + "GraphQL not supported for version: "
    ...
    public String processGraphQlGetRequest(
            ...
            @GraphQLQueryUrl String theQueryUrl) {
        if (theQueryUrl != null) {
            return processGraphQLRequest(theRequestDetails, theId, theQueryUrl);
        }
        throw new InvalidRequestException(
                Msg.code(1144) + "Unable to parse empty GraphQL expression");
    }
    ...
    public String processGraphQlPostRequest(
        ...
        throw new InvalidRequestException(
                Msg.code(1145) + "Unable to parse empty GraphQL expression");
    }

    public String processGraphQLRequest(
            ServletRequestDetails theRequestDetails, IIdType theId, String theQuery) {
        Package parsedGraphQLRequest;
        try {
            parsedGraphQLRequest = Parser.parse(theQuery);
        } catch (Exception e) {
            throw new InvalidRequestException(
                    Msg.code(1146) + "Unable to parse GraphQL Expression: " + e);
        }
    ...
    protected String processGraphQLRequest(
        ...
        try {
            ...
            if (theId != null) {
                IBaseResource focus =
                        myStorageServices.lookup(
                                theRequestDetails, theId.getResourceType(), theId.getIdPart());
    ...
    public void initialize(RestfulServer theServer) {
        ...
        if (!theServer
                ...
                .getVersion()
                .getVersion()
                .isEqualOrNewerThan(FhirVersionEnum.DSTU3)) {
            throw new ConfigurationException(
                    Msg.code(1148)
                            + "Can not use "
                            + getClass().getName()
                            + " provider on server with FHIR "
                            + theServer.getFhirContext().getVersion().getVersion().name()
                            + " context");
        }
    }
}
