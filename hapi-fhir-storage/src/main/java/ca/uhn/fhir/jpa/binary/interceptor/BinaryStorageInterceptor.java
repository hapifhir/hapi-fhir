...
public class BinaryStorageInterceptor<T extends IPrimitiveType<byte[]>> {
    ...
    @Autowired private IBinaryStorageSvc myBinaryStorageSvc;
    private final FhirContext myCtx;
    @Autowired private BinaryAccessProvider myBinaryAccessProvider;
    ...
    /**
     * Don't allow clients to submit resources with binary storage attachments declared unless the
     * ID was already in the resource. In other words, only HAPI itself may add a binary storage ID
     * extension to a resource unless that extension was already present.
     */
    private void blockIllegalExternalBinaryIds(
            IBaseResource thePreviousResource, IBaseResource theResource) {
        Set<String> existingBinaryIds = new HashSet<>();
        if (thePreviousResource != null) {
            List<T> base64fields =
                    myCtx.newTerser()
                            .getAllPopulatedChildElementsOfType(thePreviousResource, myBinaryType);
        ...
        for (IBinaryTarget nextTarget : attachments) {
            ...
            if (data != null && data.length > 0) {
                ...
                boolean shouldStoreBlob =
                        myBinaryStorageSvc.shouldStoreBlob(
                                nextPayloadLength, resourceId, nextContentType);
                if (shouldStoreBlob) {
                    ...
                    if (resourceId.hasIdPart()) {
                        ...
                        assert thePointcut == Pointcut.STORAGE_PRESTORAGE_RESOURCE_CREATED
                                : thePointcut.name();
                        ...
                        if (myBinaryStorageSvc.isValidBlobId(newBlobId)) {
                            ...
                            throw new InternalErrorException(
                                    Msg.code(2341)
                                            + "Invalid blob ID for backing storage service.[blobId="
                                            + newBlobId
                                            + ",service="
    ...
    /**
     * This invokes the {@link Pointcut#STORAGE_BINARY_ASSIGN_BLOB_ID_PREFIX} hook and returns the
     * prefix to use for the blob ID, or null if there are no implementers.
     *
    ...
    private String invokeAssignBlobPrefix(RequestDetails theRequest, IBaseResource theResource) {
        if (!CompositeInterceptorBroadcaster.hasHooks(
                Pointcut.STORAGE_BINARY_ASSIGN_BLOB_ID_PREFIX,
    ...
    @Nonnull
    private List<DeferredBinaryTarget> getOrCreateDeferredBinaryStorageMap(
            TransactionDetails theTransactionDetails) {
    ...
    public void storeLargeBinariesBeforeCreatePersistence(
        ...
        List<DeferredBinaryTarget> deferredBinaryTargets =
                theTransactionDetails.getUserData(getDeferredListKey());
        if (deferredBinaryTargets != null) {
            ...
            for (DeferredBinaryTarget next : deferredBinaryTargets) {
                ...
                myBinaryStorageSvc.storeBlob(
                        resourceId, blobId, contentType, dataStream, requestDetails);
            }
        }
    }
    ...
    public void preShow(IPreResourceShowDetails theDetails) throws IOException {
        ...
        for (IBaseResource nextResource : theDetails) {
            if (nextResource == null) {
                ourLog.warn(
                        "Received a null resource during STORAGE_PRESHOW_RESOURCES. This is a bug"
                                + " and should be reported. Skipping resource.");
                continue;
            }
            cumulativeInflatedBytes =
                    inflateBinariesInResource(cumulativeInflatedBytes, nextResource);
        ...
        ourLog.debug(
                "Exiting binary data inflation having inflated everything.[byteCount={},"
                        + " resourcesInflated={}, resourcesSkipped=0]",
                cumulativeInflatedBytes,
                inflatedResourceCount);
    }

    private long inflateBinariesInResource(
            long theCumulativeInflatedBytes, IBaseResource theResource) throws IOException {
        IIdType resourceId = theResource.getIdElement();
        List<IBinaryTarget> attachments = recursivelyScanResourceForBinaryData(theResource);
        for (IBinaryTarget nextTarget : attachments) {
            ...
            if (attachmentId.isPresent()) {
                ...
                StoredDetails blobDetails =
                        myBinaryStorageSvc.fetchBlobDetails(resourceId, attachmentId.get());
                if (blobDetails == null) {
                    String msg =
                            myCtx.getLocalizer()
                                    .getMessage(BinaryAccessProvider.class, "unknownBlobId");
                    throw new InvalidRequestException(Msg.code(1330) + msg);
                }

                if ((theCumulativeInflatedBytes + blobDetails.getBytes())
                        < myAutoInflateBinariesMaximumBytes) {
                ...
                .visit(
                        ...
                        new IModelVisitor2() {
                            ...
                            public boolean acceptElement(
                                    ...
                                    List<BaseRuntimeElementDefinition<?>>
                                            theElementDefinitionPath) {

                                if (theElement.getClass().equals(myBinaryType)) {
                                    IBase parent =
                                            theContainingElementPath.get(
                                                    theContainingElementPath.size() - 2);
                                    Optional<IBinaryTarget> binaryTarget =
                                            myBinaryAccessProvider.toBinaryTarget(parent);
    ...
    private static class DeferredBinaryTarget {
        ...
        private DeferredBinaryTarget(
                String theBlobId, IBinaryTarget theBinaryTarget, byte[] theData) {
            myBlobId = theBlobId;
