...
public class MatchResourceUrlService<T extends IResourcePersistentId> {
	...
	private static final org.slf4j.Logger ourLog =
				org.slf4j.LoggerFactory.getLogger(MatchResourceUrlService.class);
	...
	public <R extends IBaseResource> Set<T> processMatchUrl(
		...
		return processMatchUrl(
					theMatchUrl, theResourceType, theTransactionDetails, theRequest, null);
	}
	...
	public <R extends IBaseResource> Set<T> processMatchUrl(
		...
		if (resolvedInTransaction != null) {
				// If the resource has previously been looked up within the transaction, there's no need
				// to re-authorize it.
		...
		if (retVal == null) {
				...
				SearchParameterMap paramMap =
						myMatchUrlService.translateMatchUrl(matchUrl, resourceDef);
				if (paramMap.isEmpty() && paramMap.getLastUpdated() == null) {
					throw new InvalidRequestException(
								Msg.code(518)
										+ "Invalid match URL["
										+ matchUrl
										+ "] - URL has no search parameters");
				}
		...
		if (CompositeInterceptorBroadcaster.hasHooks(
				...
				SimplePreResourceShowDetails accessDetails =
						new SimplePreResourceShowDetails(resourceToPidMap.keySet());
				HookParams params =
						new HookParams()
		...
		if (retVal.size() == 1) {
				...
				if (myStorageSettings.isMatchUrlCacheEnabled()) {
					myMemoryCacheService.putAfterCommit(
								MemoryCacheService.CacheEnum.MATCH_URL, matchUrl, pid);
				}
		}
	...
	private <R extends IBaseResource> IFhirResourceDao<R> getResourceDao(Class<R> theResourceType) {
		...
		if (dao == null) {
				throw new InternalErrorException(
						Msg.code(519) + "No DAO for resource type: " + theResourceType.getName());
		}
		return dao;
	}
		...
		if (myStorageSettings.isMatchUrlCacheEnabled()) {
				...
				existing =
						myMemoryCacheService.getIfPresent(
									MemoryCacheService.CacheEnum.MATCH_URL, matchUrl);
		}
		return existing;
	}
	...
	public <R extends IBaseResource> Set<T> search(
		...
		List<T> retVal =
					dao.searchForIds(theParamMap, theRequest, theConditionalOperationTargetOrNull);
		...
		if (CompositeInterceptorBroadcaster.hasHooks(
				...
				message.setMessage(
						"Processed conditional resource URL with "
									+ retVal.size()
									+ " result(s) in "
									+ sw);
				HookParams params =
