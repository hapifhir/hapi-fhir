...
/**
 * This is a query listener designed to be plugged into a {@link ProxyDataSourceBuilder proxy
 * DataSource}. This listener keeps the last 1000 queries across all threads in a {@link
 * CircularFifoQueue}, dropping queries off the end of the list as new ones are added.
 *
 * <p>Note that this class is really only designed for use in testing - It adds a non-trivial
...
public class CircularQueueCaptureQueriesListener extends BaseCaptureQueriesListener {
    ...
    private static final Logger ourLog =
            LoggerFactory.getLogger(CircularQueueCaptureQueriesListener.class);
    ...
    public String logUpdateQueriesForCurrentThread() {
        List<String> queries =
                getUpdateQueriesForCurrentThread().stream()
    ...
    public String logSelectQueriesForCurrentThread(int... theIndexes) {
        List<String> queries =
                getSelectQueriesForCurrentThread().stream()
                        .map(CircularQueueCaptureQueriesListener::formatQueryAsSql)
                        .collect(Collectors.toList());
    ...
    public List<SqlQuery> logSelectQueries(boolean theInlineParams, boolean theFormatSql) {
        ...
        List<String> queriesStrings =
                queries.stream()
                        .map(
                                t ->
                                        CircularQueueCaptureQueriesListener.formatQueryAsSql(
                                                t, theInlineParams, theFormatSql))
                        .collect(Collectors.toList());
        ourLog.info("Select Queries:\n{}", String.join("\n", queriesStrings));
        return queries;
    }
    ...
    public String logInsertQueriesForCurrentThread() {
        List<String> queries =
                getInsertQueriesForCurrentThread().stream()
                        .map(CircularQueueCaptureQueriesListener::formatQueryAsSql)
                        .collect(Collectors.toList());
        String queriesAsString = String.join("\n", queries);
    ...
    public void logAllQueriesForCurrentThread() {
        List<String> queries =
                getAllQueriesForCurrentThread().stream()
                        .map(CircularQueueCaptureQueriesListener::formatQueryAsSql)
                        .collect(Collectors.toList());
        ourLog.info("Queries:\n{}", String.join("\n", queries));
    }
    ...
    public String logDeleteQueriesForCurrentThread() {
        List<String> queries =
                getDeleteQueriesForCurrentThread().stream()
                        .map(CircularQueueCaptureQueriesListener::formatQueryAsSql)
                        .collect(Collectors.toList());
        String joined = String.join("\n", queries);
    ...
    static String formatQueryAsSql(SqlQuery theQuery, boolean inlineParams, boolean formatSql) {
        ...
        if (theQuery.getStackTrace() != null) {
            ...
            Stream<String> stackTraceStream =
                    Arrays.stream(theQuery.getStackTrace())
