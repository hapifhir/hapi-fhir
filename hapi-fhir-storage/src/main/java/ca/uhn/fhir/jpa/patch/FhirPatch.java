...
public class FhirPatch {
     ...
     * <code>ResourceName.fieldName.fieldName</code> and wildcards are supported, such as <code>
     * *.meta</code>.
     */
    ...
    public void apply(IBaseResource theResource, IBaseResource thePatch) {
        ...
        List<IBase> opParameters =
                ParametersUtil.getNamedParameters(myContext, thePatch, PARAMETER_OPERATION);
        for (IBase nextOperation : opParameters) {
            String type =
                    ParametersUtil.getParameterPartValueAsString(
                            myContext, nextOperation, PARAMETER_TYPE);
            ...
            if (OPERATION_DELETE.equals(type)) {
                ...
                throw new InvalidRequestException(
                        Msg.code(1267) + "Unknown patch operation type: " + type);
            }
        }
    }
    ...
    private void handleAddOperation(IBaseResource theResource, IBase theParameters) {
        ...
        String path =
                ParametersUtil.getParameterPartValueAsString(
                        myContext, theParameters, PARAMETER_PATH);
        String elementName =
                ParametersUtil.getParameterPartValueAsString(
                        myContext, theParameters, PARAMETER_NAME);

        String containingPath = defaultString(path);

        List<IBase> containingElements =
                myContext.newFhirPath().evaluate(theResource, containingPath, IBase.class);
    ...
    private void handleInsertOperation(IBaseResource theResource, IBase theParameters) {
        ...
        Integer insertIndex =
                ParametersUtil.getParameterPartValueAsInteger(
                                myContext, theParameters, PARAMETER_INDEX)
                        .orElseThrow(
                                () ->
                                        new InvalidRequestException(
                                                "No index supplied for insert operation"));
        ...
        for (IBase nextElement : containingElements) {
            ...
            List<IBase> existingValues =
                    new ArrayList<>(
                            childDefinition.getChildDef().getAccessor().getValues(nextElement));
    ...
    private void handleDeleteOperation(IBaseResource theResource, IBase theParameters) {
        ...
        if (path.endsWith(")")) {
            ...
            int filterArgsIndex =
                    path.lastIndexOf('('); // Let's hope there aren't nested parentheses
            int lastDotIndex =
                    path.lastIndexOf(
                            '.',
                            filterArgsIndex); // There might be a dot inside the parentheses, so
    ...
    private void deleteFromList(
        ...
        ChildDefinition childDefinition =
                findChildDefinition(theContainingElement, theListElementName);
        ...
        List<IBase> existingValues =
                new ArrayList<>(
                        childDefinition
                                .getChildDef()
                                .getAccessor()
                                .getValues(theContainingElement));
        ...
        Integer insertIndex =
                ParametersUtil.getParameterPartValueAsInteger(
                        ...
                        .orElseThrow(
                                () ->
                                        new InvalidRequestException(
                                                "No index supplied for move operation"));
        Integer removeIndex =
                ParametersUtil.getParameterPartValueAsInteger(
                                myContext, theParameters, PARAMETER_SOURCE)
                        .orElseThrow(
                                () ->
                                        new InvalidRequestException(
                                                "No index supplied for move operation"));
    ...
    private ChildDefinition findChildDefinition(IBase theContainingElement, String theElementName) {
        BaseRuntimeElementDefinition<?> elementDef =
                myContext.getElementDefinition(theContainingElement.getClass());
    ...
    private IBase getNewValue(
            IBase theParameters, IBase theElement, ChildDefinition theChildDefinition) {
        Optional<IBase> valuePart =
                ParametersUtil.getParameterPart(myContext, theParameters, PARAMETER_VALUE);
        ...
        if (IBaseEnumeration.class.isAssignableFrom(
                ...
                || XhtmlNode.class.isAssignableFrom(
                        theChildDefinition.getChildElement().getImplementingClass())) {
            // If the compositeElementDef is an IBaseEnumeration, we will use the actual
            // compositeElementDef definition to build one, since
    ...
    private void populateNewValue(
            ChildDefinition theChildDefinition, IBase theNewValue, IBase theValueElement) {
        ...
        for (IBase nextValuePartPart : valuePartParts) {
            ...
            if (isNotBlank(name)) {
                ...
                Optional<IBase> value =
                        myContext
                                .newTerser()
                                .getSingleValue(nextValuePartPart, "value[x]", IBase.class);
                if (value.isPresent()) {
    ...
    public IBaseParameters diff(
            @Nullable IBaseResource theOldValue, @Nonnull IBaseResource theNewValue) {
        ...
        if (theOldValue == null) {
            ...
            IBase operation =
                    ParametersUtil.addParameterToParameters(myContext, retVal, PARAMETER_OPERATION);
            ...
            Validate.isTrue(
                    oldValueTypeName.equalsIgnoreCase(newValueTypeName),
                    "Resources must be of same type");
    ...
    private void compare(
        ...
        BaseRuntimeElementDefinition<?> sourceDef =
                myContext.getElementDefinition(theOldField.getClass());
        BaseRuntimeElementDefinition<?> targetDef =
                myContext.getElementDefinition(theNewField.getClass());
        if (!sourceDef.getName().equals(targetDef.getName())) {
            IBase operation =
                    ParametersUtil.addParameterToParameters(
                            myContext, theDiff, PARAMETER_OPERATION);
            ...
            if (theOldField instanceof IPrimitiveType) {
                ...
                if (!Objects.equals(oldValueAsString, newValueAsString)) {
                    ...
                    ParametersUtil.addPartCode(
                            myContext, operation, PARAMETER_TYPE, OPERATION_REPLACE);
                    ParametersUtil.addPartString(
                            myContext, operation, PARAMETER_PATH, theTargetPath);
    ...
    private void compareField(
        ...
        while (sourceIndex < sourceValues.size() && targetIndex < targetValues.size()) {
            ...
            BaseRuntimeElementDefinition<?> def =
                    myContext.getElementDefinition(sourceChildField.getClass());
            ...
            String sourcePath =
                    theSourcePath + "." + elementName + (repeatable ? "[" + sourceIndex + "]" : "");
            String targetPath =
                    theSourcePath + "." + elementName + (repeatable ? "[" + targetIndex + "]" : "");
    ...
    private void addInsertItems(
        ...
        IBase operation =
                ParametersUtil.addParameterToParameters(myContext, theDiff, PARAMETER_OPERATION);
        ParametersUtil.addPartCode(myContext, operation, PARAMETER_TYPE, OPERATION_INSERT);
        ...
        if (valueDef.isStandardType()) {
            ...
            for (BaseRuntimeChildDefinition nextChild : valueDef.getChildren()) {
                ...
                for (int index = 0; index < childValues.size(); index++) {
                    ...
                    String elementName =
                            nextChild.getChildNameByDatatype(childValues.get(index).getClass());
                    String targetPath =
                            thePath
                                    + (childRepeatable ? "[" + index + "]" : "")
                                    + "."
                                    + elementName;
    ...
    private static class ChildDefinition {
        ...
        public ChildDefinition(
                BaseRuntimeChildDefinition theChildDef,
                BaseRuntimeElementDefinition<?> theChildElement) {
