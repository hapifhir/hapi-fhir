...
public class SearchParameterDaoValidator {
    ...
    private static final Pattern REGEX_SP_EXPRESSION_HAS_PATH =
    ...
    public void validate(SearchParameter searchParameter) {
            ...
            for (IPrimitiveType<?> nextBaseType : searchParameter.getBase()) {
                ...
                RuntimeSearchParam existingSearchParam =
                        mySearchParamRegistry.getActiveSearchParam(
                                nextBase, searchParameter.getCode());
                if (existingSearchParam != null) {
                    boolean isBuiltIn = existingSearchParam.getId() == null;
                    isBuiltIn |=
                            existingSearchParam
                                    .getUri()
                                    .startsWith("http://hl7.org/fhir/SearchParameter/");
                    if (isBuiltIn) {
                        throw new UnprocessableEntityException(
                                Msg.code(1111)
                                        + "Can not override built-in search parameter "
                                        + nextBase
                                        + ":"
                                        + searchParameter.getCode()
                                        + " because overriding is disabled on this server");
                    }
                }
            }
        }
        ...
        if (searchParameter.getStatus() == null) {
            throw new UnprocessableEntityException(
                    Msg.code(1112) + "SearchParameter.status is missing or invalid");
        }
        ...
        if (isCompositeWithoutBase(searchParameter)) {
            throw new UnprocessableEntityException(
                    Msg.code(1113) + "SearchParameter.base is missing");
        }
        ...
        if (isCompositeWithoutExpression(searchParameter)) {
            ...
            throw new UnprocessableEntityException(
                    Msg.code(1114) + "SearchParameter.expression is missing");
    ...
    private boolean isCompositeWithoutBase(SearchParameter searchParameter) {
        return ElementUtil.isEmpty(searchParameter.getBase())
                && ElementUtil.isEmpty(
                        searchParameter.getExtensionsByUrl(
    ...
    private boolean isCompositeSpForUniqueIndexing(SearchParameter theSearchParameter) {
        return isCompositeSp(theSearchParameter)
                && hasAnyExtensionUniqueSetTo(theSearchParameter, true);
    }
    ...
    private void maybeValidateCompositeSpForUniqueIndexing(SearchParameter theSearchParameter) {
        if (isCompositeSpForUniqueIndexing(theSearchParameter)) {
            if (!theSearchParameter.hasComponent()) {
                throw new UnprocessableEntityException(
                        Msg.code(1115)
                                + "SearchParameter is marked as unique but has no components");
            }
            for (SearchParameter.SearchParameterComponentComponent next :
                    theSearchParameter.getComponent()) {
                if (isBlank(next.getDefinition())) {
                    throw new UnprocessableEntityException(
                            Msg.code(1116)
                                    + "SearchParameter is marked as unique but is missing"
    ...
    private void validateExpressionPath(SearchParameter theSearchParameter) {
        ...
        boolean isResourceOfTypeComposite =
                theSearchParameter.getType() == Enumerations.SearchParamType.COMPOSITE;
        boolean isResourceOfTypeSpecial =
                theSearchParameter.getType() == Enumerations.SearchParamType.SPECIAL;
        ...
        if (!isUnique
                && !isResourceOfTypeComposite
                && !isResourceOfTypeSpecial
                && !expressionHasPath) {
            throw new UnprocessableEntityException(
                    Msg.code(1120)
                            + "SearchParameter.expression value \""
                            + expression
    ...
    private void validateExpressionIsParsable(SearchParameter theSearchParameter) {
        ...
        try {
            ...
            throw new UnprocessableEntityException(
                    Msg.code(1121)
                            + "Invalid FHIRPath format for SearchParameter.expression \""
                            + expression
                            + "\": "
    ...
    private boolean hasAnyExtensionUniqueSetTo(
            SearchParameter theSearchParameter, boolean theValue) {
    ...
    private void validateCompositeSearchParameterComponents(SearchParameter theSearchParameter) {
        theSearchParameter.getComponent().stream()
                ...
                .forEach(
                        theRuntimeSp ->
                                validateComponentSpTypeAgainstWhiteList(
                                        theRuntimeSp,
                                        getAllowedSearchParameterTypes(theSearchParameter)));
    }
    ...
    private void validateComponentSpTypeAgainstWhiteList(
        ...
        if (!theAllowedSearchParamTypes.contains(theRuntimeSearchParam.getParamType())) {
            throw new UnprocessableEntityException(
                    String.format(
                            "%sInvalid component search parameter type: %s in component.definition:"
                                    + " %s, supported types: %s",
                            Msg.code(2347),
                            theRuntimeSearchParam.getParamType().name(),
                            theRuntimeSearchParam.getUri(),
                            theAllowedSearchParamTypes.stream()
                                    .map(Enum::name)
     ...
     */
    private Set<RestSearchParameterTypeEnum> getAllowedSearchParameterTypes(
            SearchParameter theSearchParameter) {
        ...
        if (hasAnyExtensionUniqueSetTo(theSearchParameter, true)) {
            ...
            // combo non-unique search parameter or composite Search Parameter with HSearch indexing
                ...
                myStorageSettings
                        .isAdvancedHSearchIndexing()) { // composite Search Parameter with HSearch
            // indexing
