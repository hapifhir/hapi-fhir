...
public abstract class BaseSearchParamWithInlineReferencesExtractor<T extends IResourcePersistentId>
    ...
    private static final Logger ourLog =
            LoggerFactory.getLogger(BaseSearchParamWithInlineReferencesExtractor.class);

    protected FhirContext myFhirContext;
    protected JpaStorageSettings myStorageSettings;
    @Autowired private MatchResourceUrlService<T> myMatchResourceUrlService;
    @Autowired private DaoResourceLinkResolver<T> myDaoResourceLinkResolver;
    ...
    public void extractInlineReferences(
        ...
        List<IBaseReference> allRefs =
                terser.getAllPopulatedChildElementsOfType(theResource, IBaseReference.class);
        for (IBaseReference nextRef : allRefs) {
            ...
            if (qmIndex != -1) {
                ...
                RuntimeResourceDefinition matchResourceDef =
                        myFhirContext.getResourceDefinition(resourceTypeString);
                ...
                Class<? extends IBaseResource> matchResourceType =
                        matchResourceDef.getImplementingClass();
                ...
                if (resolvedMatch != null
                        && !IResourcePersistentId.NOT_FOUND.equals(resolvedMatch)) {
                ...
                if (matches.isEmpty()) {
                    ...
                    if (placeholderOpt.isPresent()) {
                        ...
                        myMemoryCacheService.putAfterCommit(
                                MemoryCacheService.CacheEnum.MATCH_URL, nextIdText, match);
                    } else {
                        String msg =
                                myFhirContext
                                        .getLocalizer()
                ...
                if (newId == null) {
                    newId =
                            myIdHelperService.translatePidIdToForcedId(
                                    myFhirContext, resourceTypeString, match);
                }
                ourLog.debug(
                ...
                if (theTransactionDetails != null) {
                    ...
                    theTransactionDetails.addRollbackUndoAction(
                            () -> nextRef.setReference(previousReference));
                }
    ...
    // Removes parts of the reference keeping only the valuable parts, the resource type and
    // searchparam
