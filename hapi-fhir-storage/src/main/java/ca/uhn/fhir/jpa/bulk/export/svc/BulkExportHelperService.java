...
public class BulkExportHelperService {
	...
	/**
	* Given the parameters, create the search parameter map based on type filters and the _since
	* parameter.
	*
	* <p>The input boolean theConsiderSince determines whether to consider the lastUpdated date in
	* the search parameter map.
	*/
	public List<SearchParameterMap> createSearchParameterMapsForResourceType(
								...
								.map(
										filter ->
													buildSearchParameterMapForTypeFilter(
																filter, theDef, theParams.getStartDate()))
		...
		typeFilters.stream()
					...
					.forEach(
								filter -> {
									ourLog.warn(
												"Found a strange _typeFilter that we could not process: {}."
														+ " _typeFilters should follow the format"
														+ " ResourceType?searchparameter=value .",
												filter);
								});
	...
	private SearchParameterMap buildSearchParameterMapForTypeFilter(
		...
		SearchParameterMap searchParameterMap =
					myMatchUrlService.translateMatchUrl(theFilter, theDef);
	...
	private void enhanceSearchParameterMapWithCommonParameters(
				SearchParameterMap map, Date theSinceDate) {
	...
	/**
	* Converts the ResourceId to an IIdType. Eg: Patient/123 -> IIdType
	*
