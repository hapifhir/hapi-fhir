...
/**
 * This interceptor allows JPA servers to be partitioned by Patient ID. It selects the compartment
 * for read/create operations based on the patient ID associated with the resource (and uses a
 * default partition ID for any resources not in the patient compartment).
 */
@Interceptor
public class PatientIdPartitionInterceptor {
	...
	@Autowired private ISearchParamExtractor mySearchParamExtractor;
		...
		if (resourceDef.getName().equals("Patient")) {
				...
				if (isBlank(compartmentIdentity)) {
					throw new MethodNotAllowedException(
								Msg.code(1321)
										+ "Patient resource IDs must be client-assigned in patient"
										+ " compartment mode");
				}
		} else {
				compartmentIdentity =
						compartmentSps.stream()
									.flatMap(
												param ->
														Arrays.stream(
																	BaseSearchParamExtractor.splitPathsR4(
																				param.getPath())))
	...
	public RequestPartitionId identifyForRead(
				...
				case VREAD:
					if ("Patient".equals(theReadDetails.getResourceType())) {
						return provideCompartmentMemberInstanceResponse(
									theRequestDetails, theReadDetails.getReadResourceId().getIdPart());
					}
					break;
				case SEARCH_TYPE:
					...
					if ("Patient".equals(theReadDetails.getResourceType())) {
						...
						for (RuntimeSearchParam nextCompartmentSp : compartmentSps) {
								idPart =
										getSingleResourceIdValueOrNull(
													params, nextCompartmentSp.getName(), "Patient");
								if (idPart != null) {
									break;
								}
						}
		...
		if (theReadDetails.getConditionalTargetOrNull() != null) {
				return identifyForCreate(
						theReadDetails.getConditionalTargetOrNull(), theRequestDetails);
		}
					...
					if (isNotBlank(idParam.getQueryParameterQualifier())) {
						throw new MethodNotAllowedException(
									Msg.code(1322)
												+ "The parameter "
												+ theParamName
												+ idParam.getQueryParameterQualifier()
	...
	/**
	* Return a partition or throw an error for FHIR operations that can not be used with this
	* interceptor
	*/
	...
	/**
	* Generate the partition for a given patient resource ID. This method may be overridden in
	* subclasses, but it may be easier to override {@link
	* #providePartitionIdForPatientId(RequestDetails, String)} instead.
	*/
	@Nonnull
	...
	* Math.abs(theResourceIdPart.hashCode()) % 15000</code>.
	*
	* <p>This logic can be replaced with other logic of your choosing.
	*/
	@SuppressWarnings("unused")
	protected int providePartitionIdForPatientId(
				RequestDetails theRequestDetails, String theResourceIdPart) {
		return Math.abs(theResourceIdPart.hashCode() % 15000);
	}

	/**
	* Return a compartment ID (or throw an exception) when an attempt is made to search for a
	* resource that is in the patient compartment, but without any search parameter identifying
	* which compartment to search.
	*
	* <p>E.g. this method will be called for the search <code>Observation?code=foo</code> since the
	* patient is not identified in the URL.
	*/
	@Nonnull
	protected RequestPartitionId provideNonCompartmentMemberInstanceResponse(
	...
	/**
	* Return a compartment ID (or throw an exception) when storing/reading resource types that are
	* not in the patient compartment (e.g. ValueSet).
	*/
	@SuppressWarnings("unused")
	@Nonnull
