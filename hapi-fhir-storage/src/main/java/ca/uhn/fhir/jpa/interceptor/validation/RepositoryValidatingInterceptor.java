...
/**
 * This interceptor enforces validation rules on any data saved in a HAPI FHIR JPA repository. See
 * <a
...
public class RepositoryValidatingInterceptor {
	...
	private static final Logger ourLog =
				LoggerFactory.getLogger(RepositoryValidatingInterceptor.class);
	...
	* Constructor
	*
	* <p>If this constructor is used, {@link #setFhirContext(FhirContext)} and {@link
	* #setRules(List)} must be called manually before the interceptor is used.
	*/
	...
	*/
	public RepositoryValidatingInterceptor(
				FhirContext theFhirContext, List<IRepositoryValidatingRule> theRules) {
	...
	/**
	* Returns a multiline string describing the rules in place for this interceptor. This is mostly
	* intended for troubleshooting, and the format returned is only semi-human-consumable.
	*/
	@Nonnull
	...
	private void handle(RequestDetails theRequestDetails, IBaseResource theNewResource) {
		...
		Validate.notNull(
					myFhirContext,
					"No FhirContext has been set for this interceptor of type: %s",
					getClass());
		if (!isPlaceholderResource(theNewResource)) {
				...
				for (IRepositoryValidatingRule nextRule : rules) {
					IRepositoryValidatingRule.RuleEvaluation outcome =
								nextRule.evaluate(theRequestDetails, theNewResource);
					if (!outcome.isPasses()) {
	...
	/**
	* Return true if the given resource is a placeholder resource, as identified by a specific
	* extension
	*
		...
		if (theOutcome.getOperationOutcome() != null) {
				String firstIssue =
						OperationOutcomeUtil.getFirstIssueDetails(
									myFhirContext, theOutcome.getOperationOutcome());
				throw new PreconditionFailedException(
						Msg.code(574) + firstIssue, theOutcome.getOperationOutcome());
		}
