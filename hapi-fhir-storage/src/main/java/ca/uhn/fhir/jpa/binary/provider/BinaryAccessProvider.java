...
/**
 * This plain provider class can be registered with a JPA RestfulServer to provide the <code>
 * $binary-access-read</code> and <code>$binary-access-write</code> operations that can be used to
 * access attachment data as a raw binary.
 */
public class BinaryAccessProvider {
    ...
    public void binaryAccessRead(
        ...
        if (attachmentId.isPresent()) {
            ...
            theServletResponse.addHeader(
                    Constants.HEADER_CACHE_CONTROL, Constants.CACHE_CONTROL_PRIVATE);
            theServletResponse.addHeader(Constants.HEADER_ETAG, '"' + blobDetails.getHash() + '"');
            theServletResponse.addHeader(
                    Constants.HEADER_LAST_MODIFIED,
                    DateUtils.formatDate(blobDetails.getPublished()));

            myBinaryStorageSvc.writeBlob(
                    theResourceId, blobId, theServletResponse.getOutputStream());
    ...
    public IBaseResource binaryAccessWrite(
        ...
        if (EncodingEnum.forContentTypeStrict(requestContentType) != null) {
            throw new InvalidRequestException(
                    Msg.code(1334)
                            + "This operation is for binary content, got: "
                            + requestContentType);
        }
        ...
        if (size > 0 && myBinaryStorageSvc != null) {
            if (bytes == null || bytes.length == 0) {
                throw new IllegalStateException(
                        Msg.code(2073)
                                + "Input stream is empty! Ensure that you are uploading data, and"
                                + " if so, ensure that no interceptors are in use that may be"
            ...
            if (myBinaryStorageSvc.shouldStoreBlob(size, theResourceId, requestContentType)) {
                ...
                Validate.notBlank(
                        blobId, "BinaryStorageSvc returned a null blob ID"); // should not happen
                Validate.isTrue(
                        size == theServletRequest.getContentLength(),
    ...
    private IBinaryTarget findAttachmentForRequest(
        ...
        Optional<IBase> type =
                myCtx.newFluentPath().evaluateFirst(theResource, thePath, IBase.class);
        ...
        if (type.isEmpty()) {
            String msg =
                    this.myCtx
                            .getLocalizer()
                            .getMessageSanitized(
                                    BinaryAccessProvider.class, "unknownPath", resType, thePath);
                    ...
                    new IBinaryTarget() {
                        ...
                        public void setSize(Integer theSize) {
                            AttachmentUtil.setSize(
                                    BinaryAccessProvider.this.myCtx, attachment, theSize);
                        }

                        @Override
                        public String getContentType() {
                            return AttachmentUtil.getOrCreateContentType(
                        ...
                        public void setContentType(String theContentType) {
                            AttachmentUtil.setContentType(
                                    BinaryAccessProvider.this.myCtx, attachment, theContentType);
                        }
        ...
        if (def.getName().equals("Binary")) {
            ...
            binaryTarget =
                    new IBinaryTarget() {
                        ...
                        public IBaseHasExtensions getTarget() {
                            return (IBaseHasExtensions)
                                    BinaryUtil.getOrCreateData(
                                            BinaryAccessProvider.this.myCtx, binary);
                        }
