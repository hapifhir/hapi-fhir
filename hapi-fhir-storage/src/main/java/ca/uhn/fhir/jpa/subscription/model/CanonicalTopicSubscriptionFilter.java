...
public class CanonicalTopicSubscriptionFilter {
        ...
        params.forEach(
                (key, valueList) -> {
                    for (String value : valueList) {
                        CanonicalTopicSubscriptionFilter filter =
                                new CanonicalTopicSubscriptionFilter();
                        filter.setResourceType(resourceName);
                        filter.setFilterParameter(key);
                        // WIP STR5 set modifier and comparator properly.  This may be tricky
                        // without access to searchparameters,
    ...
    public Subscription.SubscriptionFilterByComponent toSubscriptionFilterByComponent() {
        Subscription.SubscriptionFilterByComponent retval =
                new Subscription.SubscriptionFilterByComponent();
