...
public class ExpungeOperation implements Callable<ExpungeOutcome> {
    ...
    private List<IResourcePersistentId> findHistoricalVersionsOfDeletedResources() {
        List<IResourcePersistentId> retVal =
                myExpungeDaoService.findHistoricalVersionsOfDeletedResources(
    ...
    private void expungeOldVersions() {
        ...
        getPartitionRunner()
                .runInPartitionedThreads(
                        historicalIds,
                        partition ->
                                myExpungeDaoService.expungeHistoricalVersions(
    ...
    private void deleteCurrentVersionsOfDeletedResources(
            List<IResourcePersistentId> theResourceIds) {
        getPartitionRunner()
                .runInPartitionedThreads(
                        theResourceIds,
                        partition ->
                                myExpungeDaoService.expungeCurrentVersionOfResources(
                                        myRequestDetails, partition, myRemainingCount));
    }
    ...
    private void deleteHistoricalVersions(List<IResourcePersistentId> theResourceIds) {
        getPartitionRunner()
                .runInPartitionedThreads(
                        theResourceIds,
                        partition ->
                                myExpungeDaoService.expungeHistoricalVersionsOfIds(
                                        myRequestDetails, partition, myRemainingCount));
    }
