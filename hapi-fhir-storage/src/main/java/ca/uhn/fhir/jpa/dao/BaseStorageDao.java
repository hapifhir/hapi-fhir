...
public abstract class BaseStorageDao {
    ...
    protected static final String MESSAGE_KEY_DELETE_RESOURCE_NOT_EXISTING =
            "deleteResourceNotExisting";
    protected static final String MESSAGE_KEY_DELETE_RESOURCE_ALREADY_DELETED =
            "deleteResourceAlreadyDeleted";

    @Autowired protected ISearchParamRegistry mySearchParamRegistry;
    @Autowired protected FhirContext myFhirContext;
    @Autowired protected DaoRegistry myDaoRegistry;
    @Autowired protected IResourceVersionSvc myResourceVersionSvc;
    @Autowired protected JpaStorageSettings myStorageSettings;
     ...
     * @deprecated Use {@link #preProcessResourceForStorage(IBaseResource, RequestDetails,
     *     TransactionDetails, boolean)} instead
     */
    ...
    private void replaceAbsoluteReferencesWithRelative(
            IBaseResource theResource, FhirTerser theTerser) {
     ...
     *
     * <p>We only do this if thePerformIndexing is true because if it's false, that means we're in a
     * FHIR transaction during the first phase of write operation processing, meaning that the
     * versions of other resources may not have need updatd yet. For example we're about to store an
     * Observation with a reference to a Patient, and that Patient is also being updated in the same
     * transaction, during the first "no index" phase, the Patient will not yet have its version
     * number incremented, so it would be wrong to use that value. During the second phase it is
     * correct.
     *
     * <p>Also note that {@link BaseTransactionProcessor} also has code to do auto-versioning and it
     * is the one that takes care of the placeholder IDs. Look for the other caller of {@link
     * #extractReferencesToAutoVersion(FhirContext, StorageSettings, IBaseResource)} to find this.
     */
    ...
    public IBaseOperationOutcome createInfoOperationOutcome(
        ...
        return createOperationOutcome(
                OO_SEVERITY_INFO, theMessage, "informational", theStorageResponseCode);
    }

    private IBaseOperationOutcome createOperationOutcome(
            String theSeverity, String theMessage, String theCode) {
     ...
     *
     * <p>Additional information may be set on the outcome.
     *
    ...
    protected DaoMethodOutcome createMethodOutcomeForResourceId(
        ...
        String message =
                getContext().getLocalizer().getMessage(BaseStorageDao.class, theMessageKey, id);
        String severity = "information";
        String code = "informational";
        IBaseOperationOutcome oo =
                createOperationOutcome(severity, message, code, theStorageResponseCode);
    ...
    @Nonnull
    protected ResourceGoneException createResourceGoneException(
            IBasePersistedResource theResourceEntity) {
    ...
    public void translateRawParameters(
        ...
        ResourceSearchParams searchParams =
                mySearchParamRegistry.getActiveSearchParams(getResourceName());

        Set<String> paramNames = theSource.keySet();
        for (String nextParamName : paramNames) {
            QualifierDetails qualifiedParamName =
                    QualifierDetails.extractQualifiersFromParameterName(nextParamName);
            ...
            RuntimeSearchParam paramDef =
                    mySearchParamRegistry.getActiveSearchParam(
                            getResourceName(), qualifiedParamName.getParamName());

            for (String nextValue : theSource.get(nextParamName)) {
                QualifiedParamList qualifiedParam =
                ...
                IQueryParameterAnd<?> parsedParam =
                        JpaParamUtil.parseQueryParams(
        ...
        if (!theStorageSettings.getAutoVersionReferenceAtPaths().isEmpty()) {
            ...
            for (String nextPath :
                ...
                List<IBaseReference> nextReferences =
                        theFhirContext
                                .newTerser()
                                .getValues(theResource, nextPath, IBaseReference.class);
