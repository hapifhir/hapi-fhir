...
public abstract class BaseStorageDao {
	...
	protected void preProcessResourceForStorage(
		...
		performAutoVersioning(theResource, thePerformIndexing, theRequestDetails);
	 ...
	 * to find this.
	 */
	private void performAutoVersioning(
			IBaseResource theResource, boolean thePerformIndexing, RequestDetails theRequestDetails) {
		if (thePerformIndexing) {
			...
			for (IBaseReference nextReference : referencesToVersion) {
				...
				if (!referenceElement.hasBaseUrl()) {
					...
					RequestPartitionId lookupPartitionId =
							resolveAutoVersionLookupPartition(theResource, theRequestDetails);
					ResourcePersistentIdMap resourceVersionMap = myResourceVersionSvc.getLatestVersionIdsForResourceIds(
							lookupPartitionId, Collections.singletonList(referenceElement));
	 ...
	 * during auto-versioning. When cross-partition references are not allowed, the lookup is
	 * scoped to the write partition of the source resource to prevent leaking references to
	 * resources in other partitions.
	 */
	// Generated by claude-sonnet-4-6
	private RequestPartitionId resolveAutoVersionLookupPartition(
