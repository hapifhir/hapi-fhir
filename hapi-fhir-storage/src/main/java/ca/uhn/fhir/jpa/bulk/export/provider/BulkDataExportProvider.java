...
public class BulkDataExportProvider {
	...
	public void export(
				...
				@OperationParam(
									name = JpaConstants.PARAM_EXPORT_IDENTIFIER,
									min = 0,
									max = 1,
									typeName = "string")
	...
	private void startJob(
				ServletRequestDetails theRequestDetails, BulkDataExportOptions theOptions) {
		// permission check
		HookParams params =
					(new HookParams())
		...
		BulkExportParameters parameters =
					BulkExportUtils.createBulkExportJobParametersFromExportOptions(theOptions);
		parameters.setUseExistingJobsFirst(useCache);

		// Set the original request URL as part of the job information, as this is used in the
		// poll-status-endpoint, and is needed for the report.
		...
		myRequestPartitionHelperService.validateHasPartitionPermissions(
					theRequestDetails, "Binary", partitionId);
	...
	private boolean shouldUseCache(ServletRequestDetails theRequestDetails) {
		...
		return myStorageSettings.getEnableBulkExportJobReuse()
					&& !cacheControlDirective.isNoCache();
	}
	...
	/**
	* Throw ResourceNotFound if the target resources don't exist. Otherwise, we start a bulk-export
	* job which then fails, reporting a 500.
	*
	...
	*/
	...
	private void validateResourceTypesAllContainPatientSearchParams(Set<String> theResourceTypes) {
		if (theResourceTypes != null) {
				List<String> badResourceTypes =
						theResourceTypes.stream()
									.filter(
												resourceType ->
								...
								Msg.code(512)
										+ String.format(
													"Resource types [%s] are invalid for this type of export,"
														+ " as they do not contain search parameters that refer"
														+ " to patients.",
	...
	public void patientExport(
		...
		if (thePatient != null) {
				validateTargetsExists(
						theRequestDetails,
						"Patient",
						Lists.transform(thePatient, s -> new IdDt(s.getValue())));
		}

		BulkDataExportOptions bulkDataExportOptions =
					buildPatientBulkExportOptions(
								theOutputFormat,
								theType,
								theSince,
								theTypeFilter,
	...
	public void exportPollStatus(
		...
		if (theJobId == null) {
				...
				Parameters.ParametersParameterComponent parameter =
									...
									.orElseThrow(
												() ->
														new InvalidRequestException(
																	Msg.code(2227)
																				+ "$export-poll-status requires a job"
																				+ " ID, please provide the value of"
																				+ " target jobId."));
		...
		switch (info.getStatus()) {
					...
					if (theRequestDetails.getRequestType() == RequestTypeEnum.DELETE) {
						...
						if (isEmpty(report)) {
								...
								BulkExportJobResults results =
										JsonUtil.deserialize(report, BulkExportJobResults.class);
				...
				case ERROR:
					...
					OperationOutcomeUtil.addIssue(
								myFhirContext, oo, "error", info.getErrorMsg(), null, null);
					myFhirContext
								.newJsonParser()
								.setPrettyPrint(true)
				...
				case SUBMITTED:
					if (theRequestDetails.getRequestType() == RequestTypeEnum.DELETE) {
						...
						response.addHeader(
									...
									"Build in progress - Status set to "
												+ info.getStatus()
												+ " at "
												+ dateString);
	...
	private void handleDeleteRequest(
				...
				BulkExportJobStatusEnum theOrigStatus)
				throws IOException {
		IBaseOperationOutcome outcome = OperationOutcomeUtil.newInstance(myFhirContext);
		Batch2JobOperationResult resultMessage =
					myJobRunner.cancelInstance(theJobId.getValueAsString());
	...
	private BulkDataExportOptions buildPatientBulkExportOptions(
		...
		if (thePatientIds != null) {
				bulkDataExportOptions.setPatientIds(
	...
	private BulkDataExportOptions buildBulkDataExportOptions(
				...
				List<IPrimitiveType<String>> theTypePostFetchFilterUrl) {
		String outputFormat =
					theOutputFormat != null
								? theOutputFormat.getValueAsString()
	...
	public void writePollingLocationToResponseHeaders(
				ServletRequestDetails theRequestDetails, JobInfo theOutcome) {
		String serverBase = getServerBase(theRequestDetails);
		if (serverBase == null) {
