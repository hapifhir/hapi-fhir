...
public class DaoRegistry implements ApplicationContextAware, IDaoRegistry {
    ...
    public IFhirResourceDao getResourceDao(String theResourceName) {
        ...
        if (retVal == null) {
            List<String> supportedResourceTypes =
                    myResourceNameToResourceDao.keySet().stream()
                            .sorted()
                            .collect(Collectors.toList());
            throw new InvalidRequestException(
                    Msg.code(572)
                            + "Unable to process request, this server does not know how to handle"
                            + " resources of type "
                            + theResourceName
                            + " - Can handle: "
    ...
    public <R extends IBaseResource> IFhirResourceDao<R> getResourceDao(Class<R> theResourceType) {
        ...
        Validate.notNull(
                retVal,
                "No DAO exists for resource type %s - Have: %s",
                theResourceType,
    ...
    @Deprecated
    public <T extends IBaseResource> IFhirResourceDao<T> getResourceDaoIfExists(
            Class<T> theResourceType) {
        return getResourceDaoOrNull(theResourceType);
    }

    @Nullable
    public <T extends IBaseResource> IFhirResourceDao<T> getResourceDaoOrNull(
            Class<T> theResourceType) {
    ...
    private void init() {
        ...
        Map<String, IFhirResourceDao> resourceDaos =
                myAppCtx.getBeansOfType(IFhirResourceDao.class);
    ...
    private void initializeMaps(Collection<IFhirResourceDao> theResourceDaos) {
        ...
        for (IFhirResourceDao nextResourceDao : theResourceDaos) {
            ...
            RuntimeResourceDefinition nextResourceDef =
                    myContext.getResourceDefinition(resourceType);
            if (mySupportedResourceTypes == null
                    || mySupportedResourceTypes.contains(nextResourceDef.getName())) {
    ...
    public void register(IFhirResourceDao theResourceDao) {
        RuntimeResourceDefinition resourceDef =
                myContext.getResourceDefinition(theResourceDao.getResourceType());
    ...
    public IFhirResourceDao getDaoOrThrowException(Class<? extends IBaseResource> theClass) {
        ...
        if (retVal == null) {
            List<String> supportedResourceNames =
                    myResourceNameToResourceDao.keySet().stream()
            ...
            throw new InvalidRequestException(
                    Msg.code(573)
                            + "Unable to process request, this server does not know how to handle"
                            + " resources of type "
                            + myContext.getResourceType(theClass)
                            + " - Can handle: "
