...
public class DaoResourceLinkResolver<T extends IResourcePersistentId>
    ...
    private static final org.slf4j.Logger ourLog =
            org.slf4j.LoggerFactory.getLogger(DaoResourceLinkResolver.class);
    @Autowired private JpaStorageSettings myStorageSettings;
    @Autowired private FhirContext myContext;
    @Autowired private IIdHelperService<T> myIdHelperService;
    ...
    public IResourceLookup findTargetResource(
        ...
        RuntimeSearchParam searchParam =
                mySearchParamRegistry.getActiveSearchParam(
                        theSourceResourceName, thePathAndRef.getSearchParamName());

        T persistentId = null;
        if (theTransactionDetails != null) {
            T resolvedResourceId =
                    (T) theTransactionDetails.getResolvedResourceId(targetResourceId);
        ...
        try {
            if (persistentId == null) {
                resolvedResource =
                        myIdHelperService.resolveResourceIdentity(
                                theRequestPartitionId, resourceType, idPart);
            ...
            Optional<IBasePersistedResource> createdTableOpt =
            ...
            if (!createdTableOpt.isPresent()) {
                ...
                RuntimeResourceDefinition missingResourceDef =
                        myContext.getResourceDefinition(type);
        ...
        if (!resourceType.equals(resolvedResource.getResourceType())) {
            ...
            throw new UnprocessableEntityException(
                    Msg.code(1095)
                            + "Resource contains reference to unknown resource ID "
                            + targetResourceId.getValue());
        }
                ...
                .execute(
                        () -> {
                            ...
                            IFhirResourceDao<?> dao =
                                    myDaoRegistry.getResourceDao(targetId.getResourceType());
                            return dao.read(targetId, theRequest);
                        });
    }

    /**
     * @param theIdToAssignToPlaceholder If specified, the placeholder resource created will be
     *     given a specific ID
     */
    public <T extends IBaseResource>
        ...
        if (myStorageSettings.isAutoCreatePlaceholderReferenceTargets()) {
            ...
            if (myStorageSettings.isPopulateIdentifierInAutoCreatedPlaceholderReferenceTargets()) {
                tryToCopyIdentifierFromReferenceToTargetResource(
                        theReference, missingResourceDef, newResource);
            }
            ...
            theTransactionDetails.addResolvedResourceId(
                    persistentId.getAssociatedResourceId(), persistentId);
        }
    ...
    private <T extends IBaseResource> void tryToCopyIdentifierFromReferenceToTargetResource(
        ...
        CanonicalIdentifier referenceMatchUrlIdentifier =
                extractIdentifierFromUrl(theSourceReference.getReferenceElement().getValue());
        ...
        if (referenceIdentifier == null && referenceMatchUrlIdentifier != null) {
            ...
            addSubjectIdentifierToTargetResource(
                    theSourceReference, theTargetResourceDef, theTargetResource);
        } else if (referenceIdentifier != null && referenceMatchUrlIdentifier != null) {
    ...
    private <T extends IBaseResource> void addSubjectIdentifierToTargetResource(
            ...
            RuntimeResourceDefinition theTargetResourceDef,
            T theTargetResource) {
        BaseRuntimeChildDefinition targetIdentifier =
                theTargetResourceDef.getChildByName("identifier");
        if (targetIdentifier != null) {
            BaseRuntimeElementDefinition<?> identifierElement =
                    targetIdentifier.getChildByName("identifier");
            ...
            if (targetHasIdentifierElement) {
                ...
                BaseRuntimeElementCompositeDefinition<?> referenceElement =
                        (BaseRuntimeElementCompositeDefinition<?>)
                                myContext.getElementDefinition(theSourceReference.getClass());
                BaseRuntimeChildDefinition referenceIdentifierChild =
                        referenceElement.getChildByName("identifier");
                Optional<IBase> identifierOpt =
                        referenceIdentifierChild
                                .getAccessor()
                                .getFirstValueOrNull(theSourceReference);
                identifierOpt.ifPresent(
                        theIBase ->
                                targetIdentifier
                                        .getMutator()
                                        .addValue(theTargetResource, theIBase));
            }
        }
    }
    ...
    private <T extends IBaseResource> void addMatchUrlIdentifierToTargetResource(
            ...
            T theTargetResource,
            CanonicalIdentifier referenceMatchUrlIdentifier) {
        BaseRuntimeChildDefinition identifierDefinition =
                theTargetResourceDef.getChildByName("identifier");
        ...
        IBase systemIBase =
                TerserUtil.newElement(
                        myContext,
                        "uri",
                        referenceMatchUrlIdentifier.getSystemElement().getValueAsString());
        IBase valueIBase =
                TerserUtil.newElement(
                        myContext,
                        "string",
        ...
        BaseRuntimeElementDefinition<?> elementDefinition =
                myContext.getElementDefinition(identifierIBase.getClass());
    ...
    private CanonicalIdentifier extractIdentifierReference(IBaseReference theSourceReference) {
        Optional<IBase> identifier =
                myContext
                        .newFhirPath()
                        .evaluateFirst(theSourceReference, "identifier", IBase.class);
        if (!identifier.isPresent()) {
            ...
            Optional<IPrimitiveType> system =
                    myContext
                            .newFhirPath()
                            .evaluateFirst(identifier.get(), "system", IPrimitiveType.class);
            Optional<IPrimitiveType> value =
                    myContext
                            .newFhirPath()
                            .evaluateFirst(identifier.get(), "value", IPrimitiveType.class);

            system.ifPresent(
                    theIPrimitiveType ->
                            canonicalIdentifier.setSystem(theIPrimitiveType.getValueAsString()));
            value.ifPresent(
                    theIPrimitiveType ->
                            canonicalIdentifier.setValue(theIPrimitiveType.getValueAsString()));
     ...
     * @return Returns the first available identifier in the canonical form or null if URL contains
     *     no identifier param
     * @throws IllegalArgumentException IllegalArgumentException is thrown in case identifier
     *     parameter can not be split using <code>system|value</code> pattern.
     */
    protected CanonicalIdentifier extractIdentifierFromUrl(String theValue) {
        ...
        List<NameValuePair> params =
                URLEncodedUtils.parse(
                        theValue.substring(identifierIndex), StandardCharsets.UTF_8, '&', ';');
        ...
        if (split.length != 2) {
            throw new IllegalArgumentException(
                    Msg.code(1097)
                            + "Can't create a placeholder reference with identifier "
                            + theValue
