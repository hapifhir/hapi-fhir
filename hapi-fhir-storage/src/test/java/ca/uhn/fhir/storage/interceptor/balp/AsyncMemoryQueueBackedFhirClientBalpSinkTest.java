...
public class AsyncMemoryQueueBackedFhirClientBalpSinkTest {
	...
	private RestfulServerExtension myServer =
				new RestfulServerExtension(FhirVersionEnum.DSTU3)
	...
	public void testStressTest() {
		AsyncMemoryQueueBackedFhirClientBalpSink sink =
					new AsyncMemoryQueueBackedFhirClientBalpSink(
								myServer.getFhirContext(), myServer.getBaseUrl());
		try {
				for (int i = 0; i < 10; i++) {
					for (int j = 0; j < 100; j++) {
	...
	public void recordAuditEvent() {
		...
		try {
				...
				List<String> whats =
						myAuditEventProvider.getStoredResources().stream()
	...
	public class MySystemProvider {
		...
		public Bundle transaction(
					@TransactionParam Bundle theInput, RequestDetails theRequestDetails) {
