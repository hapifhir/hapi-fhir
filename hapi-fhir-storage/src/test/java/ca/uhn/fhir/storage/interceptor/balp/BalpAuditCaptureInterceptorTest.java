...
public class BalpAuditCaptureInterceptorTest implements ITestDataBuilder {
	...
	@RegisterExtension
	@Order(1)
	public final HashMapResourceProviderExtension<Patient> myPatientProvider =
				new HashMapResourceProviderExtension<>(ourServer, Patient.class);

	@RegisterExtension
	@Order(1)
	public final HashMapResourceProviderExtension<Observation> myObservationProvider =
				new HashMapResourceProviderExtension<>(ourServer, Observation.class);

	@RegisterExtension
	@Order(1)
	public final HashMapResourceProviderExtension<CodeSystem> myCodeSystemProvider =
				new HashMapResourceProviderExtension<>(ourServer, CodeSystem.class);

	@RegisterExtension
	@Order(1)
	public final HashMapResourceProviderExtension<ListResource> myListProvider =
				new HashMapResourceProviderExtension<>(ourServer, ListResource.class);

	@Mock private IBalpAuditEventSink myAuditEventSink;

	@Mock(strictness = Mock.Strictness.LENIENT)
	private IBalpAuditContextServices myContextServices;

	@Captor private ArgumentCaptor<AuditEvent> myAuditEventCaptor;
	...
	private static List<String> getQueries(AuditEvent theAuditEvent) {
		return theAuditEvent.getEntity().stream()
	...
	private static void assertAuditEventValidatesAgainstBalpProfile(AuditEvent auditEvent) {
		...
		List<SingleValidationMessage> issues =
					outcome.getMessages().stream()
								.filter(
										t ->
													t.getSeverity().ordinal()
																>= ResultSeverityEnum.WARNING.ordinal())
								.toList();
		if (!issues.isEmpty()) {
				fail(
						"Issues:\n * "
	...
	private static void assertHasSystemObjectEntities(
				AuditEvent theAuditEvent, String... theResourceIds) {
		List<String> systemObjects =
					theAuditEvent.getEntity().stream()
								.filter(t -> t.getType().getSystem().equals(CS_AUDIT_ENTITY_TYPE))
								.filter(
										t ->
													t.getType()
																.getCode()
																.equals(CS_AUDIT_ENTITY_TYPE_2_SYSTEM_OBJECT))
								.filter(t -> t.getRole().getSystem().equals(CS_OBJECT_ROLE))
								.filter(t -> t.getRole().getCode().equals(CS_OBJECT_ROLE_4_DOMAIN_RESOURCE))
								.map(t -> t.getWhat().getReference())
								.toList();
		assertThat(
					Arrays.asList(theResourceIds).toString(),
					systemObjects,
					containsInAnyOrder(theResourceIds));
	}

	private static void assertHasPatientEntities(
				AuditEvent theAuditEvent, String... theResourceIds) {
	...
	public void before() {
		ourServer
					...
					.unregisterInterceptorsIf(t -> t instanceof BalpAuditCaptureInterceptor);
		...
		when(myContextServices.getAgentClientWho(any()))
					.thenReturn(
								new Reference()
										.setIdentifier(
													new Identifier()
																.setSystem("http://clients")
																.setValue("123")));
		when(myContextServices.getAgentUserWho(any()))
					.thenReturn(
								new Reference()
										.setIdentifier(
													new Identifier()
																.setSystem("http://users")
																.setValue("abc")));
		when(myContextServices.massageResourceIdForStorage(any(), any(), any()))
					.thenCallRealMethod();
	...
	public void testCreateCodeSystem() {
		...
		cs.setUrl("http://foo");
		MethodOutcome outcome = myClient.create().resource(cs).execute();
		...
		ourLog.info(
					"Audit Event: {}",
					ourCtx.newJsonParser().setPrettyPrint(true).encodeResourceToString(auditEvent));
	...
	public void testCreateObservation() {
		...
		MethodOutcome outcome = myClient.create().resource(obs).execute();
	...
	public void testCreatePatient() {
		...
		Patient p =
					buildResource("Patient", withId("P1"), withFamily("Simpson"), withGiven("Homer"));
	...
	public void testDeleteCodeSystem() {
		...
		cs.setUrl("http://foo2");
		MethodOutcome outcome = myClient.delete().resource(cs).execute();
		IIdType csId = outcome.getId();
	...
	public void testDeleteObservation() {
		...
		MethodOutcome outcome = myClient.delete().resource(obs).execute();
		IIdType obsId = outcome.getId();
	...
	public void testDeletePatient() {
		...
		p.setActive(false);
		MethodOutcome outcome = myClient.delete().resource(p).execute();
		IIdType patientId = outcome.getId();
	...
	public void testReadPatient() {
		...
		Patient patient = myClient.read().resource(Patient.class).withId("P1").execute();
	...
	public void testReadResourceInPatientCompartment_WithOneSubject() {
		...
		Observation observation =
					myClient.read().resource(Observation.class).withId("O1").execute();
	...
	public void testReadResourceInPatientCompartment_WithTwoSubjects_VRead() {
		...
		ListResource outcome =
					myClient.read().resource(ListResource.class).withId(listId).execute();
	...
	public void testSearch_ResponseIncludesSinglePatientCompartment_LoadPageTwo() {
		...
		assertHasPatientEntities(auditEvent, "Patient/P1");
		assertQuery(
					auditEvent, ourServer.getBaseUrl() + "/Observation?_count=5&subject=Patient%2FP1");
	...
	public void testSearch_ResponseIncludesSinglePatientCompartment_UsePost() {
		...
		assertHasPatientEntities(auditEvent, "Patient/P1");
		assertQuery(
					auditEvent, ourServer.getBaseUrl() + "/Observation/_search?subject=Patient%2FP1");
	...
	public void testUpdateCodeSystem() {
		...
		cs.setUrl("http://foo2");
		MethodOutcome outcome = myClient.update().resource(cs).execute();
		IIdType csId = outcome.getId();
	...
	public void testUpdateObservation() {
		...
		obs.setStatus(Observation.ObservationStatus.FINAL);
		MethodOutcome outcome = myClient.update().resource(obs).execute();
		IIdType obsId = outcome.getId();
	...
	private void create10Observations(String... thePatientIds) {
		for (int i = 0; i < 10; i++) {
				createObservation(
						withId("O" + i), withSubject(thePatientIds[i % thePatientIds.length]));
		}
	}
	...
	private void assertHasProfile(AuditEvent theAuditEvent, BalpProfileEnum theProfile) {
		List<String> profiles =
					theAuditEvent.getMeta().getProfile().stream()
	...
	public IIdType doUpdateResource(IBaseResource theResource) {
		return getProvider(theResource)
					.update(theResource, null, new SystemRequestDetails())
					.getId();
	}

	@SuppressWarnings("unchecked")
	private <T extends IBaseResource> HashMapResourceProviderExtension<T> getProvider(
				T theResource) {
